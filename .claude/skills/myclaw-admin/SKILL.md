...
Runtime Claude settings are generated by MyClaw at
`~/myclaw/.claude/settings.json`. Do not install separate global Claude hooks
for MyClaw memory. Generated hook commands call the installed MyClaw CLI through
...
1. Use `myclaw config set <KEY> <VALUE>` or edit `~/myclaw/.env`.
2. Run `myclaw doctor`.
3. Restart with `myclaw restart` or `myclaw service restart`.
4. Confirm with `myclaw status`.
...
```bash
myclaw config set OPENAI_API_KEY <key>
myclaw memory embeddings openai
myclaw service restart
