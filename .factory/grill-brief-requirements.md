# Cold-read grill — gate: requirements — requirements for GRANTED-1 (docs/specs/agents-can-use-what-they-are-granted.md)
...
## The artifact under interrogation (requirements for GRANTED-1 (docs/specs/agents-can-use-what-they-are-granted.md))

---
slug: agents-can-use-what-they-are-granted
title: Agents can use what they are granted
status: confirmed
saved: 2026-09-10T11:15:49+00:00
---
...
Both halves apply here, through the mechanisms that already implement them rather
than by assertion. `document_str_replace` and `document_insert` join the shared
...
granted capability, whatever its binding kind, the catalog carries the stable
capability id and a complete invocation descriptor: the tool that reaches it and
the argument shape to send. A granted capability is never elided by overflow; if
the catalog cannot fit, non-granted and descriptive material is dropped before any
granted entry, and a granted entry is never reduced to a count. That descriptor is
...
document's canonical owner as decision 0114 defines it, never from a conversation
JID or workspace; the existing conversation-derived helper
...
Acting-person resolution follows decision 0118 and distinguishes two cases. A
document whose acting person is known to be absent is shared-group scope, and an
attended edit from that group's conversation is allowed. A lookup that fails to
resolve is not shared scope and fails closed. Uniqueness is counted over the whole
stored document, never over the returned slice.
...
1c. Overflow never hides a grant: with a granted set exceeding the catalog budget,
   every granted capability still renders with its descriptor and none is reduced
   to a summary count. Proven for local-CLI, MCP, tool-rule and adapter bindings.
...
15. `document_ref` resolution is authority-checked against the canonical owner
    (0114) and acting person (0118). A caller without write authority is refused,
