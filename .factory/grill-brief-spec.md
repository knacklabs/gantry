# Cold-read grill — gate: spec — spec local-dev-launch.md
...
## The artifact under interrogation (spec local-dev-launch.md)
...
Derive public host and port from `GANTRY_CONTROL_HOST` and
`GANTRY_CONTROL_PORT`, defaulting to loopback port 3939. Fresh authentication
configuration must match this origin; existing conflicts fail with remediation.
...
reset variants, print a fresh short-lived, single-use authorization link after
health readiness. Reuse `gantry ui authorize` against the resolved runtime home
and public origin. Keep the stable UI URL for browsers with valid sessions;
issuing a new link must not revoke those sessions. Do not persist plaintext
authorization URLs in runtime files. If link issuance fails, keep the healthy
development stack running and print an actionable retry command.

| Surface | Classification | Reason |
| --- | --- | --- |
| Runtime behavior | Changed | Issues an optional link after each healthy launch. |
| settings.yaml | Read-only/observable | Existing canonical origin remains authoritative. |
| Postgres/runtime projection | Changed | Existing authorization-code writer inserts a hashed token. |
...
| CLI | Changed | Local supervisor invokes existing UI authorization command. |
...
| Docs/prompts | Changed | Describe automatic links on every local launch. |
| Audit/events | Unchanged by design | Existing authorization flow owns its events. |
| Tests/verification | Changed | Check readiness ordering, resolved environment, and issuance failure. |

Verify fresh issuance on restart, single-use redemption, and existing-session
preservation without capturing raw authorization tokens in evidence. Search
README for obsolete explicit-only authorization guidance.

### Reset boundaries

Both commands run immediately and print the resolved home and redacted database
target. Stop only verified development children before resetting. Refuse unsafe
home paths, non-loopback databases, and databases not named `gantry`.
Recreate only `gantry` and `pgboss`, then run the complete current migration chain.
Full reset additionally removes known settings, onboarding, agents, data, store,
logs, artifacts, and runtime projection paths. Preserve `.env`, `postgres/`,
unknown files, unrelated processes, and user runtime data during verification.

### Deferred

Docker-only startup on a dedicated port is explicitly deferred to a follow-up;
this change retains reachable configured loopback database reuse. No
additional dependencies or SDK/API schema changes are authorized.
...
- `npm run reset` / `gantry local reset` and `npm run reset:db` / `gantry
  local reset-db` recreate only the `gantry` and `pgboss` schemas, preserve
  `.env`, `postgres/`, unknown files, and unrelated processes, and refuse
  unsafe home paths, non-loopback databases, and databases not named `gantry`.
- `npm run dev:stop` / `gantry local stop` stops source core, Vite, and the
  ...
  or stopping unrelated Docker containers; Ctrl-C leaves Postgres warm.
- Every successful start or reset prints a fresh short-lived, single-use
  browser authorization link after health readiness via `gantry ui
  authorize`, without revoking existing valid sessions and without
  persisting plaintext authorization URLs; a failed issuance keeps the
  healthy dev stack running and prints a retry command.
...
- Verification covers home/env precedence, bootstrap preservation, database
  ...
  cleanup, reset deletion boundaries, unsafe-target refusal, and restart,
  using disposable state only.
- Focused CLI tests, core and web typechecks, the web build, and the
  deterministic Forge verification entrypoint all pass; any gate refusal is
  reported honestly rather than bypassed.
- The bounded change is committed, the autoreview helper runs once across
  quality, performance, and security (rerun only after accepted fixes), real
  evidence is recorded through Forge scripts, and a PR is opened.
