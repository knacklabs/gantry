...
(`runtime/agent-spawn-prompt.ts:87`). The agent therefore guesses, and the
KnackLabs job burned two to four failed calls at the head of seven consecutive
runs before its first successful sheet read.
...
- `inheritedToolPolicy.semanticCapabilities` (`:182`) — resolved through
  `resolveTurnToolPolicyFromSnapshot(accessSnapshot, personId)`, so it is filtered
  to what the acting person is granted. **This is the one the catalog uses.**
- `semanticCapabilities` (`:194`) — `resolveTurnSemanticCapabilitiesFromSnapshot`,
  no person filter, everything active for the app. Feeding this would advertise
  capabilities the person was never granted as ready.
...
1. **Job path builds and passes the catalog.** Call the existing resolver with
   ...
   `:287`. `runtime/group-run-context.ts:156` gains a person-filtered entry point
   or an explicit ready-set parameter so the job path reuses it rather than
   duplicating the builder.
2. **`CatalogEntry` gains an `invocation` union**
   (`application/agents/agent-prompt-capability-catalog.ts:23`), a closed union
   over the four usable implementation kinds
   (`shared/semantic-capabilities.ts:25`; legacy `mcp_tool` is rejected at
   validation and never reaches the catalog): `local_cli` carries a lane-neutral
   dispatcher reference, the capability id and the reviewed argument patterns from
   the same helper the mismatch denial uses; `mcp_pattern` a neutral proxy
   reference and the connected server name; `tool_rule` the tool name, which is
   also how skill actions appear, since they are a tool rule distinguished by
   source; `adapter` the neutral dispatcher reference and capability id, its
   reference being opaque. `resolveReadyActions` (`:137`) populates it and stops
   dropping `stableRef` at render time.
3. **Lane mapping.** A small table renders the neutral reference into the
   Anthropic runtime's tool name at compile time. The DeepAgents lane has no
   entry, and a test asserts that, so its behaviour is unchanged and the gap
   cannot drift silently.
4. **Shedding and the two budgets.** `prompt-profile-service.ts:54` gains a named
   default budget and a ceiling derived as at least the compact name-and-id list.
   The renderer (`agent-prompt-capability-guidance.ts`) sheds in order:
   non-granted entries, then descriptions, then descriptors. Because compilation
   truncates at the section budget (`:532`) and again at the total prompt budget
   (`:756`), the capability section is exempted from the second cut below its
   compact representation.
5. **Hard overflow fails closed.** When even the compact list exceeds the total
   budget, the run fails before any provider or tool call with a redacted
   `capability_catalog_overflow` startup diagnostic naming granted and renderable
   counts.
6. **Deletion and wording.** The 97ded3746 block leaves `runner/mcp/context.ts`;
...
flowchart TD
  ...
  E --> F{compact name+id list fits total budget?}
  F -- no --> G[fail run before any provider call<br/>capability_catalog_overflow diagnostic]
  F -- yes --> H[shed: non-granted, then descriptions, then descriptors]
  H --> I[render entries: name, stable id,<br/>lane tool name, reviewed argument shape]
  I --> J[spawn input carries capabilityCatalog]
  J --> K[prompt compiled, section exempt from second cut]
  K --> L[agent's first tool action is the dispatcher call]
```

## Manual Verification

1. Trigger the KnackLabs job and read its rendered guidance in the run's prompt
   record: the granted sheets capability appears with its display name, stable id,
   the Anthropic tool name and its reviewed argument shape.
...
4. Temporarily grant the agent many capabilities so the compact list cannot fit,
   and confirm the run fails at startup with the overflow diagnostic rather than
   starting and silently omitting one.

## Verify

`npx vitest run -c vitest.unit.config.ts apps/core/test/unit/agents
