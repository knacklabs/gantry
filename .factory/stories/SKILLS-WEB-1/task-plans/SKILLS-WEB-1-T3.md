...
T3 is a web-only extension over the completed T1 facade and T2 inspection page. Its write scope is limited to `skills-queries.ts`, `skills-route.tsx`, one sibling `skills-admin-dialogs.tsx`, and `skills-route.test.ts`; no backend, contract, schema, public API, CLI, or dependency change is expected.

Use the native file input and the existing CSRF-aware `browserFetch`, Dialog, Button, Input, TanStack Query, navigation-summary, and agent query-key patterns. The same-name warning is always visible because the server remains authoritative for ZIP parsing. Viewer sessions render no mutation trigger.

The install flow must prove file gating, raw `application/zip` upload, exact warning and success actions, cache invalidation, failure preservation, and no automatic attachment. The attachment flow must lazily load confirmed server state, allow disabled agents with the exact approved label, submit the complete desired set, preserve/refetch confirmed state after failure, invalidate Skills/navigation/affected Agent queries, and announce next-run success.

Required UI proof includes accessible labels, keyboard operation, focus return, pending close prevention, live status messages, and constrained dialog dimensions. Declared actions remain read-only and Agent Access remains the only authorization path.
