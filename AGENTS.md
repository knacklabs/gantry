...
- Secret-backed browser input must resolve Credential Center secrets host-side from the signed app/agent context and selected capability bindings. Public browser payloads may name a secret, but must not supply capability ids or raw values as authority; the backend receives only the internal typed value.
