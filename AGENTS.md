...
- Generic agent action schemas must project the task's `finalSchema` into the `output` property; an unconstrained empty object causes schema-driven providers to return `{}` and makes valid final output impossible.
...
- Treat provider-native structured-output schemas as an optimization: preflight compatibility and complexity generically, retain the original schema for local validation, and fall back without adding product-specific normalization.
