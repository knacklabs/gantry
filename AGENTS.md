...
- Do not add Manipal-specific or other customer/product-specific business logic to Gantry. Gantry must stay provider-neutral and product-neutral; domain behavior belongs in the source platform or a product-specific agent package, with Gantry only carrying generic routing, delivery, and runtime abstractions.
...
- External platform notification cards use Adaptive Card v1.2 `Action.Submit`;
  Gantry must sign the card action payload, verify the clicked Teams
  conversation and tenant, and pass the trusted tenant/user identity to the
  source platform for final scoped authorization.
