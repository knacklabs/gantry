...
- Model selection must go through the provider-neutral catalog and friendly aliases. Public model vocabulary is `modelAlias`, `responseFamily` (`anthropic` or `openai`), diagnostic `modelRoute`, resolver-owned `executionProviderId`, and `credentialProfileRef`; OpenRouter is route metadata on Anthropic-family aliases, not a core response family. Do not accept raw provider model IDs at user/API/job/MCP boundaries unless they are registered aliases; job defaults inherit interactive defaults before falling back to system `opus`. Do not reintroduce legacy Claude model registries or runner-ID defaults in public config paths.
...
- Prefer semantic capability requests (`capability_search`, `propose_capability`, and `manage_capability`) for app/tool access. Capabilities must come from reviewed tool, skill, MCP server, adapter, or CLI manifests; do not add source-code hardcoded capability ids for specific products, providers, or CLIs. Fall back to raw scoped `RunCommand(...)` only for one-off exact commands or when no reviewed semantic capability exists.
...
- Egress policy is runtime-owned and provider-neutral. Model provider calls use the Gantry loopback model gateway, while general outbound tool traffic uses the Gantry loopback egress gateway; `permissions.egress.denylist` is an optional hostname-glob denylist, default egress is allow, and every CONNECT decision must be audited.
