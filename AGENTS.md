...
- MCP credential sourcing depends on transport. `stdio_template` servers are spawned by core, which injects their env from the reviewed capability secret store. External `http`/`sse` connectors (e.g. `mcp:boondi-crm`, `mcp:shopify-api`) are independent processes core does not spawn: they own their credentials via their own runtime `.env`, so core ignores their env-targeted `credential_refs` and resolves only the shared caller-identity `signing_ref` from `$GANTRY_HOME/.env`. A missing signing secret for a required-identity http connector is logged loudly at materialization, never silently swallowed.
