...
- Local desired-state configuration belongs in `settings.yaml` and is mediated by `SettingsDesiredStateService`. CLI commands and approved MyClaw admin tools write the file; only the main/admin agent may use `settings_desired_state` and `request_settings_update`, and agents must not edit the file or DB directly.
