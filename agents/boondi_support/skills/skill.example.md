...
| `disclosure` | No | Gantry-specific for installed/catalog artifact skills. Only the literal value `progressive` is recognized by `buildApprovedSkillContextBlock`: Gantry injects **only** `name` + `description` + a pointer in that installed-skill prompt block. Agent-folder SDK skills are materialized to the SDK skill directory and passed through `options.skills`; code does not inline their body into `systemPrompt.append`. Verify the live SDK payload and reply before relying on progressive behavior for any specific agent-folder skill. **Omit it** only when you intentionally accept full-body preload for the runtime path being used. |
