...
- Terminal progress converges: `apps/core/test/unit/runtime/group-progress-channel-sender.test.ts` — `keeps a stalled edit before terminal Done for the same card` and `restores a failed terminal desired payload after an older stall lands late`; Slack restart convergence is pinned by `lazily marks a prior-process Slack card stale and posts fresh work` and `posts fresh Slack work when the prior-process stale edit fails` in `apps/core/test/unit/channels/slack.test.ts`.
