...
<dict>
    ...
    <string>${projectRoot}/logs/myclaw.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/myclaw.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.myclaw');
  } catch {
...
/**
 * Kill any orphaned myclaw node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned myclaw processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/myclaw.service';
    ...
    unitPath = path.join(unitDir, 'myclaw.service');
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned myclaw processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable myclaw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start myclaw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active myclaw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-myclaw.sh');
  const pidFile = path.join(projectRoot, 'myclaw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-myclaw.sh — Start MyClaw without systemd',
    ...
    '    echo "Stopping existing MyClaw (PID $OLD_PID)..."',
    ...
    `  >> ${JSON.stringify(projectRoot + '/logs/myclaw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/myclaw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "MyClaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/myclaw.log"`,
  ];
