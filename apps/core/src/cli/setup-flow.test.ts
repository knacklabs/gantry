...
async function loadSetupFlowModule(options: SetupFlowTestOptions) {
  ...
  vi.doMock('./onboarding-config.js', () => ({
    persistOnboardingConfig,
  }));
  vi.doMock('./onboarding-state.js', () => ({
  ...
  const mod = await import('./setup-flow.js');
  return {
