...
type Settings = { authentication: { mode: 'local' | 'hosted'; canonicalOrigin: string } };

export function isBrowserOnboardingPath(pathname: string) {
  return pathname === ONBOARDING_STATUS_PATH || pathname === ONBOARDING_VERIFICATIONS_PATH;
}
...
async function createVerification(req: IncomingMessage, res: ServerResponse, settings: Settings): Promise<boolean> {
  const session = await requireBrowserMutationSession({ req, res, mode: settings.authentication.mode, originIsValid: isCanonicalBrowserOrigin(req, settings.authentication.canonicalOrigin) });
  ...
  if (!agentId || !conversationId || !/^GY-[A-Z0-9]{5}$/.test(challenge)) {
    sendError(res, 400, 'INVALID_REQUEST', 'A valid agent, conversation, and challenge are required.');
    return true;
  }
  const storage = getRuntimeStorage();
  const appId = session.appId as AppId;
  const agent = await storage.repositories.agents.getAgent(agentId as never);
  const conversation = await storage.repositories.conversations.getConversation(conversationId as never);
  if (!agent || !conversation || agent.appId !== appId || conversation.appId !== appId) {
    sendError(res, 404, 'NOT_FOUND', 'The selected setup resources were not found.');
    return true;
  }
  const now = new Date();
  const expiresAt = new Date(now.getTime() + 10 * 60_000).toISOString();
  const id = `onboarding-verification:${randomUUID()}`;
  await storage.service.db.insert(onboardingVerificationsPostgres).values({ id, appId, agentId, conversationId, challenge, status: 'pending', expiresAt, createdAt: now.toISOString(), updatedAt: now.toISOString() });
  sendJson(res, 201, { verification: { id, challenge, status: 'pending', expiresAt } });
  return true;
}
