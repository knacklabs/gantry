...
export async function runDeterministicManagedBrowserActions(input: {
  ...
  timeoutMs: number;
  runId: string;
  egressSettings: Parameters<typeof ensureEgressGateway>[0]['settings'];
}): Promise<string> {
  ...
  const profileName = resolveConversationBrowserProfile({
    agentId: input.agentFolder,
    workspaceKey: input.agentFolder,
  ...
  const workspacePath = resolveWorkspaceFolderPath(input.agentFolder);
...
function managedBrowserSandboxBridgeCommand(input: {
  ...
  return [
    'set -eu',
    // Sandbox Runtime mounts /tmp read-only. Browser automation creates temporary
    // artifacts while attaching over CDP, so keep them in the writable skill
    // workspace rather than inheriting an unavailable runtime temp directory.
    'gantry_browser_tmp="$PWD/.gantry-browser-tmp"',
    'mkdir -p "$gantry_browser_tmp"',
    'export TMPDIR="$gantry_browser_tmp"',
    // The sandbox makes /tmp read-only. Keep the bridge diagnostics beside the
    // Browser artifacts in the writable run workspace.
    `gantry_browser_bridge_log="$gantry_browser_tmp/gantry-browser-cdp-${input.bridgePort}.log"`,
