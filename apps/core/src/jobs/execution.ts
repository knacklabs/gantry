...
import {
  resolveDeterministicManagedBrowserActions,
  runDeterministicManagedBrowserActions,
} from './deterministic-source-sync.js';
...
async function runActiveJob(
  ...
  const preflightSnapshotOwner = {
    appId: preflightAppSession?.appId ?? runtimeAppId,
    agentId: jobToolPolicy.agentIdForJobWorkspaceKey(execution.group.folder),
  };
  const preflightAccessSnapshot = await loadAgentAccessSnapshot(
    deps,
    preflightSnapshotOwner,
  );
  const pausedForSetup = await pauseJobForSetupIfNeeded({
    ...
    accessSnapshot: preflightAccessSnapshot,
      ...
      if (!error) {
        ...
        try {
          ...
          const [accessSnapshot, credentialBroker] = await Promise.all([
            snapshotOwner.appId === preflightSnapshotOwner.appId &&
            snapshotOwner.agentId === preflightSnapshotOwner.agentId
              ? Promise.resolve(preflightAccessSnapshot)
          ...
          if (!error) {
            const deterministicManagedBrowserActions =
              resolveDeterministicManagedBrowserActions(
                currentJob,
                semanticCapabilities,
              );
            if (deterministicManagedBrowserActions) {
              const directResult = await runDeterministicManagedBrowserActions({
                job: currentJob,
                actions: deterministicManagedBrowserActions,
                deps,
                appId: executionAppId,
                agentId: executionAgentId,
                agentFolder: execution.group.folder,
                ...
                runId,
                egressSettings:
                  getRuntimeSettingsForConfig().permissions.egress,
              });
              result = directResult;
              appendResultSummary(directResult);
            } else {
