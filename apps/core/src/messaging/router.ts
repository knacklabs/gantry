  ...
  const lines = messages.map((m) => {
    ...
    const replySnippet =
      m.reply_to_message_content && m.reply_to_sender_name
        ...
        : '';
    const attachmentDescriptors = formatAttachmentLines(
      m.attachments,
      CURRENT_MESSAGE_LINE_LIMITS,
    );
    return `<message sender="${escapeXml(m.sender_name)}" time="${escapeXml(displayTime)}"${replyAttr}>${replySnippet}${attachmentDescriptors}${escapeXml(m.content)}</message>`;
  });
