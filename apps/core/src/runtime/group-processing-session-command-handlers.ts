...
export function createGroupProcessingSessionCommandHandlers(input: {
  ...
  return {
    sendMessage: (text: string, options?: { threadId?: string }) =>
      input.sendMessage(text, input.buildMessageOptions(options?.threadId)),
    ...
    ...createSessionCommandAgentRunners({
      ...
      existingRunLeaseWorkerInstanceId:
        input.processOptions.existingRunLeaseWorkerInstanceId,
      existingRunLeaseFencingVersion:
        input.processOptions.existingRunLeaseFencingVersion,
    }),
    ...
    setGroupModelOverride: async (
      value: Parameters<GroupProcessingDeps['setGroupModelOverride']>[1],
    ) => deps.setGroupModelOverride(input.commandOverrideRouteKey, value),
    getModelStatus: modelStatus.getStatus,
    getBrowserStatus: () =>
      getGroupBrowserStatus({ group, chatJid: input.chatJid }),
    ...
    setGroupThinkingOverride: (
      value: Parameters<GroupProcessingDeps['setGroupThinkingOverride']>[1],
