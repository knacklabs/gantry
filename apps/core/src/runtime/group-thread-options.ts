...
export function createThreadOptionBuilders(input: {
  activeThreadId?: string;
  getStreamGeneration: () => number;
  getProgressGeneration: () => number;
}): {
  resolveThreadId(threadId?: string): string | undefined;
  ...
  return {
    ...
    buildStreamingOptions: (args) => ({
      generation: input.getStreamGeneration(),
    ...
    buildProgressOptions: (
      ...
      ...(resolveThreadId(args.threadId)
        ? { threadId: resolveThreadId(args.threadId) }
        : {}),
      generation: input.getProgressGeneration(),
      ...(args.done !== undefined ? { done: args.done } : {}),
