...
const sdkState = vi.hoisted(() => ({
  mode: 'success' as
    ...
    | 'structured-completion-followup-failure'
...
vi.mock('@anthropic-ai/claude-agent-sdk', () => {
  const query = async function* ({
    ...
    if (
      ...
      sdkState.mode === 'structured-completion-followup-failure'
    ) {
      yield {
        type: 'result',
      ...
      if (sdkState.mode === 'structured-completion-followup-failure') {
        throw new Error('malformed continuation envelope');
      }
      yield {
        type: 'result',
        subtype: 'success',
...
describe('Claude Agent SDK boundary integration', () => {
  ...
  it('classifies an SDK failure during completion continuation', async () => {
    sdkState.mode = 'structured-completion-followup-failure';
    completionGateState.submit.mockResolvedValueOnce({
      ok: true,
      data: {
        decision: 'continue',
        progressToken: 'draft-prepared',
    ...
    await expect(
      ...
      message: 'malformed continuation envelope',
    });
  });
