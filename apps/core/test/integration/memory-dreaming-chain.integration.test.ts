...
vi.mock('@core/config/memory.js', async () => {
  const actual =
    await vi.importActual<typeof import('@core/config/memory.js')>(
...
maybeDescribe('memory evidence -> dreaming -> memory_search -> hydration chain', () => {
  ...
  for (const scope of SCOPES) {
    describe(`${scope.label} scope`, () => {
      ...
      it('finds the promoted memory through the real memory IPC path and enforces scope isolation', async () => {
        ...
        const crossResponse = await memorySearch(scope, otherScope.searchToken);
  ...
  it('routes a faked-LLM lifecycle proposal to durable review and applies it on approve', async () => {
    ...
    expect(review, 'dreaming must create a durable pending review').toBeTruthy();
