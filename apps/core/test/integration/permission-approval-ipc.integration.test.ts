...
describe('permission approval IPC boundary', () => {
  it('holds an unattended job request until the signed host decision arrives', async () => {
    ...
    expect(pendingRequest.raw).toMatchObject({
      authPurpose: 'unbounded-interaction',
      unattended: true,
    });
    expect(pendingRequest.raw.expiresAt).toBeUndefined();
