...
class FakeDrizzleDb {
  ...
  insertedConversation: Record<string, unknown> | null = null;
    ...
    return {
      values(value: Record<string, unknown>) {
        ...
        if (table === pgSchema.conversationsPostgres) {
          db.operations.push('insert:conversations');
          db.insertedConversation = value;
          return {
            onConflictDoNothing() {
              return Promise.resolve();
            },
...
describe('PostgresRuntimeEventRepository', () => {
  ...
  it('materializes canonical conversation thread rows before appending threaded events', async () => {
    ...
    expect(db.insertedConversation).toMatchObject({
      id: 'conversation:sl:C0B3M99H1B6',
      appId: 'default',
      providerAccountId: 'channel-providerAccount:default:slack',
      kind: 'channel',
      title: 'sl:C0B3M99H1B6',
    });
  ...
  it('materializes account-scoped conversation threads with the real provider jid', async () => {
    ...
    await repository.appendRuntimeEvent({
      appId: 'default' as never,
      conversationId:
        'conversation:channel-providerAccount:default:slack:sl:C0B3M99H1B6' as never,
      threadId:
        'thread:channel-providerAccount:default:slack:sl:C0B3M99H1B6:1784789975.807219' as never,
      eventType: RUNTIME_EVENT_TYPES.SESSION_MESSAGE_OUTBOUND,
      actor: 'agent',
      payload: { text: 'done' },
    });
    ...
    expect(db.insertedConversation).toMatchObject({
      id: 'conversation:channel-providerAccount:default:slack:sl:C0B3M99H1B6',
      providerAccountId: 'channel-providerAccount:default:slack',
      kind: 'channel',
      title: 'sl:C0B3M99H1B6',
    });
    expect(db.insertedConversationThread).toMatchObject({
      id: 'thread:channel-providerAccount:default:slack:sl:C0B3M99H1B6:1784789975.807219',
      conversationId:
        'conversation:channel-providerAccount:default:slack:sl:C0B3M99H1B6',
    });
    expect(db.insertedConversationThread?.externalRefJson).toBe(
      JSON.stringify({
        kind: 'conversation_thread',
