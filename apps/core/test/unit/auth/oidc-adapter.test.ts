...
describe('OIDC adapter', () => {
  it('rejects discovery with a mismatched issuer', async () => {
  ...
  it('uses authorization-code PKCE and rejects malformed token responses', async () => {
    const adapter = new OidcAdapter(
  ...
  it('validates signed issuer, audience, expiry, and nonce claims', async () => {
