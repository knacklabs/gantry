...
describe('Teams Adaptive Card payloads', () => {
  it('reads external card action payloads from Teams adaptiveCard/action invokes', () => {
    const data = {
      ...
      subjectId: 'tender-1',
      scopeId: 'workspace-1',
      sourceConversationId: '19:abc@thread.v2',
  ...
  it('explains external card action parse misses without logging payload values', () => {
    expect(
      ...
      reason:
        'missing_fields:integrationId,subjectId,scopeId,sourceConversationId,teamsTenantId,actionType,platformOperation,nonce,expiresAt,signature',
      valueKeys: ['action', 'trigger'],
