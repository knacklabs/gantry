...
it('paginates app-scoped directory results and rejects cross-app access', () => {
  ...
  expect(isBrowserAgentsPath('/ui/api/agents/agent%3Aone/instructions')).toBe(
    true,
  );
  ...
  expect(agentRouteSource).toContain('const AGENT_INSTRUCTIONS_PATH');
  expect(agentRouteSource).toContain("personaMatch ? 'soul' : 'agents'");
  ...
  expect(agentRouteSource).toContain('defaultAgentsPromptMarkdown(agent.name)');
