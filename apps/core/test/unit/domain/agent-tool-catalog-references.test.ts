...
describe('agent tool catalog references', () => {
  ...
  it('projects seeded built-in tools into a non-default app catalog', async () => {
    const template: ToolCatalogItem = {
      id: 'tool:AgentDelegation' as never,
      appId: 'default' as never,
      ...
      selectable: true,
      status: 'active',
      adapterRef: 'builtin:AgentDelegation',
      createdAt: '2026-05-01T00:00:00.000Z' as never,
      updatedAt: '2026-05-01T00:00:00.000Z' as never,
    };
    const saveTool = vi.fn(async () => undefined);
    const repository = {
      listTools: vi.fn(async () => []),
      getTool: vi.fn(async (toolId: string) =>
        toolId === template.id ? template : null,
      ),
      saveTool,
    } as unknown as ToolCatalogRepository;
    ...
    const item = await ensureAgentToolCatalogItem({
      ...
      reference: 'AgentDelegation',
      now: '2026-08-04T00:00:00.000Z',
    });

    expect(item).toMatchObject({
      id: persistentPermissionToolId(
