...
import { EmbeddingProvider } from '@core/memory/memory-embeddings.js';
import { consolidateMemoryItems } from '@core/memory/memory-consolidation.js';
...
function makeStore(): MemoryStore {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'myclaw-consolidation-'));
  tempRoots.push(root);
