...
vi.mock('@core/core/config.js', async (importOriginal) => {
...
describe('MemoryService', () => {
  ...
  it('ingests global knowledge docs from provided directory', async () => {
    ...
    const knowledgeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'myclaw-knowledge-'),
    );
  ...
  it('ingestGlobalKnowledge returns early when directory has no .md files', async () => {
    const service = makeService();
    const knowledgeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'myclaw-empty-knowledge-'),
    );
    tempKnowledgeDirs.push(knowledgeDir);
  ...
  it('ingestGlobalKnowledge scans subdirectories and skips non-.md files', async () => {
    const service = makeService();
    const knowledgeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'myclaw-subdir-knowledge-'),
    );
    tempKnowledgeDirs.push(knowledgeDir);
  ...
  it('getInstance returns a singleton and closeInstance cleans it up', async () => {
    ...
    const { MemoryService: FreshMemoryService } =
      await import('@core/memory/memory-service.js');
