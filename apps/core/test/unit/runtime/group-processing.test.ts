...
describe('createGroupProcessor', () => {
    ...
    it('keeps streamed output delivered without provider-session retry handling', async () => {
      ...
      mockSpawnAgent.mockImplementationOnce(
        async (
          ...
          await onOutput?.({ status: 'success', result: 'Visible reply' });
          return {
            status: 'error',
            result: null,
  ...
  describe('reply trace warm-bound first replies', () => {
    ...
    it('routes a warm-bound first reply through dispatchedAt instead of startup', async () => {
      ...
      const llm = traceRow?.timingsJson.sections.find(
        (section) => section.kind === 'llm',
      );
  ...
  describe('idle timeout', () => {
    ...
    it('keeps warm-bound idle close timer after streamed run returns', async () => {
      const group = makeGroup({ requiresTrigger: false });
      const messages = [makeMessage()];
      const { deps } = setupHappyPath({ group, messages });
