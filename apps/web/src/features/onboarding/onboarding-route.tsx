...
const STEPS = [
  [
    'Create Employee',
    'Give it a name, say what it handles, and pick the model it thinks with.',
  ],
...
function onboardingVerificationQuery(id: string) {
  return queryOptions({
    ...
    queryFn: async (): Promise<{
      verification: {
        status: 'pending' | 'inbound_received' | 'expired' | 'completed';
        expiresAt: string;
...
export function OnboardingRoute() {
  ...
  const [name, setName] = useState(
    () => localStorage.getItem('gantry.onboarding.name') ?? '',
  );
  const [title, setTitle] = useState(
    () => localStorage.getItem('gantry.onboarding.title') ?? '',
  );
  const [responsibilities, setResponsibilities] = useState(
    () => localStorage.getItem('gantry.onboarding.responsibilities') ?? '',
  );
  ...
  const selectedChannel =
    ...
    channelProviders.data?.providers[0];
  ...
  useEffect(() => {
    ...
    setChallenge(
      status.data.resume.challenge?.match(/GY-[A-Z0-9]{5}/)?.[0] ?? challenge,
    );
  ...
  function saveDraft(next: {
    ...
    if (next.name !== undefined)
      localStorage.setItem('gantry.onboarding.name', next.name);
    if (next.title !== undefined)
      localStorage.setItem('gantry.onboarding.title', next.title);
    if (next.responsibilities !== undefined)
  ...
  async function createEmployee() {
    ...
    try {
      const credential = await browserFetch(
        `/ui/api/model-providers/${encodeURIComponent(selectedProvider.providerId)}`,
        {
          method: selectedProvider.configured ? 'PATCH' : 'PUT',
          credentials: 'same-origin',
          headers: {
          ...
          body: JSON.stringify(
            selectedProvider.configured
              ? { payload }
              : { authMode: selectedMode.id, payload },
          ),
        },
      );
      if (!credential.ok) throw new Error('Credential could not be saved.');
      const verify = await browserFetch(
        `/ui/api/model-providers/${encodeURIComponent(selectedProvider.providerId)}/verify`,
        {
          method: 'POST',
          credentials: 'same-origin',
      ...
      if (!verify.ok)
        throw new Error('Gantry could not verify that model connection.');
      ...
      const response = await browserFetch('/ui/api/agents', {
        ...
        body: JSON.stringify({
          ...
          onboarding: true,
          customRole: {
            name: `${name.trim()} — ${title.trim() || 'Employee'}`,
            prompt: [
              `You are the organisation's ${title.trim() || 'AI employee'}.`,
              '',
              'Responsibilities:',
              ...lines.map((line) => `- ${line}`),
            ].join('\n'),
  ...
  async function assignWork() {
    ...
    setError(null);
    try {
      const verification = await verifyConversationApprovers(conversationId, [
        approver.trim(),
      ]);
      ...
      const body = (await response.json()) as {
        verification: { id: string; challenge: string };
      };
      setVerificationId(body.verification.id);
      setChallenge(body.verification.challenge);
      setStep(4);
    } catch (reason) {
      setError(
        reason instanceof Error
  ...
  if (!started)
    return (
      <Splash resume={status.data?.resume} onStart={() => setStarted(true)} />
    );
  return (
    <div className="onboarding-page">
      ...
      <div className="onboarding-grid">
        <aside className="onboarding-rail">
          ...
          <span className="onboarding-powered">KnackLabs</span>
        </aside>
        <main className="onboarding-main">
          <section className="onboarding-content">
            {step === 1 && (
              <EmployeeStep
                ...
                onResponsibilities={(value: string) => {
                  setResponsibilities(value);
                  saveDraft({ responsibilities: value });
                }}
                provider={selectedProvider}
                providers={providers.data ?? []}
                providerId={providerId || selectedProvider?.providerId || ''}
            ...
            {step === 2 && (
              <WorkspaceStep
                ...
                channels={channelProviders.data?.providers ?? []}
                channelId={channelId || selectedChannel?.id || ''}
                setChannelId={setChannelId}
                values={channelValues}
                setValues={setChannelValues}
                connected={workspaceConnected || Boolean(accountId)}
            ...
            {step === 3 && (
              <AssignStep
                ...
                members={conversationMembers.data?.memberIds ?? []}
                onSave={assignWork}
                busy={busy}
              />
            )}
          ...
          <footer className="onboarding-actions">
            ...
            {step === 4 ? (
              <button onClick={skip} className="onboarding-primary">
...
function Splash({
  ...
  onStart: () => void;
  resume?: OnboardingStatus['resume'];
}) {
  return (
...
function GantryMark({ large, hero }: { large?: boolean; hero?: boolean }) {
  return (
    <span
...
type EmployeeStepProps = {
  identity: boolean;
  setIdentity: Dispatch<SetStateAction<boolean>>;
  name: string;
  title: string;
  responsibilities: string;
  onName: (value: string) => void;
  onTitle: (value: string) => void;
  onResponsibilities: (value: string) => void;
...
function EmployeeStep(props: EmployeeStepProps) {
  const {
    identity,
    setIdentity,
    name,
    ...
    <>
      ...
      {identity ? (
        <article className="onboarding-card">
          ...
          <Field
            label={`What should it handle? · ${responsibilities.split('\n').filter(Boolean).length} responsibilities`}
          >
            <textarea
              rows={4}
              value={responsibilities}
              onChange={(e) => onResponsibilities(e.target.value)}
              placeholder="One line per responsibility."
            />
          </Field>
          <div className="onboarding-chips">
            {DUTIES.map((duty) => (
              <button
                ...
                onClick={() =>
                  onResponsibilities(
                    responsibilities.includes(duty)
                      ? responsibilities
                      : `${responsibilities}${responsibilities ? '\n' : ''}${duty}`,
                  )
                }
          ...
          <div className="onboarding-card-action">
            <button
        ...
        <article className="onboarding-card">
          <div className="onboarding-summary">
            {name} · {title || 'AI employee'}
            <button onClick={() => setIdentity(true)}>Edit details</button>
          </div>
          <Field label="Which model should it think with?">
            <select
              value={providerId}
          ...
          <Field label="Which one should it use?">
          ...
          {provider?.credentialModes?.[0]?.fields.map((field) => (
          ...
          <small className="onboarding-help">
            Kept in the credential broker — never shown again.
          </small>
          <div className="onboarding-card-action">
            <button
              className="onboarding-primary"
            ...
            >
              {busy
                ? 'Reaching provider…'
                : 'Save employee and test connection'}{' '}
...
function WorkspaceStep(props: WorkspaceStepProps) {
  ...
  return (
    <>
      <Heading
        title="Give your agent a place to work"
        body="Install Gantry where your team already talks."
      />
      <div className="onboarding-pills">
        {channels.map((item) => (
          <button
            key={item.id}
            className={channelId === item.id ? 'is-selected' : ''}
      ...
      <div className="onboarding-workspace-phases">
        <article className="onboarding-card onboarding-workspace-phase">
          <h3>
            <span>1</span> Connect {channel?.displayName ?? 'workspace'}
          </h3>
          <p>Credentials are stored write-only.</p>
          {channel?.status !== 'available' ? (
            ...
            channel?.credentialKeys.map((key) => (
          ...
          <button
            className="onboarding-primary"
            onClick={onConnect}
            disabled={busy || connected || channel?.status !== 'available'}
          >
            {connected ? (
              <>
        ...
        <article
          className={`onboarding-card onboarding-workspace-phase ${connected ? 'is-ready' : ''}`}
        >
          <h3>
            <span>2</span> Discover conversations
          </h3>
          <p>
            {connected
              ? 'Gantry has refreshed the conversations available to this workspace.'
              : 'Available after the workspace connection succeeds.'}
          </p>
        </article>
        <article
          className={`onboarding-card onboarding-workspace-phase ${connected ? 'is-ready' : ''}`}
        >
          <h3>
            <span>3</span> Confirm the connection
          </h3>
          <p>
            {connected
              ? 'Choose where this employee starts in the next step.'
...
type AssignStepProps = {
  ...
  approver: string;
  setApprover: Dispatch<SetStateAction<string>>;
  members: string[];
  onSave: () => void;
  busy: boolean;
};
    ...
    <>
      ...
      <article className="onboarding-card">
        ...
        <button
          className="onboarding-primary"
          onClick={onSave}
          disabled={busy || !conversationId || !approver}
        >
...
function HelloStep({
  ...
  const text = `@${handle} are you there? · ${code}`;
  const verification = useQuery(onboardingVerificationQuery(verificationId))
    .data?.verification;
  const expired = verification?.status === 'expired';
  const received = verification?.status === 'inbound_received';
  const complete = verification?.status === 'completed';
  return (
    <>
      <Heading
