...
type OnboardingStatus = { firstRun: boolean; resume: { id: string; name: string; accountId: string | null; verificationId: string | null; challenge: string | null; hasWorkspace: boolean; step: 2 | 3 | 4 } | null };
...
function onboardingVerificationQuery(id: string) {
  return queryOptions({
    ...
    queryFn: async (): Promise<{ verification: { status: 'pending' | 'inbound_received' | 'expired' | 'completed'; expiresAt: string; completedAt: string | null } }> => {
      ...
      return response.json() as Promise<{ verification: { status: 'pending' | 'inbound_received' | 'expired' | 'completed'; expiresAt: string; completedAt: string | null } }>;
    },
  });
}
...
export function OnboardingRoute() {
  ...
  useEffect(() => {
    ...
    setWorkspaceConnected(Boolean(status.data.resume.accountId));
  ...
  return <div className="onboarding-page">
    ...
    <div className="onboarding-grid"><aside className="onboarding-rail"><GantryMark large /> <div className="onboarding-steps">{STEPS.map(([label, blurb], index) => { const number = index + 1; const active = step === number; const complete = step > number; return <button key={label} disabled={number > step} onClick={() => setStep(number)} className="onboarding-step"><span className={`onboarding-node ${active ? 'is-active' : ''} ${complete ? 'is-complete' : ''}`}>{complete ? <Check size={13} /> : number}</span><span><b>{label}</b><small>{blurb}</small></span></button>; })}</div><span className="onboarding-powered">KnackLabs</span></aside>
      <main className="onboarding-main"><section className="onboarding-content">
        ...
        {step === 2 && <WorkspaceStep channel={selectedChannel} channels={channelProviders.data?.providers ?? []} channelId={channelId || selectedChannel?.id || ''} setChannelId={setChannelId} values={channelValues} setValues={setChannelValues} connected={workspaceConnected || Boolean(accountId)} onConnect={connectWorkspace} onContinue={() => setStep(3)} busy={busy} />}
        {step === 3 && <AssignStep conversations={conversations.data?.conversations ?? []} conversationId={conversationId} setConversationId={setConversationId} approver={approver} setApprover={setApprover} members={conversationMembers.data?.memberIds ?? []} onSave={assignWork} busy={busy} />}
...
type WorkspaceStepProps = { channel?: ChannelProvider; channels: ChannelProvider[]; channelId: string; setChannelId: Dispatch<SetStateAction<string>>; values: Record<string, string>; setValues: Dispatch<SetStateAction<Record<string, string>>>; connected: boolean; onConnect: () => void; onContinue: () => void; busy: boolean };
function WorkspaceStep(props: WorkspaceStepProps) { const { channel, channels, channelId, setChannelId, values, setValues, connected, onConnect, onContinue, busy } = props; return <><Heading title="Give your agent a place to work" body="Install Gantry where your team already talks." /><div className="onboarding-pills">{channels.map((item) => <button key={item.id} className={channelId === item.id ? 'is-selected' : ''} onClick={() => setChannelId(item.id)}>{item.displayName}</button>)}</div><div className="onboarding-workspace-phases"><article className="onboarding-card onboarding-workspace-phase"><h3><span>1</span> Connect {channel?.displayName ?? 'workspace'}</h3><p>Credentials are stored write-only.</p>{channel?.status !== 'available' ? <p className="onboarding-error">This provider is {channel?.status === 'setup_only' ? 'setup only' : 'not available'} in this runtime.</p> : channel?.credentialKeys.map((key) => <Field key={key} label={key}><input type="password" value={values[key] ?? ''} onChange={(e) => setValues({ ...values, [key]: e.target.value })} /></Field>)}<button className="onboarding-primary" onClick={onConnect} disabled={busy || connected || channel?.status !== 'available'}>{connected ? <><Check size={15} /> Connected</> : <>{busy ? 'Connecting…' : 'Connect workspace'} <ChevronRight size={15} /></>}</button></article><article className={`onboarding-card onboarding-workspace-phase ${connected ? 'is-ready' : ''}`}><h3><span>2</span> Discover conversations</h3><p>{connected ? 'Gantry has refreshed the conversations available to this workspace.' : 'Available after the workspace connection succeeds.'}</p></article><article className={`onboarding-card onboarding-workspace-phase ${connected ? 'is-ready' : ''}`}><h3><span>3</span> Confirm the connection</h3><p>{connected ? 'Choose where this employee starts in the next step.' : 'Wait for discovery to finish.'}</p><button className="onboarding-primary" onClick={onContinue} disabled={!connected}>Continue <ChevronRight size={15} /></button></article></div></>; }
type AssignStepProps = { conversations: ChannelConversation[]; conversationId: string; setConversationId: Dispatch<SetStateAction<string>>; approver: string; setApprover: Dispatch<SetStateAction<string>>; members: string[]; onSave: () => void; busy: boolean };
function AssignStep({ conversations, conversationId, setConversationId, approver, setApprover, members, onSave, busy }: AssignStepProps) { return <><Heading title="Put your agent to work" body="Give it one place to start and choose who approves riskier actions." /><article className="onboarding-card"><Field label="Give it one place to start"><select value={conversationId} onChange={(e) => { setConversationId(e.target.value); setApprover(''); }}><option value="">Select a discovered conversation</option>{conversations.map((item) => <option key={item.id} value={item.id}>{item.title ?? item.id}</option>)}</select></Field><Field label="Who approves its riskier actions?"><select value={approver} onChange={(e) => setApprover(e.target.value)} disabled={!conversationId || members.length === 0}><option value="">{conversationId && members.length === 0 ? 'No verified members found' : 'Select a verified member'}</option>{members.map((memberId) => <option key={memberId} value={memberId}>{memberId}</option>)}</select></Field><small className="onboarding-help">Only members verified in this conversation can approve riskier actions.</small><button className="onboarding-primary" onClick={onSave} disabled={busy || !conversationId || !approver}>{busy ? 'Saving…' : 'Continue'} <ChevronRight size={15} /></button></article></>; }
function HelloStep({ handle, code, verificationId }: { handle: string; code: string; verificationId: string }) { const text = `@${handle} are you there? · ${code}`; const verification = useQuery(onboardingVerificationQuery(verificationId)).data?.verification; const expired = verification?.status === 'expired'; const received = verification?.status === 'inbound_received'; const complete = verification?.status === 'completed'; return <><Heading title={complete ? `${handle} is ready` : `Ping ${handle}`} body={complete ? 'Your agent received the message and delivered its reply.' : 'Open the selected workspace and mention it. Gantry waits here until the message lands.'} /><article className="onboarding-card onboarding-hello"><div className="onboarding-ping"><code>{text}</code><button onClick={() => navigator.clipboard?.writeText(text)}><Copy size={13} /> Copy</button></div><GantryMark hero /><span className="onboarding-sweep"><i /></span><p>{complete ? 'Connected and replying.' : expired ? 'This test code expired. Return to Assign Work to issue another.' : received ? 'Message received. Waiting for the agent’s reply…' : 'Waiting for your message and the agent’s reply…'}</p><small className="onboarding-help">{complete ? 'Verification is complete.' : 'Success is only shown after the actual inbound message and correlated reply.'}</small></article></>; }
