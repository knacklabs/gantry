...
import { hashBrowserProfileFileModel, normalizeBrowserProfileFileModel, } from '../../../shared/browser-profile-hash.js';
/**
 ...
 */
export function normalizeBrowserProfileFiles(files) {
    return normalizeBrowserProfileFileModel(files.map((file) => ({
        ...file,
        path: normalizeBrowserProfilePath(file.path),
    })));
}
export function hashBrowserProfileFiles(files) {
    return hashBrowserProfileFileModel(normalizeBrowserProfileFiles(files));
}
export function resolveBrowserProfileSymlinkTarget(root, linkPath, linkTarget) {
    if (!linkTarget ||
...
export function normalizeBrowserProfilePath(value) {
...
export function normalizeBrowserProfileStorageRef(value) {
    const normalized = value.replace(/\\/g, '/');
    const parts = normalized.split('/');
    if (!normalized ||
        normalized.includes('\0') ||
...
export function browserProfileStorageRefFor(profileName, contentHash) {
...
export function resolveBrowserProfileAssetPath(root, assetPath) {
