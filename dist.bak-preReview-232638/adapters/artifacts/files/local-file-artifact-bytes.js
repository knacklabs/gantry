...
export function hashFileArtifactBytes(content) {
...
export class LocalFileArtifactBytes {
    ...
    async getBytes(storageRef, expected) {
        let content;
        try {
        ...
        if (actualHash !== expected.hash) {
            throw new Error(`File artifact hash mismatch: expected ${expected.hash}, got ${actualHash}`);
        }
        if (content.byteLength !== expected.sizeBytes) {
            throw new Error(`File artifact size mismatch: expected ${expected.sizeBytes}, got ${content.byteLength}`);
        }
        return content;
    }
    async removeBytes(storageRef) {
...
function isNodeErrnoException(value) {
