...
export class LocalSkillArtifactStore {
    artifactRoot;
    ...
    async getSkillArtifact(storageRef) {
        ...
        if (stat.isSymbolicLink() || !stat.isDirectory()) {
            throw new Error(`Skill artifact storage ref is not a directory: ${storageRef}`);
        }
        return normalizeSkillBundle({ assets: readAssetsRecursive(target) });
    }
}
export function normalizeSkillBundle(bundle) {
...
export function hashSkillBundle(bundle) {
...
function resolveStoragePath(root, storageRef) {
...
function readAssetsRecursive(root) {
    const rootPath = path.resolve(root);
    const assets = [];
