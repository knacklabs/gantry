...
import { ArtifactIntegrityError, } from '../../../domain/ports/toolchain-artifact-store.js';
import { hashToolchainFiles, normalizeToolchainFiles, normalizeToolchainStorageRef, resolveToolchainAssetPath, resolveToolchainSymlinkTarget, toolchainStorageRefFor, } from './toolchain-artifact-bundle.js';
/**
...
export class LocalToolchainArtifactStore {
    ...
    async putToolchainArtifact(input) {
        ...
        for (const file of files) {
            ...
            if ((file.kind ?? 'file') === 'symlink') {
                await fs.symlink(resolveToolchainSymlinkTarget(target, file.path, file.linkTarget ?? ''), filePath);
                continue;
            }
    ...
    async materializeToolchainArtifact(input) {
        ...
        const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'gantry-toolchain-'));
        try {
            let sizeBytes = 0;
            for (const file of files) {
                ...
                if ((file.kind ?? 'file') === 'symlink') {
                    await fs.symlink(resolveToolchainSymlinkTarget(tempDir, file.path, file.linkTarget ?? ''), filePath);
                    continue;
                }
                const content = Buffer.from(file.content);
                await writeToolchainFile(filePath, content, file.mode);
            ...
            if (actualContentHash !== input.expectedContentHash) {
                const quarantinePath = await quarantine(tempDir, input.quarantineDir, input.storageRef);
...
async function quarantine(sourceDir, quarantineRoot, storageRef) {
...
async function writeToolchainFile(filePath, content, mode) {
    const fileMode = mode ?? 0o600;
    await fs.writeFile(filePath, content, { mode: fileMode });
    await fs.chmod(filePath, fileMode);
}
async function readToolchainFiles(root) {
    const rootPath = path.resolve(root);
    const files = [];
    async function recordSymlink(relative, entryPath) {
    ...
    async function visitWithSymlinks(directory) {
