 ...
 */
export function normalizeToolchainFiles(files) {
    return files
        .map((file) => ({
...
export function hashToolchainFiles(files) {
...
export function resolveToolchainSymlinkTarget(root, linkPath, linkTarget) {
    if (!linkTarget ||
...
export function normalizeToolchainPath(value) {
...
export function normalizeToolchainStorageRef(value) {
    const normalized = value.replace(/\\/g, '/');
    const parts = normalized.split('/');
    if (!normalized ||
        normalized.includes('\0') ||
...
export function toolchainStorageRefFor(manifestHash) {
...
export function resolveToolchainAssetPath(root, assetPath) {
