...
export function ensureBrowserArtifactRoot(dir) {
    fs.mkdirSync(dir, { recursive: true });
    return fs.realpathSync.native(dir);
}
export function writeBrowserArtifactFileSync(filename, content, encoding, options = {}) {
...
export function normalizeBrowserFilePayload(toolName, payload, options) {
    const next = { ...payload };
    const rawPathsPresent = arrayValue(next.paths).length > 0;
    const sourcePresent = next.source !== undefined;
    if (next.filename !== undefined) {
        next.filename = resolveBrowserOutputPath(next.filename, options.fileAccessRoot);
    }
    if ((toolName === 'file_upload' || toolName === 'file_attach') &&
        next.source !== undefined) {
        if (next.files !== undefined || rawPathsPresent) {
            throw new Error(`${toolName} accepts either source, files, or paths, not multiple file sources.`);
        }
    ...
    if ((toolName === 'file_upload' || toolName === 'file_attach') &&
        ...
        next.paths = materializeBrowserUploadFiles(next.files, options.fileAccessRoot);
        delete next.files;
    }
    ...
    if (next.paths !== undefined) {
        ...
        next.paths =
            toolName === 'file_attach' || sourcePresent
                ? next.paths.map((item) => resolveBrowserAttachPath(item, options.fileAccessRoot))
                : next.paths.map((item) => resolveBrowserInputFilePath(item, options.fileAccessRoot));
    }
    return next;
}
function normalizeBrowserAttachSource(value) {
    ...
    if (type === 'bytes') {
        ...
        const normalizedContent = encoding === 'base64' ? content.replace(/\s/g, '') : content;
        if (Buffer.byteLength(normalizedContent, encoding) >
            MAX_ATTACH_BYTES_SOURCE_BYTES) {
            throw new Error(`Browser file_attach bytes sources are limited to ${MAX_ATTACH_BYTES_SOURCE_BYTES} decoded bytes each.`);
        }
        return { files: [file] };
    }
    if (type === 'path') {
        ...
        const rawPaths = paths.length > 0 ? paths : pathValue !== undefined ? [pathValue] : [];
        if (rawPaths.length === 0) {
            throw new Error('file_attach path source requires path or paths.');
        }
        if (rawPaths.length > MAX_INLINE_UPLOAD_FILES) {
            throw new Error(`Browser file_attach path sources are limited to ${MAX_INLINE_UPLOAD_FILES} files.`);
        }
        return { paths: rawPaths.map((item) => stringValue(item) || '') };
    }
    if (type === 'artifact') {
        throw new Error('file_attach artifact sources must be resolved by runtime.');
    }
    throw new Error('file_attach source type must be bytes, path, or artifact.');
}
function materializeBrowserUploadFiles(value, fileAccessRoot) {
    if (!Array.isArray(value)) {
        throw new Error('Browser upload/drop files must be an array.');
    }
    if (value.length > MAX_INLINE_UPLOAD_FILES) {
        throw new Error(`Browser inline uploads are limited to ${MAX_INLINE_UPLOAD_FILES} files.`);
    }
    const requestDir = `inline-${randomUUID()}`;
    const files = value.map((item, index) => parseBrowserUploadFile(item, index, requestDir, fileAccessRoot));
    const totalBytes = files.reduce((sum, file) => sum + file.sizeBytes, 0);
    if (totalBytes > MAX_INLINE_UPLOAD_TOTAL_BYTES) {
        throw new Error(`Browser inline uploads are limited to ${MAX_INLINE_UPLOAD_TOTAL_BYTES} decoded bytes per request.`);
    }
...
function parseBrowserUploadFile(value, index, requestDir, fileAccessRoot) {
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
    ...
    if (sizeBytes > MAX_INLINE_UPLOAD_FILE_BYTES) {
        throw new Error(`Browser inline upload files are limited to ${MAX_INLINE_UPLOAD_FILE_BYTES} decoded bytes each.`);
    }
    return {
        outputPath,
...
function uploadFileName(value, index) {
    ...
    if (raw !== path.basename(raw) ||
        ...
        raw === '.' ||
        raw === '..') {
        throw new Error('Browser inline upload file names must be plain filenames.');
    }
    return raw;
}
...
function resolveBrowserPath(value, fileAccessRoot) {
    const raw = stringValue(value);
    if (!raw)
        throw new Error('Browser file action requires a path.');
    ...
    if (relative === '' ||
        ...
        throw new Error('Browser file actions are limited to the run browser artifact root.');
    }
    const segments = relative.split(path.sep);
    if (segments.some(isSensitivePathSegment)) {
        throw new Error('Browser file actions cannot access hidden or sensitive paths.');
    }
    return candidate;
}
function resolveBrowserInputFilePath(value, fileAccessRoot) {
...
function resolveBrowserAttachPath(value, fileAccessRoot) {
    const raw = stringValue(value);
    if (!raw)
        throw new Error('file_attach path source requires a path.');
    ...
    if (!roots.some((root) => isInsideRoot(realCandidate, root.realPath))) {
        throw new Error(`file_attach path source is outside allowed roots: ${roots
            .map((root) => root.label)
            .join(', ')}.`);
    }
    return realCandidate;
}
function allowedAttachRoots(fileAccessRoot) {
    const root = ensureBrowserArtifactRoot(fileAccessRoot);
...
function resolveBrowserOutputPath(value, fileAccessRoot) {
    const candidate = resolveBrowserPath(value, fileAccessRoot);
    const root = ensureBrowserArtifactRoot(fileAccessRoot);
