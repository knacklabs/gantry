...
import { resolveTargetLocator, snapshotPage, takeScreenshot, } from './browser-direct-page-actions.js';
import { formFields, normalizeBrowserDirectPayload, } from './browser-direct-payload.js';
import { allPages, closeBrowserDirectConnections, closeCachedConnection, firstContext, getBrowserConnection, observePage, pageState, safeTitle, scheduleConnectionIdleClose, } from './browser-direct-session.js';
import { isInternalChromeTarget, normalizeBrowserToolResult, textResult, writeOptionalTextOutput, } from './browser-result-hygiene.js';
import { clearBrowserTabIndexMappings, projectBrowserTabsResult, translateBrowserTabsInput, } from './browser-tabs.js';
import { browserClickModifiers, stringRecord, } from './browser-direct-values.js';
import { actionOperationTimeout, browserActionTimeoutMs, BROWSER_ACTION_TIMEOUT_MS, remainingBrowserActionTimeoutMs, withTimeout, } from './browser-direct-timeout.js';
...
const selectedBackendIndexBySession = new Map();
export async function callBrowserTool(input) {
    ...
    try {
        const args = normalizeBrowserDirectPayload(input.toolName, input.arguments, {
            fileAccessRoot: outputDir,
        });
        const translatedArgs = translateBrowserTabsInput(input.toolName, args, sessionKey);
...
async function dispatchBrowserTool(input) {
    return await withTimeout(dispatchBrowserToolInner(input), remainingBrowserActionTimeoutMs(input.deadline), `Browser backend timed out while running ${input.toolName}.`);
}
    ...
    switch (input.toolName) {
        ...
        case 'navigate':
            return await runWithActivePage(input, async (page) => {
                ...
                return textResult(`Navigated to ${page.url()}${response ? ` (${response.status()})` : ''}.`);
            });
        case 'back':
            return await runWithActivePage(input, async (page) => {
                await page.goBack({
        ...
        case 'snapshot':
            return await runWithActivePage(input, async (page) => writeOptionalTextOutput(await snapshotPage(page, input.args), input.args));
        case 'screenshot':
            return await runWithActivePage(input, async (page) => takeScreenshot(page, input.args, input.outputDir));
        case 'console_messages':
            return await runWithActivePage(input, async (page) => writeOptionalTextOutput(JSON.stringify(pageState(page).console, null, 2), input.args));
        case 'network_requests':
            return await runWithActivePage(input, async (page) => writeOptionalTextOutput(JSON.stringify(pageState(page).network, null, 2), input.args));
        case 'click':
            return await runWithTarget(input, async (locator) => {
                const button = stringValue(input.args.button);
        ...
        case 'drag':
            return await runWithActivePage(input, async (page) => {
                const start = await resolveTargetLocator(page, requiredString(input.args.startTarget, 'startTarget'));
                const end = await resolveTargetLocator(page, requiredString(input.args.endTarget, 'endTarget'));
        ...
        case 'drop':
            return await runWithTarget(input, async (locator) => {
                ...
                await locator.evaluate((element, payload) => {
                    ...
                    element.dispatchEvent(new globalThis.DragEvent('drop', {
                        bubbles: true,
                        cancelable: true,
        ...
        case 'evaluate':
            return await runWithActivePage(input, async (page) => {
                ...
                const value = target
                    ? await (await resolveTargetLocator(page, target)).evaluate((element, fnSource) => {
                ...
                return writeOptionalTextOutput(JSON.stringify(value, null, 2), input.args);
            });
        case 'file_upload':
        case 'file_attach':
            return await runWithActivePage(input, async (page) => {
                ...
                const target = stringValue(input.args.target);
                if (target) {
                    await (await resolveTargetLocator(page, target)).setInputFiles(paths, {
                        timeout: actionOperationTimeout(input.deadline),
                    });
                }
                else {
...
async function runWithTarget(input, fn) {
    return await runWithActivePage(input, async (page) => fn(await resolveTargetLocator(page, requiredString(input.args.target, 'target')), page));
}
async function runWithActivePage(input, fn) {
...
async function runBrowserTabs(input) {
    ...
    switch (action) {
        ...
        case 'close': {
            ...
            return await tabsResult(await allPages(input.connection.browser), input.sessionKey);
        }
        case 'new': {
            const context = firstContext(input.connection.browser);
            const page = await context.newPage();
            observePage(page);
            selectedBackendIndexBySession.set(input.sessionKey, (await allPages(input.connection.browser)).indexOf(page));
...
async function tabsResult(pages, sessionKey) {
        ...
        content: [
            {
                ...
                text: tabs
                    ...
                    .map((tab) => `- ${tab.index}: ${tab.current ? '(current) ' : ''}${tab.title || '(untitled)'} ${tab.url}`)
                    .join('\n'),
            },
        ],
        structuredContent: { tabs },
    }, sessionKey, 'tabs', { action: 'list' });
}
async function activePage(browser, sessionKey) {
...
export async function closeBrowserToolBackends(profileName) {
...
function backendKey(profileName, cdpEndpoint) {
    return `${profileName || 'default'}\0${cdpEndpoint}`;
}
function formatBackendError(toolName, err) {
...
function shouldReturnSnapshotAfterNavigateBackTimeout(toolName, err) {
    return toolName === 'back' && /timed? out|timeout/i.test(errorMessage(err));
}
function isStaleBrowserError(err) {
...
function requiredPositiveInteger(value, name, opts = {}) {
