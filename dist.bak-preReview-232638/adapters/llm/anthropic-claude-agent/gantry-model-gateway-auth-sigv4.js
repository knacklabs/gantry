...
export function signAwsSigV4Request(input) {
    ...
    const { canonicalHeaders, signedHeaders } = canonicalizeHeaders(input.headers);
    ...
    const signingKey = awsSigV4SigningKey(input.credentials.secretAccessKey, dateStamp, input.region, input.service);
...
function awsSigV4SigningKey(secretAccessKey, dateStamp, region, service) {
...
function decodeRawQueryPart(value) {
    try {
