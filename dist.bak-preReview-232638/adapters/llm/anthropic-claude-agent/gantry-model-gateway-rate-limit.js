...
export class GatewayRateLimiter {
    ...
    admit(appId, providerId, limit, weight = 1, nowMs = Date.now()) {
...
export async function applyRateCap(input) {
    ...
    if (limit === undefined)
        return false;
    if (input.limiter.admit(input.appId, input.providerId, limit, input.weight ?? 1))
        return false;
