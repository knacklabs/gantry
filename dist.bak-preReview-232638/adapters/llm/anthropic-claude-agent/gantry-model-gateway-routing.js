...
import { resolveModelCredentialMode, } from '../../../shared/model-provider-registry.js';
...
import { getVertexAdcBearerToken, getVertexServiceAccountBearerToken, } from './gantry-model-gateway-auth-vertex.js';
...
export function resolveGatewayUpstream(provider, authMode, payload) {
    try {
        return (provider.gateway.upstreamResolver?.({ authMode, payload }) ?? {
            origin: provider.gateway.upstreamOrigin,
            pathPrefix: provider.gateway.upstreamPathPrefix,
        });
    }
    catch (error) {
        throw new GatewayBadRequestError(error instanceof Error
            ? error.message
            : 'Model gateway upstream could not be resolved.');
    }
}
export function assertProviderPathAllowed(provider, upstreamPathname, method = 'POST', upstreamPathPrefix = provider.gateway.upstreamPathPrefix) {
    const providerPath = stripUpstreamPathPrefix(upstreamPathname, upstreamPathPrefix);
    const allowed = method === 'GET'
        ? isAllowedGatewayReadPath(provider, providerPath)
        : method === 'POST' &&
            allowedGatewayPostPaths(provider).has(providerPath);
    if (!allowed) {
        throw new GatewayBadRequestError(`Model gateway path is not allowed for ${provider.id}.`);
    }
}
export function isGatewayMethodAllowed(method, gatewayPathname) {
    if (method === 'POST')
        return true;
    if (method !== 'GET')
        return false;
    return (/^\/openai\/v1\/(?:batches|files)(?:\/|$)/.test(gatewayPathname) ||
        /^\/anthropic\/v1\/messages\/batches(?:\/|$)/.test(gatewayPathname));
}
export function isProviderBatchPath(provider, providerPath) {
...
export function isProviderBatchSubmissionPath(provider, providerPath, method) {
    if (method !== 'POST')
        return false;
...
export function isProviderBatchResultPath(provider, providerPath) {
    if (provider.id === 'openai') {
...
export function openAiBatchIdFromPath(providerPath) {
    return exactDecodedPathId(providerPath, /^\/v1\/batches\/([^/]+)$/);
}
export function openAiFileContentIdFromPath(providerPath) {
...
export async function injectProviderAuth(input) {
    ...
    if (auth.strategy !== 'bearer' &&
        ...
        throw new Error(`Model gateway auth strategy ${auth.strategy} is not implemented for ${provider.id} ${authMode}.`);
    }
    ...
    if (auth.strategy === 'aws_sigv4') {
        signAwsSigV4Request({
            ...
            credentials: {
                ...
                secretAccessKey: requirePayloadField(payload, provider.id, 'secretAccessKey'),
    ...
    if (auth.strategy === 'vertex_service_account') {
        if (!auth.field) {
            throw new Error(`Model gateway auth strategy ${auth.strategy} for ${provider.id} ${authMode} is missing a credential field.`);
        }
    ...
    if (auth.strategy === 'vertex_service_account_ref') {
        ...
        const serviceAccountJson = await resolveModelCredentialSecretRef(requirePayloadField(payload, provider.id, auth.field));
        const token = await getVertexServiceAccountBearerToken({
            serviceAccountJson,
            expectedProjectId: requirePayloadField(payload, provider.id, 'projectId'),
        });
    ...
    if (auth.strategy === 'aws_bedrock_api_key_ref') {
        headers.authorization = `Bearer ${await resolveModelCredentialSecretRef(value)}`;
        return;
    }
    headers[auth.headerName ?? auth.field] = value;
}
function allowedGatewayPostPaths(provider) {
    if (provider.id === 'openai') {
        return new Set([
            '/v1/embeddings',
            '/v1/chat/completions',
    ...
    if (provider.id === 'anthropic')
        paths.add('/v1/messages/batches');
    return paths;
}
function isAllowedGatewayReadPath(provider, providerPath) {
    if (provider.id === 'openai') {
        return (/^\/v1\/batches(?:\/[^/]+)?$/.test(providerPath) ||
            /^\/v1\/files\/[^/]+\/content$/.test(providerPath));
    }
    if (provider.id === 'anthropic') {
        return /^\/v1\/messages\/batches(?:\/[^/]+(?:\/results)?)?$/.test(providerPath);
    }
    return false;
}
function stripUpstreamPathPrefix(pathname, prefix) {
...
function exactDecodedPathId(providerPath, pattern) {
...
function requirePayloadField(payload, providerId, field) {
    const value = payload[field];
    if (!value) {
        throw new Error(`Model credential payload for ${providerId} is missing ${field}.`);
    }
    return value;
}
