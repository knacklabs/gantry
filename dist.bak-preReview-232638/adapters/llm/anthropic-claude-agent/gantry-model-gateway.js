...
import { projectGatewayTokenEnv, projectedModelCredentialEnvKeys, } from './gantry-model-gateway-sdk-projection.js';
import { applyRateCap, GatewayRateLimiter, } from './gantry-model-gateway-rate-limit.js';
import { getModelProviderByGatewayPath, getModelProviderDefinition, getDefaultModelRouteProvider, normalizeModelProviderId, resolveModelCredentialMode, } from '../../../shared/model-provider-registry.js';
import { logger } from '../../../infrastructure/logging/logger.js';
import { beginGatewayObservation, failGatewayObservation, finishGatewayNonStreaming, resolveGatewayTap, } from './gantry-model-gateway-observability.js';
import { assertProviderPathAllowed as assertPath, injectProviderAuth, isGatewayMethodAllowed, isProviderBatchResultPath, resolveGatewayUpstream, } from './gantry-model-gateway-routing.js';
import { assertGatewayBatchCredential, batchRequestCountFor, gatewayRateWeight, gatewayTokenAllowsPath, gatewayTokenAllowsRequestBody, gatewayTokenScope, isRevocableGatewayTokenScope, recordGatewayBatchFileAssociations, runtimeEventRunIdFor, } from './gantry-model-gateway-token.js';
import { DEFAULT_LOOPBACK_HOST, GatewayBadRequestError, GatewayRequestBodyTooLargeError, assertRawGatewayPathIsConfined, buildConfinedUpstreamUrl, constantTimeEquals, hostForUrl, normalizeGatewayBindHost, pipeUpstreamBody, readBearerToken, readGatewayResponsePayload, readRequestBody, usageFromGatewayPayload, sanitizeProxyHeaders, sendGatewayJson, shouldForwardGatewayResponseHeader, } from './gantry-model-gateway-http.js';
...
export class GantryModelGatewayBroker {
    credentials;
    credentialService;
    server;
    listenPromise;
    ...
    constructor(credentials, options = {}) {
        ...
        this.bindHost = normalizeGatewayBindHost(options.bindHost ?? DEFAULT_LOOPBACK_HOST);
    ...
    async getInjection(input) {
        const provider = gatewayProviderFor(input.binding.modelCredentialProviderId ??
        ...
        if (!credential) {
            throw new Error(`Model credential for ${providerId} is not configured. Run \`gantry credentials model set ${providerId}\`.`);
        }
        ...
        this.tokens.set(token, {
            ...
            purpose,
            modelBatchRequestCount: batchRequestCountFor(purpose, input.binding.modelBatchRequestCount),
        ...
        await this.publishGatewayTokenAudit(this.tokens.get(token), 'token_issued');
        ...
        return {
            env,
            credentialProviders: {
                [provider.gateway.sdkProjection.credentialProviderEnvKey]: provider.gateway.sdkProjection.credentialProvider,
            },
            applied: true,
    ...
    async healthCheck(input) {
        ...
        if (!input?.binding.appId) {
            return {
                status: 'fail',
                message: 'Gantry Model Gateway requires an app-scoped credential binding.',
    ...
    async revokeInjection(input) {
        ...
        if (!isRevocableGatewayTokenScope(tokenScope)) {
            throw new Error('Gantry Model Gateway token revocation requires runId or apiKeyId.');
        }
                ...
                void this.handleRequest(req, res).catch((error) => {
                    if (!res.headersSent) {
                        ...
                        res.end(JSON.stringify({
                            error: error instanceof GatewayRequestBodyTooLargeError
    ...
    async handleRequest(req, res) {
        ...
        const parsedUrl = new URL(req.url || '/', `http://${hostForUrl(this.bindHost)}`);
        ...
        if (credential.fingerprint !== tokenRecord.credentialFingerprint ||
        ...
        const upstream = resolveGatewayUpstream(provider, credential.authMode, credential.payload);
        const upstreamUrl = buildConfinedUpstreamUrl(provider, pathParts, parsedUrl.search, upstream);
        ...
        const rateLimited = await applyRateCap({
            ...
            audit: () => this.publishGatewayUseAudit(tokenRecord, {
                outcome: 'rate_limited',
                method: req.method ?? 'GET',
                status: 429,
            }),
            reject: (limit) => sendGatewayJson(res, 429, {
        ...
        if (!gatewayTokenAllowsRequestBody(tokenRecord, provider, providerPath, method, body)) {
            sendGatewayJson(res, 403, {
                error: 'Forbidden model gateway token scope',
            });
            return;
        ...
        const timeout = setTimeout(() => upstreamAbort.abort(new Error('Model gateway upstream request timed out.')), this.upstreamTimeoutMs);
        const onClientAbort = () => {
            if (!res.writableEnded) {
        ...
        finishGatewayNonStreaming(observation, status, response, parsedResponse?.payload, usage);
        const auditInput = {
        ...
        response.headers.forEach((value, key) => {
            if (!shouldForwardGatewayResponseHeader(key))
                return;
    ...
    async publishGatewayUseAudit(tokenRecord, input) {
        if (!this.audit)
            return;
        try {
            await this.audit({
    ...
    async publishGatewayTokenAudit(tokenRecord, outcome) {
        if (!this.audit)
            return;
        try {
            await this.audit({
    ...
    startTokenSweep() {
        if (this.tokenSweepTimer || this.tokenSweepIntervalMs <= 0)
            return;
        this.tokenSweepTimer = setInterval(() => this.sweepExpiredTokens(), this.tokenSweepIntervalMs);
...
export async function extractGatewayResponseUsage(response, requestBody) {
    return usageFromGatewayPayload(await readGatewayResponsePayload(response, requestBody));
}
function gatewayProviderFor(providerId) {
    const normalized = normalizeModelProviderId(providerId);
    const provider = getModelProviderDefinition(normalized);
    if (provider?.executable && provider.gateway)
        return provider;
    throw new Error(`Unsupported model gateway provider: ${providerId}`);
}
function defaultGatewayProviderId() {
