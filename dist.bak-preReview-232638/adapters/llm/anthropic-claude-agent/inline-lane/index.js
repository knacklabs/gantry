...
import { SYSTEM_PROMPT_DYNAMIC_BOUNDARY, createSdkMcpServer, query, tool as createSdkTool, } from '@anthropic-ai/claude-agent-sdk';
...
import { sdkResultFailureMessage, topLevelAssistantText, } from '../runner/sdk-message-output.js';
...
export const runClaudeInlineAgentLoopLane = async (input) => {
    ...
    const configuredControls = resolveConfiguredAgentControlOptions(input.configuredThinking, input.effort);
    ...
    const steeringGate = new SteeringDeliveryGate((text) => prompt.pushContent(text));
    ...
    try {
        ...
        const sdkQuery = query({
            ...
            options: {
                ...
                env: isolatedSdkEnv(input.modelCredentialEnv, path.join(input.runtimeDataDir, 'inline-claude', input.group.folder)),
                settings: {
                ...
                allowedTools: toolsDisabled
                    ? []
                    : input.coreTools.tools.map(({ name }) => `mcp__${CORE_MCP_SERVER_NAME}__${name}`),
                ...
                canUseTool: async (toolName, toolInput, options) => {
                    ...
                    const isCoreTool = input.coreTools.tools.some(({ name }) => toolName === `mcp__${CORE_MCP_SERVER_NAME}__${name}`);
                    ...
                    const authorization = await input.coreTools.authorizeThirdPartyMcpTool(toolName, toolInput, { signal: options.signal });
                ...
                mcpServers: toolsDisabled
                    ...
                    : {
                        ...
                        ...Object.fromEntries((remoteMcp?.servers ?? []).map((server) => [
                            server.name,
        ...
        try {
            for await (const message of sdkQuery) {
                ...
                if (responseSchema &&
                    ...
                    lastTerminal = structuredOutputError(sdkResultFailureMessage(message) ??
                        'Claude SDK could not produce output matching response_schema.', newSessionId);
                    await input.emitOutput(lastTerminal);
                    break;
                }
                ...
                if (responseSchema && structuredResult === undefined) {
                    lastTerminal = structuredOutputError('Claude SDK returned success without validated structured output.', newSessionId);
                    await input.emitOutput(lastTerminal);
                    break;
                }
                ...
                lastTerminal = {
                    ...
                    ...(usage
                        ? {
                            usage,
                            usageEventId: usageEventIdForMessage(message, newSessionId ?? input.input.sessionId, resultCount, queryRunId),
                        }
                        : {}),
        ...
        if (input.signal.aborted)
            return abortedOutput(newSessionId);
        if (!sawMessage && !closeRequested) {
            throw new Error('Anthropic SDK query completed without messages or results');
        }
...
function createCoreSdkMcpServer(input, toolActivity) {
    return createSdkMcpServer({
        ...
        tools: input.coreTools.tools.map((definition) => {
            ...
            return createSdkTool(definition.name, definition.description, shape, async (args) => toolActivity.run(definition.name, async () => (await input.coreTools.execute(definition.name, args, {
                signal: input.signal,
            }))));
        }),
        alwaysLoad: true,
...
function isolatedSdkEnv(modelCredentialEnv, configDir) {
    const env = {
        ...Object.fromEntries(SDK_HOST_ENV_KEYS.map((key) => [key, process.env[key]])),
        ...modelCredentialEnv,
...
function remoteMcpAuditHooks(input, toolActivity) {
    const startedAt = new Map();
    const pre = async (hookInput) => {
    ...
    const success = async (hookInput) => {
        ...
        const outcome = objectRecord(result)?.isError === true ? 'failure' : 'success';
        const activity = {
            serverName: tool.serverName,
            toolName: tool.toolName,
            toolInput: hookInput.tool_input,
            outcome,
            latencyMs: hookLatencyMs(hookInput.duration_ms, startedAt.get(hookInput.tool_use_id)),
            result,
        };
        await input.coreTools.recordThirdPartyMcpToolActivity(activity);
        startedAt.delete(hookInput.tool_use_id);
        await toolActivity.finish(hookInput.tool_use_id, hookInput.tool_name, outcome);
        return { continue: true };
    };
    const failure = async (hookInput) => {
        ...
        startedAt.delete(hookInput.tool_use_id);
        await toolActivity.finish(hookInput.tool_use_id, hookInput.tool_name, 'failure');
        return { continue: true };
    };
    return {
...
function remoteMcpTool(input, fullToolName) {
...
function hookLatencyMs(durationMs, startedAt) {
    return durationMs ?? (startedAt === undefined ? 0 : Date.now() - startedAt);
}
function textDelta(record) {
...
function structuredOutputError(error, newSessionId) {
    return {
        status: 'error',
        result: null,
        error,
...
function abortedOutput(newSessionId) {
    return {
        status: 'error',
        result: null,
