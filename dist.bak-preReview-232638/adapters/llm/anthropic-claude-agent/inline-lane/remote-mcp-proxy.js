...
export async function createPinnedClaudeMcpProxies(input) {
    ...
    try {
        for (const capability of input.servers) {
            if (capability.config.type !== 'http' &&
    ...
    return {
        ...
        close: () => Promise.all(proxies.map(({ server }) => closeServer(server))).then(() => undefined),
    };
}
function createProxyServer(input) {
    const sseEndpointState = {};
    return http.createServer((request, response) => {
        void forwardRequest(request, response, input, sseEndpointState).catch((error) => {
...
async function forwardRequest(request, response, input, sseEndpointState) {
    if (request.headers[PROXY_AUTH_HEADER] !== input.proxyToken) {
        throw new Error('Inline MCP proxy authentication failed.');
    }
    const target = proxyTarget(request.url, input.target, input.targetType, sseEndpointState);
    ...
    const isSseHandshake = input.targetType === 'sse' &&
        request.method === 'GET' &&
    ...
    await pipeline(isSseHandshake
        ? bodyStream.pipe(createSseEndpointCapture(input.target, sseEndpointState, new URL(request.url ?? '/', `http://127.0.0.1:${request.socket.localPort}`)))
        : bodyStream, response);
}
export function proxyTarget(requestUrl, configuredTarget, targetType, sseEndpointState = {}) {
    ...
    if (target.origin !== configuredTarget.origin ||
        !pathWithinMcpEndpoint(target, configuredTarget, targetType, sseEndpointState)) {
        throw new Error('Inline MCP proxy request escaped its configured endpoint.');
    }
    return target;
}
function pathWithinMcpEndpoint(candidate, configured, targetType, sseEndpointState) {
    if (targetType === 'http') {
        const prefix = configured.pathname.endsWith('/')
            ? configured.pathname
            : `${configured.pathname}/`;
        return (candidate.pathname === configured.pathname ||
            candidate.pathname.startsWith(prefix));
    }
    return (sameMcpEndpoint(candidate, configured) ||
        (sseEndpointState.advertisedTarget !== undefined &&
            sameMcpEndpoint(candidate, sseEndpointState.advertisedTarget)));
}
function sameMcpEndpoint(left, right) {
    return (left.origin === right.origin &&
        left.pathname === right.pathname &&
        left.search === right.search);
}
export function createSseEndpointCapture(configuredTarget, state, proxyTarget) {
        ...
        transform(chunk, _encoding, callback) {
            ...
            try {
                ...
                while (boundary) {
                    ...
                    const separator = pending.subarray(boundary.index, boundary.index + boundary.length);
                    pending = pending.subarray(boundary.index + boundary.length);
                    const rewritten = rewriteSseEndpointEvent(eventBlock, configuredTarget, proxyTarget);
...
function rewriteSseEndpointEvent(eventBlock, configuredTarget, proxyTarget) {
    ...
    if (eventType !== 'endpoint' || data.length === 0)
        return undefined;
    try {
        ...
        const rewritten = new URL(`${advertised.pathname}${advertised.search}${advertised.hash}`, proxyTarget.origin);
        let replaced = false;
        ...
        return {
            ...
            block: Buffer.from(rewrittenLines.join(text.includes('\r\n') ? '\r\n' : '\n')),
        };
    }
    catch {
...
function forwardedRequestHeaders(incoming, configured) {
...
function forwardedResponseHeaders(incoming) {
    const headers = {};
