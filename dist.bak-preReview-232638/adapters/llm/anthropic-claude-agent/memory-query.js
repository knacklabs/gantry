...
import { getCredentialBrokerRuntimeConfig, getRuntimeSettingsForConfig, } from '../../../config/index.js';
...
import { abortReason, runWithMemoryOperationTimeout, } from '../../../shared/memory-dreaming-timeout.js';
...
export function getClaudeAuthAvailability() {
...
export function hasClaudeAuthConfigured() {
...
function readResultText(message) {
    if (!message || typeof message !== 'object')
        return '';
    const row = message;
    if (row.type !== 'result')
...
function buildCacheableSystemPrompt(systemPrompt) {
...
async function resolveGantryMemoryInjection(appId, modelRouteId, runId) {
    const brokerConfig = getCredentialBrokerRuntimeConfig();
    ...
    if (memoryCredentialBrokerConfigKey !== configKey) {
        void memoryCredentialBrokerPromise
            ...
            .catch((error) => {
            logger.warn({ err: error }, 'Failed to close replaced memory credential broker');
        });
...
function requireGantryBroker(broker) {
    if (!broker) {
        throw new Error('Gantry Model Gateway is enabled but no model gateway broker was provided.');
    }
    return broker;
}
async function runWithGantryGateway(opts) {
    ...
    const runId = `memory-query:${randomUUID()}`;
    const gateway = await resolveGantryMemoryInjection(opts.appId, modelEntry?.modelRoute.id ?? 'anthropic', runId);
    try {
        ...
        if (!modelEntry) {
            ...
            throw new Error(`Model "${opts.modelProfile?.runnerModel ?? opts.model}" is not in the model catalog; cannot verify Gantry Model Gateway credentials for memory queries.`);
        }
...
function scrubAmbientAgentCredentials(brokerEnv) {
    const env = {
...
export async function runClaudeQuery(opts) {
    if (!hasClaudeAuthConfigured()) {
        throw new Error('Claude auth is not configured (configure brokered model access)');
    }
    return runWithMemoryOperationTimeout((signal) => runWithGantryGateway({ ...opts, signal }), {
