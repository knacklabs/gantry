...
export function denyNonPromptableAutonomousRecovery(input) {
    if (isPromptableAutonomousRecovery(input.recoveryAction))
        return undefined;
    const message = `Permission denied: ${input.recoveryMessage}`;
    log(`Autonomous run denied tool ${input.toolName}: ${message}`);
    emitJobToolActivity(input.agentInput, input.getNewSessionId, 'permission_denied', input.toolName, {
        ok: false,
        terminal: false,
...
function isPromptableAutonomousRecovery(recoveryAction) {
    if (!recoveryAction)
        return true;
