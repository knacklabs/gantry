...
export function readLocalCliCredentialDirectories() {
    return readPathListEnv(LOCAL_CLI_CREDENTIAL_DIRS_ENV);
}
function readPathListEnv(name) {
...
export function normalizeFilesystemSandboxPaths(values) {
    return [...new Set(values.flatMap(resolvePathForSandbox))].sort();
}
function resolvePathForSandbox(value) {
    ...
    try {
        ...
        log(`Failed to resolve protected filesystem path "${absolute}": ${err instanceof Error ? err.message : String(err)}`);
    }
    return [absolute];
}
function expandCredentialPathTemplate(value) {
    let missing = false;
