 ...
 */
import { drainIpcInput, prepareInteractiveIpcInputDir, shouldClose, } from './ipc-input.js';
import { readStdin } from './input.js';
import { log } from './logging.js';
import { resolveConfiguredModel, resolveThinkingOptions, } from './model-config.js';
import { writeOutput } from './output.js';
import { runQuery } from './query-loop.js';
import { buildEffectiveToolNetworkEnv, buildSdkEnv, resolveMcpServerPath, } from './runtime-env.js';
import { parseSessionSlashCommand, runSessionSlashCommand, } from './session-slash.js';
...
async function main() {
    ...
    agentInput.toolNetworkEnv = buildEffectiveToolNetworkEnv(agentInput.toolNetworkEnv);
    ...
    const configuredThinking = resolveThinkingOptions(agentInput.thinking, agentInput.configuredThinking, agentInput.effort);
    if (configuredModel.model) {
        log(`Configured model: ${configuredModel.model} (source: ${configuredModel.source})`);
    }
    else {
...
function buildInitialPrompt(agentInput) {
...
function autonomousToolContract(allowedTools) {
...
function toolAccessRequirementContract(toolAccessRequirements) {
...
function browserAccessRequirementContract(toolAccessRequirements) {
    const hasBrowserRequirement = (toolAccessRequirements ?? []).some((rule) => rule.trim() === 'Browser');
    if (!hasBrowserRequirement)
        return '';
    return [
        'Browser requirement:',
...
async function runScheduledQuery(opts) {
    ...
    try {
        const queryResult = await runQuery(opts.prompt, opts.mcpServerPath, opts.agentInput, opts.sdkEnv, opts.configuredModel, opts.configuredThinking, opts.configuredEffort, { enableIpcFollowups: false, persistSdkSession: false });
        ...
        writeOutput({
            ...
            newSessionId: diagnosticSessionId,
            error: errorMessage,
            runtimeEvents: sdkSandboxBlockedRuntimeEvents(opts.agentInput, errorMessage),
        });
        process.exit(1);
    }
}
async function runInteractiveQueryLoop(opts) {
    let diagnosticSessionId;
    try {
        log(`Starting live streaming query with ${opts.agentInput.sessionId ? 'resumed SDK session' : 'new persistent SDK session'}...`);
        const queryResult = await runQuery(opts.prompt, opts.mcpServerPath, opts.agentInput, opts.sdkEnv, opts.configuredModel, opts.configuredThinking, opts.configuredEffort, { enableIpcFollowups: true, persistSdkSession: true });
        if (queryResult.newSessionId) {
            diagnosticSessionId = queryResult.newSessionId;
        }
