...
import { decideClaudeSdkToolSearch, toolSearchStartupRuntimeEvent, } from './tool-search-decision.js';
...
function localCliCredentialDirectoriesFromRuntimeAccess(agentInput) {
    const dirs = (agentInput.runtimeAccess ?? []).flatMap((access) => access.sourceType === 'local_cli' ? access.credentialDirs : []);
    return normalizeFilesystemSandboxPaths(dirs);
}
export async function runQuery(prompt, mcpServerPath, agentInput, sdkEnv, configuredModel, queryThinking, queryEffort, options = {}) {
    ...
    const declarativePreToolUse = toolSuccessLedger
        ? async (hookInput) => {
            ...
            if (!denial)
                return { continue: true };
            emitJobToolActivity(agentInput, () => newSessionId, 'deny', hookInput.tool_name, {
                ok: false,
    ...
    const processRuntimeSignalsDuringQuery = () => {
        if (!ipcPolling)
            return false;
        ...
        if (enableIpcFollowups) {
            ...
            for (const text of messages) {
                ...
                if (delivery === 'buffered') {
                    log(`Buffering IPC message until query turn boundary (${text.length} chars)`);
                }
            }
        }
        return true;
    ...
    log(`SDK query prepared in ${sdkQueryPreparedMs}ms ` +
    ...
    log(`SDK ToolSearch ${toolSearchDecision.enableToolSearch} ` +
    ...
    const postToolUseHook = async (hookInput, toolUseID, hookOptions) => {
        if (hookInput.hook_event_name === 'PostToolUse' && toolSuccessLedger) {
            recordSuccessfulToolUse(hookInput, toolSuccessLedger);
        }
        return permissionApprovalContext.postToolUse(hookInput, toolUseID, hookOptions);
    };
    const sdkQuery = query({
        ...
        options: {
            ...
            permissionMode: capabilities.permissionMode === 'deny'
                ? 'dontAsk'
            ...
            hooks: {
                PreToolUse: [
                    {
                        hooks: [
                            createSafetyPreToolUseHook(memoryBlock, agentInput.toolNetworkEnv ?? {}),
    ...
    try {
        ...
        for await (const message of sdkQuery) {
            ...
            const errorDetail = message.error_status
                ? ` error_status=${String(message.error_status)} error=${String(message.error ?? '')}`
                : '';
            ...
            if (message.type === 'assistant') {
                ...
                if (assistantText && !sawPartialTextSinceLastResult) {
                    ...
                    const visibleText = shouldPrefixVisibleBoundary(visibleTextSinceLastResult, assistantText)
                        ? `\n\n${assistantText}`
                        : assistantText;
            ...
            if (message.type === 'system' && message.subtype === 'init') {
                ...
                log(`Session initialized after ${providerSessionMs}ms: provider resume handle received`);
                writeOutput({
                    status: 'success',
                    result: null,
                    newSessionId,
            ...
            const taskEvent = message.type === 'system'
                ? taskRuntimeEvent(agentInput, message)
                : null;
            if (taskEvent) {
            ...
            if (message.type === 'stream_event') {
                ...
                if (event?.type === 'content_block_delta') {
                    ...
                    if (delta?.type === 'text_delta' && typeof delta.text === 'string') {
                        ...
                        const visibleText = pendingStructuredToPartialBoundary &&
                            shouldPrefixVisibleBoundary(visibleTextSinceLastResult, delta.text)
                            ? `\n\n${delta.text}`
                            : delta.text;
            ...
            if (message.type === 'result') {
                ...
                const emittedVisibleText = sawPartialTextSinceLastResult || sawStructuredTextSinceLastResult;
                const canUseResultFallback = !emittedVisibleText && !sawAssistantContentSinceLastResult;
                ...
                log(`Result #${resultCount}: subtype=${message.subtype}${loggedResultText ? ` text=${loggedResultText.slice(0, 200)}` : ''}`);
                logUsage(message);
                ...
                writeOutput({
                    ...
                    ...(usage
                        ? {
                            usage,
                            usageEventId: usageEventIdForMessage(message, newSessionId ?? agentInput.sessionId, resultCount, queryRunId),
                        }
                        : {}),
                });
                emitStartupTimingDiagnostic();
    ...
    if (messageCount === 0 && resultCount === 0 && !closedDuringQuery)
        throw new Error(persistSdkSession && agentInput.sessionId
