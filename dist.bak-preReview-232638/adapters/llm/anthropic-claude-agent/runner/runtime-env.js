...
export const IPC_INTERACTION_BOUNDARY_DIR = path.join(IPC_BASE_DIR, 'interaction-boundaries');
...
export const PROVIDER_ACCOUNT_ID = process.env.GANTRY_PROVIDER_ACCOUNT_ID?.trim() || '';
...
export const JOB_RUN_LEASE_TOKEN = process.env.GANTRY_JOB_RUN_LEASE_TOKEN?.trim() || '';
export const JOB_RUN_LEASE_FENCING_VERSION = process.env.GANTRY_JOB_RUN_LEASE_FENCING_VERSION?.trim() || '';
...
export const PERMISSION_MODE = process.env.GANTRY_PERMISSION_MODE?.trim() === 'auto' ? 'auto' : 'ask';
export const PERMISSION_LANE = process.env.GANTRY_PERMISSION_LANE?.trim() === 'interactive'
    ? 'interactive'
    : 'autonomous';
export const TURN_INTENT_SUMMARY = process.env.GANTRY_TURN_INTENT_SUMMARY?.trim() || '';
export const SENDER_ID = process.env.GANTRY_MEMORY_USER_ID?.trim() || '';
export const SENDER_IS_CONTROL_APPROVER = process.env.GANTRY_MEMORY_REVIEWER_IS_CONTROL_APPROVER === '1';
export const PERMISSION_REQUEST_TIMEOUT_MS = getPermissionTimeoutMs(PERMISSION_LANE);
export const IPC_INPUT_CLOSE_SENTINEL = path.join(IPC_INPUT_DIR, '_close');
export const RUNTIME_SIGNAL_FALLBACK_POLL_MS = 500;
function copyPlaceholderEnv(target, keys) {
...
function readModelCredentialEnv(source) {
    if (!source)
        return {};
    const env = {};
...
function stripNonModelProxyEnv(target) {
    for (const key of NON_MODEL_PROXY_ENV_KEYS) {
        target[key] = undefined;
    }
}
function applySandboxRuntimeProxyEnv(target) {
...
function applySandboxRuntimeSdkDirectEgressGuards(target) {
    if (process.env.GANTRY_SANDBOX_RUNTIME_PROXY !== '1')
        return;
...
function hasProxyEnv(target) {
    return PROXY_ENV_KEYS.some((key) => !!target[key]?.trim());
}
export function buildEffectiveToolNetworkEnv(toolNetworkEnv) {
    const env = {
        ...(toolNetworkEnv ?? {}),
    };
...
export function buildSdkEnv(modelCredentialEnv) {
    const sdkEnv = {
...
export function resolveClaudeCodeExecutableFromPath(envPath = process.env.PATH) {
    if (!envPath)
        return undefined;
...
export function allowedOuterSandboxClaudeExecutable(executable) {
    if (!executable)
        return undefined;
...
export function resolveMcpServerPath(importMetaUrl) {
    const configuredPath = process.env.GANTRY_MCP_SERVER_PATH?.trim();
    if (configuredPath) {
        return configuredPath;
    }
    const dirname = path.dirname(fileURLToPath(importMetaUrl));
    return path.join(dirname, '..', '..', '..', '..', 'runner', 'mcp', 'stdio.js');
}
export function resolveWorkspaceIpcDir(workspaceFolder) {
