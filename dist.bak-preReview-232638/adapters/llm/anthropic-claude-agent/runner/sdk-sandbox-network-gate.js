...
import { normalizeEgressAuthorityHost, resolvePublicEgressAddress, } from '../../../../shared/egress-target-resolution.js';
export async function decideSdkSandboxNetworkAccess(input) {
    if (!isSdkSandboxNetworkAccessToolName(input.toolName))
        return null;
    const authority = typeof input.toolInput.host === 'string' ? input.toolInput.host : '';
    ...
    const resolution = host
        ...
        : { ok: false, host: authority.trim() };
    if (!resolution.ok) {
        return {
            behavior: 'deny',
            message: resolution.deny?.reason ??
