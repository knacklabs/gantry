...
import { buildAgentToolExecutionRequest, ToolExecutionClassifier, ToolExecutionPolicyService, } from '../../../../shared/tool-execution-policy-service.js';
import { livePermissionRulesForUpdates, permissionRequestToolName, readRunnerSkillActionCapabilities, scheduledPermissionSuggestionPlan, } from './permission-suggestions.js';
...
import { emitJobToolActivity, emitYoloDenylistHit, yoloDenylistPromptReason, } from './tool-permission-events.js';
...
const WORKSPACE_FOLDER_KEY = WORKSPACE_FOLDER_OPTION_KEY;
const RAW_REQ = /^(Agent|AskUserQuestion|TodoWrite)$/;
const REMOVED_NATIVE_SUBAGENT_TOOL = /^Task(Create|Get|List|Output|Stop|Update)?$/;
export function createPermissionApprovalContextChannel() {
    const pending = new Map();
    const postToolUse = async (hookInput, toolUseID) => {
    ...
    return {
        record: (toolUseID, context) => pending.set(toolUseID, context),
        postToolUse,
    };
}
export function createCanUseToolCallback(input) {
    ...
    const liveApprovedRules = new Set();
    ...
    const currentAutonomousAllowedToolRules = () => [
    ...
    const capabilityRequestToolsHidden = lockedAccessPreset || input.agentInput.hideAuthorityTools === true;
    return async (toolName, rawToolInput, permissionOpts) => {
        input.recordToolActivity(toolName);
        emitJobToolActivity(input.agentInput, input.getNewSessionId, 'sdk_tool_request', toolName, {
        ...
        if (REMOVED_NATIVE_SUBAGENT_TOOL.test(toolName)) {
            const message = 'Native SDK Task subagent tools are not supported. Use the Agent tool for native subagents, or request the Gantry AgentDelegation facade.';
        ...
        if (waitOnlyDenial) {
            ...
            emitJobToolActivity(input.agentInput, input.getNewSessionId, 'deny', toolName, {
                ok: false,
        ...
        if (input.agentInput.runMode === 'prime') {
            const deniedReason = 'Prime mode records requested tool access without executing tools.';
            ...
            const attempt = {
                ...
                displayName: publicToolName === toolName
                ...
                toolInput,
                suggestions: scheduledPermissionSuggestionPlan(toolName, permissionOpts.suggestions, {
                    blockedPath: permissionOpts.blockedPath,
                    toolInput,
        ...
        const trustedInput = applyBashTrustEnvWithProvenance(toolName, toolInput, input.agentInput.toolNetworkEnv ?? {});
        const trustInput = () => trustedInput.toolInput;
        const requestPermissionApprovalWithTrustProvenance = (approvalInput) => requestPermissionApproval({
            ...approvalInput,
        ...
        const sdkApprovalPrincipal = permissionOpts.agentID?.trim() ||
            input.agentInput.agentId ||
            input.workspaceFolder;
        const allowToolUse = (reason = 'allowed') => {
            emitJobToolActivity(input.agentInput, input.getNewSessionId, 'allow', toolName, {
                ok: true,
                reason,
            });
            return { behavior: 'allow', updatedInput: trustInput() };
        };
        ...
        const protectedCapabilityDenial = denyProtectedCapabilityToolUse(toolName, toolInput, permissionOpts);
        if (protectedCapabilityDenial) {
            log(`Permission denied by protected capability guard: ${protectedCapabilityDenial}`);
            writeOutput({
                status: 'success',
                result: null,
        ...
        const memoryGuardDenial = denyMemoryBoundaryToolUse(toolName, toolInput, permissionOpts, input.memoryBlock);
        ...
        const toolExecutionRequest = buildAgentToolExecutionRequest(toolExecutionClassifier, toolName, toolInput, {
        ...
        if (input.agentInput.isScheduledJob) {
            ...
            const recoveryMessage = yoloDenylistReason ??
                (toolDecision.recoveryAction
            ...
            if (nonPromptableDenial)
                return nonPromptableDenial;
            const publicToolName = permissionRequestToolName(toolName);
            log(`Autonomous run requesting permission for tool ${toolName}: ${recoveryMessage}`);
            input.emitInteractionBoundary();
            emitJobToolActivity(input.agentInput, input.getNewSessionId, 'permission_wait', toolName, {
                ok: false,
                reason: yoloDenylistReason ?? toolDecision.reason,
                ...(recoveryAction ? { recovery_action: recoveryAction } : {}),
            });
            const permissionPlan = scheduledPermissionSuggestionPlan(toolName, permissionOpts.suggestions, {
                blockedPath: permissionOpts.blockedPath,
                toolInput,
                semanticCapabilityDefinitions: skillActionCapabilities,
            });
            ...
            const decision = await requestPermissionApprovalWithTrustProvenance({
                ...
                displayName: publicToolName === toolName
                    ...
                    : publicToolName,
                description: permissionOpts.description,
                decisionReason: yoloDenylistReason ??
                ...
                agentID: permissionOpts.agentID,
                suggestions,
                semanticCapabilityDefinitions: permissionPlan.semanticCapabilityDefinitions,
            ...
            if (decision.approved) {
                ...
                for (const rule of livePermissionRulesForUpdates(persistentUpdates, permissionPlan)) {
                    liveApprovedRules.add(rule);
                }
                emitJobToolActivity(input.agentInput, input.getNewSessionId, 'permission_allowed', toolName, permissionAllowedActivityPayload(decision));
                logPermissionApproval(toolName, decision, 'Autonomous run permission');
                recordPermissionApprovalContext(input, permissionOpts.toolUseID, decision);
                return {
                    behavior: 'allow',
                    updatedInput: trustInput(),
                    ...(persistentUpdates && persistentUpdates.length > 0
                        ? { updatedPermissions: persistentUpdates }
                        : {}),
            ...
            emitJobToolActivity(input.agentInput, input.getNewSessionId, 'permission_denied', toolName, {
                ok: false,
                reason,
                ...(recoveryAction ? { recovery_action: recoveryAction } : {}),
            });
...
function permissionAllowedActivityPayload(decision) {
...
function logPermissionApproval(toolName, decision, prefix) {
    const provenanceMessage = formatPermissionAllowedMessage(decision);
    log(provenanceMessage
        ? `${prefix} for tool ${toolName}: ${provenanceMessage}`
        : `${prefix} approved for tool ${toolName}`);
}
function recordPermissionApprovalContext(input, toolUseID, decision) {
    const additionalContext = formatPermissionAllowedMessage(decision);
    if (!toolUseID || !additionalContext)
        return;
