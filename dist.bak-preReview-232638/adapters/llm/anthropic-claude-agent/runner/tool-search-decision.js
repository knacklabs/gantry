...
export function decideClaudeSdkToolSearch(input) {
    ...
    const serializedToolConfigBytes = Buffer.byteLength(JSON.stringify({
        tools: input.availableTools,
        allowedTools: input.allowedTools,
        disallowedTools: input.disallowedTools,
        mcpServers: input.mcpServers,
    }), 'utf8');
    const anthropicBaseUrlKind = classifyAnthropicBaseUrl(input.sdkEnv.ANTHROPIC_BASE_URL, input.sdkEnv[ANTHROPIC_API_KEY_ENV]);
    const metrics = {
        availableToolCount,
        allowedToolCount,
        disallowedToolCount,
    ...
    return {
        ...
        reason: anthropicBaseUrlKind === 'gantry_loopback'
...
function classifyAnthropicBaseUrl(value, apiKey) {
