...
export function assertChatBatchUploadSize(body, provider) {
    const bytes = Buffer.byteLength(body, 'utf8');
    if (bytes > CHAT_BATCH_UPLOAD_LIMIT_BYTES) {
        throw new Error(`${provider} batch upload is ${bytes} bytes; the limit is ${CHAT_BATCH_UPLOAD_LIMIT_BYTES} bytes.`);
    }
}
export async function fetchBatchJson(input) {
    ...
    try {
        ...
        throw new Error(`${input.provider} batch ${input.operation} returned invalid JSON: ${errorMessage(error)}`, { cause: error });
    }
}
export async function fetchBatchJsonl(input) {
    ...
    const parseLine = (line) => {
        ...
        if (budget.rowsRead >= maxRows) {
            throw new Error(`${input.provider} batch result exceeded the ${maxRows} row limit.`);
        }
        try {
            ...
            throw new Error(`${input.provider} batch result JSONL line ${lineNumber} is invalid: ${errorMessage(error)}`, { cause: error });
        }
    };
    while (true) {
        ...
        if (budget.bytesRead > maxBytes) {
            await reader.cancel();
            throw new Error(`${input.provider} batch result exceeded the ${maxBytes} byte limit.`);
        }
...
export function estimateChatBatchCostUsd(model, usage) {
...
async function assertBatchHttpOk(response, provider, operation) {
    if (response.ok)
        return;
    const detail = (await response.text()).slice(0, 300);
    throw new Error(`${provider} batch ${operation} failed: ${response.status} ${response.statusText}${detail ? ` - ${detail}` : ''}`);
}
