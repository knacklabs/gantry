...
export const DEEPAGENTS_CHECKPOINT_PACKAGE_NAME = '@langchain/langgraph-checkpoint-postgres';
...
export async function ensureDeepAgentsCheckpointSchema(input) {
    ...
    if (!databaseUrl || !schema) {
        throw new Error('DeepAgents checkpoint schema setup requires Postgres connection and schema.');
    }
    const key = `${databaseUrl}\0${schema}`;
    let setup = setupByTarget.get(key);
    if (!setup) {
        setup = setupDeepAgentsCheckpointSchema({ databaseUrl, schema }).catch((error) => {
