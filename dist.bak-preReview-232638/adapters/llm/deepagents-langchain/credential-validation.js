import { agentEngineLabel, DEEPAGENTS_ENGINE, } from '../../../shared/agent-engine.js';
...
const DEEPAGENTS_OAUTH_CREDENTIAL_MESSAGE = 'DeepAgents cannot use Claude OAuth/subscription credentials. Choose Anthropic SDK or configure Claude API-key Model Access.';
export function validateDeepAgentCredentialProjection(input) {
    ...
    const supportedModes = provider?.executionRoute.executionProviderId === 'deepagents:langchain'
    ...
    if (!supportedModes.includes(projection.brokerAuthMode)) {
        throw new Error(`unsupported-credential-mode: ${agentEngineLabel(DEEPAGENTS_ENGINE)} does not support credential mode "${projection.brokerAuthMode}" for ${provider.label} Model Access.`);
    }
    ...
    if (projection.env.CLAUDE_CODE_OAUTH_TOKEN ||
...
function assertGatewayToken(value, entry) {
    if (value && !value.startsWith('gtw_')) {
        throw new Error(`Gantry Model Gateway projection for ${entry.displayName} must use a run-scoped gateway token.`);
    }
}
