...
const GANTRY_DEEPAGENTS_CACHE_PROMPT_CONTROL_ENV = 'GANTRY_DEEPAGENTS_CACHE_PROMPT_CONTROL';
const GANTRY_DEEPAGENTS_PROMPT_CACHE_KEY_ENV = 'GANTRY_DEEPAGENTS_PROMPT_CACHE_KEY';
...
const GANTRY_DEEPAGENTS_MAX_INPUT_TOKENS_ENV = 'GANTRY_DEEPAGENTS_MAX_INPUT_TOKENS';
const GANTRY_DEEPAGENTS_OPENROUTER_PROVIDER_ROUTING_ENV = 'GANTRY_DEEPAGENTS_OPENROUTER_PROVIDER_ROUTING';
function openRouterProviderRoutingEnv(routing) {
    if (!routing)
        return undefined;
...
export function deepAgentsCheckpointSchema(storageSchema) {
    const suffix = '_deepagents';
    const maxBaseLength = 63 - suffix.length;
    const base = storageSchema.slice(0, maxBaseLength);
    return `${base}${suffix}`;
}
export class DeepAgentsLangChainExecutionAdapter {
    id = 'deepagents:langchain';
    isMissingProviderSessionError(error) {
        return isMissingDeepAgentSessionError(error);
    }
    async prepare(input) {
        const runnerPath = path.join(input.hostRuntime.runnerDistDir, '..', 'adapters', 'llm', 'deepagents-langchain', 'runner', 'index.js');
        if (!fs.existsSync(runnerPath)) {
            throw new Error('Host runtime is missing required DeepAgents execution adapter runner files. Reinstall Gantry from npm and restart.');
        }
        ...
        if (relativeRunnerPath.startsWith('..') ||
            ...
            throw new Error('DeepAgents execution adapter runner path escaped the Gantry package root.');
        }
        ...
        const runtimeConfigDir = path.join(runtimeScratchRoot, `deepagents-${safePathSegment(input.input.runId ?? randomUUID())}`);
        fs.mkdirSync(runtimeConfigDir, { recursive: true, mode: 0o700 });
        const modelCredentialEnv = projectDeepAgentModelCredentialEnv(input.modelCredentialProjection.env);
        const env = {};
        ...
        if (input.effectiveModelEntry) {
            ...
            const openRouterRouting = openRouterProviderRoutingEnv(input.effectiveModelEntry.providerRouting?.openrouter);
        ...
        if (!input.input.isScheduledJob) {
            ...
            const postgresUrlEnv = input.runtimeStorage?.postgresUrlEnv ?? 'GANTRY_DATABASE_URL';
            const postgresSchema = input.runtimeStorage?.postgresSchema ?? 'gantry';
            if (!postgresUrl) {
                throw new Error(`DeepAgents live sessions require runtime Postgres storage. Set ${postgresUrlEnv} before using DeepAgents live turns.`);
            }
...
export function createDeepAgentsLangChainExecutionAdapter() {
