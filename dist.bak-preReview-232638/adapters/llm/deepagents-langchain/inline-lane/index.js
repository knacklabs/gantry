...
import pg from 'pg';
import { assertMcpNetworkHostAllowed, createGuardedMcpFetch, } from '../../../../application/mcp/mcp-tool-proxy-network.js';
...
import { DEFAULT_INLINE_AGENT_MAX_TURNS, inlineAgentMaxTurnsError, } from '../../inline-lane-dispatcher.js';
import { createInlineToolActivity, } from '../../inline-lane-tool-activity.js';
import { ensureDeepAgentsCheckpointSchema } from '../checkpoint-setup.js';
import { reconcileDeepAgentSkillFiles, resolveDeepAgentSkillProjection, } from '../skill-projection.js';
...
import { buildRunnerModel, } from '../runner/model-factory.js';
import { normalizeDeepAgentStream, } from '../runner/stream-normalizer.js';
...
export function createDeepAgentsInlineAgentLoopLane(input) {
    return async (laneInput) => {
        ...
        const backend = (config) => new StateBackend(config);
        ...
        const memoryMiddleware = memory.createGantryScopedMemoryMiddleware({
            ...
            searchMemory: (query) => memory.searchGantryScopedMemory(laneInput, query, signal),
        });
        let saver;
        let remoteMcp;
        let lastTerminal;
        try {
            ...
            const model = await buildInlineModel(laneInput, sessionId, promptCache.promptCacheKey);
            ...
            const skillFiles = reconcileDeepAgentSkillFiles({
                ...
                checkpointTuple: saver && laneInput.input.sessionId
            ...
            const graph = createDeepAgent({
                ...
                ...(laneInput.input.responseSchema
                    ? {
                        responseFormat: responseFormatForSchema(laneInput.input.responseSchema, model.model),
                    }
                    : {}),
                backend,
            ...
            for (;;) {
                ...
                const messages = applyCachePromptControl(memory.buildInlineTurnMessages(prompt, firstTurn ? laneInput.input.memoryContextBlock : undefined), promptCache.cacheMode);
                firstTurn = false;
                let normalized;
                let structuredResponse;
                try {
                    normalized = await normalizeDeepAgentStream({
                        events: captureStructuredResponse(graph.streamEvents({
                            messages,
                ...
                if (signal.aborted || closeRequested)
                    break;
                ...
                if (laneInput.input.isScheduledJob)
                    break;
                if (!continuedByFollowup && pendingFollowups.length === 0)
                    break;
            }
            if (laneInput.signal.aborted)
                return abortedOutput(sessionId);
            return (lastTerminal ?? {
                status: 'success',
                result: null,
                newSessionId: sessionId,
...
async function openCheckpointer(input) {
    ...
    if (!databaseUrl || !schema) {
        throw new Error('DeepAgents inline lane requires Postgres checkpointer configuration.');
    }
    ...
    if (input.resumeSessionId) {
        try {
            ...
            if (!tuple) {
                throw new Error(`No DeepAgents session found with session ID: ${input.resumeSessionId}`);
            }
        }
        catch (error) {
            await saver.end().catch(() => undefined);
...
async function buildInlineModel(input, sessionId, promptCacheKey) {
    if (!input.resolvedModel.ok)
        throw new Error(input.resolvedModel.message);
    const baseUrl = input.modelCredentialEnv.OPENAI_BASE_URL?.trim();
    const token = input.modelCredentialEnv.OPENAI_API_KEY?.trim();
    if (!baseUrl || !token) {
        throw new Error('DeepAgents inline lane is missing loopback gateway model credentials.');
    }
    const openRouterProviderRouting = toOpenRouterProviderPreferences(input.resolvedModel.value.modelEntry.providerRouting?.openrouter);
    return buildRunnerModel({
        ...
        ...(input.resolvedModel.value.modelEntry.contextWindowTokens
            ? {
                maxInputTokens: input.resolvedModel.value.modelEntry.contextWindowTokens,
            }
            : {}),
    });
}
function toOpenRouterProviderPreferences(routing) {
    if (!routing)
        return undefined;
    return {
...
function buildCoreLangChainTools(input, toolActivity) {
    return input.coreTools.tools.map((definition) => createLangChainTool((args, config) => toolActivity.run(definition.name, () => input.coreTools.execute(definition.name, args, {
...
async function connectRemoteMcpTools(servers, input) {
    ...
    try {
        for (const server of servers) {
            ...
            const transport = server.config.type === 'sse'
            ...
            for (const remoteTool of loaded) {
                if (!server.allowedToolPatterns.some((pattern) => mcpToolPatternCovers(pattern, remoteTool.name))) {
                    continue;
                }
                const toolName = `mcp__${server.name}__${remoteTool.name}`;
                tools.push(createLangChainTool(async (args, config) => {
                    const authorization = await input.authorizeThirdPartyMcpTool(toolName, args, { signal: config?.signal ?? input.signal });
                    ...
                    return input.toolActivity.run(toolName, async () => {
                        ...
                        try {
                            const result = await remoteTool.invoke(args, config?.signal ? { signal: config.signal } : undefined);
                            const activity = {
                                serverName: server.name,
                                toolName: remoteTool.name,
                                toolInput: args,
    ...
    return {
        tools,
        close: () => Promise.all(clients.map((client) => client.close())).then(() => undefined),
    };
}
function inlineSystemPrompt(input) {
...
function isGraphRecursionLimitError(error) {
    if (!error || typeof error !== 'object')
        return false;
    const value = error;
    return (value.name === 'GraphRecursionError' ||
        value.lc_error_code === 'GRAPH_RECURSION_LIMIT');
}
async function* captureStructuredResponse(events, capture) {
...
function responseFormatForSchema(schema, { profile: { structuredOutput } }) {
    const name = 'gantry_structured_output';
    const normalized = { ...schema, name, title: name };
    if (structuredOutput === true)
        return ProviderStrategy.fromSchema(normalized);
