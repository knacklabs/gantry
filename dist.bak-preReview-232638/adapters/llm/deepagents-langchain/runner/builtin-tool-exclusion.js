...
const EXCLUDED_SET = new Set(EXCLUDED_RAW_DEEPAGENT_TOOL_NAMES);
const SKILL_READ_TOOL_SET = new Set(READONLY_SKILL_FILESYSTEM_DEEPAGENT_TOOL_NAMES);
export function createBuiltinToolExclusionMiddleware(input) {
    const excluded = input?.exposeSkillReadTools
        ? new Set([...EXCLUDED_SET].filter((name) => !SKILL_READ_TOOL_SET.has(name)))
        : EXCLUDED_SET;
    return createMiddleware({
        ...
        wrapModelCall: async (request, handler) => {
            ...
            return handler({
                ...request,
                tools: tools.filter((tool) => typeof tool?.name !== 'string' || !excluded.has(tool.name)),
            });
        },
    });
}
