...
export async function workspaceRoot(config) {
    const configured = config.cwd?.trim() ||
        process.env.GANTRY_WORKSPACE_GROUP_DIR?.trim() ||
        process.cwd();
    return fs.realpath(configured);
}
export async function resolveExistingWorkspacePath(relativePath, config) {
    const root = await workspaceRoot(config);
    const candidate = path.resolve(root, relativePath);
    ensureInsideRoot(root, candidate);
    const parent = await resolveWorkspaceParent(root, path.dirname(candidate), 'FileRead', false);
    const real = await fs.realpath(path.join(parent, path.basename(candidate)));
    ensureInsideRoot(root, real);
    return real;
}
export async function resolveWritableWorkspacePath(relativePath, config, toolName) {
    const root = await workspaceRoot(config);
    const candidate = path.resolve(root, relativePath);
    ensureInsideRoot(root, candidate);
    const parent = await resolveWorkspaceParent(root, path.dirname(candidate), toolName, toolName === 'FileWrite');
...
export async function writeFileNoFollow(target, content) {
    const handle = await fs.open(target, fsConstants.O_WRONLY |
        fsConstants.O_CREAT |
        fsConstants.O_TRUNC |
        fsConstants.O_NOFOLLOW, 0o666);
    try {
...
async function resolveWorkspaceParent(root, parent, toolName, createMissing) {
