...
import { GANTRY_FACADE_EXACT_TOOL_NAMES, GANTRY_FACADE_INPUT_SCHEMAS, validateGantryFacadeToolInput, } from '../../../../shared/gantry-tool-facades.js';
...
import { resolveExistingWorkspacePath, resolveWritableWorkspacePath, workspaceRoot, writeFileNoFollow, } from './gantry-facade-file-safety.js';
import { gatedToolErrorResult, } from './third-party-mcp-gate.js';
...
export const DEEPAGENTS_GANTRY_FACADE_TOOL_NAMES = GANTRY_FACADE_EXACT_TOOL_NAMES;
const DEEPAGENTS_FILESYSTEM_FACADE_TOOL_NAMES = new Set([
    'FileSearch',
    'FileRead',
    'FileEdit',
    'FileWrite',
...
export function createGantryFacadeTools(config) {
    return DEEPAGENTS_GANTRY_FACADE_TOOL_NAMES.filter((toolName) => (toolName !== 'AgentDelegation' ||
        (config.asyncTaskToolsEnabled === true && config.delegateTaskTool)) &&
        (config.filesystemToolsEnabled ||
            !DEEPAGENTS_FILESYSTEM_FACADE_TOOL_NAMES.has(toolName))).map((toolName) => createOneFacadeTool(toolName, config));
}
function createOneFacadeTool(toolName, config) {
    return tool(async (input) => {
        ...
        if (!approval.approved) {
            return gatedToolErrorResult(`Permission denied: ${approval.reason || 'Denied by operator'}`);
        }
        return executeFacadeTool(toolName, input, config);
    }, {
        name: toolName,
        description: facadeDescription(toolName),
        schema: GANTRY_FACADE_INPUT_SCHEMAS[toolName].schema,
    });
}
export function gantryFacadePolicyToolRequest(toolName, input) {
    const record = input;
    switch (toolName) {
        case 'WebSearch':
...
async function executeFacadeTool(toolName, input, config) {
    const record = input;
    switch (toolName) {
        case 'WebSearch':
            return webSearch(String(record.query), numberOrDefault(record.maxResults, 8), config);
        case 'WebRead':
...
async function agentDelegation(input, config) {
...
function installedDeepAgentsVersion() {
    const version = deepagentsPackageJson.version;
...
async function webRead(url, config) {
    const proxyUrl = auditedWebProxyUrl(config.toolNetworkEnv);
    if (!proxyUrl)
        return webEgressUnavailableMessage();
    ...
    return new Promise((resolve, reject) => {
        ...
        const request = http.request({
            ...
            if (Number.isFinite(contentLength) &&
                ...
                response.resume();
                finish(() => reject(new Error(`Web response exceeded ${MAX_WEB_RESPONSE_BYTES} bytes.`)));
                return;
            }
            const chunks = [];
            let totalBytes = 0;
            response.on('data', (chunk) => {
                ...
                if (totalBytes > MAX_WEB_RESPONSE_BYTES) {
                    const err = new Error(`Web response exceeded ${MAX_WEB_RESPONSE_BYTES} bytes.`);
...
function auditedWebProxyUrl(toolNetworkEnv) {
    const projectedProxy = toolNetworkEnv?.HTTPS_PROXY?.trim() || toolNetworkEnv?.HTTP_PROXY?.trim();
    if (!projectedProxy)
        return null;
    if (!isLoopbackHttpProxy(projectedProxy))
        return null;
    if (toolNetworkEnv?.NODE_USE_ENV_PROXY !== '1')
        return null;
    return projectedProxy;
}
async function fileSearch(input, config) {
    ...
    await walk(root, '', async (absolute, relative, entry) => {
        ...
        if (!matchesFilters(relative, include, exclude))
            return true;
        if (input.mode === 'path') {
            if (relative.toLowerCase().includes(queryLower))
                results.push(relative);
            return results.length < maxResults && visited < MAX_SEARCH_ENTRIES;
        }
        const stat = await fs.stat(absolute).catch(() => null);
        if (!stat || stat.size > MAX_SEARCH_FILE_BYTES)
            return true;
...
async function fileRead(relativePath, config) {
...
async function fileWrite(relativePath, content, config) {
    const target = await resolveWritableWorkspacePath(relativePath, config, 'FileWrite');
    await writeFileNoFollow(target, content);
    return `Wrote ${Buffer.byteLength(content, 'utf-8')} bytes to ${relativePath}.`;
}
async function fileEdit(relativePath, patch, config) {
    const target = await resolveWritableWorkspacePath(relativePath, config, 'FileEdit');
    const stat = await fs.stat(target);
    if (stat.size > MAX_SEARCH_FILE_BYTES) {
...
function parseEditPatch(patch) {
    try {
        const parsed = JSON.parse(patch);
        if (typeof parsed.oldText === 'string' &&
...
async function walk(root, relativeDir, visit) {
...
function isLoopbackHttpProxy(value) {
    try {
...
function facadeDescription(toolName) {
    switch (toolName) {
        case 'WebSearch':
