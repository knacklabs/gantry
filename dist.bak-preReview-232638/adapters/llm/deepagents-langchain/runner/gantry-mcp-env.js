import { GATED_GANTRY_MCP_TOOL_NAMES, selectedGantryMcpToolNames, selectedMemoryIpcActions, } from '../../../../runner/gantry-mcp-tool-surface.js';
...
const BROWSER_GATEWAY_TOOL_NAME_SET = new Set(GATED_GANTRY_MCP_TOOL_NAMES);
export function buildGantryMcpProjection(input) {
    const env = input.processEnv;
    const memoryReviewerIsControlApprover = env.GANTRY_MEMORY_REVIEWER_IS_CONTROL_APPROVER === '1';
    const browserIpcEnabled = Boolean(env.GANTRY_BROWSER_IPC_AUTH_TOKEN?.trim()) &&
    ...
    const callableAgentManifest = asyncTaskToolsEnabled &&
        ...
        : [];
    const selectedToolNamesBase = selectedGantryMcpToolNames(input.configuredAllowedTools, {
    ...
    const callableAgentToolNames = callableAgentManifest.map(callableAgentToolName);
    const selectedToolNames = browserIpcEnabled
        ? [...selectedToolNamesBase, ...callableAgentToolNames]
        : [
            ...selectedToolNamesBase.filter((toolName) => !BROWSER_GATEWAY_TOOL_NAME_SET.has(toolName)),
            ...callableAgentToolNames,
        ];
    const memoryIpcActions = selectedMemoryIpcActions(input.configuredAllowedTools, { memoryReviewerIsControlApprover });
    const serverEnv = {
        ...
        GANTRY_CONFIGURED_ALLOWED_TOOLS_JSON: JSON.stringify(input.configuredAllowedTools),
        ...
        GANTRY_SELECTED_SKILL_DISPLAYS_JSON: env.GANTRY_SELECTED_SKILL_DISPLAYS_JSON ?? '[]',
        GANTRY_SELECTED_MCP_SERVERS_JSON: env.GANTRY_SELECTED_MCP_SERVERS_JSON ?? '[]',
        GANTRY_SEMANTIC_CAPABILITIES_JSON: env.GANTRY_SEMANTIC_CAPABILITIES_JSON ?? '[]',
