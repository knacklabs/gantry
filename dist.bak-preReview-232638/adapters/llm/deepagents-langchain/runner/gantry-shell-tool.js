...
function buildShellChildEnv(toolNetworkEnv) {
    const env = {};
...
export function createGantryShellTool(config) {
    const gatedFunc = async (input) => {
        const command = typeof input?.command === 'string' ? input.command : '';
        if (!command.trim()) {
            return gatedToolErrorResult('RunCommand requires a non-empty command string.', 'validation');
        }
    ...
    return tool(gatedFunc, {
        ...
        description: 'Run a shell command inside the Gantry sandbox. Every call is policy-gated and may require per-call human approval. Output is truncated.',
...
async function runShellCommand(command, config) {
    ...
    return new Promise((resolve) => {
        ...
        const timer = setTimeout(() => {
            terminate(`Command timed out after ${DEFAULT_COMMAND_TIMEOUT_MS}ms and was killed.`);
        ...
        child.on('error', (err) => {
            finish(failedShellCommandResult({
                stdout,
                stderr,
...
function killShellProcessGroup(child, signal) {
