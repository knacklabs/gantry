...
import { drainIpcInput, prepareInteractiveIpcInputDir, } from '../../../../runner/runner-ipc-input.js';
...
import { readRunnerStdin, writeRunnerFrame, } from '../../../../runner/runner-frame.js';
import { createDeepAgentCheckpointTiming, DeepAgentSessionStore, } from './session-store.js';
...
function resolveModelId() {
    ...
    if (fromEnv)
        return fromEnv;
    throw new Error('DeepAgents runner is missing GANTRY_DEEPAGENTS_MODEL_ID for the resolved model route.');
}
function resolveModelProvider() {
    const fromEnv = process.env.GANTRY_DEEPAGENTS_MODEL_PROVIDER?.trim();
    if (fromEnv)
        return fromEnv;
    throw new Error('DeepAgents runner is missing GANTRY_DEEPAGENTS_MODEL_PROVIDER for the resolved model route.');
}
...
function resolveOpenRouterProviderRouting() {
    ...
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
        throw new Error('DeepAgents runner OpenRouter provider routing must be a JSON object.');
    }
    return parsed;
}
function runtimeEventsForTurn(turn) {
    return turn?.startupRuntimeEvents?.length
        ? { runtimeEvents: turn.startupRuntimeEvents }
        : {};
}
async function runScheduled(agentInput) {
...
async function runInteractive(agentInput) {
    prepareInteractiveIpcInputDir();
    const sessionId = agentInput.sessionId ?? DeepAgentSessionStore.newSessionId();
    ...
    try {
        ...
        const store = new DeepAgentSessionStore(agentInput.deepAgentCheckpointer, checkpointTiming);
        ...
        for (;;) {
            ...
            const turnInput = pendingFollowups.length > 0
                ? {
                    ...agentInput,
                    prompt: firstTurn
                        ? `${agentInput.prompt}\n${followupText}`
