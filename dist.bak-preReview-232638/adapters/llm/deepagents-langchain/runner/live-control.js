import { drainIpcInput, interactiveIpcInputDir, shouldClose, } from '../../../../runner/runner-ipc-input.js';
...
export function startDeepAgentLiveControl(options) {
    ...
    const drainOnce = () => {
        try {
            ...
            log?.(`live-control poll error: ${err instanceof Error ? err.message : String(err)}`);
        }
        return false;
    };
    const signalPump = startRuntimeSignalPump({
        ...
        deps: {
            ...
            onWatchError: (input) => {
                ...
                log?.(`live-control watch failed for ${input.dir}: ${input.error instanceof Error
                    ? input.error.message
                    : String(input.error)}; using fallback poll`);
            },
        },
    });
    return {
