...
export function resolveWorkspaceIpcDir(agentFolder) {
...
export function buildPermissionIpcRuntimeEnv() {
    const jobId = readEnv('GANTRY_JOB_ID');
    const permissionLane = readEnv('GANTRY_PERMISSION_LANE') === 'interactive'
        ? 'interactive'
        : 'autonomous';
    return {
        ...
        permissionLane,
        permissionMode: readEnv('GANTRY_PERMISSION_MODE') === 'auto' ? 'auto' : 'ask',
        senderId: jobId ? undefined : readEnv('GANTRY_MEMORY_USER_ID') || undefined,
        senderIsControlApprover: readEnv('GANTRY_MEMORY_REVIEWER_IS_CONTROL_APPROVER') === '1',
