...
import { cleanSkillMetadataText, parseSkillFrontmatter, } from '../../../shared/skill-artifact-helpers.js';
...
export function reconcileDeepAgentSkillFiles(input) {
    const update = { ...(input.currentFiles ?? {}) };
    ...
    for (const filePath of Object.keys(files ?? {})) {
        if (filePath.startsWith(DEEPAGENTS_SKILLS_SOURCE) &&
            !(filePath in update)) {
...
async function projectSkillFiles(input) {
    ...
    if (!skillMd) {
        throw new Error(`DeepAgents selected skill "${input.skill.id}" artifact must include SKILL.md.`);
    }
    ...
    for (const asset of input.skill.assets) {
        ...
        if (!isTextMimeType(mimeType)) {
            throw new Error(`DeepAgents selected skill "${input.skill.id}" asset "${asset.path}" has unsupported binary content type "${mimeType}".`);
        }
...
function validateDeepAgentSkillMetadata(input) {
    if (Buffer.byteLength(input.skillText, 'utf-8') > MAX_SKILL_FILE_SIZE_BYTES) {
        throw new Error(`DeepAgents selected skill "${input.skill.id}" SKILL.md exceeds the official 10 MB limit.`);
    }
    ...
    if (!name || !description) {
        throw new Error(`DeepAgents selected skill "${input.skill.id}" SKILL.md must declare frontmatter name and description.`);
    }
    if (name.length > MAX_SKILL_NAME_LENGTH) {
        throw new Error(`DeepAgents selected skill "${input.skill.id}" name exceeds the official 64 character limit.`);
    }
    if (description.length > MAX_SKILL_DESCRIPTION_LENGTH) {
        throw new Error(`DeepAgents selected skill "${input.skill.id}" description exceeds the official 1024 character limit.`);
    }
    if (!isDeepAgentSkillName(name) || name !== input.targetName) {
        throw new Error(`DeepAgents selected skill "${input.skill.id}" declares SDK skill name "${name}" but materializes as "${input.targetName}". Use a lowercase hyphenated SKILL.md name that matches the Gantry materialized skill directory.`);
    }
}
function isDeepAgentSkillName(value) {
...
function decodeUtf8Asset(input) {
    try {
        ...
        throw new Error(`DeepAgents selected skill "${input.skillId}" asset "${input.assetPath}" must be UTF-8 text.`, { cause: error });
    }
}
