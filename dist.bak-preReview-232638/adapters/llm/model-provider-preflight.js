...
import { listModelCatalogEntries, resolveModelSelectionForWorkload, } from '../../shared/model-catalog.js';
...
export async function preflightModelProvider(input) {
    ...
    if (!model.ok)
        return { ok: false, status: 'fail', message: model.message };
    if (!model.entry) {
        return {
            ok: false,
            status: 'fail',
    ...
    const runId = `model-preflight:${randomUUID()}`;
    let broker;
    try {
