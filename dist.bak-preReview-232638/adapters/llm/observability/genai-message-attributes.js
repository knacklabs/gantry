import { boundedContent, MAX_ATTRIBUTE_CHARS, TRACE_CONTENT_MAX_CHARS, } from '../../../infrastructure/observability/tracing.js';
const PROVIDER_SYSTEM_MAP = {
    anthropic: 'anthropic',
    openai: 'openai',
    bedrock: 'aws.bedrock',
...
export function boundedTraceValue(value, stringLimit, arrayLimit, depth = 0) {
    ...
    if (Array.isArray(value)) {
        return value
            .slice(0, arrayLimit)
            .map((entry) => boundedTraceValue(entry, stringLimit, arrayLimit, depth + 1));
    }
    ...
    for (const key in value) {
        ...
        if (count >= 64)
            break;
        result[key] = boundedTraceValue(value[key], stringLimit, arrayLimit, depth + 1);
        count += 1;
    }
    return result;
}
function boundedMessageJson(messages, schema) {
    ...
    for (;;) {
        ...
        else if (arrayLimit > 1)
            arrayLimit = Math.floor(arrayLimit / 2);
        else if (messageLimit > 1)
            messageLimit = Math.floor(messageLimit / 2);
        else {
            if (schema === 'legacy') {
            ...
            return JSON.stringify([
                {
                    ...
                    ...(schema === 'current-output'
                        ? {
                            finish_reason: typeof messages.at(-1)?.finish_reason === 'string'
...
function visibleAnthropicBlocks(content) {
    if (!Array.isArray(content))
        return [];
    const visible = [];
    for (const value of content) {
        if (visible.length >= MAX_TOOL_CALLS)
            break;
...
function visibleOpenAiParts(content) {
    if (!Array.isArray(content))
        return [];
    const visible = [];
    for (const value of content) {
...
function legacyMessages(kind, messages) {
...
function normalizedMessages(kind, messages) {
    const normalized = [];
    for (const message of messages) {
        ...
        if (kind === 'anthropic' && Array.isArray(message.content)) {
            ...
            parts.push({
                ...
                response: message.content === undefined ? '' : parsedJson(message.content),
            });
        }
        else {
...
export function setMessageAttributes(span, legacyKey, currentKey, messages, kind) {
    span.setAttribute(legacyKey, boundedMessageJson(legacyMessages(kind, messages), 'legacy'));
    const current = normalizedMessages(kind, messages);
    const schemaComplete = currentKey !== ATTR_OUTPUT_MESSAGES ||
        current.every((message) => typeof message.finish_reason === 'string');
    if (current.length > 0 && schemaComplete) {
        span.setAttribute(currentKey, boundedMessageJson(current, currentKey === ATTR_OUTPUT_MESSAGES
            ? 'current-output'
            : 'current-input'));
    }
}
export function providerSystemFor(providerId) {
    return PROVIDER_SYSTEM_MAP[providerId] ?? providerId;
}
export function providerNameFor(providerId) {
...
export function boundedToolValue(value) {
    let stringLimit = TRACE_CONTENT_MAX_CHARS;
    let arrayLimit = 64;
    for (;;) {
...
export function boundedToolJson(value) {
    return JSON.stringify(boundedToolValue(value));
}
export function boundedToolIdentity(value) {
    return value.length <= TRACE_CONTENT_MAX_CHARS
        ? value
        : `${value.slice(0, TRACE_CONTENT_MAX_CHARS - TRUNCATION_SUFFIX.length)}${TRUNCATION_SUFFIX}`;
}
export function promptMessages(request) {
...
export function responseAssistantMessages(kind, response) {
    ...
    if (kind === 'openai') {
        const messages = [];
        for (const [position, choice] of (response.choices ?? [])
...
export function responseToolCalls(kind, response) {
    if (kind === 'anthropic' && Array.isArray(response.content)) {
        const calls = [];
...
export function requestToolResults(kind, request) {
    if (!Array.isArray(request.messages))
        return [];
    const results = [];
    for (let index = request.messages.length - 1; index >= 0 && results.length < MAX_TOOL_CALLS; index -= 1) {
        const value = request.messages[index];
        if (!value || typeof value !== 'object')
            continue;
        const message = value;
        if (kind === 'anthropic' && Array.isArray(message.content)) {
            for (let blockIndex = message.content.length - 1; blockIndex >= 0 && results.length < MAX_TOOL_CALLS; blockIndex -= 1) {
            ...
            const resultRecord = content && typeof content === 'object'
                ? content
                : undefined;
            const errorMarker = message.is_error ?? resultRecord?.is_error ?? resultRecord?.isError;
            results.push({
                id: boundedToolIdentity(message.tool_call_id),
                content,
                status: errorMarker === true
                    ? 'error'
                    : errorMarker === false
                        ? 'success'
                        : 'unknown',
...
export function setUsageAttributes(span, usage, kind) {
    ...
    if (input !== undefined)
        span.setAttribute('gen_ai.usage.input_tokens', kind === 'anthropic'
...
export function setNormalizedUsageAttributes(span, usage, kind, rawUsage) {
    ...
    if (usage.cacheReadTokens > 0) {
        ...
        span.setAttribute('gen_ai.usage.cache_read.input_tokens', usage.cacheReadTokens);
    }
    if (usage.cacheWriteTokens > 0) {
        span.setAttribute('gen_ai.usage.cache_creation_input_tokens', usage.cacheWriteTokens);
        span.setAttribute('gen_ai.usage.cache_creation.input_tokens', usage.cacheWriteTokens);
    }
}
