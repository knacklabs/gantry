...
import { ATTR_COMPLETION, ATTR_PROMPT, boundedContent, childContextFor, contentCaptureEnabled, getTurnSpan, tracer, } from '../../../infrastructure/observability/tracing.js';
import { createSseAccumulator, } from './sse-accumulator.js';
import { createSseFrameSplitter, isOpenAiUsageOnlyFrame, } from './sse-frame-splitter.js';
import { ATTR_INPUT_MESSAGES, ATTR_OUTPUT_MESSAGES, MAX_TOOL_CALLS, numeric, promptMessages, providerNameFor, providerSystemFor, responseAssistantMessages, responseToolCalls, setMessageAttributes, setNormalizedUsageAttributes, setUsageAttributes, } from './genai-message-attributes.js';
import { failPendingToolSpans, finishPendingToolSpans, pendingToolsByRun, startPendingToolSpans, streamedToolCalls, } from './genai-tool-spans.js';
let pendingTracer;
function componentFor(token) {
    if (token.apiKeyId)
        return 'llm-api';
    const runId = token.runId === undefined ? '' : String(token.runId);
    if (runId.startsWith('memory-query:'))
        return 'memory';
...
function streamKindFor(pathname) {
...
function injectIncludeUsage(request) {
...
export function observeGatewayCall(input) {
    ...
    try {
        let request = {};
        try {
            request = JSON.parse(input.requestBody.toString('utf8'));
        }
        catch {
            // Non-JSON body: still trace timing/status.
        }
        if (NON_GENERATION_SUFFIXES.some((suffix) => input.upstreamUrl.pathname.endsWith(suffix))) {
            return undefined;
        }
        const kind = streamKindFor(input.upstreamUrl.pathname);
        const isStreaming = request.stream === true;
        const requestModel = typeof request.model === 'string' ? request.model : undefined;
        const captureContent = contentCaptureEnabled();
        const runId = input.token.runId === undefined ? undefined : String(input.token.runId);
        if (runId) {
        ...
        const span = (startedSpan = activeTracer.startSpan(`chat ${(requestModel ?? input.providerId).slice(0, 128)}`, {
            attributes: {
        ...
        if (captureContent) {
            ...
            if (messages.length > 0) {
                setMessageAttributes(span, ATTR_PROMPT, ATTR_INPUT_MESSAGES, messages, kind);
            }
        }
        const accumulator = isStreaming && kind
            ? createSseAccumulator(kind, captureContent)
            : undefined;
        let tapUsed = false;
        let sawFirstChunk = false;
        let streamStatus;
        const earlyRegisteredToolCallIds = new Set();
        const registerCompletedStreamToolCalls = () => {
            if (!accumulator?.takeToolCallsReady())
                return;
            if (!runId ||
            ...
            if (!liveParent)
                return;
            const complete = streamedToolCalls(kind, streamed).filter((call) => call.complete && !earlyRegisteredToolCallIds.has(call.id));
            ...
            for (const call of complete) {
                if (pending?.has(call.id))
                    earlyRegisteredToolCallIds.add(call.id);
            }
        };
        ...
        const buildStreamTap = () => {
            ...
            const processFrames = (frames) => {
                ...
                for (const frame of frames) {
                    ...
                    registerCompletedStreamToolCalls();
                    if (!isOpenAiUsageOnlyFrame(frame))
                        out.push(`${frame}\n\n`);
                }
        ...
        const finish = (result) => {
            ...
            try {
                ...
                if (tapUsed && accumulator) {
                    ...
                    if (typeof response.stop_reason === 'string') {
                        ...
                        finishReasons = (response.choices ?? [])
                            ...
                            .flatMap((choice) => typeof choice.finish_reason === 'string'
                ...
                if (usage)
                    setUsageAttributes(span, usage, kind);
                const normalized = result.normalizedUsage ??
                    (usage
                        ? normalizeModelUsage({
                ...
                if (captureContent && assistantMessages.length > 0) {
                    setMessageAttributes(span, ATTR_COMPLETION, ATTR_OUTPUT_MESSAGES, assistantMessages, kind);
                }
                ...
                const completeToolCalls = toolCalls.filter((call) => call.complete && !earlyRegisteredToolCallIds.has(call.id));
                if (runId &&
                    liveParent &&
                    result.status >= 200 &&
                    result.status < 300 &&
