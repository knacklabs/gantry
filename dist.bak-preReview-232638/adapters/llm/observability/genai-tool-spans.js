...
import { boundedContent, childContextFor, registerDelegationToolSpan, registerTurnSpanEndCallback, settleDelegationToolSpan, } from '../../../infrastructure/observability/tracing.js';
import { boundedToolJson, requestToolResults, } from './genai-message-attributes.js';
...
function delegationObjective(call) {
...
function delegationTaskId(value, depth = 0) {
...
export function finishPendingToolSpans(runId, kind, request, captureContent) {
        ...
        try {
            ...
            if (active.delegation) {
                settleDelegationToolSpan({
                    runId,
                    callId: result.id,
                    taskId: result.status === 'error'
                        ? undefined
                        : delegationTaskId(result.content),
                });
            }
            active.span.setAttribute('gantry.tool.latency_ms', Math.max(0, Date.now() - active.startedAt));
            active.span.setAttribute('gantry.tool.status', result.status);
            if (captureContent) {
                active.span.setAttribute('gen_ai.tool.call.result', toolPayload(result.content));
            }
    ...
    if (pending.size === 0)
        pendingToolsByRun.delete(runId);
}
export function startPendingToolSpans(input) {
    if (input.toolCalls.length === 0)
        return;
    const pending = pendingToolsByRun.get(input.runId) ?? new Map();
    ...
    for (const call of input.toolCalls) {
        ...
        try {
            ...
            const span = input.activeTracer.startSpan(`execute_tool ${call.name.slice(0, 128)}`, {
                attributes: {
            ...
            active.unregisterTurnEnd = registerTurnSpanEndCallback(input.runId, () => {
                pending.delete(call.id);
                if (pending.size === 0)
                    pendingToolsByRun.delete(input.runId);
                if (active.delegation) {
                    settleDelegationToolSpan({
                        runId: input.runId,
                        callId: call.id,
...
export function failPendingToolSpans(runId, callIds) {
    const pending = pendingToolsByRun.get(runId);
    if (!pending)
        return;
    for (const callId of callIds) {
        ...
        active.unregisterTurnEnd();
        if (active.delegation)
            settleDelegationToolSpan({ runId, callId });
        try {
            active.span.setAttribute('gantry.tool.latency_ms', Math.max(0, Date.now() - active.startedAt));
            active.span.setAttribute('gantry.tool.status', 'error');
...
function isCompleteToolCallResponse(kind, finishReason) {
    return ((kind === 'anthropic' && finishReason === 'tool_use') ||
        (kind === 'openai' && finishReason === 'tool_calls'));
}
export function streamedToolCalls(kind, streamed) {
    return (streamed.toolCalls ?? []).flatMap((call) => typeof call.id === 'string' && typeof call.name === 'string'
        ? [
            {
                ...call,
                id: call.id,
