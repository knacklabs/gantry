...
import { createSseFrameSplitter, MAX_PENDING_CHARS, sseFrameData, } from './sse-frame-splitter.js';
export { createSseFrameSplitter, isOpenAiUsageOnlyFrame, sseFrameData, } from './sse-frame-splitter.js';
...
function boundedToolValue(value, max) {
    if (value.length <= max)
        return value;
    return `${value.slice(0, max - TRUNCATED_SUFFIX.length)}${TRUNCATED_SUFFIX}`;
}
function boundedStructuredValue(value, stringLimit, arrayLimit, depth = 0) {
    ...
    if (Array.isArray(value)) {
        return value
            .slice(0, arrayLimit)
            .map((entry) => boundedStructuredValue(entry, stringLimit, arrayLimit, depth + 1));
    }
    if (typeof value !== 'object')
        return String(value ?? '');
    const result = {};
    for (const key of Object.keys(value).slice(0, 64)) {
        result[key] = boundedStructuredValue(value[key], stringLimit, arrayLimit, depth + 1);
    }
    return result;
}
function boundedStructuredArguments(value) {
...
function structuredArguments(value) {
    try {
        return boundedStructuredArguments(JSON.parse(value));
    }
    catch {
        return boundedToolValue(value, MAX_TOOL_PAYLOAD_CHARS);
    }
}
function initialArguments(value) {
    if (value === undefined)
        return undefined;
    try {
...
function isDelegationTool(name) {
    ...
    const gantryName = name.startsWith('mcp__gantry__')
        ...
        : name;
    return (gantryName === 'delegate_task' ||
        gantryName === 'AgentDelegation' ||
        gantryName === 'Agent' ||
        gantryName.startsWith('delegate_to_'));
}
export function createSseAccumulator(kind, captureContent) {
    ...
    const openAiChoices = new Map();
    const anthropicTextBlocks = new Map();
    let retainedToolArgumentChars = 0;
    let retainedToolCalls = 0;
    let retainedOpenAiContentChars = 0;
    let toolCallsReady = false;
    const captureError = (value) => {
        if (errorMessage || value === null || typeof value !== 'object')
            return;
        const err = value;
    ...
    const appendText = (text) => {
        if (!captureContent || completionCapped)
            return '';
    ...
    const appendChoiceText = (choice, text) => {
        if (!captureContent || choice.completionCapped)
            return;
    ...
    const appendChoiceRefusal = (choice, text) => {
        if (!captureContent)
            return;
        const available = MAX_COMPLETION_CHARS - retainedOpenAiContentChars;
        if (available <= 0)
    ...
    const toolIndex = (value, fallback) => typeof value === 'number' && Number.isSafeInteger(value) && value >= 0
        ? value
        : fallback;
    const getToolCall = (calls, index) => {
        const current = calls.get(index);
        if (current)
            return current;
        if (retainedToolCalls >= MAX_TOOL_CALLS)
            return undefined;
        const created = {
            order: index,
            argumentText: '',
    ...
    const appendIdentity = (current, fragment) => {
        if (typeof fragment !== 'string')
            return current;
        return boundedToolValue(`${current ?? ''}${fragment}`, MAX_TOOL_IDENTITY_CHARS);
    };
    const appendArguments = (call, fragment) => {
        if (typeof fragment !== 'string' || fragment.length === 0)
            return;
        call.sawArgumentDelta = true;
        if (call.argumentCapped)
            return;
        const available = MAX_AGGREGATE_TOOL_ARGUMENT_CHARS - retainedToolArgumentChars;
        if (available <= 0) {
    ...
    const completedToolCall = (call, choice) => {
        ...
        const mcpServer = parsedArguments &&
            typeof parsedArguments === 'object' &&
            typeof parsedArguments.serverName ===
                'string'
            ? parsedArguments.serverName
            : undefined;
        return {
            ...(call.id ? { id: call.id } : {}),
    ...
    const sortedOpenAiChoices = () => [...openAiChoices.values()].sort((left, right) => left.index - right.index);
    const completedToolCalls = () => kind === 'openai'
        ? sortedOpenAiChoices().flatMap((choice) => [...choice.pendingToolCalls.values()]
            .sort((left, right) => left.order - right.order)
            .map((call) => completedToolCall(call, choice)))
        : [...pendingToolCalls.values()]
            .sort((left, right) => left.order - right.order)
            .map((call) => completedToolCall(call));
    const anthropicAssistantMessage = (toolCalls) => {
        if (!captureContent)
            return undefined;
    ...
    const openAiAssistantMessage = (choice) => {
        if (!captureContent)
            return undefined;
        const toolCalls = [...choice.pendingToolCalls.values()]
            .sort((left, right) => left.order - right.order)
            .map((call) => completedToolCall(call, choice));
        if (!choice.completionText &&
            toolCalls.length === 0 &&
            !choice.refusalText) {
            return undefined;
        }
    ...
    const mergeUsage = (value) => {
        ...
        for (const [key, entry] of Object.entries(value)) {
            ...
            if (typeof entry === 'number') {
                ...
                const numeric = {};
                for (const [sub, subValue] of Object.entries(entry).slice(0, 8)) {
                    if (typeof subValue === 'number')
                        numeric[sub] = subValue;
                }
    ...
    const handleAnthropicEvent = (event) => {
        if (event.type === 'error') {
        ...
        if (event.type === 'content_block_start') {
            ...
            if (block?.type === 'text') {
                const captured = typeof block.text === 'string' ? appendText(block.text) : '';
        ...
        if (event.type === 'content_block_delta') {
            ...
            if (delta?.type === 'text_delta' && typeof delta.text === 'string') {
                ...
                const call = getToolCall(pendingToolCalls, index);
                if (call)
                    appendArguments(call, delta.partial_json);
            }
            return;
        }
    ...
    const handleOpenAiEvent = (event) => {
        ...
        const choices = Array.isArray(event.choices)
            ? event.choices.slice(0, MAX_TOOL_CALLS)
            : [];
        for (const [position, value] of choices.entries()) {
            ...
            if (!choiceState) {
                if (openAiChoices.size >= MAX_OPENAI_CHOICES)
                    continue;
                choiceState = {
                    index,
                    completionText: '',
            ...
            if (Array.isArray(delta?.tool_calls)) {
                for (const [toolPosition, toolValue] of delta.tool_calls.entries()) {
                    if (toolValue === null || typeof toolValue !== 'object')
                        continue;
                    const fragment = toolValue;
                    const call = getToolCall(choiceState.pendingToolCalls, toolIndex(fragment.index, toolPosition));
                    if (!call)
                        continue;
            ...
            if (typeof choice.finish_reason === 'string') {
                ...
                if (choiceState.finishReason === 'tool_calls')
                    toolCallsReady = true;
            }
        }
    };
    ...
    return {
        ...
        result: () => {
            ...
            if (kind === 'openai') {
                ...
                const finishReasons = choices.flatMap((choice) => choice.finishReason ? [choice.finishReason] : []);
