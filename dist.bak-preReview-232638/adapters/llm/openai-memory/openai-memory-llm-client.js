...
import { hasGatewayMemoryAccess, resolveGatewayMemoryInjection, } from './memory-gateway-injection.js';
 ...
 */
export function createOpenAiMemoryLlmClient() {
    return {
...
async function runOpenAiMemoryQuery(opts) {
    if (!hasGatewayMemoryAccess()) {
        throw new Error('OpenAI memory access is not configured (configure brokered model access)');
    }
...
async function runWithGantryGateway(opts) {
    ...
    try {
        opts.signal?.throwIfAborted();
        const { baseUrl, token } = readGatewayProjection(provider, gateway.injection.env);
        ...
        if (!response.ok) {
            ...
            throw new Error(`OpenAI memory query failed: ${response.status} ${response.statusText}${detail ? ` - ${detail}` : ''}`);
        }
        const parsed = (await response.json());
        opts.signal?.throwIfAborted();
        reportUsage(opts.onUsage, parsed.usage, provider.cacheSupport.prompt.usageFields.readTokens);
...
function readCompletionText(response) {
    let out = '';
...
function reportUsage(onUsage, usage, cacheReadField) {
    if (!onUsage || !usage)
        return;
...
function readNumberField(usage, path) {
    let cursor = usage;
...
function requireOpenAiCompatibleProvider(routeId) {
    ...
    if (!provider || !compatible) {
        throw new Error(`Memory model route ${routeId} is not an OpenAI-compatible model route.`);
    }
    return provider;
}
...
function readGatewayProjection(provider, env) {
    ...
    if (!baseUrl || !token) {
        throw new Error(`Setup required: configure ${provider.label} Model Access before running memory on ${provider.id} models.`);
    }
    if (!token.startsWith('gtw_')) {
        throw new Error(`Gantry Model Gateway projection for ${provider.label} memory must use a run-scoped gateway token.`);
    }
