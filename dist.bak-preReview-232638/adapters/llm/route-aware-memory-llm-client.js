...
export function createRouteAwareMemoryLlmClient(deps) {
    const batch = createRouteAwareBatchCapability(deps);
    return {
        isConfigured: () => deps.anthropic.isConfigured() || deps.openai.isConfigured(),
        query: async (opts) => clientForQuery(deps, opts).query(opts),
        ...(batch ? { batch } : {}),
    };
}
function createRouteAwareBatchCapability(deps) {
    if (!deps.anthropic.batch &&
        !deps.anthropicSingleRequest?.batch &&
        !deps.openai.batch) {
        return undefined;
    }
    return {
        preflightBatch: async (opts) => batchForScope(deps, opts).preflightBatch(opts),
        ...
        fetchBatchResults: async (opts) => batchForScope(deps, opts).fetchBatchResults(opts),
        findBatchByCorrelationId: async (opts) => batchForScope(deps, opts).findBatchByCorrelationId(opts),
    };
}
function batchForScope(deps, scope) {
    ...
    if (provider && !provider.batch) {
        throw new Error(`Memory model route ${provider.id} does not support provider batches.`);
    }
    ...
    if (!client.batch) {
        throw new Error(`Memory model route ${provider?.id ?? 'unknown'} has no chat batch transport.`);
    }
    return client.batch;
}
function clientForQuery(deps, opts) {
    ...
    const responseFamily = resolveResponseFamily(opts) ?? DEFAULT_MEMORY_RESPONSE_FAMILY;
    ...
    throw new Error(`Memory model "${opts.modelProfile?.alias ?? opts.model}" has unsupported response family "${responseFamily}". Memory supports the Anthropic and OpenAI families only.`);
}
function singleRequestRequested(opts) {
    return (opts.singleRequest ===
        true);
}
...
function isOpenAiCompatibleProvider(opts) {
...
function resolveProviderRouteId(opts) {
    if (opts.modelProfile?.modelRoute)
        return opts.modelProfile.modelRoute;
    const entry = findModelByRunnerModel(opts.model) ??
        findModelByRunnerModel(opts.modelProfile?.runnerModel);
    return entry?.modelRoute.id;
}
function resolveResponseFamily(opts) {
