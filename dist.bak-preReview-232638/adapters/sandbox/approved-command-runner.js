...
const PEM_PRIVATE_KEY_BLOCK_PATTERN = /-----BEGIN (?:RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----[\s\S]+?-----END (?:RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----/gi;
...
const PEM_ORPHAN_END_PATTERN = /^[\s\S]*?-----END (?:RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----/i;
const PEM_ORPHAN_BEGIN_PATTERN = /-----BEGIN (?:RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----[\s\S]*$/i;
const BASE64_KEY_BODY_RUN_PATTERN = /(?:^[A-Za-z0-9+/=]{40,}(?:\r?\n|$)){3,}/gm;
export function runApprovedSandboxCommand(input) {
    ...
    return new Promise((resolve, reject) => {
        ...
        child.on('close', (code, signal) => {
            ...
            if (timedOut) {
                settle(() => reject(new Error(`Command timed out${signal ? ` with signal ${signal}` : ''}.`)));
                return;
            }
            if (code === null && signal) {
                settle(() => reject(new Error(`Command timed out with signal ${signal}.`)));
                return;
            }
            ...
            const failureTail = sanitizeRetainedFailureTail(useStderr ? stderrTail : stdoutTail, useStderr ? stderrMaxBytes : stdoutMaxBytes, failureOutputTruncated);
            ...
            const callerRedacted = input.redactOutput
                ...
                : sanitized;
            settle(() => reject(new Error(`Command failed${signal ? ` with signal ${signal}` : ` with exit code ${code}`}${callerRedacted ? `: ${callerRedacted}` : ''}`)));
        });
    });
}
function retainBufferTail(retained, chunk, maxBytes) {
    const next = Buffer.concat([retained, chunk]);
    return next.length > maxBytes ? next.subarray(next.length - maxBytes) : next;
}
function decodeRetainedTail(retained, maxBytes) {
    const boundary = Math.max(0, retained.length - maxBytes);
    return retained.subarray(boundary).toString('utf8');
}
function sanitizeRetainedFailureTail(retained, maxBytes, truncated) {
    const boundary = Math.max(0, retained.length - maxBytes);
    ...
    if (truncated) {
        ...
        const startsAtLineBoundary = boundary > 0 && retained[boundary - 1] === 0x0a;
        ...
        while (start < lines.length &&
            (/^\s*$/.test(lines[start]) ||
                ...
                lines[start].trimStart().startsWith('- '))) {
            start += 1;
        }
...
function sanitizeFailureDiagnostic(value) {
    ...
    if (/^(?:authentication failed|no credentials configured|credentials could not be found|token has expired|session closed unexpectedly)$/i.test(trimmed)) {
        return trimmed;
    }
    let pemSanitized = value.replace(PEM_PRIVATE_KEY_BLOCK_PATTERN, '[REDACTED_SECRET]');
    if (/-----END (?:RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----/i.test(pemSanitized)) {
        pemSanitized = pemSanitized.replace(PEM_ORPHAN_END_PATTERN, '[REDACTED_SECRET]');
    }
    ...
    const urlSanitized = pemSanitized.replace(/[A-Za-z][A-Za-z0-9+.-]*:\/\/[^\s<>"']+/g, (rawUrl) => {
        try {
            ...
            for (const name of [...url.searchParams.keys()]) {
                ...
                if ([normalizedName, name.toLowerCase()].some((candidate) => /(?:^|[^a-z0-9])(?:(?:api|access|private)[_-]?keys?|(?:access|auth|refresh|session)[_-]?tokens?|client[_-]?secrets?|auth(?:orization)?|cookies?|credentials?|keys?|passphrases?|passwords?|passwd|sigs?|signatures?|tokens?|secrets?)(?:$|[^a-z0-9])/i.test(candidate))) {
    ...
    for (const pattern of [
        ...
        labelSanitized = labelSanitized.replace(pattern, '$1[REDACTED_POTENTIALLY_SENSITIVE]');
    }
...
const CREDENTIAL_LABEL_LINE_PATTERN = /^(\s*)["']?[A-Za-z0-9_. \t-]*(?:tokens?|secrets?|passwords?|passphrases?|credentials?|auth(?:orization)?|api[\s_-]?keys?|access[\s_-]?keys?|private[\s_-]?keys?|cookies?|bearer)[A-Za-z0-9_. \t-]*["']?\s*[:=]\s*(.*)$/i;
function redactLabelledContinuationBlocks(value) {
