...
export function createRunnerSandboxProvider(settings) {
...
export class DirectRunnerSandboxProvider {
    resourceLimits;
    id = 'direct';
    enforcing = false;
    ...
    start(input) {
        const prepared = applyResourceLimits(input.command, input.args, input.resourceLimits ?? this.resourceLimits);
...
class SandboxRuntimeRunnerSandboxProvider {
    resourceLimits;
    id = 'sandbox_runtime';
    enforcing = true;
    constructor(resourceLimits) {
    ...
    start(input) {
        ...
        if (input.sandboxProfile.network === 'required' && !input.egressProxyUrl) {
            throw new Error('Networked sandbox runs require the Gantry egress proxy.');
        }
        fs.mkdirSync(path.dirname(input.configFilePath), {
            recursive: true,
            mode: 0o700,
        });
        fs.writeFileSync(input.configFilePath, JSON.stringify(buildSandboxRuntimeConfig(input), null, 2), { mode: 0o600 });
        const runner = resolveSandboxRuntimeRunner();
        const limited = applyResourceLimits(input.command, input.args, input.resourceLimits ?? this.resourceLimits);
        const child = spawn(runner.command, [...runner.args, input.configFilePath, limited.command, ...limited.args], {
            cwd: input.cwd,
            stdio: ['pipe', 'pipe', 'pipe'],
            env: input.env,
            detached: true,
...
export function buildSandboxRuntimeWarmTemplate() {
...
function sandboxRuntimeWarmTemplateStatus() {
...
function applyResourceLimits(command, args, limits) {
    ...
    if (process.platform === 'win32') {
        throw new Error('runtime.sandbox.resource_limits are not supported on Windows direct mode.');
    }
    const script = [
...
function buildSandboxRuntimeConfig(input) {
    ...
    return {
        ...
        filesystem: {
            denyRead,
            allowRead: denyRead.length > 0
                ? uniquePaths(defaultReadAllowPaths(input, denyRead))
                : [],
            allowWrite: input.sandboxProfile.filesystem === 'workspace_write'
                ? uniquePaths([
                    input.workspaceRoot,
...
function siblingReadDenyPaths(input) {
    return uniquePaths([input.workspaceRoot, ...input.runtimeReadPaths]
        .map((item) => {
...
function isSafeSiblingDenyRoot(root) {
...
function defaultToolTempWriteAllowPaths() {
...
function installProcessGroupKill(child) {
...
function egressProxyConfig(proxyUrl) {
    if (!proxyUrl) {
...
function defaultReadDenyPaths(input) {
    const home = os.homedir();
    const paths = [
...
function defaultWriteDenyPaths(input) {
    const home = os.homedir();
    return [
        '/tmp/claude',
        '/private/tmp/claude',
        path.join(input.workspaceRoot, '.env'),
        path.join(input.cwd, '.env'),
        ...input.runtimeReadPaths.map((runtimePath) => path.join(runtimePath, '.env')),
        ...(home && home !== '/'
            ? [
...
function homeSecretReadDenyPaths(home) {
    if (!home || home === '/')
        return [];
    return buildSandboxRuntimeWarmTemplate().filesystem.homeSecretDenySuffixes.map((item) => path.join(home, item));
}
function defaultReadAllowPaths(input, denyReadPaths) {
    const paths = [
        process.execPath,
    ...
    try {
        ...
        paths.push(realNode, path.dirname(realNode), path.dirname(path.dirname(realNode)));
    }
    catch {
...
function claudeRuntimeGeneratedReadAllowPaths(runtimeReadPaths) {
...
function runtimeReadRootDirectoryAllowPaths(runtimeReadPaths) {
    if (process.platform !== 'darwin')
        return [];
    return runtimeReadPaths.flatMap((runtimePath) => exactMacosPathReadAllowPattern(runtimePath));
}
function claudeCodeExecutableReadAllowPaths(envPath) {
    if (!envPath)
        return [];
    const home = os.homedir();
    const allowedRoots = [
    ...
    for (const entry of envPath.split(path.delimiter)) {
        ...
        try {
            ...
            if (!allowedRoots.some((root) => pathContainsOrEquals(path.resolve(root), path.resolve(resolved)))) {
                continue;
            }
            return uniquePaths([
                candidate,
...
function executableReadAllowPaths(command) {
...
function workspaceCwdReadAllowPaths(input) {
    if (process.platform !== 'darwin')
        return [];
    if (!pathContainsOrEquals(input.workspaceRoot, input.cwd))
        return [];
    return exactMacosPathReadAllowPattern(input.cwd);
}
function exactMacosPathReadAllowPattern(targetPath) {
...
function expandReadAllowRoot(root, protectedReadPaths) {
    const resolvedRoot = path.resolve(root);
    const protectedPaths = protectedReadPaths.map((item) => path.resolve(item));
    if (!protectedPaths.some((protectedPath) => pathContainsOrEquals(resolvedRoot, protectedPath))) {
        return [root];
    }
    let entries;
    try {
    ...
    for (const entry of entries) {
        ...
        const matchingProtectedPath = protectedPaths.find((protectedPath) => pathContainsOrEquals(resolvedChild, protectedPath));
...
function resolveSandboxRuntimeRunner() {
    const currentPath = fileURLToPath(import.meta.url);
    const extension = path.extname(currentPath);
    const runnerPath = path.join(path.dirname(currentPath), `sandbox-runtime-runner${extension}`);
    if (extension === '.js') {
