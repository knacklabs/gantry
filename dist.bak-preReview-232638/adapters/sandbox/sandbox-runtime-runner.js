...
export const allowSandboxRuntimeDestination = async () => true;
export function sandboxRuntimeAskCallback(config) {
...
async function main() {
    ...
    const config = SandboxRuntimeConfigSchema.parse(JSON.parse(fs.readFileSync(configPath, 'utf8')));
    await SandboxManager.initialize(config, sandboxRuntimeAskCallback(config));
    const sandboxedCommand = await SandboxManager.wrapWithSandbox([command, ...args].map(shellQuote).join(' '));
...
if (process.argv[1] &&
    ...
    main().catch((error) => {
        console.error(`Sandbox runtime failed: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    });
}
