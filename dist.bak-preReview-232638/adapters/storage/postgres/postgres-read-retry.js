...
export function isRetryablePostgresReadError(error) {
    for (const item of errorChain(error)) {
        ...
        if (typeof message === 'string' &&
            RETRYABLE_POSTGRES_MESSAGE_PATTERNS.some((pattern) => pattern.test(message))) {
            return true;
        }
    }
    return false;
}
export async function retryPostgresRead(operationName, operation, options = {}) {
    ...
    for (let attempt = 0;; attempt += 1) {
        try {
            ...
            logger.warn({ err, operationName, nextAttempt: attempt + 2 }, 'Retrying transient Postgres read failure');
