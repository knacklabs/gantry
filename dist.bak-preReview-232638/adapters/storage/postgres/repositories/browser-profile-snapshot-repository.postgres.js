...
export class PostgresBrowserProfileSnapshotRepository {
    db;
    constructor(db) {
        this.db = db;
    }
    async getBrowserProfileSnapshot(profileName) {
    ...
    async upsertBrowserProfileSnapshot(input) {
        ...
        if (!current) {
            ...
            throw new Error(`Browser profile snapshot upsert for ${input.profileName} was rejected but no row exists`);
        }
