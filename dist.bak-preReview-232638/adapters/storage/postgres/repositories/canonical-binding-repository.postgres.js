...
import { CANONICAL_APP_ID, agentIdForFolder, conversationIdForJid, json, parseJson, PostgresCanonicalGraphRepository, } from './canonical-graph-repository.postgres.js';
const CONVERSATION_ROUTE_BINDING_ID_PREFIX = 'conversation-route:';
export function conversationRouteKeyFromBindingRow(row) {
...
function routeBindingId(jid) {
    return `${CONVERSATION_ROUTE_BINDING_ID_PREFIX}${jid}`;
}
export function normalizeRouteAgentId(agentId) {
    return agentIdForFolder(folderForAgentId(agentId) ?? agentId);
}
function routeMemorySubject(conversationId, group) {
    return {
        kind: 'conversation',
...
export class PostgresCanonicalBindingRepository {
    ...
    async saveConversationRoute(jid, group) {
        const parsedRouteKey = parseAgentThreadQueueKey(jid);
        const { chatJid } = parsedRouteKey;
        const requestedProviderAccountId = group.providerAccountId?.trim() || undefined;
        const resolvedAgentId = normalizeRouteAgentId(group.folder);
        if (parsedRouteKey.agentId !== undefined &&
            normalizeRouteAgentId(parsedRouteKey.agentId) !== resolvedAgentId) {
            throw new Error(`Conversation route ${jid} agent qualifier ${parsedRouteKey.agentId} does not match resolved agent ${resolvedAgentId}`);
        }
        if (requestedProviderAccountId !== undefined &&
            parsedRouteKey.providerAccountId !== undefined &&
            parsedRouteKey.providerAccountId !== requestedProviderAccountId) {
            throw new Error(`Conversation route ${jid} provider account qualifier ${parsedRouteKey.providerAccountId} does not match requested provider account ${requestedProviderAccountId}`);
        }
        const preEnsureProviderAccountId = requestedProviderAccountId ?? parsedRouteKey.providerAccountId;
        await this.db.transaction(async (tx) => {
            const resolvedProviderAccountId = preEnsureProviderAccountId ??
                (group.conversationId
                    ? (await this.graph.getConversationInstallationId(group.conversationId, tx))?.trim()
                    : undefined);
            const conversationId = await this.graph.ensureConversation(chatJid, {
                name: group.name,
                agentFolder: group.folder,
                existingConversationId: group.conversationId,
                providerAccountId: resolvedProviderAccountId,
                isGroup: group.conversationKind === 'dm'
                    ? false
                    : group.conversationKind === 'channel'
                        ? true
                        : group.requiresTrigger !== false,
            }, tx);
            const agentId = await this.graph.ensureAgent(group.folder, group.name, tx);
            const installationProviderAccountId = (await this.graph.getConversationInstallationId(conversationId, tx))?.trim();
            const providerAccountId = resolvedProviderAccountId ?? installationProviderAccountId;
            if (!providerAccountId) {
                throw new Error(`Cannot persist conversation route ${jid} without providerAccountId`);
            }
            if (installationProviderAccountId &&
                installationProviderAccountId !== providerAccountId) {
                throw new Error(`Conversation route ${jid} resolved provider account ${installationProviderAccountId}, expected ${providerAccountId}`);
            }
    ...
    async listConversationRoutes() {
        ...
        return this.db
            ...
            .where(and(eq(b.appId, CANONICAL_APP_ID), like(b.id, `${CONVERSATION_ROUTE_BINDING_ID_PREFIX}%`), eq(b.status, 'active'), eq(pa.status, 'active'), isNull(b.threadId)))
            .orderBy(asc(b.createdAt));
    }
}
export function bindingRowToGroup(row) {
    if (!row.id.startsWith(CONVERSATION_ROUTE_BINDING_ID_PREFIX))
        return undefined;
    if (row.status !== 'active' || row.threadId)
        return undefined;
    const routeSubject = parseJson(row.memorySubjectJson, {});
    ...
    const expectedCanonicalConversationId = conversationIdForJid(parsedRouteKey.chatJid, providerAccountId);
    ...
    const folder = folderForAgentId(normalizedRowAgentId) ?? row.agentId;
    const agentConfig = routeSubject.route?.agentConfig;
    const conversationKind = row.conversationKind === 'direct' || row.conversationKind === 'dm'
        ? 'dm'
        : 'channel';
    return {
        jid,
