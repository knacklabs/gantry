...
import { fallbackProviderAccountId, normalizeProviderId, providerIdForJid as resolveProviderIdForJid, } from '../../../../channels/provider-registry.js';
...
export function providerIdForJid(jid) {
    return resolveProviderIdForJid(jid);
}
export function externalConversationIdForJid(jid) {
    const idx = jid.indexOf(':');
    return idx > 0 ? jid.slice(idx + 1) : jid;
}
export function conversationIdForJid(jid, providerAccountId) {
...
export function configVersionIdForAgent(agentId) {
    return `config:${agentId}:1`;
}
export function threadIdFor(chatJid, threadId, providerAccountId) {
...
export function canonicalProviderThreadForIds(input) {
    if (!input.conversationId || !input.threadId)
        return null;
    const conversationPrefix = 'conversation:';
    if (!input.conversationId.startsWith(conversationPrefix))
        return null;
    ...
    if (!input.threadId.startsWith(threadPrefix))
        return null;
...
export function jsonb(value) {
    ...
    try {
        ...
        throw new Error(`Invalid JSON string passed to jsonb column writer: ${err instanceof Error ? err.message : String(err)}`, { cause: err });
    }
}
...
export class PostgresCanonicalGraphRepository {
    ...
    async ensureApp(executor = this.db) {
        await executor
    ...
    async ensureAgent(folder, name = folder, executor = this.db) {
    ...
    async ensureAgentExists(folder, name = folder, executor = this.db) {
        await this.ensureApp(executor);
        const agentId = agentIdForFolder(folder);
        const configVersionId = configVersionIdForAgent(agentId);
        await executor
    ...
    async ensureConversation(jid, input = {}, executor = this.db) {
        const providerId = normalizeProviderId(input.channel || providerIdForJid(jid)) || 'app';
        const providerAccountId = input.providerAccountId ??
            fallbackProviderAccountId(CANONICAL_APP_ID, providerId);
        const canonicalConversationId = conversationIdForJid(jid, input.providerAccountId);
        ...
        if (existingConversationId &&
            ...
            if (existing) {
                const ref = parseJson(existing.externalRefJson, {});
                ...
                const matchesRoute = existing.appId === CANONICAL_APP_ID &&
                    existing.providerAccountId === providerAccountId &&
                    externalIds.length > 0 &&
                    externalIds.every(([value, expected]) => value === expected || value === jid) &&
                ...
                if (!matchesRoute) {
                    throw new Error(`Existing conversation ${existingConversationId} does not match route ${jid} for app ${CANONICAL_APP_ID} and provider account ${providerAccountId}`);
                }
        ...
        await executor
            ...
            .onConflictDoNothing();
        const providerAccountAgentId = await this.ensureAgent(input.agentFolder || providerId, input.agentFolder || providerId, executor);
        await executor
    ...
    async ensureThread(chatJid, threadId, executor = this.db, input = {}) {
        const canonicalThreadId = threadIdFor(chatJid, threadId, input.providerAccountId);
        if (!canonicalThreadId)
            return null;
        const conversationId = await this.ensureConversation(chatJid, { channel: input.channel, providerAccountId: input.providerAccountId }, executor);
        await executor
    ...
    async ensureParticipant(input, executor = this.db) {
    ...
    async listChats() {
        ...
        return rows.map((row) => {
            const ref = parseJson(row.externalRefJson, {});
            return {
                jid: ref.jid || row.id,
    ...
    async getConversationInstallationId(conversationId, executor = this.db) {
        const rows = await executor
            .select({
    ...
    async findConversationIdForJid(jid, executor = this.db) {
        const conversations = pgSchema.conversationsPostgres;
        const rows = await executor
            .select({ id: conversations.id })
            .from(conversations)
            .where(and(eq(conversations.appId, CANONICAL_APP_ID), eq(sql `${conversations.externalRefJson}::jsonb->>'jid'`, jid)))
            .limit(1);
        return rows[0]?.id;
    }
