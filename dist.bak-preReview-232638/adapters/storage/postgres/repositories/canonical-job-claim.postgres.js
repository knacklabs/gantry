...
import { parseJson, } from './canonical-graph-repository.postgres.js';
import { claimRunLeaseInTx, isRunLeaseClaimConflict, } from './worker-coordination-lease.postgres.js';
/**
...
export async function claimDueCanonicalJobRunStart(input) {
    try {
        return await input.db.transaction(async (tx) => {
            ...
            if (!job)
                return null;
            const target = parseJson(job.targetJson, {});
            ...
            if (existingRun && existingRun.jobId !== input.jobId)
                return null;
            if (existingRun) {
                if (!['running', 'timeout'].includes(existingRun.status))
                    return null;
                const expiredLeaseRows = await tx
                    .select({ runId: pgSchema.runLeasesPostgres.runId })
                    .from(pgSchema.runLeasesPostgres)
                    .where(and(eq(pgSchema.runLeasesPostgres.runId, input.run.run_id), eq(pgSchema.runLeasesPostgres.status, 'expired')))
                    .limit(1);
            ...
            if (!existingRun && !inserted)
                return null;
            const startedAtMs = parseIso(input.run.started_at)?.getTime() ?? Date.now();
            const leaseExpiresAtMs = parseIso(input.leaseExpiresAt)?.getTime() ?? startedAtMs;
        ...
        if (isRunLeaseClaimConflict(err))
            return null;
        throw err;
    }
}
