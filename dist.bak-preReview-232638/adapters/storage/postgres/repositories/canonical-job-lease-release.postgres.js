 ...
 */
export async function releaseStaleCanonicalJobLeases(db, nowIso) {
    return db.transaction(async (tx) => {
      ...
      WHERE rl.run_id = ${jobs.leaseRunId}
        ...
        const stalePredicate = and(eq(jobs.status, 'running'), isNotNull(jobs.leaseExpiresAt), noLiveRunLease);
        ...
        const releasedJobs = await tx
            ...
            .where(and(inArray(jobs.id, staleJobs.map((job) => job.id)), stalePredicate))
            .returning({ id: jobs.id });
        const releasedJobIds = new Set(releasedJobs.map((job) => job.id));
        const releasedStaleJobs = staleJobs.filter((job) => releasedJobIds.has(job.id));
        const runIds = releasedStaleJobs
            .map((job) => job.leaseRunId)
            .filter((runId) => Boolean(runId));
        const timedOutRunIds = new Set();
        if (runIds.length > 0) {
            ...
            await tx
                ...
                .set({ status: 'expired' })
                .where(and(inArray(pgSchema.runLeasesPostgres.runId, runIds), eq(pgSchema.runLeasesPostgres.status, 'active')));
        }
