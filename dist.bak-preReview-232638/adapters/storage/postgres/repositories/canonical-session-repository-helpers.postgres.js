...
export function makeOwnedAgentSessionScopeKey(agentId, routeScopeKey, appId = CANONICAL_APP_ID) {
...
export function makeOwnedAgentSessionId(agentId, routeScopeKey, appId = CANONICAL_APP_ID) {
    return `agent-session:${makeOwnedAgentSessionScopeKey(agentId, routeScopeKey, appId)}`;
}
function isScopedSessionKey(scopeKey) {
    return /::(?:conversation|user|thread):/.test(scopeKey);
}
export function buildCurrentScopeResetMatcher(scopeKey) {
...
export function conversationKindInput(kind) {
    if (kind === 'channel')
        return { isGroup: true };
    if (kind === 'dm')
...
function compactionDeltaReplay(metadata) {
...
export async function isProviderSessionMaintenanceLocked(executor, id) {
    ...
    const [locked] = await executor
        ...
        .where(and(eq(pgSchema.providerSessionsPostgres.id, id), eq(pgSchema.providerSessionsPostgres.status, 'maintenance_compact')))
        .for('update')
        .limit(1);
    return Boolean(locked);
}
export async function releaseStaleProviderSessionMaintenanceLocks(executor, input) {
    await executor
        .update(pgSchema.providerSessionsPostgres)
        .set({ status: 'active', updatedAt: sql `now()` })
        .where(and(input.providerSessionId
        ? eq(pgSchema.providerSessionsPostgres.id, input.providerSessionId)
        : undefined, input.agentSessionId
        ? eq(pgSchema.providerSessionsPostgres.agentSessionId, input.agentSessionId)
        : undefined, input.provider
        ? eq(pgSchema.providerSessionsPostgres.provider, input.provider)
        : undefined, eq(pgSchema.providerSessionsPostgres.status, 'maintenance_compact'), sql `${pgSchema.providerSessionsPostgres.updatedAt} < ${MAINTENANCE_COMPACT_STALE_LOCK_SQL}`, sql `not exists (
...
export async function markLatestProviderSessionMaintenance(executor, input) {
    ...
    const result = await executor
        ...
        .where(and(eq(pgSchema.providerSessionsPostgres.id, input.providerSessionId), eq(pgSchema.providerSessionsPostgres.agentSessionId, input.agentSessionId), eq(pgSchema.providerSessionsPostgres.provider, input.provider), eq(pgSchema.providerSessionsPostgres.externalSessionId, input.externalSessionId), eq(pgSchema.providerSessionsPostgres.status, 'active'), sql `EXISTS (
...
export async function finishProviderSessionMaintenance(executor, input) {
    const metadataPatch = input.status === 'ready'
        ? jsonb({
    ...
    const result = await executor
        ...
        .where(and(eq(pgSchema.providerSessionsPostgres.id, input.providerSessionId), eq(pgSchema.providerSessionsPostgres.agentSessionId, input.agentSessionId), eq(pgSchema.providerSessionsPostgres.provider, input.provider), eq(pgSchema.providerSessionsPostgres.externalSessionId, input.externalSessionId), eq(pgSchema.providerSessionsPostgres.status, 'maintenance_compact')));
    if (Number(result.rowCount ?? 0) === 0 || input.status !== 'active')
        return;
    await executor
...
export async function promoteReadyProviderSession(executor, input) {
    const result = await executor
        .update(pgSchema.providerSessionsPostgres)
        .set({
        status: 'active',
        ...
        .where(and(eq(pgSchema.providerSessionsPostgres.id, input.providerSessionId), eq(pgSchema.providerSessionsPostgres.agentSessionId, input.agentSessionId), eq(pgSchema.providerSessionsPostgres.provider, input.provider), eq(pgSchema.providerSessionsPostgres.externalSessionId, input.externalSessionId), eq(pgSchema.providerSessionsPostgres.status, 'ready'), sql `EXISTS (
          SELECT 1 FROM ${pgSchema.agentSessionsPostgres} agent_session
          WHERE agent_session.id = ${input.agentSessionId}
            AND agent_session.latest_provider_session_id = ${input.providerSessionId}
        )`));
    ...
    await executor
        ...
        .where(and(eq(pgSchema.agentSessionsPostgres.id, input.agentSessionId), eq(pgSchema.agentSessionsPostgres.latestProviderSessionId, input.providerSessionId)));
    return true;
}
export async function markProviderSessionDeltaReplay(executor, input) {
    await executor
        .update(pgSchema.providerSessionsPostgres)
        .set({
        metadataJson: sql `${pgSchema.providerSessionsPostgres.metadataJson} || ${jsonb({
        ...
        .where(and(eq(pgSchema.providerSessionsPostgres.id, input.providerSessionId), eq(pgSchema.providerSessionsPostgres.agentSessionId, input.agentSessionId), eq(pgSchema.providerSessionsPostgres.provider, input.provider), eq(pgSchema.providerSessionsPostgres.externalSessionId, input.externalSessionId)));
}
export async function promoteLatestReadyProviderSession(executor, input) {
    const [ready] = await executor
        ...
        .from(pgSchema.providerSessionsPostgres)
        .where(and(eq(pgSchema.providerSessionsPostgres.agentSessionId, input.agentSessionId), eq(pgSchema.providerSessionsPostgres.provider, input.provider), eq(pgSchema.providerSessionsPostgres.status, 'ready'), sql `EXISTS (
          SELECT 1 FROM ${pgSchema.agentSessionsPostgres} agent_session
          WHERE agent_session.id = ${input.agentSessionId}
...
export async function expireProviderSession(executor, input) {
    ...
    await executor
        ...
        .where(and(eq(pgSchema.providerSessionsPostgres.id, providerSessionId), eq(pgSchema.providerSessionsPostgres.agentSessionId, agentSessionId), eq(pgSchema.providerSessionsPostgres.provider, provider), eq(pgSchema.providerSessionsPostgres.externalSessionId, externalSessionId)));
}
export async function findControlSessionForChatJid(executor, appId, chatJid) {
    const [session] = await executor
        ...
        .where(and(eq(pgSchema.controlHttpSessionsPostgres.appId, appId), sql `${pgSchema.controlHttpSessionsPostgres.externalRefJson}->>'chatJid' = ${chatJid}`))
        .limit(1);
    return session;
}
