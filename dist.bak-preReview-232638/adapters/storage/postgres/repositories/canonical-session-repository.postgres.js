...
import { CANONICAL_APP_ID, agentIdForFolder, conversationIdForJid, json, jsonb, PostgresCanonicalGraphRepository, threadIdFor, } from './canonical-graph-repository.postgres.js';
import { buildCurrentScopeResetMatcher, conversationKindInput, escapeLikePattern, expireProviderSession, findControlSessionForChatJid, finishProviderSessionMaintenance, isProviderSessionMaintenanceLocked, markLatestProviderSessionMaintenance, markProviderSessionDeltaReplay, makeOwnedAgentSessionId, makeOwnedAgentSessionScopeKey, promoteLatestReadyProviderSession, providerSessionContext, RESUMABLE_PROVIDER_SESSION_STATUSES, releaseStaleProviderSessionMaintenanceLocks, resolveSessionAppId, } from './canonical-session-repository-helpers.postgres.js';
...
export class PostgresCanonicalSessionRepository {
    ...
    async getAgentTurnContext(input) {
        ...
        const [providerSession] = await this.db
            ...
            .where(and(eq(pgSchema.providerSessionsPostgres.agentSessionId, ensured.agentSessionId), eq(pgSchema.providerSessionsPostgres.provider, executionProviderId), inArray(pgSchema.providerSessionsPostgres.status, RESUMABLE_PROVIDER_SESSION_STATUSES)))
    ...
    async ensureAgentSession(input) {
        const { workspaceFolder: folder, appId, chatJid, providerAccountId, threadId, scopeKey, conversationKind, memoryUserId, jobId, } = input;
        const normalizedJobId = jobId?.trim() || null;
        const resolvedMemoryUserId = memoryUserId?.trim() || null;
        const sessionUserId = conversationKind === 'dm' && resolvedMemoryUserId
            ? resolvedMemoryUserId
            : scopeKey;
        const shouldUpdateMemoryUserId = conversationKind === 'dm' && Boolean(resolvedMemoryUserId);
        const ensured = await this.db.transaction(async (tx) => {
            ...
            const agentSessionId = makeOwnedAgentSessionId(route.agentId, scopeKey, appId);
            await tx
    ...
    async resolveSessionRoute(input, executor) {
        if (input.appId === CANONICAL_APP_ID) {
            const conversationInput = {
                ...conversationKindInput(input.conversationKind),
                providerAccountId: input.providerAccountId,
            };
            const conversationId = await this.graph.ensureConversation(input.chatJid, conversationInput, executor);
            const canonicalThreadId = threadIdFor(input.chatJid, input.threadId, input.providerAccountId);
        ...
        const controlSession = await findControlSessionForChatJid(executor, input.appId, input.chatJid);
        if (!controlSession) {
            throw new Error(`App session not found for conversation: ${input.chatJid}`);
        }
    ...
    async ensureAppThread(input, executor) {
    ...
    async resolveBoundAgentId(input, executor) {
        const selectedBindingAgentId = await this.findBoundAgentId({ ...input, agentId: agentIdForFolder(input.folder) }, executor);
        if (selectedBindingAgentId)
            return selectedBindingAgentId;
        return this.graph.ensureAgent(input.folder, input.folder, executor);
    }
    async findBoundAgentId(input, executor) {
        const b = pgSchema.conversationInstallsPostgres;
        if (input.threadId) {
            const [threadBinding] = await executor
                .select({ agentId: b.agentId })
                .from(b)
                .where(and(eq(b.appId, input.appId), eq(b.conversationId, input.conversationId), eq(b.threadId, input.threadId), input.agentId ? eq(b.agentId, input.agentId) : undefined, eq(b.status, 'active')))
                .limit(1);
            if (threadBinding?.agentId)
                return threadBinding.agentId;
        }
        const [conversationBinding] = await executor
            .select({ agentId: b.agentId })
            .from(b)
            .where(and(eq(b.appId, input.appId), eq(b.conversationId, input.conversationId), isNull(b.threadId), input.agentId ? eq(b.agentId, input.agentId) : undefined, eq(b.status, 'active')))
            .limit(1);
    ...
    async setProviderSession(input) {
        ...
        assertSafeExecutionProviderId(input.executionProviderId);
        const { workspaceFolder: folder, scopeKey, executionProviderId, sessionId, chatJid, providerAccountId, threadId, conversationKind, memoryUserId, jobId, expectedAgentSessionId, expectedAgentSessionResetAt, accessFingerprint, } = input;
        ...
        return this.db.transaction(async (tx) => {
            if (appId !== CANONICAL_APP_ID && !chatJid) {
                throw new Error('App-scoped provider session persistence requires a conversation JID');
            }
            const route = chatJid
            ...
            const [agentSession] = await tx
                ...
                latestProviderSessionId: pgSchema.agentSessionsPostgres.latestProviderSessionId,
            })
                .from(pgSchema.agentSessionsPostgres)
                .where(eq(pgSchema.agentSessionsPostgres.id, agentSessionId))
                .for('update')
                .limit(1);
            if (expectedAgentSessionId !== undefined &&
                expectedAgentSessionId !== agentSessionId) {
                return false;
            }
            if (expectedAgentSessionResetAt !== undefined &&
                (agentSession?.resetAt ?? null) !== expectedAgentSessionResetAt) {
                return false;
            }
            if (agentSession?.latestProviderSessionId &&
                (await isProviderSessionMaintenanceLocked(tx, agentSession.latestProviderSessionId)))
                return false;
            await tx
            ...
            if (!existing ||
                ...
                throw new Error(`Provider session id is already owned by another session: ${sessionId}`);
            }
            await tx
                ...
                .where(and(eq(pgSchema.providerSessionsPostgres.id, sessionId), eq(pgSchema.providerSessionsPostgres.appId, appId), eq(pgSchema.providerSessionsPostgres.provider, executionProviderId), eq(pgSchema.providerSessionsPostgres.agentSessionId, agentSessionId), eq(pgSchema.providerSessionsPostgres.externalSessionId, sessionId)));
            await tx
                .delete(pgSchema.providerSessionsPostgres)
                .where(and(eq(pgSchema.providerSessionsPostgres.appId, appId), eq(pgSchema.providerSessionsPostgres.agentSessionId, agentSessionId), eq(pgSchema.providerSessionsPostgres.provider, executionProviderId), ne(pgSchema.providerSessionsPostgres.id, sessionId)));
            await tx
    ...
    async finishProviderSessionMaintenance(input) {
        await this.db.transaction((tx) => finishProviderSessionMaintenance(tx, input));
    }
    async resetScope(input) {
        ...
        if (matcher.currentScopeDescendantLike) {
            predicates.push(sql `${pgSchema.agentSessionsPostgres.scopeKey} LIKE ${matcher.currentScopeDescendantLike} ESCAPE '\\'`);
        }
        const appId = resolveSessionAppId({
            appId: input.appId,
            chatJid: input.chatJid,
        ...
        await this.db.transaction(async (tx) => {
            ...
            let ownerAgentId = explicitAgentId || undefined;
            if (!ownerAgentId && input.chatJid) {
                if (appId === CANONICAL_APP_ID) {
                    ...
                    ownerAgentId = (await findControlSessionForChatJid(tx, appId, input.chatJid))?.agentId;
                }
            }
            const rows = await tx
                .select({ id: pgSchema.agentSessionsPostgres.id })
                .from(pgSchema.agentSessionsPostgres)
                .where(and(eq(pgSchema.agentSessionsPostgres.appId, appId), or(...predicates), ...(ownerAgentId
            ...
            await tx
                .delete(pgSchema.providerSessionsPostgres)
                .where(inArray(pgSchema.providerSessionsPostgres.agentSessionId, sessionIds));
            await tx
                .update(pgSchema.agentSessionsPostgres)
                .set({
    ...
    async deleteWorkspaceFolder(agentFolder) {
        const escapedAgentFolder = escapeLikePattern(agentFolder);
        await this.db
            .delete(pgSchema.agentSessionsPostgres)
            .where(and(eq(pgSchema.agentSessionsPostgres.appId, CANONICAL_APP_ID), or(eq(pgSchema.agentSessionsPostgres.scopeKey, agentFolder), sql `${pgSchema.agentSessionsPostgres.scopeKey} LIKE ${`${escapedAgentFolder}::%`} ESCAPE '\\'`)));
    }
}
