...
function agentIdForFolder(folder) {
    return `agent:${folder || 'default'}`;
}
function controlInstallationId(appId) {
    return `control:${appId}`;
}
function controlConversationId(appId, externalConversationId) {
    return `control:${appId}:conversation:${externalConversationId}`;
}
export async function ensureControlGraph(db, input) {
    ...
    const conversationId = controlConversationId(appId, input.externalConversationId);
    await db
