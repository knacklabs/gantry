...
export async function getOldestWaitingLiveAdmission(db, input) {
    if (input.conversationJids.length === 0)
        return null;
    ...
    const routed = sql.join(input.conversationJids.map((jid) => sql `(${jid}::text, ${`conversation:${jid}`}::text)`), sql `, `);
