...
import { getProvider, normalizeProviderId, providerJidPrefix, } from '../../../../channels/provider-registry.js';
...
export async function resolveOutboundDeliveryDestination(db, input) {
    if (input.threadId) {
        const rows = await db
            .select({
            conversationId: pgSchema.conversationsPostgres.id,
            providerAccountId: pgSchema.conversationsPostgres.providerAccountId,
            conversationExternalRefJson: pgSchema.conversationsPostgres.externalRefJson,
            providerId: pgSchema.providerAccountsPostgres.providerId,
            threadId: pgSchema.conversationThreadsPostgres.id,
            threadExternalRefJson: pgSchema.conversationThreadsPostgres.externalRefJson,
        })
            .from(pgSchema.conversationsPostgres)
            .innerJoin(pgSchema.providerAccountsPostgres, and(eq(pgSchema.providerAccountsPostgres.id, pgSchema.conversationsPostgres.providerAccountId), eq(pgSchema.providerAccountsPostgres.appId, pgSchema.conversationsPostgres.appId)))
            .innerJoin(pgSchema.conversationThreadsPostgres, and(eq(pgSchema.conversationThreadsPostgres.id, input.threadId), eq(pgSchema.conversationThreadsPostgres.appId, input.appId), eq(pgSchema.conversationThreadsPostgres.conversationId, pgSchema.conversationsPostgres.id)))
            .where(and(eq(pgSchema.conversationsPostgres.id, input.conversationId), eq(pgSchema.conversationsPostgres.appId, input.appId)))
            .limit(1);
        const row = rows[0];
        if (!row?.threadId)
            return null;
        const conversationJid = resolveConversationDestinationJid(row.conversationId, row.providerAccountId, row.conversationExternalRefJson, row.providerId);
        const threadDestinationId = resolveThreadDestinationId(row.threadExternalRefJson);
        if (!conversationJid || !threadDestinationId)
            return null;
        return {
            conversationJid,
...
function resolveConversationDestinationJid(conversationId, providerAccountId, externalRefJson, providerId) {
    const parsed = parseJsonRecord(externalRefJson);
    const directProviderDestination = pickSingleString(collectRefCandidates(parsed, [
        'jid',
        'conversationJid',
        'chatJid',
    ...
    if (directProviderDestination) {
        ...
        if (directResolved.explicitPrefixMismatch)
            return null;
        if (directResolved.value)
            return directResolved.value;
    }
    const resolved = normalizeResolvedJid({
        value: pickSingleString(collectRefCandidates(parsed, ['value', 'externalConversationId'])),
        providerId,
    });
    if (resolved.explicitPrefixMismatch)
        return null;
    if (resolved.value)
        return resolved.value;
    const canonicalHint = extractCanonicalConversationHint(conversationId, providerAccountId);
...
function resolveThreadDestinationId(externalRefJson) {
    const parsed = parseJsonRecord(externalRefJson);
    return pickSingleString(collectRefCandidates(parsed, ['value', 'threadId', 'externalThreadId']));
}
...
function collectRefCandidates(record, keys) {
    const values = [];
...
function pickSingleString(values) {
    if (values.length === 0)
        return null;
...
function extractCanonicalConversationHint(conversationId, providerAccountId) {
...
function extractControlConversationHint(conversationId) {
    if (!conversationId.startsWith(CONTROL_CONVERSATION_PREFIX))
        return null;
    ...
    const externalConversationId = afterPrefix
        ...
        .trim();
    if (!appId || !externalConversationId)
        return null;
...
function explicitProviderIdForValue(value) {
    const separator = value.indexOf(':');
    if (separator <= 0)
        return null;
    const normalizedProviderId = normalizeKnownProviderId(value.slice(0, separator));
    return normalizedProviderId || null;
}
function jidPrefixForProviderId(providerId) {
    const normalized = normalizeKnownProviderId(providerId);
    if (!normalized)
        return null;
    const prefix = getProvider(normalized)?.jidPrefix ?? providerJidPrefix(normalized);
    return prefix || null;
}
