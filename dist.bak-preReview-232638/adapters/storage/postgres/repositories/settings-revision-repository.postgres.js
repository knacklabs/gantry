...
function toSettingsRevision(row) {
    return {
        appId: row.appId,
        revision: row.revision,
        settingsDocument: (row.settingsDocumentJson ?? {}),
...
export class PostgresSettingsRevisionRepository {
    ...
    async appendSettingsRevision(input) {
        ...
        throw new Error(`Failed to allocate a settings revision for ${input.appId} after ${MAX_APPEND_ATTEMPTS} attempts`);
    }
    /**
     ...
     */
    async appendAtExpectedRevision(input) {
        const latest = await this.getLatestSettingsRevision(input.appId);
    ...
    async getLatestSettingsRevision(appId) {
    ...
    async getSettingsRevision(input) {
        const rows = await this.db
            .select()
            .from(pgSchema.settingsRevisionsPostgres)
            .where(and(eq(pgSchema.settingsRevisionsPostgres.appId, input.appId), eq(pgSchema.settingsRevisionsPostgres.revision, input.revision)))
            .limit(1);
        return rows[0] ? toSettingsRevision(rows[0]) : null;
    }
