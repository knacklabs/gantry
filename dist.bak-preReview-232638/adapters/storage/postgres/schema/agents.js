...
export const agentConfigVersionsPostgres = pgTable('agent_config_versions', {
    id: text('id').primaryKey(),
    appId: text('app_id')
        .notNull()
        .references(() => appsPostgres.id, { onDelete: 'cascade' }),
    ...
    permissionPolicyIdsJson: text('permission_policy_ids_json')
        .notNull()
        .default('[]'),
    sandboxProfileId: text('sandbox_profile_id').references(() => sandboxProfilesPostgres.id),
    ...
    agentVersion: unique('agent_config_versions_agent_id_version_unique').on(table.agentId, table.version),
}));
