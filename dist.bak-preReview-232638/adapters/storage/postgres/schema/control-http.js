...
export const controlHttpSessionsPostgres = pgTable('control_http_sessions', {
    ...
    appConversationUnique: unique('control_http_sessions_app_id_external_conversation_id_key').on(table.appId, table.externalConversationId),
    chatJidIdx: index('idx_control_http_sessions_chat_jid').on(sql `(${table.externalRefJson}->>'chatJid')`),
}));
export const controlHttpResponseRoutesPostgres = pgTable('control_http_response_routes', {
    sessionId: text('session_id')
        .notNull()
...
export const controlHttpWebhooksPostgres = pgTable('control_http_webhooks', {
    ...
    appNameUnique: unique('control_http_webhooks_app_id_name_key').on(table.appId, table.name),
...
export const controlHttpWebhookDeliveriesPostgres = pgTable('control_http_webhook_deliveries', {
    ...
    webhookEventUnique: unique('control_http_webhook_deliveries_webhook_id_event_id_key').on(table.webhookId, table.eventId),
    dueIdx: index('idx_control_http_webhook_deliveries_due').on(table.status, table.nextAttemptAt),
}));
