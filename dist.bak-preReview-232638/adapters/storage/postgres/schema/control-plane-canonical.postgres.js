function column(row, snakeName, camelName) {
...
function stringArray(value) {
    if (!Array.isArray(value))
        return null;
    const values = value.filter((entry) => typeof entry === 'string' && entry.length > 0);
...
function folderFromAgentId(agentId) {
    return agentId.startsWith('agent:')
        ? agentId.slice('agent:'.length)
        : agentId;
}
export function mapSession(row) {
    const external = parseJsonObject(column(row, 'external_ref_json', 'externalRefJson'));
    const agentId = String(column(row, 'agent_id', 'agentId'));
    const workspaceKey = text(external.workspaceFolder) ?? folderFromAgentId(agentId);
    const conversationId = String(column(row, 'external_conversation_id', 'externalConversationId'));
    return {
        sessionId: String(column(row, 'session_id', 'sessionId')),
        appId: String(column(row, 'app_id', 'appId')),
        conversationId,
        chatJid: text(external.chatJid) ??
            text(external.externalConversationRef) ??
            conversationId,
        workspaceFolder: workspaceKey,
        workspaceKey,
        title: text(external.title),
        defaultResponseMode: column(row, 'default_response_mode', 'defaultResponseMode') ?? 'sse',
        defaultWebhookId: text(column(row, 'default_webhook_id', 'defaultWebhookId')),
...
export function mapRoute(row) {
    return {
        ...
        responseMode: column(row, 'response_mode', 'responseMode'),
...
export function mapWebhook(row) {
    return {
...
export function mapTrigger(row) {
    return {
