...
export const runtimeDependenciesPostgres = pgTable('runtime_dependencies', {
    ...
    appManifestUnique: uniqueIndex('uq_runtime_dependencies_app_manifest').on(table.appId, table.manifestHash),
    appStatusIdx: index('idx_runtime_dependencies_app_status').on(table.appId, table.status, table.updatedAt),
}));
export const settingsRevisionsPostgres = pgTable('settings_revisions', {
    appId: text('app_id')
        .notNull()
        .references(() => appsPostgres.id, { onDelete: 'cascade' }),
    ...
    appCreatedIdx: index('idx_settings_revisions_app_created').on(table.appId, table.createdAt),
}));
