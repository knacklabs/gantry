...
export const groupJoinOnboardingPostgres = pgTable('group_join_onboarding', {
    ...
    providerChatUnique: uniqueIndex('group_join_onboarding_provider_chat_unique').on(table.providerAccountId, table.chatJid),
    statusIdx: index('idx_group_join_onboarding_status').on(table.status),
    statusCheck: check('group_join_onboarding_status_check', sql `${table.status} IN ('prompted', 'dismissed', 'registered')`),
}));
