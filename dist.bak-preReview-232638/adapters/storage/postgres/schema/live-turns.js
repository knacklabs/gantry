...
export const liveTurnsPostgres = pgTable('live_turns', {
    ...
    scopeCreatedIdx: index('idx_live_turns_scope').on(table.scopeKey, table.createdAt),
...
export const liveTurnCommandsPostgres = pgTable('live_turn_commands', {
    id: text('id').primaryKey(),
    ...
    idempotencyUnique: uniqueIndex('uq_live_turn_commands_idempotency').on(table.liveTurnId, table.idempotencyKey),
    turnSeqUnique: uniqueIndex('uq_live_turn_commands_turn_seq').on(table.liveTurnId, table.seq),
    pendingIdx: index('idx_live_turn_commands_pending')
        .on(table.liveTurnId, table.seq)
        .where(sql `${table.status} = 'pending'`),
}));
export const liveAdmissionWorkItemsPostgres = pgTable('live_admission_work_items', {
    id: text('id').primaryKey(),
    appId: text('app_id').notNull(),
    ...
    idempotencyUnique: uniqueIndex('uq_live_admission_work_items_idempotency').on(table.idempotencyKey),
    ...
    deferredDueIdx: index('idx_live_admission_work_items_deferred_due')
        ...
        .where(sql `${table.state} = 'deferred' AND ${table.deferUntil} IS NOT NULL`),
    deferredNullFifoIdx: index('idx_live_admission_work_items_deferred_null_fifo')
        .on(table.appId, table.createdAt, table.id)
    ...
    claimedExpiredIdx: index('idx_live_admission_work_items_claimed_expired')
        ...
        .where(sql `${table.state} = 'claimed' AND ${table.claimExpiresAt} IS NOT NULL`),
}));
