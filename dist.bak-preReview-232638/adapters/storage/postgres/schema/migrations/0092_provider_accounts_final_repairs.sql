...
WITH live_admission_targets AS (
  SELECT DISTINCT ON (lawi."id")
    lawi."id",
    ci."provider_account_id",
    replace(replace(ci."provider_account_id", ':', '%3A'), '/', '%2F') AS "encoded_provider_account_id"
  FROM "live_admission_work_items" lawi
  JOIN "conversations" c ON c."app_id" = lawi."app_id"
  JOIN LATERAL (
    SELECT CASE
      WHEN NULLIF(btrim(c."external_ref_json"::text), '') IS NULL THEN '{}'::jsonb
      WHEN btrim(c."external_ref_json"::text) ~ '^\{' THEN c."external_ref_json"::jsonb
      ELSE '{}'::jsonb
    END AS ref
  ) conversation_ref ON TRUE
  JOIN "conversation_installs" ci ON ci."app_id" = lawi."app_id"
    AND ci."conversation_id" = c."id"
    AND ci."status" = 'active'
  JOIN "provider_accounts" pa ON pa."app_id" = ci."app_id"
    AND pa."id" = ci."provider_account_id"
  WHERE (
      lawi."agent_id" = ci."agent_id"
      OR lawi."agent_id" IS NULL
    )
    AND lawi."conversation_id" IN (
      c."id",
      CASE
        WHEN c."id" LIKE 'conversation:%' THEN substring(c."id" from 14)
        ELSE NULL
      END,
      NULLIF(conversation_ref.ref ->> 'jid', ''),
      NULLIF(conversation_ref.ref ->> 'value', ''),
      CASE
        WHEN NULLIF(conversation_ref.ref ->> 'value', '') IS NULL THEN NULL
        WHEN conversation_ref.ref ->> 'value' LIKE '%:%' THEN conversation_ref.ref ->> 'value'
        WHEN pa."provider_id" = 'slack' THEN 'sl:' || (conversation_ref.ref ->> 'value')
        WHEN pa."provider_id" = 'telegram' THEN 'tg:' || (conversation_ref.ref ->> 'value')
        WHEN pa."provider_id" = 'teams' THEN 'teams:' || (conversation_ref.ref ->> 'value')
        WHEN pa."provider_id" = 'discord' THEN 'dc:' || (conversation_ref.ref ->> 'value')
        WHEN pa."provider_id" = 'whatsapp' THEN 'wa:' || (conversation_ref.ref ->> 'value')
        ELSE NULL
      END
    )
  ORDER BY
    lawi."id",
    CASE WHEN lawi."conversation_id" = c."id" THEN 0 ELSE 1 END,
    ci."created_at",
    ci."id"
),
live_admission_repairs AS (
  SELECT
    *,
    count(*) OVER (PARTITION BY "idempotency_key") AS "target_count"
  FROM (
    SELECT
      targets."id",
      regexp_replace(lawi."queue_jid", '::provider_account:[^:]*$', '')
        || '::provider_account:' || targets."encoded_provider_account_id" AS "queue_jid",
      regexp_replace(lawi."idempotency_key", '::provider_account:[^:]*$', '')
        || '::provider_account:' || targets."encoded_provider_account_id" AS "idempotency_key"
    FROM live_admission_targets targets
    JOIN "live_admission_work_items" lawi ON lawi."id" = targets."id"
  ) repairs
)
UPDATE "live_admission_work_items" lawi
SET
  "queue_jid" = repairs."queue_jid",
  "idempotency_key" = repairs."idempotency_key"
FROM live_admission_repairs repairs
WHERE lawi."id" = repairs."id"
  AND (
    lawi."queue_jid" <> repairs."queue_jid"
    OR lawi."idempotency_key" <> repairs."idempotency_key"
  )
  AND repairs."target_count" = 1
  AND NOT EXISTS (
    SELECT 1
    FROM "live_admission_work_items" existing
    WHERE existing."id" <> lawi."id"
