...
export const providerAccountsPostgres = pgTable('provider_accounts', {
    id: text('id').primaryKey(),
    appId: text('app_id')
        .notNull()
    ...
    providerIdx: index('idx_provider_accounts_provider').on(table.appId, table.providerId),
    agentIdx: index('idx_provider_accounts_agent').on(table.appId, table.agentId),
    activeIdentityUnique: uniqueIndex('uniq_provider_accounts_active_identity')
        ...
        .where(sql `${table.status} = 'active' AND ${table.externalIdentityRefJson} IS NOT NULL`),
}));
export const conversationInstallsPostgres = pgTable('conversation_installs', {
    id: text('id').primaryKey(),
    appId: text('app_id')
        .notNull()
        .references(() => appsPostgres.id, { onDelete: 'cascade' }),
    ...
    conversationIdx: index('idx_conversation_installs_conversation').on(table.conversationId, table.threadId),
    accountIdx: index('idx_conversation_installs_account').on(table.providerAccountId),
}));
