...
import { agentConfigVersionsPostgres, agentsPostgres, llmProfilesPostgres, } from './agents.js';
...
export const agentRunsPostgres = pgTable('agent_runs', {
    ...
    jobStartedIdx: index('idx_agent_runs_job_started').on(table.jobId, table.startedAt.desc().nullsLast(), table.createdAt.desc()),
    jobShortIdUnique: uniqueIndex('idx_agent_runs_job_short_id_unique').on(table.jobId, table.shortId),
    startedCreatedIdx: index('idx_agent_runs_started_created').on(table.startedAt.desc().nullsLast(), table.createdAt.desc()),
    providerSessionIdx: index('idx_agent_runs_provider_session').on(table.providerSessionId),
