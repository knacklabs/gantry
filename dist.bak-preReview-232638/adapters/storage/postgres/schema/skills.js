...
export const skillCatalogPostgres = pgTable('skill_catalog', {
    ...
    appStatusIdx: index('idx_skill_catalog_app_status').on(table.appId, table.status),
    appAgentStatusIdx: index('idx_skill_catalog_app_agent_status').on(table.appId, table.agentId, table.status),
    appSkillSlugUnique: uniqueIndex('idx_skill_catalog_app_skill_slug_installed')
        .on(table.appId, sql `lower(regexp_replace(regexp_replace(trim(${table.name}), '[^A-Za-z0-9._-]+', '-', 'g'), '-+', '-', 'g'))`)
        .where(sql `${table.status} = 'installed'`),
}));
export const agentSkillBindingsPostgres = pgTable('agent_skill_bindings', {
    id: text('id').primaryKey(),
    appId: text('app_id')
        .notNull()
        .references(() => appsPostgres.id, { onDelete: 'cascade' }),
    ...
    agentSkillUnique: uniqueIndex('idx_agent_skill_bindings_unique').on(table.appId, table.agentId, table.skillId),
}));
