...
export const workerInstancesPostgres = pgTable('worker_instances', {
    ...
    statusHeartbeatIdx: index('idx_worker_instances_status_heartbeat').on(table.status, table.heartbeatAt),
}));
export const runLeasesPostgres = pgTable('run_leases', {
    runId: text('run_id')
        .notNull()
    ...
    leaseTokenUnique: uniqueIndex('uq_run_leases_lease_token').on(table.leaseToken),
    ...
    statusExpiresIdx: index('idx_run_leases_status_expires').on(table.status, table.expiresAt),
    workerIdx: index('idx_run_leases_worker').on(table.workerInstanceId, table.status),
}));
export const runSlotsPostgres = pgTable('run_slots', {
...
export const permissionPromptsPostgres = pgTable('permission_prompts', {
    id: text('id').primaryKey(),
    appId: text('app_id')
        .notNull()
    ...
    renderedDecisionOptionsJson: jsonb('rendered_decision_options_json').notNull(),
    ...
    parentEnvelopeId: text('parent_envelope_id').references(() => permissionPromptsPostgres.id, { onDelete: 'set null' }),
    createdAt: timestamp('created_at', {
        withTimezone: true,
        mode: 'string',
    })
    ...
    scopeIdx: index('idx_permission_prompts_scope').on(table.appId, table.sourceAgentFolder, table.interactionId, table.settlementState),
    promptMessageIdx: index('idx_permission_prompts_message').on(table.appId, table.externalPromptProvider, table.externalPromptConversationId, table.externalPromptMessageId, table.externalPromptThreadId),
    parentEnvelopeIdx: index('idx_permission_prompts_parent').on(table.parentEnvelopeId),
}));
export const pendingInteractionsPostgres = pgTable('pending_interactions', {
    id: text('id').primaryKey(),
    appId: text('app_id')
        .notNull()
        .references(() => appsPostgres.id, { onDelete: 'cascade' }),
    ...
    envelopeId: text('envelope_id').references(() => permissionPromptsPostgres.id),
    ...
    idempotencyUnique: uniqueIndex('uq_pending_interactions_idempotency').on(table.idempotencyKey),
    appStatusIdx: index('idx_pending_interactions_app_status').on(table.appId, table.status, table.expiresAt),
    runStatusIdx: index('idx_pending_interactions_run').on(table.runId, table.status),
    requestLookupIdx: index('idx_pending_interactions_request_lookup').on(table.appId, table.kind, table.sourceAgentFolder, table.requestId, table.status, table.expiresAt),
    envelopeMemberUnique: uniqueIndex('uq_pending_interactions_envelope_member').on(table.envelopeId, table.memberIndex),
    envelopeRequestUnique: uniqueIndex('uq_pending_interactions_envelope_request').on(table.envelopeId, table.requestId),
    envelopeStatusIdx: index('idx_pending_interactions_envelope_status').on(table.envelopeId, table.status, table.expiresAt),
}));
export const runnerControlEventsPostgres = pgTable('runner_control_events', {
    id: text('id').primaryKey(),
    runId: text('run_id')
        .notNull()
        .references(() => agentRunsPostgres.id, { onDelete: 'cascade' }),
    ...
    runCreatedIdx: index('idx_runner_control_events_run').on(table.runId, table.createdAt),
    unexposedIdx: index('idx_runner_control_events_unexposed')
        .on(table.createdAt)
        .where(sql `${table.exposedAt} IS NULL`),
}));
export const runnerControlNoncesPostgres = pgTable('runner_control_nonces', {
...
export const transientGrantsPostgres = pgTable('transient_grants', {
    id: text('id').primaryKey(),
    appId: text('app_id')
        .notNull()
        .references(() => appsPostgres.id, { onDelete: 'cascade' }),
