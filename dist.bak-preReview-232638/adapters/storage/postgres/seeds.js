...
import { ADMIN_MCP_TOOL_FULL_NAMES, SEEDED_SCHEDULER_MCP_TOOL_FULL_NAMES, adminMcpToolIdForFullName, } from '../../../shared/admin-mcp-tools.js';
import { GANTRY_FACADE_EXACT_TOOL_NAMES, GANTRY_FACADE_INPUT_SCHEMAS, } from '../../../shared/agent-tool-references.js';
...
export const DEFAULT_PERMISSION_RULE_ID = 'permission-rule:default:approval-required';
...
export async function seedDefaultRuntimeData(db) {
...
export const DEFAULT_TOOL_CATALOG = [
    ...
    ...GANTRY_FACADE_EXACT_TOOL_NAMES.map((name) => gantryFacadeTool(name, gantryFacadeDisplayName(name), gantryFacadeDescription(name), gantryFacadeCategory(name), gantryFacadeRisk(name))),
    ...ADMIN_MCP_TOOL_FULL_NAMES.map((name) => hostTool(name, adminToolDisplayName(name), adminToolDescription(name), 'admin', 'high')),
    ...SEEDED_SCHEDULER_MCP_TOOL_FULL_NAMES.map((name) => hostTool(name, schedulerToolDisplayName(name), schedulerToolDescription(name), 'admin', schedulerToolRisk(name))),
];
function gantryFacadeTool(name, displayName, description, category, risk) {
    return {
        id: `tool:${name}`,
        name,
        kind: 'host',
...
function hostTool(name, displayName, description, category, risk) {
    return {
...
function gantryFacadeDisplayName(name) {
    switch (name) {
        case 'WebSearch':
            return 'Web search';
        case 'WebRead':
...
function gantryFacadeDescription(name) {
    switch (name) {
        case 'WebSearch':
...
function gantryFacadeCategory(name) {
    switch (name) {
        case 'WebSearch':
        case 'WebRead':
            return 'web';
...
function gantryFacadeRisk(name) {
    switch (name) {
        case 'FileEdit':
        case 'FileWrite':
            return 'high';
...
function adminToolDisplayName(name) {
    switch (name) {
...
function adminToolDescription(name) {
    switch (name) {
        case 'mcp__gantry__settings_desired_state':
...
function schedulerToolDisplayName(name) {
    return name
        .replace(/^mcp__gantry__scheduler_/, 'Scheduler ')
        .replaceAll(/[._-]+/g, ' ')
        .replace(/\b\w/g, (letter) => letter.toUpperCase());
}
function schedulerToolDescription(name) {
    switch (name) {
...
function schedulerToolRisk(name) {
