...
import { bindingRowToGroup, conversationRouteKeyFromBindingRow, normalizeRouteAgentId, } from '../repositories/canonical-binding-repository.postgres.js';
...
function routeAliasTier(row, routeKey) {
    const parsed = parseAgentThreadQueueKey(routeKey);
    if (parsed.threadId)
        return undefined;
    if (!parsed.agentId && !parsed.providerAccountId)
        return 1;
    if (!parsed.agentId)
        return undefined;
    if (normalizeRouteAgentId(parsed.agentId) !== normalizeRouteAgentId(row.agentId))
        return undefined;
    if (!parsed.providerAccountId)
        return 2;
    return parsed.providerAccountId === row.providerAccountId ? 3 : undefined;
}
function compareRouteAliasPreference(left, right) {
...
export class CanonicalBindingOpsService {
    ...
    async getAllConversationRoutes() {
        const rows = (await this.repository.listConversationRoutes()).map(normalizeBindingRow);
        ...
        for (const identityAliases of aliasesByIdentity.values()) {
            ...
            const [winner, ...droppedAliases] = [...identityAliases].sort(compareRouteAliasPreference);
            if (!winner)
                continue;
        ...
        for (const row of rows) {
            ...
            if (binding)
                result[binding.jid] = binding.group;
        }
        return result;
    }
}
