export function parseSetupState(input) {
    ...
    return {
        ...
        fingerprint,
        blockers,
        notified_fingerprint: normalizeString(record.notified_fingerprint ?? record.notifiedFingerprint) ?? null,
    };
}
export function parseRequiredCapabilities(input) {
...
export function parseRecoveryIntent(input) {
    if (!input || typeof input !== 'object' || Array.isArray(input)) {
        return undefined;
    }
    const record = input;
    ...
    if (!dedupeKey || !createdAt || !updatedAt)
        return undefined;
    ...
    return {
        ...
        source_run_id: normalizeNullableString(record.source_run_id ?? record.sourceRunId),
        setup_fingerprint: normalizeNullableString(record.setup_fingerprint ?? record.setupFingerprint),
        requirement_type: normalizeRecoveryRequirementType(record.requirement_type ?? record.requirementType),
        requirement_id: normalizeNullableString(record.requirement_id ?? record.requirementId),
        next_action: normalizeNullableString(record.next_action ?? record.nextAction),
        attempts,
        last_error: normalizeNullableString(record.last_error ?? record.lastError),
    };
}
function parseSetupBlocker(input) {
    if (!input || typeof input !== 'object' || Array.isArray(input))
        return [];
    const record = input;
    const state = normalizeString(record.state);
    ...
    const requirementType = normalizeRecoveryRequirementType(record.requirementType);
    ...
    if (!requirementType || !message || !nextAction || !requirementId)
        return [];
    return [
        {
            state,
...
function normalizeRecoveryRequirementType(input) {
