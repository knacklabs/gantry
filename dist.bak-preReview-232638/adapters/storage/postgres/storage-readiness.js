...
import { getOptionalRuntimeSecret, normalizeRuntimeSecretRefString, parseRuntimeSecretRefString, } from '../../../domain/ports/runtime-secret-provider.js';
...
function defaultPostgresNextAction() {
    return [
...
export async function inspectRuntimeStorageReadiness(runtimeHome) {
    let settings;
    try {
    ...
    try {
        ...
        if (!failure) {
            return {
                status: 'pass',
                message: 'Postgres capabilities are ready (pgvector, search extension, pg-boss, durable runtime events, event outbox).',
            };
        }
        return {
            status: 'fail',
...
export async function inspectRuntimeSecretReadiness(runtimeHome, settings) {
    const refsToResolve = [];
    for (const [providerAccountId, account] of Object.entries(settings.providerAccounts)) {
        if (!account)
            continue;
    ...
    try {
        ...
        for (const ref of refsToResolve) {
            ...
            if (!value?.trim()) {
                missing.push(`provider_accounts.${ref.providerAccountId}.provider ${ref.providerId} runtime_secret_refs.${ref.refKey} runtime secret ref ${ref.ref} did not resolve.`);
            }
        }
        if (missing.length === 0) {
            return {
