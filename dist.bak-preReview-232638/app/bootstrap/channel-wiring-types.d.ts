import type { MessageDeliveryResult, MessageActionCallbackInput, MessageSendOptions, PermissionApprovalCancellation, PermissionApprovalDecision, PermissionApprovalRequest, ProgressUpdateOptions, RichInteractionRequest, StreamingChunkOptions, UserQuestionRequest, UserQuestionResponse, UserQuestionCancellation } from '../../domain/types.js';
import type { RuntimeSettings } from '../../config/settings/runtime-settings.js';
import type { isSenderControlAllowed, isSenderAllowed, loadSenderControlAllowlist, loadSenderAllowlist, shouldDropMessage, shouldLogDenied } from '../../platform/sender-allowlist.js';
...
import type { ConversationContextHydrationRequest, ConversationContextHydrationResult } from '../../channels/channel-provider.js';
import type { BrainChannelHarvestTap } from '../../brain/brain-channel-harvest.js';
export type ChannelWiringRepository = RuntimeChatMetadataRepository & RuntimeMessageRepository;
...
export type RetryTailRecoveryEnqueue = (input: RetryTailRecoveryEnqueueInput) => Promise<void>;
...
export type DurableOutboundAttemptFactory = (input: DurableOutboundAttemptInput) => Promise<DurableOutboundAttempt>;
...
export interface ChannelWiring {
    ...
    connectEnabledChannels: (runtimeSettings: RuntimeSettings, options?: {
        providerInbound?: boolean;
    }) => Promise<void>;
    hasConnectedChannels: () => boolean;
    hasChannel: (jid: string, options?: ChannelAccountOptions) => boolean;
    supportsStreaming: (jid: string, options?: {
        providerAccountId?: string;
    }) => boolean;
    supportsProgress: (jid: string, options?: {
        providerAccountId?: string;
    }) => boolean;
    sendMessage: (jid: string, rawText: string, options: {
        durability: 'required' | 'best_effort';
        throwOnMissing?: boolean;
        messageOptions?: MessageSendOptions;
    }) => Promise<void>;
    sendProviderMessage: (jid: string, rawText: string, options: {
        permit: RecoveryDispatchPermit;
        throwOnMissing?: boolean;
        messageOptions?: MessageSendOptions;
    }) => Promise<MessageDeliveryResult | undefined>;
    createRecoveryDispatchPermit: (input: RecoveryDispatchPermitInput) => RecoveryDispatchPermit;
    setRetryTailRecoveryEnqueue: (enqueue: RetryTailRecoveryEnqueue | undefined) => void;
    setDurableOutboundAttemptFactory: (factory: DurableOutboundAttemptFactory | undefined) => void;
    setRuntimeSecrets: (provider: RuntimeSecretProvider) => void;
    setMessageActionHandler: (handler: ((input: MessageActionCallbackInput) => Promise<void>) | undefined) => void;
    sendStreamingChunk: (jid: string, rawText: string, options?: StreamingChunkOptions) => Promise<boolean>;
    resetStreaming: (jid: string, options?: ChannelStreamResetOptions) => void;
    setTyping: (jid: string, isTyping: boolean, options?: ChannelAccountOptions) => Promise<void>;
    sendProgressUpdate: (jid: string, text: string, options?: ProgressUpdateOptions) => Promise<void>;
    addReaction: (jid: string, messageRef: string, emoji: string, options?: ChannelAccountOptions) => Promise<void>;
    syncGroups: (force: boolean) => Promise<void>;
    requestPermissionApproval: (request: PermissionApprovalRequest) => Promise<PermissionApprovalDecision>;
    cancelPermissionApproval: (cancellation: PermissionApprovalCancellation) => Promise<'settled' | 'queued' | 'not_found'>;
    ...
    renderAgentTodo: (jid: string, render: AgentTodoRender, options?: ChannelAccountOptions) => Promise<boolean>;
    renderRichInteraction: (jid: string, request: RichInteractionRequest, options?: ChannelAccountOptions) => Promise<boolean>;
    ...
    }, options?: ChannelAccountOptions) => Promise<boolean>;
...
export type { ConversationContextHydrationRequest, ConversationContextHydrationResult, };
