...
import { parseAgentThreadQueueKey, parseThreadQueueKey, } from '../../shared/thread-queue-key.js';
import { consumeBrowserProfileActivity, isBrowserProfileSyncEnabled, snapshotBrowserProfile, } from '../../runtime/browser-profile-sync.js';
function resolveConversationRouteFolder(routes, queueJid) {
    ...
    if (parsedQueue.agentId) {
        const exactMatch = candidates.find((candidate) => (candidate.agentId ?? agentIdForFolder(candidate.folder)) ===
            parsedQueue.agentId);
        if (exactMatch)
...
export function buildLiveTurnBrowserFinalizer(deps) {
    return async (input) => {
        ...
        const folder = resolveConversationRouteFolder(deps.getConversationRoutes(), input.queueJid);
        if (!folder)
            return;
        ...
        if (!used)
            return;
        try {
            if (!isBrowserProfileSyncEnabled())
                return;
            ...
            deps.warn({ err, queueJid: input.queueJid, profileName }, 'Failed to snapshot live-turn browser profile after finalize');
        }
    };
}
