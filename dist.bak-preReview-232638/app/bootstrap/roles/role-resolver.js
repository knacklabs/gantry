import { DEFAULT_PROCESS_ROLE, PROCESS_ROLE_ENV_VAR, PROCESS_ROLES, } from './process-role.js';
/**
 ...
 */
export function resolveProcessRole(env) {
    const raw = env[PROCESS_ROLE_ENV_VAR]?.trim();
    if (!raw)
        return DEFAULT_PROCESS_ROLE;
    if (PROCESS_ROLES.includes(raw)) {
        return raw;
    }
    throw new Error(`Invalid ${PROCESS_ROLE_ENV_VAR}=${JSON.stringify(raw)}. ` +
