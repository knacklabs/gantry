import { ASYNC_TASK_STALE_AFTER_MS, AsyncCommandTaskService, } from '../../jobs/async-command-task-service.js';
import { DEFAULT_ASYNC_COMMAND_TIMEOUT_MS, DEFAULT_ASYNC_RESOURCE_LIMITS, buildAsyncCommandEnv, runSandboxedAsyncCommand, } from '../../jobs/async-command-sandbox-runner.js';
...
import { releaseCompactionLockFromTask, SESSION_COMPACTION_TIMEOUT_MS, } from '../../runtime/group-session-command-state.js';
...
export async function recoverStaleAsyncCommandTasks(appId, deps) {
    ...
    try {
        const timedOutCompactions = await recoverStaleSessionCompactionTasks(appId, deps);
        if (timedOutCompactions > 0) {
            deps.logger.warn({ timedOutCompactions }, 'Recovered stale session compaction tasks');
        }
...
export async function recoverStaleSessionCompactionTasks(appId, deps) {
    const repository = deps.getAsyncTaskRepository?.();
    if (!repository)
        return 0;
    ...
    for (const task of tasks) {
        const activityAt = Date.parse(task.heartbeatAt ?? task.updatedAt ?? task.createdAt);
        if (Number.isFinite(activityAt) && activityAt >= staleBefore)
            continue;
        ...
        if (deps.opsRepository) {
            await releaseCompactionLockFromTask(deps.opsRepository, stringValue(terminal.privateCorrelationJson.provider) ?? '', terminal);
        }
        await publishSessionCompactionTimeoutEvent(deps.publishRuntimeEvent, terminal, errorSummary).catch((err) => deps.logger.warn({ err, taskId: task.id }, 'Failed to publish session compaction timeout event'));
    }
    return recovered;
}
async function publishSessionCompactionTimeoutEvent(publishRuntimeEvent, task, errorSummary) {
    ...
    await publishRuntimeEvent({
        ...
        ...(stringValue(task.privateCorrelationJson.agentSessionId)
            ? {
                sessionId: stringValue(task.privateCorrelationJson.agentSessionId),
            }
            : {}),
        ...(task.conversationId
            ? { conversationId: task.conversationId }
...
let activeAsyncTaskRecoveryLoop;
export function startAsyncTaskRecoveryLoop(appId, deps) {
...
export function stopAsyncTaskRecoveryLoop() {
    if (!activeAsyncTaskRecoveryLoop)
        return;
    clearInterval(activeAsyncTaskRecoveryLoop);
    activeAsyncTaskRecoveryLoop = undefined;
}
function createRecoveredDelegatedAgentRun(deps) {
    return (_task, taskInput) => async (runInput) => {
        ...
        if (routedAgentId !== recoveryAgentId) {
            throw new Error(`Delegated task route mismatch: expected ${recoveryAgentId}, resolved ${routedAgentId}.`);
        }
        ...
        const attachedMcpSourceIds = await resolveTurnSelectedMcpServerIds(deps, scopedTaskOwner);
        const runAgent = deps.runAgent ?? spawnAgent;
        let latestResult = null;
        let processHandlePersisted = null;
        const output = await runAgent(group, {
            ...
            if (!proc.pid)
                return;
            processHandlePersisted = Promise.resolve(runInput.onProcessStarted?.({
                pid: proc.pid,
...
async function createRecoveredMcpProxy(deps, task) {
