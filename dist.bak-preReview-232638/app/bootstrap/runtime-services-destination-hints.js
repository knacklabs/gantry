...
const PREFIX_SL = 'sl:';
const PREFIX_TG = 'tg:';
export function canonicalConversationIdForJid(jid, providerAccountId) {
...
export function resolveDurableOutboundTarget(input) {
    ...
    return {
        ...
        conversationId: canonicalConversationIdForJid(input.jid, input.providerAccountId),
    };
}
...
function providerJidFromCanonicalConversationId(rawConversationId, canonicalConversationJid) {
...
function providerJidFromDestinationHint(rawHint, canonicalConversationJid) {
    if (typeof rawHint !== 'string')
        return { malformedCanonicalHint: false };
    const hint = rawHint.trim();
    if (!hint)
        return { malformedCanonicalHint: false };
    if (hint.startsWith(CANONICAL_CONVERSATION_PREFIX)) {
        const providerJid = providerJidFromCanonicalConversationId(hint, canonicalConversationJid);
        if (!providerJid) {
...
export function normalizeDestinationHintAgainstCanonical(rawHint, canonicalConversationJid) {
    const parsed = providerJidFromDestinationHint(rawHint, canonicalConversationJid);
    ...
    const canonicalProviderPrefix = canonicalConversationJid.slice(0, separator + 1);
...
export function sanitizeRetryTailProviderPayloadDestinationMetadata(providerPayload, canonicalConversationJid) {
    if (!providerPayload ||
        typeof providerPayload !== 'object' ||
        Array.isArray(providerPayload)) {
        return undefined;
    }
    const sanitized = { ...providerPayload };
    const canonicalRawDestinationId = rawConversationIdFromJid(canonicalConversationJid);
    if (!canonicalRawDestinationId)
        return sanitized;
...
export function sanitizeRetryTailForCanonicalDestination(retryTail, canonicalConversationJid) {
    if (!retryTail)
        return undefined;
    const providerPayload = sanitizeRetryTailProviderPayloadDestinationMetadata(retryTail.providerPayload, canonicalConversationJid);
    return {
        canonicalText: retryTail.canonicalText,
        ...(providerPayload !== undefined ? { providerPayload } : {}),
    };
}
function rawConversationIdFromJid(jid) {
...
function normalizeMetadataDestinationValue(value, acceptedPrefix) {
...
function sanitizeDestinationMetadataField(input) {
    ...
    if (typeof raw !== 'string')
        return;
    const normalized = normalizeMetadataDestinationValue(raw, input.acceptedPrefix);
...
function parseAppSessionJid(jid) {
    ...
    if (delimiterIndex <= 0)
        return null;
    if (rest.indexOf(':', delimiterIndex + 1) !== -1)
        return null;
