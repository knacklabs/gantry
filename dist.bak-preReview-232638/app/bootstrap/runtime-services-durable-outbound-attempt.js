export function createDurableOutboundAttempt(input) {
    ...
    const settleRemainingAsAmbiguous = async (settleInput) => {
        for (let index = settleInput.fromIndex; index < input.claimedItems.length; index += 1) {
            const claimedItem = input.claimedItems[index];
            const settled = await input.outboundDeliveryService.settlePartiallyDelivered({
            ...
            if (!settled.applied) {
                throw new Error(`Durable outbound ambiguous settlement was not applied for ${input.deliveryId} item ${claimedItem.itemId}.`);
            }
    ...
    return {
        settleSent: async (settleInput) => {
            for (let index = nextUnsettledIndex; index < input.claimedItems.length; index += 1) {
                const claimedItem = input.claimedItems[index];
                try {
                    ...
                    if (!settled.applied) {
                        throw new Error(`Durable outbound sent settlement was not applied for ${input.deliveryId} item ${claimedItem.itemId}.`);
                    }
                    nextUnsettledIndex = index + 1;
                    continue;
                }
        ...
        settleFailed: async (settleInput) => {
            for (let index = nextUnsettledIndex; index < input.claimedItems.length; index += 1) {
                ...
                if (!settled.applied) {
                    throw new Error(`Durable outbound failed settlement was not applied for ${input.deliveryId}.`);
                }
                nextUnsettledIndex = index + 1;
            }
        },
        settlePartiallyDelivered: async (settleInput) => {
            ...
            const settled = await input.outboundDeliveryService.settlePartiallyDelivered({
                deliveryId: input.deliveryId,
                itemId: firstClaimedItem.itemId,
            ...
            if (!settled.applied) {
                throw new Error(`Durable outbound partial settlement was not applied for ${input.deliveryId}.`);
            }
            nextUnsettledIndex += 1;
            for (let index = nextUnsettledIndex; index < input.claimedItems.length; index += 1) {
                const claimedItem = input.claimedItems[index];
                const terminalFailure = await input.outboundDeliveryService.settleFailed({
                    deliveryId: input.deliveryId,
                    itemId: claimedItem.itemId,
                    claimToken: claimedItem.claimToken,
                    error: settleInput.error,
                ...
                if (!terminalFailure.applied) {
                    throw new Error(`Durable outbound residual item settlement was not applied for ${input.deliveryId}.`);
                }
                nextUnsettledIndex = index + 1;
            }
        },
