import { installGlobalErrorHandlers, logger, } from '../infrastructure/logging/logger.js';
...
import { startRuntimeServices, beginDrainingLiveTurnAdmission, shutdownLiveTurnAuthority, stopAsyncTaskRecoveryLoop, stopLiveTurnRecoveryLoop, stopLiveAdmissionLoop, } from './bootstrap/runtime-services.js';
import { installShutdownHandlers } from './bootstrap/shutdown.js';
import { runStartup } from './bootstrap/startup.js';
import { closeRuntimeStorage, getRuntimeControlRepository, getRuntimeEventExchange, getRuntimeSkillArtifactStore, getRuntimeStorage, tryAcquireRuntimeAdvisoryLease, } from '../adapters/storage/postgres/runtime-store.js';
...
import { GANTRY_HOME, createGroupJoinOnboardingCoordinator, getDeploymentMode, getRuntimeQueueConfig, getRuntimeSettingsForConfig, loadRuntimeSettings, RUNTIME_MEMORY_DREAMING_ENABLED, RUNTIME_MEMORY_ENABLED, } from '../config/index.js';
...
import { createControlAgentSettingsPort, createControlSettingsImportPort, prepareFleetSettings, startFleetSubsystems, } from './bootstrap/fleet-boot.js';
import { formatRuntimePreflightFailure, validateRuntimePreflight, validateRuntimePreflightWithStorage, } from '../config/preflight.js';
...
import { createResolveObserverStatus, } from '../application/control-plane/control-plane-storage-model.js';
...
export async function startGantryRuntime(options = {}) {
    ...
    app.setChannelRuntime({
        ...
        sendMessage: (chatJid, rawText, options) => channelWiring.sendMessage(chatJid, rawText, {
            durability: 'required',
    ...
    channelWiring.setRuntimeSecrets(createRepositoryRuntimeSecretProvider({
        appId: 'default',
        repository: storage.repositories.capabilitySecrets,
    }));
    const isFleet = getDeploymentMode() === 'fleet' || shouldDeferPreflightForFleetRole;
    ...
    let browserToolModule;
    const loadBrowserToolModule = () => (browserToolModule ??= import(browserToolModulePath));
    ...
    let approvedCommandModule;
    const loadApprovedCommandModule = () => (approvedCommandModule ??= import(approvedCommandModulePath));
    const liveRecoveryCoordinatorLeaseManager = startLiveRecoveryCoordinatorLeaseAcquisition({
        runtimeSettings,
    ...
    installShutdownHandlers({
        ...
        closeBrowserToolBackends: async () => (await loadBrowserToolModule()).closeBrowserToolBackends(),
    });
    ...
    try {
        if (!runtimeSettings.runtime.liveTurns.enabled) {
            logger.info('Live recovery coordinator disabled by runtime.live_turns.enabled=false; connecting channels in outbound-only mode');
        }
        if (!roleCaps.providerInbound) {
            logger.info({ processRole }, 'Process role has no provider inbound; connecting channels in outbound-only mode');
        }
        ...
        if (!channelWiring.hasConnectedChannels()) {
            logger.warn('No channels connected; runtime will continue without inbound/outbound channel delivery');
        }
        await startRuntimeServices({
            ...
            runApprovedCommand: async (input) => (await loadApprovedCommandModule()).runApprovedSandboxCommand(input),
            getSkillArtifactStore: getRuntimeSkillArtifactStore,
            getPermissionRepository: () => storage.repositories.permissions,
            getPermissionPromotionRepository: () => storage.repositories.permissionPromotions,
            getPermissionDecisionMemoryRepository: () => storage.repositories.permissionDecisionMemory,
            settingsRepositories: storage.repositories,
            getOutboundDeliveryRepository: () => storage.repositories.outboundDeliveries,
            getWorkerCoordinationRepository: () => storage.repositories.workerCoordination,
            getLiveTurnRepository: () => storage.repositories.liveTurns,
            getLiveAdmissionWakeupSource: () => storage.liveAdmissionWakeupSource,
            getLiveTurnCommandWakeupSource: () => storage.liveTurnCommandWakeupSource,
            getRuntimeDependencyRepository: () => storage.repositories.runtimeDependencies,
            publishRuntimeEvent: async (event) => {
                await getRuntimeEventExchange().publish(event);
            },
            subscribeRuntimeEvents: (filter) => getRuntimeEventExchange().subscribe(filter),
            callBrowserTool: async (input) => (await loadBrowserToolModule()).callBrowserTool(input),
            ...
            closeBrowserToolBackends: async (profileName) => (await loadBrowserToolModule()).closeBrowserToolBackends(profileName),
            ...(holdSchedulerUntilSettingsLoaded
                ? {
                    startSchedulerLoop: (async (schedulerDeps) => {
                        ...
                        logger.warn('Fleet worker has no settings revision; scheduler job ' +
            ...
            fleetSubsystems = await startFleetSubsystems({
                ...
                onSettingsReady: async (settings) => {
                    ...
                    try {
                        ...
                        logger.warn({ err }, 'Failed to initialize tracing on first settings revision');
                    }
                    ...
                    if (!start)
                        return;
                    await start();
                    logger.info('First settings revision applied; scheduler job claiming started');
                },
        ...
        controlServerRef.current = startControlServer({
            ...
            roleReadinessRequirements: processRole === 'all'
                ? {
            ...
            addMessageReaction: (jid, messageRef, emoji, options) => channelWiring.addReaction(jid, messageRef, emoji, options),
        });
    }
    catch (err) {
        await liveRecoveryCoordinatorLeaseManager.stop();
