...
import { canonicalToolReferenceForView, skillActionDefinitionsForBindings, } from './agent-capability-skill-actions.js';
export async function replaceAgentAccessDocument(input) {
    ...
    const skillMap = await input.requireInstalledSkills(input.appId, sourceSkillIds);
    const mcpServerMap = await input.requireActiveMcpServers(input.appId, sourceMcpServerIds);
...
function nextCapabilityToolBindings(input) {
    const existingByToolId = new Map(input.existingBindings.map((binding) => [binding.toolId, binding]));
    const selected = new Set(input.toolIds);
    return [
        ...input.existingBindings
            .filter((binding) => binding.status === 'active' && !selected.has(binding.toolId))
            .map((binding) => ({
            ...binding,
            status: 'disabled',
            updatedAt: input.now,
...
function nextSkillSourceBindings(input) {
    const existingBySkillId = new Map(input.existingBindings.map((binding) => [binding.skillId, binding]));
