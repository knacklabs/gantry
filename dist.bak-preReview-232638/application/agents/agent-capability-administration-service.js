...
import { displayToolReference, isGantryFacadeExactToolRule, validateReadableAgentToolRule, } from '../../shared/agent-tool-references.js';
...
import { buildConfiguredAgentToolAccess, buildRequestableAdminToolAccess, } from '../../shared/tool-access-view.js';
import { adminMcpToolNameFromFullName, isAdminMcpToolFullName, } from '../../shared/admin-mcp-tools.js';
import { nowIso } from '../../shared/time/datetime.js';
import { buildSelectedCapabilities, canonicalToolReferenceForView, skillActionDefinitionsForAgent, skillActionDefinitionsForBindings, } from './agent-capability-skill-actions.js';
import { buildAgentSources, readableSkillSources, } from './agent-source-views.js';
...
import { summarizeAgentAccess, } from './agent-access-summary.js';
export class AgentCapabilityAdministrationService {
    ...
    async getCapabilities(input) {
        ...
        const [toolBindings, toolSources, skillBindings, mcpBindings] = await Promise.all([
        ...
        const activeToolBindings = toolBindings.filter((binding) => binding.status === 'active');
        const [selectedTools, configuredSkillSources] = await Promise.all([
            Promise.all(activeToolBindings.map((binding) => this.repositories.tools.getTool(binding.toolId))),
            readableSkillSources({
                skillBindings,
                repository: this.repositories.skills,
            }),
        ]);
        const configuredToolEntries = activeToolBindings.flatMap((binding, index) => {
        ...
        const configuredTools = configuredToolEntries.flatMap((entry) => canonicalToolReferenceForView(entry.reference, {
        ...
        const capabilities = buildSelectedCapabilities(configuredToolEntries, semanticCapabilityDefinitions);
        const toolAccess = buildConfiguredAgentToolAccess(configuredTools, buildRequestableAdminToolAccess(enabledAdminTools));
        return {
    ...
    async replaceCapabilities(input) {
        ...
        if (agent.status !== 'active') {
            throw new ApplicationError('INVALID_REQUEST', `Agent is not active: ${input.agentId}`);
        }
        ...
        const selectedToolReferences = resolveSelectedToolReferences(input.capabilities, semanticCapabilityDefinitions);
        const capabilityToolIds = await Promise.all(selectedToolReferences.map(async (reference) => {
        ...
        const toolBindingMap = new Map(toolBindings.map((binding) => [binding.toolId, binding]));
        const toolSelection = new Set(capabilityToolIds);
        const nextToolBindings = [
            ...toolBindings
                .filter((binding) => binding.status === 'active' && !toolSelection.has(binding.toolId))
                .map((binding) => ({
                ...binding,
                status: 'disabled',
                updatedAt: now,
            ...
            ...capabilityToolIds.map((toolId) => {
                ...
                return {
                    id: `agent-tool-binding:${input.agentId}:${toolId}`,
                    appId: input.appId,
                    agentId: input.agentId,
                    toolId,
        ...
        const capabilities = buildSelectedCapabilities(configuredToolEntries, semanticCapabilityDefinitions);
        const toolAccess = buildConfiguredAgentToolAccess(configuredTools, buildRequestableAdminToolAccess(selectedAdminToolNames(configuredTools)));
        return {
            agentId: input.agentId,
            sources,
            capabilities,
    ...
    async replaceAccessDocument(input) {
        return replaceAgentAccessDocument({
            ...
            requireInstalledSkills: (appId, skillIds) => this.requireInstalledSkills(appId, skillIds),
            requireActiveMcpServers: (appId, serverIds) => this.requireActiveMcpServers(appId, serverIds),
            requireSelectableTools: (appId, toolIds) => this.requireSelectableTools(appId, toolIds),
    ...
    async replaceSources(input) {
        ...
        const now = this.clock.now();
        const sourceSkillIds = unique(input.sources.skills.map((source) => source.id));
        const sourceMcpServerIds = unique(input.sources.mcpServers.map((source) => source.id));
        const sourceTools = uniqueToolSources(input.sources.tools, {
            appId: input.appId,
            agentId: input.agentId,
            now,
        });
        const skillMap = await this.requireInstalledSkills(input.appId, sourceSkillIds);
        const mcpServerMap = await this.requireActiveMcpServers(input.appId, sourceMcpServerIds);
        ...
        const skillBindingMap = new Map(skillBindings.map((binding) => [binding.skillId, binding]));
        const nextSkillBindings = sourceSkillIds.map((skillId) => {
            const existing = skillBindingMap.get(skillId);
            return {
                id: `agent-skill-binding:${input.agentId}:${skillId}`,
                appId: input.appId,
                agentId: input.agentId,
                skillId,
                configVersionId: existing?.configVersionId,
    ...
    async replaceAgentToolSources(input) {
        if (!this.repositories.tools.replaceAgentToolSources) {
            throw new ApplicationError('INVALID_REQUEST', 'Agent tool source repository is unavailable.');
        }
        await this.repositories.tools.replaceAgentToolSources(input);
    }
    async requireAgent(appId, agentId) {
    ...
    async requireSelectableTools(appId, toolIds) {
        const tools = new Map();
        await Promise.all(toolIds.map(async (toolId) => {
            ...
            if (tool.status !== 'active' || !tool.selectable) {
                throw new ApplicationError('INVALID_REQUEST', `Tool is not selectable: ${toolId}`);
            }
            const readableRule = displayToolReference({ toolId, tool });
            const validation = validateReadableAgentToolRule(readableRule);
            if (!validation.ok) {
                throw new ApplicationError('INVALID_REQUEST', `Tool is not selectable: ${readableRule}: ${validation.reason}`);
            }
            tools.set(toolId, tool);
        }));
        return tools;
    }
    async requireInstalledSkills(appId, skillIds) {
        ...
        await Promise.all(skillIds.map(async (skillId) => {
            ...
            if (!isSkillUsableForBinding(skill)) {
                throw new ApplicationError('INVALID_REQUEST', `Skill is not installed: ${skillId}`);
            }
            skills.set(skillId, skill);
        }));
        return skills;
    }
    async requireActiveMcpServers(appId, serverIds) {
        ...
        await Promise.all(serverIds.map(async (serverId) => {
            ...
            if (!isMcpServerActive(server)) {
                throw new ApplicationError('INVALID_REQUEST', `MCP server is not active: ${serverId}`);
            }
...
function assertUniqueSkillMaterializationKeys(skillIds, skills) {
    const [collision] = skillMaterializationCollisions(skillIds.flatMap((skillId) => {
        const skill = skills.get(skillId);
        return skill ? [skill] : [];
    }));
    if (!collision)
        return;
    throw new ApplicationError('CONFLICT', formatSkillMaterializationCollision(collision));
}
function capabilitySelectionToToolReference(capabilityId) {
    const id = capabilityId.trim();
    if (id === 'browser.use')
        return 'Browser';
    if (id.startsWith('RunCommand('))
        return id;
    if (isAdminMcpToolFullName(id) || isGantryFacadeExactToolRule(id))
        return id;
 ...
 */
function resolveSelectedToolReferences(capabilities, semanticCapabilityDefinitions) {
    return unique(capabilities.flatMap((capability) => {
        ...
        if (canonical.length === 0) {
            throw new ApplicationError('INVALID_REQUEST', `Capability selection ${capability.id} is not a durable access rule.`);
        }
        return canonical;
    }));
}
function selectedAdminToolNames(tools) {
    const names = new Set();
...
function uniqueToolSources(sources, input) {
    const byKey = new Map();
    for (const source of sources) {
        const version = source.version ?? source.kind;
        const key = `${source.kind}:${source.id}:${version}`;
        byKey.set(key, {
            id: `agent-tool-source:${input.agentId}:${source.kind}:${source.id}:${version}`,
            appId: input.appId,
            agentId: input.agentId,
            sourceId: source.id,
            kind: source.kind,
