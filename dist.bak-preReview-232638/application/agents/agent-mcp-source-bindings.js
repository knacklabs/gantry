...
export function nextMcpSourceBindings(input) {
    const existingByServerId = new Map(input.existingBindings.map((binding) => [binding.serverId, binding]));
    const sourceByServerId = new Map(input.sources.map((source) => [source.id, source]));
