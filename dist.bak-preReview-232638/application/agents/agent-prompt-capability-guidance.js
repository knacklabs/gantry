import { compareCatalogEntries, } from './agent-prompt-capability-catalog.js';
export function renderCapabilityGuidancePrompt(input) {
    ...
    const connectedMcpSources = sortedCatalogEntries(input.catalog?.connectedMcpSources);
    const intro = [
        '# Capability catalog',
    ...
    const assemble = (ready, skills, sources) => [
        ...intro,
        '',
        'Ready actions',
        ...ready,
    ...
    const fits = (ready, skills, sources) => assemble(ready, skills, sources).length <= input.budget;
    const sourceReservation = reserveConnectedSourcePresence({
        entries: connectedMcpSources,
        fits,
    });
    let readyDescriptionLimit = 160;
    let renderedReady = readyActions.map((entry) => renderCatalogEntry(entry, 'ready', readyDescriptionLimit));
    while (readyDescriptionLimit > 0 &&
        !fits(renderedReady, [], sourceReservation.lines)) {
        readyDescriptionLimit = Math.max(0, readyDescriptionLimit - 20);
        renderedReady = readyActions.map((entry) => renderCatalogEntry(entry, 'ready', readyDescriptionLimit));
    }
    ...
    return {
        ...
        diagnostics: {
            ...
            omitted: {
                ...
                installedSkills: installedSkills.length - renderedCounts.installedSkills,
                connectedMcpSources: connectedMcpSources.length - renderedCounts.connectedMcpSources,
            },
        },
    };
}
function reserveConnectedSourcePresence(input) {
    if (input.entries.length === 0)
        return { lines: [], descriptionLimit: 160 };
...
function appendWholeEntriesWithinBudget(input) {
    ...
    for (let index = 0; index < input.entries.length; index += 1) {
        const line = renderCatalogEntry(input.entries[index], input.section, input.descriptionLimit);
...
function renderCatalogEntry(entry, section, descriptionLimit) {
    const displayName = oneLine(entry.displayName);
    const account = entry.accountLabel ? ` (${oneLine(entry.accountLabel)})` : '';
    const label = section === 'ready'
        ? `${oneLine(entry.category)} · ${displayName}${account}`
        : `${displayName}${account}`;
    const description = truncateCatalogDescription(oneLine(entry.description), descriptionLimit);
    return description ? `- ${label} — ${description}` : `- ${label}`;
}
function sortedCatalogEntries(entries) {
    return [...(entries ?? [])].sort(compareCatalogEntries);
}
function oneLine(value) {
    return value.replace(/\s+/g, ' ').trim();
}
function truncateCatalogDescription(value, limit) {
    if (limit <= 0)
        return '';
...
function renderedEntryCount(lines) {
