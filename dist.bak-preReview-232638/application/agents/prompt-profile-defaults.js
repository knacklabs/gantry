...
import { DEFAULT_RELATIONSHIP_MODE, resolveAgentRelationshipMode, } from '../../shared/agent-relationship-mode.js';
export const AGENTS_FILENAME = 'AGENTS.md';
export const SOUL_FILENAME = 'SOUL.md';
export const promptProfileAgentIdForFolder = (agentFolder) => agentIdForFolder(agentFolder);
export function defaultAgentsPromptMarkdown(agentName, relationshipMode = DEFAULT_RELATIONSHIP_MODE, accessPreset = 'full') {
    const displayName = agentName.trim() || 'Agent';
    const mode = resolveAgentRelationshipMode(relationshipMode);
    const modeLine = mode === 'organization'
    ...
    const howToLines = accessPreset === 'locked'
        ? [
...
export function defaultSoulPromptMarkdown(agentName, relationshipMode = DEFAULT_RELATIONSHIP_MODE) {
    const displayName = agentName.trim() || 'Agent';
    const mode = resolveAgentRelationshipMode(relationshipMode);
    const relationshipLine = mode === 'organization'
...
export function promptProfileAgentsPath(agentFolder) {
    return `${agentFolder}/${AGENTS_FILENAME}`;
}
export function promptProfileSoulPath(agentFolder) {
