...
import { resolveAgentRelationshipMode, } from '../../shared/agent-relationship-mode.js';
import { PROACTIVE_RECOMMENDATION_GUIDANCE } from '../../shared/capability-guidance.js';
import { renderCapabilityGuidancePrompt, } from './agent-prompt-capability-guidance.js';
import { isValidPromptAgentFolder } from './prompt-profile-folder.js';
import { AGENTS_FILENAME, defaultAgentsPromptMarkdown, defaultSoulPromptMarkdown, promptProfileAgentIdForFolder, promptProfileAgentsPath, promptProfileSoulPath, SOUL_FILENAME, } from './prompt-profile-defaults.js';
...
export const DEFAULT_PROMPT_SECTION_BUDGETS = {
    RUNTIME_RULES: 2200,
    PERSONA: 1200,
    SOUL: 3000,
...
function personaPrompt(persona, accessPreset) {
    switch (persona) {
        case 'generalist':
            return [
                '# Generalist persona',
...
export function capabilityGuidancePrompt(catalog, accessPreset, budget = DEFAULT_PROMPT_SECTION_BUDGETS.CAPABILITY_GUIDANCE, mcpInventoryToolsMounted = true) {
...
let channelPromptPresentationRenderer = () => undefined;
export function registerChannelPromptPresentationRenderer(renderer) {
    channelPromptPresentationRenderer = renderer;
}
export function renderChannelPromptPresentationLine(chatJid, conversationKind) {
    return channelPromptPresentationRenderer(chatJid, conversationKind);
}
function makeSection(name, source, content, budget) {
...
function truncateDeterministically(content, budget) {
    if (budget <= 0)
        return '';
...
function runtimeContextLines(options) {
    const lines = [];
    if (options.modelIdentity) {
        const { alias, modelId, provider } = options.modelIdentity;
        lines.push(`- You are running on ${alias} (${modelId}) via ${provider}. State this plainly if the user asks which model you are; deeper runtime internals stay internal.`);
    }
    const context = options.runtimeContext;
    if (!context)
        return lines;
    if (context.channelContextLine)
        lines.push(context.channelContextLine);
    if (context.workspacePath) {
        lines.push(`- Workspace root: ${context.workspacePath}. Durable outputs belong under media/ inside the workspace; tmp paths are ephemeral and may not survive between runs.`);
    }
    if (context.job) {
        const label = context.job.name && context.job.id
            ? `"${context.job.name}" (${context.job.id})`
            : context.job.name
                ? `"${context.job.name}"`
                : (context.job.id ?? 'this scheduled job');
        lines.push(`- This run executes scheduled job ${label}. Job runs are quiet until terminal: deliver one final outcome report; do not send interim progress messages.`);
    }
    else {
        lines.push('- New user messages may arrive mid-run and supersede the current plan; treat messages delivered mid-run as fresh instructions, not history.');
    }
    return lines;
}
...
export class PromptProfileService {
    ...
    async ensureAgentDefaults(options) {
        if (!isValidPromptAgentFolder(options.agentFolder))
            return;
        const relationshipMode = resolveAgentRelationshipMode(options.relationshipMode);
        await this.writeDefaultIfMissing({
            appId: options.appId || this.appId,
            agentId: options.agentId || promptProfileAgentIdForFolder(options.agentFolder),
            agentFolder: options.agentFolder,
            fileName: AGENTS_FILENAME,
            virtualPath: promptProfileAgentsPath(options.agentFolder),
            content: options.groupContext ||
                defaultAgentsPromptMarkdown(options.agentName, relationshipMode, options.accessPreset === 'locked' ? 'locked' : 'full'),
            createdBy: 'runtime',
        ...
        await this.writeDefaultIfMissing({
            ...
            content: options.soul ||
                defaultSoulPromptMarkdown(options.agentName, relationshipMode),
            createdBy: 'runtime',
            metadata: { promptProfileKind: 'soul' },
        });
    }
    async compileSystemPrompt(options) {
        const sections = [];
        const accessPreset = options.accessPreset === 'locked' ? 'locked' : 'full';
        const runtimeRules = makeSection('RUNTIME_RULES', 'gantry://runtime-rules', [
            accessPreset === 'locked'
        ...
        if (runtimeRules)
            sections.push(runtimeRules);
        const personaSection = makeSection('PERSONA', PERSONA_SOURCE, personaPrompt(resolveAgentPersona(options.persona), accessPreset), this.sectionBudgets.PERSONA);
        if (personaSection)
            sections.push(personaSection);
        const appId = options.appId || this.appId;
        const agentId = options.agentId || promptProfileAgentIdForFolder(options.agentFolder);
        ...
        const capabilityGuidance = makeSection('CAPABILITY_GUIDANCE', CAPABILITY_GUIDANCE_SOURCE, renderedCapabilityGuidance.prompt, this.sectionBudgets.CAPABILITY_GUIDANCE);
        if (capabilityGuidance)
            sections.push(capabilityGuidance);
        const operatingGuidance = makeSection('OPERATING_GUIDANCE', OPERATING_GUIDANCE_SOURCE, accessPreset === 'locked'
            ? LOCKED_OPERATING_GUIDANCE_BLOCK
            : OPERATING_GUIDANCE_BLOCK, this.sectionBudgets.OPERATING_GUIDANCE);
        if (operatingGuidance)
            sections.push(operatingGuidance);
        if (agentInstructions)
            sections.push(agentInstructions);
        return this.composeWithinTotalBudget(sections);
    }
    async writeDefaultIfMissing(input) {
    ...
    async reseedMirrorIfMissing(store, input) {
    ...
    async readSoulSection(agentFolder, appId, agentId) {
    ...
    async readAgentInstructionsSection(agentFolder, appId, agentId) {
        if (!isValidPromptAgentFolder(agentFolder))
            return null;
        try {
            const raw = await this.readPromptArtifact({
            ...
            const content = truncateDeterministically(framed, this.sectionBudgets.AGENT_INSTRUCTIONS);
            if (!content)
                return null;
            return {
    ...
    async readPromptArtifact(input) {
        const store = this.fileArtifactStore();
        if (!store)
            return null;
        try {
    ...
    composeWithinTotalBudget(sections) {
        if (this.totalBudget <= 0 || sections.length === 0)
            return '';
        let output = '';
        for (const section of sections) {
            ...
            const nextBlock = block.length <= availableForBlock
                ? block
