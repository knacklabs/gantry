...
function brokerBindingFor(input) {
    ...
    return {
        ...
        ...('modelBatchRequestCount' in input && input.modelBatchRequestCount
            ...
            : {}),
        ...('modelBatchId' in input && input.modelBatchId
            ? { modelBatchId: input.modelBatchId }
            : {}),
...
export async function getAgentCredentialInjection(input) {
    ...
    try {
        ...
        const purpose = input.purpose ?? 'model_runtime';
        const suffix = purpose === 'model_runtime' || purpose === 'model_batch'
        ...
        throw new Error(`Gantry Model Gateway is enabled but is not reachable${suffix}. ${details.join(' ')}`, { cause: err });
    }
}
