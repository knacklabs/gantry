import { applyPermissionInteractionDecision, cancelPendingQuestionInteractionIfRunLeaseInactive, recordPendingInteractionRequested, releasePermissionInteractionCallback, resolvePendingInteractionRecord, resolvePendingInteractionRecordOutcome, } from './pending-interaction-durability.js';
...
const defaultOperations = {
    ...
    cancelPendingQuestionInteractionIfRunLeaseInactive: cancelPendingQuestionInteractionIfRunLeaseInactive,
};
...
export async function beginDurableQuestionInteraction(input) {
    const questionTexts = input.request.questions.map((question) => question.question);
    if (new Set(questionTexts).size !== questionTexts.length) {
        throw new Error('ask_user_question requires unique question text; duplicate question labels are not allowed');
    }
    ...
    if (!recorded)
        throw new Error('Question prompt was not durably recorded');
    if (typeof recorded === 'boolean' || recorded.id === interactionId) {
        return true;
    }
    if (recorded.status === 'pending') {
        const terminalized = await operations.cancelPendingQuestionInteractionIfRunLeaseInactive({
            id: recorded.id,
            resolution: {
                answers: {},
                reason: 'Runtime restarted while user question was pending; re-ask required.',
            },
        });
        if (!terminalized)
            return false;
...
function recordQuestionInteraction(input) {
    return input.operations.record({
        ...
        callbackRoute: input.callbackRoute === undefined
            ? {
                targetJid: input.request.targetJid ?? null,
                threadId: input.request.threadId ?? null,
            }
