...
export function readQuestionRecoveryEnvelope(value) {
    ...
    if (envelope.version !== 1 ||
        ...
        !envelope.completedQuestionIndexes.every((index) => Number.isInteger(index) && index >= 0)) {
        return null;
    }
    const request = envelope.request;
    if (envelope.completedQuestionIndexes.some((index) => index >= request.questions.length) ||
        envelope.selections.some((selection) => !selection ||
            ...
            selection.optionIndexes.some((index) => !Number.isInteger(index) ||
                index < 0 ||
