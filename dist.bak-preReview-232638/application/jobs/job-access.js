...
export function isDefaultRuntimeJobScope(appId) {
...
export async function filterJobsByCanonicalAppSession(input) {
    ...
    const sessionIds = Array.from(new Set(input.jobs
    ...
    const allowedSessionIds = new Set(sessions
        .filter((session) => session.appId === input.appId)
        .map((session) => session.sessionId));
    const sessionlessJobs = input.jobs.filter((job) => !job.session_id);
    const sessionlessConversationJids = Array.from(new Set(sessionlessJobs
        .map((job) => job.execution_context?.conversationJid?.trim())
        .filter((jid) => Boolean(jid))));
    const conversationSessions = sessionlessConversationJids.length > 0
        ? input.control.getAppSessionsByChatJids
            ? await input.control.getAppSessionsByChatJids(sessionlessConversationJids)
            : (await Promise.all(sessionlessConversationJids.map((jid) => input.control.getAppSessionByChatJid(jid)))).filter((session) => session !== undefined)
        : [];
    const allowedConversationJids = new Set(conversationSessions
        .filter((session) => session.appId === input.appId)
        .map(sessionConversationJid)
        .filter((jid) => Boolean(jid)));
    const knownConversationJids = new Set(conversationSessions
        .map(sessionConversationJid)
        .filter((jid) => Boolean(jid)));
    return input.jobs.filter((job) => job.session_id
...
function sessionConversationJid(session) {
