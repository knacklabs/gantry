...
import { isDefaultRuntimeJobScope, resolveJobAppSession, } from './job-access.js';
import { assertExecutionContextMatchesAuthenticatedContext, authenticatedContextFromAccess, normalizeExecutionContext, } from './job-management-helpers.js';
...
export async function resolveAuthenticatedRouteContextForUpdate(input) {
    if (input.access) {
        const authenticatedContext = authenticatedContextFromAccess(input.access, input.workspaceKey);
    ...
    if (!input.deps.control)
        return null;
    const requestedExecutionContext = input.patchExecutionContext !== undefined
        ? normalizeExecutionContext(input.patchExecutionContext)
        : undefined;
    const sessionId = normalizeOptional(requestedExecutionContext?.sessionId) ??
        normalizeOptional(input.job.session_id);
    if (!sessionId) {
        if (requestedExecutionContext) {
            throw new ApplicationError('FORBIDDEN', 'Cannot update executionContext without an authenticated app session.');
        }
        return null;
    }
    ...
    return {
        ...
        threadId: requestedExecutionContext?.threadId ??
            normalizeOptional(input.job.thread_id) ??
            null,
    };
}
export async function assertJobAppAccess(input) {
    if (!input.job.session_id && isDefaultRuntimeJobScope(input.appId))
        return;
    if (!input.deps.control) {
    ...
    if (appSession?.appId === input.appId)
        return;
    throw new ApplicationError('FORBIDDEN', 'API key cannot access this job');
}
