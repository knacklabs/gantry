...
import { normalizeExecutionContext, normalizeNotificationRoutes, assertPublicJobNamespace, requireJobNotificationRouteApproval, routesBeyondAuthenticatedContext, } from './job-management-helpers.js';
...
export async function createManagedJob(deps, input) {
    if (!deps.control) {
    ...
    if (!input.name.trim() || !input.prompt.trim() || !session) {
        throw new ApplicationError('INVALID_REQUEST', 'name, prompt, and sessionId are required');
    }
    ...
    const effectiveModelAlias = modelAlias ?? resolveRequestedJobModel(input.effectiveModelAlias, workload);
    ...
    if (executionContext.conversationJid !== sessionBoundContext.conversationJid ||
        executionContext.workspaceKey !== sessionBoundContext.workspaceKey) {
        throw new ApplicationError('FORBIDDEN', 'executionContext must match authenticated job context.');
    }
    if (executionContext.sessionId !== undefined &&
        executionContext.sessionId !== session.sessionId) {
        throw new ApplicationError('FORBIDDEN', 'executionContext.sessionId must match the authenticated app session.');
    }
    ...
    const notificationRoutes = normalizeNotificationRoutes(input.notificationRoutes ?? [
        {
