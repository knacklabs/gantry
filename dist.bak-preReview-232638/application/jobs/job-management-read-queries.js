...
export async function listManagedJobRuns(input) {
    ...
    const ownerAppId = !input.jobId && input.appId && !input.access ? input.appId : undefined;
    const visibleJobIds = input.jobId || ownerAppId
        ? undefined
    ...
    if (visibleJobIds?.length === 0)
        return { runs: [] };
    const runs = await input.deps.ops.listJobRuns(input.jobId, resolveLimit(input.limit, DEFAULT_RUN_LIMIT), input.jobId ? undefined : { jobIds: visibleJobIds, ownerAppId });
    return { runs };
}
export async function listManagedJobEvents(input) {
    ...
    if (input.eventType !== undefined && !eventType) {
        throw new ApplicationError('INVALID_REQUEST', `Unknown runtime event type "${input.eventType}".`);
    }
    if (input.jobId) {
        const job = await input.visibility.getVisibleJobForScopedRead({
            jobId: input.jobId,
    ...
    if (visibleJobIds?.length === 0)
        return { events: [] };
    const events = await input.deps.ops.listRecentJobEvents(resolveLimit(input.limit, DEFAULT_EVENT_LIMIT), {
        app_id: input.jobId || visibleJobIds || ownerAppId ? undefined : input.appId,
...
export async function listManagedDeadLetterRuns(input) {
    const runs = await input.deps.ops.listDeadLetterRuns(resolveLimit(input.limit, DEFAULT_DEAD_LETTER_LIMIT));
    if (!input.appId && !input.access)
        return { deadLetterRuns: runs };
