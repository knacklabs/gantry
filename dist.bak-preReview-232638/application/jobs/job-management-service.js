...
import { assertSchedulerJobAccess, normalizeOptional, } from './job-management-access.js';
import { createManagedJob } from './job-management-create.js';
import { assertExecutionContextMatchesAuthenticatedContext, assertPublicJobNamespace, authenticatedContextFromAccess, encodeTriggerRequester, normalizeNotificationRoutes, normalizeStoredNotificationRoutes, normalizeScheduleType, requireJobNotificationRouteApproval, resolveCanonicalAppSessionForOrigin, resolveLimit, routesBeyondAuthenticatedContext, } from './job-management-helpers.js';
import { assertJobModelHarnessCompatible, resolveRequestedJobModel, } from './job-model-selection.js';
// prettier-ignore
...
import { applyJobReadinessToUpdates, evaluateManagedJobReadiness, pauseJobForSetup, recordJobSetupRequired, setupBlockerDetails, } from './job-management-readiness.js';
...
export class JobManagementService {
    ...
    async upsertJobFromIpc(input) {
        ...
        if (!name || !prompt) {
            throw new ApplicationError('INVALID_REQUEST', 'scheduler_upsert_job requires name and prompt.');
        }
        if (input.scheduleType === undefined || input.scheduleType === null) {
            throw new ApplicationError('INVALID_REQUEST', 'scheduler_upsert_job requires scheduleType.');
        }
        ...
        const modelAlias = resolveRequestedJobModel(input.modelAlias, scheduleType === 'cron' || scheduleType === 'interval'
            ? 'recurring_job'
            : 'one_time_job');
        assertJobModelHarnessCompatible({
            modelAlias,
            workload: scheduleType === 'cron' || scheduleType === 'interval'
                ? 'recurring_job'
                : 'one_time_job',
            agentHarness: input.agentHarness,
        });
        const workspaceKey = (input.workspaceKey || access.sourceAgentFolder).trim();
        if (workspaceKey !== access.sourceAgentFolder) {
            throw new ApplicationError('FORBIDDEN', 'Scheduler jobs cannot be created outside the source group.');
        }
        ...
        const authenticatedContext = authenticatedContextFromAccess(access, workspaceKey);
        ...
        if (requestedJobId) {
            ...
            if (existingJob)
                assertSchedulerJobAccess(existingJob, access);
            id = requestedJobId;
        }
        ...
        const existingNotificationRoutes = normalizeStoredNotificationRoutes(existingJob?.notification_routes);
        const requestedNotificationRoutes = normalizeNotificationRoutes(input.notificationRoutes ??
        ...
        const accessRequirements = normalizeAccessRequirements(input.accessRequirements ?? []);
        const { canonicalSession } = await resolveCanonicalAppSessionForOrigin({
            access,
            control: this.deps.control,
        });
        const storedExecutionContext = canonicalSession?.sessionId && executionContext.sessionId == null
    ...
    async listJobs(input) {
        const queryWorkspaceKey = input.access
            ...
            : input.workspaceKey;
        const repositoryAppId = input.appId === DEFAULT_JOB_RUNTIME_APP_ID ? undefined : input.appId;
        const jobs = await this.deps.ops.listJobs({
            ...
            limit: Math.min(resolveLimit(input.limit, DEFAULT_JOB_LIST_LIMIT), MAX_JOB_LIST_LIMIT),
        });
        const appScopedJobs = input.appId && this.deps.control
            ...
            : jobs;
        const visibleJobs = appScopedJobs.filter((job) => isVisibleJob(job, {
            ...input,
    ...
    async triggerJob(input) {
        ...
        if (!appSession) {
            throw new ApplicationError('FORBIDDEN', 'API key cannot access this job session');
        }
        ...
        if (job.status === 'paused' || job.status === 'dead_lettered') {
            throw new ApplicationError('CONFLICT', `Cannot trigger job while status is ${job.status}; resume the job explicitly first.`);
        }
        const readiness = await evaluateManagedJobReadiness({
            deps: this.deps,
            job,
        ...
        if (!readiness.ready) {
            ...
            throw new ApplicationError('CONFLICT', 'Job requires setup before it can be triggered.', { details: setupBlockerDetails(readiness.setupState) });
        }
        if (input.consumeRateLimit &&
            (!input.consumeRateLimit(`app:${input.appId}:job:${job.id}`, input.perJobLimit) ||
                !input.consumeRateLimit(`app:${input.appId}`, input.perAppLimit))) {
            throw new ApplicationError('RATE_LIMITED', 'Too many job trigger requests');
        }
        ...
        try {
            ...
            throw new ApplicationError('SCHEDULER_NOT_READY', err instanceof Error
                ? err.message
                : 'Scheduler is not ready to accept job triggers');
        }
    ...
    async waitForTrigger(input) {
        return waitForTriggerCompletion({
            ...
            assertJobAppAccess: (job, appId) => assertJobAppAccess({ deps: this.deps, job, appId }),
        });
    }
