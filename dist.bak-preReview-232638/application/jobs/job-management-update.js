...
import { assertSchedulerJobAccess, validateSchedulerUpdate, } from './job-management-access.js';
import { buildJobUpdates, assertPublicJobNamespace, normalizeExecutionContext, normalizeNotificationRoutes, normalizeStoredNotificationRoutes, requireJobNotificationRouteApproval, routesBeyondAuthenticatedContext, } from './job-management-helpers.js';
import { normalizeAccessRequirementsInput } from './job-access-requirements.js';
import { applyJobReadinessToUpdates, evaluateManagedJobReadiness, recordJobSetupRequired, } from './job-management-readiness.js';
import { assertJobAppAccess, resolveAuthenticatedRouteContextForUpdate, } from './job-management-context-access.js';
import { assertJobModelHarnessCompatible, resolveOptionalJobModel, } from './job-model-selection.js';
export async function updateManagedJob(deps, input, clock) {
    ...
    const targetWorkload = targetScheduleType === 'cron' || targetScheduleType === 'interval'
        ? 'recurring_job'
        : 'one_time_job';
    ...
    const normalizedExecutionContext = patch.executionContext === undefined
        ? undefined
        : normalizeExecutionContext(patch.executionContext);
    if (normalizedExecutionContext && !authenticatedContext) {
        throw new ApplicationError('FORBIDDEN', 'Cannot authorize executionContext changes without authenticated job context.');
    }
    if (normalizedExecutionContext &&
        ...
        throw new ApplicationError('FORBIDDEN', 'executionContext must match the authenticated job context.');
    }
    const normalizedNotificationRoutes = patch.notificationRoutes === undefined
        ? undefined
        : normalizeNotificationRoutes(patch.notificationRoutes);
    const routeAuthorizationContext = authenticatedContext ??
        defaultRuntimeSameConversationRouteContext({
            appId: input.appId,
            job,
            routes: normalizedNotificationRoutes,
        });
    const normalizedAccessRequirements = normalizeAccessRequirementsInput(patch.accessRequirements, 'accessRequirements');
    if (normalizedNotificationRoutes) {
        if (!routeAuthorizationContext) {
            throw new ApplicationError('FORBIDDEN', 'Cannot authorize notification route changes without authenticated job context.');
        }
        await requireJobNotificationRouteApproval({
            ...
            request: {
                ...
                existingRoutes: normalizeStoredNotificationRoutes(job.notification_routes),
    ...
    const updates = buildJobUpdates(job, {
        ...patch,
    ...
    if (mergedForReadiness.status === 'active' ||
        normalizedAccessRequirements !== undefined) {
...
function defaultRuntimeSameConversationRouteContext(input) {
    if (input.appId !== 'default' || !input.routes?.length)
        return null;
    const existingConversationJid = input.job.execution_context?.conversationJid ??
        input.job.notification_routes?.[0]?.conversationJid;
    if (!existingConversationJid)
        return null;
    if (input.routes.some((route) => route.conversationJid !== existingConversationJid)) {
        return null;
    }
    const targetThreadId = input.routes.length === 1 ? (input.routes[0]?.threadId ?? null) : null;
    if (input.routes.some((route) => (route.threadId ?? null) !== targetThreadId)) {
        return null;
    }
    return {
...
async function requireJob(deps, jobId) {
    const job = await deps.ops.getJobById(jobId);
    if (!job)
        throw new ApplicationError('NOT_FOUND', 'Job not found');
    return job;
}
async function assertAccess(deps, job, input) {
    if (input.appId)
        await assertJobAppAccess({ deps, job, appId: input.appId });
    if (input.access)
