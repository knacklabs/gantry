...
import { isCanonicalBrowserCapabilityRule, isGantryFacadeExactToolRule, parseReadableScopedToolRule, RUN_COMMAND_TOOL_NAME, } from '../../shared/agent-tool-references.js';
...
import { bashExecutableName, formatBashArgv, parseBashCommand, } from '../../shared/bash-command-parser.js';
export function normalizeToolAccessRequirements(values, fieldName = 'toolAccessRequirements') {
    const out = [];
    const seen = new Set();
    for (const raw of values) {
        const rule = typeof raw === 'string' ? raw.trim() : '';
        if (!rule) {
            throw new ApplicationError('INVALID_REQUEST', `${fieldName} entries must be non-empty strings.`);
        }
        const validation = validateDurableAccessRule(rule, {
            allowUnknownSemanticCapability: true,
        });
        if (!validation.ok) {
            throw new ApplicationError('INVALID_REQUEST', `${fieldName} entry "${rule}" is not supported: ${validation.reason}`);
        }
        if (!seen.has(rule)) {
            seen.add(rule);
            out.push(rule);
...
export function normalizeRequiredMcpServers(values, fieldName = 'requiredMcpServers') {
    const out = [];
    const seen = new Set();
    for (const raw of values) {
...
export function evaluateToolAccessRequirements(input) {
    const toolAccessRequirements = normalizeToolAccessRequirements(input.toolAccessRequirements ?? []);
...
function matchingAllowedRule(required, allowed) {
    for (const allowedRule of allowed) {
        ...
        const projectedRequired = absoluteRunCommandRuleForBareExecutableRule(allowedRule, required);
...
function absoluteRunCommandRuleForBareExecutableRule(allowedRule, requiredRule) {
    const allowed = parseReadableScopedToolRule(allowedRule);
    const required = parseReadableScopedToolRule(requiredRule);
    if (allowed?.toolName !== RUN_COMMAND_TOOL_NAME ||
...
export function toolAccessRequirementRecoveryAction(toolName) {
...
export function missingToolAccessRequirementError(toolName) {
