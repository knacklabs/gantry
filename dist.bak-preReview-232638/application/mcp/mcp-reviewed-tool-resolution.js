...
import { isReviewedMcpToolAllowed, } from './mcp-tool-authorization.js';
export async function resolveReviewedMcpTool(input) {
    const capability = input.capabilities.find((candidate) => candidate.name === input.serverName);
    if (!capability) {
...
function reviewedCapabilityDenialGuidance(capability) {
    ...
    const label = capabilityIds.length === 1
