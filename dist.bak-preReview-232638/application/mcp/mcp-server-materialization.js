...
export function materializeMcpRecord(record, credentialEnv) {
    const config = record.definition.config;
    const credentialValues = resolveCredentialValues(record.definition.credentialRefs, credentialEnv);
    ...
    const allowedToolPatterns = bindingPatterns.length > 0 ? bindingPatterns : definitionPatterns;
    const allowedToolNames = allowedToolPatterns.map((tool) => `mcp__${record.definition.name}__${tool}`);
    const autoApproveToolNames = record.definition.autoApproveToolPatterns.map((tool) => `mcp__${record.definition.name}__${tool}`);
    ...
    if (!template) {
        throw new ApplicationError('INVALID_REQUEST', 'Stored MCP stdio_template config references an unsupported templateId.');
    }
    const env = {
        ...(config.env ?? {}),
...
function mcpSourceRevision(record) {
...
function resolveCredentialValues(refs, credentialEnv) {
    const env = {};
    const headers = {};
    for (const ref of refs) {
        const value = credentialEnv[ref.name];
        if (!value) {
            throw new ApplicationError('INVALID_REQUEST', formatMissingGantrySecretsMessage([ref.name]));
        }
        if (ref.target === 'env')
            env[ref.key] = value;
        else
