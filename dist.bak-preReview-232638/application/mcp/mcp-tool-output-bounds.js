...
export function boundMcpToolResultForReturn(result) {
...
export function serializeMcpToolResult(result, maxChars = MAX_MCP_TOOL_RESULT_CHARS) {
...
function boundMcpToolResultText(text, maxChars) {
...
function boundMcpToolJsonValue(value, state) {
    ...
    if (typeof value === 'function' || typeof value === 'symbol') {
        const bounded = boundMcpToolJsonString('[unsupported MCP result value]', state);
    ...
    try {
        return Array.isArray(value)
            ...
            : boundMcpToolJsonObject(value, state);
    }
    finally {
        state.seen.delete(objectValue);
    }
}
function boundMcpToolJsonArray(value, state) {
    const output = [];
    let truncated = false;
    const childState = {
        ...state,
...
function boundMcpToolJsonObject(value, state) {
    const output = {};
    let truncated = false;
    let itemCount = 0;
    const childState = {
    ...
    for (const key in value) {
        if (!Object.hasOwn(value, key))
            continue;
        if (itemCount >= MAX_MCP_TOOL_RESULT_COLLECTION_ITEMS ||
...
function boundMcpToolJsonString(value, state) {
...
function spendMcpToolResultBudget(state, chars) {
