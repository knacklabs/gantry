...
export function assertMcpNetworkHostAllowed(input) {
    ...
    if (deny) {
        throw new ApplicationError('FORBIDDEN', `Network access denied: MCP server ${input.serverName} host ${hostLabel} matches the egress denylist.`);
    }
}
...
export function isLocalLoopbackHttpMcpUrl(url) {
