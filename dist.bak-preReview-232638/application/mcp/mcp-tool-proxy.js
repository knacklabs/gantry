...
import { assertRemoteMcpDestinationPublic, } from './mcp-server-policy.js';
import { assertMcpNetworkHostAllowed, createGuardedMcpFetch, isLocalLoopbackHttpMcpUrl, } from './mcp-tool-proxy-network.js';
import { isReviewedMcpToolAllowed, isSourceInventoryToolAllowed, } from './mcp-tool-authorization.js';
import { resolveReviewedMcpTool } from './mcp-reviewed-tool-resolution.js';
import { materializeReviewedMcpCapabilities, materializeSourceMcpCapabilities, } from './mcp-tool-proxy-capabilities.js';
import { cacheMcpInventory, compareMcpToolSearchResults, invalidateMcpToolProxyInventoryCacheForCapability, listedMcpTool, mcpToolMatchesQuery, normalizeMcpListCursor, normalizeMcpListLimit, readCachedMcpInventory, readCachedMcpToolDetail, } from './mcp-tool-inventory.js';
import { classifyMcpToolAuditError, summarizeMcpToolArguments, summarizeMcpToolError, } from './mcp-tool-audit.js';
...
import { fetchAndCacheMcpToolDetail, resolveMcpToolOutputSchema, } from './mcp-tool-detail-fetch.js';
import { boundMcpToolResultForReturn } from './mcp-tool-output-bounds.js';
import { cacheMcpClient, closeCachedMcpClient, releaseMcpClient, readCachedMcpClient, retainMcpClient, scheduleMcpClientIdleClose, } from './mcp-tool-proxy-client-cache.js';
...
export class McpToolProxy {
    ...
    async listTools(input) {
        ...
        const matchingCapabilities = capabilities.filter((capability) => !input.serverName || capability.name === input.serverName);
        const shouldFetchUncached = Boolean(input.serverName) ||
            Boolean(input.query?.trim()) ||
            matchingCapabilities.length <= 1;
        const connectedServerNames = [];
        const allowedToolCountByServer = new Map();
        const diagnostics = {
        ...
        for (let offset = 0; offset < uncachedCapabilities.length; offset += MCP_INVENTORY_SEARCH_CONCURRENCY) {
            const fetched = await Promise.all(uncachedCapabilities
                ...
                try {
                    const inventory = await Promise.race([
                        this.fetchAndCacheInventory(input, capability, controller.signal),
        ...
        if (!query) {
            for (const serverName of connectedServerNames) {
                if ((allowedToolCountByServer.get(serverName) ?? 0) === 0 &&
                    !serversByName.has(serverName)) {
                    serversByName.set(serverName, { name: serverName, tools: [] });
                }
            }
        }
        const nextOffset = cursor + limit < filteredTools.length
            ? String(cursor + limit)
            : undefined;
        return {
    ...
    async searchTools(input) {
        ...
        const reviewedByServer = new Map(reviewedCapabilities.map((capability) => [capability.name, capability]));
        const matches = listed.servers
            ...
            .sort((left, right) => compareMcpToolSearchResults({ serverName: left.serverName, tool: left }, { serverName: right.serverName, tool: right }, input.query));
        return {
            query: input.query,
            limit: listed.limit,
            total: listed.total,
    ...
    async describeTool(input) {
        const capabilities = await this.materializeSourceCapabilities(input);
        const capability = capabilities.find((candidate) => candidate.name === input.serverName);
        if (!capability) {
            throw new ApplicationError('NOT_FOUND', `MCP server is not approved for this agent: ${input.serverName}`);
        }
        if (!isSourceInventoryToolAllowed(capability, input.toolName)) {
            throw new ApplicationError('NOT_FOUND', `MCP tool is not available from source inventory: ${input.serverName}.${input.toolName}`);
        }
    ...
    async callTool(input) {
        ...
        let finalized = false;
        let selectedToolRule;
        let selectedCapability;
        const finalize = async (resultClass, extra = {}) => {
            await this.publishMcpToolActivity({
                input,
                resultClass,
        ...
        try {
            ...
            try {
                ...
                try {
                    await finalize(validationAudit.toolResultError ? 'failure' : 'success', validationAudit);
                }
                catch {
            ...
            if (!finalized) {
                const finalizeFailure = () => finalize(classifyMcpToolAuditError(err), {
    ...
    async assertToolAllowed(input) {
        const argumentSummary = summarizeMcpToolArguments(input.arguments ?? {});
        const startedAt = Date.now();
        await this.resolveReviewedTool(input, (resultClass, extra = {}) => this.publishMcpToolActivity({
            input,
            resultClass,
            latencyMs: Date.now() - startedAt,
            argumentSummary,
    ...
    async fetchAndCacheInventory(input, capability, signal) {
        const client = await this.connect(capability);
        try {
            ...
            for (const tool of tools.tools) {
                if (!isSourceInventoryToolAllowed(capability, tool.name))
                    continue;
                totalAllowed += 1;
    ...
    async publishMcpToolActivity(input) {
        const payload = {
        ...
        if (!this.options.publishRuntimeEvent)
            return;
        try {
    ...
    async materializeSourceCapabilities(input) {
    ...
    async materializeReviewedCapabilities(input) {
    ...
    async resolveReviewedTool(input, finalizeDenied) {
    ...
    async connect(capability) {
        ...
        const cached = readCachedMcpClient(capability);
        if (cached)
            return cached;
        const client = new Client({ name: 'gantry-mcp-proxy', version: '1.0.0' }, {
            capabilities: {},
            listChanged: {
                tools: {
                    autoRefresh: false,
        ...
        const existing = readCachedMcpClient(capability);
        if (existing) {
            await client.close();
            return existing;
        }
    ...
    async createTransport(capability) {
        const config = capability.config;
        if (config.type === 'http' || config.type === 'sse') {
            if (!isLocalLoopbackHttpMcpUrl(new URL(config.url))) {
                await assertRemoteMcpDestinationPublic({ transport: config.type, url: config.url, headers: config.headers }, this.options.lookupHostname, { cache: this.options.dnsValidationCache });
            }
        ...
        throw new ApplicationError('FORBIDDEN', 'stdio_template MCP servers are approved durable capabilities, but current-session proxy execution is disabled until sandboxed stdio execution is implemented.');
    }
    assertNetworkAllowedForCapability(capability) {
        const config = capability.config;
        if (config.type !== 'http' && config.type !== 'sse')
            return;
