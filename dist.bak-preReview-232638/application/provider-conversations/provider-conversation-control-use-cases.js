...
const SECRET_KEY_PATTERN = /(token|secret|password|credential|api[_-]?key|app[_-]?token|bot[_-]?token)/i;
function assertNoRawSecrets(value, path) {
    ...
    for (const [key, nested] of Object.entries(value)) {
        if (SECRET_KEY_PATTERN.test(key)) {
            throw new ApplicationError('INVALID_REQUEST', `${path}.${key} looks like a raw secret. Store channel credentials behind runtimeSecretRefs.`);
        }
        assertNoRawSecrets(nested, `${path}.${key}`);
    }
}
function assertAllowedRuntimeSecretRefs(provider, refs) {
    if (refs === undefined)
        return;
    const allowed = new Set(provider.allowedRuntimeSecretKeys ?? []);
    const invalid = Object.keys(refs).filter((key) => !allowed.has(key));
    if (invalid.length > 0) {
        throw new ApplicationError('INVALID_REQUEST', `runtimeSecretRefs contains unsupported keys for provider ${provider.id}: ${invalid.join(', ')}`);
    }
    for (const [key, ref] of Object.entries(refs)) {
        try {
            const normalized = normalizeRuntimeSecretRefString(ref, `runtimeSecretRefs.${key}`);
            const parsed = parseRuntimeSecretRefString(normalized);
            if (parsed.source === 'env' &&
                isForbiddenRuntimeSecretEnvName(parsed.name)) {
                throw new Error(`${parsed.name} is not allowed for provider '${provider.id}' runtime secret ref ${normalized}. Use a channel runtime secret name, not model/provider credential authority.`);
            }
            if (!isProviderRuntimeSecretRefTarget(provider.id, key, normalized)) {
                throw new Error(`runtimeSecretRefs.${key} must point to the canonical ${provider.id} credential for ${key}.`);
            }
            refs[key] = normalized;
        }
        catch (error) {
            throw new ApplicationError('INVALID_REQUEST', error instanceof Error
                ? error.message
                : `Invalid runtimeSecretRefs.${key}`);
        }
    }
}
function explicitProviderIdForExternalId(externalId) {
...
function normalizeProviderAccountStatus(status) {
    if (!status)
        return undefined;
...
function assertOwnedProviderAccount(providerAccount, appId) {
    if (providerAccount.appId !== appId) {
        throw new ApplicationError('FORBIDDEN', 'API key cannot access this provider account');
    }
}
function memorySubjectForScope(input) {
    if (input.memorySubject)
        return input.memorySubject;
    switch (input.memoryScope) {
        ...
        case 'user':
            throw new ApplicationError('INVALID_REQUEST', 'memoryScope=user requires an explicit user memorySubject');
    }
}
export class ProviderAccountControlService {
    ...
    async get(input) {
        const providerAccount = await this.deps.providerAccounts.getProviderAccount(input.providerAccountId);
        if (!providerAccount) {
    ...
    async create(input) {
        ...
        if (!provider || provider.capabilityFlags.includes('placeholder')) {
            throw new ApplicationError('NOT_IMPLEMENTED', `Provider ${input.providerId} is not implemented`);
        }
        ...
        if (agent.appId !== input.appId) {
            throw new ApplicationError('FORBIDDEN', 'API key cannot access this agent');
        }
    ...
    async update(input) {
        ...
        const providers = await this.deps.providers.listProviders();
        const provider = providers.find((entry) => entry.id === existing.providerId);
        assertAllowedRuntimeSecretRefs(provider ?? {
...
export class DiscoverProviderConversationsService {
    ...
    async execute(input) {
        ...
        for (const item of discovered) {
            const explicitProviderId = explicitProviderIdForExternalId(item.externalId);
            if (explicitProviderId &&
                explicitProviderId !== providerAccount.providerId) {
                throw new ApplicationError('INVALID_REQUEST', `Discovered conversation externalId "${item.externalId}" has provider prefix "${explicitProviderId}:" but provider account ${providerAccount.id} is ${providerAccount.providerId}.`);
            }
            const existing = await this.deps.conversations.getConversationByExternalRef({
                appId: input.appId,
            ...
            const conversation = {
                id: existing?.id ??
                    `conversation:${providerAccount.id}:${item.externalId}`,
                appId: input.appId,
                providerAccountId: providerAccount.id,
...
export class ConversationInstallControlService {
    ...
    async list(input) {
        ...
        return await this.deps.providerAccounts.listConversationInstalls(input.appId, input.agentId);
    }
    async enable(input) {
    ...
    async disable(input) {
        ...
        const disabled = await this.deps.providerAccounts.disableConversationInstall({
            appId: input.appId,
            agentId: input.agentId,
            conversationId: input.conversationId,
    ...
    async upsert(input) {
        ...
        const providerAccountId = input.patch.providerAccountId ?? conversation.providerAccountId;
        if (providerAccountId !== conversation.providerAccountId) {
            throw new ApplicationError('INVALID_REQUEST', 'Conversation install provider account must match the conversation provider account');
        }
        const providerAccount = await this.deps.providerAccounts.getProviderAccount(providerAccountId);
        if (!providerAccount) {
            throw new ApplicationError('NOT_FOUND', 'provider account not found');
        }
        assertOwnedProviderAccount(providerAccount, input.appId);
        if (providerAccount.agentId !== input.agentId) {
            throw new ApplicationError('FORBIDDEN', 'Provider account is owned by a different agent');
        }
        ...
        const memoryScope = input.patch.memoryScope ?? existing?.memoryScope ?? 'conversation';
        ...
        const install = {
            ...
            memoryScope,
            memorySubject,
            workspaceSnapshotId: input.patch.workspaceSnapshotId === null
                ? undefined
                : (input.patch.workspaceSnapshotId ?? existing?.workspaceSnapshotId),
            permissionPolicyIds: input.patch.permissionPolicyIds ?? existing?.permissionPolicyIds ?? [],
            createdAt: existing?.createdAt ?? now,
            updatedAt: now,
        };
    ...
    async assertAgent(appId, agentId) {
