...
export class MemoryLlmBrainSynthesis {
...
export function parseBrainSynthesisOutput(text, fallback, results) {
    ...
    try {
        ...
        const answer = typeof parsed.answer === 'string' && parsed.answer.trim()
            ? parsed.answer.trim()
            : fallback.answer;
        const gaps = typeof parsed.gaps === 'string' && parsed.gaps.trim()
            ? parsed.gaps.trim()
            : fallback.gaps;
        const allowedPages = new Map(results.map((result) => [result.page.id, result.page]));
...
function fallbackSynthesis(input) {
    ...
    return {
        answer: input.results[0]?.snippet ||
            'No matching company brain pages were found.',
        citations,
        gaps: input.results.length > 0
