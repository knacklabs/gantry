...
export function discordActionComponents(options) {
    const stopAction = options?.actionAffordances?.find((action) => action.kind === 'live_turn_stop');
    const runNowAction = options?.actionAffordances?.find((action) => action.kind === 'scheduler_run_now' && action.jobId.trim());
    const buttons = [];
...
export function questionComponents(request, questionIndex, providerAlias) {
...
export function permissionCustomId(providerAlias, mode) {
    return `${PERMISSION_CUSTOM_ID_PREFIX}${providerAlias}:${mode}`;
}
export function parsePermissionCustomId(customId) {
...
export function questionCustomId(providerAlias, optionIndex) {
    return `${QUESTION_CUSTOM_ID_PREFIX}${providerAlias}:${optionIndex}`;
}
export function questionDoneCustomId(providerAlias) {
    return questionCustomId(providerAlias, -1);
}
export function parseQuestionCustomId(customId) {
