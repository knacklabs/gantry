...
export async function hydrateDiscordConversationContext(input) {
    const requestedChannelId = discordChannelIdFromJid(input.request.conversationJid);
    ...
    try {
        ...
        const rawMessages = await input.requestJson(`/channels/${encodeURIComponent(targetChannelId)}/messages?${query.toString()}`, { method: 'GET', headers: input.headers(input.botToken) }, 'Discord message history request failed');
        const threadRootMessage = input.request.threadId
            ? await fetchDiscordThreadRootMessage(input)
            : null;
        const firstReplyLimit = input.request.threadId && limit > 0
            ? Math.min(DISCORD_THREAD_FIRST_REPLY_LIMIT, Math.max(0, limit - latestLimit - (threadRootMessage ? 1 : 0)))
            : 0;
        const firstReplyMessages = firstReplyLimit > 0
        ...
        const fallbackThreadId = input.request.threadId || context.threadId || undefined;
        const fallbackConversationJid = input.request.threadId && !context.threadId
...
async function fetchDiscordThreadFirstReplies(input, limit) {
    ...
    try {
        ...
        return await input.requestJson(`/channels/${encodeURIComponent(threadId)}/messages?${query.toString()}`, { method: 'GET', headers: input.headers(input.botToken) }, 'Discord first thread replies request failed');
    }
    catch (err) {
        logger.debug({
            providerId: 'discord',
...
async function fetchDiscordThreadRootMessage(input) {
    ...
    if (!threadId)
        return null;
    try {
        return await input.requestJson(`/channels/${encodeURIComponent(threadId)}/messages/${encodeURIComponent(threadId)}`, { method: 'GET', headers: input.headers(input.botToken) }, 'Discord thread root message request failed');
    }
    catch (err) {
        logger.debug({
            providerId: 'discord',
...
export async function resolveDiscordConversationContext(input) {
    ...
    try {
        const info = await input.requestJson(`/channels/${encodeURIComponent(input.channelId)}`, { method: 'GET', headers: input.headers(input.botToken) }, 'Discord channel lookup failed');
        const context = info.parent_id && DISCORD_PUBLIC_THREAD_TYPES.has(info.type ?? -1)
            ? {
...
function normalizeDiscordContextMessages(input) {
    ...
    for (const message of input.rawMessages) {
        if (byExternalId.size >= input.limit)
            break;
        if (!message.id)
            continue;
...
export function discordMessageAttachments(message) {
...
function userName(user, fallback = 'unknown') {
    return user?.username || user?.id || fallback;
}
function discordChannelIdFromJid(jid) {
