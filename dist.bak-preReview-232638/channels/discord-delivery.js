...
import { agentTodoLines, formatAgentProgressLine, formatAgentTodoHeader, hasAgentTodoCardHeader, } from './agent-todo-render.js';
...
export function splitDiscordText(text) {
    const value = text || ' ';
    const parts = [];
    for (let index = 0; index < value.length; index += DISCORD_MESSAGE_MAX_LENGTH) {
        parts.push(value.slice(index, index + DISCORD_MESSAGE_MAX_LENGTH));
    }
    return parts.length ? parts : [' '];
}
export function formatDiscordAgentTodo(render) {
    if (render.cardKind === 'progress')
        return formatAgentProgressLine(render);
...
export async function postDiscordMessageParts(input) {
    ...
    const oversized = input.files?.filter((file) => file.sizeBytes > DISCORD_FILE_MAX_BYTES) ??
        [];
    ...
    for (let index = 0; index < parts.length; index += 1) {
        ...
        try {
            ...
            const canUploadFiles = index === parts.length - 1 &&
            ...
            if (posted.id)
                externalMessageIds.push(posted.id);
            deliveredParts += 1;
        }
        catch (err) {
            if (deliveredParts > 0) {
    ...
    if (oversized.length > 0) {
        const warning = await input
            ...
            content: oversized
                .map((file) => `Attachment unavailable in Discord: ${file.filename} exceeds 25 MB.`)
                .join('\n'),
            allowed_mentions: { parse: [] },
        })
            .catch(() => undefined);
        if (warning?.id)
            externalMessageIds.push(warning.id);
    }
    return {
...
async function postDiscordFilesOrTextFallback(input, body) {
    try {
        return await postDiscordFiles(input.apiRoot, input.botToken, input.channelId, body, input.files ?? []);
    }
    catch {
...
async function postDiscordFiles(apiRoot, botToken, channelId, body, files) {
    const uploads = files.filter((file) => file.sizeBytes <= DISCORD_FILE_MAX_BYTES);
    if (uploads.length === 0)
        return { id: undefined };
    ...
    uploads.forEach((file, index) => {
        form.set(`files[${index}]`, new Blob([file.content], { type: file.contentType }), file.filename);
    });
    const response = await fetch(`${apiRoot}/channels/${encodeURIComponent(channelId)}/messages`, {
        method: 'POST',
        headers: { authorization: `Bot ${botToken}`, accept: 'application/json' },
        body: form,
    });
    if (!response.ok)
        throw new Error('Discord file upload failed');
