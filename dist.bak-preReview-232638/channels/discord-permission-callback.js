import { claimPermissionInteractionCallback, recoverDurablePermissionDecision, releasePermissionInteractionCallback, } from '../application/interactions/pending-interaction-durability.js';
import { decisionForMode, permissionDecisionOptions, } from './permission-interaction.js';
...
import { consume, settle, } from './discord-permission-prompt-settlement.js';
export async function handleDiscordPermissionCallback(input) {
    ...
    if (!(await settle(input.pendingPermissions, parsed.providerAlias, decision, input))) {
...
async function recoverDurablePermission(input) {
    ...
    await recoverDurablePermissionDecision({
        ...
        authorize: (durable) => input.isApproverAllowed(input.userId, durable.sourceAgentFolder, durable.decisionPolicy, durable.threadId ?? undefined, durable.approvalContextJid ?? undefined),
        terminalize: async (receipt) => {
            ...
            const response = await fetch(`${DISCORD_API_ROOT}/channels/${encodeURIComponent(channelId)}/messages/${encodeURIComponent(messageId)}`, {
                method: 'PATCH',
