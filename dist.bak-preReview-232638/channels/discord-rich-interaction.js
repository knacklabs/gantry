...
import { isRichForm, richArrayItems, richDescriptor, RICH_INTERACTION_FALLBACK_COPY, RICH_INTERACTION_OPEN_FORM_LABEL, richFallbackText, richTextLines, richTruncate, } from './rich-interaction.js';
export const DISCORD_RICH_FORM_OPEN_PREFIX = 'gantry:rich_form_open:';
export function buildDiscordRichInteractionPayload(input) {
                ...
                components: [
                    {
                        ...
                        style: 1,
                        custom_id: `${DISCORD_RICH_FORM_OPEN_PREFIX}${item.id}`.slice(0, 100),
                    },
                ],
            },
        ]
        ...
        embeds: [
            {
                ...
                description: richTruncate(richLines.slice(1).join('\n') || richFallbackText(input), 4096),
                fields,
            },
        ],
...
export function buildDiscordRichInteractionFormModalResponse(input) {
    const item = richDescriptor(input);
    ...
    return {
        ...
        data: {
            ...
            components: (fields.length
                ? fields
                : [{ label: RICH_INTERACTION_OPEN_FORM_LABEL }]).map((field, index) => ({
                type: 1,
                components: [
                    {
                        type: 4,
                        custom_id: `field_${index}`,
                        label: richTruncate(String(field.label || field.id || `Field ${index + 1}`), 45),
...
export async function renderDiscordRichInteraction(input) {
    const { jid, channelId, render, richForms, postMessage, sendFallback } = input;
    if (!channelId)
        return false;
    try {
        if (isRichForm(render))
            richForms.set(render.descriptor.id, render);
...
export async function openDiscordRichFormInteraction(input) {
    const { apiRoot, headers, interaction, customId, richForms, ackInteraction } = input;
    ...
    await fetch(`${apiRoot}/interactions/${encodeURIComponent(interaction.id || '')}/${encodeURIComponent(interaction.token || '')}/callback`, {
        method: 'POST',
        headers,
        body: JSON.stringify(buildDiscordRichInteractionFormModalResponse(request)),
    });
    richForms.delete(id);
}
