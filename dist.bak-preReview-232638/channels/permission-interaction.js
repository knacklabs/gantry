...
import { isCanonicalBrowserCapabilityRule, isThirdPartyMcpToolRule, parseReadableScopedToolRule, publicGantryToolNameForSdkTool, } from '../shared/agent-tool-references.js';
...
import { buildPermissionPromptFullView, formatInteractionDetailLine as formatPromptInteractionDetailLine, formatInteractionFileLines as formatPromptInteractionFileLines, } from './permission-full-view.js';
export { buildPermissionPromptFullView, } from './permission-full-view.js';
import { formatPermissionAgentDisplayName, permissionPromptTitle, } from './permission-agent-display.js';
import { formatPermissionToolInputLines, permissionRiskLines, runtimeDisplayCommand, } from './permission-tool-input-format.js';
import { limitPermissionMessage, sanitizePermissionCommandText, sanitizePermissionText, sanitizeReceiptDetail, } from './permission-text-sanitizer.js';
import { decisionForPermissionInteraction, buildPermissionBatchPromptParts, formatPermissionBatchPromptText, isPermissionBatchRequest, permissionBatchButtonLabel, withRecoveredBatchOption, } from './permission-batch-coalescer.js';
...
export function normalizePermissionAction(action) {
    if (action === 'allow_once')
        return 'allow_once';
    if (action === 'allow_persistent_rule')
        return 'allow_persistent_rule';
    if (action === 'cancel')
        return 'cancel';
    return null;
}
export function permissionDecisionOptions(request, matchKind) {
    ...
    if (!requested?.length && !rule)
        logOptionDrop(request);
    return withRecoveredBatchOption(options, matchKind);
}
function logOptionDrop(request) {
...
function persistentOptionDropReason(request) {
    ...
    if (candidates.length !== 1)
        return 'expected exactly one allow rule update';
    if (!candidates[0].rules?.length)
        return 'expected at least one rule';
...
export function permissionButtonLabel(mode, _request) {
...
export function formatPermissionPromptText(request, timeoutMs, options = {}) {
    const batchText = formatPermissionBatchPromptText(request, timeoutMs);
    if (batchText)
        return limitPermissionMessage(batchText);
    const timeoutMinutes = Math.max(1, Math.round(timeoutMs / 60000));
    if (request.interaction) {
        return formatInteractionPermissionPrompt(request, timeoutMinutes, options.budget);
    }
    const rule = firstPersistentRule(request);
    const capabilityName = semanticCapabilityName(request, rule);
    if (capabilityName) {
        return formatSemanticPermissionPrompt(request, capabilityName, timeoutMinutes, rule);
    }
    ...
    const inputLines = formatPermissionToolInputLines(request, sanitizePermissionText, { sanitizeCommandText: sanitizePermissionCommandText });
    if (inputLines.length > 0)
        lines.push('', ...inputLines);
    if (request.blockedPath)
        lines.push(`Path: ${sanitizePermissionText(request.blockedPath, 250, 100)}`);
...
export function formatPermissionReceiptText(_requestId, request, decision) {
    ...
    if (decision.mode === 'allow_persistent_rule') {
        ...
        const agentName = request
            ...
            : 'this agent';
        return limitPermissionMessage(`Allowed for future: ${summary}. Saved for ${agentName}. Manage access to revoke it later.`);
    }
    return limitPermissionMessage(`Allowed once: ${summary}. The agent will continue this request.`);
}
export const PERMISSION_GLYPH = '🔐';
export function buildPermissionPromptParts(request, timeoutMs) {
    ...
    if (request.interaction) {
        ...
        const rule = firstPersistentRule(request);
        const capabilityName = semanticCapabilityName(request, rule);
        const title = permissionPromptTitle(request.sourceAgentFolder, capabilityName ?? permissionAccessLabel(request));
        ...
        if (typeof accountLabel === 'string' && accountLabel.trim()) {
            bodyLines.push(`Account: ${sanitizePermissionText(accountLabel.trim(), 100, 40)}`);
        }
        if (interaction.body) {
            bodyLines.push(sanitizePermissionText(interaction.body, 500, 160));
        }
        if (interaction.details?.length) {
            bodyLines.push(...interaction.details.map((detail) => formatPromptInteractionDetailLine(detail.label, detail.value, detail.mono, sanitizePermissionText)));
        }
        if (interaction.files?.length) {
            bodyLines.push(...formatPromptInteractionFileLines(interaction.files, sanitizePermissionText));
        }
        return {
            title,
    ...
    if (capabilityName) {
        ...
        const bodyLines = permissionRiskLines(request);
        const accountLabel = definition?.accountLabel ?? request.toolInput?.accountLabel;
        if (typeof accountLabel === 'string' && accountLabel.trim()) {
            bodyLines.push(`Account: ${sanitizePermissionText(accountLabel.trim(), 100, 40)}`);
        }
    ...
    if (request.blockedPath) {
        bodyLines.push(`Path: ${sanitizePermissionText(request.blockedPath, 250, 100)}`);
    }
    return {
...
export function formatPermissionPromptPartsText(parts) {
    const lines = [`${PERMISSION_GLYPH} ${parts.title}`];
    if (parts.bodyLines.length > 0)
        lines.push('', ...parts.bodyLines);
    if (parts.contextLines.length > 0)
        lines.push('', ...parts.contextLines);
    lines.push(`Reply in ${parts.replyInMinutes}m`);
    return limitPermissionMessage(lines.join('\n'));
}
function stripFullPayloadBodyLines(lines) {
    const stripped = [];
...
function formatPermissionContextLines(request) {
    if (!request)
        return [];
    ...
    if (request.promotionHintCount) {
        lines.push(`You've allowed me to do this ${request.promotionHintCount} times — want me to stop asking?`);
    }
    lines.push('The agent cannot approve this itself.');
    return lines;
}
function formatInteractionPermissionPrompt(request, timeoutMinutes, budget) {
    const interaction = request.interaction;
    const rule = firstPersistentRule(request);
    const capabilityName = semanticCapabilityName(request, rule);
    const title = `🔐 ${permissionPromptTitle(request.sourceAgentFolder, capabilityName ?? permissionAccessLabel(request))}`;
    ...
    const accountLabel = request.toolInput?.accountLabel;
    if (typeof accountLabel === 'string' && accountLabel.trim()) {
        lines.push(`Account: ${sanitizePermissionText(accountLabel.trim(), 100, 40)}`);
    }
    if (interaction.body)
        lines.push('', sanitizePermissionText(interaction.body, 500, 160));
    if (interaction.details?.length) {
        lines.push('', ...interaction.details.map((detail) => formatPromptInteractionDetailLine(detail.label, detail.value, detail.mono, sanitizePermissionText)));
    }
    if (interaction.files?.length) {
        lines.push('', ...formatPromptInteractionFileLines(interaction.files, sanitizePermissionText));
    }
    lines.push('', ...formatPermissionContextLines(request));
    lines.push(`Reply in ${timeoutMinutes}m`);
    return limitPermissionMessage(lines.join('\n'), budget ??
...
function formatSemanticPermissionPrompt(request, capabilityName, timeoutMinutes, rule) {
    const definition = semanticCapabilityDefinition(request, rule);
    const lines = [
...
function semanticCapabilityNetworkLine(definition) {
    const hosts = [
        ...new Set((definition?.networkHosts ?? [])
            .map((host) => host.trim())
            .filter(Boolean)),
    ];
    if (hosts.length === 0)
...
function requestHasThreadRoute(request) {
    return (typeof request?.threadId === 'string' && request.threadId.trim() !== '');
}
function permissionAccessLabel(request) {
    if (!request)
    ...
    if (capabilityName)
        return capabilityName;
    const toolName = scopedRule?.toolName || requestedToolName || request.toolName;
...
function requestedToolNameFromInput(request) {
...
function humanizeMcpServerName(toolName) {
...
function semanticCapabilityName(request, rule) {
    const fromInteraction = request.interaction?.requestContext?.capabilityDisplayName?.trim();
    const definition = semanticCapabilityDefinition(request, rule);
    if (definition)
        return definition.displayName;
    const capabilityId = semanticCapabilityId(request, rule);
    if (!capabilityId && rule)
        return undefined;
    if (capabilityId?.startsWith('skill.'))
        return undefined;
    if (fromInteraction)
        return sanitizePermissionText(fromInteraction, 120, 40);
...
function semanticCapabilityDefinition(request, rule) {
    const capabilityId = semanticCapabilityId(request, rule);
    if (!capabilityId)
        return undefined;
    return request.semanticCapabilityDefinitions?.[capabilityId];
}
function semanticCapabilityId(request, rule) {
    if (rule) {
...
function formatPermissionRequestLabel(label) {
    const trimmed = label.trim();
    const toolLabel = userFacingToolLabel(trimmed);
    if (toolLabel)
        return humanizeIdentifier(toolLabel);
    return neutralizeImplementationTerms(sanitizePermissionText(trimmed, 160, 40));
}
function neutralizeImplementationTerms(input) {
    let text = input
...
function userFacingToolLabel(toolName) {
...
export function formatPermissionReceiptActionSummary(request) {
    ...
    if (capabilityName)
        return capabilityName;
    const tool = request.displayName ||
        request.title ||
    ...
    if (command) {
        ...
        const generatedSkillPath = generatedRuntimeSkillPathDisplay(displayCommand.command);
        ...
        const safeCommand = sanitizeReceiptDetail([...displayCommand.runtimeEnvAssignments, displayCommand.command].join(' '));
