...
function usesCapabilitySummary(request) {
...
export function formatStructuredPermissionReceiptActionSummary(request) {
...
function bulkEligibleScope(toolName, input) {
    if (toolName === 'Bash' || toolName === 'RunCommand') {
        ...
        if (generatedRuntimeSkillPathDisplay(display.command))
            return null;
    ...
    if (toolName === 'WebFetch')
        return onlyStringField(input, 'url');
    if (PATTERN_TOOLS.has(toolName))
        return onlyStringField(input, 'pattern');
    return null;
}
function onlyStringField(input, key) {
