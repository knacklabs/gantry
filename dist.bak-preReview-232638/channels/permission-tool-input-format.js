import { destructiveBashCommandHint, firstDestructiveRedirectTarget, summarizeBashCommandPrograms, } from '../shared/bash-command-parser.js';
import { generatedRuntimeSkillPathDisplay } from '../shared/generated-runtime-paths.js';
import { stripHostInjectedEnvPrefix, stripRuntimeEnvPrefix, } from '../shared/runtime-env-command.js';
...
const SENSITIVE_INPUT_KEY_PATTERN = /(secret|token|password|credential|api[_-]?key|private[_-]?key|session|cookie|authorization)/i;
...
function isInternalPlumbingKey(key) {
    const k = key.toLowerCase();
    return (k.endsWith('jid') ||
        k.includes('ipcdir') ||
...
const REDACTION_MARKER_PATTERN = /\[REDACTED_(?:SECRET|POTENTIALLY_SENSITIVE)\]/;
export function permissionRiskLines(request) {
    if (!request.risk_level && !request.risk_category)
        return [];
    return [
...
export function formatPermissionToolInputLines(request, sanitizePermissionText, options = {}) {
    ...
    if (typeof input.command === 'string' && input.command.trim()) {
        ...
        const leadLine = commandLeadLine(input, displayCommand.command, sanitizePermissionText);
        ...
        if (generatedSkillPath) {
            const runtimeEnvLine = displayCommand.runtimeEnvAssignments.length > 0
                ? `Runtime environment: ${sanitizePermissionText(displayCommand.runtimeEnvAssignments.join(' '), 600, 200)}`
                : null;
            return [
                leadLine,
        ...
        const command = (options.sanitizeCommandText ?? sanitizePermissionText)(displayCommand.command, 900, 300);
    ...
    if (request.toolName === 'Edit' || request.toolName === 'Write') {
        const lines = formatFileToolInputLines(request.toolName, input, sanitizePermissionText);
        if (lines.length > 0)
            return lines;
    }
    const fieldLines = formatKnownToolInputFields(request.toolName, input, sanitizePermissionText);
...
function formatGenericInputFields(input, sanitizePermissionText) {
    const lines = [];
    let shown = 0;
    for (const [key, value] of Object.entries(input)) {
        ...
        if (typeof value === 'string' ||
            ...
            if (!text)
                continue;
            lines.push(`  ${labelizeKey(key)}: ${sanitizePermissionText(text, 200, 80)}`);
            shown += 1;
        }
            ...
            if (scalars.length === 0)
                continue;
            lines.push(`  ${labelizeKey(key)}: ${sanitizePermissionText(scalars.slice(0, PERMISSION_JSON_MAX_ARRAY_ITEMS).join(', '), 200, 60)}`);
            shown += 1;
        }
...
function formatFileToolInputLines(toolName, input, sanitizePermissionText) {
    const lines = [];
    if (typeof input.file_path === 'string' && input.file_path.trim()) {
        lines.push(`File: ${sanitizePermissionText(input.file_path.trim(), 200, 80)}`);
    }
    const diffLines = [];
    if (toolName === 'Edit') {
        if (typeof input.old_string === 'string' && input.old_string.trim()) {
            diffLines.push(`-${escapeMarkdownFenceDelimiters(sanitizePermissionText(input.old_string.trim(), 200, 100))}`);
        }
        if (typeof input.new_string === 'string' && input.new_string.trim()) {
            diffLines.push(`+${escapeMarkdownFenceDelimiters(sanitizePermissionText(input.new_string.trim(), 200, 100))}`);
        }
    }
    else if (typeof input.content === 'string' && input.content.trim()) {
        diffLines.push(`+${escapeMarkdownFenceDelimiters(sanitizePermissionText(input.content.trim(), 200, 100))}`);
    }
...
function commandLeadLine(input, command, sanitizePermissionText) {
    if (typeof input.description === 'string' && input.description.trim()) {
        return `What it does: ${sanitizePermissionText(input.description.trim(), 300, 100)}`;
    }
    const programs = summarizeBashCommandPrograms(command) ?? shellProgramLabel(command);
    return `Runs: ${programs ? sanitizePermissionText(programs, 200, 80) : 'command'}`;
}
function commandRiskLines(command) {
    const lines = [];
    const redirectTarget = firstDestructiveRedirectTarget(command);
    if (redirectTarget)
        lines.push(`Redirect: ${redirectTarget}`);
    const hint = destructiveBashCommandHint(command);
    if (hint)
        lines.push(`⚠️ ${hint}`);
    return lines;
}
function shellProgramLabel(command) {
    const first = command
        .trim()
...
export function runtimeDisplayCommand(command) {
    ...
    const parsed = stripRuntimeEnvPrefix(stripHostInjectedEnvPrefix(command).command);
    return {
...
function skillMarkdownPreviewContent(input) {
...
function skillReviewFileLines(input, sanitizePermissionText) {
    if (!Array.isArray(input.files) || input.files.length === 0)
        return [];
    const lines = ['Review files:'];
...
function formatKnownToolInputFields(toolName, input, sanitizePermissionText) {
    const lines = [];
    const add = (label, value, limit = 300) => {
    ...
    const addList = (label, value, max = 8) => {
        ...
        const overflow = items.length > shown.length
            ...
            : '';
        lines.push(`${label}: ${sanitizePermissionText(shown.join(', '), 280, 80)}${overflow}`);
    };
    if (toolName === 'Read')
        add('Path', input.file_path);
    if (toolName === 'LS')
        add('Path', input.path);
    if (toolName === 'Glob') {
    ...
    if (toolName.startsWith('mcp__gantry__scheduler_') ||
        toolName.startsWith('scheduler_')) {
        add('Job ID', input.job_id ?? input.jobId);
        add('Name', input.name);
        add('Schedule', input.schedule ?? input.schedule_value ?? input.scheduleValue);
        add('Prompt', input.prompt, 300);
    }
    ...
    if (toolName === 'request_skill_dependency_install') {
        ...
        if (Array.isArray(input.commandArgv) && input.commandArgv.length > 0) {
            const argv = input.commandArgv
                ...
                .filter(Boolean);
            if (argv.length > 0) {
                lines.push('Command:', '```', escapeMarkdownFenceDelimiters(sanitizePermissionText(argv.join(' '), 900, 300)), '```');
            }
        }
    }
    if (toolName === 'request_skill_install' ||
        toolName === 'request_skill_proposal') {
        add('Description', input.description, 200);
        add('Install', input.commandSummary, 200);
        if (Array.isArray(input.files) && input.files.length > 0) {
            const size = typeof input.totalSizeBytes === 'number'
        ...
        if (Array.isArray(input.requiredEnvVars) &&
            input.requiredEnvVars.length > 0) {
            lines.push('Credentials: required before some skill actions can run; add them in Credential Center.');
        }
        const preview = skillMarkdownPreviewContent(input);
        if (preview) {
            lines.push(preview.truncated
                ? 'SKILL.md preview (truncated):'
                : 'SKILL.md preview:', '```markdown', escapeMarkdownFenceDelimiters(sanitizePermissionText(preview.content, 2400, 600)), '```');
        }
    }
    ...
    if (toolName === 'register_agent') {
        if (typeof input.trigger === 'string' && input.trigger.trim()) {
            lines.push(`Trigger: ${sanitizePermissionText(input.trigger.trim(), 80, 20)}${input.requiresTrigger ? ' (required)' : ''}`);
        }
    }
    if (toolName === 'request_agent_profile_update') {
        const fileName = typeof input.fileName === 'string' ? input.fileName : String(input.file);
        add('File', fileName);
        ...
        if (typeof input.proposedContent === 'string' &&
            ...
            if (proposedContent.length <= 1600) {
                lines.push('Proposed content:', '```markdown', escapeMarkdownFenceDelimiters(sanitizePermissionText(proposedContent, proposedContent.length, 0)), '```');
            }
            else {
                lines.push('Proposed content: full content is attached to the approval evidence; do not approve from this text summary alone.');
            }
        }
        if (typeof input.diffPreview === 'string' && input.diffPreview.trim()) {
            lines.push('Change:', '```diff', escapeMarkdownFenceDelimiters(sanitizePermissionText(input.diffPreview.trim(), 1800, 400)), '```');
        }
    ...
    if (toolName === 'request_settings_update') {
        ...
        if (Array.isArray(input.diffSummary) && input.diffSummary.length > 0) {
            lines.push('Change summary:', ...input.diffSummary
                .map((entry) => (typeof entry === 'string' ? entry.trim() : ''))
                .filter(Boolean)
                .slice(0, 12)
