 ...
 */
function providerKeyForJid(conversationJid) {
    if (conversationJid.startsWith('tg:'))
        return 'telegram';
    if (conversationJid.startsWith('sl:'))
        return 'slack';
    if (conversationJid.startsWith('dc:'))
        return 'discord';
    if (conversationJid.startsWith('teams:'))
        return 'teams';
    if (conversationJid.startsWith('app:'))
        return 'app';
    return undefined;
}
 ...
 */
export function deliveryLabel(conversationJid, threadId, conversationKind) {
 ...
 */
export function ownerLabel(conversationJid, conversationKind) {
    switch (providerKeyForJid(conversationJid)) {
        case 'telegram':
            return isTelegramGroupJid(conversationJid)
                ? 'Telegram group'
...
function isTelegramGroupJid(conversationJid) {
    return conversationJid.startsWith('tg:-');
}
function isSlackDmJid(conversationJid) {
    return conversationJid.startsWith('sl:D');
}
function isSlackChannelJid(conversationJid) {
