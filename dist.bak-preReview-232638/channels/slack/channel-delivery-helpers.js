...
import { SLACK_FALLBACK_CHUNK_MAX_LENGTH, splitSlackTextByCodeUnits, } from './text-limits.js';
...
import { handleSlackThreadProgressStatus, isSlackTerminalSuccessText, } from './thread-progress-status.js';
import { clampSlackRetryDelayMs, slackRateLimitRetryDelayMs, } from './channel-retry-delay.js';
import { uploadSlackAttachments } from './file-delivery.js';
function slackActionBlocks(text, options) {
...
async function waitForPostMessageRetry(delayMs) {
    await new Promise((resolve) => setTimeout(resolve, clampSlackRetryDelayMs(delayMs)));
}
export function isSlackPayloadTooLarge(err) {
    const candidate = err;
...
export async function postSlackMessageWithRetry(app, payload, context, warnings, log) {
    if (!app)
        throw new Error('Slack app not initialized');
    for (let attempt = 0; attempt < 3; attempt += 1) {
        try {
            const posted = (await app.client.chat.postMessage(payload));
            if (posted.ok === false) {
                const retryDelayMs = slackRateLimitRetryDelayMs(posted);
                if (retryDelayMs !== null && attempt < 2) {
                    warnings.push('slack.rate_limited_retry');
                    log.warn({ ...context, attempt: attempt + 1, retryDelayMs }, 'Slack postMessage rate-limited; retrying');
            ...
            if (retryDelayMs !== null && attempt < 2) {
                warnings.push('slack.rate_limited_retry');
                log.warn({ ...context, attempt: attempt + 1, retryDelayMs }, 'Slack postMessage rate-limited via error; retrying');
                await waitForPostMessageRetry(retryDelayMs);
                continue;
            }
            throw err;
...
export async function sendSlackMessage(input) {
    ...
    if (!formatted)
        return;
    const parts = splitSlackTextByCodeUnits(formatted, SLACK_FALLBACK_CHUNK_MAX_LENGTH);
    const warnings = [];
    if (parts.length > 1)
        warnings.push(`slack.message.chunked:${parts.length}`);
    ...
    for (let partIndex = 0; partIndex < parts.length; partIndex += 1) {
        ...
        const actionBlocks = partIndex === parts.length - 1
            ? slackActionBlocks(part, input.options)
            : undefined;
        try {
            const posted = await postSlackMessageWithRetry(input.app, {
                channel: input.channelId,
                text: part,
                ...(threadTs ? { thread_ts: threadTs } : {}),
                ...(actionBlocks ? { blocks: actionBlocks } : {}),
            }, { jid: input.jid, part: partIndex + 1, totalParts: parts.length }, warnings, input.log);
            if (posted.ts)
                externalMessageIds.push(posted.ts);
            deliveredParts += 1;
        }
        catch (err) {
...
export async function sendSlackFallbackStreamParts(input) {
    ...
    let deliveredParts = 0;
    const visibleFallbackMessageIds = () => input.state.fallbackMessageTs.filter(Boolean);
    const retryTailFromFallbackParts = () => {
        const tail = input.fallbackParts.slice(deliveredParts).join('');
        if (deliveredParts > 0 || !tail)
            return tail;
        const previousFallbackText = input.state.lastNativeText &&
    ...
    for (let partIndex = 0; partIndex < input.fallbackParts.length; partIndex += 1) {
        ...
        try {
            ...
            const visibleDeliveredChunks = Math.max(deliveredParts, externalMessageIds.length > 0 ? 1 : 0);
            if (visibleDeliveredChunks > 0) {
                const unsentTail = retryTailFromFallbackParts();
                const totalChunks = Math.max(input.fallbackParts.length, visibleDeliveredChunks + (unsentTail.trim() ? 1 : 0));
                const partial = new PartialMessageDeliveryError({
                    cause: err,
...
export async function sendSlackProgressUpdate(input) {
    ...
    if (await handleSlackThreadProgressStatus({
        app: input.app,
        channelId: input.channelId,
        key: input.key,
        text: input.text,
...
export function loadPersistedSlackProgress(botToken, activeProgress) {
    const entries = readProgressStateEntries(channelProgressStateFilePath('slack', botToken), 'Slack');
    for (const [key, state] of entries) {
        if (typeof state.channelId === 'string' &&
            typeof state.lastText === 'string') {
            activeProgress.set(key, state);
        }
    }
}
export function persistSlackProgress(botToken, activeProgress) {
    writeProgressStateEntries(channelProgressStateFilePath('slack', botToken), 'Slack', activeProgress.entries());
}
...
export async function syncSlackGroups(input) {
    ...
    do {
        ...
        for (const channel of channels) {
            ...
            if (!channelId)
                continue;
            if (!input.force && input.channelNameCache.has(channelId))
                continue;
            const name = channel.name || (await input.resolveChannelName(channelId));
            input.channelNameCache.set(channelId, name);
            await input.onChatMetadata(`sl:${channelId}`, now, name, 'slack', !channel.is_im, { providerAccountId: input.providerAccountId });
        }
