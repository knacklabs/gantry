...
import { formatOutboundForChannel, stripInternalTagsPreserveWhitespace, } from '../../messaging/router.js';
import { disconnectSlackDelivery, loadPersistedSlackProgress, persistSlackProgress, sendSlackFallbackStreamParts, sendSlackMessage, sendSlackProgressUpdate, syncSlackGroups, } from './channel-delivery-helpers.js';
import { SlackChannelInteractions } from './channel-interactions.js';
import { SLACK_FALLBACK_CHUNK_MAX_LENGTH, SLACK_STREAM_UPDATE_INTERVAL_MS, splitSlackTextByCodeUnits, } from './text-limits.js';
import { nowMs as currentTimeMs } from '../../shared/time/datetime.js';
import { renderSlackAgentTodo } from './agent-todo-delivery.js';
import { connectSlackApp } from './channel-connect.js';
import { requestSlackPermissionApproval, slackPermissionApproverIds, } from './permission-approval-delivery.js';
...
export class SlackChannelDelivery extends SlackChannelInteractions {
    interactionCallbacksEnabled = true;
    reactionKeys = new Set();
    async sendSnippetFallback(_input) {
        return null;
    }
    ...
    async sendMessage(jid, text, options = {}) {
        ...
        return sendSlackMessage({
            ...
            options: {
                ...options,
                providerAccountId: options.providerAccountId ?? this.opts.providerAccountId,
            },
            log: logger,
    ...
    async renderAgentTodo(jid, render) {
        ...
        if (!parsed)
            return false;
        const todoKey = this.streamKey(`${jid}:${render.cardKind ?? 'todo'}`, render.threadId ?? undefined);
    ...
    async sendStreamingChunk(jid, text, options = {}) {
        ...
        const sendFallbackParts = (fallbackParts) => sendSlackFallbackStreamParts({
            app: this.app,
            jid,
            state,
            fallbackParts,
        ...
        const rendered = formatOutboundForChannel(stripInternalTagsPreserveWhitespace(state.rawBuffer), 'slack');
        ...
        try {
            ...
            if (state.nativeEnabled && !state.nativeStreamTs && nextText) {
                const nativeStreamTs = await this.tryNativeStreamStart(state.channelId, state.threadId, nextText);
            ...
            if (state.nativeEnabled && state.nativeStreamTs) {
                const delta = startedNativeThisFlush
                    ...
                    : nextText.startsWith(state.lastSentText)
                        ...
                        : nextText;
                if (delta) {
                    const appendResult = await this.tryNativeStreamAppend(state.channelId, state.nativeStreamTs, delta);
                ...
                if (options.done && state.nativeEnabled) {
                    const stopped = await this.tryNativeStreamStop(state.channelId, state.nativeStreamTs);
                    if (!stopped)
                        state.nativeEnabled = false;
                    if (stopped)
                        delivered = true;
            ...
            if (!state.nativeEnabled) {
                const fallbackTextRaw = state.lastNativeText && nextText.startsWith(state.lastNativeText)
                    ? nextText.slice(state.lastNativeText.length)
                    : nextText;
                const fallbackParts = splitSlackTextByCodeUnits(fallbackTextRaw, SLACK_FALLBACK_CHUNK_MAX_LENGTH);
                if (options.done &&
            ...
            if (isPartialMessageDeliveryError(err)) {
                ...
                if (options.done && sentPrefix.length > 0) {
                    ...
                    const fallbackParts = splitSlackTextByCodeUnits(fallbackTextRaw, SLACK_FALLBACK_CHUNK_MAX_LENGTH);
                    try {
                        if (fallbackParts.length >= SLACK_STREAM_SNIPPET_FALLBACK_MIN_PARTS) {
                            if (!this.streamResetEpochs.isCurrent(key, streamEpoch))
                                return false;
                            const fallback = await this.sendSnippetFallback({
                                channelId: state.channelId,
                        ...
                        if (isPartialMessageDeliveryError(fallbackErr)) {
                            const fallbackMetadata = getPartialMessageDeliveryMetadata(fallbackErr);
                            if (!fallbackMetadata.retryTail?.canonicalText) {
                                const deliveredParts = fallbackMetadata.deliveredParts;
                                if (typeof deliveredParts === 'number' &&
            ...
            logger.warn({ jid, err }, 'Slack streaming update failed; preserving current stream state');
        }
        finally {
            if (options.done &&
    ...
    async requestPermissionApproval(jid, request, onPromptDelivered) {
        ...
        if (Array.from(this.pendingPermissionPrompts.values()).some((pending) => pending.request.requestId === request.requestId &&
        ...
        return requestSlackPermissionApproval({
            ...
            request,
            timeoutMs,
            approverUserIds: slackPermissionApproverIds(this.opts.runtimeSettings, this.opts.providerAccountId, parsed.channelId),
            pendingPermissionPrompts: this.pendingPermissionPrompts,
            timeoutPermissionPrompt: (providerAlias, retryWindowMs) => this.timeoutPermissionPrompt(providerAlias, retryWindowMs),
            onPromptDelivered,
        });
    }
    ...
    async requestUserAnswer(jid, request, onPromptDelivered) {
        ...
        return requestSlackUserAnswer({
            ...
            pendingUserQuestionKey: (callback) => this.pendingUserQuestionKey(callback),
            formatPromptText: (promptRequest, question, timeoutMs) => this.formatUserQuestionPromptText(promptRequest, question, timeoutMs),
            buildBlocks: (pending) => this.buildUserQuestionBlocks(pending),
            finalizeTimedOut: (pending) => this.finalizeUserQuestionPrompt(pending, pending.question.multiSelect ? [] : '', 'system', 'timed out'),
            onPromptDelivered,
        });
    }
    async syncGroups(force = false) {
    ...
    async disconnect() {
        ...
        for (const providerAlias of this.pendingPermissionPrompts.keys()) {
            const result = await this.claimAndResolvePermissionPrompt(providerAlias, 'cancel', 'system', undefined, 'Slack channel disconnected', true);
        ...
        this.app = await disconnectSlackDelivery({
            ...
            activeProgress: this.activeProgress,
            pendingUserQuestions: this.pendingUserQuestions,
            stopNativeStream: (channelId, streamTs) => this.tryNativeStreamStop(channelId, streamTs),
        });
    }
}
