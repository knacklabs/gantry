...
import { SLACK_NATIVE_APPEND_MAX_LENGTH, splitSlackTextByCodeUnits, } from './text-limits.js';
...
export async function tryNativeStreamAppend(input) {
    ...
    const chunks = splitSlackTextByCodeUnits(input.text, SLACK_NATIVE_APPEND_MAX_LENGTH);
    if (chunks.length > 1) {
        logger.warn({
