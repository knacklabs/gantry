...
export function slackPermissionApproverIds(runtimeSettings, providerAccountId, channelId) {
    try {
        const conversations = Object.values(runtimeSettings?.().conversations || {});
        return [
            ...new Set(conversations.flatMap((conversation) => conversation.externalId === channelId &&
...
export async function requestSlackPermissionApproval(input) {
    ...
    const postPromptDeliveryFailureNotice = async (reason) => {
        try {
            ...
            logger.warn({ jid: input.jid, requestId: input.request.requestId, err }, 'Slack permission prompt failure notice could not be delivered');
        }
    };
    const postPrivatePrompt = async (blocks) => {
        let first = null;
        let lastError;
        for (const user of userIds) {
            try {
                ...
                lastError = err;
                logger.debug({ jid: input.jid, requestId: input.request.requestId, user, err }, 'Slack ephemeral permission prompt failed for approver');
            }
        }
        if (!first && lastError)
            throw lastError;
    ...
    try {
        if (userIds.length === 0) {
            const reason = 'Approval prompt could not be shown because this Slack conversation has no configured approvers. Add at least one conversation approver and retry.';
        ...
        try {
            ...
            logger.warn({ jid: input.jid, requestId: input.request.requestId, err: blocksErr }, 'Slack native permission blocks rejected; retrying with simple layout');
            const simpleBlocks = [
                {
                    type: 'section',
            ...
            try {
                ...
                const reason = 'Approval prompt could not be shown to any configured Slack approver. Check that the approver is a member of this conversation and that the Slack app can post ephemeral messages here.';
                await postPromptDeliveryFailureNotice(reason);
                throw simpleErr;
            }
        }
        ...
        const timer = settlementDelayMs !== undefined
            ? setTimeout(() => {
                void input.timeoutPermissionPrompt(callback.providerAlias, settlementDelayMs);
            }, settlementDelayMs)
            : undefined;
        const livePending = {
            callback,
        ...
        try {
            ...
            if (!bound)
                throw new Error('Slack permission message binding failed');
        }
        catch (err) {
            ...
            if (!livePending.settled) {
                ...
                clearTimeout(timer);
                if (input.pendingPermissionPrompts.get(callback.providerAlias) ===
                    livePending) {
            ...
            logger.error({ jid: input.jid, requestId: input.request.requestId, err }, 'Failed to send Slack permission prompt');
