...
export async function requestSlackUserAnswer(input) {
    ...
    for (let i = 0; i < input.request.questions.length; i += 1) {
        ...
        if (input.pendingUserQuestions.has(pendingKey)) {
            logger.warn({ requestId: input.request.requestId, questionIndex: i }, 'Duplicate pending Slack user question request detected');
            continue;
        }
        const promptText = input.formatPromptText(input.request, question, input.timeoutMs);
        try {
            const pendingState = {
                callback,
            ...
            const questionThreadTs = slackThreadTsFromThreadId(input.request.threadId);
            const questionThreadPayload = questionThreadTs
                ? { thread_ts: questionThreadTs }
                : {};
            const fullBlocks = input.buildBlocks(pendingState);
            const postQuestion = (blocks) => input.app.client.chat.postMessage({
            ...
            try {
                ...
                sent = await postQuestion(fullBlocks.filter((block) => block.type !== 'header'));
            }
            const messageTs = sent.ts;
            if (!messageTs) {
                logger.warn({ requestId: input.request.requestId, questionIndex: i }, 'Slack did not return a message timestamp for user question prompt');
                continue;
            }
            ...
            if (isEmptySelection) {
                ...
                if (!progressRecorded) {
                    throw new DurableInteractionPersistenceError('Slack user question progress was not persisted');
                }
                continue;
            }
            ...
            if (err instanceof DurableInteractionPersistenceError)
                throw err;
            logger.warn({ requestId: input.request.requestId, questionIndex: i, err }, 'Failed to run Slack user question prompt');
        }
    }
    return {
        requestId: input.request.requestId,
