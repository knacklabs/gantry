...
export function registerSlackUserQuestionHandlers(input) {
    input.app.action(/^gantry_userq_select_\d+$/, async (args) => {
        ...
        try {
            ...
            if (!parsed || parsed.optionIndex === undefined)
                return;
            const key = input.pendingKey(parsed.callback);
            const candidate = input.pendingUserQuestions.get(key);
            const pending = candidate && sameQuestionCallback(candidate.callback, parsed.callback)
                ? candidate
                : undefined;
            ...
            if (!userId)
                return;
            const answeredBy = body.user?.name || body.user?.username || body.user?.id || 'unknown';
            if (!pending)
                return;
            if (pending.settled)
                return;
            if (!callbackChannelId || callbackChannelId !== pending.channelId)
                return;
            if (!(await input.canAnswer(userId, pending.sourceAgentFolder, `sl:${pending.channelId}`))) {
                try {
            ...
            if (!pending.question.multiSelect) {
                const label = pending.question.options[parsed.optionIndex]?.label?.trim() || '';
    ...
    input.app.action('gantry_userq_done', async (args) => {
        await args.ack();
        const action = args.action;
        const body = args.body;
        const parsed = input.parseActionValue(action.value);
        ...
        if (!pending)
            return;
        if (pending.settled || !pending.question.multiSelect)
            return;
        if (!callbackChannelId || callbackChannelId !== pending.channelId)
            return;
        if (!(await input.canAnswer(userId, pending.sourceAgentFolder, `sl:${pending.channelId}`))) {
    ...
    input.app.action('gantry_userq_other', async (args) => {
        await args.ack();
        const action = args.action;
        const body = args.body;
        const parsed = input.parseActionValue(action.value);
        ...
        const callbackChannelId = body.channel?.id || '';
        const userId = body.user?.id || '';
        if (!userId || !callbackChannelId || pending?.settled)
            return;
        if (!pending || callbackChannelId !== pending.channelId)
            return;
        if (!(await input.canAnswer(userId, pending.sourceAgentFolder, `sl:${pending.channelId}`))) {
            return;
        }
    ...
    input.app.view('gantry_userq_other_modal', async (args) => {
        await args.ack();
        const body = args.body;
        ...
        if (!meta.callback)
            return;
        const text = (view.state?.values?.['gantry_userq_other_block']?.['gantry_userq_other_input']?.value || '').trim();
        if (!text)
            return;
        ...
        if (pending && sameQuestionCallback(pending.callback, meta.callback)) {
            if (userId &&
                !(await input.canAnswer(userId, pending.sourceAgentFolder, `sl:${pending.channelId}`))) {
                return;
            }
...
function sameQuestionCallback(left, right) {
