...
import { agentTodoLines, agentTodoStopActions, countCompletedAgentTodos, formatAgentProgressLine, formatAgentTodoHeader, hasAgentTodoCardHeader, } from './agent-todo-render.js';
export { agentTodoLines } from './agent-todo-render.js';
import { formatPermissionPromptText, permissionButtonLabel, permissionDecisionOptions, } from './permission-interaction.js';
...
const TEAMS_ATTACHMENT_UNAVAILABLE_LINE = '- Attachment unavailable in Teams until signed artifact links are added.';
export function formatTeamsAttachmentUnavailableCopy(text, filesPresent = false) {
...
export function buildTeamsApprovalAdaptiveCard(request, callback = {
    ...
    const promptText = formatPermissionPromptText(request, PERMISSION_APPROVAL_TIMEOUT_MS);
    return {
...
export function buildTeamsApprovalDescriptorPayload(request) {
    return {
        attachments: [
            {
...
export function buildTeamsAgentTodoCard(render, targetJid = '') {
    ...
    return {
        ...
        actions: stopAction?.kind === 'live_turn_stop'
            ? [
                {
                    type: 'Action.Execute',
...
export function buildTeamsUserQuestionCard(request, callback, startIndex = 0) {
    const body = [];
...
export function buildTeamsUserQuestionReceiptCard(text) {
    return {
        $schema: 'http://adaptivecards.io/schemas/adaptive-card.json',
        type: 'AdaptiveCard',
        version: '1.5',
