...
export async function hydrateTeamsConversationContext(request, sdkClient, botUserId) {
    ...
    try {
        ...
        const messages = normalizeTeamsContextMessages(request.conversationJid, request.threadId || undefined, rawMessages, limit, botUserId);
        logger.debug({
            providerId: 'teams',
...
function skippedTeamsHydration(request) {
    logger.debug({
        providerId: 'teams',
        conversationJid: request.conversationJid,
        threadId: request.threadId,
...
function normalizeTeamsContextMessages(jid, requestedThreadId, rawMessages, limit, botUserId) {
...
function isTeamsSelfMessage(sender, botUserId) {
    const normalizedBotUserId = botUserId?.trim().toLowerCase();
    if (!normalizedBotUserId)
        return false;
    const normalizedSender = sender.trim().toLowerCase();
    return (normalizedSender === normalizedBotUserId ||
        normalizedSender === `28:${normalizedBotUserId}`);
}
export function teamsMessageAttachments(message) {
