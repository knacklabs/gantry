...
function splitTeamsTextByCodeUnits(text, maxCodeUnits) {
    if (!text)
        return [];
...
export function splitTeamsTextByByteBudget(text, maxBytes) {
    if (!text)
        return [];
...
function splitTeamsPartFor413Retry(text) {
    if (!text)
        return [];
    const bytes = Buffer.byteLength(text, 'utf8');
    if (bytes <= 1)
        return [];
    const retryBudget = Math.min(TEAMS_413_RETRY_MAX_BYTES, Math.max(1024, Math.floor(bytes / 2)));
    const parts = splitTeamsTextByByteBudget(text, retryBudget).filter(Boolean);
    return parts.length >= 2 ? parts : [];
}
function isTeamsPayloadTooLarge(err) {
    const candidate = err;
...
export async function sendTeamsTextMessage(sdkClient, conversationId, text, options = {}, shouldContinue = () => true) {
    const initialParts = splitTeamsTextByByteBudget(text, TEAMS_SOFT_MESSAGE_BYTES)
        .flatMap((part) => splitTeamsTextByCodeUnits(part, TEAMS_HARD_MESSAGE_BYTES))
    ...
    const warnings = [];
    if (queue.length > 1) {
        warnings.push(`teams.message.chunked:${queue.length}:${TEAMS_SOFT_MESSAGE_BYTES}`);
    }
