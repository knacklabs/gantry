...
import { richDescriptor, RICH_INTERACTION_FALLBACK_COPY, RICH_INTERACTION_SUBMIT_LABEL, richFallbackText, richFormFields, richTextLines, } from './rich-interaction.js';
export function buildTeamsRichInteractionPayload(input) {
    const item = richDescriptor(input);
    const fields = richFormFields(input);
    const body = richTextLines(input).map((line, index) => ({
        type: 'TextBlock',
        text: line,
        wrap: true,
