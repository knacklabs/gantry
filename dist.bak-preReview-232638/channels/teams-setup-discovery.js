...
const TOKEN_BOUND_NETWORK_GUIDANCE = 'Check network access, provider app credentials, provider permissions, and provider service status, then retry. Raw credential-bearing transport details are intentionally not printed.';
const TOKEN_BOUND_TEAMS_GUIDANCE = 'Check Teams app permissions, tenant admin consent, channel access, and network, then retry. Raw credential-bearing transport details are intentionally not printed.';
export function trimTeamsSetupCredentials(credentials) {
    return {
...
async function fetchTeamsGraphToken(credentials, timeoutMs = 10_000) {
    ...
    const response = await fetchWithTimeout(`https://login.microsoftonline.com/${encodeURIComponent(trimmed.tenantId)}/oauth2/v2.0/token`, timeoutMs, {
        method: 'POST',
        headers: {
...
async function listGraphPages(options) {
    const rows = [];
    let nextUrl = options.firstUrl;
    while (nextUrl) {
        const response = await fetchWithTimeout(nextUrl, options.timeoutMs ?? 10_000, {
            headers: { authorization: `Bearer ${options.accessToken}` },
        });
        if (!response.ok)
            throw new Error('Teams Graph request failed');
        const payload = await readJson(response);
...
export class GraphTeamsSetupDiscoveryClient {
...
export async function validateTeamsAppCredentials(credentials) {
    const trimmed = trimTeamsSetupCredentials(credentials);
    if (!trimmed.clientId || !trimmed.clientSecret || !trimmed.tenantId) {
        return {
            ok: false,
            message: 'Teams app credentials are incomplete.',
            nextAction: 'Enter Microsoft Teams client ID, client secret, and tenant ID.',
        };
    }
    try {
...
export async function listTeamsChannels(options) {
    ...
    try {
        const accessToken = await fetchTeamsGraphToken(options.credentials, options.timeoutMs);
        const teams = await listGraphPages({
        ...
        for (const team of teams) {
            ...
            if (!teamId || !isSafeGraphId(teamId))
                continue;
            ...
            for (const channel of graphChannels) {
                ...
                if (!channelId || !isSafeGraphId(channelId))
                    continue;
        ...
        if (channels.length === 0) {
            return {
                ...
                nextAction: 'Confirm the Teams app has Graph permissions and admin consent, then use manual team/channel IDs if needed.',
            };
        }
        return {
            ok: true,
...
export async function verifyTeamsChannelAccess(options) {
    ...
    if (!teamId ||
        ...
        return {
            ...
            nextAction: 'Use Microsoft Graph team and channel IDs, for example team-id and 19:channel@thread.tacv2.',
        };
    }
    try {
        const accessToken = await fetchTeamsGraphToken(options.credentials, options.timeoutMs);
        const teamResponse = await fetchWithTimeout(`${GRAPH_ROOT}/teams/${encodeURIComponent(teamId)}`, options.timeoutMs ?? 10_000, { headers: { authorization: `Bearer ${accessToken}` } });
        if (!teamResponse.ok)
            throw new Error('Teams team verification failed');
        const team = await readJson(teamResponse);
        const channelResponse = await fetchWithTimeout(`${GRAPH_ROOT}/teams/${encodeURIComponent(teamId)}/channels/${encodeURIComponent(channelId)}`, options.timeoutMs ?? 10_000, { headers: { authorization: `Bearer ${accessToken}` } });
        ...
        const normalized = normalizeGraphChannel({
            ...
            channelId,
            channelName: String(channel.displayName || channel.id || channelId).trim(),
            channelType: String(channel.membershipType || 'standard').trim(),
            isArchived: channel.isArchived === true,
        });
        if (!normalized)
            throw new Error('Teams channel id is invalid');
        return {
            ok: true,
