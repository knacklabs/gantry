...
import { TEAMS_HARD_MESSAGE_BYTES, sendTeamsTextMessage, splitTeamsTextByByteBudget, } from './teams-delivery.js';
...
export async function applyTeamsStreamingChunk(input) {
    ...
    if (current !== input.state)
        return false;
    if (input.text)
        input.state.rawBuffer += input.text;
...
async function flushTeamsStreamingState(input) {
    ...
    const parts = splitTeamsTextByByteBudget(input.state.rawBuffer, TEAMS_HARD_MESSAGE_BYTES);
    const headText = parts[0] ?? ' ';
    const hasNativeStreaming = input.sdkClient.sendAdaptiveCard && input.sdkClient.updateAdaptiveCard;
    if (!hasNativeStreaming) {
        if (!options.done)
            return false;
        if (!input.shouldContinue())
            return false;
        await sendTeamsTextMessage(input.sdkClient, input.state.conversationId, input.state.rawBuffer, options, input.shouldContinue);
        return true;
    }
    ...
    if (options.done && parts.length > 1) {
        ...
        await sendTeamsTextMessage(input.sdkClient, input.state.conversationId, parts.slice(1).join(''), options, input.shouldContinue);
    }
    return true;
}
