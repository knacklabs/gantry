...
export async function renderTeamsAgentTodo(input) {
    ...
    if (existing?.messageId && input.sdkClient.updateAdaptiveCard) {
        try {
            ...
            logger.debug({ jid: input.jid, threadId: input.render.threadId, err }, 'Teams todo update failed; posting a fresh card');
    ...
    await sendTeamsTextMessage(input.sdkClient, conversationId, [heading, ...agentTodoLines(input.render)].join('\n'), { ...(input.render.threadId ? { threadId: input.render.threadId } : {}) });
    return true;
}
