...
import { normalizePermissionAction, permissionDecisionOptions, } from '../permission-interaction.js';
...
import { TELEGRAM_PERMISSION_CALLBACK_PATTERN, TELEGRAM_DEAD_LETTER_ACTION_CALLBACK_PATTERN, TELEGRAM_USER_QUESTION_CALLBACK_PATTERN, } from './channel-shared.js';
import { createTelegramBotRuntime, registerTelegramBotCommands, } from './bot-setup.js';
...
import { handleTelegramGroupJoinCallback, handleTelegramGroupMembershipUpdate, } from './group-join-onboarding.js';
export class TelegramChannelConnect extends TelegramChannelPrompts {
    async clearRestoredProgressActions() {
        this.loadPersistedProgressMessages();
        for (const [key, state] of this.activeProgressMessages.entries()) {
            if (!state.restored || !state.messageId)
                continue;
            ...
            }).catch((err) => logger.debug({ key, err: this.sanitizeErrorMessage(err) }, 'Failed to clear restored Telegram progress actions'));
        }
    }
    async connect(options = {}) {
        ...
        this.bot.on('callback_query:data', async (ctx) => {
            const data = typeof ctx.callbackQuery?.data === 'string'
                ? ctx.callbackQuery.data
                : '';
            if (await handleTelegramGroupJoinCallback({
                ctx,
                opts: this.opts,
                assistantName: ASSISTANT_NAME,
                isApproverAuthorized: (chatId, userId, sourceAgentFolder) => this.isTelegramApproverAuthorized(chatId, userId, sourceAgentFolder),
                sanitizeErrorMessage: (err) => this.sanitizeErrorMessage(err),
            })) {
                return;
            }
            const userQuestionMatch = TELEGRAM_USER_QUESTION_CALLBACK_PATTERN.exec(data);
            if (userQuestionMatch) {
                const action = userQuestionMatch[1];
                const callbackId = userQuestionMatch[2];
                const callbackTarget = this.pendingUserQuestionCallbackIds.get(callbackId);
                if (!callbackTarget) {
                ...
                if (!Number.isInteger(questionIndex))
                    return;
                const key = this.pendingUserQuestionKey(callbackTarget.appId, callbackTarget.sourceAgentFolder, requestId, questionIndex);
                ...
                const authorized = await this.isTelegramApproverAuthorized(pending.chatId, userId, pending.sourceAgentFolder);
                if (!authorized) {
                    await ctx.answerCallbackQuery({
                ...
                if (action === 'other') {
                    ...
                    let promptMessageId;
                    try {
                        const prompt = await ctx.api.sendMessage(pending.chatId, 'Reply to this message with your answer.', {
                        ...
                        logger.debug({ requestId, err: this.sanitizeErrorMessage(err) }, 'Failed to send Telegram Other free-text prompt');
                    }
                    ...
                    this.pendingUserQuestionOtherPrompts.set(`${pending.chatId}:${promptMessageId}`, {
                        appId: pending.appId,
                ...
                const answeredBy = ctx.from?.first_name || ctx.from?.username || userId || 'unknown';
                if (action === 'done') {
                    ...
                    const selectedLabels = [...pending.selectedOptionIndexes]
                        ...
                        .filter(Boolean);
                    await this.finalizeUserQuestionPrompt(pending, selectedLabels, answeredBy, 'answered via Telegram');
                    await ctx.answerCallbackQuery({
                        text: 'Saved.',
                    });
                    return;
                ...
                await this.finalizeUserQuestionPrompt(pending, selected, answeredBy, 'answered via Telegram');
                await ctx.answerCallbackQuery({
                    text: 'Saved.',
                });
                return;
            ...
            if (data.startsWith('lt:stop:')) {
                ...
                const chatId = callbackMessage?.chat?.id?.toString() ||
                ...
                await this.opts.onMessageAction?.({
                    ...
                    ...(this.opts.providerAccountId
                        ...
                        : {}),
                    threadId: typeof callbackMessage?.message_thread_id === 'number'
            ...
            if (compactRetryJobId || deadLetterActionMatch) {
                if (compactRetryJobId ||
                    (deadLetterActionMatch?.[1] === 'retry' && deadLetterActionMatch[2])) {
                    let jobId;
                    try {
                        jobId = decodeURIComponent(compactRetryJobId || deadLetterActionMatch[2]);
                    }
                    catch {
                        await ctx.answerCallbackQuery({
            ...
            if (!pending) {
                await resolveDurableTelegramPermissionCallback({
                    ...
                    mode,
                    sanitizeErrorMessage: (err) => this.sanitizeErrorMessage(err),
                    isAuthorized: (approvalContextJid, userId, recovered) => this.isTelegramApproverAuthorized(approvalContextJid.replace(/^tg:/, ''), userId, recovered.sourceAgentFolder, recovered.decisionPolicy, recovered.threadId ?? undefined),
                });
                return;
            }
            ...
            const userId = callbackQuery?.from?.id?.toString() || ctx.from?.id?.toString() || '';
            if (!userId) {
                await ctx.answerCallbackQuery({
            ...
            const authorized = await this.isTelegramApproverAuthorized((pending.approvalContextJid || `tg:${pending.chatId}`).replace(/^tg:/, ''), userId, pending.sourceAgentFolder, pending.decisionPolicy, pending.request.threadId);
            if (!authorized) {
                logger.warn({
            ...
            const settled = await this.claimAndResolvePermissionPrompt(callbackId, mode, userId, mode === 'allow_once'
            ...
            await ctx.answerCallbackQuery({
                text: mode === 'allow_persistent_rule' && pending.request.permissionBatch
        ...
        this.bot.on('my_chat_member', (ctx) => handleTelegramGroupMembershipUpdate({
            ctx,
            opts: this.opts,
            assistantName: ASSISTANT_NAME,
            isApproverAuthorized: (chatId, userId, sourceAgentFolder) => this.isTelegramApproverAuthorized(chatId, userId, sourceAgentFolder),
        ...
        this.bot.on('message:text', (ctx) => handleTelegramTextMessage({
            ctx,
            opts: this.opts,
            assistantName: ASSISTANT_NAME,
            triggerPattern: TRIGGER_PATTERN,
            tryResolveOther: (input) => this.tryResolveUserQuestionOtherReply(input),
        }));
        registerTelegramMediaHandlers({
            bot: this.bot,
            opts: this.opts,
            mediaIngestionQueue: this.mediaIngestionQueue,
            downloadFile: (fileId, folder, filename) => this.downloadFile(fileId, folder, filename),
            sanitizeErrorMessage: (err) => this.sanitizeErrorMessage(err),
        });
        // Handle errors gracefully
        this.bot.catch((err) => {
            logger.error({ error: this.sanitizeErrorMessage(err) }, 'Telegram bot error');
        });
