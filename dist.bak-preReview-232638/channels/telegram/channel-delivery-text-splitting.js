const TELEGRAM_MARKDOWN_V2_PROTECTED_PATTERN = /```[\s\S]*?```|`[^`\n]+`|\[[^\]\n]+\]\((?:\\.|[^\\\n)])+\)/g;
const TELEGRAM_MARKDOWN_V2_LINK_PATTERN = /^\[([\s\S]+)]\(([\s\S]+)\)$/;
const TELEGRAM_STYLE_MARKERS = new Set(['*', '_', '~']);
export function splitTelegramTextByCodeUnits(text, maxCodeUnits) {
    if (!text)
        return [];
    if (maxCodeUnits <= 0)
        return [text];
...
function previousCodePointStart(text, endExclusive) {
...
function nextCodePointEnd(text, startInclusive, maxCodeUnits) {
    ...
    for (let cursor = startInclusive; cursor < text.length;) {
        ...
        if (used > 0 && used + codePointLength > maxCodeUnits)
            break;
...
function splitTelegramTextByCodeUnitsEscapeAware(text, maxCodeUnits) {
    if (!text)
        return [];
    if (maxCodeUnits <= 0)
        return [text];
...
function tokenizeTelegramStyledPlainText(text) {
    if (!text)
        return [];
    const tokens = [];
    let cursor = 0;
    ...
    while (index < text.length) {
        ...
        for (let i = index + 1; i < text.length; i += 1) {
            if (text[i] !== marker || isEscapedAt(text, i))
                continue;
            closing = i;
            break;
        }
...
function tokenizeTelegramMarkdownV2(text) {
    const segments = [];
    const pattern = new RegExp(TELEGRAM_MARKDOWN_V2_PROTECTED_PATTERN.source, 'g');
    let lastIndex = 0;
    let match;
...
function splitTelegramFencedCodeSegment(segment, maxCodeUnits) {
    if (segment.length <= maxCodeUnits)
        return [segment];
    ...
    const parts = splitTelegramTextByCodeUnitsEscapeAware(code, availableCodeUnits);
    if (parts.length === 0)
        return [`${prefix}${suffix}`];
    return parts.map((part) => `${prefix}${part}${suffix}`);
}
function splitTelegramInlineCodeSegment(segment, maxCodeUnits) {
    if (segment.length <= maxCodeUnits)
        return [segment];
...
function splitTelegramLinkSegment(segment, maxCodeUnits) {
    if (segment.length <= maxCodeUnits)
        return [segment];
...
function splitTelegramPlainSegment(text, maxCodeUnits) {
    if (text.length <= maxCodeUnits)
        return [text];
    const out = [];
    ...
    for (const token of tokens) {
        ...
        const contentParts = splitTelegramTextByCodeUnitsEscapeAware(token.content, contentBudget);
        if (contentParts.length === 0) {
            out.push(wrapped);
            continue;
        }
        out.push(...contentParts.map((part) => `${token.marker}${part}${token.marker}`));
    }
    return out;
}
function splitTelegramMarkdownSegment(segment, maxCodeUnits) {
    if (segment.text.length <= maxCodeUnits)
        return [segment.text];
    switch (segment.kind) {
        case 'plain':
    ...
    const exhaustiveCheck = segment;
    throw new Error(`Unhandled Telegram markdown segment: ${exhaustiveCheck}`);
}
function planTelegramMarkdownV2Chunks(text, maxCodeUnits) {
    if (!text)
        return [];
    if (text.length <= maxCodeUnits)
        return [text];
...
export function splitTelegramDeliveryTextWithLimits(text, softCodeUnitBudget, hardCodeUnitLimit) {
    if (!text)
        return [];
    const planned = planTelegramMarkdownV2Chunks(text, Math.max(1, Math.min(softCodeUnitBudget, hardCodeUnitLimit)));
    const out = [];
