...
import { TELEGRAM_MESSAGE_MAX_LENGTH, TELEGRAM_STREAM_CHUNK_MAX_LENGTH, createPendingTelegramUserQuestion, editTelegramMessage, escapeTelegramMarkdownV2, sendTelegramMessageWithResult, splitTelegramDeliveryText, telegramThreadOptionsFromString, telegramQuestionCallbackId, } from './channel-shared.js';
import { telegramActionReplyMarkup } from './message-action-affordances.js';
import { clearProgressActions, prepareTelegramProgressHandle, progressActionOptions, sendNewProgressMessage, } from './progress-message-actions.js';
...
export class TelegramChannelDelivery extends TelegramChannelConnect {
    ...
    async sendMessage(jid, text, options = {}) {
        ...
        try {
            ...
            const escapedChunks = splitTelegramDeliveryText(escapedText, TELEGRAM_STREAM_CHUNK_MAX_LENGTH, TELEGRAM_MESSAGE_MAX_LENGTH);
            ...
            const warnings = [];
            if (chunks.length > 1) {
                warnings.push(`telegram.message.chunked:${chunks.length}:${TELEGRAM_STREAM_CHUNK_MAX_LENGTH}`);
            }
            ...
            for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex += 1) {
                ...
                const replyMarkup = chunkIndex === chunks.length - 1
                    ? telegramActionReplyMarkup(options.actionAffordances)
                    : undefined;
                try {
                    const sent = await sendTelegramPlannedChunk(this.bot.api, numericId, chunk.escapedText, {
            ...
            logger.info({ jid, length: text.length, threadId: options.threadId }, 'Telegram message sent');
            return {
            ...
            logger.error({ jid, error: this.sanitizeErrorMessage(err) }, 'Failed to send Telegram message');
            throw err;
        }
    }
    ...
    async sendStreamingChunk(jid, text, options = {}) {
        ...
        if (!state) {
            ...
            streamState.streamPromise = this.draftStreamApi
                .streamMessage(parsedChatId, draftIdOffset, queue.iterator, draftOptions, draftOptions)
                .then(() => undefined)
                .catch(async (err) => {
                logger.warn({ jid, err: this.sanitizeErrorMessage(err) }, 'Telegram stream send failed; falling back to final message send');
        ...
        if (text) {
            ...
            for (const chunk of splitTelegramDeliveryText(escaped, TELEGRAM_STREAM_CHUNK_MAX_LENGTH)) {
                if (chunk.length > TELEGRAM_MESSAGE_MAX_LENGTH) {
                    logger.warn({ jid, length: chunk.length }, 'Skipping oversize Telegram stream chunk');
                    continue;
                }
    ...
    async sendProgressUpdate(jid, text, options = {}) {
        if (!this.bot) {
            logger.info({ jid, progressText: text, options }, 'Progress lifecycle telegram skipped without bot');
            return;
        }
        const numericId = jid.replace(/^tg:/, '');
        const parsedThreadId = options.threadId
        ...
        else if (!this.shouldAcceptProgressUpdate(key, options.generation, options.done)) {
            return;
        }
        ...
        if (!nextText) {
            if (options.done) {
                this.activeProgressMessages.delete(key);
                this.persistProgressMessages();
                logger.info({ jid, key, generation: options.generation }, 'Progress lifecycle telegram cleared empty done');
            }
            return;
        }
        ...
        if (!existing && options.replaceOnly) {
            logger.info({ jid, key, progressText: nextText, generation: options.generation }, 'Progress lifecycle telegram dropped replaceOnly without handle');
            return;
        }
        if (!existing) {
        ...
        if (existing.lastText === nextText) {
            if (options.done) {
                ...
                this.activeProgressMessages.delete(key);
                this.persistProgressMessages();
                logger.info({ jid, key, generation: options.generation }, 'Progress lifecycle telegram cleared unchanged done');
            }
            else if (!options.replaceOnly) {
                existing.messageId = await sendTelegramMessageWithResult(this.bot.api, numericId, nextText, sendOptions);
                ...
                logger.info({ jid, key, generation: options.generation }, 'Progress lifecycle telegram skipped unchanged text');
            }
            return;
        }
        if (existing.messageId) {
            try {
                await editTelegramMessage(this.bot.api, numericId, existing.messageId, nextText, {}, actionOptions.editReplyMarkup);
            }
            catch (err) {
                logger.debug({ jid, err }, 'Failed to edit progress message, creating a fresh one');
                existing.messageId = await sendTelegramMessageWithResult(this.bot.api, numericId, nextText, sendOptions);
                logger.info({
                    jid,
                    key,
    ...
    async requestPermissionApproval(jid, request, onPromptDelivered) {
        return requestTelegramPermissionApproval({
            ...
            settlePrompt: (providerAlias, mode, approverRef, reason) => this.claimAndResolvePermissionPrompt(providerAlias, mode, approverRef, reason),
            onPromptDelivered,
    ...
    async requestUserAnswer(jid, request, onPromptDelivered) {
        ...
        for (let i = 0; i < request.questions.length; i += 1) {
            ...
            const pendingKey = this.pendingUserQuestionKey(request.appId || 'default', request.sourceAgentFolder, request.requestId, i);
            if (this.pendingUserQuestions.has(pendingKey)) {
                logger.warn({ requestId: request.requestId, questionIndex: i }, 'Duplicate pending user question request detected');
                continue;
            }
            try {
                ...
                const selectionPromise = createPendingTelegramUserQuestion({
                    ...
                    finalize: (pending, selection, selectedBy, outcome) => this.finalizeUserQuestionPrompt(pending, selection, selectedBy, outcome),
                });
                ...
                if (isEmptySelection) {
                    ...
                    if (!progressRecorded) {
                        throw new DurableInteractionPersistenceError('Telegram user question progress was not persisted');
                    }
                    continue;
                }
                if (selection.answeredBy)
                    answeredBy = selection.answeredBy;
                answers[question.question] = selection.selected;
                const progressRecorded = await recordDurableQuestionAnswerProgress({
                    requestId: request.requestId,
                    appId: request.appId,
    ...
    async renderAgentTodo(jid, render) {
        ...
        return renderTelegramChannelAgentTodo({
            ...
            jid,
            render,
            buildDraftStreamKey: (key, threadId) => this.buildDraftStreamKey(key, threadId),
    ...
    async disconnect() {
        ...
        const disconnected = await disconnectTelegramDelivery({
            ...
            settlePermissionPrompt: (providerAlias) => this.claimAndResolvePermissionPrompt(providerAlias, 'cancel', 'system', 'Telegram channel disconnected'),
