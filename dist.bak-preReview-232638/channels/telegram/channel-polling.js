...
export class TelegramChannelPolling extends TelegramChannelState {
    startPolling() {
        if (!this.bot || this.isStopping)
            return;
        void this.startPollingWithLease();
    }
    async startPollingWithLease() {
        if (!this.bot ||
            this.isStopping ||
            this.pollingLease ||
        ...
        try {
            ...
            this.pollingStartInFlight = false;
            logger.warn({ err, leaseKey }, 'Telegram polling lease acquisition failed; scheduling retry');
            this.schedulePollingRetry();
            return;
        }
        if (!lease && this.opts.runtimeLease) {
            this.pollingStartInFlight = false;
            logger.warn({ leaseKey }, 'Telegram polling lease is held by another runtime; skipping poller start');
            this.schedulePollingRetry();
            return;
        }
        ...
        lease?.onLost?.((err) => {
            ...
            if (this.isStopping)
                return;
            logger.warn({ err, leaseKey }, 'Telegram polling lease connection was lost; scheduling retry');
            this.schedulePollingRetry();
        });
        if (this.isTelegramBotRunning()) {
            this.pollingStartInFlight = false;
            logger.info({ leaseKey }, 'Telegram poller already running; retaining polling lease');
            return;
        }
        ...
        Promise.resolve(pollingRun)
            ...
            this.pollingStartInFlight = false;
            if (this.isTelegramBotRunning()) {
                logger.info({ leaseKey }, 'Telegram poller remains active after duplicate start; retaining polling lease');
                return;
            }
