...
export async function handleTelegramGroupMembershipUpdate(input) {
    ...
    await input.opts.onChatMetadata(chatJid, new Date(update.date * 1000).toISOString(), title, 'telegram', true, { providerAccountId: input.opts.providerAccountId });
    const oldMember = update.old_chat_member;
    const newMember = update.new_chat_member;
    if (isPresentStatus(oldMember) && isAbsentStatus(newMember)) {
        logger.info({ provider: 'telegram', providerAccountId, chatId, chatJid }, 'Telegram bot left a group');
    ...
    if (!isAbsentStatus(oldMember) || !isPresentStatus(newMember))
        return;
    const registered = findConversationRoutesForChat(input.opts.conversationRoutes(), chatJid, undefined, input.opts.providerAccountId).length > 0;
    if (registered)
        return;
    ...
    const contexts = settings
        ...
        : [];
    const adderIsKnownApprover = await anyAuthorizedContext(contexts.filter((context) => context.controlApprovers.includes(adder)), adder, input.isApproverAuthorized);
    if (!adderIsKnownApprover) {
        logger.info({ provider: 'telegram', providerAccountId, chatId, chatJid, adder }, 'Telegram group join ignored: adder is not a registered control approver');
        return;
    }
    const promptTarget = await firstAuthorizedControlDm(contexts, input.isApproverAuthorized);
    if (!promptTarget) {
        logger.info({ provider: 'telegram', providerAccountId, chatId, chatJid, adder }, 'Telegram group join has no registered control DM for onboarding');
        return;
    }
    if (!input.opts.groupJoinOnboarding) {
        logger.info({ provider: 'telegram', providerAccountId, chatId, chatJid, adder }, 'Telegram group join onboarding persistence is unavailable');
        return;
    }
    ...
    try {
        await input.ctx.api.sendMessage(promptTarget.jid.replace(/^tg:/, ''), `${adderLabel} added ${input.assistantName} to '${title}' (${chatId}). Respond there?`, {
            reply_markup: {
                inline_keyboard: [
                    [
...
export async function handleTelegramGroupJoinCallback(input) {
    const data = typeof input.ctx.callbackQuery?.data === 'string'
    ...
    const callbackChatId = String(callbackMessage?.chat?.id ?? input.ctx.chat?.id ?? '');
    const callbackChatJid = callbackChatId ? `tg:${callbackChatId}` : '';
    const userId = String(input.ctx.callbackQuery?.from?.id ?? input.ctx.from?.id ?? '');
    if (!callbackChatId ||
        callbackChatJid !== record.promptConversationJid ||
        !userId ||
        userId !== record.approver ||
        !(await input.isApproverAuthorized(callbackChatId, userId, record.promptAgentFolder))) {
        await input.ctx.answerCallbackQuery({
    ...
    try {
        ...
        try {
            const group = await input.ctx.api.getChat(record.chatJid.replace(/^tg:/, ''));
        ...
        await editCallbackMessage(input.ctx, callbackMessage, `Registered. Members can reach the agent with @${botUsername}. Anyone in the group can @mention; actions still need your approval.`);
        await input.ctx
...
function registeredTelegramConversationContexts(input) {
    const contexts = [];
    for (const [, conversation] of Object.entries(input.settings.conversations).sort(([left], [right]) => left.localeCompare(right))) {
        if (!conversation)
            continue;
        const account = conversation.providerAccount ?? conversation.providerConnection;
        if (account !== input.providerAccountId)
            continue;
        const jid = `tg:${conversation.externalId.replace(/^tg:/, '')}`;
        const routes = findConversationRoutesForChat(input.routes, jid, undefined, input.providerAccountId);
...
async function anyAuthorizedContext(contexts, userId, isAuthorized) {
    for (const context of contexts) {
        if (await isAuthorized(context.jid.replace(/^tg:/, ''), userId, context.agentFolder)) {
            return true;
        }
    }
    return false;
}
async function firstAuthorizedControlDm(contexts, isAuthorized) {
    for (const context of contexts) {
        if (context.kind !== 'dm' && context.kind !== 'direct')
            continue;
        ...
        for (const rawApprover of context.controlApprovers) {
            ...
            if (approver !== participant)
                continue;
            if (await isAuthorized(context.jid.replace(/^tg:/, ''), approver, context.agentFolder)) {
...
async function editCallbackMessage(ctx, message, text) {
