import { formatAgentProgressLine, formatAgentTodoHeader, formatAgentTodoLine, hasAgentTodoCardHeader, } from '../agent-todo-render.js';
 ...
 */
function escapeInlineWithCode(line) {
    if (!line.includes('`'))
 ...
 */
export function renderBodyLinesHtml(lines) {
    const out = [];
    let code = null;
...
export function renderPermissionPromptHtml(parts, options = {}) {
    const segments = [
    ...
    if (parts.contextLines.length > 0) {
        segments.push('', parts.contextLines
            .map((line) => `<i>${escapeTelegramHtml(line)}</i>`)
            .join('\n'));
    }
    if (options.includeFullView && parts.fullView) {
        segments.push('', `<b>${escapeTelegramHtml(parts.fullView.label)}</b>`, `<blockquote expandable>${escapeTelegramHtml(parts.fullView.content)}</blockquote>`);
    }
    segments.push('', `<i>Reply in ${parts.replyInMinutes}m</i>`);
    return segments.join('\n');
}
export function renderUserQuestionPromptHtml(question) {
    const lines = [
    ...
    question.options.forEach((option, optionIndex) => {
        const description = option.description
            ...
            : '';
        lines.push(`${optionIndex + 1}. <b>${escapeTelegramHtml(option.label)}</b>${description}`);
        if (option.preview) {
            lines.push(`   <i>Preview: ${escapeTelegramHtml(truncateText(option.preview, 180))}</i>`);
        }
    });
