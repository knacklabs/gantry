...
import { bindTelegramPermission, registerAndBindTelegramPermissionPrompt, } from './prompt-binding.js';
export async function requestTelegramPermissionApproval(input) {
    ...
    if ([...input.pendingPrompts.values()].some((pending) => pending.callback.scope.appId === (input.request.appId || 'default') &&
    ...
    const timeoutPermissionPrompt = async (retryWindowMs) => {
        let result = await input.settlePrompt(callback.providerAlias, 'cancel', 'system', 'timed out');
        if (result === 'settled')
            return;
        ...
        if (result === 'retryable') {
            ...
            for (const delayMs of [firstDelay, retryWindowMs - firstDelay]) {
                ...
                if (!input.pendingPrompts.has(callback.providerAlias))
                    return;
                result = await input.settlePrompt(callback.providerAlias, 'cancel', 'system', 'timed out');
                if (result !== 'retryable')
                    break;
            }
        }
    ...
    try {
        if (!(await bindTelegramPermission(input.request, chatId, undefined, callback.providerAlias))) {
