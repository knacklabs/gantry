import { claimPermissionInteractionCallback, releasePermissionInteractionCallback, } from '../../application/interactions/pending-interaction-durability.js';
import { logger } from '../../infrastructure/logging/logger.js';
import { decisionForMode, formatPermissionReceiptText, } from '../permission-interaction.js';
...
export async function claimAndSettleTelegramPermissionPrompt(input) {
    ...
    if (await settleTelegramPermissionPrompt(input, decision))
        return 'settled';
    await releasePermissionInteractionCallback({ claim: claimed.claim });
    return 'retryable';
}
async function settleTelegramPermissionPrompt(input, decision) {
    const pending = input.pendingPrompts.get(input.providerAlias);
    if (!pending || !input.api)
        return false;
    const requestId = pending.request.requestId;
    const receipt = formatPermissionReceiptText(requestId, pending.request, decision);
    let deleted = false;
    if (decision.approved && decision.mode !== 'cancel') {
        try {
            ...
            logger.debug({ requestId, err: input.sanitizeErrorMessage(err) }, 'Failed to delete approved Telegram permission prompt; editing fallback receipt');
        }
    }
    ...
    if (!deleted) {
        try {
            ...
            logger.debug({ requestId, err: input.sanitizeErrorMessage(err) }, 'Failed to update Telegram permission prompt message');
            try {
                ...
                logger.debug({ requestId, err: input.sanitizeErrorMessage(sendErr) }, 'Failed to send Telegram permission receipt fallback');
                return false;
            }
        }
    }
