export function escapeTelegramMarkdownV2Plain(text) {
    return text.replace(/[_*~[\]()`>#+\-=|{}.!\\]/g, '\\$&');
}
export function escapeTelegramMarkdownV2Literal(text) {
    return text.replace(/[_*[\]()~`>#+\-=|{}.!\\]/g, '\\$&');
}
export function escapeTelegramMarkdownV2CodeSegment(segment) {
...
export function escapeTelegramMarkdownV2LinkSegment(segment) {
    const match = /^\[([\s\S]+)]\(([\s\S]+)\)$/.exec(segment);
    if (!match)
        return escapeTelegramMarkdownV2Plain(segment);
 ...
 */
export function escapeTelegramMarkdownV2(text, options = {}) {
    if (!text)
        return text;
    ...
    while ((match = protectedPattern.exec(text)) !== null) {
        if (match.index > lastIndex) {
            out += escapeTelegramMarkdownV2PlainSegment(text.slice(lastIndex, match.index), options);
        }
        const token = match[0];
        if (token.startsWith('`')) {
...
const TELEGRAM_STYLE_MARKERS = new Set([
    '_',
    '*',
    '~',
    '|',
...
function escapeTelegramMarkdownV2PlainSegment(text, options) {
    if (!text)
        return text;
    if (!options.preserveStyleMarkers) {
