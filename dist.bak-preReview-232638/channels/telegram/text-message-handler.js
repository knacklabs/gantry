...
export async function handleTelegramTextMessage(input) {
    ...
    await input.opts.onChatMetadata(chatJid, timestamp, chatName, 'telegram', isGroup, { providerAccountId: input.opts.providerAccountId });
    const hasRegisteredRoute = findConversationRoutesForChat(input.opts.conversationRoutes(), chatJid, threadId?.toString()).length > 0;
