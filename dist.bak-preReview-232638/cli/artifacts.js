...
import { ARTIFACTS_DIR, STORAGE_POSTGRES_SCHEMA, STORAGE_POSTGRES_URL, } from '../config/index.js';
...
export async function runArtifactsCommand(args) {
...
async function rebake(manifestHash) {
    if (!manifestHash) {
    ...
    try {
        ...
        if (outcome === 'in_flight') {
            p.log.error(`Manifest ${manifestHash} is baking right now (started ${row.updatedAt}). Wait for it to finish or fail, then rebake.`);
            return 1;
        }
        if (outcome === 'lost_race') {
            p.log.error(`Manifest ${manifestHash} changed concurrently; re-run the command to see its current status.`);
            return 1;
        }
        p.log.success(`Re-queued bake for ${manifestHash}. A fleet worker will rebuild and re-verify it.`);
        return 0;
    }
    catch (err) {
