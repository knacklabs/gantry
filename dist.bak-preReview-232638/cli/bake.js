...
import { bakeReapStalenessMs, resetToolchainBakeForRequeue, } from '../jobs/toolchain-bake-reaper.js';
...
export async function runBakeCommand(args) {
...
async function bakeStatus() {
    ...
    try {
        const rows = await getRuntimeStorage().repositories.runtimeDependencies.listRuntimeDependencies({ appId: 'default' });
        if (rows.length === 0) {
            p.note('No toolchain bake manifests recorded.', 'Bake Status');
            return 0;
        }
        const lines = rows.map((row) => `${row.manifestHash.slice(0, 16)}  status=${row.status}` +
 ...
 */
async function bakeRebake(manifestHashInput) {
    ...
    try {
        ...
        switch (outcome) {
            case 'requeued':
                p.log.success(`Re-queued bake for ${row.manifestHash}. A fleet worker will rebuild it.`);
                return 0;
            case 'in_flight':
                p.log.error(`Manifest ${row.manifestHash} is baking right now (started ${row.updatedAt}). Wait for it to finish or fail, then rebake.`);
                return 1;
            case 'lost_race':
                p.log.error(`Manifest ${row.manifestHash} changed concurrently; re-run \`gantry bake status\` to see where it landed.`);
                return 1;
            case 'not_resettable':
                p.log.error(`Manifest ${row.manifestHash} is ${row.status}; bake rebake only resets failed or stale baking manifests. For a quarantined uploaded/activated artifact use: gantry artifacts quarantine rebake ${row.manifestHash}`);
                return 1;
        }
    }
    catch (err) {
        p.log.error(`Rebake failed: ${err instanceof Error ? err.message : String(err)}`);
        return 1;
    }
    finally {
        await queue.stop();
 ...
 */
async function findManifest(repository, manifestHashInput) {
    ...
    const matches = rows.filter((row) => row.manifestHash.replace(/^sha256:/, '').startsWith(needle));
    if (matches.length === 1)
        return matches[0];
    if (matches.length > 1) {
        p.log.error(`Manifest hash prefix ${manifestHashInput} is ambiguous: ${matches
