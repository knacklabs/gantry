...
export async function runBrainCommand(runtimeHome, args) {
    ...
    if (command === 'import' && value) {
        return withBrain(runtimeHome, async (brain, appId, close) => {
            try {
                ...
                p.log.success(`Brain import complete: ${summary.created} created, ${summary.updated} updated, ${summary.files} files scanned.`);
                return 0;
            }
            finally {
                await close();
