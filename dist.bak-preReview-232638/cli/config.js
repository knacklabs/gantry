...
function isBlockedDirectProviderCredential(key) {
    const classified = classifyConfigKey(key);
    return (classified?.lane === 'agent-credential' ||
        classified?.lane === 'non-secret-setting');
}
function blockedKeyMessage(key) {
    return (classifyConfigKey(key)?.message ||
...
function isSensitiveKey(key) {
    ...
    if (classifyConfigKey(key)?.lane === 'runtime-secret')
        return true;
    return /(TOKEN|SECRET|PASSWORD|API_KEY|ACCESS_KEY|PRIVATE_KEY|DATABASE_URL|DB_URL|POSTGRES_URL)/i.test(key);
}
...
function formatValue(key, value, raw) {
    if (raw || (!isSensitiveKey(key) && !hasUrlUserInfo(value))) {
        return value;
    }
    return maskValue(value);
}
function runList(runtimeHome) {
...
function runGet(runtimeHome, args) {
    const [key, ...rest] = args;
    if (!key) {
...
function runSet(runtimeHome, args) {
    ...
    if (!value) {
        p.log.error('Value cannot be empty. Use `gantry config unset <KEY>` to remove a key.');
        return 1;
    }
    ensureRuntimeLayout(runtimeHome);
...
function runUnset(runtimeHome, args) {
    const [key] = args;
    if (!key) {
...
export function runConfigCommand(runtimeHome, args) {
