...
import { CONTROL_API_SCOPES, isValidControlId, } from '../control/server/auth.js';
import { parseEnvContent } from '../shared/env-file.js';
export async function controlApiRequest(runtimeHome, input) {
...
function readRuntimeControlEnv(runtimeHome) {
    return {
...
function isCompleteControlApiKeyEntry(input) {
    if (!isRecord(input))
        return false;
    return (typeof input.kid === 'string' &&
        ...
        input.scopes.every((scope) => typeof scope === 'string' &&
            CONTROL_API_SCOPES.includes(scope.trim())));
}
...
function controlSocketPath(runtimeHome, env) {
    if (env.GANTRY_CONTROL_BASE_URL?.trim())
        return undefined;
    if (Number(env.GANTRY_CONTROL_PORT || 0) > 0)
        return undefined;
    return (env.GANTRY_CONTROL_SOCKET_PATH?.trim() ||
