...
import { RestDiscordSetupDiscoveryClient, trimDiscordSetupCredentials, } from '../channels/discord-setup-discovery.js';
...
function parseDiscordApproverIds(raw) {
    if (!raw?.trim())
        return [];
    return [
        ...new Set(raw
...
async function chooseDiscordChannelForConnect(credentials, discoveryClient) {
...
export async function runDiscordConnectCommand(runtimeHome, discoveryClient = new RestDiscordSetupDiscoveryClient(), requestedAgentId, requestedAgentName) {
    ...
    const botToken = await promptForValue({
        ...
        hide: true,
        validate: (value) => value?.trim() ? undefined : 'Discord bot token is required.',
    });
    if (botToken === null)
        return 1;
    const applicationId = await promptForValue({
        message: 'Discord application ID',
        validate: (value) => value?.trim() ? undefined : 'Discord application ID is required.',
    });
    ...
    if (!applicationSecret)
        return 1;
    const channelChoice = await chooseDiscordChannelForConnect(credentials, discoveryClient);
    ...
    const approverInput = channelChoice.type === 'selected'
        ? await promptForValue({
            message: 'Discord approver user IDs (comma-separated; must be members of this conversation)',
            defaultValue: '',
        })
        : '';
    ...
    if (channelChoice.type === 'selected') {
        ...
        p.log.success(`Registered ${registered.groupName} for Discord channel ${registeredChatTitle} in folder ${registered.folder}.`);
    }
    ...
    settings.providerAccounts[providerAccountId] = {
        agentId: providerAgentId,
        provider: 'discord',
        label: settings.providerAccounts[providerAccountId]?.label || 'Discord Default',
        runtimeSecretRefs: {
    ...
    p.outro(channelChoice.type === 'selected'
