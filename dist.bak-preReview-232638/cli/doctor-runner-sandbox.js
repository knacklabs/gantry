...
import { commandExists, detectPlatform, } from '../infrastructure/service/platform.js';
const RUNNER_SANDBOX_CHECK_ID = 'runner-sandbox';
const RUNNER_SANDBOX_CHECK_TITLE = 'Runner Sandbox';
function runnerSandboxCheck(input) {
    return {
...
export function inspectRunnerSandbox(settings) {
    if (!settings)
        return undefined;
    ...
    if (sandbox.provider === 'direct') {
        return runnerSandboxCheck({
            status: 'pass',
            message: 'direct compatibility mode is configured; no outer OS sandbox is enforced and this is not organisation-safe. Setup required: sandbox_runtime is required for safe-host execution.',
        });
    }
    ...
    if (platform === 'windows') {
        return runnerSandboxCheck({
            ...
            nextAction: 'Set runtime.sandbox.provider to direct or run Gantry on macOS/Linux.',
            action: {
                type: 'run_verification',
                label: 'Set runtime.sandbox.provider to direct or run Gantry on macOS/Linux.',
            },
        });
    }
    try {
    ...
    return runnerSandboxCheck({
        status: 'pass',
        message: 'sandbox_runtime is configured and OS sandbox support is available. Networked CLI/MCP tools must honor standard proxy env or they fail closed.',
    });
}
function inspectLinuxSandboxRuntimeTools() {
