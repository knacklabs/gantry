...
import { assertRuntimeEntryExists, getRuntimeEntryPath, } from '../infrastructure/service/package-paths.js';
import { commandExists, detectPlatform, getNodeMajorVersion, getNodeVersion, hasSystemdUser, } from '../infrastructure/service/platform.js';
import { envFilePath, ensureRuntimeWritable, } from '../config/settings/runtime-home.js';
...
import { hasRuntimeCredentialConfigured, resolveRuntimeEnvValue, } from './runtime-credential-check.js';
import { collectUnresolvedRuntimeSecretProviderIds } from './runtime-secret-status.js';
import { inspectSlackTokenLiveCheck, inspectTelegramTokenLiveCheck, } from './model-credential-verify.js';
import { openRuntimeGroupDb } from './runtime-group-db.js';
import { hasConfiguredChannelProvider, hasProcessableGroupForConfiguredChannelSettings, } from './doctor-runtime-config.js';
export function hasRuntimeConfig(runtimeHome) {
    try {
...
export async function hasProcessableGroupForConfiguredChannel(runtimeHome) {
    let settings;
    try {
...
function add(checks, check) {
    checks.push(check);
}
function addToReport(report, check) {
    const checks = [...report.checks, check];
    const blockingFailures = checks.filter((entry) => entry.status === 'fail').length;
...
function channelTokenRestartNextAction(providerId) {
    if (providerId !== 'slack' && providerId !== 'telegram')
        return undefined;
    return `re-run \`gantry provider connect ${providerId}\`, then \`gantry restart\``;
}
function loadSettingsForDoctor(runtimeHome) {
    try {
...
export function runDoctor(importMetaUrl, runtimeHome, runtimeSecretOptions = {}) {
    const checks = [];
    ...
    try {
        ...
        const runtimeHomeNextAction = err instanceof Error
    ...
    try {
        ...
        add(checks, {
            ...
            title: 'IPC Layout',
            status: 'pass',
            message: 'IPC base directory is writable. Use `gantry status` for Postgres-backed group counts.',
        });
    }
    catch (err) {
        const ipcLayoutNextAction = err instanceof Error
    ...
    if (settings) {
        if (enabledProviders.length > 0) {
            ...
            add(checks, {
                ...
                title: 'Runtime Settings',
                status: 'fail',
                message: 'Runtime settings are valid, but no providers are enabled in settings.yaml.',
        ...
        if (storageStatus === 'fail') {
            add(checks, {
                ...
                status: 'warn',
                message: 'Use the provided docker-compose.yml, a locally installed Postgres, or hosted Postgres.',
                nextAction: 'Start or provision Postgres yourself, then run `gantry setup` and paste the database URLs.',
                action: {
                    type: 'run_verification',
                    label: 'Start or provision Postgres yourself, then run `gantry setup` and paste the database URLs.',
                },
            });
        }
        const credentialMode = settings.credentialBroker.mode;
        const modelCredentialSecretValid = hasValidEncryptionSecret({
            SECRET_ENCRYPTION_KEY: resolveRuntimeEnvValue(env, 'SECRET_ENCRYPTION_KEY'),
            SECRET_ENCRYPTION_KEYRING_JSON: resolveRuntimeEnvValue(env, 'SECRET_ENCRYPTION_KEYRING_JSON'),
        });
        const modelCredentialNextAction = credentialMode === 'gantry' && !modelCredentialSecretValid
        ...
        add(checks, {
            ...
            status: credentialMode === 'gantry' && !modelCredentialSecretValid
                ? 'fail'
                : 'pass',
            message: credentialMode === 'gantry'
    ...
    const processViolations = validateRuntimeEnvPolicy(process.env, 'process environment').violations;
    ...
    if (sandboxCheck)
        add(checks, sandboxCheck);
    const credentialMode = settings?.credentialBroker.mode || 'gantry';
    const unresolvedRuntimeSecretProviderIds = runtimeSecretOptions.unresolvedRuntimeSecretProviderIds;
    for (const provider of providers) {
        const enabled = settings?.providers[provider.id]?.enabled ?? false;
        const credentialConfigured = (envKey) => hasRuntimeCredentialConfigured({
            settings,
            env,
        ...
        const missingKeys = provider.setup.envKeys.filter((envKey) => !credentialConfigured(envKey));
        const envCheckId = provider.id === 'telegram'
            ? 'telegram-token'
            : provider.id === 'slack'
                ? 'slack-tokens'
                : `${provider.id}-credentials`;
        const envCheckTitle = provider.id === 'telegram'
            ? 'Telegram Token'
            : provider.id === 'slack'
                ? 'Slack Tokens'
        ...
        if (!enabled) {
            ...
            const nextAction = (unresolvedRuntimeSecretProviderIds?.has(provider.id)
    ...
    const modelAccessStatus = credentialMode === 'gantry' ? 'pass' : 'warn';
    const modelAccessMessage = credentialMode === 'gantry'
...
export async function runDoctorWithNetwork(importMetaUrl, runtimeHome, options = {}) {
    ...
    if (validateTelegramToken) {
        const telegramCheck = settings
            ...
            : null;
        if (telegramCheck)
            report = addToReport(report, telegramCheck);
    }
    if (validateSlackToken) {
        const slackCheck = settings
            ...
            : null;
        if (slackCheck)
            report = addToReport(report, slackCheck);
    }
    ...
    if (settings) {
        report = addToReport(report, await inspectModelCredentialReadiness(runtimeHome, settings, {
