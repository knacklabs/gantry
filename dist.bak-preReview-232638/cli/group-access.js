...
function fallbackSummary(access) {
    ...
    for (const skill of asArray(sources.skills)) {
        ...
        if (name)
            connected.push({ label: name, detail: 'skill' });
    }
    ...
    for (const tool of asArray(sources.tools)) {
        ...
        if (id)
            connected.push({ label: id, detail: String(tool.kind ?? 'tool') });
    }
 ...
 */
function formatAgentAccess(_agentId, access) {
    const summary = isRecord(access) && isRecord(access.summary)
        ? access.summary
    ...
    section('Connected', summaryEntries(summary.connected), (entry) => `  - ${entry.label} (${entry.detail})`);
    section('Allowed', summaryEntries(summary.allowed), (entry) => `  - ${entry.label} (${entry.detail})`);
    section('Needs attention', summaryEntries(summary.needsAttention), (entry) => `  - ${entry.label}. Next: ${entry.detail}`);
    section('Suggested cleanup', summaryEntries(summary.suggestedCleanup), (entry) => `  - ${entry.label}. Reason: ${entry.detail}`);
    lines.push('', 'Details: use --json or audit/events for exact ids and rule details.');
    return lines.join('\n');
}
function agentIdFromSelector(value) {
    const trimmed = value.trim();
    return trimmed.startsWith('agent:') ? trimmed : `agent:${trimmed}`;
}
export async function runAccess(runtimeHome, rest) {
    ...
    if (!action || !selector || (action !== 'show' && action !== 'apply')) {
        p.log.error('Usage: gantry agent access show <agent> [--json] | gantry agent access apply <agent> --file <path|-> | gantry agent access preset <agent> <full|locked>');
        return 1;
    }
    ...
    try {
        if (action === 'show') {
            ...
            p.note(formatAgentAccess(agentIdFromSelector(selector), access), 'Agent skills & permissions');
            return 0;
        }
        ...
        try {
            ...
            p.log.error('Access document must be valid JSON with sources and selections.');
            return 1;
        }
...
function agentFolderFromSelector(value) {
    return value.trim().replace(/^agent:/, '');
}
async function runAccessPreset(runtimeHome, selector, preset) {
    if (!selector || !preset) {
    ...
    try {
        ...
        if (!agent) {
            p.log.error(`No configured agent named "${folder}". Run "gantry agent list" to see configured agents.`);
            return 1;
        }
        if (agent.accessPreset === preset) {
            p.log.success(`Agent "${folder}" already uses the ${preset} access preset.`);
            return 0;
        }
        const settings = {
            ...previousSettings,
        ...
        p.log.success(`Set agent "${folder}" access preset to ${preset} in settings.yaml.${reconciled ? '' : ' Restart Gantry to apply.'}`);
        if (preset === 'locked') {
            p.log.info('Locked agents cannot request skills, MCP servers, access, settings, or admin tools; they run only with pre-provisioned capabilities.');
