...
function parseModeArg(raw) {
    ...
    if (trimmed === 'trigger' || trimmed === 'drop')
        return trimmed;
    return null;
}
export function parseGroupAddArgs(args) {
    const options = {
    ...
    for (let i = 0; i < args.length; i += 1) {
        ...
        if (arg === '--requires-trigger') {
            ...
            if (parsed === null) {
                return {
                    error: 'Invalid value for --requires-trigger. Use true/false (or yes/no, on/off).',
                };
            }
...
export function parseGroupRemoveArgs(args) {
    const options = {
        assumeYes: false,
    };
    for (let i = 0; i < args.length; i += 1) {
...
export function parseGroupTriggerArgs(args) {
    const options = {
        disable: false,
    };
    for (let i = 0; i < args.length; i += 1) {
    ...
    if (!options.selector) {
        return {
            error: 'Missing agent selector. Usage: gantry agent trigger <jid|folder> <word>|--off',
        };
    }
    if (!options.disable && !options.trigger) {
        return {
            error: 'Missing trigger word. Usage: gantry agent trigger <jid|folder> <word>',
        };
    }
    return options;
}
export function parseGroupPolicyArgs(args) {
    const options = {
        clear: false,
    };
    for (let i = 0; i < args.length; i += 1) {
        ...
        if (arg === '--allow') {
            ...
            if (parsed === null) {
                return {
                    error: 'Invalid value for --allow. Use "*" or a comma-separated sender list.',
                };
            }
    ...
    if (!options.selector) {
        return {
            error: 'Missing agent selector. Usage: gantry agent policy <jid|folder> --allow <...> [--mode trigger|drop] or --clear',
        };
    }
    if (options.clear &&
...
export function parseGroupPolicyDefaultArgs(args) {
    const options = {};
    for (let i = 0; i < args.length; i += 1) {
        const arg = args[i];
        if (arg === '--channel') {
...
export function parseGroupPolicyShowArgs(args) {
    const options = {};
    for (let i = 0; i < args.length; i += 1) {
        const arg = args[i];
        if (arg === '--channel') {
