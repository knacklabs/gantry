...
import { createProfileFileMirrorExists, createProfileFileMirrorWriter, } from '../platform/profile-file-mirror.js';
...
import { addControlSenderForAgent, ensureConfiguredConversationBinding, loadRuntimeSettings, writeDesiredRuntimeSettings, } from '../config/settings/runtime-settings.js';
...
export function findConversationIdForAgent(settings, agentId, providerId) {
...
export function conversationIdsForProvider(settings, providerId) {
    return Object.entries(settings.conversations)
        .filter(([, conversation]) => settings.providerAccounts[conversation.providerAccount]?.provider ===
        providerId)
        .map(([conversationId]) => conversationId);
}
export async function pruneAgentSenderPolicyOverride(runtimeHome, jid, folder) {
    ...
    try {
        ...
        for (const [bindingId, binding] of Object.entries(settings.bindings)) {
            ...
            const installKey = binding.installKey ??
                Object.entries(conversation.installedAgents).find(([, install]) => install.agentId === folder &&
                    (install.threadId ?? '') === (binding.threadId ?? ''))?.[0] ??
                folder;
...
export async function pruneDesiredStateAgent(input) {
    ...
    try {
        ...
        const delegateReferences = Object.entries(settings.agents)
            .filter(([folder, agent]) => folder !== input.folder &&
            (agent.delegates ?? []).some((delegate) => delegate === input.folder ||
                delegate?.agent === input.folder))
            .map(([folder]) => folder);
        ...
        for (const [accountId, account] of Object.entries(settings.providerAccounts)) {
            if (account.agentId !== input.folder)
                continue;
            for (const [conversationId, conversation] of Object.entries(settings.conversations)) {
                ...
                // their configuration.
                for (const [installKey, install] of Object.entries(conversation.installedAgents)) {
                    if (install.providerAccountId !== accountId &&
                ...
                if (conversation.providerAccount === accountId) {
                    const replacement = survivingInstalls.find((install) => install.providerAccountId &&
                        install.providerAccountId !== accountId)?.providerAccountId;
                    if (!replacement) {
                        throw new Error(`cannot remove ${input.folder}: conversation ${conversationId} still hosts ${survivingInstalls.length} agent(s) but has no other provider account to own it`);
                    }
 ...
 */
export async function disableRemovedAgentProjection(folder) {
    try {
        const { closeRuntimeStorage, getRuntimeStorage, initializeRuntimeStorage } = await import('../adapters/storage/postgres/runtime-store.js');
        await initializeRuntimeStorage();
        try {
            const disabled = await getRuntimeStorage().repositories.agents.disableAgent({
                appId: 'default',
        ...
        p.log.warn(`Agent ${folder} removed from desired state, but its projected agents row could not be disabled (${error}); it stays removed and will not resurrect.`);
...
export function resolveRoutelessAgentFolder(input) {
    ...
    if (!folder || !Object.hasOwn(input.settings.agents, folder))
        return null;
    const hasRoutes = Object.values(input.groups).some((group) => group.folder === folder);
...
function inferTelegramPrivateChatApprover(chatJid) {
...
export async function seedTelegramControlApproverForAgent(input) {
    ...
    const settings = loadRuntimeSettings(input.runtimeHome);
    const previousSettings = structuredClone(settings);
    const added = addControlSenderForAgent(settings, 'telegram', input.agentFolder, approver);
    if (added) {
        await writeDesiredRuntimeSettings({
            runtimeHome: input.runtimeHome,
            settings,
...
export function normalizeGroupAddSelector(raw) {
...
function normalizeAgentDisplayName(raw) {
    const value = raw.trim();
    return value || 'Assistant';
}
export async function loadDatabase(runtimeHome) {
    ensureRuntimeLayout(runtimeHome);
    return openRuntimeGroupDb(runtimeHome);
}
export function listGroupsWithJid(groups) {
    return Object.entries(groups)
        .map(([jid, group]) => ({ jid, group }))
        .sort((a, b) => a.group.name.localeCompare(b.group.name));
}
function resolveGroup(groups, selector) {
    if (groups[selector]) {
        return { jid: selector, group: groups[selector] };
    }
    const byFolder = Object.entries(groups).find(([, group]) => group.folder === selector);
    if (!byFolder)
        return null;
    return {
        jid: byFolder[0],
...
function resolveBareJidRoute(groups, rawSelector, bareJid) {
    const matches = Object.entries(groups).filter(([jid]) => jid !== bareJid && parseAgentThreadQueueKey(jid).chatJid === bareJid);
    if (matches.length === 0)
        return { found: null };
    if (matches.length > 1) {
        return {
...
function resolveAgentIdRoute(groups, selector) {
    const matches = Object.entries(groups).filter(([jid]) => parseAgentThreadQueueKey(jid).agentId === selector);
    if (matches.length === 0)
        return { found: null };
...
export function resolveGroupSelector(groups, rawSelector) {
    ...
    const telegramMatch = normalizedTelegram && groups[normalizedTelegram]
    ...
    if (folderMatch)
        return { found: folderMatch };
    if (telegramMatch)
        return { found: telegramMatch };
    const bareJid = normalizeGroupAddSelector(selector);
    const bareRouteMatch = bareJid
        ? resolveBareJidRoute(groups, selector, bareJid)
        : { found: null };
    if (bareRouteMatch.error)
        return bareRouteMatch;
    if (bareRouteMatch.found)
        return { found: bareRouteMatch.found };
    return { found: null };
}
export function isInteractiveTerminal() {
    return Boolean(process.stdin.isTTY && process.stdout.isTTY);
}
function ensureFolderAvailable(runtimeHome, desiredFolder, groups) {
    const inDb = Object.values(groups).some((group) => group.folder === desiredFolder);
    if (inDb) {
        throw new Error(`Folder "${desiredFolder}" is already used by another group.`);
    }
    const folderPath = path.join(runtimeHome, 'agents', desiredFolder);
    if (fs.existsSync(folderPath)) {
        throw new Error(`Folder already exists on disk: ${folderPath}. Choose another folder with --folder.`);
    }
}
export function allocateGroupFolder(options) {
    if (options.preferredFolder) {
        ...
        if (!isValidWorkspaceFolder(explicit)) {
            throw new Error(`Invalid folder "${explicit}". Use letters, numbers, _ or - only.`);
        }
        ensureFolderAvailable(options.runtimeHome, explicit, options.groups);
        return explicit;
    }
    const base = slugifyFolder(options.seed);
    const usedFolders = new Set(Object.values(options.groups).map((group) => group.folder));
    for (let i = 0; i < 1000; i += 1) {
        const candidate = i === 0 ? base : `${base}_${i + 1}`;
        if (!isValidWorkspaceFolder(candidate))
            continue;
        if (usedFolders.has(candidate))
            continue;
...
export async function ensureGroupFiles(runtimeHome, folder, agentName, fileArtifactStore) {
