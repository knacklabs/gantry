...
import { defaultAgentNameFromSettings, displayAgentName, } from './main-agent.js';
...
export async function runList(runtimeHome) {
    let db = null;
    try {
    ...
    try {
        ...
        try {
            ...
            p.log.error(`Could not read registered groups from database. The DB may be corrupted. Details: ${errorMessage(err)}`);
            return 1;
        }
        if (groups.length === 0) {
            p.log.warn('No agents are registered in this runtime home.');
            const connectCommands = getProviderIds().map((channel) => `\`gantry provider connect ${channel}\``);
            p.log.info(`Next action: run \`gantry agent add <chat-id>\` or ${connectCommands.join(' / ')}.`);
            return 0;
        }
