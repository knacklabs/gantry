...
import { capabilityToToolRule, ensureConfiguredConversationBinding, loadDesiredRuntimeSettingsForWrite, loadRuntimeSettings, writeDesiredRuntimeSettings, } from '../config/settings/runtime-settings.js';
import { defaultTriggerForAgentName, displayAgentName, defaultAgentNameFromSettings, isDefaultAgentLastRoute, normalizeDefaultAgentName, } from './main-agent.js';
...
import { parseGroupAddArgs, parseGroupPolicyArgs, parseGroupPolicyDefaultArgs, parseGroupPolicyShowArgs, parseGroupRemoveArgs, parseGroupTriggerArgs, } from './group-args.js';
import { allocateGroupFolder, conversationIdsForProvider, disableRemovedAgentProjection, ensureGroupFiles, findConversationIdForAgent, formatAgentHarnessLine, isInteractiveTerminal, loadDatabase, normalizeGroupAddSelector, pruneAgentSenderPolicyOverride, pruneDesiredStateAgent, resolveGroupSelector, seedTelegramControlApproverForAgent, usage, } from './group-helpers.js';
import { printPolicyChannel } from './group-policy-format.js';
import { buildAgentToolAccessView, buildRequestableAdminToolAccess, formatAgentToolAccess, PERMISSION_GATED_NATIVE_TOOLS, } from '../shared/tool-access-view.js';
...
function displayNameForConfiguredConversation(settings, jid, fallback) {
    const provider = providerFromGroupJid(jid);
    if (!provider)
        return fallback;
    const existingConversation = Object.values(settings.conversations).find((conversation) => {
...
async function runInfo(runtimeHome, rawSelector) {
    if (!rawSelector) {
    ...
    try {
        ...
        const configuredTools = (settings.agents[found.group.folder]?.capabilities ?? []).map(({ id }) => capabilityToToolRule(id));
        const enabledAdminTools = selectedAdminToolNames(configuredTools);
        const gatedTools = PERMISSION_GATED_NATIVE_TOOLS.filter((toolName) => !configuredTools.some((configured) => configured === toolName || configured.startsWith(`${toolName}(`)));
        const lines = [
            `JID: ${found.jid}`,
            ...
            '',
            formatAgentToolAccess(buildAgentToolAccessView({
                configuredTools,
                defaultTools: [],
                availableButGatedTools: gatedTools,
                requestableAdminTools: buildRequestableAdminToolAccess(enabledAdminTools),
...
const selectedAdminToolNames = (tools) => new Set(tools.map(adminMcpToolNameFromFullName).filter((name) => name !== null));
async function runAdd(runtimeHome, args) {
    ...
    try {
        ...
        if (normalized.startsWith('tg:')) {
            const providerAccountId = Object.entries(settings.providerAccounts).find(([, account]) => account.provider === 'telegram')?.[0];
            ...
            if (!token) {
                ...
                p.log.info('Next action: run `gantry provider connect telegram` or configure provider_accounts.telegram_default.runtime_secret_refs.bot_token.');
                return 1;
            }
            ...
            if (!access.ok) {
                ...
                p.note(`${access.message}\nNext action: ${access.nextAction || 'Fix Telegram access and retry.'}`, 'Telegram Check');
                return 1;
            }
        ...
        try {
            await ensureGroupFiles(runtimeHome, agentFolder, displayName, db.getFileArtifactStore());
        }
        catch (err) {
        ...
        const defaultTrigger = defaultTriggerForAgentName(defaultAgentNameFromSettings(settings));
        const record = {
            name: displayName,
            folder: agentFolder,
        ...
        try {
            await db.setConversationRoute(makeAgentThreadQueueKey(normalized, agentIdForFolder(agentFolder)), record);
            try {
            ...
            try {
                ...
                if (seededApprover) {
                    p.log.info(`Enabled permission approvals for Telegram sender ${seededApprover} in ${agentFolder}.`);
                }
            }
            catch (err) {
                p.log.warn(`Agent was added, but Telegram permission approver seeding failed: ${errorMessage(err)}`);
            }
        }
        catch (err) {
        ...
        p.log.success(`Added agent ${displayName} (${normalized}) in folder ${agentFolder}.`);
        if (chatProbeMessage)
...
async function runName(runtimeHome, args) {
    ...
    try {
        ...
        p.log.success(`Default agent name updated from "${previous}" to "${nextName}".`);
        return 0;
    }
    catch (err) {
        p.log.error(`Could not update default agent name: ${err instanceof Error ? err.message : String(err)}`);
        return 1;
    }
}
async function runRemove(runtimeHome, args) {
    ...
    try {
        ...
        if (!resolved.found) {
            ...
            if (routelessExit !== null)
                return routelessExit;
        ...
        if (isDefaultAgentLastRoute(groups, found.group.folder)) {
            p.log.error(`${found.group.folder} is the default agent and must keep at least one route; it cannot be removed.`);
            return 1;
        }
        if (!parsed.assumeYes) {
        ...
        const policyPrune = await pruneAgentSenderPolicyOverride(runtimeHome, parseAgentThreadQueueKey(found.jid).chatJid, found.group.folder);
        if (policyPrune.error) {
            p.log.warn(`Agent removed, but sender policy cleanup failed for folder ${found.group.folder}: ${policyPrune.error}`);
        }
        else if (policyPrune.pruned) {
            p.log.info(`Removed sender policy override for folder ${found.group.folder}.`);
        }
        ...
        const remainingRoutes = Object.entries(await db.getAllConversationRoutes()).filter(([, group]) => group.folder === found.group.folder).length;
        ...
        if (desiredPrune.error) {
            ...
            p.log.error(`Removal did not persist: ${found.group.folder} still exists in desired state (${desiredPrune.error}). It will be recreated on the next reload.`);
            return 1;
        }
        if (desiredPrune.keptForDelegates?.length) {
            // Retained because other agents delegate to it.
            p.log.warn(`Route removed, but agent ${found.group.folder} is retained in desired state: still referenced as a delegate by ${desiredPrune.keptForDelegates.join(', ')}.`);
            p.log.info('Next action: remove those delegate references first if you want the agent fully deleted. The agent folder was left untouched.');
            return 0;
        }
        if (desiredPrune.pruned) {
            ...
            p.log.info(`Removed agent ${found.group.folder} from desired state${accounts > 0 ? ` (and ${accounts} orphaned provider account(s))` : ''}.`);
        }
        else if (remainingRoutes > 0) {
            p.log.info(`Agent ${found.group.folder} kept in desired state: ${remainingRoutes} route(s) still bound.`);
        }
...
async function runTrigger(runtimeHome, args) {
    ...
    try {
        ...
        try {
            ...
            if (providerId) {
                ...
                ensureConfiguredConversationBinding(settings, {
                    ...
                    jid: chatJid,
                    displayName: displayNameForConfiguredConversation(settings, chatJid, found.group.name),
        ...
        if (parsed.disable) {
            p.log.success(`Trigger requirement disabled for ${found.group.name} (${found.jid}).`);
            return 0;
        }
        p.log.success(`Trigger for ${found.group.name} (${found.jid}) is now "${nextGroup.trigger}".`);
        return 0;
    }
    finally {
        await db?.close();
...
async function runPolicy(runtimeHome, args) {
    ...
    try {
        ...
        if (!channel) {
            p.log.error(`Agent ${found.group.name} (${found.jid}) does not map to a registered provider.`);
            return 1;
        }
        const settings = loadRuntimeSettings(runtimeHome);
        const previousSettings = structuredClone(settings);
        ...
        const conversation = settings.conversations[findConversationIdForAgent(settings, found.group.folder, channel) ||
            ensured.conversationId];
        if (!conversation) {
            p.log.error(`Agent ${found.group.name} (${found.jid}) does not have a configured conversation.`);
            return 1;
        }
        if (parsed.clear) {
            ...
            p.log.success(`Cleared sender policy for ${found.group.name} (${found.group.folder}) in ${channel}.`);
            return 0;
        }
        ...
        p.log.success(`Updated ${channel} sender policy for ${found.group.name} (${found.group.folder}).`);
        return 0;
    }
    catch (err) {
...
async function runPolicyDefault(runtimeHome, args) {
    ...
    try {
        ...
        if (conversationIds.length === 0) {
            p.log.error(`No configured ${channel} conversations found. Add or connect a conversation before setting its sender policy.`);
            return 1;
        }
        ...
        p.log.success(`Updated sender policy for ${conversationIds.length} ${channel} conversation${conversationIds.length === 1 ? '' : 's'}.`);
        return 0;
    }
    catch (err) {
...
async function runPolicyShow(runtimeHome, args) {
...
export async function runAgentCommand(runtimeHome, args) {
