...
import { formatJobToolAccess, } from '../shared/tool-access-view.js';
...
export async function runJobsCommand(runtimeHome, args) {
    const [action, maybeJobId, ...rest] = args;
    if (action === 'list')
        return listJobs(runtimeHome, [maybeJobId, ...rest]);
    if (action === 'show' && maybeJobId)
        return showJob(runtimeHome, maybeJobId);
    ...
    p.log.error('Usage: gantry jobs list|show <job_id>|resume <job_id>|trigger <job_id>|set-route <job_id> --conversation <jid> --thread <id|null>|events <job_id> [--run <run_id>] [--full|--json]');
    return 1;
}
async function listJobs(runtimeHome, args) {
...
async function showJob(runtimeHome, jobId) {
...
async function resumeJob(runtimeHome, jobId) {
    const response = (await controlApiRequest(runtimeHome, {
        method: 'POST',
    ...
    if (setup?.blockers?.length) {
        lines.push('Setup Blockers:', ...setup.blockers.map((blocker) => `  ${blocker.requirementType ?? 'requirement'}:${blocker.requirementId ?? 'unknown'} ${blocker.message ?? ''}`));
    }
    p.note(lines.join('\n'), `Job ${jobId}`);
    return 0;
}
async function triggerJob(runtimeHome, jobId) {
    const response = (await controlApiRequest(runtimeHome, {
        method: 'POST',
        path: `/v1/jobs/${encodeURIComponent(jobId)}/trigger`,
    }));
    p.note([`Trigger ID: ${response.triggerId ?? '(unknown)'}`].join('\n'), `Job ${jobId} queued`);
    return 0;
}
async function setJobRoute(runtimeHome, jobId, args) {
    const parsed = parseSetRouteArgs(args);
    if (!parsed) {
        p.log.error('Usage: gantry jobs set-route <job_id> --conversation <jid> --thread <id|null> [--label <label>] [--workspace <workspace_key>]');
        return 1;
    }
...
async function listJobEvents(runtimeHome, jobId, args) {
    const params = new URLSearchParams();
    let full = false;
    let json = false;
    for (let index = 0; index < args.length; index += 1) {
    ...
    const response = (await controlApiRequest(runtimeHome, {
        method: 'GET',
        path: `/v1/jobs/${encodeURIComponent(jobId)}/events${params.toString() ? `?${params}` : ''}`,
    }));
...
function jobAgentLabel(job) {
    return agentIdForJobWorkspaceKey(job.workspaceKey);
}
function jobConversationLabel(job) {
    const route = job.notificationRoutes?.find((entry) => entry.conversationJid || entry.label);
    if (route?.label)
        return route.label;
    const conversationJid = job.executionContext?.conversationJid ?? route?.conversationJid;
    return conversationJid ?? 'none';
}
function jobAccessRequirementsLabel(job) {
    const requirements = formatAccessRequirements(job.accessRequirements);
    return requirements.length > 0 ? requirements.join(', ') : 'none';
}
function jobNextActionLabel(job) {
    const nextAction = job.setup?.nextAction ?? job.recovery?.nextAction ?? job.health?.nextAction;
    if (!job.setup?.blockers?.length && !nextAction)
        return 'none';
...
function formatJobDetail(job) {
    ...
    if (job.setup?.blockers?.length) {
        lines.push('', 'Setup Blockers:', ...job.setup.blockers.map((blocker) => `  ${blocker.requirementType ?? 'requirement'}:${blocker.requirementId ?? 'unknown'} ${blocker.message ?? ''}`));
    }
    if (job.recovery?.lastError) {
        lines.push('', `Recovery Error: ${job.recovery.lastError}`);
    }
    if (job.recentRunErrors?.length) {
        lines.push('', 'Recent Run Errors:', ...job.recentRunErrors.map((error) => `  ${error.runId} ${error.status}: ${error.errorSummary}`));
    }
    return lines.join('\n');
}
function formatJobRecovery(recovery) {
...
function formatJobNextAction(setup, fallbackNextAction) {
    const blocker = jobSetupBlockerFromUnknown(setup?.blockers?.[0]);
    if (blocker)
        return setupActionLabel(blocker);
    return setupActionLabelFromNextAction(setup?.nextAction ?? fallbackNextAction, 'Fix setup, then resume the job.');
}
function formatJobEvents(events, options = {}) {
...
function parseSetRouteArgs(args) {
    let conversationJid = '';
    let threadId;
    let label = 'primary';
    let workspaceKey;
    ...
    if (!conversationJid || threadId === undefined)
        return null;
    return { conversationJid, threadId, label, workspaceKey };
}
function formatEventPayload(payload, options = {}) {
    if (!payload)
        return '';
...
function formatAccessRequirements(accessRequirements) {
    return (accessRequirements ?? [])
        ...
        if (!capabilityId)
            return undefined;
        const implementationLabel = target.implementation?.name || target.implementation?.kind;
