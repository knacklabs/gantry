...
function localEnvSummary(runtimeHome) {
    ...
    return [
        ...
        `Credential encryption: ${hasValidEncryptionSecret({
...
async function runLocalDoctor(runtimeHome) {
...
export async function runLocalCommand(runtimeHome, args) {
    const command = args[0] || 'status';
    if (command === 'setup' || command === 'start') {
        p.note(composeGuidance(), command === 'setup' ? 'Local Setup' : 'Local Start');
        return 0;
    }
    if (command === 'status') {
        p.note([localEnvSummary(runtimeHome), '', composeGuidance()].join('\n'), 'Local Status');
        return 0;
    }
    if (command === 'stop') {
        p.note('Gantry does not stop local databases. Use `docker compose stop` if you started the provided Compose stack.', 'Local Stop');
        return 0;
    }
    if (command === 'logs') {
        p.note('Gantry does not own local service logs. Use `docker compose logs --tail 160` for the provided Compose stack.', 'Local Logs');
        return 0;
    }
    if (command === 'doctor') {
        return runLocalDoctor(runtimeHome);
    }
    p.log.error('Unknown local command. Use setup, start, stop, status, logs, or doctor.');
    return 1;
}
