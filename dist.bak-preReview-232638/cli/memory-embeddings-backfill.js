...
function reportResult(result) {
    ...
    if (result.alreadyRunning) {
        p.log.warn('Memory embedding backfill skipped: another run is already in progress.');
        return 0;
    }
    if (result.mode === 'provider_batch' && result.submitted > 0) {
        p.log.success(`Memory embedding batch submitted: ${fmt(result.submitted)} items queued. Status will update from scheduled polling. Run \`gantry memory status\` (or /memory-status) to track ready/pending counts.`);
        return 0;
    }
    if (result.status === 'failed') {
        p.log.error(`Memory embedding backfill failed: ${result.errorMessage ?? 'storage failure'}`);
        return 1;
    }
    ...
    p.log.success(`Memory embedding backfill complete: ${fmt(result.indexed)} indexed, ${fmt(result.pending)} pending.`);
    return 0;
}
function pausedMessage(result) {
...
export async function runEmbeddingBackfillCommand(runtimeHome, args) {
    ...
    if (!embeddings.enabled || embeddings.provider === 'disabled') {
        p.log.error('Memory embedding backfill failed: embeddings are not enabled. Run `gantry memory embeddings openai` first.');
        return 1;
    }
