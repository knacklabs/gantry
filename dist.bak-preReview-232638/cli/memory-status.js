...
export function deriveMemoryMode(health) {
    const embeddingsOn = health.embeddingsEnabled && health.embeddingProvider !== 'disabled';
    const dreamingOn = health.dreamingEnabled;
    const hybridLiveNote = 'hybrid lexical + semantic recall is live for indexed memories. Run `gantry memory embeddings backfill` to index existing memories; check `/memory-status` for live ready/pending counts';
...
export function collectMemoryStatus(runtimeHome) {
...
export function formatMemoryStatusExtras(snapshot) {
    const lines = [];
    ...
    lines.push('  counts: available through runtime control events and DB observability');
    lines.push('');
    lines.push('Dreaming');
    lines.push('  audit: persisted in Postgres memory_dream_runs and memory_dream_decisions');
    return lines.join('\n');
}
