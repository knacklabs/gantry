...
import { isEmbeddingProviderRegistered, validateEmbeddingProviderReady, } from '../memory/memory-embeddings.js';
import { readEnvFile } from '../config/env/file.js';
import { collectMemoryStatus, formatMemoryStatusExtras, } from './memory-status.js';
import { inspectMemoryHealth } from './memory-health.js';
import { envFilePath } from '../config/settings/runtime-home.js';
import { DEFAULT_EMBED_DIMENSIONS, DEFAULT_EMBED_MODEL, getProviderManagedMemoryDefaults, loadRuntimeSettings, noteRestartRequired, writeDesiredRuntimeSettings, } from '../config/settings/runtime-settings.js';
...
function resolveEffectiveModel(configuredModel, globalModel, hardDefault) {
...
function formatMemoryStatus(runtimeHome) {
    ...
    const extractorModel = resolveEffectiveModel(settings.memory.llm.models.extractor, globalModel, hardDefaults.extractor);
    const dreamingModel = resolveEffectiveModel(settings.memory.llm.models.dreaming, globalModel, hardDefaults.dreaming);
    const consolidationModel = resolveEffectiveModel(settings.memory.llm.models.consolidation, globalModel, hardDefaults.consolidation);
...
async function setEmbeddings(runtimeHome, provider) {
    const settings = loadRuntimeSettings(runtimeHome);
...
async function setDreaming(runtimeHome, enabled) {
    const settings = loadRuntimeSettings(runtimeHome);
    const previousSettings = structuredClone(settings);
    settings.memory.dreaming.enabled = enabled;
    if (enabled && !settings.memory.enabled)
...
export async function runMemoryCommand(runtimeHome, args) {
    ...
    if (command === 'embeddings') {
        ...
        const result = await setEmbeddings(runtimeHome, normalized);
        if (!result.ok) {
