...
export function formatPreviewWhy(preview) {
    ...
    if (selection?.model?.modelRoute?.metadata?.providerModelId) {
        lines.push(`provider model id: ${selection.model.modelRoute.metadata.providerModelId}`);
    }
...
export function parseAgentFlag(args) {
    ...
    if (inline)
        return inline.slice('--agent='.length).trim();
