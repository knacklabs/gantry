...
import { DEFAULT_SETUP_MODEL_ALIAS, memoryModelDefaultsForProvider, resolveModelSelection, resolveModelSelectionForWorkload, } from '../shared/model-catalog.js';
import { isModelFamilyAlias, resolveModelFamilyAlias, resolveModelSelectionForWorkloadWithFamilies, } from '../shared/model-families.js';
import { formatModelWhy } from '../shared/model-why-format.js';
import { chatAlias, configuredProviderIdsForCli, effectiveJobAlias, familyOrderFromSettings, fetchConfiguredProviders, formatModelList, formatModelStatus, memoryProviderFromSettings, memoryResetProviderFromSettings, providerForAlias, providerFromSettings, resolveSlot, } from './model-list-format.js';
import { providerLabel } from '../shared/model-catalog-availability.js';
import { applyProviderManagedMemoryDefaults, ensureRuntimeSettings, noteRestartRequired, writeDesiredRuntimeSettings, } from '../config/settings/runtime-settings.js';
...
async function preflightAliasProviders(input) {
    ...
    const hasFamilyAlias = input.aliases.some(({ alias }) => alias && isModelFamilyAlias(alias));
    ...
    for (const { alias, workload } of input.aliases) {
        ...
        if (resolved.ok) {
            ...
            providers.set(providerId, workload === 'chat' ? resolved.alias : providers.get(providerId));
        }
    }
    for (const [providerId, chatAlias] of providers) {
        const result = await input.preflight(input.runtimeHome, providerId, input.settings, chatAlias);
        if (result.ok)
            continue;
...
async function noteUnconfiguredProvider(runtimeHome, alias, providerId) {
    if (isModelFamilyAlias(alias))
        return;
    const configured = await fetchConfiguredProviders(runtimeHome);
    if (!configured || configured.has(providerId))
        return;
    console.warn(`Note: ${alias} runs on the ${providerLabel(providerId)} provider, which has no active credential. Run \`gantry credentials model set ${providerId}\` to use it.`);
}
async function noteUnconfiguredMemoryProviders(runtimeHome, settings) {
...
function modelValidationFailures(settings) {
    const slots = [
...
function selectedModelProviders(settings) {
    const selected = new Set();
    for (const { alias, workload } of [
        { alias: chatAlias(settings), workload: 'chat' },
        {
            alias: effectiveJobAlias(settings, 'oneTime'),
            workload: 'one_time_job',
        },
...
function formatTarget(settings, target) {
    if (target === 'chat') {
        return [
            'Chat model',
    ...
    if (target === 'jobs') {
        return [
            ...
            `recurring: ${settings.agent.recurringJobDefaultModel
                ? resolveSlot(settings.agent.recurringJobDefaultModel, 'recurring_job')
...
function formatWhy(settings, target) {
    if (target === 'chat') {
        return [
...
export async function runModelCommand(runtimeHome, args, options = {}) {
    const [action, target, alias] = args;
    const settings = ensureRuntimeSettings(runtimeHome);
    const preflight = options.preflightProvider ??
        ((runtimeHome, providerId, settings, chatAlias) => preflightModelProvider({
            runtimeHome,
            providerId,
            chatAlias,
            settings,
        }));
    const persistSettings = async (previousSettings, nextSettings) => {
    ...
    if (action === 'set') {
        ...
        if (target === 'chat' && alias) {
            ...
            if (agentId !== undefined) {
                ...
                if (!(await preflightAliasProviders({
                    runtimeHome,
                    settings,
                    preflight,
                ...
                console.log(`agent ${agentId} chat: ${resolved.alias} (${resolved.entry.displayName})`);
                noteRestartRequired(writeResult);
                await noteUnconfiguredProvider(runtimeHome, resolved.alias, resolved.entry.modelRoute.id);
                return 0;
            }
            ...
            const resolved = resolveModelSelectionForWorkloadWithFamilies(alias, 'chat', familyOrder);
            if (!resolved.ok) {
                console.error(resolved.message);
                return 1;
            }
            ...
            const newMemoryProvider = await memoryResetProviderFromSettings(runtimeHome, settings);
            if (oldMemoryProvider !== newMemoryProvider) {
                console.warn(`Memory models still on ${oldMemoryProvider} — run \`gantry model reset memory\` to re-derive.`);
            }
            return 0;
        }
        if (target === 'jobs' && alias) {
            ...
            const oneTime = resolveModelSelectionForWorkloadWithFamilies(alias, 'one_time_job', familyOrder);
            if (!oneTime.ok) {
                console.error(oneTime.message);
                return 1;
            }
            const recurring = resolveModelSelectionForWorkloadWithFamilies(alias, 'recurring_job', familyOrder);
            if (!recurring.ok) {
            ...
            console.log(`recurring: ${recurring.alias} (${recurring.entry.displayName})`);
            await noteUnconfiguredProvider(runtimeHome, oneTime.alias, oneTime.entry.modelRoute.id);
            if (recurring.entry.modelRoute.id !== oneTime.entry.modelRoute.id) {
                await noteUnconfiguredProvider(runtimeHome, recurring.alias, recurring.entry.modelRoute.id);
            }
            return 0;
        }
        console.error(usage());
    ...
    if (action === 'reset') {
        const memoryProviderId = target === 'memory'
        ...
        const aliases = target === 'chat'
            ? [{ alias: DEFAULT_SETUP_MODEL_ALIAS, workload: 'chat' }]
            : target === 'jobs'
                ? [
                    { alias: chatAlias(settings), workload: 'one_time_job' },
                    {
    ...
    if (action === 'doctor') {
        ...
        const preflightResults = validationFailures.length === 0
            ? await Promise.all(selectedModelProviders(settings).map(async (providerId) => ({
                providerId,
                result: await preflight(runtimeHome, providerId, settings),
            })))
            : [];
        const status = validationFailures.length === 0 &&
