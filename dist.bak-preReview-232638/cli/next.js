...
import { controlPlaneMemoryStatus, controlPlaneProviderInputs, } from '../application/control-plane/control-plane-settings-inputs.js';
import { resolveControlPlaneGuidedAction, } from '../application/guided-actions/guided-action-model.js';
import { formatGuidedActionPreview, formatGuidedActionResult, GuidedActionService, } from '../application/guided-actions/guided-action-service.js';
...
function buildExecutors(importMetaUrl, runtimeHome, restartRuntime) {
    return {
            ...
            try {
                ...
                return response.resumed
                    ...
                    : {
                        ...
                        savedTo: 'runtime state',
                        restartRequired: false,
                        nextAction: response.setup?.nextAction ?? 'Resolve job setup blockers.',
                    };
            }
            catch (err) {
                return {
...
async function resolveCurrentGuidedAction(runtimeHome, settings) {
    ...
    try {
        ...
        if (!isStorageUnavailableError(err)) {
            p.log.warn(`Storage degraded: ${err instanceof Error ? err.message : String(err)}. Next action may be incomplete.`);
        }
...
export async function runNextCommand(importMetaUrl, runtimeHome, args, settings, restartRuntime) {
    const ref = await resolveCurrentGuidedAction(runtimeHome, settings);
    if (ref.type === 'none') {
        p.log.info('No next action. Everything looks ready.');
        return 0;
    }
    const service = new GuidedActionService(buildExecutors(importMetaUrl, runtimeHome, restartRuntime));
