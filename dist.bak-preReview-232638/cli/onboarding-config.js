...
import { applyModelDefaults, ensureConfiguredAgent, loadDesiredRuntimeSettingsForWrite, loadRuntimeSettings, noteRestartRequired, writeDesiredRuntimeSettings, } from '../config/settings/runtime-settings.js';
...
import { gantryRuntimeSecretRef, normalizeRuntimeSecretRefString, parseRuntimeSecretRefString, } from '../domain/ports/runtime-secret-provider.js';
...
const STALE_SETTINGS_MESSAGE = 'Settings mutation is based on stale settings; reload latest desired state and retry.';
const MAX_STALE_SETTINGS_RETRIES = 3;
function enabledProviderEnvSecretNames(settings) {
    const names = new Set();
    for (const account of Object.values(settings.providerAccounts)) {
        ...
        for (const [key, value] of Object.entries(account.runtimeSecretRefs)) {
            ...
            if (!ref)
                continue;
            const normalized = normalizeRuntimeSecretRefString(ref, `provider account ${account.provider} secret ref ${key}`);
...
export async function persistOnboardingConfig(input) {
    ensureRuntimeLayout(input.runtimeHome);
    const envPath = envFilePath(input.runtimeHome);
    const existingEnv = readEnvFile(envPath);
    const existingCredentialSecret = existingEnv.SECRET_ENCRYPTION_KEY?.trim() ||
        process.env.SECRET_ENCRYPTION_KEY?.trim() ||
        '';
    const credentialSecretEncryptionKey = isValidCredentialEncryptionKey(existingCredentialSecret)
    ...
    upsertEnvFile(envPath, Object.fromEntries(CHANNEL_ENV_KEYS.filter((key) => !envSecretRefs.has(key)).map((key) => [
        key,
        null,
    ])));
}
async function writeOnboardingSettingsWithRetry(input) {
    ...
    for (let attempt = 0; attempt <= MAX_STALE_SETTINGS_RETRIES; attempt += 1) {
        const previousSettings = await loadOnboardingSettingsBase(input.config, input.postgresSchema);
        ...
        if (!secretsStored) {
            const storedProviders = await storeOnboardingRuntimeSecrets(input.config, settings);
            markOnboardingRuntimeSecretsStored(input.config.runtimeHome, storedProviders);
            secretsStored = true;
        }
        try {
...
async function loadOnboardingSettingsBase(input, postgresSchema) {
...
function buildOnboardingSettings(input) {
    ...
    const nextChatAlias = input.model.alias ||
    ...
    const hasSlackTokens = Boolean(input.config.slackBotToken?.trim() && input.config.slackAppToken?.trim());
    const useStoredTelegramSecretRefs = Boolean(input.config.primaryProvider === 'telegram' &&
        input.config.hasStoredTelegramSecretRefs);
    const useStoredSlackSecretRefs = Boolean(input.config.primaryProvider === 'slack' &&
    ...
    const preserveTelegram = !hasTelegramBotToken &&
        hasEnabledProviderWithStoredSecretRefs(input.baseSettings, 'telegram');
    const preserveSlack = !hasSlackTokens &&
    ...
    if (input.config.telegramBotToken?.trim() || useStoredTelegramSecretRefs) {
        ...
        settings.providerAccounts.telegram_default = {
            agentId: DEFAULT_AGENT_FOLDER,
            provider: 'telegram',
            label: settings.providerAccounts.telegram_default?.label || 'Telegram Default',
            runtimeSecretRefs: {
    ...
    if ((input.config.slackBotToken?.trim() &&
    ...
    settings.memory = {
        ...
        embeddings: {
            ...
            provider: input.config.memoryEnabled && input.config.embeddingsEnabled
                ? 'openai'
                : 'disabled',
        },
        dreaming: {
...
async function storeOnboardingRuntimeSecrets(input, runtimeSettings) {
...
function markOnboardingRuntimeSecretsStored(runtimeHome, providers) {
    if (providers.length === 0)
        return;
    const state = readOnboardingState(runtimeHome);
    if (!state || state.status !== 'in_progress')
        return;
    const storedProviderSecretRefs = new Set(state.data.storedProviderSecretRefs ?? []);
...
function isStaleSettingsWriteError(err) {
    if (!(err instanceof Error))
        return false;
    return (err.name === 'SettingsRevisionConflictError' ||
...
export async function prepareOnboardingCredentialStorage(input) {
    ...
    const envPath = envFilePath(input.runtimeHome);
    const existingEnv = readEnvFile(envPath);
    const existingCredentialKey = existingEnv.SECRET_ENCRYPTION_KEY?.trim() ||
        process.env.SECRET_ENCRYPTION_KEY?.trim() ||
        '';
...
function resolveOnboardingModel(value) {
    ...
    const resolved = resolveModelSelectionForWorkloadWithFamilies(trimmed, 'chat');
    if (!resolved.ok) {
