...
import { getProvider, listConnectableChannelProviders, } from '../channels/provider-registry.js';
import { applyConversationInstallToSettings } from '../config/settings/conversation-install-settings.js';
import { loadDesiredRuntimeSettingsForWrite, noteRestartRequired, writeDesiredRuntimeSettings, } from '../config/settings/desired-settings-writer.js';
...
import { assertRuntimeSecretRef, conversationIdFromConfigured, option, parseRuntimeSecretRefOptions, soleProviderAccountIdForJid, storedConversationIdCandidates, } from './provider-utils.js';
...
async function formatProviderList(runtimeHome) {
    ...
    for (const provider of listConnectableChannelProviders()) {
        ...
        const accounts = Object.entries(settings.providerAccounts).filter(([, candidate]) => candidate.provider === provider.id && candidate.status !== 'disabled');
        ...
        lines.push(`${provider.label}: ${enabled ? 'enabled' : 'disabled'} | credentials: ${credentialStatus}`);
    }
    return lines.join('\n');
}
function scopeProviderDoctorReport(report) {
    ...
    const blockingFailures = checks.filter((check) => check.status === 'fail').length;
    return {
        ...report,
        checks,
        blockingFailures,
...
export async function runProviderCommand(importMetaUrl, runtimeHome, args) {
    ...
    if (command === 'doctor') {
        const { formatDoctorReport, runDoctorWithNetwork } = await import('./doctor.js');
    ...
    if (command === 'info') {
        ...
        try {
            const conversationId = await resolveConversationIdArgument(runtimeHome, providerId, { providerAccountId: option(args, '--provider-account') });
            p.note(await withRuntimeStorage(() => formatConversationInfo(conversationId)), 'Conversation Info');
            return 0;
        }
        catch (error) {
    ...
    if (command === 'control-allowlist' || command === 'approvers') {
        ...
        try {
            ...
            if (allowIndex >= 0) {
                ...
                p.note(formatUserList(controlAllowlist.userIds), 'Conversation Approvers');
                return 0;
            }
            ...
            p.note(formatUserList(summary.controlAllowlist.userIds), 'Conversation Approvers');
            return 0;
        }
        catch (error) {
            p.log.error(formatConversationAdminError(error));
...
export async function runConversationCommand(runtimeHome, args) {
    ...
    if (command === 'installs' && conversationId === 'list') {
        try {
            p.note(await withRuntimeStorage(() => formatConversationInstalls()), 'Conversation Installs');
            return 0;
        }
        catch (error) {
            p.log.error(formatConversationAdminError(error));
    ...
    if (command === 'info' && conversationId) {
        try {
            const resolvedConversationId = await resolveConversationIdArgument(runtimeHome, conversationId, { providerAccountId: option(args, '--provider-account') });
            p.note(await withRuntimeStorage(() => formatConversationInfo(resolvedConversationId)), 'Conversation Info');
            return 0;
        }
        catch (error) {
            p.log.error(formatConversationAdminError(error));
...
async function runProviderAccountCommand(runtimeHome, args) {
    const [command, idOrProvider] = args;
    try {
        if (!command || command === 'list') {
            p.note(await withRuntimeStorage(() => formatProviderAccounts()), 'Provider Accounts');
            return 0;
        }
        if (command === 'connect') {
    ...
    p.log.error(usage());
    return 1;
    async function connectProviderAccount(providerId, rest) {
        if (!providerId) {
            p.log.error(usage());
            return 1;
        }
        ...
        const label = option(rest, '--label') || `${provider.label} Provider Account`;
    ...
    async function rotateProviderAccountSecret(providerAccountId, rest) {
        ...
        if (!key || !ref) {
            p.log.error('Provider Account rotate-secret requires --key <key> --ref <runtime-secret-ref>.');
            return 1;
        }
...
async function runConversationInstallCommand(runtimeHome, args) {
    ...
    try {
        ...
        if (accountSettings.agentId !== agentFolder) {
            throw new ApplicationError('INVALID_REQUEST', 'Provider Account belongs to a different Agent');
        }
        const resolvedConversationId = await resolveConversationIdArgument(runtimeHome, conversationId, { providerAccountId, settings });
        const { conversation, controlApprovers } = await withRuntimeStorage(async () => {
            const repositories = await runtimeRepositories();
            const account = await getProviderAccountsRepo(repositories).getProviderAccount(providerAccountId);
            if (!account || account.appId !== 'default') {
                throw new ApplicationError('NOT_FOUND', 'Provider Account not found');
            }
            if (folderForAgentId(account.agentId) !== agentFolder) {
                throw new ApplicationError('INVALID_REQUEST', 'Provider Account belongs to a different Agent');
            }
            let storedConversation = null;
            let accountMismatch = false;
            for (const candidateId of storedConversationIdCandidates(resolvedConversationId, providerAccountId)) {
                const candidate = await repositories.conversations.getConversation(candidateId);
                if (!candidate || candidate.appId !== 'default')
                    continue;
            ...
            if (!storedConversation && accountMismatch) {
                throw new ApplicationError('INVALID_REQUEST', 'Conversation belongs to a different Provider Account');
            }
            if (!storedConversation) {
                throw new ApplicationError('NOT_FOUND', 'Conversation not found');
            }
            const approvers = await repositories.conversations.listConversationApprovers(storedConversation.id);
            return {
                conversation: storedConversation,
                controlApprovers: approvers.map((approver) => approver.externalUserId),
            };
        });
        const now = nowIso();
...
function settingsAgentFolder(settings, agentId) {
    if (settings.agents[agentId])
        return agentId;
    const folder = folderForAgentId(agentId);
    return folder && settings.agents[folder] ? folder : undefined;
}
async function formatProviderAccounts() {
    const repositories = await runtimeRepositories();
    const rows = await getProviderAccountsRepo(repositories).listProviderAccounts('default');
    if (rows.length === 0)
        return 'Provider Accounts: none';
    return rows
        .map((account) => [
...
async function formatConversationInstalls() {
    const repositories = await runtimeRepositories();
    const repo = getProviderAccountsRepo(repositories);
    const rows = await repo.listConversationInstalls('default');
    if (rows.length === 0)
        return 'Conversation Installs: none';
    return rows
        .map((install) => [
...
async function formatConversationInfo(conversationId) {
    const repositories = await runtimeRepositories();
    ...
    const conversationInstalls = installs.filter((install) => install.conversationId === conversation.id);
    return [
...
async function conversationAdministrationService() {
    const repositories = await runtimeRepositories();
    return new ConversationAdministrationService({
        providerAccounts: getProviderAccountsRepo(repositories),
...
async function runtimeRepositories() {
    const { getRuntimeStorage } = await import('../adapters/storage/postgres/runtime-store.js');
    return getRuntimeStorage().repositories;
}
function getProviderAccountsRepo(repositories) {
    return repositories.providerAccounts;
}
async function listInstallsForInfo(repositories) {
    const repo = getProviderAccountsRepo(repositories);
    return repo.listConversationInstalls('default');
}
async function resolveConversationIdArgument(runtimeHome, conversationIdOrJid, options = {}) {
    ...
    if (configured)
        return conversationIdFromConfigured(settings, configured);
    ...
    if (matchingConfigured.length === 1)
        return matchingConfigured[0];
    if (/^[a-z][a-z0-9_-]*:/i.test(value)) {
        const providerAccountId = options.providerAccountId || soleProviderAccountIdForJid(settings, value);
        if (!providerAccountId) {
            throw new ApplicationError('INVALID_REQUEST', 'Raw conversation IDs require --provider-account <id> or a configured conversation key.');
        }
        return `conversation:${providerAccountId}:${value}`;
    }
    return value;
}
async function withRuntimeStorage(fn) {
    const { closeRuntimeStorage, initializeRuntimeStorage } = await import('../adapters/storage/postgres/runtime-store.js');
...
function formatConversationAdminError(error) {
