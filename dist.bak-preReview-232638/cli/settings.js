...
import { closeRuntimeStorage, getRuntimeStorage, initializeRuntimeStorage, } from '../adapters/storage/postgres/runtime-store.js';
import { getDeploymentMode, loadRuntimeSettings, loadRuntimeSettingsFromPath, SettingsDesiredStateService, } from '../config/index.js';
import { importFleetSettingsRevision, importWorkstationSettings, } from '../config/settings/settings-import-service.js';
function usage() {
    return [
        'Usage:',
...
export function reportWorkstationSettingsImportOutcome(command, outcome) {
    const status = outcome.status === 'revision_created'
        ? `revision_created(${outcome.revision})`
        : outcome.status;
    console.log(`${command} outcome: ${status}`);
}
function parseImportFlags(args) {
    const flags = { fleet: false };
...
export async function runSettingsCommand(runtimeHome, args) {
    ...
    if (subcommand === 'validate') {
        try {
            ...
            p.log.error(`settings.yaml schema is invalid: ${err instanceof Error ? err.message : String(err)}`);
            return 1;
        }
    }
    ...
    try {
        ...
        if (subcommand === 'export') {
            ...
            p.log.success(`Exported ${agentCount} agent desired-state record(s) to desired state and settings.yaml.`);
            reportWorkstationSettingsImportOutcome('settings export', outcome);
            p.log.info('Review settings.yaml before setting desired_state.authoritative=true.');
            return 0;
        }
...
async function runImport(runtimeHome, args, storage) {
    ...
    try {
        ...
        p.log.error(`settings file failed to parse: ${err instanceof Error ? err.message : String(err)}`);
        return 1;
    }
    ...
    if (!fleet) {
        ...
        try {
            ...
            p.log.error(`settings import failed: ${err instanceof Error ? err.message : String(err)}`);
            return 1;
        }
        p.log.success(`Imported ${flags.file} into desired state and settings.yaml.`);
    ...
    if (outcome.status === 'invalid') {
        ...
        for (const detail of outcome.errors)
            p.log.error(`  - ${detail}`);
        return 1;
    }
    if (outcome.status === 'conflict') {
        p.log.error(`Stale revision: expected ${outcome.expectedRevision}, current is ${outcome.actualRevision}. Re-run against the latest.`);
        return 1;
    }
    p.log.success(`Appended fleet settings revision ${outcome.revision}.`);
    return 0;
}
async function runRevisions(args, storage) {
    if (args[0] !== 'list') {
        p.log.error('Usage: gantry settings revisions list');
        return 1;
    }
    const revisions = await storage.repositories.settingsRevisions.listRecentSettingsRevisions({
        appId: 'default',
        limit: 50,
    });
    ...
    const lines = revisions.map((revision) => `#${revision.revision}  ${revision.createdAt}  by ${revision.createdBy}` +
