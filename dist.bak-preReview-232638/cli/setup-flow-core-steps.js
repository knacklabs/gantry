...
import { ensureRuntimeWritable, resolveRuntimeHome, } from '../config/settings/runtime-home.js';
import { ensureConfiguredAgent, loadDesiredRuntimeSettingsForWrite, noteRestartRequired, writeDesiredRuntimeSettings, } from '../config/settings/runtime-settings.js';
...
import { isInputFlowControl, parseInputFlowControl, toAction, } from './setup-flow-control.js';
import { chooseProgressAction } from './setup-flow-prompts.js';
function agentIdFromName(name, agents) {
...
async function ensureModelCredentialForProvider(runtimeHome, providerId) {
    const { listReadyModelCredentialProviders, promptModelCredentialPayload, storeModelCredentialInput, verifyModelCredentialInputWithPrompt, } = await import('./credentials.js');
...
export async function runAddAgentSetupSlice(runtimeHome) {
    ...
    if (p.isCancel(modelValue) || modelValue === 'cancel')
        return 1;
    ...
    if (!(await ensureModelCredentialForProvider(runtimeHome, resolved.entry.modelRoute.id))) {
        return 1;
    }
    ...
    const connectCode = await runProviderConnectCommand(runtimeHome, String(provider), agentId, name);
    if (connectCode !== 0)
        return connectCode;
    ...
    try {
        ...
        const bound = Object.values(routes).some((route) => route.folder === agentId);
        if (!bound) {
            ...
            current.providerAccounts = structuredClone(channelStateSnapshot.providerAccounts);
...
export async function runRuntimeHomeStep(draft) {
    const defaultRuntimeHome = draft.runtimeHome || '~/gantry';
    const value = await p.text({
        message: 'Where should Gantry store runtime data? (/back, /resume, /cancel)',
        placeholder: '~/gantry',
    ...
    const resolved = resolveRuntimeHome(String(value).trim() || defaultRuntimeHome);
    try {
...
export async function runStorageStep(draft) {
    p.note([
    ...
    const url = await p.text({
        ...
        placeholder: 'postgres://user:pass@db.example.com:5432/gantry?sslmode=require',
        ...
        validate: (input) => {
            ...
            if (isInputFlowControl(trimmed))
                return undefined;
            if (!trimmed)
                return 'Postgres URL is required.';
            try {
...
export async function runChannelStep(draft) {
...
export async function runModelStep(draft) {
    const agentName = await p.text({
    ...
    if (agentNameControl)
        return agentNameControl;
    draft.agentName = String(agentName).trim();
    const chatModelOptions = chatModelSelectOptions();
    const initialModel = chatModelOptions.some((option) => option.value === draft.selectedModel) &&
    ...
    draft.selectedModel = resolvedModel.ok
        ...
        : DEFAULT_SETUP_MODEL_ALIAS;
    if (resolvedModel.ok) {
        p.note(`Memory LLM defaults derive from ${resolvedModel.entry.modelRoute.id}: ${formatMemoryDefaultAliases(memoryModelDefaultsForProvider(resolvedModel.entry.modelRoute.id))}. Memory embeddings use OpenAI when enabled.`);
    }
    draft.agentHarness = AUTO_AGENT_HARNESS;
    return { type: 'next' };
}
export function chatModelSelectOptions() {
    return listModelCatalogEntries()
        .filter((entry) => entry.supportedWorkloads.includes('chat'))
        .map((entry) => ({
        value: entry.recommendedAlias,
        label: entry.recommendedAlias === DEFAULT_SETUP_MODEL_ALIAS
...
export async function runMemoryStep(draft) {
    const memoryEnabled = await p.confirm({
        message: 'Enable memory?',
        initialValue: draft.memoryEnabled ?? true,
    });
    if (p.isCancel(memoryEnabled))
        return { type: 'resume' };
    draft.memoryEnabled = Boolean(memoryEnabled);
    if (draft.memoryEnabled) {
        const chatModel = resolveModelSelectionForWorkload(draft.selectedModel || DEFAULT_SETUP_MODEL_ALIAS, 'chat');
        ...
        const memoryModel = resolveModelSelectionForWorkload(memoryDefaults.extractor, 'memory_extractor');
        const memoryProviderId = memoryModel.ok
            ? memoryModel.entry.modelRoute.id
            : chatProviderId;
        if (chatProviderId && memoryProviderId !== chatProviderId) {
            const memoryProvider = getModelProviderDefinition(memoryProviderId)?.label ?? memoryProviderId;
            p.note(`Memory models run on ${memoryProvider} and require its credential.`);
            ...
            if (p.isCancel(memoryChoice))
                return { type: 'resume' };
            if (memoryChoice === 'back')
                return { type: 'back' };
            if (memoryChoice === 'resume')
                return { type: 'resume' };
    ...
    if (draft.memoryEnabled) {
        const embeddingsEnabled = await p.confirm({
            message: 'Enable semantic search? Requires an OpenAI API key for embeddings',
            initialValue: draft.embeddingsEnabled ?? false,
        });
        if (p.isCancel(embeddingsEnabled))
            return { type: 'resume' };
...
function formatMemoryDefaultAliases(defaults) {
