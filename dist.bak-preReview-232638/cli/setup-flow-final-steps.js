...
import { ensureConfiguredConversationBinding, loadRuntimeSettings, noteRestartRequired, writeDesiredRuntimeSettings, } from '../config/settings/runtime-settings.js';
import { formatDoctorReport, hasProcessableGroupForConfiguredChannel, hasRuntimeConfig, runDoctorWithNetwork, } from './doctor.js';
...
import { requiredModelCredentialProviderReasonsForSetupDraft, requiredModelCredentialProvidersForSetupDraft, verifyModelAccess, } from './setup-credentials.js';
function setupBlocked(reason, nextAction) {
    return [`Setup blocked: ${reason}`, `Next action: ${nextAction}`].join('\n');
}
function parseApproverIds(raw) {
    return [
        ...new Set(raw
            .split(/[,\s]+/)
...
export async function runConfigStep(draft) {
    const channelLabel = draft.primaryProvider === 'slack'
    ...
    const action = await chooseProgressAction({
        message: 'Create this Gantry runtime now? After this point setup writes config and cannot be cancelled transactionally.',
    ...
    try {
        ...
        if (!draft.postgresDatabaseUrl) {
            ...
            p.log.error(setupBlocked('missing GANTRY_DATABASE_URL', 'return to the Database step and provide the Postgres URL.'));
        ...
        const nextAction = message.includes('Settings mutation is based on stale settings')
...
export async function runGroupStep(draft) {
    const spinner = p.spinner();
    ...
    try {
        if (draft.primaryProvider === 'slack') {
            ...
            const conversationLabel = draft.telegramDisplayName || draft.telegramChatJid;
            const result = await registerTelegramMainGroup({
                runtimeHome: draft.runtimeHome,
                chatJid: draft.telegramChatJid,
                displayName: draft.agentName,
            });
            const approverIds = parseApproverIds(draft.telegramPermissionApproverIds || draft.telegramAdminSenderId);
        ...
        const message = err instanceof Error ? err.message : String(err);
        p.log.error(setupBlocked(`could not register ${draft.primaryProvider} conversation (${message})`, `return to the ${draft.primaryProvider === 'slack' ? 'Slack' : 'Telegram'} step and choose Try again.`));
        return {
            type: 'goto',
...
export async function runVerifyStep(importMetaUrl, draft) {
    const credentialLiveSkipProviderIds = draft.credentialLiveSkipProviderIds ?? [];
    ...
    const hasProcessableGroup = await hasProcessableGroupForConfiguredChannel(draft.runtimeHome);
    p.note(formatDoctorReport(report), 'Verification');
    if (!runtimeConfigured) {
        const connectCommand = draft.primaryProvider === 'slack'
            ? '`gantry provider connect slack`'
            : '`gantry provider connect telegram`';
        p.log.warn(setupBlocked('no channel connected', `connect a channel with ${connectCommand}.`));
        return {
            type: 'goto',
            step: draft.primaryProvider === 'slack' ? 'slack' : 'telegram',
        };
    ...
    if (!hasProcessableGroup) {
        const connectCommand = draft.primaryProvider === 'slack'
            ? '`gantry provider connect slack`'
            : '`gantry provider connect telegram`';
        const providerAvailable = listConnectableChannelProviders().some((provider) => provider.id === draft.primaryProvider);
        p.log.warn(setupBlocked('no processable conversation for the configured channel', providerAvailable
            ? `run ${connectCommand}.`
    ...
    if (!report.ok) {
        ...
        p.log.warn(setupBlocked(failure?.message || 'verification found blocking issues', failure?.nextAction || 'run `gantry doctor`, then run `gantry setup`.'));
        if (failure?.id === 'model-access-credentials') {
            return { type: 'goto', step: 'credentials' };
        }
        return { type: 'resume' };
    }
    const modelAccess = await verifyModelAccess(draft.runtimeHome, loadRuntimeSettings(draft.runtimeHome), { skipLiveProviderIds: credentialLiveSkipProviderIds });
    if (!modelAccess.ok) {
        p.log.warn(setupBlocked(modelAccess.message, modelAccess.nextAction ||
...
function formatRequiredProviderReasons(draft) {
    return requiredModelCredentialProviderReasonsForSetupDraft(draft).map(({ providerId, reasons }) => `  ${providerId}: ${reasons.length ? reasons.join('; ') : 'selected defaults'}`);
}
...
function resolvedModelProvider(alias) {
    const resolved = resolveModelSelectionForWorkload(alias, 'chat');
