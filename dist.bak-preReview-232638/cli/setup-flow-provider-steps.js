...
import { normalizeSlackChatJid, validateSlackAppToken, validateSlackBotToken, verifySlackChatAccess, } from './slack.js';
import { listSlackRecentChats } from './slack-chat-discovery.js';
import { normalizeTelegramChatJid, validateTelegramBotToken, verifyTelegramChatAccess, } from './telegram.js';
import { DEFAULT_AGENT_CLI_NAME } from './main-agent.js';
import { listTelegramRecentChats } from './telegram-chat-discovery.js';
import { isInputFlowControl, parseInputFlowControl, } from './setup-flow-control.js';
import { chooseProgressAction } from './setup-flow-prompts.js';
import { normalizeSlackPermissionApproverIds, validateSlackPermissionApproverIdsInput, } from './setup-slack-approvers.js';
function setupBlocked(reason, nextAction) {
    return [`Setup blocked: ${reason}`, `Next action: ${nextAction}`].join('\n');
}
async function promptTelegramAdminSenderIdForManualChat(draft) {
    const adminInput = await p.text({
        message: 'Telegram sender/user ID for session admin (optional, /back, /resume, /cancel)',
        placeholder: 'Press Enter to skip; enter only your own trusted Telegram user ID',
...
function normalizeTelegramPermissionApproverIds(raw) {
    return raw
        .split(',')
...
function validateTelegramPermissionApproverIdsInput(value) {
    const trimmed = String(value ?? '').trim();
    if (isInputFlowControl(trimmed))
        return undefined;
    if (!trimmed)
        return 'At least one Telegram approver user ID is required.';
...
async function promptTelegramPermissionApproverIds(draft) {
    const defaultValue = draft.telegramPermissionApproverIds || draft.telegramAdminSenderId;
    const input = await p.text({
        message: 'Telegram approver user IDs; seeds conversation approvers (/back, /resume, /cancel)',
    ...
    if (control)
        return control;
    draft.telegramPermissionApproverIds = normalizeTelegramPermissionApproverIds(String(input));
    return { type: 'next' };
}
export async function runTelegramStep(draft) {
    ...
    while (true) {
        if (token) {
            ...
            if (tokenChoice === 'back')
                return { type: 'back' };
            if (tokenChoice === 'resume')
                return { type: 'resume' };
            if (tokenChoice === 'cancel')
                return { type: 'cancel' };
            if (tokenChoice === 'enter_new')
                token = '';
        }
        if (!token) {
            const entered = await p.password({
                message: 'Paste your Telegram bot token from BotFather (/back, /resume, /cancel)',
                validate: (value) => {
                    const trimmed = String(value ?? '').trim();
                    if (isInputFlowControl(trimmed))
                        return undefined;
        ...
        if (!validation.ok) {
            ...
            p.note(setupBlocked(validation.message, validation.nextAction || 'choose Try token again.'), 'Blocked');
        ...
        if (discovered.ok && discovered.chats.length > 0) {
            discoverySpinner.stop(`Found ${discovered.chats.length} recent Telegram chat(s).`);
            ...
            const selectedChat = discovered.chats.find((chat) => chat.chatJid === normalizedJid);
        ...
        if (!normalizedJid) {
            p.log.error(setupBlocked('invalid Telegram chat ID format', 'choose Try again with a numeric Telegram chat ID.'));
            continue;
        }
        ...
        if (!chatAccess.ok) {
            ...
            p.note(setupBlocked(chatAccess.message, chatAccess.nextAction ||
                'choose Try again after fixing chat permissions.'), 'Blocked');
            const retryChoice = await p.select({
                message: 'What do you want to do?',
                options: [
        ...
        p.note([
            ...
            `Session admin: ${draft.telegramAdminSenderId
                ? draft.telegramAdminSenderName || draft.telegramAdminSenderId
                : 'not detected yet'}`,
...
export async function runSlackStep(draft) {
    ...
    p.note([
        ...
        '6. Invite the app to the target channel or DM it once before discovery.',
    ...
    while (true) {
        ...
        if (!botToken) {
            const entered = await p.password({
                message: 'Paste your Slack bot token (xoxb-...) (/back, /resume, /cancel)',
                validate: (value) => {
                    const trimmed = String(value ?? '').trim();
                    if (isInputFlowControl(trimmed))
                        return undefined;
        ...
        if (!botValidation.ok) {
            p.note(setupBlocked(botValidation.message, botValidation.nextAction || 'choose Try token again.'), 'Blocked');
            const retryChoice = await p.select({
                message: 'What do you want to do?',
                options: [
                    { value: 'retry', label: 'Try token again (Recommended)' },
        ...
        if (appToken) {
            ...
            if (appChoice === 'cancel')
                return { type: 'cancel' };
            if (appChoice === 'enter_new')
                appToken = '';
        }
        if (!appToken) {
            const entered = await p.password({
                message: 'Paste your Slack app token (xapp-...) for Socket Mode (/back, /resume, /cancel)',
                validate: (value) => {
                    const trimmed = String(value ?? '').trim();
                    if (isInputFlowControl(trimmed))
                        return undefined;
        ...
        if (!appValidation.ok) {
            p.note(setupBlocked(appValidation.message, appValidation.nextAction || 'choose Try app token again.'), 'Blocked');
            const retryChoice = await p.select({
                message: 'What do you want to do?',
                options: [
        ...
        if (!normalizedJid) {
            p.log.error(setupBlocked('invalid Slack conversation ID format', 'choose Try again with a valid Slack conversation ID.'));
            continue;
        }
        ...
        if (!access.ok) {
            p.note(setupBlocked(access.message, access.nextAction ||
                'choose Try again after fixing conversation access.'), 'Blocked');
            const retryChoice = await p.select({
                message: 'What do you want to do?',
                options: [
                    { value: 'retry', label: 'Try again (Recommended)' },
        ...
        const approverInput = await p.text({
            message: 'Slack admin/approver user IDs; seeds this conversation approvers (/back, /resume, /cancel)',
        ...
        if (p.isCancel(approverInput))
            return { type: 'resume' };
        const approverControl = parseInputFlowControl(approverInput);
        if (approverControl)
            return approverControl;
        draft.slackPermissionApproverIds = normalizeSlackPermissionApproverIds(String(approverInput));
        p.note([
            `Agent: ${draft.agentName || DEFAULT_AGENT_CLI_NAME}`,
