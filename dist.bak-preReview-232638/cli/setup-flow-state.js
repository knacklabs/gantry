...
import { envFilePath, settingsFilePath, } from '../config/settings/runtime-home.js';
import { AUTO_AGENT_HARNESS, isAgentHarness, } from '../shared/agent-engine.js';
import { createDefaultRuntimeSettings, loadRuntimeSettingsFromPath, } from '../config/settings/runtime-settings.js';
import { DEFAULT_SETUP_MODEL_ALIAS, resolveModelAlias, } from '../shared/model-catalog.js';
...
export function defaultStepIndex(step) {
    if (!step)
        return 0;
    const idx = FULL_SEQUENCE.indexOf(step);
    return idx >= 0 ? idx : 0;
}
export function shouldSkipStep(step, draft) {
    ...
    if (step === 'telegram' && draft.primaryProvider !== 'telegram')
        return true;
    if (step === 'slack' && draft.primaryProvider !== 'slack')
        return true;
    return false;
}
export function shouldAutoSkipAnsweredProviderStep(step, draft, state) {
    const savedTelegramAnswer = Boolean(state?.status === 'in_progress' &&
        state.data.completedProviderSteps?.includes('telegram') &&
        state.data.telegramChatJid &&
        state.data.telegramPermissionApproverIds);
    const savedSlackAnswer = Boolean(state?.status === 'in_progress' &&
...
export function updateStateData(state, draft) {
    ...
    state.data = {
        ...
        telegramPermissionApproverIds: draft.telegramPermissionApproverIds || undefined,
...
export function updateDraftFromState(draft, state) {
    ...
    draft.credentialMode = resolveHostCredentialMode(state.data.credentialMode || draft.credentialMode);
...
export function persistProgress(state, runtimeHome) {
    writeOnboardingState(runtimeHome, state);
}
function loadExistingRuntimeSettings(runtimeHome) {
...
function hasEnabledProviderWithStoredSecretRefs(settings, providerId) {
    if (!settings.providers[providerId]?.enabled)
        return false;
    return Object.values(settings.providerAccounts).some((account) => account.provider === providerId &&
        account.status !== 'disabled' &&
        REQUIRED_CHANNEL_SECRET_REFS[providerId].every((key) => Boolean(account.runtimeSecretRefs[key]?.trim())));
}
export function restoreDraft(runtimeHome, state) {
    const env = readEnvFile(envFilePath(runtimeHome));
    const settings = loadExistingRuntimeSettings(runtimeHome);
    const primaryProvider = state?.data.primaryProvider ||
        (settings.providers.slack?.enabled ? 'slack' : 'telegram');
    const credentialMode = resolveHostCredentialMode(state?.data.credentialMode || settings.credentialBroker.mode);
    const hasConfiguredProvider = Object.values(settings.providers).some((provider) => provider.enabled);
    const defaultDreamingEnabled = hasConfiguredProvider
        ? settings.memory.dreaming.enabled
        : true;
    const postgresUrlEnv = settings.storage.postgres.urlEnv || 'GANTRY_DATABASE_URL';
    const postgresDatabaseUrl = env[postgresUrlEnv]?.trim() || process.env[postgresUrlEnv]?.trim() || '';
    const readySummary = resolveReadySummary(settings, primaryProvider);
    const hasConfiguredChannelBinding = Boolean(settings.providers[primaryProvider]?.enabled &&
        readySummary.conversationJid);
    const hasStoredTelegramSecretRefs = hasEnabledProviderWithStoredSecretRefs(settings, 'telegram') ||
        Boolean(state?.data.storedProviderSecretRefs?.includes('telegram'));
    const hasStoredSlackSecretRefs = hasEnabledProviderWithStoredSecretRefs(settings, 'slack') ||
        Boolean(state?.data.storedProviderSecretRefs?.includes('slack'));
    const draft = {
        runtimeHome,
        postgresSetupKind: state?.data.postgresSetupKind ||
            (postgresDatabaseUrl.includes('localhost') ? 'local' : 'existing'),
        postgresDatabaseUrl,
        postgresSchema: state?.data.postgresSchema ||
            settings.storage.postgres.schema ||
            'gantry',
        primaryProvider,
        credentialMode,
        agentName: state?.data.agentName || settings.agent.name || DEFAULT_AGENT_CLI_NAME,
        selectedModel: resolveModelAlias(state?.data.selectedModel || settings.agent.defaultModel) || DEFAULT_SETUP_MODEL_ALIAS,
        agentHarness: isAgentHarness(state?.data.agentHarness)
            ? state.data.agentHarness
            : settings.agent.agentHarness || AUTO_AGENT_HARNESS,
        credentialLiveSkipProviderIds: state?.data.credentialLiveSkipProviderIds ?? [],
        telegramBotToken: env.TELEGRAM_BOT_TOKEN || '',
        telegramChatJid: primaryProvider === 'telegram' ? readySummary.conversationJid : '',
        ...
        slackChatJid: primaryProvider === 'slack' ? readySummary.conversationJid : '',
        slackDisplayName: settings.agent.name || DEFAULT_AGENT_CLI_NAME,
        slackPermissionApproverIds: firstConversationApprovers(settings, 'slack').join(','),
        memoryEnabled: state?.data.memoryEnabled ?? settings.memory.enabled,
        embeddingsEnabled: state?.data.embeddingsEnabled ?? settings.memory.embeddings.enabled,
    ...
    if (state)
        updateDraftFromState(draft, state);
    return draft;
}
function resolveReadySummary(settings, providerId) {
...
function firstConversationApprovers(settings, providerId) {
