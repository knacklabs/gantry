...
import { clearOnboardingState, createInitialState, readOnboardingState, } from './onboarding-state.js';
import { runCredentialsStep } from './setup-credentials.js';
import { runChannelStep, runMemoryStep, runModelStep, runRuntimeHomeStep, runStorageStep, runWelcomeStep, } from './setup-flow-core-steps.js';
import { runConfigStep, runGroupStep, runVerifyStep, } from './setup-flow-final-steps.js';
import { runSlackStep, runTelegramStep } from './setup-flow-provider-steps.js';
import { defaultStepIndex, FULL_SEQUENCE, persistProgress, restoreDraft, shouldAutoSkipAnsweredProviderStep, shouldSkipStep, updateStateData, } from './setup-flow-state.js';
export { restoreDraft, updateStateData, } from './setup-flow-state.js';
...
export async function runSetupFlow(options) {
    ...
    const state = readOnboardingState(runtimeHome) || createInitialState(runtimeHome);
    const draft = restoreDraft(runtimeHome, state);
    const autoSkipState = state.status === 'in_progress'
        ? {
            ...state,
            data: { ...state.data },
        }
    ...
    const initialStep = resolveInitialStep(options.initialStep || state.currentStep, draft);
    let index = defaultStepIndex(initialStep);
    let explicitStep = null;
...
function resolveInitialStep(initialStep, draft) {
    if (initialStep !== 'config')
        return initialStep;
...
function markCompletedProviderStep(state, step, action) {
    if (action.type !== 'next')
        return false;
    if (step !== 'telegram' && step !== 'slack')
        return false;
    const completedProviderSteps = new Set(state.data.completedProviderSteps ?? []);
    completedProviderSteps.add(step);
    state.data.completedProviderSteps = [...completedProviderSteps];
    return true;
}
function mergeStoredProviderSecretRefs(state, latestState) {
    const latestRefs = latestState?.data.storedProviderSecretRefs;
    if (!latestRefs?.length)
        return;
...
function visibleSteps(draft) {
    return FULL_SEQUENCE.filter((candidate) => candidate !== 'welcome' &&
        candidate !== 'ready' &&
        !shouldSkipStep(candidate, draft));
}
function logStepHeader(step, draft) {
    const steps = visibleSteps(draft);
    const position = steps.indexOf(step);
    if (position < 0)
        return;
    const details = STEP_DETAILS[step];
    p.log.step(`Step ${position + 1}/${steps.length} · ${details.label} — ${details.purpose}`);
}
function logStepRecap(step, draft, action) {
    if (action.type !== 'next')
        return;
...
function stepRecap(step, draft) {
    const providerLabel = draft.primaryProvider === 'slack' ? 'Slack' : 'Telegram';
    const selectedModel = resolveModelSelectionForWorkload(draft.selectedModel, 'chat');
