...
export async function runSkillCommand(runtimeHome, args) {
    ...
    if (family === 'list')
        return listSkills(runtimeHome, [action, ...rest]);
    if (family === 'doctor')
        return doctorSkill(runtimeHome, action);
    if (family === 'remove')
        return removeSkill(runtimeHome, action, rest);
    p.note(usage(), 'Skill');
    return 1;
}
...
async function installSkill(runtimeHome, options) {
    ...
    const skill = isRecord(uploaded) && isRecord(uploaded.skill) ? uploaded.skill : null;
    ...
    if (capabilityIds.length > 0) {
        ...
        const existing = Array.isArray(isRecord(current) ? current.selections : undefined)
            ? current.selections
            : [];
        const sources = isRecord(current) && isRecord(current.sources)
            ? current.sources
...
async function uploadSkillResponse(runtimeHome, options) {
    ...
    return controlApiRequest(runtimeHome, {
        ...
        missingKeyMessage: 'GANTRY_CONTROL_API_KEYS_JSON with at least one complete key record is required for skill install.',
    });
}
async function listSkills(runtimeHome, args) {
...
async function doctorSkill(runtimeHome, skillId = '') {
    if (!skillId) {
...
async function removeSkill(runtimeHome, skillId = '', args) {
    if (!skillId) {
...
function formatSkillActionLines(skill) {
...
function skillActionCapabilityIds(skill) {
    const actions = skill?.actionPermissions;
    if (!Array.isArray(actions))
        return [];
    return [
        ...new Set(actions
            .map((action) => isRecord(action) ? String(action.capabilityId ?? '') : '')
            .filter(Boolean)),
    ];
}
