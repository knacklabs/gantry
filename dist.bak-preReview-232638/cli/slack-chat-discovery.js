...
function normalizeSlackChatJid(raw) {
...
export async function listSlackRecentChats(options) {
    ...
    try {
        const response = await fetchWithTimeout(`https://slack.com/api/users.conversations?types=public_channel,private_channel,mpim,im&exclude_archived=${excludeArchived}&limit=${limit}`, timeoutMs, {
            headers: {
        ...
        if (!response.ok) {
            return {
                ...
                nextAction: 'Check token scopes/network and retry. Raw token-bearing transport details are intentionally not printed.',
            };
        }
        ...
        if (!payload.ok) {
            return {
                ...
                nextAction: 'Ensure bot has conversations:read and is invited to target channels.',
            };
        }
        ...
        for (const row of rows) {
            ...
            const chatTitle = row.name?.trim() ||
                (chatType === 'im' ? `dm-${row.user || 'unknown'}` : normalized);
            const sourceTs = parseTs(row.latest?.ts) || parseTs(row.updated) || parseTs(row.created);
            chats.push({
                chatJid: normalized,
                chatTitle,
                chatType,
        ...
        if (chats.length === 0) {
            return {
                ...
                nextAction: 'Invite the bot to a channel/DM and rerun `gantry provider connect slack`.',
            };
        }
        return {
            ok: true,
