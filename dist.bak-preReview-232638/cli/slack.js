...
import { safeSlackErrorCode, TOKEN_BOUND_HTTP_GUIDANCE, TOKEN_BOUND_NETWORK_GUIDANCE, } from './provider-error-guidance.js';
import { openRuntimeGroupDb } from './runtime-group-db.js';
import { envFilePath, ensureRuntimeLayout, } from '../config/settings/runtime-home.js';
import { allocateDefaultAgentFolder, DEFAULT_AGENT_FOLDER, defaultTriggerForAgentName, normalizeDefaultAgentName, } from './main-agent.js';
import { ensureConfiguredAgent, ensureConfiguredConversationBinding, loadRuntimeSettings, noteRestartRequired, writeDesiredRuntimeSettings, } from '../config/settings/runtime-settings.js';
...
function parseSlackApproverIds(raw) {
    if (!raw?.trim())
        return [];
    return [
        ...new Set(raw
...
async function readSlackPayload(response) {
    return (await response.json());
}
export function normalizeSlackChatJid(raw) {
...
export async function validateSlackBotToken(token, timeoutMs = 10_000) {
    ...
    try {
        const response = await fetchWithTimeout('https://slack.com/api/auth.test', timeoutMs, {
            headers: {
        ...
        if (!payload.ok) {
            return {
                ...
                nextAction: 'Reinstall the app to workspace and copy the latest Bot User OAuth token.',
            };
        }
        return {
            ok: true,
...
export async function validateSlackAppToken(appToken, timeoutMs = 10_000) {
    const trimmed = appToken.trim();
    if (!trimmed) {
        return {
            ok: false,
            message: 'Slack app token is empty.',
            nextAction: 'Create an app token (xapp-...) with connections:write scope.',
        };
    }
    ...
    try {
        const response = await fetchWithTimeout('https://slack.com/api/apps.connections.open', timeoutMs, {
            method: 'POST',
            headers: {
                authorization: `Bearer ${trimmed}`,
            },
        ...
        if (!response.ok) {
            return {
                ...
                nextAction: 'Verify the app token has connections:write and Socket Mode is enabled.',
            };
        }
        const payload = await readSlackPayload(response);
        if (!payload.ok || !payload.url) {
            return {
                ...
                nextAction: 'Enable Socket Mode, regenerate app token, and confirm connections:write scope.',
            };
        }
        return {
            ok: true,
...
export async function verifySlackChatAccess(options) {
    ...
    if (!botToken) {
        return {
            ok: false,
            message: 'Slack bot token is empty.',
            nextAction: 'Run `gantry provider connect slack` or configure the Slack bot_token runtime secret ref.',
        };
    }
    ...
    try {
        const infoResponse = await fetchWithTimeout(`https://slack.com/api/conversations.info?channel=${encodeURIComponent(channelId)}`, timeoutMs, {
            headers: {
        ...
        const infoPayload = await readSlackPayload(infoResponse);
        ...
        if (shouldSendProbe) {
            const sendResponse = await fetchWithTimeout('https://slack.com/api/chat.postMessage', timeoutMs, {
                method: 'POST',
                headers: {
                    authorization: `Bearer ${botToken}`,
            ...
            const sendPayload = await readSlackPayload(sendResponse);
...
export async function registerSlackMainGroup(options) {
    ...
    try {
        ...
        const folder = existingGroup?.folder ||
        ...
        const keepExistingRoute = Boolean(existingGroup &&
            requestedAgentId &&
...
export async function runSlackConnectCommand(runtimeHome, requestedAgentId, requestedAgentName) {
    ...
    const botTokenInput = await promptForValue({
        ...
        hide: true,
        validate: (value) => value?.trim() ? undefined : 'Slack bot token is required.',
    });
    ...
    if (!botValidation.ok) {
        ...
        if (botValidation.nextAction)
            p.log.info(botValidation.nextAction);
        return 1;
    }
    ...
    const appTokenInput = await promptForValue({
        ...
        hide: true,
        validate: (value) => value?.trim() ? undefined : 'Slack app token is required.',
    });
    ...
    if (!appValidation.ok) {
        ...
        if (appValidation.nextAction)
            p.log.info(appValidation.nextAction);
        return 1;
    }
    ...
    const normalizedChatJid = chatChoice.type === 'selected' ? chatChoice.chatJid : '';
    const approverInput = normalizedChatJid
        ? await promptForValue({
            message: 'Slack approver user IDs (comma-separated; seeds conversation approvers; must be members of this conversation)',
            defaultValue: '',
        })
        : '';
    ...
    if (normalizedChatJid) {
        ...
        if (!access.ok) {
            ...
            if (access.nextAction)
                p.log.info(access.nextAction);
            return 1;
        }
        ...
        const registered = await registerSlackMainGroup({
            runtimeHome,
            chatJid: normalizedChatJid,
            displayName: (requestedAgentId && currentSettings.agents[requestedAgentId]?.name) ||
        ...
        p.log.success(`Registered ${registered.groupName} for Slack conversation ${normalizedChatJid} in folder ${registered.folder}.`);
    }
    ...
    const providerAgentId = registeredFolder || requestedAgentId || DEFAULT_AGENT_FOLDER;
    ensureConfiguredAgent(settings, {
        agentId: providerAgentId,
        agentName: settings.agents[providerAgentId]?.name ||
            requestedAgentDisplayName ||
    ...
    if (registeredFolder) {
        const binding = ensureConfiguredConversationBinding(settings, {
            ...
            displayName: conversationDisplayName || conversationRouteName || settings.agent.name,
    ...
    settings.providerAccounts[providerAccountId] = {
        agentId: providerAgentId,
        provider: 'slack',
        label: settings.providerAccounts[providerAccountId]?.label || 'Slack Default',
        runtimeSecretRefs: {
    ...
    if (normalizedChatJid) {
        ...
        p.outro('Slack connected. Secret stored encrypted in Gantry. Next: run `gantry provider connect slack` to register a conversation.');
    }
    return 0;
}
