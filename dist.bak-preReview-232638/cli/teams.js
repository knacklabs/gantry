...
import { GraphTeamsSetupDiscoveryClient, trimTeamsSetupCredentials, } from '../channels/teams-setup-discovery.js';
import { ensureRuntimeLayout } from '../config/settings/runtime-home.js';
import { ensureConfiguredAgent, ensureConfiguredConversationBinding, loadRuntimeSettings, writeDesiredRuntimeSettings, } from '../config/settings/runtime-settings.js';
...
function parseTeamsApproverIds(raw) {
    if (!raw?.trim())
        return [];
    return [
        ...new Set(raw
...
async function promptManualTeamsChannel(credentials, discoveryClient) {
    const teamId = await promptForValue({
        message: 'Teams team ID',
        validate: (value) => value?.trim() ? undefined : 'Teams team ID is required.',
    });
    if (teamId === null)
        return { type: 'cancel' };
    const channelId = await promptForValue({
        message: 'Teams channel ID (for example 19:...@thread.tacv2)',
        validate: (value) => value?.trim() ? undefined : 'Teams channel ID is required.',
    });
    if (channelId === null)
        return { type: 'cancel' };
    ...
    if (!verified.ok || !verified.chatJid) {
        ...
        if (verified.nextAction)
            p.log.info(verified.nextAction);
        return { type: 'cancel' };
    }
    return {
        type: 'selected',
...
async function chooseTeamsChannelForConnect(credentials, discoveryClient) {
    ...
    if (selected === 'skip')
        return { type: 'skip' };
    const channel = discovery.channels.find((entry) => entry.chatJid === selected);
    return channel ? { type: 'selected', channel } : { type: 'skip' };
}
export async function runTeamsConnectCommand(runtimeHome, discoveryClient = new GraphTeamsSetupDiscoveryClient(), requestedAgentId, requestedAgentName) {
    ...
    const clientId = await promptForValue({
        ...
        validate: (value) => value?.trim() ? undefined : 'Teams client ID is required.',
    });
    if (clientId === null) {
    ...
    const clientSecret = await promptForValue({
        ...
        hide: true,
        validate: (value) => value?.trim() ? undefined : 'Teams client secret is required.',
    });
    ...
    const tenantId = await promptForValue({
        ...
        validate: (value) => value?.trim() ? undefined : 'Teams tenant ID is required.',
    });
    if (tenantId === null) {
        p.outro('Teams connect cancelled.');
        return 1;
    ...
    const channelChoice = await chooseTeamsChannelForConnect(credentials, discoveryClient);
    ...
    const approverInput = channelChoice.type === 'selected'
        ? await promptForValue({
            message: 'Teams admin/approver user IDs (comma-separated; seeds this conversation approvers; must be members of this conversation)',
            defaultValue: '',
        })
        : '';
    ...
    if (channelChoice.type === 'selected') {
        ...
        p.log.success(`Registered ${registered.groupName} for Teams conversation ${registeredChatTitle} in folder ${registered.folder}.`);
    }
    await Promise.all([
    ...
    settings.providerAccounts[providerAccountId] = {
        agentId: providerAgentId,
        provider: 'teams',
        label: settings.providerAccounts[providerAccountId]?.label || 'Teams Default',
        runtimeSecretRefs: {
    ...
    if (channelChoice.type === 'selected') {
        ...
        p.outro('Teams connected. Secret stored encrypted in Gantry. Next: run `gantry provider connect teams` to register a conversation.');
    }
    return 0;
}
