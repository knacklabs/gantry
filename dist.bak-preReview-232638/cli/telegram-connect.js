...
import { normalizeTelegramChatJid, readTelegramFromRuntimeEnv, registerTelegramMainGroup, validateTelegramBotToken, verifyTelegramChatAccess, } from './telegram.js';
import { planRuntimeSecretInput } from './runtime-secret-ref-prompt.js';
import { providerAccountIdForAgent } from './provider-utils.js';
function parseTelegramApproverIds(raw) {
    return [
        ...new Set(raw
            .split(/[,\s]+/)
...
async function promptTelegramToken(defaultValue) {
    const result = await p.password({
        message: 'Telegram bot token',
        mask: '*',
        validate: (value) => String(value ?? '').trim() ? undefined : 'Telegram token is required.',
    });
...
async function promptManualTelegramChatId(defaultChatJid = '') {
...
async function promptManualTelegramAdminSenderId() {
    const result = await p.text({
...
async function promptTelegramPermissionApproverIds(defaultValue) {
    const result = await p.text({
        message: 'Telegram approver user IDs for permissions; seeds conversation approvers (required)',
        placeholder: '12345,67890',
        defaultValue,
        validate: (value) => {
            const parsed = normalizeTelegramPermissionApproverIds(String(value || '').trim());
            return parsed
                ? undefined
...
async function chooseChatFromDiscovery(token) {
    ...
    if (!discovery.ok) {
        ...
        if (discovery.nextAction)
            p.log.info(discovery.nextAction);
    ...
    if (!normalized)
        return { type: 'skip' };
    const selectedChat = discovery.chats.find((chat) => chat.chatJid === normalized);
    const discoveredSender = selectedChat?.chatType === 'private'
        ? /^tg:(\d+)$/.exec(selectedChat.chatJid)?.[1]
        : undefined;
    const adminSenderId = discoveredSender || (await promptManualTelegramAdminSenderId());
    if (adminSenderId === null)
        return { type: 'cancel' };
    return adminSenderId
        ? { type: 'selected', chatJid: normalized, adminSenderId }
...
export async function runTelegramConnectCommand(runtimeHome, requestedAgentId, requestedAgentName) {
    ...
    if (!validation.ok) {
        ...
        if (validation.nextAction)
            p.log.info(validation.nextAction);
        return 1;
    }
    ...
    const adminSenderId = chatChoice.type === 'selected' ? chatChoice.adminSenderId : undefined;
    let approverInput = '';
    if (normalizedChatJid) {
        const promptedApprovers = await promptTelegramPermissionApproverIds(adminSenderId || '');
    ...
    if (normalizedChatJid) {
        ...
        p.log.success(`Registered ${registered.groupName} for Telegram chat ${normalizedChatJid} in folder ${registered.folder}.`);
    }
    ...
    if (registeredFolder) {
        const approverIds = parseTelegramApproverIds(approverInput || adminSenderId || '');
        ...
        if (approverIds.length > 0) {
            p.log.success(`Enabled session/admin commands and permission approvals for Telegram sender(s) ${approverIds.join(', ')}.`);
        }
        else {
            p.log.info('No Telegram conversation approver was configured. Run `gantry provider connect telegram` again and enter your own Telegram user ID if you want chat commands.');
        }
    }
    else {
    ...
    settings.providerAccounts[providerAccountId] = {
        agentId: providerAgentId,
        provider: 'telegram',
        label: settings.providerAccounts[providerAccountId]?.label || 'Telegram Default',
        runtimeSecretRefs: {
    ...
    if (normalizedChatJid) {
        ...
        p.outro('Telegram connected. Secret stored encrypted in Gantry. Next: run `gantry provider connect telegram` to register a conversation.');
    }
    return 0;
}
