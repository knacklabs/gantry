...
import { safeTelegramDescription, TOKEN_BOUND_HTTP_GUIDANCE, TOKEN_BOUND_NETWORK_GUIDANCE, } from './provider-error-guidance.js';
...
import { allocateDefaultAgentFolder, defaultTriggerForAgentName, normalizeDefaultAgentName, } from './main-agent.js';
...
export function normalizeTelegramChatJid(raw) {
...
export async function validateTelegramBotToken(token, timeoutMs = 10_000) {
    ...
    try {
        const response = await fetch(`https://api.telegram.org/bot${trimmed}/getMe`, { signal: controller.signal });
        if (!response.ok) {
            return {
                ok: false,
        ...
        if (!payload.ok || !payload.result?.id) {
            return {
                ok: false,
                message: safeTelegramDescription(payload.description, 'Telegram rejected this token.'),
...
export async function verifyTelegramChatAccess(options) {
    ...
    if (!normalizedJid) {
        return {
            ...
            nextAction: 'Use a numeric Telegram chat ID (for example: -1001234567890).',
        };
    }
    const chatId = normalizedJid.slice(3);
    const timeoutMs = options.timeoutMs ?? 10_000;
    try {
        const getChatResponse = await fetchWithTimeout(`https://api.telegram.org/bot${token}/getChat?chat_id=${encodeURIComponent(chatId)}`, timeoutMs);
        ...
        const chatPayload = await readTelegramPayload(getChatResponse);
        if (!chatPayload.ok || !chatPayload.result?.id) {
            return {
                ok: false,
                message: safeTelegramDescription(chatPayload.description, 'Telegram could not resolve this chat.'),
                nextAction: 'Add the bot to the chat and grant message permission, then retry.',
            };
        }
        const chatTitle = chatPayload.result.title ||
            chatPayload.result.username ||
            chatPayload.result.first_name ||
            normalizedJid;
        if (options.botId) {
            const getMemberResponse = await fetchWithTimeout(`https://api.telegram.org/bot${token}/getChatMember?chat_id=${encodeURIComponent(chatId)}&user_id=${options.botId}`, timeoutMs);
            ...
            const memberPayload = await readTelegramPayload(getMemberResponse);
            if (!memberPayload.ok) {
                return {
                    ok: false,
                    chatTitle,
                    message: safeTelegramDescription(memberPayload.description, 'Telegram could not verify bot membership in this chat.'),
                    nextAction: 'Ensure the bot has access to this chat and can read/write messages.',
                };
            }
        ...
        if (shouldSendProbe) {
            const sendResponse = await fetchWithTimeout(`https://api.telegram.org/bot${token}/sendMessage`, timeoutMs, {
                method: 'POST',
                headers: {
            ...
            const sendPayload = await readTelegramPayload(sendResponse);
            if (!sendPayload.ok || !sendPayload.result?.message_id) {
                return {
                    ok: false,
                    chatTitle,
                    message: safeTelegramDescription(sendPayload.description, 'Telegram rejected the setup test message.'),
                    nextAction: 'Grant the bot permission to post in this chat and retry.',
                };
            }
        }
        return {
...
export function readTelegramFromRuntimeEnv(runtimeHome) {
