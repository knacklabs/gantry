...
import { readRuntimeMemorySettingsSnapshot, } from './settings/runtime-settings.js';
...
export let runtimeMemorySettings = {};
let runtimeMemorySettingsError = null;
try {
...
if (runtimeMemorySettingsError) {
    throw new Error(`Invalid runtime memory settings: ${runtimeMemorySettingsError.message}`);
}
export const RUNTIME_MEMORY_ENABLED = runtimeMemorySettings.enabled ?? true;
export const RUNTIME_MEMORY_DREAMING_ENABLED = runtimeMemorySettings.dreamingEnabled ?? false;
export const RUNTIME_MEMORY_DREAMING_ALERTS_ENABLED = runtimeMemorySettings.dreamingAlerts ?? DEFAULT_MEMORY_DREAMING_ALERTS;
