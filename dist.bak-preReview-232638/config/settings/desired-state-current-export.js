import { configuredBindingId, configuredConversationId, dedupeConfiguredConversation, activeSources, readableActiveCapabilities, stableBindingId, stableSettingsId, } from './desired-state-export-helpers.js';
import { defaultRuntimeSecretRefs, providerInfoForJid, stripProviderPrefix, } from './desired-state-provider-conversations.js';
import { agentIdForFolder, folderForAgentId, groupByAgentId, groupByConversationId, storedConversationKey, } from './desired-state-service-helpers.js';
export async function exportCurrentDesiredState(input) {
    ...
    const activeStoredAgents = storedAgents.filter((agent) => agent.status === 'active');
    const agentIds = [
        ...new Set([
    ...
    const [toolBindingRows, toolSourceRows, skillBindingRows, mcpBindingRows, toolCatalogRows, skillCatalogRows, storedProviderAccounts, storedConversationBindings, storedConversations,] = await Promise.all([
    ...
    const skillCatalogById = new Map(skillCatalogRows.map((skill) => [skill.id, skill]));
    const storedConversationsByExternal = new Map();
    for (const conversation of storedConversations) {
        const externalId = conversation.externalRef?.value?.trim();
        if (!externalId)
            continue;
        storedConversationsByExternal.set(storedConversationKey(conversation.providerAccountId, externalId), conversation);
    }
    const publicThreadIdsByCanonicalId = new Map();
    if (typeof deps.repositories.conversations?.listThreads === 'function') {
        const storedThreads = await Promise.all(storedConversations.map((conversation) => deps.repositories.conversations.listThreads(conversation.id)));
    ...
    const conversationIdsWithActiveInstalls = new Set(storedConversationBindings
        .filter((binding) => binding.status === 'active')
        .map((binding) => String(binding.conversationId)));
    const storedApproversByConversation = groupByConversationId(deps.repositories.conversations
        ? await deps.repositories.conversations.listConversationApproversForConversations(storedConversations.map((conversation) => conversation.id))
        : []);
    for (const connection of storedProviderAccounts.filter((connection) => !isInternalAppControlProviderAccount(connection) &&
        !isCanonicalFallbackProviderAccount(connection))) {
        const providerId = connection.providerId;
        const agentFolder = folderForAgentId(connection.agentId) ?? String(connection.agentId);
        ...
        providerAccounts[connectionId] = {
            ...
            config: Object.fromEntries(Object.entries(connection.config).filter((entry) => typeof entry[1] === 'string')),
        };
        providers[providerId] = {
            enabled: providers[providerId]?.enabled === true ||
                connection.status === 'active',
        };
    }
    for (const conversation of storedConversations.filter((conversation) => conversation.status === 'active')) {
        const providerAccountId = conversation.providerAccountId;
        ...
        const conversationId = configuredConversationId({
            ...
            stableSettingsId(`${connection.provider}_${externalId || conversation.id}`, conversations);
        const existingConversation = settings.conversations[conversationId];
        const storedApproverRows = storedApproversByConversation.get(conversation.id) ?? [];
        ...
        conversations[conversationId] = {
            ...
            controlApprovers: [...new Set(controlApprovers)].sort((a, b) => a.localeCompare(b)),
    ...
    for (const agent of activeStoredAgents) {
        ...
        agents[folder] = {
            ...
            sources: activeSources(skillBindingsByAgent.get(agent.id) ?? [], mcpBindingsByAgent.get(agent.id) ?? [], skillCatalogById, toolSourcesByAgent.get(agent.id) ?? []),
            capabilities: readableActiveCapabilities(toolBindingsByAgent.get(agent.id) ?? [], toolCatalogById, {
    ...
    const storedConversationsById = new Map(storedConversations.map((conversation) => [conversation.id, conversation]));
    for (const binding of storedConversationBindings.filter((binding) => binding.status === 'active')) {
        const folder = folderForAgentId(binding.agentId);
        if (!folder)
            continue;
        const storedConversation = storedConversationsById.get(binding.conversationId);
        if (!storedConversation)
            continue;
        const externalId = storedConversation.externalRef?.value?.trim() ||
            String(storedConversation.id).replace(/^conversation:/, '');
        const conversationId = configuredConversationId({
            providerConnectionId: binding.providerAccountId,
            externalId,
            conversations,
        }) ??
        ...
        if (!conversationId || !conversations[conversationId])
            continue;
        const canonicalThreadId = typeof binding.threadId === 'string' && binding.threadId.trim()
        ...
        const existingBindingId = threadId
            ? Object.entries(settings.bindings).find(([, candidate]) => candidate.agent === folder &&
                candidate.conversation === conversationId &&
                candidate.threadId === threadId)?.[0]
            : configuredBindingId({
                agent: folder,
                conversationId,
        ...
        bindings[bindingId] = {
            ...
            model: typeof routeAgentConfig?.model === 'string'
                ? routeAgentConfig.model
                : existingBinding?.model,
            permissionMode: routeAgentConfig?.permissionMode === 'ask' ||
        ...
        conversations[conversationId].installedAgents[threadId ? `${folder}_${threadId}` : folder] = {
            agentId: folder,
            providerAccountId: binding.providerAccountId,
            threadId,
    ...
    for (const exported of exportedGroups) {
        const { jid, group, toolBindings, toolSources, skillBindings, mcpBindings, } = exported;
        ...
        const groupProviderAccountId = typeof group.providerAccountId === 'string' &&
            group.providerAccountId.trim()
            ? group.providerAccountId.trim()
            : undefined;
        const connectionId = groupProviderAccountId ??
            Object.entries(providerAccounts).find(([, account]) => account.provider === providerId && account.agentId === folder)?.[0] ??
            Object.entries(settings.providerAccounts).find(([, account]) => account.provider === providerId && account.agentId === folder)?.[0] ??
            `${providerId}_default`;
        ...
        const storedConversation = storedConversationsByExternal.get(storedConversationKey(connectionId, externalId)) ?? null;
        const storedApprovers = !storedConversation
            ? []
            : (storedApproversByConversation.get(storedConversation.id) ?? []).map((approver) => approver.externalUserId);
        const existingConversation = conversations[conversationId] ?? settings.conversations[conversationId];
        ...
        const desiredBindingId = configuredBindingId({
            agent: folder,
            conversationId,
            bindings,
        ...
        agents[folder] = {
            ...
            relationshipMode: existing?.relationshipMode ??
            ...
            sources: activeSources(skillBindings, mcpBindings, skillCatalogById, toolSources),
...
function isInternalAppControlProviderAccount(connection) {
...
function isCanonicalFallbackProviderAccount(connection) {
    return isCanonicalFallbackProviderAccountId(String(connection.id));
}
function isCanonicalFallbackProviderAccountId(id) {
    return (id.startsWith('channel-providerAccount:') ||
        id.startsWith('channel-providerConnection:'));
}
function runtimeSecretRefsForConnection(connection) {
    return Object.fromEntries(Object.entries(connection.runtimeSecretRefs).sort(([a], [b]) => a.localeCompare(b)));
}
function nonEmptyTrigger(...candidates) {
...
function defaultRequiresTriggerForConversationKind(kind) {
    return kind !== 'direct';
}
