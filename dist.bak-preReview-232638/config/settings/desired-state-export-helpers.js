...
export function activeCapabilities(toolBindings) {
    return toolBindings
        .filter((binding) => binding.status === 'active')
        .map((binding) => capabilityFromToolBinding(binding));
}
export function activeSources(skillBindings, mcpBindings, skillCatalogById, toolSources = []) {
    return {
...
export function readableActiveCapabilities(toolBindings, toolCatalogById, _options = {}) {
...
function capabilityFromToolBinding(binding) {
    return {
        id: String(binding.toolId).replace(/^tool:/, ''),
        version: 'builtin',
    };
}
function capabilityFromToolReference(reference, tool) {
...
export function dedupeConfiguredConversation(input) {
    ...
    for (const duplicateId of matches) {
        if (duplicateId === input.canonicalId)
            continue;
        const duplicate = input.conversations[duplicateId];
        if (!duplicate)
            continue;
        if (canonical.controlApprovers.length === 0 &&
        ...
        for (const [bindingId, binding] of Object.entries(input.bindings)) {
            if (binding.conversation !== duplicateId)
                continue;
            const redundantBinding = Object.entries(input.bindings).some(([candidateId, candidate]) => candidateId !== bindingId &&
...
function rankedConversationMatches(input) {
    return Object.entries(input.conversations)
        .filter(([, conversation]) => (conversation.providerAccount ?? conversation.providerConnection) ===
        input.providerConnectionId &&
        conversation.externalId === input.externalId)
        .sort(([leftId, left], [rightId, right]) => {
        const score = conversationSettingsScore(right) - conversationSettingsScore(left);
        return score || leftId.localeCompare(rightId);
    })
        .map(([id]) => id);
}
function conversationSettingsScore(conversation) {
    return conversation.controlApprovers.length > 0 ? 1 : 0;
}
export function configuredBindingId(input) {
    return Object.entries(input.bindings).find(([, binding]) => binding.agent === input.agent &&
        binding.conversation === input.conversationId)?.[0];
}
export function stableBindingId(jid, existing) {
    const matching = Object.entries(existing).find(([, binding]) => binding &&
...
export function stableSettingsId(seed, existing) {
    const base = seed
