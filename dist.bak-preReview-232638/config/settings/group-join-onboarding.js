...
import { loadDesiredRuntimeSettingsForWrite, writeDesiredRuntimeSettings, } from './desired-settings-writer.js';
import { SettingsRevisionConflictError, SettingsStaleMutationError, } from './settings-import-service.js';
export function createGroupJoinOnboardingCoordinator(deps) {
    ...
    return {
        recordPrompt: (input) => resolved.repository().recordPrompt({
            id: resolved.newId(),
            ...input,
            now: resolved.now(),
        }),
        getById: (id) => resolved.repository().getById(id),
        dismiss: (id) => resolved.repository().markDismissed({ id, now: resolved.now() }),
        markLeft: (input) => resolved.repository().markLeft({ ...input, now: resolved.now() }),
        register: async ({ id, externalId, title, approvedBy }) => {
            ...
            try {
                for (let attempt = 0; attempt < 2; attempt += 1) {
                    ...
                    if (!account) {
                        throw new Error(`Provider Account not found: ${record.providerAccountId}`);
                    }
                    applyConversationInstallToSettings({
                        settings,
                        conversation: {
                            id: `conversation:${record.providerAccountId}:${record.chatJid}`,
                            externalRef: {
                                kind: 'conversation',
                                value: externalId,
                            },
