...
import { applyProviderManagedMemoryDefaults, loadRuntimeSettings, } from './runtime-settings.js';
...
function providerFromSettings(settings) {
    ...
    const resolved = resolveModelSelectionForWorkloadWithFamilies(settings.agent.defaultModel || DEFAULT_SETUP_MODEL_ALIAS, 'chat', settings.modelFamilies);
    if (resolved.ok)
        return resolved.entry.modelRoute.id;
    const fallback = resolveModelSelectionForWorkload(DEFAULT_SETUP_MODEL_ALIAS, 'chat');
...
function applyAliasOverride(input) {
    if (!(input.field in input.body))
        return undefined;
    const value = input.body[input.field];
    if (value === null) {
        if (!input.reset)
            return `${input.field} cannot be reset.`;
        input.reset();
        return undefined;
    }
    if (value === 'inherit') {
        if (!input.reset)
            return `${input.field} does not support inheritance.`;
        input.reset();
        return undefined;
    }
    ...
    // stored verbatim.
    const resolved = resolveModelSelectionForWorkloadWithFamilies(value, input.workload, input.settings.modelFamilies);
    if (!resolved.ok)
...
function applyJobsPatch(input) {
    ...
    const oneTime = resolveModelSelectionForWorkloadWithFamilies(input.value, 'one_time_job', input.settings.modelFamilies);
    if (!oneTime.ok)
        return oneTime.message;
    const recurring = resolveModelSelectionForWorkloadWithFamilies(input.value, 'recurring_job', input.settings.modelFamilies);
    if (!recurring.ok)
...
function resetMemoryDefaults(settings, providerId = providerFromSettings(settings)) {
    applyProviderManagedMemoryDefaults(settings, providerId);
}
async function memoryResetProviderFromSettings(settings, getConfiguredModelProviderIds) {
...
export async function updateRuntimeModelDefaults(input) {
    ...
    if ('memory' in input.body) {
        ...
        resetMemoryDefaults(settings, await memoryResetProviderFromSettings(settings, input.getConfiguredModelProviderIds));
    }
