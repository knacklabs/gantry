export function deriveAgentBindingsFromDesiredState(input) {
    const agents = Object.fromEntries(Object.entries(input.agents).map(([agentId, agent]) => [
        agentId,
    ...
    for (const [bindingId, binding] of Object.entries(input.bindings)) {
        ...
        const install = conversation.installedAgents[binding.installKey ?? ''] ??
            Object.values(conversation.installedAgents).find((candidate) => candidate.agentId === binding.agent &&
...
export function deriveBindingsFromConversationInstalls(conversations) {
    const bindings = {};
...
export function flattenConversationInstalls(conversations) {
    const installs = {};
    for (const [conversationId, conversation] of Object.entries(conversations)) {
        for (const [installId, install] of Object.entries(conversation.installedAgents)) {
...
function derivedInstallKey(installId, conversationId, install) {
    const scope = install.threadId ?? (installId === install.agentId ? '' : installId);
    return scope
