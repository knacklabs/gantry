...
export function getDefaultRuntimeSandboxSettings() {
    return {
...
export function getDefaultMemoryBackfillSettings() {
    return {
...
function providerForChatAlias(chatAlias, familyOrder) {
    // Family-aware: a family chat alias derives its provider from the selected
    // member, not the default-provider fallback.
    const resolved = resolveModelSelectionForWorkloadWithFamilies(chatAlias, 'chat', familyOrder);
    if (resolved.ok)
...
export function getProviderManagedMemoryDefaults(providerId = providerForChatAlias(DEFAULT_SETUP_MODEL_ALIAS)) {
...
export function createDefaultRuntimeSettings() {
    const storage = {
        postgres: {
    ...
    return {
        ...
        providers: Object.fromEntries(listChannelProviders().map((provider) => [
            provider.id,
            { enabled: false },
        ])),
        providerAccounts: {},
...
export function applyProviderManagedMemoryDefaults(settings, providerId = providerForChatAlias(settings.agent.defaultModel || DEFAULT_SETUP_MODEL_ALIAS, settings.modelFamilies)) {
    settings.memory.llm.models = getProviderManagedMemoryDefaults(providerId);
}
export function applyModelDefaults(settings, chatAlias) {
    const resolved = resolveModelSelectionForWorkloadWithFamilies(chatAlias, 'chat', settings.modelFamilies);
    settings.agent.defaultModel = resolved.ok
        ? resolved.alias
        : DEFAULT_SETUP_MODEL_ALIAS;
    settings.agent.oneTimeJobDefaultModel = '';
    settings.agent.recurringJobDefaultModel = '';
    applyProviderManagedMemoryDefaults(settings, providerForChatAlias(settings.agent.defaultModel, settings.modelFamilies));
}
