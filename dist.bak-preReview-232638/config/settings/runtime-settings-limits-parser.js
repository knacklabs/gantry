...
function parsePositiveInt(raw, pathPrefix) {
    if (typeof raw !== 'number' || !Number.isInteger(raw) || raw <= 0) {
        throw new Error(`${pathPrefix} must be a positive integer`);
    }
    return raw;
}
export function parseLimitsSettings(raw) {
    ...
    for (const [providerId, providerRaw] of Object.entries(raw)) {
        if (!providerIds.has(providerId)) {
            throw new Error(`limits.${providerId} is not a supported model provider. Supported providers: ${[
                ...providerIds,
            ]
                .sort()
                .join(', ')}.`);
        ...
        for (const key of Object.keys(map)) {
            if (key !== 'requests_per_minute') {
                throw new Error(`limits.${providerId}.${key} is not supported. Configure requests_per_minute.`);
            }
        }
        providers[providerId] = {
            requestsPerMinute: parsePositiveInt(map.requests_per_minute, `limits.${providerId}.requests_per_minute`),
        };
    }
    return { providers };
}
