import { executableModelEntry, providerRoute, } from '../../shared/model-catalog.js';
import { getModelProviderDefinition, normalizeModelRouteProviderId, } from '../../shared/model-provider-registry.js';
import { containsControlCharacter, parseBooleanValue, parsePositiveIntegerValue, parseStringArrayValue, parseStringValue, } from './runtime-settings-parse-primitives.js';
const MODEL_WORKLOADS = [
    'chat',
    'one_time_job',
    'recurring_job',
    'memory_extractor',
...
export function parseModelAliases(raw) {
    ...
    const aliases = {};
    for (const [aliasId, aliasRaw] of Object.entries(raw)) {
        aliases[aliasId] = parseModelAlias(aliasRaw, aliasId);
    }
    return aliases;
}
export function modelAliasesToCatalogEntries(aliases) {
...
function cacheSettingsForProvider(providerId) {
...
function cacheModeForPromptMode(mode) {
    if (mode === 'none')
        return undefined;
    const providerPrefix = mode.split('_', 1)[0];
    if (mode.endsWith('_cache_control')) {
        return `${providerPrefix}-prompt`;
    }
    if (mode.endsWith('_automatic_prefix')) {
        const suffix = providerPrefix === 'openrouter' ? 'provider-prompt' : 'automatic-prompt';
        return `${providerPrefix}-${suffix}`;
    }
    return undefined;
}
function parseModelAlias(raw, aliasId) {
    ...
    for (const key of Object.keys(map)) {
        if (key !== 'provider' &&
            ...
            key !== 'supports_tools' &&
            key !== 'source') {
            throw new Error(`${pathPrefix}.${key} is not supported. Configure provider, provider_model_id, display_name, aliases, supported_workloads, pricing, capability flags, or source.`);
        }
    }
    const provider = normalizeModelRouteProviderId(parseStringValue(map.provider, `${pathPrefix}.provider`));
    const providerModelId = parseStringValue(map.provider_model_id, `${pathPrefix}.provider_model_id`);
    const aliases = parseAliasList(map.aliases, aliasId, pathPrefix);
    const recommendedAlias = parseStringValue(map.recommended_alias, `${pathPrefix}.recommended_alias`, aliases[0] ?? aliasId);
    ...
    return {
        provider,
        providerModelId,
        displayName: parseStringValue(map.display_name, `${pathPrefix}.display_name`, providerModelId),
        aliases,
        recommendedAlias,
        supportedWorkloads: parseWorkloads(map.supported_workloads, pathPrefix),
        contextWindowTokens: parseOptionalPositiveInteger(map.context_window_tokens, `${pathPrefix}.context_window_tokens`),
        maxOutputTokens: parseOptionalPositiveInteger(map.max_output_tokens, `${pathPrefix}.max_output_tokens`),
        inputUsdPerMillionTokens: parseOptionalPrice(map.input_usd_per_million_tokens, `${pathPrefix}.input_usd_per_million_tokens`),
        outputUsdPerMillionTokens: parseOptionalPrice(map.output_usd_per_million_tokens, `${pathPrefix}.output_usd_per_million_tokens`),
        cachedInputUsdPerMillionTokens: parseOptionalPrice(map.cached_input_usd_per_million_tokens, `${pathPrefix}.cached_input_usd_per_million_tokens`),
        cacheWriteUsdPerMillionTokens: parseOptionalPrice(map.cache_write_usd_per_million_tokens, `${pathPrefix}.cache_write_usd_per_million_tokens`),
        supportsThinking: map.supports_thinking === undefined
            ? undefined
            : parseBooleanValue(map.supports_thinking, `${pathPrefix}.supports_thinking`),
        supportsTools: map.supports_tools === undefined
            ? undefined
            : parseBooleanValue(map.supports_tools, `${pathPrefix}.supports_tools`),
        source: parseSource(map.source, pathPrefix, aliasId),
    };
}
function parseAliasList(raw, aliasId, pathPrefix) {
...
function parseWorkloads(raw, pathPrefix) {
    const values = raw === undefined
        ? ['chat', 'one_time_job', 'recurring_job']
        : parseStringArrayValue(raw, `${pathPrefix}.supported_workloads`);
    return values.map((workload, index) => {
        if (MODEL_WORKLOADS.includes(workload)) {
            return workload;
        }
        throw new Error(`${pathPrefix}.supported_workloads[${index}] must be one of ${MODEL_WORKLOADS.join(', ')}`);
    });
}
function parseOptionalPositiveInteger(raw, pathPrefix) {
    if (raw === undefined)
        return undefined;
    return parsePositiveIntegerValue(raw, pathPrefix, 1);
}
function parseOptionalPrice(raw, pathPrefix) {
    if (raw === undefined)
        return undefined;
    const value = typeof raw === 'string' && /^([0-9]+)(\.[0-9]+)?$/.test(raw.trim())
        ? Number(raw)
        : raw;
...
function parseSource(raw, pathPrefix, aliasId) {
    if (raw === undefined) {
        return {
            label: `${pathPrefix}`,
            url: 'settings.yaml',
    ...
    for (const key of Object.keys(map)) {
        if (key !== 'label' && key !== 'url' && key !== 'verified_at') {
            throw new Error(`${pathPrefix}.source.${key} is not supported. Configure label, url, or verified_at.`);
        }
    }
    return {
        label: parseStringValue(map.label, `${pathPrefix}.source.label`, aliasId),
        url: parseStringValue(map.url, `${pathPrefix}.source.url`),
        verifiedAt: parseStringValue(map.verified_at, `${pathPrefix}.source.verified_at`),
    };
}
