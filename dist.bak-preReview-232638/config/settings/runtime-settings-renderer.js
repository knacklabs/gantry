...
import { DEFAULT_AGENT_NAME, DEFAULT_AGENT_SESSION_MAX_MEMORY_CONTEXT_CHARS, DEFAULT_AGENT_SESSION_MEMORY_ITEM_LIMIT, DEFAULT_BROWSER_USAGE_ENABLED, DEFAULT_BROWSER_USAGE_MAX_ACTIONS_PER_WINDOW, DEFAULT_BROWSER_USAGE_MAX_CONCURRENT_PER_SITE, DEFAULT_BROWSER_USAGE_MODE, DEFAULT_BROWSER_USAGE_WINDOW_MS, DEFAULT_EMBED_DIMENSIONS, DEFAULT_EMBED_MODEL, DEFAULT_LLM_GLOBAL_MAX_IN_FLIGHT, DEFAULT_LLM_PER_APP_KEY_MAX_IN_FLIGHT, DEFAULT_MEMORY_BACKFILL_CRON, DEFAULT_MEMORY_BACKFILL_ENABLED, DEFAULT_MEMORY_BACKFILL_MAX_ITEMS_PER_RUN, DEFAULT_MEMORY_BACKFILL_MODE, DEFAULT_MEMORY_BACKFILL_PROVIDER_BATCH_MIN_ITEMS, DEFAULT_MEMORY_DREAMING_ALERTS, DEFAULT_MEMORY_DREAMING_CRON, DEFAULT_MEMORY_EMBED_BATCH_SIZE, DEFAULT_MEMORY_EXTRACTOR_MAX_FACTS, DEFAULT_MEMORY_EXTRACTOR_MIN_CONFIDENCE, DEFAULT_MEMORY_MAINTENANCE_MAX_PENDING, DEFAULT_MODEL_GATEWAY_BIND_HOST, DEFAULT_OPENAI_DAILY_EMBED_LIMIT, DEFAULT_STORAGE_POSTGRES_SCHEMA, DEFAULT_STORAGE_POSTGRES_URL_ENV, getProviderManagedMemoryDefaults, } from './runtime-settings-defaults.js';
import { quoteYamlKey, renderAgentDelegatesYaml, renderLimitsSettingsYaml, renderModelAliasesYaml, renderModelFamiliesYaml, renderObserverSettingsYaml, renderObservabilitySettingsYaml, } from './runtime-settings-optional-blocks-renderer.js';
...
const MODEL = 'opus';
function renderDefaultsYaml(lines, agent) {
    lines.push('defaults:');
    ...
    if (agent.oneTimeJobDefaultModel || agent.recurringJobDefaultModel) {
        ...
        if (agent.oneTimeJobDefaultModel) {
            lines.push(`    one_time_model: ${quoteYamlString(agent.oneTimeJobDefaultModel)}`);
        }
        if (agent.recurringJobDefaultModel) {
            lines.push(`    recurring_model: ${quoteYamlString(agent.recurringJobDefaultModel)}`);
        }
    }
    if (agent.sessions.memoryItemLimit !==
        DEFAULT_AGENT_SESSION_MEMORY_ITEM_LIMIT ||
        agent.sessions.maxMemoryContextChars !==
            DEFAULT_AGENT_SESSION_MAX_MEMORY_CONTEXT_CHARS) {
        lines.push('  sessions:');
        if (agent.sessions.memoryItemLimit !== DEFAULT_AGENT_SESSION_MEMORY_ITEM_LIMIT) {
            lines.push(`    memory_item_limit: ${agent.sessions.memoryItemLimit}`);
        }
        if (agent.sessions.maxMemoryContextChars !==
            DEFAULT_AGENT_SESSION_MAX_MEMORY_CONTEXT_CHARS) {
            lines.push(`    max_memory_context_chars: ${agent.sessions.maxMemoryContextChars}`);
        }
    }
    lines.push('');
}
function renderDesiredStateYaml(lines, desiredState) {
    lines.push('desired_state:', `  authoritative: ${desiredState.authoritative ? 'true' : 'false'}`, '');
}
function renderMemorySettingsYaml(lines, memory) {
    lines.push('memory:', `  enabled: ${memory.enabled ? 'true' : 'false'}`);
    lines.push('  embeddings:', `    enabled: ${memory.embeddings.enabled ? 'true' : 'false'}`, `    provider: ${memory.embeddings.provider}`, `    model: ${quoteYamlString(memory.embeddings.model)}`, `    dimensions: ${memory.embeddings.dimensions}`, `    daily_limit: ${memory.embeddings.dailyLimit}`, `    batch_size: ${memory.embeddings.batchSize}`, '    backfill:', `      enabled: ${memory.embeddings.backfill.enabled ? 'true' : 'false'}`, `      cron: ${quoteYamlString(memory.embeddings.backfill.cron)}`, `      max_items_per_run: ${memory.embeddings.backfill.maxItemsPerRun}`, `      mode: ${memory.embeddings.backfill.mode}`, `      provider_batch_min_items: ${memory.embeddings.backfill.providerBatchMinItems}`, '  dreaming:', `    enabled: ${memory.dreaming.enabled ? 'true' : 'false'}`, `    cron: ${quoteYamlString(memory.dreaming.cron)}`, `    alerts: ${memory.dreaming.alerts ? 'true' : 'false'}`, '    embeddings:', `      enabled: ${memory.dreaming.embeddings.enabled ? 'true' : 'false'}`, `      provider: ${memory.dreaming.embeddings.provider}`, `      model: ${quoteYamlString(memory.dreaming.embeddings.model)}`, '  llm:', `    extractor_max_facts: ${memory.llm.extractorMaxFacts}`, `    extractor_min_confidence: ${memory.llm.extractorMinConfidence}`, '    models:', `      extractor: ${quoteYamlString(memory.llm.models.extractor)}`, `      dreaming: ${quoteYamlString(memory.llm.models.dreaming)}`, `      consolidation: ${quoteYamlString(memory.llm.models.consolidation)}`, '  maintenance:', `    max_pending: ${memory.maintenance.maxPending}`, '');
}
function renderStorageSettingsYaml(lines, storage) {
    lines.push('storage:', '  postgres:', `    url_env: ${quoteYamlString(storage.postgres.urlEnv)}`, `    schema: ${quoteYamlString(storage.postgres.schema)}`, '');
}
function renderPermissionSettingsYaml(lines, permissions) {
    lines.push('permissions:', '  yolo_mode:', `    enabled: ${permissions.yoloMode.enabled ? 'true' : 'false'}`);
    if (permissions.yoloMode.denylist.length > 0) {
        lines.push(`    denylist: ${JSON.stringify(permissions.yoloMode.denylist)}`);
    }
    if (permissions.yoloMode.denylistPaths.length > 0) {
        lines.push(`    denylist_paths: ${JSON.stringify(permissions.yoloMode.denylistPaths)}`);
    }
    lines.push('  egress:', `    denylist: ${JSON.stringify(permissions.egress.denylist)}`);
    ...
    if (permissions.autoMode.model) {
        lines.push('  auto_mode:', `    model: ${quoteYamlString(permissions.autoMode.model)}`);
    }
    lines.push('');
}
function renderConfiguredAgentsYaml(lines, agents) {
    ...
    for (const [folder, agent] of entries) {
        lines.push(`  ${quoteYamlKey(folder)}:`, `    name: ${quoteYamlString(agent.name)}`);
        ...
        if (agent.relationshipMode && agent.relationshipMode !== 'personal') {
            lines.push(`    relationship_mode: ${quoteYamlString(agent.relationshipMode)}`);
        }
        ...
        if (agent.thinking !== undefined) {
            if (agent.thinking.budgetTokens === undefined) {
                ...
                lines.push('    thinking:', '      mode: on', `      budget_tokens: ${agent.thinking.budgetTokens}`);
            }
        }
        ...
        if (agent.permissionMode) {
            lines.push(`    permission_mode: ${quoteYamlString(agent.permissionMode)}`);
        }
        if (agent.oneTimeJobDefaultModel) {
            lines.push(`    one_time_job_default_model: ${quoteYamlString(agent.oneTimeJobDefaultModel)}`);
        }
        if (agent.recurringJobDefaultModel) {
            lines.push(`    recurring_job_default_model: ${quoteYamlString(agent.recurringJobDefaultModel)}`);
        }
        ...
        if (agent.toolRules?.length) {
            ...
            for (const rule of agent.toolRules) {
                lines.push(`      - tool: ${quoteYamlString(rule.tool)}`, `        action: ${rule.action}`);
                if (rule.action === 'block' && rule.when) {
                    lines.push('        when:', `          arg: ${quoteYamlString(rule.when.arg)}`, `          matches: ${quoteYamlString(rule.when.matches)}`);
                }
...
function renderAgentAccessYaml(lines, agent) {
    const hasSources = agent.sources.skills.length > 0 ||
...
function renderAgentSourceListYaml(lines, key, sources) {
    if (sources.length === 0)
        return;
    lines.push(`        ${key}:`);
...
function renderProviderAccountsYaml(lines, accounts) {
    ...
    for (const [accountId, account] of entries) {
        lines.push(`  ${quoteYamlKey(accountId)}:`, `    agent: ${quoteYamlString(account.agentId)}`, `    provider: ${quoteYamlString(account.provider)}`, `    label: ${quoteYamlString(account.label)}`);
        if (account.status === 'disabled') {
            lines.push('    status: disabled');
        }
        const refs = Object.entries(account.runtimeSecretRefs).sort(([a], [b]) => a.localeCompare(b));
        if (refs.length === 0) {
        ...
        const identities = Object.entries(account.externalIdentityRef ?? {}).sort(([a], [b]) => a.localeCompare(b));
        ...
        const config = Object.entries(account.config ?? {}).sort(([a], [b]) => a.localeCompare(b));
        if (config.length > 0) {
            lines.push('    config:');
...
function renderConversationsYaml(lines, conversations) {
    const entries = Object.entries(conversations).sort(([a], [b]) => a.localeCompare(b));
    if (entries.length === 0) {
        return;
    }
    ...
    for (const [conversationId, conversation] of entries) {
        ...
        lines.push(`    provider_account: ${quoteYamlString(conversation.providerAccount ?? conversation.providerConnection)}`);
        lines.push(`    id: ${quoteYamlString(conversation.externalId)}`, `    type: ${quoteYamlString(conversation.kind === 'group' ? 'channel' : conversation.kind)}`, `    display_name: ${quoteYamlString(conversation.displayName)}`);
        if (conversation.brainHarvest) {
            lines.push('    brain_harvest: true');
        }
        lines.push('    sender_policy:', `      allow: ${conversation.senderPolicy.allow === '*' ? '"*"' : JSON.stringify(conversation.senderPolicy.allow)}`, `      mode: ${conversation.senderPolicy.mode}`);
        if (conversation.controlApprovers.length > 0) {
            lines.push(`    control_approvers: ${JSON.stringify(conversation.controlApprovers)}`);
        }
        const installs = Object.entries(conversation.installedAgents ?? {}).sort(([a], [b]) => a.localeCompare(b));
        if (installs.length > 0) {
            lines.push('    installed_agents:');
            for (const [agentId, install] of installs) {
                lines.push(`      ${quoteYamlKey(agentId)}:`, `        provider_account: ${quoteYamlString(install.providerAccountId)}`, `        added_at: ${quoteYamlString(install.addedAt)}`);
                ...
                if (install.memoryScope !== 'conversation') {
                    lines.push(`        memory_scope: ${quoteYamlString(install.memoryScope)}`);
                }
                if (install.trigger) {
                ...
                if (install.permissionMode) {
                    lines.push(`        permission_mode: ${quoteYamlString(install.permissionMode)}`);
                }
            }
        }
    }
...
function renderModelAccessSettingsYaml(lines, credentialBroker) {
    lines.push('model_access:', `  enabled: ${credentialBroker.mode === 'gantry' ? 'true' : 'false'}`, '  gateway:', `    bind_host: ${quoteYamlString(credentialBroker.gateway.bindHost)}`, '');
}
function isDefaultStorage(storage) {
    return (storage.postgres.urlEnv === DEFAULT_STORAGE_POSTGRES_URL_ENV &&
        storage.postgres.schema === DEFAULT_STORAGE_POSTGRES_SCHEMA);
}
function isDefaultCredentialBroker(credentialBroker) {
    return (credentialBroker.mode === 'gantry' &&
        credentialBroker.gateway.bindHost === DEFAULT_MODEL_GATEWAY_BIND_HOST);
}
function isDefaultMemory(memory) {
...
function isDefaultRuntime(runtime) {
    return (runtime.queue.maxMessageRuns === 3 &&
...
function isDefaultBrowserSettings(browser) {
    return (browser.usage.enabled === DEFAULT_BROWSER_USAGE_ENABLED &&
...
function isDefaultPermissionSettings(permissions) {
    return (permissions.yoloMode.enabled === true &&
...
function renderBrowserSettingsYaml(lines, browser) {
    lines.push('browser:', '  usage:', `    enabled: ${browser.usage.enabled ? 'true' : 'false'}`, `    mode: ${quoteYamlString(browser.usage.mode)}`, `    window_ms: ${browser.usage.windowMs}`, `    max_actions_per_window: ${browser.usage.maxActionsPerWindow}`, `    max_concurrent_per_site: ${browser.usage.maxConcurrentPerSite}`);
    const overrides = Object.entries(browser.usage.overrides).sort(([a], [b]) => a.localeCompare(b));
    if (overrides.length > 0) {
        ...
        for (const [site, override] of overrides) {
            ...
            if (override.maxActionsPerWindow !== undefined) {
                lines.push(`        max_actions_per_window: ${override.maxActionsPerWindow}`);
            }
            if (override.maxConcurrentPerSite !== undefined) {
                lines.push(`        max_concurrent_per_site: ${override.maxConcurrentPerSite}`);
            }
        }
    }
    else {
...
function renderRuntimeProcessYaml(lines, runtime) {
    lines.push('runtime:', '  queue:', `    max_message_runs: ${runtime.queue.maxMessageRuns}`, `    max_job_runs: ${runtime.queue.maxJobRuns}`, `    max_message_backlog: ${runtime.queue.maxMessageBacklog}`, `    max_live_admission_backlog: ${runtime.queue.maxLiveAdmissionBacklog}`, `    max_task_backlog: ${runtime.queue.maxTaskBacklog}`, `    max_retries: ${runtime.queue.maxRetries}`, `    base_retry_ms: ${runtime.queue.baseRetryMs}`, `    drain_deadline_ms: ${runtime.queue.drainDeadlineMs}`, '  live_turns:', `    enabled: ${runtime.liveTurns.enabled ? 'true' : 'false'}`, '  llm_admission:', `    global_max_in_flight: ${runtime.llmAdmission.globalMaxInFlight}`, `    per_app_key_max_in_flight: ${runtime.llmAdmission.perAppKeyMaxInFlight}`, '  sandbox:', `    provider: ${quoteYamlString(runtime.sandbox.provider)}`, '    resource_limits:', `      cpu_seconds: ${runtime.sandbox.resourceLimits.cpuSeconds}`, `      memory_mb: ${runtime.sandbox.resourceLimits.memoryMb}`, `      max_processes: ${runtime.sandbox.resourceLimits.maxProcesses}`);
...
export function renderRuntimeSettingsYaml(settings) {
    const lines = [];
