...
import { activateRuntimeModelAliases, parseRuntimeSettings, withRuntimeModelAliases, } from './runtime-settings.js';
...
import { PostgresSettingsRevisionNotifier, } from './settings-revision-notify.js';
...
export class SettingsStaleMutationError extends Error {
    constructor() {
        super('Settings mutation is based on stale settings; reload latest desired state and retry.');
 ...
 */
export async function validateSettingsForImport(deps, settings) {
    const errors = [];
    const schema = withRuntimeModelAliases(settings, () => validateLoadedRuntimeSettings(deps.runtimeHome, settings));
    ...
    const invalidReferences = await service.validateCapabilityReferences(settings);
 ...
 * change.
 */
export async function importWorkstationSettings(deps, settings) {
    if (deps.revisionMirrorRequired &&
        (!deps.previousSettings || !deps.revisionMirror)) {
        throw new Error('Settings mutation requires previous settings and a settings revision mirror for stale revision protection.');
    }
    ...
    const appId = deps.appId ?? 'default';
    if (deps.revisionMirrorRequired && deps.revisionMirror) {
        const revisionSettings = (await normalizeConfiguredCapabilitiesInSettings({
            settings,
            repositories: deps.repositories,
            appId,
        })).settings;
        const previousRevisionSettings = (await normalizeConfiguredCapabilitiesInSettings({
            settings: deps.previousSettings,
            repositories: deps.repositories,
            appId,
        })).settings;
        const latest = await deps.revisionMirror.settingsRevisions.getLatestSettingsRevision(appId);
        const actualRevision = latest?.revision ?? 0;
        if (deps.expectedRevision !== undefined &&
        ...
        if (latest &&
            !revisionDocumentMatchesSettings(latest.settingsDocument, previousRevisionSettings)) {
        ...
        if (outcome.status === 'invalid') {
            throw new Error(['settings validation failed.', ...outcome.errors].join('\n'));
        }
    ...
    try {
        ...
        if (outcome.status === 'invalid') {
            const error = new Error(['settings validation failed.', ...outcome.errors].join('\n'));
            if (deps.revisionMirrorRequired)
                throw error;
            deps.revisionMirror.logWarn?.({ errors: outcome.errors }, 'settings revision mirror failed validation after workstation settings applied');
            return { status: 'applied_no_revision' };
        }
        if (outcome.status === 'conflict') {
            const error = new Error(`settings revision conflicted: expected revision ${outcome.expectedRevision}, actual revision ${outcome.actualRevision}`);
            if (deps.revisionMirrorRequired)
                throw error;
        ...
        deps.revisionMirror.logWarn?.({ err }, 'settings revision mirror failed after workstation settings applied');
        return { status: 'applied_no_revision' };
    }
}
/**
 ...
 */
export async function importFleetSettingsRevision(deps, settings, options = {}) {
    const validation = await validateSettingsForImport(deps, settings);
    if (!validation.ok) {
    ...
    if (deps.pool) {
        const notifier = new PostgresSettingsRevisionNotifier(deps.pool, deps.logWarn);
        const wakeup = {
            appId,
 ...
 * lossless.
 */
export function settingsToRevisionDocument(settings) {
    ...
    return buildRevisionDocument(parseRuntimeSettings(renderRuntimeSettingsYaml(settings)));
}
function buildRevisionDocument(settings) {
    return stripUndefinedDeep({
        ...
        provider_accounts: mapRecord(settings.providerAccounts, (account) => ({
            ...
            config: Object.keys(account.config ?? {}).length > 0
                ? account.config
                : undefined,
        })),
        conversations: mapRecord(settings.conversations, (conversation) => ({
            provider_account: conversation.providerAccount ?? conversation.providerConnection,
            ...
            installed_agents: Object.fromEntries(Object.entries(conversation.installedAgents ?? {}).map(([installId, install]) => [
                installId,
                {
                    provider_account: install.providerAccountId,
                    agent: installId === install.agentId ? undefined : install.agentId,
        ...
        agents: mapRecord(settings.agents, (agent) => ({
            ...
            relationship_mode: agent.relationshipMode && agent.relationshipMode !== 'personal'
            ...
            effort: agent.effort,
            thinking: agent.thinking?.budgetTokens === undefined
                ? agent.thinking?.mode
                : {
            ...
            tool_rules: agent.toolRules && agent.toolRules.length > 0
                ? agent.toolRules
                : undefined,
            access: {
...
export function settingsFromRevisionDocument(document) {
...
export async function settingsMatchesLatestRevision(input) {
    ...
    if (!latest)
        return false;
    return revisionDocumentMatchesSettings(latest.settingsDocument, input.settings);
}
function revisionDocumentMatchesSettings(document, settings) {
    return (stableJson(canonicalizeRevisionDocument(document)) ===
        stableJson(canonicalizeRevisionDocument(settingsToRevisionDocument(settings))));
}
function canonicalizeRevisionDocument(document) {
    try {
...
async function validateProjectionPreconditions(input) {
    ...
    if (!providerAccounts)
        return;
    for (const [accountId, account] of Object.entries(input.settings.providerAccounts)) {
        const existing = await providerAccounts.getProviderAccount(accountId);
        if (!existing)
            continue;
        if (existing.appId !== input.appId) {
            throw new Error(`provider_accounts.${accountId} already belongs to another app`);
        }
        if (existing.providerId !== account.provider) {
            throw new Error(`provider_accounts.${accountId}.provider cannot change from ${existing.providerId} to ${account.provider}; use a new provider account id.`);
        }
    }
}
function mapRecord(record, mapValue) {
    return Object.fromEntries(Object.entries(record).map(([key, value]) => [key, mapValue(value)]));
}
function snakeRecord(value) {
    if (Array.isArray(value))
        return value.map(snakeRecord);
    if (typeof value !== 'object' || value === null)
        return value;
    return stripUndefined(Object.fromEntries(Object.entries(value).map(([key, item]) => [
...
function stripUndefinedDeep(value) {
    ...
    return Object.fromEntries(Object.entries(value).flatMap(([key, item]) => item === undefined ? [] : [[key, stripUndefinedDeep(item)]]));
}
