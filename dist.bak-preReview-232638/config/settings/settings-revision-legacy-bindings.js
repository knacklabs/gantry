export function migrateLegacyAgentBindings(document) {
    ...
    if (!agents || !conversations)
        return document;
    if (!Object.values(agents).some((agent) => recordOrUndefined(agent)?.bindings !== undefined)) {
        return document;
    }
    ...
    if (!nextAgents || !nextConversations)
        return document;
    for (const [agentId, agentRaw] of Object.entries(nextAgents)) {
        ...
        for (const [bindingId, bindingRaw] of Object.entries(bindings)) {
            ...
            if (!installProviderAccount)
                continue;
            const installedAgents = recordOrUndefined(conversationDoc.installed_agents) ?? {};
...
function findLegacyBindingConversation(input) {
    ...
    const explicitAccount = stringValue(input.binding.provider_account_id ??
    ...
    const jidSuffix = input.jid.includes(':')
        ...
        : input.jid;
    const candidates = Object.entries(input.conversations).filter(([, conversationRaw]) => {
        const conversation = recordOrUndefined(conversationRaw);
        if (!conversation)
            return false;
        const externalId = stringValue(conversation.external_id ?? conversation.id);
        return externalId === input.jid || externalId === jidSuffix;
    });
    return (candidates.find(([, conversationRaw]) => {
        const conversation = recordOrUndefined(conversationRaw);
        return (explicitAccount &&
            stringValue(conversation?.provider_account ?? conversation?.provider_connection) === explicitAccount);
    }) ?? candidates[0]);
}
function providerAccountForLegacyInstall(input) {
    const requested = stringValue(input.binding.provider_account_id ??
        input.binding.providerAccountId ??
        input.binding.provider_account ??
        input.binding.provider_connection_id ??
        input.binding.providerConnectionId) ??
        stringValue(input.conversation.provider_account ??
    ...
    const existing = Object.entries(input.providerAccounts).find(([, candidateRaw]) => {
...
function uniqueInstallId(installId, installedAgents) {
    if (!Object.hasOwn(installedAgents, installId))
        return installId;
    let index = 2;
    while (Object.hasOwn(installedAgents, `${installId}_${index}`))
        index += 1;
