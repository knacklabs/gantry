...
export function resolveRuntimeStorageConfig(gantryHome, _runtimeRoot) {
    let settings;
    ...
    try {
        ...
        throw new Error(`Invalid runtime storage settings: ${storageError.message}`, { cause: err });
    }
    return resolveRuntimeStorageConfigFromSettings(settings);
}
export function resolveRuntimeBootstrapStorageConfigFromEnv() {
...
export function resolveRuntimeStorageConfigFromSettings(settings) {
    ...
    const postgresPlaintextHostAllowlist = fleetRehearsalPlaintextPostgresHosts({
        GANTRY_FLEET_REHEARSAL_AUTO_SECRETS: runtimeEnvValueDynamic('GANTRY_FLEET_REHEARSAL_AUTO_SECRETS'),
    });
    if (postgresUrl) {
        try {
            ...
            throw new Error(`Invalid runtime storage settings: ${postgresUrlEnv} ${message}`, { cause: err });
        }
    }
    return {
        postgresUrlEnv,
...
function fallbackStorageSettingsFromEnv() {
    if (!runtimeEnvValueDynamic('GANTRY_DATABASE_URL').trim())
        return null;
    return {
...
function bootstrapStorageSettingsIfMissing(gantryHome) {
    ...
    const deploymentMode = runtimeEnvValueDynamic('GANTRY_BOOTSTRAP_DEPLOYMENT_MODE') ||
        runtimeEnvValueDynamic('GANTRY_DEPLOYMENT_MODE') ||
        'workstation';
    const sandboxProvider = runtimeEnvValueDynamic('GANTRY_BOOTSTRAP_SANDBOX_PROVIDER') ||
        'sandbox_runtime';
    const content = [
        'runtime:',
...
function resolveBootstrapSettingsSchema() {
    const explicit = runtimeEnvValueDynamic('GANTRY_SETTINGS_POSTGRES_SCHEMA').trim();
    if (explicit)
        return explicit;
