...
import { controlPlaneJobStatus, controlPlaneMemoryStatus, controlPlaneProviderInputs, } from '../../application/control-plane/control-plane-settings-inputs.js';
 ...
 */
export async function buildControlPlaneReadModelForRequest(ctx, appId) {
    const settings = ctx.getInternalRuntimeSettings();
    const requiredProviders = requiredModelCredentialProviders(settings);
    const activeProviderIds = new Set(await ctx.getActiveModelCredentialProviderIds(appId));
