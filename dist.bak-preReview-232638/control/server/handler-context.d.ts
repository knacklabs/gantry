...
import type { ControlAgentSettingsPort, ControlAgentSettingsView, ControlObserverStatus, ControlSettingsImportPort, EffectiveControlRuntimeSettings, ResolveControlObserverStatus } from '../../application/control-plane/control-plane-storage-model.js';
import type { AppId } from '../../domain/app/app.js';
import type { ModelCatalogEntry, ModelWorkload } from '../../shared/model-catalog.js';
import type { AgentHarness } from '../../shared/agent-engine.js';
import type { AgentRuntime } from '../../shared/agent-runtime.js';
import type { EgressSettings } from '../../shared/egress-policy.js';
import { type ApiKeyRecord, type Scope } from './auth.js';
...
export type { ControlAgentSettingsPort, ControlAgentSettingsView, ControlObserverStatus, ControlSettingsImportPort, };
...
export type ControlRouteContext = {
    ...
    getDefaultModelConfig: (kind?: 'interactive' | 'oneTimeJob' | 'recurringJob', agentFolder?: string) => ControlDefaultModelConfig;
    getModelDefaults: () => ControlModelDefaults;
    patchModelDefaults: (body: Record<string, unknown>, appId?: AppId, createdBy?: string, options?: {
        getConfiguredModelProviderIds?: () => Promise<ReadonlySet<string>>;
    }) => Promise<ControlModelDefaultsPatchResult>;
    preflightModelProvider: (providerId: string, appId?: AppId, chatAlias?: string) => Promise<ControlModelProviderPreflightResult>;
    getActiveModelCredentialProviderIds: (appId: AppId) => Promise<string[]>;
    countPendingAccessRequests: (appId: AppId) => Promise<number>;
    listControlPlaneJobs: (appId: AppId) => Promise<Array<{
        id: string;
        status?: string;
    ...
    addMessageReaction?: (jid: string, messageRef: string, emoji: string, options?: {
        providerAccountId?: string;
    }) => Promise<void>;
    getBrowserStatus?: JobManagementServiceDeps['getBrowserStatus'];
    syncSettingsFromProjection: (appId: AppId, overrides?: ProjectionSettingsOverrides) => Promise<void>;
...
export declare function authorizeControlRequest(req: IncomingMessage, res: ServerResponse, keys: ApiKeyRecord[], scopes: Scope[]): ApiKeyRecord | null;
