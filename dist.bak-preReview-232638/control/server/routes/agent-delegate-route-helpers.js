...
import { callableAgentToolName, conversationBoundAgentIdsForRoute, projectCallableAgentTools, } from '../../../application/core-tools/callable-agent-tools.js';
...
export async function loadAgentDelegateSettings(ctx, appId) {
    const latest = await getRuntimeStorage().repositories.settingsRevisions.getLatestSettingsRevision(appId);
    return latest
        ? {
            settings: ctx.agentSettings.decodeRevisionDocument(latest.settingsDocument),
...
export async function resolveCallableDelegateRoster(input) {
    ...
    for (const [routeKey, route] of Object.entries(input.conversationRoutes)) {
        ...
        const threadIds = threadIdsByConversationAndChat.get(contextKey) ?? new Set();
        threadIds.add(parsed.threadId);
        threadIdsByConversationAndChat.set(contextKey, threadIds);
    }
    for (const [routeKey, route] of Object.entries(input.conversationRoutes)) {
        const routeAgentId = route.agentId ?? String(agentIdForFolder(route.folder));
        if (routeAgentId !== String(input.orchestrator.id))
            continue;
        const parsed = parseAgentThreadQueueKey(routeKey);
        const threadIds = parsed.threadId
            ...
            : [
                undefined,
                ...(threadIdsByConversationAndChat.get(`${route.conversationId ?? ''}\0${parsed.chatJid}`) ?? []),
            ];
        for (const threadId of threadIds) {
            for (const agentId of conversationBoundAgentIdsForRoute({
                ...
                callerProviderAccountId: parsed.providerAccountId ?? route.providerAccountId,
            })) {
                conversationBoundAgentIds.add(agentId);
            }
        }
    }
    const personasByAgentId = Object.fromEntries(Object.entries(input.settings.agents).map(([folder, agent]) => [
        String(agentIdForFolder(folder)),
        agent.persona,
    ]));
    const identities = agentIdentityMap(agents);
    const toolPolicyRules = input.settings.agents[input.folder]?.accessPreset === 'locked'
        ? []
    ...
    return projectCallableAgentTools({
        ...
        ref: input.delegates.find((delegate) => String(identities.get(delegate)?.id) === entry.targetAgentId) ?? entry.targetAgentId,
