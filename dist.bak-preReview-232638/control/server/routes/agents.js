...
import { CreateAgentRequestSchema, PutAgentProfileFileRequestSchema, ReplaceAgentDelegatesRequestSchema, UpdateAgentRequestSchema, } from '@gantry/contracts';
...
import { AgentProfileService, MAX_PROFILE_CONTENT_BYTES, ProfileContentTooLargeError, ProfileVersionConflictError, isProfileFileKind, } from '../../../application/agents/agent-profile-service.js';
import { PROFILE_FILE_NAMES } from '../../../application/agents/prompt-profile-service.js';
import { logger } from '../../../infrastructure/logging/logger.js';
import { getRuntimeFileArtifactStore, getRuntimeStorage, } from '../../../adapters/storage/postgres/runtime-store.js';
...
import { agentIdentityMap, loadAgentDelegateSettings, resolveCallableDelegateRoster, } from './agent-delegate-route-helpers.js';
const PROFILE_JSON_BODY_MAX_BYTES = MAX_PROFILE_CONTENT_BYTES * 6 + 64 * 1024;
function buildAgentProfileService(actorAgentId, appId, runtimeHome) {
    ...
    return new AgentProfileService({
        ...
        storage.runtimeEvents
            ...
            .then(() => undefined)
            .catch((err) => {
            logger.warn({ err, agentId: actorAgentId, action: input.action }, 'Failed to publish profile audit event');
        }),
    });
}
...
export async function handleAgentRoutes(req, res, ctx, pathname) {
    if (pathname === '/v1/agents' && req.method === 'GET') {
        const auth = authorizeControlRequest(req, res, ctx.keys, ['agents:admin']);
        if (!auth)
            return true;
        const agents = await getRuntimeStorage().repositories.agents.listAgents(auth.appId);
        sendJson(res, 200, {
    ...
    if (agentAdminMatch && req.method === 'GET') {
        ...
        if (!auth)
            return true;
        const agentId = decodeURIComponent(agentAdminMatch[1]);
        ...
        try {
            ...
            const capabilitiesView = repositories.tools && repositories.skills && repositories.mcpServers
    ...
    if (delegatesMatch) {
        ...
        const agentId = decodeURIComponent(delegatesMatch[1]);
        const repositories = getRuntimeStorage().repositories;
        ...
        if (!folder) {
            sendError(res, 400, 'INVALID_REQUEST', 'Agent does not have a settings folder.');
            return true;
        }
        if (req.method === 'GET') {
            const { settings, revision } = await loadAgentDelegateSettings(ctx, appId);
        ...
        if (req.method === 'PUT') {
            if (orchestrator.status !== 'active') {
                sendError(res, 400, 'INVALID_REQUEST', 'Delegates can only be updated for active agents.');
                return true;
            }
            const parsed = ReplaceAgentDelegatesRequestSchema.safeParse(await readJson(req));
            if (!parsed.success) {
            ...
            for (const delegate of parsed.data.delegates) {
                ...
                if (!target) {
                    sendError(res, 400, 'INVALID_REQUEST', `Delegate ${delegate} is not a registered agent in this app.`);
                    return true;
                }
                if (target.id === orchestrator.id) {
                    sendError(res, 400, 'INVALID_REQUEST', 'An agent cannot delegate to itself.');
                    return true;
                }
                if (seen.has(String(target.id))) {
                    sendError(res, 400, 'INVALID_REQUEST', `Delegate ${delegate} resolves to a duplicate agent.`);
                    return true;
                }
                seen.add(String(target.id));
                canonicalDelegates.push(folderForAgentId(target.id) ?? String(target.id));
            }
            const { settings, revision } = await loadAgentDelegateSettings(ctx, appId);
            const configuredOrchestrator = settings.agents[folder];
            if (!configuredOrchestrator) {
                sendError(res, 409, 'SETTINGS_NOT_CONFIGURED', 'Agent is not present in desired settings. Seed desired state through /v1/settings/desired-state before updating delegates.');
                return true;
            }
    ...
    const profileListMatch = pathname.match(/^\/v1\/agents\/([^/]+)\/profile-files$/);
    if (profileListMatch && req.method === 'GET') {
        const auth = authorizeControlRequest(req, res, ctx.keys, ['agents:admin']);
        if (!auth)
            return true;
        const agentId = decodeURIComponent(profileListMatch[1]);
        const folder = await resolveProfileAgentFolder(res, auth.appId, agentId);
        if (!folder)
            return true;
        const files = await buildAgentProfileService(agentId, auth.appId, ctx.runtimeHome).listProfileFiles(folder);
        sendJson(res, 200, { agentId, files });
        return true;
    }
    const profileKindMatch = pathname.match(/^\/v1\/agents\/([^/]+)\/profile-files\/([^/]+)$/);
    if (profileKindMatch && req.method === 'GET') {
        const auth = authorizeControlRequest(req, res, ctx.keys, ['agents:admin']);
        if (!auth)
            return true;
        const agentId = decodeURIComponent(profileKindMatch[1]);
        ...
        if (!folder)
            return true;
        try {
            const file = await buildAgentProfileService(agentId, auth.appId, ctx.runtimeHome).readProfileFile(folder, kind, { actor: 'control' });
    ...
    if (profileKindMatch && req.method === 'PUT') {
        ...
        const parsed = PutAgentProfileFileRequestSchema.safeParse(await readJson(req, PROFILE_JSON_BODY_MAX_BYTES));
        if (!parsed.success) {
        ...
        if (!folder)
            return true;
        try {
            const result = await buildAgentProfileService(agentId, auth.appId, ctx.runtimeHome).writeProfileFile({
                agentFolder: folder,
                kind,
            ...
            if (error instanceof ProfileVersionConflictError) {
                sendError(res, 409, 'CONFLICT', `${error.message} Latest version is ${error.latestVersion}.`);
                return true;
            }
    ...
    if (agentMatch && req.method === 'GET') {
        ...
        if (!auth)
            return true;
        const agent = await getRuntimeStorage().repositories.agents.getAgent(decodeURIComponent(agentMatch[1]));
        if (!agent || agent.appId !== auth.appId) {
            sendError(res, 404, 'NOT_FOUND', 'Agent not found');
            return true;
        }
    ...
    if (patchMatch && req.method === 'PATCH') {
        ...
        const agent = await repository.getAgent(decodeURIComponent(patchMatch[1]));
        if (!agent || agent.appId !== auth.appId) {
            sendError(res, 404, 'NOT_FOUND', 'Agent not found');
            return true;
        }
        ...
        if (parsed.data.agentHarness && updated.status !== 'active') {
            sendError(res, 400, 'INVALID_REQUEST', 'agentHarness can only be updated for active agents.');
            return true;
        }
...
async function resolveProfileAgentFolder(res, appId, agentId) {
    const agent = await getRuntimeStorage().repositories.agents.getAgent(agentId);
    ...
    if (!folder || !isValidWorkspaceFolder(folder)) {
        sendError(res, 400, 'INVALID_REQUEST', 'Agent does not have profile files (no workspace folder).');
        return null;
    }
    return folder;
}
function agentToResponse(ctx, agent) {
...
async function agentBoundConversations(input) {
    const repositories = getRuntimeStorage().repositories;
    const bindings = await repositories.providerAccounts.listConversationInstalls(input.appId, input.agentId);
    const activeBindings = bindings
        .filter((binding) => binding.status === 'active')
        .sort((a, b) => String(a.conversationId).localeCompare(String(b.conversationId)));
    const summaries = await Promise.all(activeBindings.map(async (binding) => agentBoundConversation(input.appId, binding)));
    return summaries.filter((summary) => summary !== null);
}
async function agentBoundConversation(appId, binding) {
    const repositories = getRuntimeStorage().repositories;
    const conversation = await repositories.conversations.getConversation(binding.conversationId);
    if (!conversation || conversation.appId !== appId)
        return null;
    const providerConnection = await repositories.providerAccounts.getProviderAccount(conversation.providerAccountId);
    if (!providerConnection || providerConnection.appId !== appId)
        return null;
    const approvers = await repositories.conversations.listConversationApprovers(conversation.id);
    const route = binding.memorySubject &&
