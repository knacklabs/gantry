...
function service() {
    return new AgentCapabilityAdministrationService(getRuntimeStorage().repositories);
}
export async function handleCapabilityCatalogRoutes(req, res, ctx, pathname) {
    if (pathname === '/v1/capability-catalog') {
        sendError(res, 410, 'GONE', 'Capability catalog moved to /v1/inventory and /v1/capabilities.');
        return true;
    }
    if (pathname === '/v1/inventory' && req.method === 'GET') {
        ...
        if (!auth)
            return true;
        const catalog = await service().listCatalog(auth.appId);
        sendJson(res, 200, {
            inventory: {
    ...
    if (agentAccessMatch && req.method === 'GET') {
        ...
        try {
            const access = await service().getCapabilities({
                appId: auth.appId,
                agentId: decodeURIComponent(agentAccessMatch[1]),
            });
    ...
    if (agentAccessMatch && req.method === 'PUT') {
        ...
        const agentId = decodeURIComponent(agentAccessMatch[1]);
        try {
...
function toolToCapabilityResponse(tool) {
    ...
    if (tool.name === 'Browser') {
        return {
            ...
            cannot: 'Receive private browser backend credentials or persist per-action browser tools.',
            source: 'builtin',
...
function semanticCapabilityInput(tool) {
...
function mcpServerToResponse(server) {
    return {
        id: server.id,
        appId: server.appId,
        name: server.name,
...
function toSchemaDescriptor(value) {
    if (value &&
        typeof value === 'object' &&
        'format' in value &&
        'schema' in value) {
    ...
    return {
        format: 'unknown',
        schema: value && typeof value === 'object' && !Array.isArray(value) ? value : {},
    };
}
