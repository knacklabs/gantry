...
import { readJson, readRawBody, sendApplicationError, sendError, sendJson, } from '../http.js';
import { createExternalIngressModule, invokeExternalIngressForControl, } from '../external-ingress-adapter.js';
const MAX_INGRESS_BODY_BYTES = 256 * 1024;
export async function handleExternalIngressRoutes(req, res, ctx, pathname) {
    ...
    if (route.action === 'get' && req.method === 'GET') {
        ...
        if (!auth)
            return true;
        try {
            sendJson(res, 200, await createExternalIngressModule(ctx).get({
                appId: auth.appId,
    ...
    if (route.action === 'get' && req.method === 'PATCH') {
        ...
        if (Object.keys(patch).length === 0) {
            sendError(res, 400, 'INVALID_REQUEST', 'PATCH body must include name, enabled, or metadata');
            return true;
        }
        try {
            sendJson(res, 200, await createExternalIngressModule(ctx).update({
                appId: auth.appId,
                ingressId: route.ingressId,
                patch,
            }));
    ...
    if (route.action === 'rotate' && req.method === 'POST') {
        ...
        if (!auth)
            return true;
        try {
            sendJson(res, 200, await createExternalIngressModule(ctx).rotate({
                appId: auth.appId,
                ingressId: route.ingressId,
            }));
        }
...
async function readIngressRawBody(req, res) {
    try {
...
function parseIngressRoute(pathname) {
    ...
    const actionMatch = /^\/v1\/ingresses\/([^/]+)\/(rotate|invoke|wait)$/.exec(pathname);
    if (actionMatch) {
        return {
...
function readSignatureHeaders(req, res) {
    ...
    const missing = [
        ...
        .map(([name]) => name);
    if (missing.length > 0) {
        sendError(res, 400, 'INVALID_REQUEST', `Missing required ingress signature header: ${missing.join(', ')}`);
        return null;
    }
