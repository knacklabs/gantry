...
export function modelPreviewFor(input) {
    ...
    const workload = input.kind === 'recurring' ? 'recurring_job' : 'one_time_job';
    const defaultConfig = input.getDefaultModelConfig(modelKind, input.agentFolder);
...
export function resolveCreateJobModel(input) {
    const requested = resolveRequestedJobModel(input.modelAlias, input.kind === 'recurring' ? 'recurring_job' : 'one_time_job');
    if (requested) {
        return { modelAlias: requested, source: 'explicit', explicit: true };
    }
    const modelKind = input.kind === 'recurring' ? 'recurringJob' : 'oneTimeJob';
    const defaultConfig = input.getDefaultModelConfig(modelKind, input.agentFolder);
    const resolved = resolveModelSelectionForWorkload(defaultConfig.model, input.kind === 'recurring' ? 'recurring_job' : 'one_time_job');
    if (!resolved.ok) {
