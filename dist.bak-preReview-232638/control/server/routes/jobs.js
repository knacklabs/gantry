import { CreateJobRequestSchema, UpdateJobRequestSchema, } from '@gantry/contracts';
...
import { enqueueJobTrigger, isJobTriggerQueueReady, runtimeJobSchedulePlanner, requestSchedulerSync, schedulerNotReadyReason, } from '../../../jobs/scheduler.js';
...
function formatJobRequestIssue(issue) {
    if (issue.code === 'unrecognized_keys' && issue.keys.length > 0) {
        ...
        const oldToolField = issue.keys.find((key) => key === 'requiredTools' || key === 'required_tools');
        if (oldToolField) {
...
function parseCreateJobRequest(res, body) {
    const parsed = CreateJobRequestSchema.safeParse(body);
    if (parsed.success)
        return parsed.data;
    sendError(res, 400, 'INVALID_REQUEST', formatJobRequestIssue(parsed.error.issues[0]));
    return undefined;
}
function parseUpdateJobRequest(res, body) {
...
export function createJobManagementService(ctx) {
    ...
    return new JobManagementService({
        ...
        getCredentialBroker: ctx && typeof ctx.app.getCredentialBroker === 'function'
            ? () => ctx.app.getCredentialBroker()
            : undefined,
        getBrowserStatus: ctx?.getBrowserStatus,
    });
}
function adaptJobControl(control) {
    return {
...
export async function handleJobRoutes(req, res, ctx, url, pathname) {
    if (pathname === '/v1/jobs' && req.method === 'POST') {
        const auth = authorizeControlRequest(req, res, ctx.keys, ['jobs:write']);
        if (!auth)
            return true;
        const body = parseCreateJobRequest(res, (await readJson(req)));
        if (!body)
            return true;
        ...
        try {
            ...
            const executionContext = requestExecutionContextToInternal(body.executionContext);
            const created = await createJobManagementService(ctx).createJob({
                ...
                agentHarness: ctx.getSelectedAgentHarness(executionContext.workspaceKey),
                dryRun: body.dryRun,
            });
            ...
            const runtimePreviewNotificationRoutes = Array.isArray(body.notificationRoutes) &&
            ...
            sendJson(res, body.dryRun === true ? 200 : 201, {
                ...
                status: created.setupState && created.setupState.state !== 'ready'
                    ? 'paused'
                    : 'active',
                ...
                runtimeContext: await runtimeContextPreviewFor({
                    ...
                    groups: typeof ctx.app.getConversationRoutes === 'function'
    ...
    if (jobRoute && req.method === 'GET' && jobRoute.action === 'get') {
        ...
        try {
            ...
            sendJson(res, 200, mapManualJobToStored(job, await buildJobVisibilityMetadata({
                job,
                ops: getRuntimeRepositories(),
                toolRepository: getRuntimeToolRepositoryIfReady(),
                skillRepository: getRuntimeStorage().repositories.skills,
    ...
    if (jobRoute && req.method === 'PATCH' && jobRoute.action === 'get') {
        ...
        if (!auth)
            return true;
        const body = parseUpdateJobRequest(res, (await readJson(req)));
        if (!body)
            return true;
        try {
            const service = createJobManagementService(ctx);
            ...
            const patchExecutionContext = body.executionContext !== undefined
                ? requestExecutionContextToInternal(body.executionContext)
                : undefined;
            const { job: updated } = await service.updateJob({
                appId: auth.appId,
                jobId: jobRoute.jobId,
                agentHarness: ctx.getSelectedAgentHarness(patchExecutionContext?.workspaceKey ?? existingJob.workspace_key),
                patch: {
            ...
            sendJson(res, 200, mapManualJobToStored(updated, await buildJobVisibilityMetadata({
                job: updated,
                ops: getRuntimeRepositories(),
                toolRepository: getRuntimeToolRepositoryIfReady(),
                skillRepository: getRuntimeStorage().repositories.skills,
            ...
            sendJson(res, 200, {
                ...
                setup: result.job.setup_state
                    ? {
                        ...
                        nextAction: result.job.setup_state.blockers[0]?.nextAction ?? null,
                    }
                    : undefined,
            });
        }
    ...
    if (jobRoute?.action === 'trigger' && req.method === 'POST') {
        ...
        try {
            const result = await createJobManagementService(ctx).triggerJob({
                appId: auth.appId,
                jobId: jobRoute.jobId,
                consumeRateLimit: (key, limit) => ctx.triggerRateLimiter.consume(key, limit),
    ...
    if (triggerId && req.method === 'GET') {
        ...
        if (ctx.state.activeTriggerWaits >= ctx.maxConcurrentTriggerWaits) {
            sendError(res, 429, 'TOO_MANY_WAITS', 'Too many active trigger wait requests');
            return true;
        }
        ...
        try {
            ...
            ctx.state.activeTriggerWaits = Math.max(0, ctx.state.activeTriggerWaits - 1);
        }
    }
    return false;
}
