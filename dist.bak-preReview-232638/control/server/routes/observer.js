...
import { buildDigestPreview, digestPrefetchLimit, } from '../../../brain/observer-digest.js';
import { MessageInsightFreshnessProbe } from '../../../brain/observer-evidence-freshness.js';
import { resolveObserverDeliveryStatus, resolveObserverOwnerRoute, } from '../../../config/settings/observer-activation.js';
import { OBSERVER_INSIGHT_TYPES, OBSERVER_INSIGHT_STATES, isObserverSubjectKey, } from '../../../domain/ports/observer-insights.js';
...
function readLimit(res, url) {
    const raw = url.searchParams.get('limit');
    if (raw === null)
        return DEFAULT_LIMIT;
    const value = Number(raw);
    if (!Number.isInteger(value) || value < 1 || value > MAX_LIMIT) {
        sendError(res, 400, 'INVALID_REQUEST', `limit must be an integer between 1 and ${MAX_LIMIT}`);
        return null;
    }
    return value;
}
function readState(res, url) {
    const raw = url.searchParams.get('state');
    if (raw === null)
        return undefined;
    if (!OBSERVER_INSIGHT_STATES.includes(raw)) {
        sendError(res, 400, 'INVALID_REQUEST', 'state is invalid');
        return null;
    }
    return raw;
}
function readType(res, url) {
    const raw = url.searchParams.get('type');
    if (raw === null)
        return undefined;
    if (!OBSERVER_INSIGHT_TYPES.includes(raw)) {
        sendError(res, 400, 'INVALID_REQUEST', 'type is invalid');
        return null;
    }
    return raw;
}
function decodeCursor(res, raw) {
    if (raw === null)
        return undefined;
    let parsed;
    try {
...
export async function handleObserverRoutes(req, res, ctx, url, pathname) {
    if (!pathname.startsWith('/v1/observer'))
        return false;
    ...
    if (pathname === '/v1/observer/status') {
        const status = await ctx.resolveObserverStatus(appId);
    ...
    if (pathname === '/v1/observer/preview') {
        ...
        const freshnessProbe = new MessageInsightFreshnessProbe(getRuntimeStorage().ops);
    ...
    if (pathname === '/v1/observer/deliveries') {
        ...
        const owner = resolveObserverOwnerRoute(ctx.getInternalRuntimeSettings());
        if (!owner.ok) {
