...
function projectRunForControlApi(run) {
    const { provider_session_id: _providerSessionId, provider_run_id: _providerRunId, providerSessionId: _providerSessionIdCamel, providerRunId: _providerRunIdCamel, ...safeRun } = run;
    return safeRun;
}
export async function handleRunRoutes(req, res, ctx, url, pathname) {
    ...
    if (runEventsId && req.method === 'GET') {
        ...
        sendJson(res, 200, {
            events: events.map((event) => projectRuntimeEventToRunEvent(event, run.run_id)),
        });
        return true;
    }
