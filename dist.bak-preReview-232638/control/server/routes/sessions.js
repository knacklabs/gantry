...
import { readJson, sendApplicationError as sendApplicationErrorResponse, sendError, sendJson, } from '../http.js';
import { parseSessionRoute } from '../route-parser.js';
import { acceptMessageForControl, createSessionInteractionModule, ensureSessionForControl, } from '../session-interaction-adapter.js';
import { listSessionPendingInteractions, respondToSessionPermissionInteraction, SESSION_INTERACTION_DECISIONS, } from '../session-interaction-approvals.js';
import { nowMs as currentTimeMs } from '../../../shared/time/datetime.js';
function sendApplicationError(res, error) {
    const notFoundCode = error instanceof Error && error.message === 'Webhook not found'
        ? 'WEBHOOK_NOT_FOUND'
...
function formatSessionRequestIssue(issue) {
...
function responseSchemaCompileFailure(schema) {
    try {
        const validate = responseSchemaCompiler.compile(schema);
...
const SESSION_EFFORTS = ['low', 'medium', 'high', 'xhigh', 'max'];
function parseSessionThinking(value) {
    ...
    const record = value;
    const unsupported = Object.keys(record).find((key) => key !== 'mode' && key !== 'budget_tokens');
    if (unsupported)
        return `thinking.${unsupported} is not supported`;
    ...
    if (record.budget_tokens !== undefined &&
    ...
    return record.budget_tokens === undefined
        ? { mode: record.mode }
        : { mode: 'on', budgetTokens: record.budget_tokens };
}
function parseSessionAgentControls(body) {
    const controls = {};
    if (body.effort !== undefined) {
        if (!SESSION_EFFORTS.includes(body.effort)) {
            return `effort must be one of ${SESSION_EFFORTS.join(', ')}`;
        }
        controls.effort = body.effort;
    }
...
export async function handleSessionRoutes(req, res, ctx, url, pathname) {
    if (pathname === '/v1/sessions/ensure' && req.method === 'POST') {
        ...
        if (!parsed.success) {
            sendError(res, 400, 'INVALID_REQUEST', formatSessionRequestIssue(parsed.error.issues[0]));
            return true;
        }
        ...
        if (assertedAppId && !isValidControlId(assertedAppId)) {
            sendError(res, 400, 'INVALID_REQUEST', 'appId and conversationId must contain only letters, numbers, dot, underscore, or dash');
            return true;
        }
        if (!appId) {
    ...
    if (sessionRoute?.action === 'interaction-respond' && req.method === 'POST') {
        ...
        if (!SESSION_INTERACTION_DECISIONS.includes(decision)) {
            sendError(res, 400, 'INVALID_REQUEST', `decision must be one of ${SESSION_INTERACTION_DECISIONS.join(', ')}`);
            return true;
        }
        try {
            const session = await createSessionInteractionModule().requireSession({
    ...
    if (sessionRoute?.action === 'messages' && req.method === 'POST') {
        ...
        if (body.response_schema !== undefined &&
            ...
            sendError(res, 400, 'INVALID_REQUEST', 'response_schema must be a JSON Schema object');
            return true;
        }
        if (body.response_schema !== undefined) {
            const compileFailure = responseSchemaCompileFailure(body.response_schema);
            if (compileFailure) {
        ...
        try {
            const accepted = await acceptMessageForControl(ctx, {
                ...
                senderName: typeof body.senderName === 'string' ? body.senderName : 'SDK',
                ...
                agentControls: Object.keys(agentControls).length > 0 ? agentControls : undefined,
            });
            sendJson(res, 202, {
                accepted: true,
    ...
    if (sessionRoute?.action === 'events' && req.method === 'GET') {
        ...
        if (req.headers.accept?.includes('text/event-stream')) {
            ...
            let subscription;
            const cleanup = () => {
                if (closed && !streamActive && !subscription)
                    return;
                closed = true;
                subscription?.close();
                if (streamActive) {
            ...
            const pump = async () => {
                while (!closed) {
                    try {
                        ...
                        if (closed)
                            return;
                        logger.warn({ err: error, sessionId: sessionRoute.sessionId }, 'Failed streaming runtime events');
                        await delay(1000);
                    }
                }
            };
    ...
    if (sessionRoute?.action === 'wait' && req.method === 'GET') {
        ...
        const afterEventId = Number(url.searchParams.get('afterEventId') || 0);
        const timeoutMs = Math.min(300_000, Math.max(1000, Number(url.searchParams.get('timeoutMs') || 60_000)));
        const startedAt = currentTimeMs();
        try {
            const visible = await createSessionInteractionModule().waitForVisibleEvent({
                appId: auth.appId,
                sessionId: sessionRoute.sessionId,
                afterEventId,
...
function sendSessionInteractionRespondOutcome(res, outcome) {
    switch (outcome.status) {
        case 'resolved':
            sendJson(res, 200, outcome);
            return;
        case 'not_found':
            sendError(res, 404, 'INTERACTION_NOT_FOUND', 'No pending interaction with this id exists for this session.');
            return;
        case 'already_resolved':
            sendError(res, 409, 'INTERACTION_ALREADY_RESOLVED', 'This interaction was already decided.');
            return;
        case 'question_unsupported':
            sendError(res, 409, 'INTERACTION_KIND_UNSUPPORTED', 'Question interactions cannot be answered through this API yet; only permission interactions are supported.');
            return;
        case 'batch_unsupported':
            sendError(res, 409, 'INTERACTION_BATCH_UNSUPPORTED', 'This interaction is part of a batched channel prompt; decide it from the channel that rendered it.');
            return;
        case 'option_unavailable':
            sendError(res, 409, 'DECISION_UNAVAILABLE', `This decision is not available for this interaction. Available: ${outcome.options.join(', ') || 'none'}.`);
            return;
        case 'malformed':
            sendError(res, 409, 'INTERACTION_MALFORMED', 'The pending interaction record is missing its permission request snapshot.');
            return;
        default:
            sendError(res, 503, 'INTERACTION_RETRYABLE', 'Could not record the decision. Please retry.');
    }
}
async function writeSseEvent(res, event, isClosed = () => false) {
...
function sanitizeSseEventType(eventType) {
    return /^[a-z0-9._-]+$/.test(eventType) ? eventType : 'runtime_event';
}
function serializeSessionEventEnvelope(event) {
    return {
