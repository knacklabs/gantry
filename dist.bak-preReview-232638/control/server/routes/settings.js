...
function omitObservability(document) {
    const { observability: _observability, ...rest } = document;
    return rest;
}
export async function handleSettingsRoutes(req, res, ctx, pathname) {
    ...
    if (req.method === 'GET') {
        ...
        sendJson(res, 200, {
            settings: omitObservability(ctx.getRuntimeSettings()),
        });
        return true;
    }
    if (req.method === 'PATCH') {
        ...
        sendError(res, 409, 'SETTINGS_READ_ONLY', 'The typed settings API is read-only. Use CLI settings commands or the reviewed settings_desired_state/request_settings_update admin tools to change settings.');
        return true;
    }
 ...
 */
async function handleDesiredState(req, res, ctx, write) {
    if (req.method === 'GET') {
        const key = authorizeControlRequest(req, res, ctx.keys, ['agents:admin']);
        if (!key)
            return true;
        const appId = key.appId;
        const latest = await getRuntimeStorage().repositories.settingsRevisions.getLatestSettingsRevision(appId);
        if (!latest) {
        ...
        sendJson(res, 200, {
            ...
            settings: omitObservability(latest.settingsDocument),
    ...
    if (req.method === 'PUT' || req.method === 'POST') {
        ...
        if (!key)
            return true;
        const appId = key.appId;
        const body = (write ? write.body : await readJson(req));
        if (typeof body.settings !== 'object' ||
            body.settings === null ||
            Array.isArray(body.settings)) {
            sendError(res, 400, 'INVALID_REQUEST', 'settings is required and must be a settings document object.');
            return true;
        }
        if (body.expectedRevision !== undefined &&
            body.expectedRevision !== null &&
            !Number.isInteger(body.expectedRevision)) {
            sendError(res, 400, 'INVALID_REQUEST', 'expectedRevision must be an integer when provided.');
            return true;
        }
        const callerGuard = typeof body.expectedRevision === 'number' ? body.expectedRevision : null;
        ...
        for (let attempt = 0;; attempt += 1) {
            const head = await storage.repositories.settingsRevisions.getLatestSettingsRevision(appId);
            const preservedObservability = head?.settingsDocument
                ?.observability ??
                (head
                    ? undefined
                    : ctx.settingsImport.serializeRevisionDocument(ctx.getInternalRuntimeSettings()).observability);
            const inboundDocument = {
                ...omitObservability(body.settings),
            ...
            try {
                ...
                sendError(res, 400, 'INVALID_SETTINGS', err instanceof Error
                    ? err.message
            ...
            if (isWorkstation) {
                ...
                try {
                    const outcome = await ctx.settingsImport.importWorkstation({
                        ...
                        appId,
                        previousSettings: ctx.getInternalRuntimeSettings(),
                    ...
                    revision =
                        outcome.status === 'revision_created'
                            ? outcome.revision
                            : ((await storage.repositories.settingsRevisions.getLatestSettingsRevision(appId))?.revision ?? 0);
                }
                catch (err) {
                    ...
                    // as a CAS conflict.
                    if (importError?.kind === 'stale' &&
                        callerGuard === null &&
                        !lastAttempt) {
                        continue;
                    }
                    if (importError?.kind === 'conflict') {
                        if (callerGuard === null && !lastAttempt)
                            continue;
                        sendError(res, 409, 'REVISION_CONFLICT', `expectedRevision ${importError.expectedRevision} does not match the current revision ${importError.actualRevision}.`, {
            ...
            if (outcome.status === 'invalid') {
                sendError(res, 400, 'INVALID_SETTINGS', 'Settings document failed validation.', { errors: outcome.errors });
                return true;
            }
            if (outcome.status === 'conflict') {
                if (callerGuard === null && !lastAttempt)
                    continue;
                sendError(res, 409, 'REVISION_CONFLICT', `expectedRevision ${outcome.expectedRevision} does not match the current revision ${outcome.actualRevision}.`, {
...
export function writeControlDesiredState(input) {
    return handleDesiredState({ method: 'PUT' }, input.res, input.ctx, { key: input.key, body: input.body });
}
function currentDeploymentMode(ctx) {
    return ctx.getRuntimeSettings().runtime.deploymentMode === 'workstation'
        ? 'workstation'
        : 'fleet';
}
async function handleRevisionsList(req, res, ctx) {
    if (req.method !== 'GET') {
    ...
    if (!key)
        return true;
    const appId = key.appId;
    const revisions = await getRuntimeStorage().repositories.settingsRevisions.listRecentSettingsRevisions({ appId, limit: 50 });
    sendJson(res, 200, {
