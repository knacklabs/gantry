...
import { evaluateReadiness, renderMetrics, } from '../system-health.js';
...
async function runtimeQuery(sql) {
...
export async function handleSystemRoutes(req, res, ctx, pathname) {
    ...
    if (pathname === '/metrics' && req.method === 'GET') {
        const deps = {
            ...
            currentWorkerInstanceId: () => ctx.currentWorkerInstanceId?.() ?? null,
            liveCapacityLimit: () => ctx.liveCapacityLimit?.() ?? 0,
            jobCapacityLimit: () => getRuntimeSettingsForConfig().runtime.queue.maxJobRuns,
            oldestWaitingLiveAdmissionSeconds: () => ctx.oldestWaitingLiveAdmissionSeconds?.() ?? 0,
        };
    ...
    if (pathname === '/v1/status' && req.method === 'GET') {
        ...
        if (!key)
            return true;
        const model = await buildControlPlaneReadModelForRequest(ctx, key.appId);
