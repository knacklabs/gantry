...
function parseWebhookSubscriptionPatch(body, current) {
    const patch = {};
    if (hasOwn(body, 'eventTypes')) {
        if (body.eventTypes === null) {
            patch.eventTypes = null;
        }
        else if (!Array.isArray(body.eventTypes) ||
    ...
    for (const key of ['agentId', 'sessionId', 'jobId']) {
        if (!hasOwn(body, key))
            continue;
        if (body[key] === null) {
            patch[key] = null;
...
function hasWebhookSubscriptionPatch(body) {
    return WEBHOOK_SUBSCRIPTION_FIELDS.some((field) => hasOwn(body, field));
}
export async function handleWebhookRoutes(req, res, ctx, pathname) {
    ...
    if (webhookRoute?.action === 'delete' && req.method === 'PATCH') {
        ...
        if (hasWebhookSubscriptionPatch(body)) {
            const existing = await getRuntimeControlRepository().getWebhookById(webhookRoute.webhookId, auth.appId);
            if (!existing) {
        ...
        const webhook = await getRuntimeControlRepository().updateWebhook(webhookRoute.webhookId, auth.appId, patch);
        if (!webhook) {
            sendError(res, 404, 'WEBHOOK_NOT_FOUND', 'Webhook not found');
            return true;
        }
    ...
    if (webhookRoute?.action === 'delete' && req.method === 'DELETE') {
        ...
        if (!auth)
            return true;
        await getRuntimeControlRepository().deleteWebhook(webhookRoute.webhookId, auth.appId);
    ...
    if (webhookRoute?.action === 'replay-dead-letter' && req.method === 'POST') {
        ...
        const replayed = await getRuntimeControlRepository().replayWebhookDeadLetters(webhookRoute.webhookId, auth.appId);
    ...
    if (webhookRoute?.action === 'purge-dead-letter' && req.method === 'POST') {
        ...
        const purged = await getRuntimeControlRepository().purgeWebhookDeadLetters(webhookRoute.webhookId, auth.appId);
    ...
    if (webhookRoute?.action === 'test' && req.method === 'POST') {
        ...
        const webhook = await control.getWebhookById(webhookRoute.webhookId, auth.appId);
        if (!webhook) {
            sendError(res, 404, 'WEBHOOK_NOT_FOUND', 'Webhook not found');
            return true;
        }
