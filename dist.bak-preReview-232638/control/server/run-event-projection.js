...
import { formatDuration, formatRunLabel, formatRunShortId, } from '../../shared/human-format.js';
export function projectRuntimeEventToRunEvent(event, fallbackRunId) {
    const runId = event.runId ?? fallbackRunId;
    if (!runId) {
        throw new Error(`Runtime event ${event.eventId} cannot be projected to a run event without runId`);
    }
    return {
...
function enrichRunEventPayload(payload, runId, eventCreatedAt) {
    ...
    const { agent_engine: _agentEngine, agentEngine: _camelAgentEngine, ...publicSource } = source;
    ...
    const executionProviderId = stringValue(source.execution_provider_id ?? source.executionProviderId);
    return {
        ...publicSource,
        ...
        duration_text: durationMs === undefined ? undefined : formatDuration(durationMs),
...
function publicRunEventTypeFor(eventType) {
    switch (eventType) {
