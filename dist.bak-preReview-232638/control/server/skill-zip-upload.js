...
export function parseSkillZipUpload(input) {
...
function readCentralDirectory(zip) {
    ...
    if (centralOffset + centralSize > zip.byteLength ||
        centralOffset < 0 ||
        centralSize < 0) {
...
function normalizeZipPath(value) {
    ...
    if (parts.some((part) => ['.DS_Store', '.git', '__MACOSX', 'node_modules'].includes(part))) {
        throw new Error(`Unsupported skill zip path: ${value}`);
    }
    return parts.join('/');
}
function resolveSkillRoot(paths) {
...
function stripRoot(entryPath, prefix) {
    if (!prefix)
        return entryPath;
...
function contentTypeForPath(entryPath) {
