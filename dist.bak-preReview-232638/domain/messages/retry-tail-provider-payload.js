...
const SECRET_LIKE_WARNING_TEXT = /\b(token|secret|authorization|bearer|api[_-]?key|password)\b|sk-[a-z0-9_-]{8,}|xox[a-z]-[a-z0-9-]{8,}/i;
export function sanitizeRetryTailProviderPayload(payload) {
...
function readWarningCodeArray(value, options) {
    const entries = readStringArray(value, options);
    return entries.filter((entry) => SAFE_WARNING_CODE.test(entry) && !SECRET_LIKE_WARNING_TEXT.test(entry));
}
function readInt(value) {
    if (!Number.isSafeInteger(value))
        return undefined;
    const num = value;
    if (num < 0 || num > MAX_PART_COUNT)
        return undefined;
    return num;
}
