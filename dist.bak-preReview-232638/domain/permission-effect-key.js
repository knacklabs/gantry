...
export function computePermissionEffectHash(input) {
    const { request } = input;
 ...
 */
function canonicalEffectJson(request, toolInput, cwd) {
    if (SHELL_TOOLS.has(request.toolName)) {
        const command = commandText(toolInput);
        if (command === undefined)
            return undefined; // no command ⇒ uncacheable
