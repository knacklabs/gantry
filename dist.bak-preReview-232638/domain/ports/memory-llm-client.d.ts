...
export type MemoryLlmBatchState = 'pending' | 'completed' | 'failed' | 'expired' | 'cancelled';
...
export interface MemoryLlmBatchCapability {
    preflightBatch(opts: Omit<MemoryLlmBatchSubmitOpts, 'onSubmissionStart'>): Promise<void>;
    submitBatch(opts: MemoryLlmBatchSubmitOpts): Promise<{
        batchId: string;
    }>;
    pollBatch(opts: MemoryLlmBatchScope & {
        batchId: string;
    }): Promise<MemoryLlmBatchPoll>;
    fetchBatchResults(opts: MemoryLlmBatchScope & {
        batchId: string;
    }): Promise<MemoryLlmBatchResultRow[]>;
    findBatchByCorrelationId(opts: MemoryLlmBatchScope & {
        correlationId: string;
    }): Promise<{
        batchId: string;
    } | null>;
