import type { Agent, AgentConfigVersion, AgentConfigVersionId, AgentId } from '../agent/agent.js';
import type { App, AppId } from '../app/app.js';
import type { CapabilitySecret, CapabilitySecretMetadata } from '../capability-secrets/capability-secrets.js';
import type { ModelCredential, ModelCredentialFieldFingerprint, ModelCredentialMetadata, ModelCredentialProvider } from '../model-credentials/model-credentials.js';
import type { ModelCredentialPayload } from '../../shared/model-provider-registry.js';
import type { ConversationInstall, ConversationApprover, ProviderAccount, ProviderAccountId, ProviderId } from '../provider/provider.js';
import type { Conversation, ConversationId, ConversationThread, ConversationThreadId, ExternalConversationId, UserId } from '../conversation/conversation.js';
import type { AgentRun, AgentRunId, RuntimeEvent, RuntimeEventFilter, RuntimeEventPublishInput, UsageAggregate, UsageQuery } from '../events/events.js';
import type { Message, MessageId } from '../messages/messages.js';
import type { ClaimedOutboundDeliveryItem, OutboundDelivery, OutboundDeliveryFinalAnswer, OutboundDeliveryId, OutboundDeliveryItem, OutboundDeliveryItemId, OutboundDeliveryReceipt, OutboundDeliveryReceiptId, OutboundDeliveryResolvedDestination } from '../outbound-delivery/outbound-delivery.js';
import type { AgentMcpServerBinding, MaterializedMcpServer, McpServerAuditEvent, McpServerDefinition, McpServerId } from '../mcp/mcp-servers.js';
import type { PermissionDecision, PermissionDecisionId, PermissionPolicy, PermissionRule } from '../permissions/permissions.js';
import type { SandboxLease, SandboxLeaseId, SandboxProfile, SandboxProfileId, WorkspaceSnapshot, WorkspaceSnapshotId } from '../sandbox/sandbox.js';
import type { AgentSessionDigest, AgentSessionDigestScopeMetadata, AgentSessionDigestId, AgentSession, AgentSessionId, AgentSessionSummary, AgentSessionSummaryId, ProviderSession, ProviderSessionId, ExecutionProviderId } from '../sessions/sessions.js';
import type { AgentSkillBinding, SkillCatalogItem, SkillId } from '../skills/skills.js';
import type { AgentToolBinding, AgentToolSource, ToolCatalogItem, ToolId } from '../tools/tools.js';
...
export interface AgentConfigRepository {
    getConfigVersion(id: AgentConfigVersionId): Promise<AgentConfigVersion | null>;
...
export interface ProviderAccountRepository {
    ...
    listConversationInstalls(appId: AppId, agentId?: AgentId): Promise<ConversationInstall[]>;
...
export interface ConversationRepository {
    ...
    listParticipantExternalUserIds(conversationId: ConversationId): Promise<string[]>;
    listConversationApprovers(conversationId: ConversationId): Promise<ConversationApprover[]>;
    listConversationApproversForConversations(conversationIds: readonly ConversationId[]): Promise<ConversationApprover[]>;
...
export interface ProviderSessionRepository {
    ...
    markProviderSessionStatus(id: ProviderSessionId, status: ProviderSession['status'], updatedAt: string): Promise<void>;
}
export interface AgentSessionSummaryRepository {
    getAgentSessionSummary(id: AgentSessionSummaryId): Promise<AgentSessionSummary | null>;
    getLatestAgentSessionSummary(agentSessionId: AgentSessionId): Promise<AgentSessionSummary | null>;
...
export interface AgentSessionDigestRepository {
    getAgentSessionDigest(id: AgentSessionDigestId): Promise<AgentSessionDigest | null>;
...
export interface SandboxRepository {
    ...
    getWorkspaceSnapshot(id: WorkspaceSnapshotId): Promise<WorkspaceSnapshot | null>;
}
