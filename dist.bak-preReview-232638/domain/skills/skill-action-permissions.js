import { RUN_COMMAND_TOOL_NAME, validateReadableAgentToolRule, } from '../../shared/agent-tool-references.js';
import { normalizeBashLeafRuleContent, parseBashCommand, } from '../../shared/bash-command-parser.js';
import { isValidSemanticCapabilityId, semanticCapabilityIdValidationReason, } from '../../shared/semantic-capability-ids.js';
import { parseDeclaredNetworkHost } from '../../shared/network-host-declaration.js';
import { assertValidCapabilitySecretName, normalizeCapabilitySecretName, } from '../capability-secrets/capability-secrets.js';
export const SKILL_ACTION_MANIFEST_FILE = 'gantry.skill.json';
export function sanitizeSkillDirectoryName(value) {
    const safe = value
        .trim()
...
export function parseSkillActionPermissionsFromAssets(input) {
    const manifest = input.assets.find((asset) => asset.path === SKILL_ACTION_MANIFEST_FILE);
    if (!manifest)
        return [];
    let parsed;
    try {
    ...
    if (parsed.actions.length > 20) {
        throw new Error(`${SKILL_ACTION_MANIFEST_FILE} supports at most 20 actions.`);
    }
    ...
    return parsed.actions.map((entry, index) => {
        ...
        if (seenCapabilities.has(action.capabilityId)) {
            throw new Error(`${SKILL_ACTION_MANIFEST_FILE} duplicates capabilityId ${action.capabilityId}.`);
        }
...
export function skillActionSemanticCapability(input) {
    ...
    return {
        ...
        networkHosts: input.action.networkHosts?.length
            ...
            : undefined,
        redactionPolicy: input.action.requiredEnvVars.length > 0
            ? { env: input.action.requiredEnvVars }
            : undefined,
        source,
    };
}
export function skillActionSource(capability) {
    const source = capability.source;
    if (!isRecord(source) || source.kind !== 'skill_action')
        return undefined;
    const skillId = typeof source.skillId === 'string' ? source.skillId : '';
    const skillName = typeof source.skillName === 'string' ? source.skillName : '';
    const actionId = typeof source.actionId === 'string' ? source.actionId : '';
    if (!skillId || !skillName || !actionId)
        return undefined;
    return {
        kind: 'skill_action',
        skillId,
...
export function skillActionSemanticCapabilitiesForSkills(skills) {
    const definitions = {};
...
function parseSkillActionPermission(raw, context) {
    if (!isRecord(raw)) {
        throw new Error(`Skill action at index ${context.index} must be an object.`);
    }
    const id = stringField(raw, 'id');
    if (!/^[a-z][a-z0-9_-]{0,79}$/.test(id)) {
        throw new Error(`Skill action ${context.index} id must use lowercase letters, numbers, dashes, or underscores.`);
    }
    const capabilityId = stringField(raw, 'capabilityId');
    if (!isValidSemanticCapabilityId(capabilityId) ||
        !capabilityId.startsWith('skill.')) {
        throw new Error(semanticCapabilityIdValidationReason(capabilityId) ??
    ...
    if (risk !== 'read' && risk !== 'write' && risk !== 'admin') {
        throw new Error(`Skill action ${capabilityId} risk must be read, write, or admin.`);
    }
    const can = stringField(raw, 'can');
    const cannot = stringField(raw, 'cannot');
    const commandTemplates = stringArray(raw.commandTemplates, 'commandTemplates').map((template) => normalizeSkillActionCommandTemplate(template, context.skillName));
    ...
    for (const envVar of requiredEnvVars)
        assertValidCapabilitySecretName(envVar);
 ...
 */
function normalizeSkillActionNetworkHost(value, capabilityId) {
    const result = parseDeclaredNetworkHost(value);
    if (!result.ok) {
        throw new Error(`Skill action ${capabilityId} networkHosts ${result.reason}`);
    }
    return result.host;
}
function normalizeSkillActionCommandTemplate(value, skillName) {
    ...
    if (hasEnvAssignmentToken(withSkillRoot)) {
        throw new Error('Skill action command templates cannot include shell environment assignments.');
    }
    if (hasSecretLikeCommandPart(withSkillRoot)) {
        throw new Error('Skill action command templates cannot include secret-like command parts.');
    }
    ...
    if (parsed.leaves.length !== 1) {
        throw new Error('Skill action command templates must contain exactly one command leaf.');
    }
    ...
    const stableNormalized = normalized.startsWith('./')
        ? normalized.slice(2)
        : normalized;
    if (stableNormalized !== skillDir &&
        !stableNormalized.startsWith(`${skillDir}/`)) {
        throw new Error(`Skill action command template must run under ${skillDir}.`);
    }
    ...
    if (destructiveRedirect) {
        throw new Error('Skill action command templates cannot include destructive redirection.');
    }
    const readableRule = `${RUN_COMMAND_TOOL_NAME}(${stableNormalized})`;
    const readable = validateReadableAgentToolRule(readableRule);
    if (!readable.ok) {
        throw new Error(`Invalid skill action command template: ${readable.reason}`);
    }
...
function stringArray(raw, field, options = {}) {
    ...
    return raw.map((entry) => {
        if (typeof entry !== 'string' || !entry.trim()) {
            throw new Error(`Skill action ${field} entries must be non-empty strings.`);
        }
        return entry.trim();
    });
}
function hasEnvAssignmentToken(command) {
    return command
        .split(/\s+/)
        .some((part) => /^[A-Za-z_][A-Za-z0-9_]*=/.test(part));
}
function hasSecretLikeCommandPart(command) {
    const parts = command.split(/\s+/);
    return parts.some((part) => /(?:^|[-_])(token|secret|password|api[_-]?key|credential)(?:$|[-_=])/i.test(part));
}
