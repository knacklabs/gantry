...
export type { Job, JobAccessRequirement, JobAccessRequirementTarget, JobCapabilityRequirement, JobCapabilityRequirementImplementation, JobCapabilityRequirementImplementationKind, JobEvent, JobExecutionContext, JobNotificationRoute, JobRecoveryIntent, JobRecoveryIntentKind, JobRecoveryIntentState, JobRun, JobRunStatus, JobScheduleType, JobSetupBlocker, JobSetupReadinessState, JobSetupState, JobStatus, } from './job-types.js';
...
export type InteractionSeverity = 'info' | 'success' | 'warning' | 'error' | 'critical';
...
export interface InteractionResult {
    status: 'pending' | 'approved' | 'denied' | 'expired' | 'failed' | 'completed';
    message?: string;
    decidedBy?: string;
    decidedAt?: string;
}
export type RichInteractionKind = 'status' | 'facts' | 'list' | 'table' | 'form' | 'media' | 'progress';
export declare const RICH_INTERACTION_NATIVE_FALLBACK_TEXT = "Rich view unavailable in this conversation. Showing text version.";
...
export type MessageActionAffordanceKind = 'scheduler_run_now' | 'scheduler_pause_job' | 'live_turn_stop';
...
export type OnMessageAction = (input: MessageActionCallbackInput) => Promise<void>;
...
export type MessageDeliveryStatus = 'pending' | 'sent' | 'failed' | 'partially_sent';
...
export type OnInboundMessage = (conversationJid: string, message: NewMessage) => Promise<void>;
export type OnChatMetadata = (conversationJid: string, timestamp: string, name?: string, channel?: string, isGroup?: boolean, options?: {
    providerAccountId?: string;
}) => Promise<void>;
...
export interface MessageSink {
    name: string;
    sendMessage(jid: string, text: string, options?: MessageSendOptions): Promise<void | MessageDeliveryResult>;
}
...
export interface InteractionSurface {
    ...
    cancelPendingQuestion?(request: UserQuestionCancellation): Promise<'settled' | 'already_decided' | 'retryable' | 'not_found'>;
}
...
export interface PlanReviewSurface {
    requestPlanReview(jid: string, request: PlanReviewRequest): Promise<PlanReviewResponse>;
}
