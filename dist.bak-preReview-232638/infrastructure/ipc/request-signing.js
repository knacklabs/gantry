...
export function verifyIpcRequestPayload(requestSigningKey, payload, signature) {
    const key = requestSigningKey?.trim();
...
export function validateIpcRequestFreshness(payload, nowMs = currentTimeMs(), maxAgeMs = IPC_REQUEST_MAX_AGE_MS) {
    ...
    const expiresAtRaw = typeof payload.authExpiresAt === 'string'
