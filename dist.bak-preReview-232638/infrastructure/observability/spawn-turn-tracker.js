...
import { childContextFor, contentCaptureEnabled, getTurnSpan, startTurnSpan, takeDelegationToolSpan, tracingEnabled, TRACE_CONTENT_MAX_CHARS, } from './tracing.js';
...
// multiple turns.
export function createSpawnTurnTracker(agentName, input, onOutput) {
    ...
    const delegationParent = input.parentRunId
        ...
        : undefined;
    const openTurnSpan = (continuation) => {
        const parentSpan = (!continuation ? delegationParent : undefined) ??
    ...
    let closed = false;
    const tracedOnOutput = onOutput && tracingEnabled()
        ? async (frame) => {
            ...
            let continuationTraceId;
            if (frame.continuedByFollowup && !closed) {
                if (streamedTurnOutput)
                    turnSpan.setOutput(streamedTurnOutput);
    ...
    return {
        ...
        finish: (output) => {
            closed = true;
            const turnOutput = output?.status === 'success'
                ? streamedTurnOutput || output.result || ''
                : '';
            if (turnOutput)
                turnSpan.setOutput(turnOutput);
            turnSpan.end(output?.status === 'success'
                ? 'success'
