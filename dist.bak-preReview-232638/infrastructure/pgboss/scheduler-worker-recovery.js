...
export async function recoverExpiredWorkerLeases(input) {
    ...
    try {
        ...
        if (unhealthy.length > 0) {
            logger.warn({ workerInstanceIds: unhealthy }, 'Marked heartbeat-lapsed worker instances unhealthy');
        }
        ...
        const releasedSlots = (await coordination.releaseRunSlotsForStaleWorkers?.({
            staleBefore,
        })) ?? 0;
        if (releasedSlots > 0) {
            logger.warn({ count: releasedSlots, staleBefore }, 'Released run slots held by lapsed worker instances');
        }
    }
    catch (err) {
