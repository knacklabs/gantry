...
export const DEFAULT_ASYNC_RESOURCE_LIMITS = {
    cpuSeconds: 300,
    memoryMb: 1024,
    maxProcesses: 64,
};
export async function runSandboxedAsyncCommand(provider, input) {
    if (input.signal.aborted)
        throw new Error('Command aborted.');
    const configFilePath = input.launchControl
        ? path.join(input.launchControl.directory, 'sandbox-runtime.json')
        : path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'gantry-async-command-')), 'sandbox-runtime.json');
    ...
    const completion = new Promise((resolve, reject) => {
        const settle = (fn) => {
            ...
            if (outputSnapshotTimer)
                clearTimeout(outputSnapshotTimer);
        ...
        const emitOutputSnapshot = () => {
            ...
            Promise.resolve(input.onOutputSnapshot?.({ stdoutTail: stdout, stderrTail: stderr }))
                .catch(() => undefined)
                .finally(() => {
                outputSnapshotInFlight = false;
                if (outputSnapshotPending && !settled)
                    scheduleOutputSnapshot();
            });
        };
        const scheduleOutputSnapshot = () => {
            if (!input.onOutputSnapshot)
                return;
            outputSnapshotPending = true;
            if (outputSnapshotInFlight || outputSnapshotTimer)
                return;
        ...
        child.on('close', (code, signal) => {
            ...
            settle(() => reject(new Error(`Command failed${signal ? ` with signal ${signal}` : ` with exit code ${code}`}${stderr.trim() ? `: ${stderr.trim()}` : ''}`)));
        });
    });
    if (child.pid) {
        try {
...
export function buildAsyncCommandEnv() {
    const env = {};
...
function asyncCommandLaunchScript() {
    return [
        'set -eu',
...
async function waitForLaunchReady(readyFile) {
