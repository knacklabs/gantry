...
export function evaluateAsyncCommandStartPolicy(input) {
    ...
    const protectedDenial = evaluateProtectedCapabilityToolUse(SHELL_POLICY_TOOL_NAME, policyInput);
    if (protectedDenial) {
        return {
            ok: false,
    ...
    const memoryDenial = denyMemoryBoundaryToolUse(SHELL_POLICY_TOOL_NAME, policyInput, {}, input.memoryBlock ?? '');
    if (memoryDenial)
        return { ok: false, message: memoryDenial };
    const request = buildAgentToolExecutionRequest(input.classifier, SHELL_POLICY_TOOL_NAME, policyInput, {
    ...
    if (decision.status !== 'allow') {
        return {
            ok: false,
            message: 'This command is not approved for this agent. Request access or choose an approved capability.',
        };
    }
