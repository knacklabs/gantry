...
import { commandSummary, errorMessage, isTimeoutError, taskInScope, truncate, } from './async-command-task-helpers.js';
...
import { activeChildCount, childTaskResult, } from './async-delegated-agent-task-results.js';
...
class DelegatedChildFailureError extends Error {
    ...
    failure;
    terminalChildren;
    constructor(subtasks, failure, terminalChildren) {
...
export async function sendDelegatedAgentTaskMessage(input) {
    ...
    if (!message)
        return { ok: false, message: 'task_message requires message.' };
...
async function appendSteeringMessage(repository, initialTask, entry) {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
        ...
        if (!latest || isAsyncTaskTerminal(latest.status))
            return null;
        task = latest;
    }
...
export async function executeDelegatedAgentTask(input) {
    const { task, taskInput, controller, repository, active, cancelLinkedChildTasks, waitForTaskChange, transitionTask, } = input;
    ...
    try {
        ...
        if (result.failure) {
            throw Object.assign(new Error(result.errorSummary ?? 'Delegated agent run failed.'), { failure: result.failure });
        }
        await transitionPrivateCorrelation(repository, task, {
            update: (privateCorrelationJson) => ({
                ...privateCorrelationJson,
        ...
        const cancelledChildren = err instanceof DelegatedChildFailureError
            ? 0
        ...
        await finishDelegatedAgentTask(repository, transitionTask, task, {
            ...
            subtasks: err instanceof DelegatedChildFailureError
                ? err.subtasks
                : cancelledChildren > 0
            ...
            failure: err instanceof DelegatedChildFailureError
                ? err.failure
                : failureMetadata(err, task, { aborted, timedOut }),
            terminalChildren: err instanceof DelegatedChildFailureError
...
async function finishDelegatedAgentTask(repository, transitionTask, task, input) {
    let latest = (await repository.getTask(task.id)) ?? task;
    for (let attempt = 0; attempt < 3; attempt += 1) {
        if (isAsyncTaskTerminal(latest.status))
            return;
        const now = nowIso();
...
async function waitForLinkedChildTasks(repository, parent, input) {
    while (!input.signal.aborted) {
        const counts = await linkedChildTaskCounts(repository, parent);
        if (activeChildCount(counts) === 0) {
            const terminalChildren = await linkedTerminalChildTasks(repository, parent);
...
async function linkedTerminalChildTasks(repository, parent) {
...
async function waitForChildTaskWakeup(parent, input) {
...
async function linkedChildTaskCounts(repository, parent) {
    return repository.countTasksByStatus({
        appId: parent.appId,
        parentTaskId: parent.id,
    });
}
function failureMetadata(error, task, state) {
...
async function transitionPrivateCorrelation(repository, initialTask, input) {
...
async function updateSteering(repository, task, steeringId, patch) {
    let latest = task;
    for (let attempt = 0; attempt < 3; attempt += 1) {
        const updated = await repository.transitionTask({
            taskId: latest.id,
            ...
            privateCorrelationJson: {
                ...latest.privateCorrelationJson,
                steering: steeringMessages(latest.privateCorrelationJson).map((entry) => (entry.id === steeringId ? { ...entry, ...patch } : entry)),
            },
        });
        if (updated)
            return;
        const reloaded = await repository.getTask(latest.id);
        if (!reloaded || isAsyncTaskTerminal(reloaded.status))
            return;
        latest = reloaded;
    }
...
function steeringMessages(value) {
    return Array.isArray(value.steering)
        ? value.steering.filter((entry) => Boolean(entry && typeof entry === 'object' && !Array.isArray(entry)))
        : [];
}
