import { skillCapabilityId, toolchainCapabilityId, } from './worker-capability-reconciler.js';
/**
...
export const EMPTY_REQUIRED_CAPABILITIES = [];
/**
 ...
 */
export async function resolveRequiredCapabilities(deps, input) {
    ...
    if (deps.runtimeDependencies) {
        ...
        for (const row of rows) {
            if (row.requestedByAgentId !== input.agentId)
                continue;
...
export function normalizeCapabilitySet(capabilities) {
 ...
 */
export function isWorkerEligibleForRequiredCapabilities(required, advertised) {
    if (!required || required.length === 0)
        return true;
    const advertisedSet = new Set(advertised ?? []);
    for (const capabilityId of required) {
        if (!advertisedSet.has(capabilityId))
            return false;
    }
    return true;
}
 ...
 */
export function missingRequiredCapabilities(required, advertised) {
    if (!required || required.length === 0)
        return [];
    const advertisedSet = new Set(advertised ?? []);
