...
import { isWorkerEligibleForRequiredCapabilities, missingRequiredCapabilities, } from './capability-eligibility.js';
/**
    ...
    async alert(signal) {
        ...
        try {
            await this.deps.publishRuntimeEvent({
                ...
                actor: 'scheduler',
                jobId: (signal.jobId ?? undefined),
                runId: (signal.runId ?? undefined),
                payload: {
            ...
            this.deps.warn?.({ err, cause: signal.cause, key: signal.key }, 'Failed to publish capability-starvation alert');
            return false;
        }
    }
...
function starvationRemediation(signal) {
    const missing = signal.missingCapabilities.length > 0
 ...
 */
export function fleetCanSatisfyRequiredCapabilities(required, activeWorkerCapabilities) {
    if (required.length === 0)
        return true;
    return activeWorkerCapabilities.some((advertised) => isWorkerEligibleForRequiredCapabilities(required, advertised));
}
/**
 ...
 */
export function fleetMissingRequiredCapabilities(required, activeWorkerCapabilities) {
    if (required.length === 0)
        return [];
