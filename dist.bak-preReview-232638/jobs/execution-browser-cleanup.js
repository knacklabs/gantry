...
import { isBrowserProfileSyncEnabled, snapshotBrowserProfile, } from '../runtime/browser-profile-sync.js';
import { toolAccessRequirementsIncludeBrowser, } from './execution-diagnostics.js';
export async function closeBrowserAfterJobRun(input) {
    if (!input.executionGroupFolder || !input.executionJid)
        return;
    const usedBrowser = input.diagnostics.browserActivityCount > 0 ||
        toolAccessRequirementsIncludeBrowser(splitAccessRequirements(input.currentJob.access_requirements)
    ...
    try {
        ...
        if (input.diagnostics.browserActivityCount > 0 &&
        ...
        input.logger.warn({ err, jobId: input.currentJob.id, profileName }, 'Failed to close scheduled job browser profile after run');
        await input.emitJobEvent(RUNTIME_EVENT_TYPES.JOB_TOOL_ACTIVITY, {
            phase: 'browser_cleanup',
            tool: 'Browser',
            profile_name: profileName,
