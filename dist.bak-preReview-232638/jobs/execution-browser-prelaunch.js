...
import { toolAccessRequirementsIncludeBrowser, updateDiagnosticsFromRuntimeEvent, } from './execution-diagnostics.js';
export async function prelaunchBrowserForJobRun(input) {
    if (!input.executionGroupFolder || !input.executionJid)
        return null;
    if (!toolAccessRequirementsIncludeBrowser(splitAccessRequirements(input.currentJob.access_requirements)
        .toolAccessRequirements))
        return null;
    if (!input.deps.openBrowserSession)
        return null;
    ...
    try {
        ...
        updateDiagnosticsFromRuntimeEvent(input.diagnostics, RUNTIME_EVENT_TYPES.JOB_TOOL_ACTIVITY, payload);
        ...
        input.logger.warn({ err, jobId: input.currentJob.id, profileName }, 'Failed to prelaunch scheduled job browser profile');
        const payload = {
            phase: 'browser_prelaunch',
            tool: 'Browser',
            public_tool: 'browser_open',
