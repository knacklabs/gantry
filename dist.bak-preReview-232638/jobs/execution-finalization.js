import { SETUP_REQUIRED_PAUSE_REASON, setupStateForDeniedTool, setupStateForTransientPermission, } from '../application/jobs/job-readiness-service.js';
...
import { formatTerminalDiagnostics, } from './execution-diagnostics.js';
...
export async function finalizeSchedulerJobRun(input) {
    const { currentJob, deps, diagnostics, pausedForSetupDuringRun, runtimeAppId, appSession, } = input;
    const nextRunOnSuccess = computeNextJobRun(currentJob, input.scheduledFor);
    let runStatus = 'completed';
    let nextRun = nextRunOnSuccess;
    ...
    const transientPermissionApproval = diagnostics.transientPermissionApprovals[0] ?? null;
    ...
    const beforeJobStateUpdate = async () => {
        if (beforeJobStateUpdateCalled)
            return;
    ...
    if (input.deletedDuringRun) {
        ...
        if (!pausedForSetupDuringRun && toolDenial) {
            ...
            await notifyJobSetupRequired({
                ...
                appSession,
                setupState,
                source: isPermissionTimeout(safePrimaryErrorSummary, diagnostics.terminalToolDenial?.reason)
                    ? 'permission_timeout'
                    : 'permission_denied',
                runId: input.runId,
                publishRuntimeEvent: input.publishRuntimeEvent,
            ...
            const exceededConsecutive = retryCount >= currentJob.max_consecutive_failures;
    ...
    else if (transientPermissionApproval &&
        currentJob.schedule_type !== 'manual') {
        nextRun = null;
        pauseReason = SETUP_REQUIRED_PAUSE_REASON;
        ...
        await updateJob({
            status: currentJob.schedule_type === 'manual' || nextRunOnSuccess
                ? 'active'
                : 'completed',
...
export function retryBackoffMs(job, retryCount) {
...
function isPermissionTimeout(errorSummary, denialReason) {
    return /timed out waiting|permission approval.*timed out|approval timeout/i.test([errorSummary, denialReason].filter(Boolean).join(' '));
}
