import { evaluateJobReadiness, SETUP_REQUIRED_PAUSE_REASON, } from '../application/jobs/job-readiness-service.js';
...
import { getRuntimeStorage, getWorkerCoordinationRepository, } from '../adapters/storage/postgres/runtime-store.js';
import { evaluateFleetCapabilityReadiness, fleetCapabilitySetupState, } from './capability-readiness.js';
export async function pauseJobForSetupIfNeeded(input) {
    const appId = input.appSession?.appId ?? input.runtimeAppId;
    const agentId = input.agentId ?? agentIdForJobWorkspaceKey(input.executionAgentFolder);
