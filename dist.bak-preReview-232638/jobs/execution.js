...
import { resolveTurnSemanticCapabilities, resolveTurnSelectedMcpServerIds, resolveTurnSelectedSkillContext, } from '../runtime/group-run-context.js';
// prettier-ignore
...
import { buildExecutionTurnContextInput, resolveExecutionContext, resolveExecutionMemoryContext, } from './execution-context.js';
import { logMemoryDreamJobFailure, notifySchedulerTerminalRunState, } from './execution-notifications.js';
import { claimSchedulerRunLease, createSchedulerRunLeaseAbort, startSchedulerRunLeaseHeartbeat, } from './execution-lease.js';
...
import { jobCompletedModelPayload, jobStartedModelPayload, modelUseKindForJobSchedule, resolveJobExecutionProviderId, resolveJobModel, } from './model-resolution.js';
// prettier-ignore
...
import { bindSchedulerRunEventState, createRuntimeEventPublisher as createEventPublisher, createSchedulerJobEventEmitter, publishSchedulerCompletionEvent, } from './execution-runtime-events.js';
...
import { recordJobAgentRunFailure, requireTerminalSettlement, } from './execution-operational-errors.js';
export async function runJob(job, deps, queueJid, dispatch, control) {
    return runActiveJobWithLogContext({
        requestedJob: job,
        dispatch,
        getJobById: (jobId) => deps.opsRepository.getJobById(jobId),
        run: ({ job: currentJob, scheduledFor, runId }) => runActiveJob(currentJob, deps, queueJid, dispatch, control, scheduledFor, runId),
    });
}
async function runActiveJob(currentJob, deps, queueJid, dispatch, control, scheduledFor, runId) {
    ...
    const warn = (context, message) => logger.warn(context, message);
    ...
    const resolvedModel = resolveJobModel({ ...currentJob, model: jobModelForResolution || currentJob.model }, getEffectiveModelConfig(undefined, jobModelUseKind, execution.group.folder), agentHarness);
    const eventControl = getRuntimeControlRepository();
    const preflightAppSession = await resolveAppSessionForJob(currentJob, eventControl);
    ...
    const leaseContext = await claimSchedulerRunLease({
        ...
        startedAt,
        leaseExpiresAt,
        requireNextRun: currentJob.schedule_type !== 'manual' && !dispatch?.triggerId,
    ...
    let terminalRunRecorded = false, deletedDuringRun = false;
    try {
        ...
        const emitJobEvent = createSchedulerJobEventEmitter({
            ...
            runtimeAppId,
            state: eventState,
            resolveEventAppSession: () => resolveAppSessionForJob(currentJob, eventControl),
            publishRuntimeEvent,
            deletionGuard,
            logger,
        });
        ...
        if (!error && isTrustedSystemJob(currentJob)) {
            ...
            if (!error) {
                let turnContext;
                const failRun = () => failSessionRun(deps.opsRepository, agentRunId, error);
                ...
                const streamingFlusher = createStreamingEventFlusher({
                    nowMs,
                    emit: (payload) => emitJobEvent(RUNTIME_EVENT_TYPES.JOB_STREAMING, payload),
                });
                try {
                    turnContext = await deps.opsRepository.getAgentTurnContext?.(buildExecutionTurnContextInput({
                    ...
                    const executionAppId = turnContext?.appId ??
                        eventState.eventAppSession?.appId ??
                        runtimeAppId;
                    const executionAgentId = turnContext?.agentId ??
                        jobToolPolicy.agentIdForJobWorkspaceKey(execution.group.folder);
                    updateLogContext({
                        appId: executionAppId,
                        agentId: executionAgentId,
                    });
                    const [toolPolicy, selectedSkillContext, semanticCapabilities, credentialBroker,] = await Promise.all([
                    ...
                    const attachedMcpSourceIds = await resolveTurnSelectedMcpServerIds(deps, {
                        appId: executionAppId,
                        agentId: executionAgentId,
                    });
                    const toolAccessRequirementPreflight = await assertToolAccessRequirementsReadyForRun({
                        toolAccessRequirements: splitAccessRequirements(currentJob.access_requirements).toolAccessRequirements,
                    ...
                    if (!error) {
                        ...
                        const output = await runJobAgentWithFailover({
                            ...
                            baseInput: {
                                ...
                                toolAccessRequirements: toolAccessRequirementPreflight.toolAccessRequirements,
                            ...
                            onProcess: (proc, runHandle) => {
                                ...
                                deps.onProcess(queueJid, proc, runHandle, execution.group.folder, execution.stopAliasJids);
                            },
                            streamHandler: async (streamedOutput) => {
                                if (runLeaseAbort.isAborted())
                                    return;
                                for (const event of streamedOutput.runtimeEvents ?? []) {
                                    const eventKey = runnerRuntimeEventKey(event);
                                    if (eventKey)
                                        streamedRuntimeEventKeys.add(eventKey);
                                }
                                ...
                                if (streamedOutput.usage)
                                    accumulatedUsage = accumulateModelUsage(accumulatedUsage, streamedOutput.usage);
                                const streamedProviderSessionId = providerSessionExternalSessionId(streamedOutput);
                        ...
                        if (runLeaseAbort.isAborted()) {
                            ...
                            await forwardRunnerRuntimeEvents({
                                events: filterUnforwardedRunnerRuntimeEvents(output.runtimeEvents, streamedRuntimeEventKeys),
                                diagnostics,
                                emitJobEvent,
                            });
        ...
        const { runStatus, nextRun, retryCount, pauseReason, safeErrorSummary, toolDenial, } = await finalizeSchedulerJobRun({
            currentJob,
            deps,
            scheduledFor,
            now,
            ...
            updateJobState: async (jobUpdates, state) => {
                ...
                await requireTerminalSettlement(finalizeWithLease?.call(deps.opsRepository, {
                    jobId: currentJob.id,
                    runId,
                    leaseToken: leaseContext.lease.leaseToken,
                    workerInstanceId: leaseContext.lease.workerInstanceId,
                    fencingVersion: leaseContext.lease.fencingVersion,
                    leaseOutcome: state.runStatus === 'paused'
                        ? 'released'
                        : error
                            ? 'failed'
                            : 'completed',
                    ...
                    errorSummary: state.safeErrorSummary
                        ...
                        : null,
                    jobUpdates,
                }), 'Scheduler run lease finalization is unavailable for terminal job write.', 'Scheduler run lease is no longer active during terminal finalization.');
        ...
        if (!terminalRunRecorded && !deletionGuard.deletedDuringRun) {
            ...
            await requireTerminalSettlement(finalizeRunLease?.call(deps.opsRepository, {
                runId,
                leaseToken: leaseContext.lease.leaseToken,
                workerInstanceId: leaseContext.lease.workerInstanceId,
                fencingVersion: leaseContext.lease.fencingVersion,
                leaseOutcome: runStatus === 'paused'
                    ? 'released'
                    : error
                        ? 'failed'
                        : 'completed',
                ...
                errorSummary: safeErrorSummary
                    ...
                    : null,
            }), 'Scheduler run lease finalization is unavailable for terminal run write.', 'Scheduler run lease is no longer active during terminal finalization.');
            terminalRunRecorded = true;
        }
        if (runLeaseAbort.isAborted())
        ...
        if (toolDenial)
            await emitJobEvent(RUNTIME_EVENT_TYPES.JOB_TOOL_DENIED, toolDenialEventPayload(toolDenial, safeErrorSummary));
        ...
        const notified = !(await deletionGuard.shouldSuppressDelivery()) &&
        ...
        if (notified) {
            ...
            await requireTerminalSettlement(markJobRunNotified?.call(deps.opsRepository, runId, {
                leaseToken: leaseContext.lease.leaseToken,
                workerInstanceId: leaseContext.lease.workerInstanceId,
                fencingVersion: leaseContext.lease.fencingVersion,
            }), 'Scheduler run lease notification finalization is unavailable.', 'Scheduler run lease is no longer valid during notification finalization.');
        }
        await emitJobEvent(runStatus === 'completed'
