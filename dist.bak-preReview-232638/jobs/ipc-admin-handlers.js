...
import { getRuntimeRepositories, getRuntimeStorage, } from '../adapters/storage/postgres/runtime-store.js';
import { GANTRY_HOME, getRuntimeSettingsForConfig, syncRuntimeSettingsFromProjection, } from '../config/index.js';
...
import { isPermanentPermissionDecision, formatDurableAccessRulesForUser, pendingAccessTargetSummary, persistRequestPermissionRules, requestPermissionDescription, requestPermissionOnceLiveRules, requestPermissionQueuedMessage, requestPermissionReviewEffect, requestPermissionReviewSuggestions, requestPermissionSetupDecisionOptions, resolveTrustedSemanticCapabilityDefinitions, validateRequestPermissionCapabilityProposal, validateRequestPermissionSemanticCapability, } from './request-permission-review.js';
import { guidedActionPreviewHandler, requestSettingsUpdateHandler, serviceRestartHandler, settingsDesiredStateHandler, } from './ipc-runtime-admin-handlers.js';
import { adminCapabilityRequiredMessage, sourceAgentHasAdminToolCapability, } from './ipc-admin-authorization.js';
import { BROWSER_ACTION_MCP_RULE_REJECTION_REASON, BROWSER_PROJECTED_MCP_RULE_REJECTION_REASON, isBrowserActionMcpToolRule, isProjectedBrowserMcpToolRule, } from '../shared/agent-tool-references.js';
...
import { configureSkillInstallHandlers, requestSkillInstallHandler, requestSkillProposalHandler, } from './ipc-skill-install-handlers.js';
import { configurePatternCandidateIpcHandlers, patternCandidateDecisionHandler, } from './pattern-candidate-ipc-handlers.js';
import { configureProactiveSurfacingConsentIpcHandlers, proactiveSurfacingConsentHandler, } from './proactive-surfacing-consent-ipc-handlers.js';
import { credentialRefsForRequestedMcp, headerNameForCredentialNeed, } from './ipc-mcp-server-request-credentials.js';
...
import { appendLiveToolRules, readLiveToolRules, } from '../shared/live-tool-rules.js';
import { formatRequestAccessPersistentGrantMessage, recheckPausedSetupJobsAfterRequestAccessGrant, } from './request-access-job-recovery.js';
import { requestOnlyCapabilityPendingKey } from './request-only-capability-dedupe.js';
const pendingRequestOnlyCapabilityReviews = new Set();
const { asyncMcpCallToolHandler, mcpCallToolHandler, mcpDescribeToolHandler, mcpListToolsHandler, mcpSearchToolsHandler, } = createMcpToolHandlers(createMcpProxyForSourceGroup);
...
const refreshGroupsHandler = async (context) => {
    const { sourceAgentFolder, deps, conversationBindings } = context;
    const { accept, reject } = createContextTaskResponder(context);
    try {
        logger.info({ sourceAgentFolder }, 'Group metadata refresh requested via IPC');
        await deps.syncGroups(true);
        const availableGroups = await deps.getAvailableGroups();
        await deps.writeGroupsSnapshot(sourceAgentFolder, availableGroups, new Set(Object.keys(conversationBindings)));
        accept('Group metadata refresh completed.');
    }
    catch (err) {
        logger.error({ err, sourceAgentFolder }, 'refresh_groups failed unexpectedly');
        reject('I could not refresh the conversation list. Explain this in plain language and say you can try again after the sync issue is fixed.', 'internal_error');
    }
};
// prettier-ignore
const registerAgentHandler = async (context) => {
    const { data, sourceAgentFolder, deps, sourceAgentFolderJids } = context;
    const { accept, reject } = createContextTaskResponder(context);
    if (!data.appId) {
        reject('Agent registration requires signed app scope.', 'forbidden');
        return;
    }
    if (!(await sourceAgentHasAdminToolCapability(context, 'register_agent'))) {
        logger.warn({ sourceAgentFolder }, 'Unauthorized register_agent attempt blocked');
        reject(adminCapabilityRequiredMessage('register_agent'), 'missing_capability');
        return;
    }
    if (!data.jid || !data.name || !data.folder || !data.trigger) {
        logger.warn({ data }, 'Invalid register_agent request - missing required fields');
        reject('Missing required fields: jid, name, folder, trigger.', 'invalid_request');
        return;
    }
    ...
    if (!requestedTargetJid)
        return;
    if (data.jid !== requestedTargetJid) {
        reject('Agent registration can only bind the originating conversation.', 'forbidden');
        return;
    }
    if (typeof deps.requestPermissionApproval !== 'function' || typeof deps.sendMessage !== 'function') {
        reject('Agent registration requests require a configured approval surface.', 'preflight_failed');
        return;
    }
    if (!isValidWorkspaceFolder(data.folder)) {
        logger.warn({ sourceAgentFolder, folder: data.folder }, 'Invalid register_agent request - unsafe folder name');
    ...
    if (!decision.approved || !decision.decidedBy) {
        const message = `Rejected agent registration: ${decision.reason || 'not approved'}.`;
    ...
    await syncApprovedCapabilitySettings(data.appId);
    accept(`Agent "${data.name}" registered.`);
};
const requestMcpServerHandler = async (context) => {
    ...
    if (typeof deps.requestPermissionApproval !== 'function' ||
        ...
        reject('MCP server requests require a configured approval surface.', 'preflight_failed');
        return;
    }
    if (transport !== 'stdio_template') {
        reject('request_mcp_server supports only stdio_template servers until Gantry has a DNS-pinned remote MCP transport.', 'invalid_request');
        return;
    }
    if (!templateId || !sandboxProfileId) {
        reject('stdio_template MCP server requests require templateId and sandboxProfileId.', 'invalid_request');
        return;
    }
    ...
    const requestedToolPatterns = Array.isArray(payload.requestedToolPatterns)
        ? payload.requestedToolPatterns.filter((item) => Boolean(toTrimmedString(item, { maxLen: 160 })))
        : [];
    const credentialNeeds = Array.isArray(payload.credentialNeeds)
        ? payload.credentialNeeds.filter((item) => {
    ...
    if (Array.isArray(payload.networkHosts)) {
        for (const item of payload.networkHosts) {
            ...
            if (!networkHosts.includes(result.host))
                networkHosts.push(result.host);
        }
    }
    ...
    try {
        ...
        const credentialRefs = credentialRefsForRequestedMcp(name, transport, credentialNeeds);
        ...
        logger.error({ err, sourceAgentFolder }, 'MCP server request failed unexpectedly');
        reject('The MCP server request could not be completed. Explain this in plain language and say you can try again after the setup issue is fixed.', 'invalid_request');
    }
};
// prettier-ignore
const requestOnlyCapabilitySpecs = {
...
// prettier-ignore
const requestOnlyCapabilityHandler = async (context) => {
    const { data, deps, sourceAgentFolder, sourceAgentFolderJids } = context;
    const { accept, reject } = createContextTaskResponder(context);
    const toolName = data.type;
    ...
    if (!data.appId) {
        reject(`${parsed.review.requestKind} requests require signed app scope.`, 'forbidden');
        return;
    }
    ...
    if (!requestedTargetJid)
        return;
    if (typeof deps.requestPermissionApproval !== 'function' || typeof deps.sendMessage !== 'function') {
        reject(`${parsed.review.requestKind} requests require a configured approval surface.`, 'preflight_failed');
        return;
    }
    const jobConflict = await jobLocalCliCapabilityConflict({ deps, jobId: data.jobId, review: parsed.review });
    if (jobConflict) {
        reject(jobConflict, 'wrong_capability_lane');
        return;
    }
    const catalogConflict = await missingReviewedCapabilityCatalogEntry({ deps, appId: data.appId, agentId: memoryAgentIdForWorkspaceFolder(sourceAgentFolder), review: parsed.review });
    if (catalogConflict) {
        reject(catalogConflict, 'invalid_request');
        return;
    }
    ...
    if (pendingRequestOnlyCapabilityReviews.has(pendingKey)) {
        accept(`${parsed.review.displayName} request is already waiting for approval in this chat.`, 'capability_request_already_pending');
        return;
    }
    pendingRequestOnlyCapabilityReviews.add(pendingKey);
    startRequestOnlyCapabilityReview({ deps, appId: data.appId, agentId: memoryAgentIdForWorkspaceFolder(sourceAgentFolder), sourceAgentFolder, targetJid: requestedTargetJid, threadId: data.authThreadId, ipcDir: context.ipcBaseDir ? path.join(context.ipcBaseDir, sourceAgentFolder) : undefined, runHandle: data.runHandle, jobId: data.jobId, review: parsed.review, pendingKey });
...
async function missingReviewedCapabilityCatalogEntry(input) {
    ...
    if (skillRepository) {
        ...
        if (skillCapabilities[capabilityId])
            return undefined;
    }
...
// prettier-ignore
const adminPermissionRevokeHandler = async (context) => {
    const { data, deps, sourceAgentFolder, sourceAgentFolderJids } = context;
    const { acceptData, reject } = createContextTaskResponder(context);
    if (!data.appId) {
        reject('Permission revoke requires signed app scope.', 'forbidden');
        return;
    }
    if (!(await sourceAgentHasAdminToolCapability(context, 'admin_permission_revoke'))) {
        logger.warn({ sourceAgentFolder }, 'Unauthorized admin_permission_revoke attempt blocked');
        reject(adminCapabilityRequiredMessage('admin_permission_revoke'), 'missing_capability');
        return;
    }
    ...
    const reason = toTrimmedString(payload.reason, { maxLen: 2000 });
    if (!toolName && !toolId) {
        reject('admin_permission_revoke requires tool_name or tool_id.', 'invalid_request');
        return;
    }
    if (!reason) {
        reject('admin_permission_revoke requires reason.', 'invalid_request');
        return;
    }
    ...
    if (!toolRepository || !mirrorAgentToolRulesToSettings) {
        reject('Permission revoke requires tool repository and settings mirror.', 'preflight_failed');
        return;
    }
    try {
...
function validateSameChannelApprovalTarget(input) {
    ...
    if (!requestedTargetJid || !input.sourceAgentFolderJids.includes(requestedTargetJid)) {
...
// prettier-ignore
function parseRequestOnlyCapabilityReview(toolName, payload) {
    const spec = requestOnlyCapabilitySpecs[toolName];
    const reason = toTrimmedString(payload.reason, { maxLen: 2000 }) || '';
    const missing = spec.required.filter((field) => !payloadHasValue(payload[field]));
    if (spec.any && !spec.any.some((field) => payloadHasValue(payload[field])))
        missing.push(spec.any.join(' or '));
    if (!reason)
        missing.push('reason');
    if (missing.length > 0)
        return { ok: false, error: `Missing required fields: ${missing.join(', ')}.` };
    if (toolName === 'request_permission' && isTemporaryBrowserPermissionRequest(payload))
        return { ok: false, error: 'Browser cannot be approved as temporary current-run access through request_permission. Request persistent Browser access with temporaryOnly=false, then use the projected browser_* tools.' };
    if (toolName === 'request_permission' && isBrowserPermissionRequest(payload, isBrowserActionMcpToolRule))
        return { ok: false, error: BROWSER_ACTION_MCP_RULE_REJECTION_REASON };
    if (toolName === 'request_permission' && isBrowserPermissionRequest(payload, isProjectedBrowserMcpToolRule))
        return { ok: false, error: BROWSER_PROJECTED_MCP_RULE_REJECTION_REASON };
    if (toolName === 'request_permission' && hasAgentSuppliedCapabilityDefinition(payload))
        return { ok: false, error: 'Capability definitions are host-owned catalog metadata and cannot be supplied in request_permission input.' };
    ...
    if (toolName === 'request_skill_dependency_install' && !['npm', 'brew', 'go', 'uv', 'download'].includes(String(toolInput.ecosystem)))
        return { ok: false, error: 'ecosystem must be npm, brew, go, uv, or download.' };
    if (toolName === 'request_permission') {
        ...
        if (capabilityProposalError)
            return { ok: false, error: capabilityProposalError };
...
// prettier-ignore
function requestOnlyCapabilityEffect(toolName, toolInput, fallback) {
    return toolName === 'request_permission' ? requestPermissionReviewEffect(toolInput, fallback) : fallback;
}
// prettier-ignore
function requestOnlyCapabilityQueuedMessage(review) {
...
// prettier-ignore
function payloadHasValue(value) { return Array.isArray(value) ? value.some((item) => Boolean(toTrimmedString(item, { maxLen: 300 }))) : Boolean(toTrimmedString(value, { maxLen: 512 })); }
// prettier-ignore
function sanitizeCapabilityPayload(payload) {
    const output = {};
    for (const [key, value] of Object.entries(payload)) {
        if (key === 'reason')
            continue;
        if (key === 'semanticCapabilityDefinition' || key === 'capabilityDefinition')
            continue;
...
// prettier-ignore
function isTemporaryBrowserPermissionRequest(payload) {
    if (payload.temporaryOnly !== true)
        return false;
    if (payload.permissionKind && payload.permissionKind !== 'tool')
        return false;
...
// prettier-ignore
function isBrowserPermissionRequest(payload, predicate) {
    if (payload.permissionKind && payload.permissionKind !== 'tool')
        return false;
    return sanitizedStringList([payload.toolName, ...(Array.isArray(payload.toolNames) ? payload.toolNames : [])]).some(predicate);
}
// prettier-ignore
function capabilityDisplayValue(payload, spec) {
    const primary = payload[spec.display];
    if (Array.isArray(primary)) {
        const joined = primary.map((item) => toTrimmedString(item, { maxLen: 512 })).filter((item) => Boolean(item)).join(', ');
        if (joined)
            return joined;
    }
    ...
    for (const field of spec.any || []) {
        ...
        if (Array.isArray(value)) {
            const joined = value.map((item) => toTrimmedString(item, { maxLen: 512 })).filter((item) => Boolean(item)).join(', ');
            if (joined)
                return joined;
            continue;
        }
...
// prettier-ignore
function startRequestOnlyCapabilityReview(input) {
    void (async () => {
        let message;
        ...
        try {
            ...
            const semanticCapabilityDefinitions = await resolveTrustedSemanticCapabilityDefinitions({
                deps: input.deps,
                appId: input.appId,
                agentId: input.agentId,
            });
            const decision = await input.deps.requestPermissionApproval({
                ...
                ...(input.review.toolName === 'request_permission'
                    ? {
                        suggestions: requestPermissionReviewSuggestions(input.review.toolInput, { semanticCapabilityDefinitions }),
                        decisionOptions: requestPermissionSetupDecisionOptions(input.review.toolInput, { semanticCapabilityDefinitions }),
                    }
                    : {}),
            });
            ...
            if (transientRules.length > 0)
                liveRules = appendLiveToolRules({ ipcDir: input.ipcDir, runHandle: input.runHandle, rules: transientRules });
            try {
            ...
            const bakeMessage = decision.approved && decision.decidedBy
            ...
            logger.error({ err, sourceAgentFolder: input.sourceAgentFolder, toolName: input.review.toolName }, 'Capability permission review failed');
            message =
        ...
        .finally(() => {
        if (input.pendingKey)
            pendingRequestOnlyCapabilityReviews.delete(input.pendingKey);
    });
}
/**
...
async function maybeEnqueueDependencyBakeOnApproval(input) {
    if (input.review.toolName !== 'request_skill_dependency_install')
        return null;
    ...
    if (ecosystem !== 'npm')
        return null;
    const packages = sanitizedStringList(Array.isArray(input.review.toolInput.packages)
    ...
    try {
        ...
        logger.warn({ err, appId: input.appId }, 'Failed to enqueue approved toolchain bake');
...
function hasAgentSuppliedCapabilityDefinition(payload) {
    return (Object.prototype.hasOwnProperty.call(payload, 'semanticCapabilityDefinition') || Object.prototype.hasOwnProperty.call(payload, 'capabilityDefinition'));
}
// prettier-ignore
function startMcpPermissionReview(input) {
    void completeMcpPermissionReview(input).catch((err) => {
        logger.error({ err, serverName: input.server.name, sourceAgentFolder: input.sourceAgentFolder }, 'MCP source review failed');
        input.responder.reject('The MCP server request could not be completed. Explain this in plain language and say you can try again after the setup issue is fixed.', 'permission_review_failed');
    });
}
async function completeMcpPermissionReview(input) {
    const decision = await input.deps.requestPermissionApproval({
        ...
        description: 'Only configured approvers can decide this request. Approval records this service as an agent source through the Gantry MCP proxy; durable action authority still requires an agent allowed capability selected from a reviewed definition.',
    ...
    await input.deps.sendMessage(input.targetJid, `Connected MCP source ${input.server.name}. Review a capability before using durable MCP actions.`, mcpReviewMessageOptions(input));
    input.responder.acceptData(`Connected MCP source ${input.server.name}. Review a capability before using durable MCP actions.`, sameSessionContext, 'mcp_connected');
}
async function rejectMcpRequestFromPermission(input, reason) {
    ...
    await input.deps.sendMessage(input.targetJid, message, mcpReviewMessageOptions(input));
    input.responder.reject(message, 'permission_denied');
}
function mcpReviewMessageOptions(input) {
...
async function createMcpProxyForSourceGroup(input) {
    ...
    const credentialEnv = await resolveMcpCredentialEnvForAgent({
        ...
        secrets: input.deps.getCapabilitySecretRepository?.() ??
...
async function syncApprovedCapabilitySettings(appId) {
    const storage = getRuntimeStorage();
