...
import { createTaskResponder, respondTaskLifecycleResult, toTrimmedString, } from './ipc-shared.js';
import { resolveConfiguredAllowedTools } from '../runtime/configured-agent-tools.js';
import { resolveWorkspaceFolderPath } from '../platform/workspace-folder.js';
import { taskContinuationThreadId, writeContinuationInput, } from '../runtime/continuation-input.js';
...
import { closeEgressGateway, ensureEgressGateway, } from '../runtime/egress-gateway.js';
import { DEFAULT_ASYNC_COMMAND_TIMEOUT_MS, DEFAULT_ASYNC_RESOURCE_LIMITS, buildAsyncCommandEnv, runSandboxedAsyncCommand, } from './async-command-sandbox-runner.js';
...
function validateSameConversation(context) {
...
function taskService(context) {
    ...
    const service = new AsyncCommandTaskService(repository, {
        run: async (input) => runSandboxedAsyncCommand(runnerSandboxProvider, {
            ...input,
...
function taskBackend(context, service, owner, parent, deliverTaskMessage) {
...
function taskScope(context) {
    ...
    const expectedAgentId = memoryAgentIdForWorkspaceFolder(context.sourceAgentFolder);
    ...
    if (sandboxPolicy.conversationId !== conversationId)
        return null;
    if (sandboxPolicy.providerAccountId &&
        sandboxPolicy.providerAccountId !== context.data.providerAccountId)
        return null;
    if (sandboxPolicy.threadId !== undefined &&
...
async function validateParentTaskScope(context, scope) {
...
async function configuredAllowedTools(context, scope) {
    const durableRules = (await resolveConfiguredAllowedTools({
...
const todoUpdateHandler = async (context) => {
    const { accept, reject } = responder(context);
    const conversationId = validateSameConversation(context);
    if (!conversationId) {
        reject('todo_update must target the originating conversation.', 'forbidden');
        return;
    }
    const payload = context.data.payload ?? {};
    const items = normalizeTodoItems(payload.items);
    if (!items) {
        reject('todo_update requires 1-50 unique items with id, title, and status.', 'invalid_request');
        return;
    }
    ...
    if (context.deps.renderAgentTodo && !context.data.jobId) {
        await context.deps
            ...
            : undefined)
            .catch((err) => {
            logger.debug({ err, conversationId }, 'todo_update channel render failed');
        });
    }
    accept('Plan updated.');
};
const asyncRunCommandHandler = async (context) => {
    const { acceptData, reject } = responder(context);
    const scope = taskScope(context);
    if (!scope) {
        reject('async_run_command must target the originating app, agent, and conversation.', 'forbidden');
        return;
    }
    ...
    if (!command) {
        reject('async_run_command requires a non-empty command.', 'invalid_request');
        return;
    }
...
const taskGetHandler = async (context) => {
    const { reject } = responder(context);
    const scope = taskScope(context);
    if (!scope) {
        reject('task_get must target the originating app, agent, and conversation.', 'forbidden');
        return;
    }
    const service = taskService(context);
    if (!service) {
...
const taskListHandler = async (context) => {
    const { reject } = responder(context);
    const scope = taskScope(context);
    if (!scope) {
        reject('task_list must target the originating app, agent, and conversation.', 'forbidden');
        return;
    }
    const service = taskService(context);
    if (!service) {
...
const taskCancelHandler = async (context) => {
    const { reject } = responder(context);
    const scope = taskScope(context);
    if (!scope) {
        reject('task_cancel must target the originating app, agent, and conversation.', 'forbidden');
        return;
    }
    const service = taskService(context);
    if (!service) {
...
const delegateTaskHandler = async (context) => {
    const { reject } = responder(context);
    const scope = taskScope(context);
    if (!scope) {
        reject('delegate_task must target the originating app, agent, and conversation.', 'forbidden');
        return;
    }
    const service = taskService(context);
    if (!service) {
    ...
    if (context.data.parentTaskId) {
        reject('delegate_task cannot be called from a delegated task.', 'forbidden');
        return;
    }
    const payload = context.data.payload ?? {};
...
const taskMessageHandler = async (context) => {
    const { reject } = responder(context);
    const scope = taskScope(context);
    if (!scope) {
        reject('task_message must target the originating app, agent, and conversation.', 'forbidden');
        return;
    }
    const service = taskService(context);
    if (!service) {
    ...
    const tasks = taskBackend(context, service, scopedTaskOwner, parentTask, (task, text) => {
        const workspaceFolder = typeof task.privateCorrelationJson.workspaceFolder === 'string'
            ? task.privateCorrelationJson.workspaceFolder
            : context.sourceAgentFolder;
        writeContinuationInput(workspaceFolder, text, `${Date.now()}-${randomUUID()}`, taskContinuationThreadId(task.threadId, task.id));
    });
    respondTaskLifecycleResult(context, await tasks.task_message({ taskId, message }));
};
export const agentTaskLifecycleHandlers = {
