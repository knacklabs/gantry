import { callableAgentToolName, dispatchCallableAgentTool, } from '../application/core-tools/callable-agent-tools.js';
...
import { resolveDelegatedAgentTarget, resolveDelegatedAgentTimeouts, } from './ipc-agent-delegation-target.js';
...
export async function executeResolvedDelegation(input) {
    ...
    const backend = createCoreTaskLifecycleBackend({
        ...
        authorityToolName: target.callableAgentEntry
            ? 'AgentDelegation'
            : undefined,
        enableDelegatedAsyncFollowUp: Boolean(target.callableAgentEntry && !input.trustedJobId),
        parentRunId: input.trustedJobId ? null : (input.trustedParentRunId ?? null),
        workspaceFolder: target.group.folder,
        runDelegatedAgent: async ({ task, prompt, signal, onProcessStarted, onProgress, timeoutMs, }) => {
            const runAgent = context.deps.runAgent ?? spawnAgent;
            let latestResult = null;
            let processHandlePersisted = null;
            const output = await runAgent(target.group, {
                prompt,
                ...
                parentRunId: input.trustedParentRunId ?? task.parentRunId ?? undefined,
                ...
                if (!proc.pid)
                    return;
                processHandlePersisted = Promise.resolve(onProcessStarted?.({
                    pid: proc.pid,
                ...
                asyncTaskRepositoryAvailable: Boolean(context.deps.getAsyncTaskRepository?.()),
            });
            if (processHandlePersisted)
                await processHandlePersisted;
            if (output.status === 'error') {
                return AsyncCommandTaskService.delegatedAgentFailureResult(output, latestResult, task.summary ?? 'Complete delegated task.');
            }
            return {
    ...
    const args = {
        ...
        context: toTrimmedString(input.payload.context, { maxLen: 20_000 }) ?? undefined,
        expectedOutput: toTrimmedString(input.payload.expectedOutput, { maxLen: 2_000 }) ??
            undefined,
        ...resolveDelegatedAgentTimeouts(input.payload, DEFAULT_DELEGATED_AGENT_TIMEOUT_MS),
    };
...
async function revalidateCallableAgentEntry(input, entry) {
