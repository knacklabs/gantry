...
import { isLockedDeniedIpcTaskType, resolveAgentLockStatus, } from '../config/profiles.js';
import { memoryAgentIdForWorkspaceFolder } from '../memory/app-memory-boundaries.js';
import { RUNTIME_EVENT_TYPES } from '../domain/events/runtime-event-types.js';
import { incrementOperationalError } from '../shared/operational-error-counters.js';
import { beginDurablePermissionInteraction, durablePermissionRequestSnapshot, } from '../application/interactions/durable-interaction-handler.js';
const DENIED_BY_PROFILE_REASON = 'denied_by_profile';
export async function requestDurableTaskPermissionApproval(request, prompt) {
...
async function denyLockedIpcTask(data, sourceAgentFolder, deps, lockStatus) {
    logger.warn({
        type: data.type,
        sourceAgentFolder,
    ...
    await deps
        ...
        .catch((err) => {
        logger.error({ err, type: data.type, sourceAgentFolder }, 'Failed to publish denied_by_profile audit event');
    });
    writeTaskIpcResponse(sourceAgentFolder, data.taskId, {
        ok: false,
    ...
    }, data.authThreadId, data.responseKeyId);
}
const taskHandlers = {
...
export async function processTaskIpc(data, sourceAgentFolder, deps, ipcBaseDir) {
    ...
    const sourceAgentFolderJids = Array.from(new Set(Object.entries(conversationBindings)
    ...
    if (!handler) {
        logger.warn({ type: data.type, sourceAgentFolder }, 'Unknown IPC task type');
        writeTaskIpcResponse(sourceAgentFolder, data.taskId, {
            ok: false,
    ...
    const resolvedDeps = {
        ...deps,
        requestPermissionApproval: (request) => requestDurableTaskPermissionApproval(request, deps.requestPermissionApproval),
        opsRepository: deps.opsRepository ?? getRuntimeRepositories(),
        getToolRepository: deps.getToolRepository ??
            (() => {
                try {
        ...
        getSkillRepository: deps.getSkillRepository ??
            (() => {
                try {
        ...
        getPermissionRepository: deps.getPermissionRepository ??
            (() => {
                try {
        ...
        getJobControl: deps.getJobControl ??
    ...
    try {
        ...
        logger.error({ err, type: data.type, sourceAgentFolder }, 'Unhandled IPC task handler error');
        writeTaskIpcResponse(sourceAgentFolder, data.taskId, {
            ok: false,
            code: 'internal_error',
