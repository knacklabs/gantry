...
import { getRuntimeSettingsRevision, readRuntimeSettingsYaml, } from '../config/settings/runtime-settings.js';
...
import { createTaskResponder, restartServiceForRuntimeHome, toTrimmedString, } from './ipc-shared.js';
...
export const serviceRestartHandler = async (context) => {
    const { data, deps, sourceAgentFolder, sourceAgentFolderJids } = context;
    const taskId = toTrimmedString(data.taskId, { maxLen: 128 });
    const { accept, reject } = createTaskResponder(sourceAgentFolder, taskId, data.authThreadId, data.responseKeyId);
    if (!(await sourceAgentHasAdminToolCapability(context, 'service_restart'))) {
        logger.warn({ sourceAgentFolder }, 'Unauthorized service_restart attempt blocked');
        reject(adminCapabilityRequiredMessage('service_restart'), 'missing_capability');
        return;
    }
    try {
        ...
        if (typeof deps.requestPermissionApproval !== 'function' ||
            ...
            reject('Service restart requests require a configured approval surface.', 'preflight_failed');
            return;
        }
        const validation = await validateRuntimePreflightWithStorage(GANTRY_HOME);
        if (!validation.ok) {
            reject(validation.failure?.summary ||
                'Runtime configuration validation failed.', 'preflight_failed', validation.failure?.details || []);
            return;
        }
        const reason = toTrimmedString(data.payload?.reason, { maxLen: 2000 }) ||
        ...
        const decision = await deps.requestPermissionApproval({
            ...
            description: 'Approving restarts the local Gantry runtime service after runtime preflight passes.',
        ...
        setTimeout(() => {
            ...
            if (!restartOutcome.ok) {
                logger.error({ sourceAgentFolder, taskId, error: restartOutcome.message }, 'Service restart failed after acknowledgment');
                return;
            }
            logger.info({ sourceAgentFolder, taskId, message: restartOutcome.message }, 'Service restart completed');
        }, 0);
    }
    catch (err) {
        ...
        logger.error({ sourceAgentFolder, taskId, err }, 'Error while handling service_restart IPC task');
        reject(message, 'internal_error');
    }
};
export const settingsDesiredStateHandler = async (context) => {
    ...
    if (!(await sourceAgentHasAdminToolCapability(context, 'settings_desired_state'))) {
        reject(adminCapabilityRequiredMessage('settings_desired_state'), 'missing_capability');
        return;
    }
    try {
        ...
        reject(err instanceof Error ? err.message : 'Failed to read settings.yaml.', 'invalid_settings');
    }
};
export const guidedActionPreviewHandler = async (context) => {
    const { data, sourceAgentFolder } = context;
    const { acceptData, reject } = createTaskResponder(sourceAgentFolder, data.taskId, data.authThreadId, data.responseKeyId);
    if (!(await sourceAgentHasAdminToolCapability(context, 'guided_action_preview'))) {
        reject(adminCapabilityRequiredMessage('guided_action_preview'), 'missing_capability');
        return;
    }
    try {
        ...
        const model = await buildControlPlaneReadModelFromRepositories({
            ...
            modelCredentialsRepository: getRuntimeStorage().repositories.modelCredentials,
            pendingAccessRequestsRepository: getRuntimeStorage().repositories.pendingAccessRequests,
        });
        const ref = resolveControlPlaneGuidedAction(model.nextAction);
        acceptData('Guided action preview ready.', new GuidedActionService().preview(ref));
    }
    catch (err) {
        reject(err instanceof Error
            ? err.message
            : 'Failed to build guided action preview.', 'internal_error');
    }
};
export const requestSettingsUpdateHandler = async (context) => {
    const { data, deps, sourceAgentFolder, sourceAgentFolderJids } = context;
    const { accept, reject } = createTaskResponder(sourceAgentFolder, data.taskId, data.authThreadId, data.responseKeyId);
    if (!(await sourceAgentHasAdminToolCapability(context, 'request_settings_update'))) {
        reject(adminCapabilityRequiredMessage('request_settings_update'), 'missing_capability');
        return;
    }
    ...
    if (!replacementYaml || !expectedRevision || !reason) {
        reject('Missing required fields: replacementYaml, expectedRevision, and reason.', 'invalid_request');
        return;
    }
    const currentRevision = getRuntimeSettingsRevision(GANTRY_HOME);
    if (expectedRevision !== currentRevision) {
        reject('settings.yaml changed since it was read. Reload settings_desired_state and retry with the latest revision.', 'stale_settings');
        return;
    }
    ...
    if (typeof deps.requestPermissionApproval !== 'function' ||
        typeof deps.sendMessage !== 'function') {
        reject('Settings update requests require a configured approval surface.', 'preflight_failed');
        return;
    }
    let parsed;
    try {
        ...
        reject(err instanceof Error ? err.message : 'settings.yaml did not parse.', 'invalid_settings');
        return;
    }
    const validation = validateLoadedRuntimeSettings(GANTRY_HOME, parsed);
    if (!validation.ok) {
        reject(validation.failure?.summary || 'settings.yaml validation failed.', 'invalid_settings', validation.failure?.details || []);
        return;
    }
    ...
    const invalidReferences = await desiredState.validateCapabilityReferences(parsed);
    if (invalidReferences.length > 0) {
        reject('settings.yaml contains unavailable capability references.', 'invalid_settings', invalidReferences);
        return;
    }
    void (async () => {
        let message;
        try {
            ...
            const decision = await deps.requestPermissionApproval({
                ...
                description: 'Approving writes validated local desired-state settings.yaml. Safe changes reload automatically; some topology changes may require restart.',
                decisionReason: reason,
                toolInput: {
            ...
            const finalInvalidReferences = await desiredState.validateCapabilityReferences(parsed);
            ...
            message = settingsUpdateSuccessMessage(beforeSettings, parsed, sourceAgentFolder, requestingInstall);
            accept(message, 'settings_updated');
        }
        catch (err) {
            logger.error({ err, sourceAgentFolder }, 'Settings update review failed');
            reject('The settings update could not be completed. Explain this in plain language and say you can try again after the setup issue is fixed.', 'settings_review_failed');
            return;
        }
    })().catch((err) => logger.error({ err, sourceAgentFolder }, 'Settings update final message failed'));
};
function settingsUpdateSuccessMessage(before, after, sourceAgentFolder, requestingInstall) {
    if (permissionPostureBecameAuto(before, after, sourceAgentFolder, requestingInstall)) {
        return "Done — I'll only check with you for risky actions now.";
    }
    if (!before.permissions.yoloMode.denylist.includes('rm *') &&
...
function resolveRequestingInstallIdentity(input) {
    ...
    const routes = findConversationRoutesForChat(input.conversationBindings, input.requestedTargetJid, input.threadId, input.providerAccountId).filter(([, route]) => route.folder === input.sourceAgentFolder);
    for (const [routeKey, route] of routes) {
        ...
        const routeProviderAccountId = parsedRoute.providerAccountId ?? route.providerAccountId?.trim();
        for (const [installId, install] of Object.entries(conversation.installedAgents)) {
            ...
            identities.set(`${identity.conversationId}:${identity.installId}`, identity);
        }
    }
    return identities.size === 1 ? [...identities.values()][0] : null;
}
function permissionPostureBecameAuto(before, after, sourceAgentFolder, requestingInstall) {
    const beforeInstall = requestingInstall
        ? before.conversations[requestingInstall.conversationId]?.installedAgents[requestingInstall.installId]
        : undefined;
    const afterInstall = requestingInstall
        ? after.conversations[requestingInstall.conversationId]?.installedAgents[requestingInstall.installId]
        : undefined;
    const beforeMode = resolveEffectivePermissionMode(beforeInstall?.permissionMode, before.agents[sourceAgentFolder]?.permissionMode);
    const afterMode = resolveEffectivePermissionMode(afterInstall?.permissionMode, after.agents[sourceAgentFolder]?.permissionMode);
    return beforeMode !== 'auto' && afterMode === 'auto';
}
function summarizeYamlDiff(before, after) {
    ...
    for (let index = 0; index < max; index += 1) {
        ...
        if (left === right)
            continue;
        if (left)
            diff.push(`- ${index + 1}: ${left}`);
        if (right)
            diff.push(`+ ${index + 1}: ${right}`);
        if (diff.length >= 80) {
