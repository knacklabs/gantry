...
import { getDefaultModelConfig, getSelectedAgentHarness, } from '../config/index.js';
...
const schedulerUpsertJobHandler = async (context) => {
    const { data, sourceAgentFolder } = context;
    const { accept, acceptData, reject } = createTaskResponder(sourceAgentFolder, data.taskId, data.authThreadId, data.responseKeyId);
    ...
    try {
        ...
        if (data.confirm !== true) {
            acceptData(formatSchedulerJobPlan({ ...planInput, confirmationToken }), {
        ...
        if (data.confirmationToken !== confirmationToken) {
            reject('Scheduler upsert confirmation token is missing or does not match the current job plan.', 'confirmation_mismatch');
            return;
        }
        ...
        logger.info({ id: result.jobId, created: result.created, sourceAgentFolder }, 'Job upserted via IPC');
        invalidateSystemJobRegistrationSignature(context.deps.opsRepository);
        const defaultModel = normalizedScheduleType === 'once'
        ...
        const catalogModel = selectedModel
            ? resolveModelSelectionForWorkload(selectedModel, normalizedScheduleType === 'once' ? 'one_time_job' : 'recurring_job')
            : undefined;
        const resolvedModel = (catalogModel?.ok ? catalogModel.entry : undefined) ??
        ...
        const runtimeThreadId = data.executionContext?.threadId ?? data.authThreadId;
