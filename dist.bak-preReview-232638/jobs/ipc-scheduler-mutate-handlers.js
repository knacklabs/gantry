...
import { enqueueJobTrigger, isJobTriggerQueueReady, schedulerNotReadyReason, } from './scheduler.js';
import { jobSetupBlockerFromUnknown, setupActionLabel, } from '../shared/job-setup-labels.js';
...
function makeRunNowJobService(context) {
    return new JobManagementService({
        ops: context.deps.opsRepository,
        scheduler: { requestSchedulerSync: context.deps.onSchedulerChanged },
        schedulePlanner: runtimeJobSchedulePlanner,
...
function scheduleType(raw) {
    return raw === 'cron' || raw === 'interval' || raw === 'once'
        ? raw
        : undefined;
}
async function resumeDeadLetterDetails(context, jobId, err) {
    ...
    try {
        ...
        if (job?.status !== 'dead_lettered')
            return undefined;
        const pauseReason = typeof job?.pause_reason === 'string' ? job.pause_reason.trim() : '';
        if (!pauseReason)
            return undefined;
        return [pauseReason, 'Job has been moved to dead_lettered state.'];
    }
    catch (lookupErr) {
        logger.warn({ err: lookupErr, sourceAgentFolder: context.sourceAgentFolder, jobId }, 'Failed to read dead-lettered job details after scheduler_resume_job failure');
        return undefined;
    }
}
const schedulerUpdateJobHandler = async (context) => {
    ...
    try {
        ...
        if (data.timeoutMs !== undefined)
            patch.timeoutMs = data.timeoutMs;
        if (data.maxRetries !== undefined)
            patch.maxRetries = data.maxRetries;
        ...
        accept(`Scheduler job updated (${jobId}).${formatSetupOutcome(result?.job)}`);
    }
    catch (err) {
        const mapped = mapApplicationError(err, 'Failed to mutate scheduler job.');
        logger.error({ err, sourceAgentFolder, jobId }, 'scheduler_update_job failed unexpectedly');
        reject(mapped.message, mapped.code);
    }
};
const schedulerDeleteJobHandler = async (context) => {
    const { data, sourceAgentFolder } = context;
    const { accept, reject } = createTaskResponder(sourceAgentFolder, data.taskId, data.authThreadId, data.responseKeyId);
    const jobId = toTrimmedString(data.jobId, { maxLen: 128 });
    if (!jobId) {
    ...
    try {
        ...
        const mapped = mapApplicationError(err, 'Failed to mutate scheduler job.');
        logger.error({ err, sourceAgentFolder, jobId }, 'scheduler_delete_job failed unexpectedly');
        reject(mapped.message, mapped.code);
    }
};
const schedulerPauseJobHandler = async (context) => {
    const { data, sourceAgentFolder } = context;
    const { accept, reject } = createTaskResponder(sourceAgentFolder, data.taskId, data.authThreadId, data.responseKeyId);
    const jobId = toTrimmedString(data.jobId, { maxLen: 128 });
    if (!jobId) {
    ...
    try {
        ...
        const mapped = mapApplicationError(err, 'Failed to mutate scheduler job.');
        logger.error({ err, sourceAgentFolder, jobId }, 'scheduler_pause_job failed unexpectedly');
        reject(mapped.message, mapped.code);
    }
};
const schedulerResumeJobHandler = async (context) => {
    const { data, sourceAgentFolder } = context;
    const { accept, reject } = createTaskResponder(sourceAgentFolder, data.taskId, data.authThreadId, data.responseKeyId);
    const jobId = toTrimmedString(data.jobId, { maxLen: 128 });
    if (!jobId) {
    ...
    try {
        ...
        logger.error({ err, sourceAgentFolder, jobId }, 'scheduler_resume_job failed unexpectedly');
        reject(mapped.message, mapped.code, details);
    }
};
const schedulerRunNowHandler = async (context) => {
    const { data, sourceAgentFolder } = context;
    ...
    try {
        ...
        logger.error({ err, sourceAgentFolder, jobId }, 'scheduler_run_now failed unexpectedly');
        reject(mapped.message, mapped.code);
    }
};
function formatSetupOutcome(job) {
    const setupState = job?.setup_state;
    if (!setupState || typeof setupState !== 'object')
        return '';
    const state = setupState.state;
    if (state === 'ready')
        return '';
...
export const schedulerMutateTaskHandlers = {
