...
import { buildJobListVisibilityMetadata, buildJobVisibilityMetadata, } from '../application/jobs/job-visibility-metadata.js';
...
function makeJobService(context) {
...
async function resolveMetadataAppId(context) {
...
function normalizeSchedulerWaitTimeoutMs(value) {
    const raw = typeof value === 'number' && Number.isFinite(value) ? value : 30_000;
    return Math.max(SCHEDULER_WAIT_MIN_TIMEOUT_MS, Math.min(raw, SCHEDULER_WAIT_MAX_TIMEOUT_MS));
}
function schedulerEventFilters(data) {
    ...
    return {
        ...
        sinceId: typeof data.sinceId === 'number' && Number.isFinite(data.sinceId)
            ? Math.max(0, Math.floor(data.sinceId))
            : undefined,
        limit: data.limit,
    };
}
async function schedulerRuntimeEventFilter(context, filters) {
    ...
    const eventType = filters.eventType
        ...
        : undefined;
    if (filters.eventType && !eventType)
        return undefined;
    return {
        appId: appId,
        ...(filters.sinceId !== undefined
            ? { afterEventId: filters.sinceId }
            : {}),
...
async function schedulerWaitAppId(context, filters) {
    const explicitAppId = toTrimmedString(context.data.appId, { maxLen: 128 });
    if (explicitAppId)
        return explicitAppId;
    const originAppId = appIdFromConversationJid(schedulerAccessFromContext(context).originConversationJid);
    if (originAppId)
        return originAppId;
    if (filters.jobId) {
...
const schedulerGetJobHandler = async (context) => {
    ...
    try {
        ...
        const data = result.job
            ? {
                job: {
                    ...result.job,
                    visibility: publicJobVisibility(await buildJobVisibilityMetadata({
                        job: result.job,
                        appId: metadataAppId,
                        ops: context.deps.opsRepository,
                        toolRepository: context.deps.getToolRepository?.(),
            ...
            : result;
        acceptData(result.job ? `Loaded scheduler job (${jobId}).` : 'Job not found.', data);
    }
    catch (err) {
...
const schedulerListJobsHandler = async (context) => {
    const { data, sourceAgentFolder } = context;
    const { acceptData, reject } = createTaskResponder(sourceAgentFolder, data.taskId, data.authThreadId, data.responseKeyId);
    try {
        const service = makeJobService(context);
...
function publicJobVisibility(metadata) {
    return {
...
const schedulerListRunsHandler = async (context) => {
    const { data, sourceAgentFolder } = context;
    const { acceptData, reject } = createTaskResponder(sourceAgentFolder, data.taskId, data.authThreadId, data.responseKeyId);
    const jobId = toTrimmedString(data.jobId, { maxLen: 128 });
    try {
        ...
        const mapped = mapApplicationError(err, 'Failed to query scheduler jobs.');
        logger.error({ err, sourceAgentFolder, jobId: jobId || undefined }, 'scheduler_list_runs failed unexpectedly');
        reject(mapped.message, mapped.code);
    }
};
const schedulerListEventsHandler = async (context) => {
    const { data, sourceAgentFolder } = context;
    const { acceptData, reject } = createTaskResponder(sourceAgentFolder, data.taskId, data.authThreadId, data.responseKeyId);
...
const schedulerWaitForEventsHandler = async (context) => {
    const { data, sourceAgentFolder } = context;
    const { acceptData, reject } = createTaskResponder(sourceAgentFolder, data.taskId, data.authThreadId, data.responseKeyId);
    const filters = schedulerEventFilters(data);
    ...
    try {
        const service = makeJobService(context);
        const access = schedulerAccessFromContext(context);
        const listEvents = () => service.listJobEvents({
            access,
            ...filters,
        });
        const subscriptionFilter = await schedulerRuntimeEventFilter(context, filters);
        ...
        while (true) {
            ...
            if (subscription) {
                ...
                await new Promise((resolve) => setTimeout(resolve, Math.min(remainingMs, SCHEDULER_WAIT_FALLBACK_INTERVAL_MS)));
            }
        }
    }
    catch (err) {
...
const schedulerGetDeadLetterHandler = async (context) => {
    const { data, sourceAgentFolder } = context;
    const { acceptData, reject } = createTaskResponder(sourceAgentFolder, data.taskId, data.authThreadId, data.responseKeyId);
    try {
        ...
        acceptData(`Listed ${result.deadLetterRuns.length} dead-letter run(s).`, result);
    }
    catch (err) {
        const mapped = mapApplicationError(err, 'Failed to query scheduler jobs.');
        logger.error({ err, sourceAgentFolder }, 'scheduler_get_dead_letter failed');
        reject(mapped.message, mapped.code);
    }
};
const schedulerListNotificationTargetsHandler = async (context) => {
    const { data, sourceAgentFolder } = context;
    const { acceptData, reject } = createTaskResponder(sourceAgentFolder, data.taskId, data.authThreadId, data.responseKeyId);
    try {
        const access = schedulerAccessFromContext(context);
        ...
        const originConversation = access.conversationBindings[originConversationJid] ?? null;
        ...
        const mapped = mapApplicationError(err, 'Failed to list scheduler notification targets.');
        logger.error({ err, sourceAgentFolder }, 'scheduler_list_notification_targets failed unexpectedly');
        reject(mapped.message, mapped.code);
    }
};
export const schedulerQueryTaskHandlers = {
