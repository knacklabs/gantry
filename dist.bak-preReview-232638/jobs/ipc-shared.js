...
import { getServiceStatus, startService, stopService, } from '../infrastructure/service/manager.js';
...
export function taskIpcResponsePath(sourceAgentFolder, taskId) {
    return path.join(DATA_DIR, 'ipc', sourceAgentFolder, 'task-responses', `task-${taskId}.json`);
}
export function writeTaskIpcResponse(sourceAgentFolder, taskId, payload, authThreadId, responseKeyId) {
    if (!taskId || !TASK_IPC_RESPONSE_ID_PATTERN.test(taskId))
        return;
    if (!isValidWorkspaceFolder(sourceAgentFolder))
        return;
    const responseDir = path.join(DATA_DIR, 'ipc', sourceAgentFolder, 'task-responses');
    ...
    const privateKeyPem = getIpcResponseSigningPrivateKey(sourceAgentFolder, authThreadId, responseKeyId);
...
export function createTaskResponder(sourceAgentFolder, taskIdRaw, authThreadId, responseKeyId) {
    const taskId = toTrimmedString(taskIdRaw, { maxLen: 128 });
    return {
        accept: (message, code, details) => {
            writeTaskIpcResponse(sourceAgentFolder, taskId, {
                ok: true,
                ...(code ? { code } : {}),
                message,
                ...(details && details.length > 0 ? { details } : {}),
            }, authThreadId, responseKeyId);
        },
        acceptData: (message, data, code, details) => {
            writeTaskIpcResponse(sourceAgentFolder, taskId, {
                ok: true,
                ...(code ? { code } : {}),
                message,
...
export function respondTaskLifecycleResult(context, result) {
    const responder = createTaskResponder(context.sourceAgentFolder, context.data.taskId, context.data.authThreadId, context.data.responseKeyId);
    if (!result.ok)
        responder.reject(result.message, result.code);
    else if (result.data === undefined) {
        responder.accept(result.message, result.code);
    }
    else
        responder.acceptData(result.message, result.data, result.code);
}
export function restartServiceForRuntimeHome(runtimeHome) {
    try {
