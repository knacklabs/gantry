...
import { resolveTaskAgentId, validateSameChannelApprovalTarget, } from './ipc-skill-request-context.js';
import { startSkillPermissionReview } from './ipc-skill-permission-review.js';
import { formatArgvForDisplay, skillInstallCommandDisplayName, } from './skill-install-display.js';
import { boundedSkillInstallFailureReason, collectInstalledSkillAssets, discoverInstalledSkillRoots, installedSkillContext, reportUnattemptedSkillRoots, rollbackFreshInstallBinding, rollbackInstalledSkillReplacement, safeInstallerEnv, withSkillMaterializationLock, snapshotInstalledSkill, skillInstallCommandReceipt, skillNameForReceipt, } from './skill-install-assets.js';
...
import { redactCommandOutput, sanitizedStringList, } from './skill-install-command-sanitization.js';
const pendingSkillInstallCommandReviews = new Set();
const pendingSkillPackageReviews = new Set();
let runtimeDeps = null;
export function configureSkillInstallHandlers(deps) {
    runtimeDeps = deps;
}
...
export const requestSkillProposalHandler = async (context) => {
...
export const requestSkillInstallHandler = async (context) => {
    const payload = context.data.payload || {};
    const installCommandArgv = sanitizedStringList(Array.isArray(payload.installCommandArgv) ? payload.installCommandArgv : []);
    const hasPackageFiles = Array.isArray(payload.files) && payload.files.length > 0;
    if (installCommandArgv.length > 0 && hasPackageFiles) {
        const { reject } = createContextTaskResponder(context);
        reject('Skill install requests must use either files or installCommandArgv, not both.', 'invalid_request');
        return;
    }
    if (installCommandArgv.length > 0 && !hasPackageFiles) {
...
const requestSkillInstallCommandHandler = async (context, installCommandArgv) => {
    ...
    if (typeof deps.requestPermissionApproval !== 'function' ||
        ...
        reject('Skill install requests require a configured approval and command execution surface.', 'preflight_failed');
        return;
    }
    ...
    if (pendingSkillInstallCommandReviews.has(pendingKey)) {
        accept(`${displayName} is already waiting for approval in this chat.`, 'skill_install_already_pending');
        return;
    }
...
async function completeSkillInstallCommandReview(input) {
    ...
    try {
        const decision = await deps.requestPermissionApproval({
            ...
            description: 'Only configured approvers can decide this request. Approval lets the agent prepare or import the skill package, then makes the skill available to this agent.',
            ...
            toolInput: {
                ...
                effect: 'prepares_or_imports_skill_package_and_enables_skill_after_approval',
            },
        });
        getRuntimeDeps().logInfo({
            appId: data.appId,
        ...
        if (!decision.approved || !decision.decidedBy) {
            ...
            await deps.sendMessage(input.targetJid, message, skillInstallMessageOptions(data));
            return;
        }
        ...
        const installed = await installSkillFromApprovedCommand({
            ...
            sourceAgentFolder,
            installCommandArgv: input.installCommandArgv,
            requiredEnvVars: sanitizedStringList(Array.isArray(data.payload?.requiredEnvVars)
        ...
        if (installed.skills.length > 0) {
            await deps.sendMessage(input.targetJid, skillInstallCommandReceipt({ ...installed, failed: [] }), skillInstallMessageOptions(data));
        }
        if (installed.installed.length === 0) {
            input.responder.reject(message, 'skill_install_failed');
            return;
        }
        input.responder.acceptData(message, installedSkillContext(installed.installed), 'skill_installed');
        getRuntimeDeps().logInfo({
            appId: data.appId,
            sourceAgentFolder,
            targetJid: input.targetJid,
            ...
            requiredEnvVars: [
                ...new Set(installed.skills.flatMap((skill) => skill.requiredEnvVars ?? [])),
            ],
        }, 'Skill install completed');
    }
    catch (err) {
        getRuntimeDeps().logError({ err, sourceAgentFolder, toolName: 'request_skill_install' }, 'Skill install command review failed');
        const message = 'The skill could not be installed. Explain this in plain language and say you can try again after the setup issue is fixed.';
...
const requestSkillPackageHandler = async (context, input) => {
    const { data, deps, sourceAgentFolder, sourceAgentFolderJids } = context;
    const { accept, acceptData, reject } = createContextTaskResponder(context);
    const payload = data.payload || {};
    if (!data.appId) {
        reject(`${input.requestKind} requests require signed app scope.`, 'forbidden');
        return;
    }
    const agentId = resolveTaskAgentId(data, sourceAgentFolder);
    const reason = toTrimmedString(payload.reason, { maxLen: 2000 }) || '';
    ...
    if (typeof deps.requestPermissionApproval !== 'function' ||
        typeof deps.sendMessage !== 'function') {
        reject(`${input.requestKind} requests require a configured approval surface.`, 'preflight_failed');
        return;
    }
    const parsed = parseSkillPackageAssets(payload.files);
    if (!parsed.ok) {
        reject(skillPackageParseError(input.requestToolName, parsed.error), 'invalid_request');
        return;
    }
    const pendingKey = [
        data.appId,
    ...
    if (pendingSkillPackageReviews.has(pendingKey)) {
        accept(`${input.requestKind} for this skill package is already waiting for approval in this chat.`, 'skill_package_already_pending');
        return;
    }
    ...
    try {
        ...
        const service = new SkillService(storage.repositories.skills, storage.skillArtifacts);
        ...
        startSkillPermissionReview({
            ...
            service,
            syncApprovedCapabilitySettings: getRuntimeDeps().syncApprovedCapabilitySettings,
            appId: data.appId,
            agentId,
            sourceAgentFolder,
            targetJid: requestedTargetJid,
            ...
            fileSummaries: parsed.fileSummaries.map(({ path, sizeBytes, fingerprint }) => ({
                path,
                sizeBytes,
                fingerprint,
            })),
        ...
        pendingSkillPackageReviews.delete(pendingKey);
        getRuntimeDeps().logError({ err, sourceAgentFolder, toolName: input.requestToolName }, `${input.requestKind} request failed`);
        reject('The skill request could not be completed. Explain this in plain language and say you can try again after the setup issue is fixed.', 'invalid_request');
    }
};
function skillInstallMessageOptions(data) {
...
function skillPackageParseError(requestToolName, error) {
    if (requestToolName !== 'request_skill_install')
        return error;
    if (!error.toLowerCase().includes('files'))
        return error;
...
async function installSkillFromApprovedCommand(input) {
    const storage = getRuntimeDeps().getStorage();
    const stagingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gantry-skill-install-'));
    try {
        ...
        const installedMaterializationKeys = new Set();
        for (const root of discovery.roots) {
            ...
            try {
                ...
                const materializationKey = materializedSkillDirectoryNameFor(name).toLowerCase();
                ...
                const outcome = await withSkillMaterializationLock(materializationKey, async () => {
                    let skill;
                    ...
                    try {
                        ...
                        if (previousSkill) {
                            const rollback = await rollbackInstalledSkillReplacement({
                                ...
                                skills: storage.repositories.skills,
                                artifacts: storage.skillArtifacts,
                                syncAfterRestore: () => getRuntimeDeps().syncApprovedCapabilitySettings(input.appId),
                            });
                            return {
                                kind: 'failed',
                        ...
                        if (skill) {
                            ...
                            const cleanup = await rollbackFreshInstallBinding({
                                reason,
                                syncAttempted,
                                rollbackBinding: () => service.rollbackInstalledSkillBinding({
                                    appId: input.appId,
                                    agentId: input.agentId,
                                    skillId: installedSkill.id,
                                }),
                                isBindingActive: async () => (await service.resolveLocalSkillsForAgent({
                                    appId: input.appId,
                                    agentId: input.agentId,
                                })).some((activeSkill) => activeSkill.id === installedSkill.id),
                                sync: () => getRuntimeDeps().syncApprovedCapabilitySettings(input.appId),
                            });
