...
export const runtimeJobSchedulePlanner = {
    ...
    planAppSchedule(input) {
        if (input.kind === 'once') {
            ...
            if (!scheduleValue) {
                throw new ApplicationError('INVALID_SCHEDULE', 'runAt is required for once jobs');
            }
        ...
        if (input.kind === 'recurring') {
            if (input.schedule?.type === 'interval') {
                ...
                if (!/^[0-9]+$/.test(scheduleValue) || ms <= 0) {
                    throw new ApplicationError('INVALID_SCHEDULE', 'interval schedules require a positive numeric value');
                }
                return { scheduleType: 'interval', scheduleValue, nextRun: nowIso() };
            }
            const scheduleValue = String(input.schedule?.value || '').trim();
            if (!scheduleValue) {
                throw new ApplicationError('INVALID_SCHEDULE', 'cron schedules require a non-empty value');
            }
    ...
    planInitial(input) {
        ...
        if (input.scheduleType === 'cron') {
            ...
            if (validationError) {
                throw new ApplicationError('INVALID_SCHEDULE', 'Invalid cron expression for scheduler job.');
            }
            return {
                nextRun: computeNextJobRun({
                    schedule_type: input.scheduleType,
                    schedule_value: scheduleValue,
                }, nowIso()) ?? nowIso(),
            };
        ...
        if (input.scheduleType === 'interval') {
            ...
            if (Number.isNaN(ms) || ms <= 0) {
                throw new ApplicationError('INVALID_SCHEDULE', 'Invalid interval milliseconds for scheduler job.');
            }
            return { nextRun: new Date(currentTimeMs() + ms).toISOString() };
        }
        const date = Date.parse(scheduleValue);
        if (!Number.isFinite(date)) {
            throw new ApplicationError('INVALID_SCHEDULE', 'Invalid once timestamp for scheduler job.');
        }
