...
import { modelUseKindForJobSchedule, resolveDefaultJobExecutionProviderId, resolveJobModel, } from '../application/jobs/job-model-resolution.js';
...
export function resolveJobExecutionProviderId(input) {
    ...
    let routed;
    if (resolution?.ok) {
        const route = input.resolvedModel.routeResolution ??
            resolveExecutionRoute({
                entry: resolution.entry,
                agentHarness: input.resolvedModel.agentHarness,
            });
        if (route.ok) {
            routed = route.value.executionProviderId;
        }
    }
    return (routed ??
...
function modelAuditPayload(resolved) {
    return {
...
function modelSelectionReason(resolved) {
...
function resolvedRunDiagnostics(resolved) {
    const provider = resolved.entry
        ? getModelProviderDefinition(resolved.entry.modelRoute.id)
        : undefined;
    let executionProviderId = null;
    let supportedCredentialModes = [];
    if (resolved.resolution?.ok) {
        const route = resolved.routeResolution ??
            resolveExecutionRoute({
...
export function jobStartedModelPayload(resolved) {
    return {
...
export function jobCompletedModelPayload(resolved, usage) {
    return {
        usage,
        ...modelAuditPayload(resolved),
    };
