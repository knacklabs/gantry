...
import { OBSERVER_DIGEST_JOB_ID_PREFIX, OBSERVER_DIGEST_SYSTEM_PROMPT, } from '../shared/system-job-identity.js';
import { runObserverDigest, noopDigestDeliveryPort, createOutboundDigestDeliveryPort, } from '../brain/observer-digest.js';
...
let observerDigestGateway = null;
export function setObserverDigestGateway(gateway) {
...
export function observerRegistrationSignatureFields(observerDeliveryStatus) {
    return {
...
export async function registerObserverDigestJob(deps, input) {
    ...
    const computedNextRun = computeNextJobRun({ schedule_type: 'cron', schedule_value: OBSERVER_DIGEST_CRON }, nowIso);
    ...
    await deps.opsRepository.upsertJob(observerDigestJob);
}
export async function runScheduledObserverDigest(signal) {
    signal?.throwIfAborted();
    const storage = getRuntimeStorage();
    const repository = storage.repositories.observerInsights;
    if (!observerDigestGateway) {
        logger.warn({}, 'observer digest gateway is not registered; reserving without sending (will retry once wired)');
    }
