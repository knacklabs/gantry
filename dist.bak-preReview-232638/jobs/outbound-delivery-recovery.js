...
let activeRecoveryLoop = null;
export async function runBoundedOutboundDeliveryRecovery(input) {
        ...
        for (const claimedItem of claimed) {
            ...
            try {
                ...
                if (isPartialMessageDeliveryError(err)) {
                    ...
                    dispatchResult = {
                        status: 'partially_delivered',
                        error: `Outbound delivery may already be visible and cannot be retried safely. ${err.message}`.trim(),
                    };
                }
                else {
            ...
            if (dispatchResult.status === 'sent') {
                ...
                try {
                    const settled = await input.service.settleSent({
                        ...
                        itemId: claimedItem.item.id,
                        claimToken,
                        receiptIdempotencyKey: input.receiptIdempotencyKeyForItem?.(claimedItem) ??
                ...
                await settleAmbiguousNonRetryable({
                    claimed: claimedItem,
                    claimToken,
                    error: `Outbound delivery may already be visible and cannot be retried safely. ${settledError}`.trim(),
                    partialAt: now(),
                    warnMessage: 'Outbound delivery ambiguous post-send settlement failed',
                });
                continue;
            }
            if (dispatchResult.status === 'partially_delivered') {
                const partialError = dispatchResult.error?.trim() ||
                ...
                try {
                    ...
                    await settleAmbiguousNonRetryable({
                        ...
                        partialAt: now(),
                        warnMessage: 'Outbound delivery partial retry-tail settlement was not applied',
                    });
                }
                catch (err) {
                    incrementOperationalError('delivery', 'partial_settlement');
                    ...
                    await settleAmbiguousNonRetryable({
                        ...
                        partialAt: now(),
                        warnMessage: 'Outbound delivery partial retry-tail settlement fallback failed',
                    });
                }
                continue;
            }
            const settled = await input.service.settleFailed({
                ...
                itemId: claimedItem.item.id,
                claimToken,
                error: dispatchResult.error?.trim() ||
...
export function startOutboundDeliveryRecoveryLoop(input) {
...
export async function stopOutboundDeliveryRecoveryLoop() {
