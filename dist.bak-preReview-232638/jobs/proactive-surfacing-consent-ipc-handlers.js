...
export function configureProactiveSurfacingConsentIpcHandlers(deps) {
    runtimeDeps = deps;
}
function getRuntimeDeps() {
    if (!runtimeDeps) {
        throw new Error('Proactive surfacing consent IPC handlers are not configured.');
    }
    return runtimeDeps;
}
function proactiveSurfacingId(subject) {
...
export const proactiveSurfacingConsentHandler = async (context) => {
    ...
    if (!data.appId) {
        reject('Proactive surfacing consent requires signed app scope.', 'forbidden');
        return;
    }
    ...
    if (!data.memoryUserId) {
        reject('Proactive surfacing consent change requires a real user reply this turn.', 'forbidden');
        return;
    }
    ...
    if (!context.sourceAgentFolderJids.includes(targetJid)) {
        reject('Proactive surfacing consent must target a chat bound to the requesting agent.', 'forbidden');
        return;
    }
    const repo = getRuntimeDeps().getStorage().repositories.proactiveSurfacing;
    if (!repo) {
        reject('Proactive surfacing repository is not available.', 'preflight_failed');
        return;
    }
