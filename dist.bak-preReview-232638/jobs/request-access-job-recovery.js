...
export async function recheckPausedSetupJobsAfterRequestAccessGrant(input) {
    ...
    try {
        ...
        input.logWarn?.({ err, sourceAgentFolder: input.sourceAgentFolder, jobId: input.jobId }, 'Failed to recheck setup-paused jobs after access grant');
        return undefined;
    }
}
export function formatRequestAccessPersistentGrantMessage(input) {
    ...
    if (input.recovery?.queued.length) {
        lines.push(`Job resumed: ${input.recovery.queued
            .map((job) => job.name || job.jobId)
            .join(', ')}.`);
    }
    if (input.recovery?.stillBlocked.length) {
        const blocker = input.recovery.stillBlocked[0];
        lines.push(`Job still needs setup: ${blocker.nextAction ?? 'review job setup'}.`);
    }
    return lines.join('\n');
}
