...
import { isBrowserActionMcpToolRule, isProjectedBrowserMcpToolRule, publicGantryToolNameForSdkTool, RUN_COMMAND_TOOL_NAME, validateReadableAgentToolRule, } from '../shared/agent-tool-references.js';
import { PermissionManagementService, semanticCapabilityDefinitionsFromToolCatalog, } from '../application/permissions/permission-management-service.js';
import { skillActionDefinitionsForAgent } from '../application/agents/agent-capability-skill-actions.js';
import { formatDurableAccessRulesForUser, isDurableAccessRuleAllowed, } from '../shared/durable-access-policy.js';
import { expandSemanticCapabilityPermissionRules, } from '../shared/semantic-capabilities.js';
import { normalizePersistentBashRuleContent } from '../shared/bash-command-parser.js';
import { isValidSemanticCapabilityId, semanticCapabilityRule, } from '../shared/semantic-capability-ids.js';
import { formatApprovalRequestedMessage } from '../shared/user-visible-messages.js';
export function requestPermissionQueuedMessage(review) {
    return `${formatApprovalRequestedMessage(review.displayName)} Choose one of the options in the approval prompt.`;
}
export function requestPermissionDescription() {
...
export function requestPermissionReviewEffect(toolInput, fallback) {
...
export async function persistRequestPermissionRules(input) {
    ...
    if (!input.appId || !input.agentId) {
        throw new Error('Persistent permission approval requires explicit appId and agentId');
    }
    const repository = input.deps.getToolRepository?.();
    if (!repository) {
        throw new Error('Tool repository unavailable for persistent permission approval');
    }
    const mirrorAgentToolRulesToSettings = input.deps.mirrorAgentToolRulesToSettings;
    if (!mirrorAgentToolRulesToSettings) {
        throw new Error('Settings mirror unavailable for persistent permission approval');
    }
...
export function isPermanentPermissionDecision(decision) {
...
export function requestPermissionReviewSuggestions(toolInput, options = {}) {
    if (toolInput.temporaryOnly === true)
        return undefined;
    ...
    const toolNames = sanitizedStringList(Array.isArray(toolInput.toolNames)
        ? toolInput.toolNames
        : [toolInput.toolName]);
    if (capabilityId && toolNames.length === 0) {
        ...
        if (options.semanticCapabilityDefinitions &&
            !isDurableAccessRuleAllowed(publicToolRule, {
                semanticCapabilityDefinitions: options.semanticCapabilityDefinitions,
            })) {
            return undefined;
        }
        const [publicToolName, publicRuleContent] = splitReadableToolRule(publicToolRule);
        return [
            {
                type: 'addRules',
                behavior: 'allow',
    ...
    if (rawToolName.includes('(') || rawToolName.includes(')'))
        return undefined;
    const publicToolName = isProjectedBrowserMcpToolRule(rawToolName)
        ? 'Browser'
        : publicGantryToolNameForSdkTool(rawToolName);
    const ruleContent = canonicalRequestPermissionCommandRule(publicToolName, strictRuleContent(toolInput.rule));
    if (ruleContent === null)
        return undefined;
    if (isBrowserActionMcpToolRule(rawToolName)) {
        return undefined;
    }
    const publicToolRule = publicToolName === RUN_COMMAND_TOOL_NAME && ruleContent
    ...
    const [suggestedToolName, publicRuleContent] = splitReadableToolRule(publicToolRule);
    return [
        {
            type: 'addRules',
            behavior: 'allow',
...
export function requestPermissionTransientLiveRules(toolInput) {
    if (toolInput.temporaryOnly !== true)
        return [];
    if (toolInput.permissionKind && toolInput.permissionKind !== 'tool')
        return [];
...
export async function resolveTrustedSemanticCapabilityDefinitions(input) {
    ...
    if (toolRepository && typeof toolRepository.listTools === 'function') {
        ...
        if (catalog)
            Object.assign(definitions, catalog);
    }
    const skillRepository = input.deps.getSkillRepository?.();
    if (skillRepository) {
        Object.assign(definitions, await skillActionDefinitionsForAgent({
            appId: input.appId,
            agentId: input.agentId,
            skillRepository,
        }));
...
export function requestPermissionOnceLiveRules(toolInput, definitions) {
...
export function requestPermissionSetupDecisionOptions(toolInput, options = {}) {
...
export function validateRequestPermissionSemanticCapability(toolInput) {
    const capabilityId = toTrimmedString(toolInput.capabilityId, {
        maxLen: 160,
    });
    if (!capabilityId)
...
export function validateRequestPermissionCapabilityProposal(input) {
    if (!input.capabilityId || input.toolNames.length > 0)
        return undefined;
    ...
    if (Object.prototype.hasOwnProperty.call(input.toolInput, 'semanticCapabilityDefinition') ||
        Object.prototype.hasOwnProperty.call(input.toolInput, 'capabilityDefinition')) {
        return 'Capability definitions are host-owned catalog metadata and cannot be supplied in request_permission input.';
    }
    return undefined;
}
function strictRuleContent(value) {
...
function canonicalRequestPermissionCommandRule(toolName, ruleContent) {
    if (toolName !== RUN_COMMAND_TOOL_NAME || !ruleContent)
        return ruleContent;
