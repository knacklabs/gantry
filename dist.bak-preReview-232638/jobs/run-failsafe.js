export async function completeFailedRunFailsafe(input) {
    try {
        ...
        if (!finalizeRunLease) {
            input.logger.warn({ jobId: input.jobId, runId: input.runId }, 'Skipped run failsafe terminal write: lease finalization is unavailable');
            return;
        }
        ...
        if (!finalized) {
            input.logger.warn({ jobId: input.jobId, runId: input.runId }, 'Skipped run failsafe terminal write: lease is no longer held');
            return;
        }
        ...
        input.logger.warn({ err, jobId: input.jobId, runId: input.runId }, 'Failed to record scheduler run failsafe terminal state');
    }
}
