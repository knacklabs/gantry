export function createRunProviderMetadataUpdater(input) {
    let persistedProviderRunId;
    let persistedProviderSessionId;
    let pendingProviderRunId;
    let pendingProviderSessionId;
    let lastProviderMetadataUpdateMs = 0;
    return async (metadata) => {
        const updateMetadata = input.opsRepository.updateAgentRunProviderMetadata?.bind(input.opsRepository);
        if (metadata.providerRunId !== undefined &&
            metadata.providerRunId !== persistedProviderRunId) {
            pendingProviderRunId = metadata.providerRunId;
        }
        if (metadata.providerSessionId !== undefined &&
        ...
        if (!updateMetadata) {
            if (metadata.force) {
                throw new Error('Cannot force-persist scheduler run provider metadata: repository does not implement provider metadata updates.');
            }
            return;
        }
        ...
        const runIds = sessionRunId && sessionRunId !== input.outerRunId
        ...
        try {
            ...
            if (input.leaseToken && !updated) {
                throw new Error('Scheduler run lease is no longer active during provider metadata persistence.');
            }
            if (sessionRunId && sessionRunId !== input.outerRunId) {
                ...
                if (input.leaseToken && !sessionUpdated) {
                    throw new Error('Scheduler run lease is no longer active during session run provider metadata persistence.');
                }
            }
            if (pendingProviderRunId !== undefined) {
            ...
            input.logger.warn({ err, jobId: input.jobId, runIds }, 'Failed to update scheduler run provider metadata');
            if (metadata.force) {
                throw err;
            }
        }
