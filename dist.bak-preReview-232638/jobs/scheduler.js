...
import { getRuntimeControlRepository, getRuntimeEventExchange, getRuntimeRepositories, getRuntimeStorage, getWorkerCoordinationRepository, } from '../adapters/storage/postgres/runtime-store.js';
import { PgBossSchedulerEngine, enqueueSchedulerTriggerDelivery, ensureSchedulerQueues, } from '../infrastructure/pgboss/scheduler-engine.js';
import { STORAGE_POSTGRES_SCHEMA, STORAGE_POSTGRES_URL, } from '../config/index.js';
import { configureRunSlotBackend, resetSchedulerRunSlots, } from './concurrency.js';
import { registerWorkerInstance, stopWorkerHeartbeat, } from './worker-identity.js';
...
import { _setMemoryMaintenanceQueueForTests, registerSystemJobs, resetSystemJobStateForTests, } from './system-jobs.js';
let activeSchedulerEngine = null;
let schedulerRunning = false;
let sendOnlyPgBossFactory = (options) => new PgBoss(options);
/** @internal test hook */
export function _setSendOnlyPgBossFactoryForTests(factory) {
...
export function markRoleHasNoJobExecution() {
 ...
 */
export function schedulerNotReadyReason() {
    if (!roleHasNoJobExecution)
        return undefined;
    return ('This process role cannot enqueue job triggers without a configured ' +
...
export function requestSchedulerSync(jobId) {
    activeSchedulerEngine?.requestSync(jobId);
}
export async function enqueueJobTrigger(jobId, triggerId, options) {
...
async function enqueueJobTriggerFromNonExecutingRole(jobId, triggerId, options) {
...
export async function startSchedulerLoop(deps) {
    if (schedulerRunning)
        return;
    ...
    const resolvedDeps = {
        ...
        hasLiveAdmissionBacklog: deps.hasLiveAdmissionBacklog ?? hasQueuedLiveAdmissionWork,
    };
    ...
    const engine = new PgBossSchedulerEngine(resolvedDeps, {
        ...
        handleReleasedStaleLeases: (releases, callbackDeps) => notifyReleasedStaleJobLeases({
            releases,
    ...
    try {
        ...
        await engine
            .stop()
            .catch((stopErr) => logger.warn({ err: stopErr }, 'Failed to stop pg-boss after startup failure'));
        throw err;
    }
}
async function hasQueuedLiveAdmissionWork() {
    const result = await getRuntimeStorage().service.pool.query(`SELECT EXISTS (
...
export function isJobTriggerQueueReady() {
