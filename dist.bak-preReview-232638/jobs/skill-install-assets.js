...
export function skillInstallCommandReceipt(result) {
    const lines = [];
    ...
    if (result.skills.length > 0) {
        lines.push(`I installed ${result.skills.map((skill) => skill.name).join(', ')}.`);
    }
    if (result.failed.length > 0) {
        const names = result.failed.map(({ name }) => name).join(', ');
        lines.push(`I couldn't install ${names}. I left ${result.failed.length === 1 ? 'it' : 'them'} unchanged and can try again after the setup issue is fixed.`);
    }
    if (result.skills.length > 1) {
        lines.push('The installed skill content is shared with me in this conversation now, up to a size budget; every installed skill is registered and loads automatically from your next message.');
    }
    if (result.skippedBeyondLimit) {
        lines.push(`I stopped after ${MAX_SKILLS_PER_INSTALL_COMMAND} skills because one request cannot install more than that.`);
    }
    lines.push(...formatDeclaredGantrySecretLines([
        ...new Set(result.skills.flatMap((skill) => skill.requiredEnvVars ?? [])),
    ], 'The installed skill set'));
    return lines.join('\n');
}
export function safeInstallerEnv(source) {
    const env = {};
    for (const key of [
        'PATH',
        'HOME',
...
export function boundedSkillInstallFailureReason(err) {
...
export function discoverInstalledSkillRoots(stagingDir) {
    const rootSkillMarkdown = path.join(stagingDir, 'SKILL.md');
    if (fs.existsSync(rootSkillMarkdown) &&
...
export function collectInstalledSkillAssets(root) {
    const assets = [];
    const stack = [
    ...
    while (stack.length > 0) {
        ...
        if (item.directory) {
            ...
            const entries = readBoundedDirectoryEntries(item.path, MAX_SKILL_TRAVERSAL_ENTRIES - visitedEntries, 'Installed skill package exceeds the traversal limit.');
        ...
        if (assets.length === MAX_SKILL_PACKAGE_FILES) {
            throw new Error(`Installed skill package exceeds the ${MAX_SKILL_PACKAGE_FILES}-file limit.`);
        }
...
export function skillNameForReceipt(assets, fallback) {
    const skillMarkdown = assets.find((asset) => asset.path === 'SKILL.md');
    return skillMarkdown
        ? (readSkillFrontmatterName(Buffer.from(skillMarkdown.content).toString('utf-8')) ?? fallback)
        : fallback;
}
export async function snapshotInstalledSkill(input) {
    const key = materializedSkillDirectoryNameFor(input.name).toLowerCase();
    const skill = (await input.skills.listSkills({
        appId: input.appId,
        statuses: ['installed'],
    })).find((candidate) => materializedSkillDirectoryNameFor(candidate.name).toLowerCase() === key);
    if (!skill?.storage)
        return undefined;
    const binding = (await input.skills.listAgentSkillBindings({
        appId: input.appId,
...
export function reportUnattemptedSkillRoots(result, roots, currentRoot) {
...
export async function rollbackFreshInstallBinding(input) {
    ...
    try {
        ...
        const cleanupFailure = boundedSkillInstallFailureReason(`${reason} Cleanup failed: ${boundedSkillInstallFailureReason(rollbackError)}`);
        if (await input.isBindingActive()) {
            try {
                ...
                reason = boundedSkillInstallFailureReason(`${cleanupFailure} Reconciliation failed while the binding remains active: ${boundedSkillInstallFailureReason(reconciliationError)}`);
    ...
    if (syncAttempted) {
        try {
            ...
            reason = boundedSkillInstallFailureReason(`${reason} Reconciliation failed: ${boundedSkillInstallFailureReason(reconciliationError)}`);
            stopAfterFailure = true;
        }
    }
...
export async function rollbackInstalledSkillReplacement(input) {
    try {
        ...
        const currentBinding = (await input.skills.listAgentSkillBindings({
        ...
        const currentFingerprint = currentSkill?.storage
            ? await input.artifacts
                ...
                .catch(() => undefined)
            : undefined;
        const unchanged = isDeepStrictEqual(currentSkill, input.snapshot.skill) &&
        ...
        try {
            ...
            if (!isDeepStrictEqual(currentSkill, input.snapshot.skill) &&
        ...
        return {
            reason: boundedSkillInstallFailureReason(`${input.reason} Restore failed: ${boundedSkillInstallFailureReason(restoreError)}`),
            stopAfterFailure: true,
        };
    }
}
...
export function installedSkillContext(installed) {
    ...
    return {
        ...
        ...skillEntry(first),
        requiredEnvVars: [
            ...new Set(installed.flatMap((entry) => entry.skill.requiredEnvVars ?? [])),
        ],
        ...(rest.length > 0 ? { additionalSkills: rest.map(skillEntry) } : {}),
    };
}
function findNestedSkillRoots(start, roots) {
    const stack = [start];
    ...
    while (stack.length > 0) {
        ...
        if (fs.existsSync(skillMarkdown) && fs.statSync(skillMarkdown).isFile()) {
            const name = readSkillFrontmatterName(readSkillMarkdownPrefix(dir)) ??
                path.basename(dir);
            ...
            if (previousFingerprints) {
                if (fingerprint && previousFingerprints.has(fingerprint))
                    continue;
                if (fingerprint)
                    previousFingerprints.add(fingerprint);
            }
            else {
                seenFingerprints.set(key, new Set(fingerprint === undefined ? [] : [fingerprint]));
            }
            if (roots.length === MAX_SKILLS_PER_INSTALL_COMMAND)
                return true;
            roots.push(dir);
            continue;
        }
        const entries = readBoundedDirectoryEntries(dir, MAX_SKILL_TRAVERSAL_ENTRIES - visitedEntries, 'Installer output exceeds the skill discovery limit.');
        visitedEntries += entries.length;
        for (let index = entries.length - 1; index >= 0; index -= 1) {
            const entry = entries[index];
...
function readSkillMarkdownPrefix(root) {
...
function safeSkillPackageFingerprint(root) {
    try {
...
function skillArtifactBundleFingerprint(bundle) {
    const hash = createHash('sha256');
    ...
    for (const asset of [...bundle.assets].sort((left, right) => compareText(left.path, right.path))) {
        hash.update(asset.path).update('\0').update(asset.content).update('\0');
    }
    return hash.digest('hex');
}
const IGNORED_ENTRIES = new Set(['.git', 'node_modules', '.DS_Store']);
function readBoundedDirectoryEntries(dir, remaining, limitError) {
    const output = [];
