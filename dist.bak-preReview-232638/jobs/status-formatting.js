...
export function formatRunStatusMessage(args) {
    ...
    const duration = args.durationMs === undefined
        ? ''
...
function statusEmoji(statusText) {
    switch (statusText) {
        case 'Completed':
...
export function selectJobNotificationSummary(summary) {
    const normalized = summary.replace(/^\[output truncated; showing tail\]\s*/i, '');
    const markers = [
        '## Final Job Report',
        '# Final Job Report',
        '## Scoring Summary',
    ...
    const selected = markerIndex >= 0 ? normalized.slice(markerIndex) : normalized;
    return stripTrailingEmptyReceiptLines(selected).trim() || summary;
}
function statusLabel(status, summary, denial) {
    if (denial)
        return 'Needs permission';
    if (status === 'paused')
        return 'Needs permission';
    if (status === 'completed') {
        if (realNeedsAttention(summary))
            return 'Completed with issues';
        if (hasPendingMemoryReviewSummary(summary))
            return 'Needs memory review';
...
function compactSummary(summary, max = 180) {
...
function stripDiagnosticSuffix(summary) {
    return summary.replace(/\nDiagnostics:[\s\S]*$/i, '');
}
function humanizeJsonSummary(summary) {
    ...
    try {
        ...
        if ('queued' in record &&
            'pending' in record &&
            'deduped' in record &&
            Object.keys(record).every((key) => ['queued', 'pending', 'deduped'].includes(key))) {
            return record.deduped
                ? 'Memory maintenance was already running for this conversation.'
                : 'Memory maintenance completed.';
        }
        const usefulEntries = Object.entries(record).filter(([, value]) => value !== null &&
            value !== undefined &&
            (typeof value === 'string' ||
                typeof value === 'number' ||
...
function notificationOutcome(summary, status, denial) {
    if (denial) {
...
function notificationAction(status, summary, denial) {
    if (denial) {
        if (denial.toolName.startsWith('mcp__gantry__browser_')) {
...
function hasPendingMemoryReviewSummary(summary) {
    return (/\b\d+\s+sent to review\b/i.test(summary) ||
        /\b\d+\s+(?:pending\s+)?memory reviews?\s+(?:are\s+)?(?:waiting|pending|needs? review)\b/i.test(summary));
}
function pendingMemoryReviewCount(summary) {
...
function nextRunLabel(nextRun, status) {
    const formattedNextRun = nextRun ? formatNextRun(nextRun) : null;
    if (formattedNextRun)
        return `Runs again ${formattedNextRun}.`;
    if (status === 'completed')
        return null;
...
function stripTrailingEmptyReceiptLines(summary) {
    const lines = summary.split('\n');
    const emptyReceipt = /^(?:Used:\s*none(?: reported)?|Changed:\s*none|Delegated:\s*no|Needs attention:\s*(?:none|no|n[/-]a))\.?$/i;
    while (lines.length) {
        const tail = lines.at(-1)?.trim() ?? '';
        if (tail !== '' && !emptyReceipt.test(tail))
            break;
        lines.pop();
    }
    return lines.join('\n');
}
function realNeedsAttention(summary) {
    for (const match of summary.matchAll(/^Needs attention:\s*(.*?)\s*$/gim)) {
        if (hasMeaningfulReceiptValue(match[1]))
            return match[1].trim();
    }
    return null;
}
function hasMeaningfulReceiptValue(value) {
    return (Boolean(value?.trim()) && !/^(?:none|no|n[/-]a)\.?$/i.test(value.trim()));
}
