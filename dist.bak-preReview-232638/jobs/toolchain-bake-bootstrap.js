import { ARTIFACTS_DIR, STORAGE_POSTGRES_SCHEMA, STORAGE_POSTGRES_URL, getDeploymentMode, getRuntimeSettingsForConfig, } from '../config/index.js';
...
import { enqueueToolchainBake, } from './toolchain-bake-enqueue.js';
 ...
 */
export async function startToolchainBakeSubsystem(deps) {
    if (getDeploymentMode() !== 'fleet')
        return null;
    if (activeQueue)
        return activeQueue;
    if (!STORAGE_POSTGRES_URL) {
        throw new Error('Postgres URL is required to start the toolchain bake queue');
    }
    const storage = getRuntimeStorage();
    const notifier = new PostgresToolchainManifestNotifier(storage.service.pool, (context, message) => logger.warn(context, message));
    const executorDeps = {
...
export async function stopToolchainBakeSubsystem() {
...
export async function maybeEnqueueApprovedDependencyBake(input) {
    ...
    const queue = activeQueue;
    if (!queue) {
        throw new Error('Toolchain bake queue is not running; cannot enqueue dependency bake in fleet mode.');
    }
    const storage = getRuntimeStorage();
...
function createToolchainArtifactStore() {
