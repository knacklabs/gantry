 ...
 */
export async function enqueueToolchainBake(deps, input) {
    const manifest = normalizeToolchainManifest({
        packages: input.packages,
        registry: deps.registry,
    });
    const existing = await deps.runtimeDependencies.getRuntimeDependencyByManifestHash({
        appId: input.appId,
    ...
    if (existing && existing.status === 'failed') {
        ...
        const requeued = (await deps.runtimeDependencies.getRuntimeDependency(existing.id)) ??
            existing;
