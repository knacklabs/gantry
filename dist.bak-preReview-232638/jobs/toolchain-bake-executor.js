...
import { NATIVE_MODULE_SCRIPT_ALLOWLIST, renderBakeNpmrc, renderBakePackageJson, } from './toolchain-bake-manifest.js';
 ...
 */
export async function executeToolchainBake(deps, input) {
    const dependency = await deps.runtimeDependencies.getRuntimeDependency(input.dependencyId);
    if (!dependency) {
    ...
    try {
        ...
        const uploaded = await deps.runtimeDependencies.updateRuntimeDependencyStatus({
            id: dependency.id,
            status: 'uploaded',
            fromStatus: 'baking',
            artifact: {
        ...
        try {
            ...
            deps.logWarn?.({ err: noticeErr, dependencyId: dependency.id }, 'Failed to deliver toolchain bake success notice');
        }
        ...
        if (!failed) {
            ...
            deps.logWarn?.({ dependencyId: dependency.id, reason }, 'Toolchain bake lost its row before the failed write; dropping outcome');
            return { result: 'skipped', reason: 'not_claimable' };
        }
        await deps.notifier.notifyManifestChanged({
            appId: dependency.appId,
        ...
        try {
            ...
            deps.logWarn?.({ err: noticeErr, dependencyId: dependency.id }, 'Failed to deliver toolchain bake failure notice');
        }
...
async function writeBakeInputs(workDir, packages, registry) {
    await fs.writeFile(path.join(workDir, 'package.json'), renderBakePackageJson(packages), { mode: 0o600 });
    await fs.writeFile(path.join(workDir, '.npmrc'), renderBakeNpmrc(registry), {
        mode: 0o600,
    });
}
async function runNpmInstall(deps, dependency, workDir) {
    ...
    if (result.exitCode !== 0) {
        throw new Error(`npm install failed (exit ${result.exitCode}) for ${dependency.requestedPackages.join(', ')}: ${tail(result.stderr || result.stdout)}`);
    }
...
async function packToolchain(workDir) {
    const files = [];
...
function bakeEnv() {
    ...
    const env = { npm_config_audit: 'false' };
