...
import { DEFAULT_INSTALL_TIMEOUT_MS, } from './toolchain-bake-executor.js';
/**
...
export function bakeReapStalenessMs(installTimeoutMs = DEFAULT_INSTALL_TIMEOUT_MS) {
 ...
 */
export async function resetToolchainBakeForRequeue(deps, input) {
...
export class ToolchainBakeReaper {
    ...
    start() {
        ...
        const timer = setIntervalFn(() => void this.tick(), this.deps.intervalMs ?? DEFAULT_BAKE_REAP_INTERVAL_MS);
        timer.unref?.();
        this.timer = timer;
        void this.tick();
    }
    ...
    async reap() {
        ...
        const stale = await this.deps.runtimeDependencies.listStaleRuntimeDependencies({
        ...
        for (const dependency of stale) {
            try {
                ...
                this.deps.logWarn?.({ err, dependencyId: dependency.id }, 'Failed to reap stale toolchain bake');
            }
        }
        return result;
    }
