 ...
 */
export class PostgresManifestWakeupSource {
    pool;
    logWarn;
    listeners = new Set();
    clientPromise = null;
    ...
    async ensureListening() {
        ...
        try {
            ...
            client.on('notification', (message) => {
                if (message.channel !== TOOLCHAIN_MANIFEST_CHANNEL)
                    return;
                this.wakeListeners();
            });
            client.on('error', (err) => {
            ...
            this.logWarn?.({ err }, 'Failed to start toolchain manifest LISTEN client');
            if (client)
