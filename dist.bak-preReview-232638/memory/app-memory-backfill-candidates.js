...
function itemValueExpr() {
    return sql `COALESCE(${Items.valueJson}->>'value', '')`;
}
function itemWhyExpr() {
    return sql `COALESCE(${Items.valueJson}->>'why', '')`;
}
function itemEmbeddingContentHashExpr() {
    return sql `encode(digest(${Items.key} || E'\n' || ${itemValueExpr()} || E'\n' || ${itemWhyExpr()}, 'sha256'), 'hex')`;
}
function activeItemWhere(scope) {
    return and(eq(Items.appId, scope.appId), eq(Items.status, 'active'), scope.agentId ? eq(Items.agentId, scope.agentId) : undefined);
}
 ...
 */
export async function selectBackfillCandidates(db, scope) {
    ...
    if (itemRows.length === 0) {
        const [readyRow] = await db
            ...
            .from(Items)
            .where(and(baseWhere, hasCurrentReadyEmbedding({
            provider: scope.provider,
            model: scope.model,
            dimensions: scope.dimensions,
        })));
