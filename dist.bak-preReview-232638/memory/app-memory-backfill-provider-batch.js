...
export function supportsProviderBatch(provider) {
...
export async function submitProviderEmbeddingBatch(input) {
    if (input.candidates.length === 0 || !input.provider.batch)
        return 0;
    const { batchId } = await input.provider.batch.submitBatch(input.candidates.map((candidate) => ({
        customId: candidate.itemId,
        input: candidate.text,
    })), { signal: input.signal });
    const stamp = input.now();
    for (const candidate of input.candidates) {
        await markEmbeddingState(input.db, keyFor(input, candidate.itemId, candidate.contentHash, input.dimensions), 'submitted', stamp, {
            runId: input.runId,
...
export async function pollAndImportProviderBatches(input) {
    ...
    const maxBatches = Math.max(1, input.maxBatches ?? DEFAULT_MAX_BATCHES_PER_POLL);
    const summary = {
        batchesPolled: 0,
        imported: 0,
        retried: 0,
    ...
    const submitted = (await input.db
        ...
        .from(Embeddings)
        .where(and(eq(Embeddings.provider, input.providerName), eq(Embeddings.model, input.model), eq(Embeddings.status, 'submitted'), isNotNull(Embeddings.providerBatchId)))
        .orderBy(asc(Embeddings.updatedAt), asc(Embeddings.providerBatchId), asc(Embeddings.itemId))
        .limit(maxRows));
    ...
    for (const [batchId, rows] of batches.slice(0, maxBatches)) {
        ...
        try {
            ...
            logger.warn({ batchId, err: error }, 'failed to poll provider embedding batch; will retry next run');
        ...
        for (const row of rows) {
            ...
            if (result?.embedding && result.embedding.length === row.dimensions) {
                const stored = await writeReadyEmbedding(input.db, key, result.embedding, stamp, row.runId);
                if (stored) {
                    summary.imported += 1;
                }
                else {
                ...
                await markEmbeddingState(input.db, key, 'blocked_invalid_dimension', stamp, {
                    runId: row.runId,
                ...
                await markEmbeddingState(input.db, key, 'retryable_error', stamp, {
                    ...
                    resumeAfter: new Date(Date.parse(stamp) + IMPORT_RETRY_BACKOFF_MS).toISOString(),
                });
                summary.retried += 1;
            }
        }
    ...
    for (const runId of affectedRunIds) {
        await finalizeRunFromRows(input.db, runId, input.providerName, input.model, now());
    }
    return summary;
}
async function finalizeRunFromRows(db, runId, providerName, model, stamp) {
    const [run] = await db
    ...
    const rows = await db
        ...
        .from(Embeddings)
        .where(and(eq(Embeddings.runId, runId), eq(Embeddings.provider, providerName), eq(Embeddings.model, model)));
    let ready = 0;
    let retryable = 0;
    let blocked = 0;
    let pending = 0;
    ...
    for (const row of rows) {
        if (row.status === 'ready')
            ready += 1;
        else if (row.status === 'retryable_error')
            retryable += 1;
        else if (row.status === 'blocked_invalid_dimension')
            blocked += 1;
        else if (row.status === 'stale_content')
            stale += 1;
        else if (row.status === 'submitted' || row.status === 'processing')
            pending += 1;
    }
    if (pending > 0)
        return; // batch still in flight; leave run running.
    ...
    await finalizeBackfillRun(db, runId, {
        ...
        resumeAfter: retryable > 0
            ? new Date(Date.parse(stamp) + IMPORT_RETRY_BACKOFF_MS).toISOString()
            : null,
        now: stamp,
    });
}
function keyFor(input, itemId, contentHash, dimensions) {
    return {
        itemId,
