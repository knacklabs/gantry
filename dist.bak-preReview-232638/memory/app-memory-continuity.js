...
export async function buildAppMemoryContinuityStatus(memory, input = {}) {
...
export async function buildAppMemoryContinuitySummary(memory, input = {}) {
    const subject = normalizeSubject(input);
    const startedAtMs = input.nowMs ?? currentTimeMs();
    const hasBlockedDreamDecisionSection = memory.listRecentBlockedDreamDecisions !== undefined;
    const [memoriesResult, runsResult, reviewsResult, blockedResult] = await Promise.all([
        settleContinuitySection((signal, statementTimeoutMs) => memory.list({ ...subject, limit: 100 }, { signal, statementTimeoutMs }), input.deadlineAtMs, startedAtMs, input.signal, input.statementTimeoutMs),
        settleContinuitySection((signal, statementTimeoutMs) => memory.dreamingStatus(subject, { signal, statementTimeoutMs }), input.deadlineAtMs, startedAtMs, input.signal, input.statementTimeoutMs),
        settleContinuitySection((signal, statementTimeoutMs) => memory.listPendingReviews(subject, { signal, statementTimeoutMs }), input.deadlineAtMs, startedAtMs, input.signal, input.statementTimeoutMs),
        hasBlockedDreamDecisionSection
            ? settleContinuitySection((signal, statementTimeoutMs) => memory.listRecentBlockedDreamDecisions?.(subject, {
                signal,
                statementTimeoutMs,
                limit: 10,
            }) ?? Promise.resolve([]), input.deadlineAtMs, startedAtMs, input.signal, input.statementTimeoutMs)
    ...
    const unavailableCount = sectionResults.filter((result) => result.status === 'unavailable').length;
    const sectionCount = sectionResults.length;
    return {
        overall_status: unavailableCount === 0
            ? 'complete'
        ...
        sections: {
            recent_decisions: section(memoriesResult.status === 'unavailable'
                ? 'unavailable'
                : recentDecisions.length > 0
                    ? 'populated'
                    : 'empty', recentDecisions, memoriesResult.status === 'unavailable'
                ? memoriesResult.reason
                : undefined),
            active_paused_jobs: sectionFromInjected(injected?.sections.active_paused_jobs),
            last_runs: sectionFromInjected(injected?.sections.recent_session_digests, 'No session digest loader was available for this subject.'),
            last_dream_summary: section(runsResult.status === 'unavailable'
                ? 'unavailable'
                : lastRun && latestDreamSummary
                    ? 'populated'
                    : 'empty', lastRun && latestDreamSummary
                ? [
                    {
            ...
            blocked_dream_decisions: section(blockedResult.status === 'unavailable'
                ? 'unavailable'
                : blocked.length > 0
                    ? 'populated'
                    : 'empty', blocked.map((item) => ({
                id: item.id,
                run_id: item.runId,
                ...
                : undefined),
            issue_index: section('deferred', [], 'No issue index repository is wired into memory continuity.'),
        },
    };
}
function statusFromParts(subject, runs, reviewCount) {
...
function injectedStatus(subject) {
...
function section(status, items, reason) {
    return { status, count: items.length, items, ...(reason ? { reason } : {}) };
}
function sectionFromInjected(source, reason = 'No continuity jobs loader has injected data for this subject.') {
    return source
        ? {
...
async function settleContinuitySection(work, deadlineAtMs, nowMs, parentSignal, statementTimeoutMs) {
    ...
    const abortFromParent = () => controller.abort(parentSignal?.reason ?? new Error('memory IPC aborted'));
    parentSignal?.addEventListener('abort', abortFromParent, { once: true });
    const effectiveStatementTimeoutMs = statementTimeoutMs ??
