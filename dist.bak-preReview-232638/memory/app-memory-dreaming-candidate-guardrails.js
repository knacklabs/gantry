...
const CANONICAL_DREAM_KINDS = new Set([
    'preference',
    'decision',
    'fact',
    'correction',
...
const CANONICAL_DREAM_SCOPES = new Set([
    'user',
    'group',
    'global',
]);
...
const TRANSIENT_MEMORY_PATTERN = /\b(today|tomorrow|right now|for now|temporary|temp|quick fix|debugging this run|in this turn|next step|later today)\b/i;
...
function scopeMatchesSubject(scope, subject) {
    if (scope === 'user')
        return subject.subjectType === 'user';
    if (scope === 'global')
        return subject.subjectType === 'common';
    return subject.subjectType === 'group' || subject.subjectType === 'channel';
}
function isSafetyMarkedSafe(value) {
    if (value === 'safe' || value === 'validated')
        return true;
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return false;
    const record = value;
    if (record.status === 'safe' || record.status === 'validated')
        return true;
    if (record.classification === 'safe')
        return true;
    return record.blocked === false && record.reason === undefined;
}
function findStructuredCandidatePayload(metadata) {
...
export function parseStructuredEvidenceCandidate(evidence, subject) {
    ...
    if (!value)
        return { rejection: 'candidate value is missing or too long' };
    ...
    const operationRaw = trimString(payload.operation ?? payload.action, 16)?.toLowerCase() ||
        'promote';
    if (operationRaw !== 'promote' && operationRaw !== 'retire') {
        return { rejection: 'candidate operation must be promote or retire' };
    }
    const retireKey = trimString(payload.retire_key ?? payload.retireKey, 128) || key;
...
export function parseStagedCandidateMetadata(candidate) {
    const metadata = parseJsonObject(candidate.metadataJson);
    const operation = trimString(metadata.operation ?? metadata.action, 16)?.toLowerCase() ||
        'promote';
    const retireKey = trimString(metadata.retire_key ?? metadata.retireKey, 128) || candidate.key;
    if (operation === 'retire') {
        return {
            operation: 'retire',
            retireKey,
...
export function validatePromotableCandidate(candidate) {
    if (!CANONICAL_DREAM_KINDS.has(candidate.kind)) {
        return {
            ok: false,
    ...
    if (candidate.kind === 'preference') {
        return {
            ok: false,
            needsReview: true,
            rationale: 'Staged preference candidates require review before becoming durable memory.',
        };
    }
    const metadata = parseJsonObject(candidate.metadataJson);
    if (metadata.requiresReview === true ||
        ...
        return {
            ok: false,
            needsReview: true,
            rationale: 'Staged candidate is marked risky and requires memory review before promotion.',
        };
    }
    if (metadata.contradiction === true ||
        ...
        return {
            ok: false,
            needsReview: true,
            rationale: 'Staged candidate may contradict existing memory and requires review before promotion.',
        };
    }
    if (!KEY_PATTERN.test(candidate.key) ||
        candidate.key.startsWith('evidence:')) {
        return {
            ok: false,
            rationale: 'Staged candidate key is not canonical; raw evidence keys are not promotable.',
        };
    }
    ...
    if (!candidate.value.trim() ||
        ...
        return {
            ok: false,
            rationale: 'Staged candidate value failed durability or safety guardrails.',
        };
    }
    if (candidate.reason &&
...
export function extractMemoryValue(row) {
