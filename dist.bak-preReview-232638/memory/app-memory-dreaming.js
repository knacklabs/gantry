...
import { extractMemoryValue, parseStagedCandidateMetadata, parseStructuredEvidenceCandidate, validatePromotableCandidate, } from './app-memory-dreaming-candidate-guardrails.js';
import { isUnsafeEvidence, parseJsonArray, parseJsonObject, } from './app-memory-dreaming-evidence.js';
import { nowIso as currentIso } from '../shared/time/datetime.js';
import { recordDreamDecision, routeMemoryProposalToReview, setCandidateStatus, } from './app-memory-dreaming-review-routing.js';
function nowIso() {
    return currentIso();
}
async function candidateCitesUnsafeEvidence(db, evidenceIds) {
...
// prettier-ignore
function candidateMemoryKind(kind) { return ['preference', 'decision', 'fact', 'correction', 'constraint'].includes(kind) ? kind : undefined; }
...
export async function runAppMemoryDreamPass(input) {
    ...
    const recentEvidence = await db
        .select()
        .from(pgSchema.memoryEvidencePostgres)
        .where(and(eq(pgSchema.memoryEvidencePostgres.appId, subject.appId), eq(pgSchema.memoryEvidencePostgres.agentId, subject.agentId), eq(pgSchema.memoryEvidencePostgres.subjectType, subject.subjectType), eq(pgSchema.memoryEvidencePostgres.subjectId, subject.subjectId)))
        .orderBy(desc(pgSchema.memoryEvidencePostgres.createdAt))
        .limit(25);
    input.signal?.throwIfAborted();
    const unsafeEvidenceIds = new Set(recentEvidence.filter(isUnsafeEvidence).map((evidence) => evidence.id));
    const safeRecentEvidence = recentEvidence.filter((evidence) => !unsafeEvidenceIds.has(evidence.id));
    ...
    if (phase === 'rem' || phase === 'all') {
        ...
        for (const item of items) {
            ...
            const value = typeof payload.value === 'string' ? payload.value.toLowerCase() : '';
            if (/\b(no longer|instead|actually|correction|wrong)\b/.test(value)) {
                ...
                const action = await routeMemoryProposalToReview({
                    ...
                    proposal: {
                        ...
                        reason: 'REM dreaming found correction language; human or admin review should decide whether to rewrite or retire related memory.',
                        confidence: item.row.confidence,
                        evidenceIds,
                    },
                    reviewRationale: 'REM dreaming routed correction language to memory review',
                    blockRationale: evidenceIds.length > 0
    ...
    if (phase === 'deep' || phase === 'all') {
        ...
        const candidates = await db
            ...
            .where(and(eq(pgSchema.memoryCandidatesPostgres.appId, subject.appId), eq(pgSchema.memoryCandidatesPostgres.agentId, subject.agentId), eq(pgSchema.memoryCandidatesPostgres.subjectType, subject.subjectType), eq(pgSchema.memoryCandidatesPostgres.subjectId, subject.subjectId), eq(pgSchema.memoryCandidatesPostgres.status, 'staged')))
            .orderBy(desc(pgSchema.memoryCandidatesPostgres.confidence))
            .limit(10);
        input.signal?.throwIfAborted();
        const llmDreamingProposals = (await input.proposeDreaming?.({
            evidence: safeRecentEvidence,
            candidates,
            activeItems: activeItems.map((item) => item.row),
        })) || [];
        input.signal?.throwIfAborted();
        const llmConsolidationProposals = (await input.proposeConsolidation?.({
            activeItems: activeItems.map((item) => item.row),
        })) || [];
        input.signal?.throwIfAborted();
        if (!dryRun) {
        ...
        for (const proposal of [
            ...
            if (proposal.action === 'retire' ||
                ...
                const action = await routeMemoryProposalToReview({
                    ...
                    blockRationale: 'LLM proposal blocked because memory review creation failed.',
                });
                decisions.push({ action });
            }
        }
        for (const candidate of candidates) {
            ...
            if (evidenceIds.some((id) => unsafeEvidenceIds.has(id)) ||
                ...
                await blockCandidate({
                    ...
                    rationale: 'Deep dreaming blocked candidate because it cites quarantined or unsafe Memory Source evidence.',
                    evidenceIds,
                    dryRun,
                });
            ...
            if (metadata.operation === 'retire') {
                const retireTarget = activeByKey.get(metadata.retireKey || candidate.key);
                if (!retireTarget) {
                    if (!dryRun) {
                    ...
                    await recordDreamDecision({
                        ...
                        action: 'skip',
                        candidateId: candidate.id,
                        rationale: 'Deep dreaming skipped retire candidate because no active target memory was found.',
                        evidenceIds,
                        applied: false,
                    });
                    decisions.push({ action: 'skip' });
                ...
                if (dryRun) {
                    await recordDreamDecision({
                        ...
                        candidateId: candidate.id,
                        rationale: 'Deep dreaming dry run would retire the targeted active memory item.',
                        evidenceIds,
                        applied: false,
                    });
                ...
                const action = await routeMemoryProposalToReview({
                    ...
                    reviewRationale: 'Deep dreaming routed retire candidate to memory review',
                    blockRationale: 'Deep dreaming blocked retire candidate because memory review creation failed.',
                });
                decisions.push({ action });
                continue;
            }
            ...
            if (existing && existing.value !== candidate.value) {
                const action = await routeMemoryProposalToReview({
                    ...
                    reviewRationale: 'Deep dreaming routed candidate to memory review because it changes an active memory with the same key',
                    blockRationale: 'Deep dreaming blocked candidate because memory review creation failed.',
                });
                decisions.push({ action });
                continue;
            }
            if (existing &&
                ...
                await recordDreamDecision({
                    ...
                    itemId: existing.id,
                    candidateId: candidate.id,
                    rationale: 'Deep dreaming skipped candidate because active memory already matches the staged value.',
                    evidenceIds,
                    applied: false,
                });
                decisions.push({ action: 'skip' });
            ...
            await setCandidateStatus(db, candidate.id, existing ? 'updated' : 'promoted');
            const action = existing ? 'update' : 'promote';
            await recordDreamDecision({
                db,
                runId,
                subject,
            ...
            if (embeddingResult?.status === 'retryable') {
                await recordDreamDecision({
                    ...
                    itemId: saved.id,
                    candidateId: candidate.id,
                    rationale: embeddingResult.reason ||
