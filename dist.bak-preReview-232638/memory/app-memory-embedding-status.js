...
import { MEMORY_EMBED_DIMENSIONS, MEMORY_EMBED_MODEL, MEMORY_EMBED_PROVIDER, } from '../config/memory.js';
...
function mapPauseReason(reason) {
    switch (reason) {
 ...
 */
export async function getEmbeddingBackfillStatus(db, scope) {
    const enabled = MEMORY_EMBED_PROVIDER !== 'disabled';
    const activeWhere = and(eq(Items.appId, scope.appId), eq(Items.status, 'active'), scope.agentId ? eq(Items.agentId, scope.agentId) : undefined);
    ...
    if (enabled) {
        const [readyRow] = await db
            .select({ count: sql `count(*)::int` })
            .from(Items)
            .where(and(activeWhere, sql `exists (select 1 from ${Embeddings} e where e.item_id = ${Items.id} and e.provider = ${MEMORY_EMBED_PROVIDER} and e.model = ${MEMORY_EMBED_MODEL} and e.dimensions = ${MEMORY_EMBED_DIMENSIONS} and e.status = 'ready' and e.embedding is not null)`));
    ...
    const pauseReason = latestRun?.status === 'paused'
        ? mapPauseReason(latestRun.pauseReason)
        : undefined;
    let searchMode = 'lexical_keyword';
    let vectorSearch = 'inactive';
