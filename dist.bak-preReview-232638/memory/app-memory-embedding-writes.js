...
async function currentContentHashForItem(db, itemId) {
    const [item] = await db
        .select({
        key: Items.key,
 ...
 */
export async function writeReadyEmbedding(db, key, embedding, now, runId = null) {
    ...
    await db
        .delete(T)
        .where(and(eq(T.itemId, key.itemId), eq(T.provider, key.provider), eq(T.model, key.model), ne(T.contentHash, key.contentHash)));
    return true;
}
/**
 ...
 */
export async function markEmbeddingState(db, key, status, now, options = {}) {
    await db
        .insert(T)
        .values({
        itemId: key.itemId,
