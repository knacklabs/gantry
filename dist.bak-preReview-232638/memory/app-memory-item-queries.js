...
import { detectPatternCandidates, } from '../shared/pattern-candidate-detection.js';
import { PATTERN_INTENSIFY_DELTA } from '../shared/pattern-candidate-policy.js';
import { itemMatchesSubjectBoundary, parseJsonObject, parseItemSource, } from './app-memory-canonical-codec.js';
...
export async function listObserverActiveMemoryValues(input) {
    ...
    if (input.subject.startsWith('conversation:')) {
        filters.push(eq(pgSchema.memoryItemsPostgres.conversationId, input.subject));
    }
    ...
    const rows = await input.db
        .select({
        value: sql `${pgSchema.memoryItemsPostgres.valueJson}->>'value'`,
    })
...
export async function findActiveMemoryByKey(input) {
    const rows = await input.db
        .select()
        .from(pgSchema.memoryItemsPostgres)
        .where(and(eq(pgSchema.memoryItemsPostgres.status, 'active'), eq(pgSchema.memoryItemsPostgres.appId, input.subject.appId), eq(pgSchema.memoryItemsPostgres.agentId, input.subject.agentId), eq(pgSchema.memoryItemsPostgres.subjectType, input.subject.subjectType), eq(pgSchema.memoryItemsPostgres.subjectId, subjectIdFor(input.subject)), sql `${pgSchema.memoryItemsPostgres.sourceRefJson} @> ${JSON.stringify({ subject: { agentId: input.subject.agentId, subjectType: input.subject.subjectType, subjectId: input.subject.subjectId } })}::jsonb`, eq(pgSchema.memoryItemsPostgres.key, input.key.trim())))
        .limit(1);
    return rows[0] ?? null;
}
export async function listDreamingStatuses(db, input = {}, options = {}) {
    ...
    const rows = (await withStatementTimeout(db, options.statementTimeoutMs, (timeoutMs) => sql `select set_config('statement_timeout', ${String(timeoutMs)}, true)`, (queryDb) => queryDb
        ...
        .where(and(eq(pgSchema.memoryDreamRunsPostgres.appId, subject.appId), eq(pgSchema.memoryDreamRunsPostgres.agentId, subject.agentId), ...subjectFilters))
        .orderBy(desc(pgSchema.memoryDreamRunsPostgres.startedAt))
        .limit(20)));
    options.signal?.throwIfAborted();
    return rows.map(toRun);
}
export async function listRecentBlockedDreamDecisions(db, input = {}, options = {}) {
    options.signal?.throwIfAborted();
    const hasSubjectScope = hasDreamingStatusSubjectScope(input);
    const subject = normalizeSubject(input);
    const subjectFilters = hasSubjectScope
        ...
        : [];
    const limit = options.limit && Number.isSafeInteger(options.limit) && options.limit > 0
    ...
    const rows = (await withStatementTimeout(db, options.statementTimeoutMs, (timeoutMs) => sql `select set_config('statement_timeout', ${String(timeoutMs)}, true)`, (queryDb) => queryDb
        ...
        itemValue: sql `${pgSchema.memoryItemsPostgres.valueJson}->>'value'`,
        ...
        .innerJoin(pgSchema.memoryDreamRunsPostgres, and(eq(pgSchema.memoryDreamDecisionsPostgres.runId, pgSchema.memoryDreamRunsPostgres.id), eq(pgSchema.memoryDreamRunsPostgres.appId, subject.appId), eq(pgSchema.memoryDreamRunsPostgres.agentId, subject.agentId)))
        .leftJoin(pgSchema.memoryCandidatesPostgres, and(eq(pgSchema.memoryDreamDecisionsPostgres.candidateId, pgSchema.memoryCandidatesPostgres.id), eq(pgSchema.memoryCandidatesPostgres.appId, subject.appId), eq(pgSchema.memoryCandidatesPostgres.agentId, subject.agentId)))
        .leftJoin(pgSchema.memoryItemsPostgres, and(eq(pgSchema.memoryDreamDecisionsPostgres.itemId, pgSchema.memoryItemsPostgres.id), eq(pgSchema.memoryItemsPostgres.appId, subject.appId), eq(pgSchema.memoryItemsPostgres.agentId, subject.agentId)))
        .where(and(eq(pgSchema.memoryDreamDecisionsPostgres.appId, subject.appId), eq(pgSchema.memoryDreamDecisionsPostgres.agentId, subject.agentId), eq(pgSchema.memoryDreamDecisionsPostgres.action, 'blocked'), ...subjectFilters))
...
export async function getOwnedMemoryItem(input) {
    const rows = await input.db
        .select()
        .from(pgSchema.memoryItemsPostgres)
        .where(and(eq(pgSchema.memoryItemsPostgres.id, input.id), eq(pgSchema.memoryItemsPostgres.status, 'active'), eq(pgSchema.memoryItemsPostgres.appId, input.context.appId)))
        .limit(1);
...
export async function deleteOwnedMemoryItem(input) {
    ...
    const [deleted] = await input.db
        ...
        .where(and(eq(pgSchema.memoryItemsPostgres.id, current.id), input.expectedVersion === undefined
        ? undefined
...
export async function demoteDreamingPromotedMemoryItem(input) {
    ...
    if (currentSource.source !== 'dreaming' ||
    ...
    const [demoted] = await input.db
        ...
        .where(and(eq(pgSchema.memoryItemsPostgres.id, current.id), eq(pgSchema.memoryItemsPostgres.status, 'active'), input.expectedVersion === undefined
        ? undefined
        : sql `(${pgSchema.memoryItemsPostgres.sourceRefJson}->>'version')::int = ${input.expectedVersion}`))
        .returning({ id: pgSchema.memoryItemsPostgres.id });
    if (!demoted)
 ...
 */
export function buildDetectedRowValues(subject, draft, window) {
    return {
