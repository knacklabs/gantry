 ...
 */
export function buildRecallEmbeddingCapability(db, appId) {
    ...
    return {
        ...
        embedQuery: async (query, signal) => {
            try {
                ...
                const cached = new CachedEmbeddingProvider(inner, new PostgresEmbeddingCacheStore(db), MEMORY_EMBED_MODEL, MEMORY_EMBED_DIMENSIONS);
                ...
                logger[expected ? 'debug' : 'warn']({ err: error }, 'memory recall query embedding unavailable; falling back to lexical recall');
                return null;
            }
        },
    };
