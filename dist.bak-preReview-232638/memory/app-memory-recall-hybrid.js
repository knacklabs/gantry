...
export function hybridCandidateLimit(limit) {
    return Math.min(100, Math.max(limit * 4, 20));
}
function toVectorLiteral(vector) {
    return `[${vector.join(',')}]`;
}
function visibleClause(i, input, sqlOps) {
    const visible = visibleSubjectFilters(i, input);
    if (visible.length === 0)
        return sqlOps.sql `false`;
    if (visible.length === 1)
        return visible[0];
    return sqlOps.or(...visible);
}
async function withHybridSearchSettings(db, timeoutMs, sql, work) {
    const boundedTimeoutMs = timeoutMs === undefined || !Number.isFinite(timeoutMs)
        ? undefined
    ...
    return db.transaction(async (tx) => {
        ...
        if (boundedTimeoutMs !== undefined) {
            await tx.execute(sql `select set_config('statement_timeout', ${String(boundedTimeoutMs)}, true)`);
        }
        return work(tx);
    });
}
 ...
 */
export async function runHybridRecall(db, input, queryVector, deps, options = {}) {
    ...
    const distanceExpr = sql `${emb.embedding} <=> ${toVectorLiteral(queryVector)}::vector`;
    const { lexicalRows, vectorRows } = await withHybridSearchSettings(db, options.statementTimeoutMs, sql, async (queryDb) => {
        const lexical = query
            ? (await queryDb
                .select({ row: i, lexicalScore: lexicalScoreExpr })
                .from(i)
                .where(and(eq(i.status, 'active'), eq(i.appId, context.appId), visible, sql `${document} @@ ${searchQuery}`))
        ...
        const vector = (await queryDb
            ...
            .where(and(eq(i.status, 'active'), eq(i.appId, context.appId), visible, eq(emb.provider, deps.embeddings.provider), eq(emb.model, deps.embeddings.model), eq(emb.dimensions, deps.embeddings.dimensions), eq(emb.status, 'ready'), sql `${emb.embedding} is not null`, sql `${emb.contentHash} = ${currentContentHash}`))
    ...
    for (let index = 0; index < vectorRows.length; index += 1) {
        ...
        if (currentHash !== candidate.contentHash)
            continue; // stale vector for changed text
        const entry = ensure(candidate.row);
    ...
    ranked.sort((a, b) => {
        ...
        const updatedCompare = String(b.row.updatedAt).localeCompare(String(a.row.updatedAt));
        if (updatedCompare !== 0)
