...
export async function createPendingMemoryReview(input) {
    ...
    if (contentFingerprint) {
        ...
        const duplicates = await input.db
            ...
            .where(and(eq(pgSchema.memoryReviewRequestsPostgres.appId, input.subject.appId), eq(pgSchema.memoryReviewRequestsPostgres.agentId, input.subject.agentId), eq(pgSchema.memoryReviewRequestsPostgres.subjectType, input.subject.subjectType), eq(pgSchema.memoryReviewRequestsPostgres.subjectId, input.subject.subjectId), eq(pgSchema.memoryReviewRequestsPostgres.flaggedContentHash, contentFingerprint)))
            .limit(20);
        const pending = duplicates.find((row) => row.status === 'pending_review');
        if (pending)
            return { status: 'pending_exists', reviewId: pending.id };
        const decided = duplicates[0];
        if (decided)
            return { status: 'adjudicated', reviewId: decided.id };
    }
    const now = nowIso();
