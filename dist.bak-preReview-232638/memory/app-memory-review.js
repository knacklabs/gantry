...
import { itemMatchesSubjectBoundary, parseItemSource, parseJsonObject, } from './app-memory-canonical-codec.js';
import { normalizePendingReviewLimit, normalizePendingReviewOffset, reviewEvidenceIds, reviewItemIds, toMemoryReviewDisplayPage, toMemoryReviewEvidenceSnippet, toReadableReviewItem, withProposedChanges, } from './app-memory-review-readable.js';
...
import { nowIso, withStatementTimeout, } from './app-memory-service-query-helpers.js';
const REVIEW_APPLYABLE_ACTIONS = new Set('promote retire rewrite merge needs_review'.split(' '));
function pendingMemoryReviewFilter(subject) {
    return and(eq(pgSchema.memoryReviewRequestsPostgres.appId, subject.appId), eq(pgSchema.memoryReviewRequestsPostgres.agentId, subject.agentId), eq(pgSchema.memoryReviewRequestsPostgres.subjectType, subject.subjectType), eq(pgSchema.memoryReviewRequestsPostgres.subjectId, subject.subjectId), eq(pgSchema.memoryReviewRequestsPostgres.status, 'pending_review'));
}
async function itemMapForReviews(db, reviews, statementTimeoutMs) {
    const itemIds = [
        ...new Set(reviews.flatMap((review) => reviewItemIds(review.proposal))),
    ];
    if (!itemIds.length)
        return new Map();
    const rows = (await withStatementTimeout(db, statementTimeoutMs, (timeoutMs) => sql `select set_config('statement_timeout', ${String(timeoutMs)}, true)`, (tx) => tx
        .select()
...
async function evidenceMapForReviews(db, subject, reviews, statementTimeoutMs) {
    ...
    const rows = (await withStatementTimeout(db, statementTimeoutMs, (timeoutMs) => sql `select set_config('statement_timeout', ${String(timeoutMs)}, true)`, (tx) => tx
        ...
        .where(and(inArray(pgSchema.memoryEvidencePostgres.id, evidenceIds), eq(pgSchema.memoryEvidencePostgres.appId, subject.appId), eq(pgSchema.memoryEvidencePostgres.agentId, subject.agentId), eq(pgSchema.memoryEvidencePostgres.subjectType, subject.subjectType), eq(pgSchema.memoryEvidencePostgres.subjectId, subject.subjectId)))));
    return new Map(rows.map((row) => [row.id, toMemoryReviewEvidenceSnippet(row)]));
}
export async function countPendingMemoryReviews(input) {
    const rows = (await withStatementTimeout(input.db, input.statementTimeoutMs, (timeoutMs) => sql `select set_config('statement_timeout', ${String(timeoutMs)}, true)`, (db) => db
...
export async function listPendingMemoryReviews(input) {
    ...
    const rows = (await withStatementTimeout(input.db, input.statementTimeoutMs, (timeoutMs) => sql `select set_config('statement_timeout', ${String(timeoutMs)}, true)`, (db) => {
        const query = db
            .select()
            .from(pgSchema.memoryReviewRequestsPostgres)
            .where(pendingMemoryReviewFilter(input.subject))
    ...
    const itemsById = await itemMapForReviews(input.db, reviews, input.statementTimeoutMs);
...
export async function listPendingMemoryReviewPage(input) {
    ...
    const evidenceById = await evidenceMapForReviews(input.db, input.subject, reviews, input.statementTimeoutMs);
    return {
        reviews,
...
export async function decideMemoryReview(input) {
    ...
    if (input.decision.decision === 'reject') {
        const [updated] = await input.db
            ...
            .where(and(eq(pgSchema.memoryReviewRequestsPostgres.id, input.decision.reviewId), pendingMemoryReviewFilter(input.subject)))
            .returning();
        if (!updated)
            throw new Error('pending memory review not found');
    ...
    if (!validation.ok) {
        const [updated] = await input.db
            ...
            .where(and(eq(pgSchema.memoryReviewRequestsPostgres.id, review.id), eq(pgSchema.memoryReviewRequestsPostgres.status, 'approved')))
            .returning();
        if (!updated)
            throw new Error('memory review decision claim was lost');
        return toMemoryReview(updated);
    }
...
export async function validateMemoryReviewProposal(input) {
    ...
    if (proposal.action === 'promote' &&
        (!proposal.candidateId ||
            ...
            !proposal.key ||
            !proposal.value)) {
        return failure('promote proposal requires a candidate, kind, key, and value');
    }
    if ((proposal.action === 'rewrite' || proposal.action === 'needs_review') &&
        (!proposal.itemId || !proposal.value)) {
        return failure('rewrite proposal requires a target item and replacement value');
    }
    if (proposal.action === 'merge') {
        ...
        if (!proposal.targetItemId ||
            ...
            return failure('merge proposal requires multiple item ids including the target item');
        }
    }
    ...
    for (const value of [proposal.key, proposal.value, proposal.reason]) {
        ...
        if (sensitiveReason) {
            return failure(`proposal contains sensitive material: ${sensitiveReason}`);
        }
    }
    if (proposal.kind &&
        !['preference', 'decision', 'fact', 'correction', 'constraint'].includes(proposal.kind)) {
    ...
    for (const evidence of evidenceRows) {
        ...
        if (metadata.unsafeSource === true ||
            metadata.quarantined === true ||
            metadata.promptInjection === true ||
            metadata.safety === 'unsafe' ||
            metadata.safety === 'quarantined') {
            return failure('proposal references quarantined or unsafe Memory Source evidence');
        }
        if (evidence.appId !== input.subject.appId ||
    ...
    if (itemIds.length) {
        ...
        for (const item of itemRows
            ...
            fingerprintParts.push(`item:${item.id}:${item.kind}:${item.key}:${itemValue}`);
        }
    }
    ...
    if (proposal.candidateId) {
        ...
        if (candidate.appId !== input.subject.appId ||
        ...
        if (input.expectedCandidateVersions &&
        ...
        fingerprintParts.push(`candidate:${candidate.id}:${candidate.kind}:${candidate.key}:${candidate.value}`);
    }
    if (fingerprintParts.length === 0) {
        fingerprintParts.push(`proposal:${proposal.key ?? ''}:${proposal.value ?? ''}`);
    }
...
async function applyMemoryReviewProposal(input) {
    ...
    if (proposal.action === 'merge' &&
        ...
        const retiredIds = proposal.itemIds.filter((id) => id !== proposal.targetItemId);
        try {
            await input.db.transaction(async (tx) => {
                for (const id of retiredIds) {
                    ...
                    const [deleted] = await tx
                        ...
                        .where(and(eq(pgSchema.memoryItemsPostgres.id, id), eq(pgSchema.memoryItemsPostgres.status, 'active'), sql `(${pgSchema.memoryItemsPostgres.sourceRefJson}->>'version')::int = ${expectedVersion}`))
