...
import { clampConfidence, encodeItemSource, itemMatchesSubjectBoundary, parseItemSource, parseItemValue, parseJsonObject, toAppItem, } from './app-memory-canonical-codec.js';
import { conversationIdForChannel, toEvidence, } from './app-memory-service-record-mappers.js';
import { buildMemoryItemWriteBase, isUniqueViolation, memoryContentHash, } from './app-memory-service-helpers.js';
import { nowIso } from './app-memory-service-query-helpers.js';
import { queryAppMemoryItems, recordAppMemoryRecallEvents, toAppMemoryItems, toAppMemorySearchResults, } from './app-memory-recall.js';
import { decideMemoryReview, listPendingMemoryReviewPage, listPendingMemoryReviews, } from './app-memory-review.js';
import { demoteDreamingPromotedMemoryItem, deleteOwnedMemoryItem, findActiveMemoryByKey, getOwnedMemoryItem, listDreamingStatuses, listRecentBlockedDreamDecisions, } from './app-memory-item-queries.js';
import { buildAppMemoryContinuityStatus, buildAppMemoryContinuitySummary, } from './app-memory-continuity.js';
...
export class AppMemoryService {
    ...
    constructor(explicitDb = null) {
        this.explicitDb = explicitDb;
    }
    get db() {
        if (this.explicitDb)
            return this.explicitDb;
        return getRuntimeStorage().service.db;
    }
    recallDeps(input) {
        const embeddings = buildRecallEmbeddingCapability(this.db, normalizeSubject(input).appId);
        if (!embeddings)
            return APP_MEMORY_RECALL_DEPS;
    ...
    async recordEvidence(input) {
        ...
        if (!text)
            throw new Error('memory evidence text is required');
        const sensitiveReason = classifySensitiveMemoryMaterial(text);
        if (sensitiveReason) {
            throw new Error(`sensitive material blocked in memory evidence: ${sensitiveReason}`);
        }
        const row = {
    ...
    async save(input) {
        ...
        for (const [field, value] of [
            ...
            if (reason) {
                throw new Error(`sensitive material blocked in memory ${field}: ${reason}`);
            }
        }
        ...
        try {
            ...
            const [row] = await this.db
                ...
                .set(conflictBase)
                .where(and(eq(pgSchema.memoryItemsPostgres.id, conflicting.id), eq(pgSchema.memoryItemsPostgres.status, 'active')))
                .returning();
            if (!row) {
                throw err;
            }
    ...
    async list(input = {}, options = {}) {
        if (!this.isEnabled())
            return [];
        const rows = await queryAppMemoryItems(this.db, input, false, APP_MEMORY_RECALL_DEPS, {
    ...
    recordRecallEvents = (input, results) => recordAppMemoryRecallEvents(this.db, input, results, APP_MEMORY_RECALL_DEPS);
    async searchReadOnly(input = {}, options = {}) {
        if (!this.isEnabled())
            return [];
        const rows = await queryAppMemoryItems(this.db, input, true, this.recallDeps(input), {
            signal: options.signal,
            statementTimeoutMs: options.statementTimeoutMs,
        });
        return toAppMemorySearchResults(rows);
    }
    async listForHydrationReadOnly(input = {}, options = {}) {
        if (!this.isEnabled())
            return [];
        const rows = await queryAppMemoryItems(this.db, input, false, APP_MEMORY_RECALL_DEPS, {
            signal: options.signal,
    ...
    async searchForHydrationReadOnly(input = {}, options = {}) {
        if (!this.isEnabled())
            return [];
        const rows = await queryAppMemoryItems(this.db, input, true, options.allowEmbeddings === false
    ...
    async patch(input) {
        ...
        for (const value of [input.key, input.value, input.why || undefined]) {
            ...
            const reason = classifySensitiveMemoryMaterial(value);
            if (reason)
                throw new Error(`sensitive material blocked in memory patch: ${reason}`);
        }
        const currentValue = parseItemValue(current);
        const nextKey = input.key !== undefined ? input.key.trim() : current.key;
        const nextValue = input.value !== undefined ? input.value.trim() : currentValue.value;
        if (nextKey !== current.key) {
            ...
            if (collision && collision.id !== current.id) {
                throw new ApplicationError('CONFLICT', 'Memory key already exists for this subject');
            }
        }
        const nextValueJson = {
            ...parseJsonObject(current.valueJson),
            value: nextValue,
            why: input.why !== undefined ? input.why?.trim() || null : currentValue.why,
        ...
        try {
            ...
            if (isUniqueViolation(err)) {
                throw new ApplicationError('CONFLICT', 'Memory key already exists for this subject', { cause: err });
            }
            throw err;
        }
        if (!row)
    ...
    async delete(input) {
        this.assertEnabled();
        return deleteOwnedMemoryItem({ db: this.db, context: normalizeSubject(input), ...input });
    }
    // prettier-ignore
    async demoteDreamingPromoted(input) {
        this.assertEnabled();
        return demoteDreamingPromotedMemoryItem({ db: this.db, context: normalizeSubject(input), ...input });
    }
    async demote(input) {
        return this.demoteDreamingPromoted(input);
    }
    async continuityStatus(input = {}) {
        this.assertEnabled();
        return buildAppMemoryContinuityStatus(this, input);
    }
    async continuitySummary(input = {}) {
        this.assertEnabled();
        return buildAppMemoryContinuitySummary(this, input);
    }
    async triggerDreaming(input = {}) {
        this.assertEnabled();
    ...
    async dreamingStatus(input = {}, options = {}) {
        if (!this.isEnabled())
            return [];
        return listDreamingStatuses(this.db, input, options);
    }
    async listPendingReviews(input = {}, options = {}) {
        if (!this.isEnabled())
            return [];
    ...
    async listRecentBlockedDreamDecisions(input = {}, options = {}) {
        if (!this.isEnabled())
            return [];
        return listRecentBlockedDreamDecisions(this.db, input, options);
    }
    async listPendingReviewPage(input = {}, options = {}) {
        if (!this.isEnabled()) {
