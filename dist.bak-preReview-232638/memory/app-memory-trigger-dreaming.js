...
import { MEMORY_DREAMING_EMBED_MODEL, MEMORY_DREAMING_EMBED_PROVIDER, MEMORY_DREAMING_EMBEDDINGS_ENABLED, RUNTIME_MEMORY_DREAMING_ENABLED, } from '../config/memory.js';
import * as pgSchema from '../adapters/storage/postgres/schema/schema.js';
import { ApplicationError } from '../application/common/application-error.js';
import { abortReason, createMemoryOperationDeadline, isMemoryOperationTimeoutError, normalizeMemoryTimeoutMs, } from '../shared/memory-dreaming-timeout.js';
...
import { DREAM_EMBEDDING_DEADLINE_MS, runWithTimeout, storeDreamItemEmbedding, } from './app-memory-dream-embeddings.js';
...
import { MEMORY_REVIEW_SUMMARY_STATEMENT_TIMEOUT_MS, safeCountPendingMemoryReviews, withPendingReviews, } from './app-memory-dreaming-review-summary.js';
import { createPendingMemoryReview } from './app-memory-review-create.js';
import { proposeMemoryConsolidationActions, proposeMemoryDreamingActions, } from './memory-llm-proposals.js';
...
function boundedRemainingTimeoutMs(remainingMs, maxMs) {
...
function structuredPatternIntentFromEvidence(row) {
...
export function patternTranscriptTurnsFromEvidence(evidence) {
    return evidence
        .map((row) => ({
        intent: structuredPatternIntentFromEvidence(row),
        messageId: row.id,
    }))
        .filter((turn) => typeof turn.intent === 'string' && turn.intent.length > 0);
}
export async function triggerAppMemoryDreaming(input) {
    ...
    if (!RUNTIME_MEMORY_DREAMING_ENABLED) {
        throw new ApplicationError('CONFLICT', 'memory dreaming is disabled in runtime settings');
    }
    const deadlineRemainingMs = typeof triggerInput.deadlineAtMs === 'number' &&
        Number.isFinite(triggerInput.deadlineAtMs)
        ? triggerInput.deadlineAtMs - Date.now()
        : undefined;
    const requestedTimeoutMs = typeof triggerInput.timeoutMs === 'number' &&
    ...
    const dreamDeadline = createMemoryOperationDeadline({
        timeoutMs: normalizeMemoryTimeoutMs(requestedTimeoutMs, pgSchema.MEMORY_DREAM_RUN_TIMEOUT_MS),
    ...
    const finalizeRun = async (status, summary) => {
        const [row] = await db
    ...
    try {
        await db.insert(pgSchema.memoryDreamRunsPostgres).values({
            ...
            summaryJson: '{}',
            startedAt: now,
            leaseExpiresAt: pgSchema.dreamRunLeaseExpiresAt(now, dreamDeadline.deadlineAtMs),
        });
    }
    catch (error) {
    ...
    const embeddingsEnabled = MEMORY_DREAMING_EMBEDDINGS_ENABLED &&
    ...
    if (embeddingProvider) {
        try {
            ...
            }, boundedRemainingTimeoutMs(dreamDeadline.remainingTimeoutMs(), DREAM_EMBEDDING_DEADLINE_MS) ?? DREAM_EMBEDDING_DEADLINE_MS, { signal: dreamDeadline.signal });
        }
        catch (error) {
            ...
            const run = await finalizeRun('failed', withPendingReviews({
    ...
    try {
        decisions = await runAppMemoryDreamPass({
            ...
            listItems: () => {
                dreamDeadline.throwIfExpired();
                return queryAppMemoryItems(db, { ...subject, limit: 100 }, false, APP_MEMORY_TRIGGER_RECALL_DEPS, {
                    signal: dreamDeadline.signal,
                    statementTimeoutMs: boundedRemainingTimeoutMs(dreamDeadline.remainingTimeoutMs(), 30_000),
                });
            },
            save: input.save,
            retire: input.retire,
            detectPatternCandidates: async ({ subject: dreamSubject }) => {
                dreamDeadline.throwIfExpired();
                const windowStart = new Date(new Date(now).getTime() -
                    PATTERN_DETECTION_WINDOW_DAYS * 24 * 60 * 60 * 1000).toISOString();
                const windowedEvidence = await db
                    .select()
                    .from(pgSchema.memoryEvidencePostgres)
                    .where(and(eq(pgSchema.memoryEvidencePostgres.appId, dreamSubject.appId), eq(pgSchema.memoryEvidencePostgres.agentId, dreamSubject.agentId), eq(pgSchema.memoryEvidencePostgres.subjectType, dreamSubject.subjectType), eq(pgSchema.memoryEvidencePostgres.subjectId, dreamSubject.subjectId), gte(pgSchema.memoryEvidencePostgres.createdAt, windowStart)))
                ...
                const transcriptTurns = patternTranscriptTurnsFromEvidence(windowedEvidence);
            ...
            storeDreamEmbedding: async (value) => {
                ...
                return storeDreamItemEmbedding({
                    ...
                    model: MEMORY_DREAMING_EMBED_MODEL,
                    ...value,
                    timeoutMs: boundedRemainingTimeoutMs(dreamDeadline.remainingTimeoutMs(), DREAM_EMBEDDING_DEADLINE_MS) ?? DREAM_EMBEDDING_DEADLINE_MS,
                    signal: dreamDeadline.signal,
                });
            },
            proposeDreaming: ({ evidence, candidates, activeItems }) => proposeMemoryDreamingActions({
                subject,
                evidence,
                candidates,
                activeItems,
                timeoutMs: boundedRemainingTimeoutMs(dreamDeadline.remainingTimeoutMs(), 120_000),
                signal: dreamDeadline.signal,
            }),
            proposeConsolidation: ({ activeItems }) => proposeMemoryConsolidationActions({
                subject,
                activeItems,
                timeoutMs: boundedRemainingTimeoutMs(dreamDeadline.remainingTimeoutMs(), 120_000),
                signal: dreamDeadline.signal,
            }),
            createPendingReview: (proposal, reviewDb = db) => createPendingMemoryReview({
                db: reviewDb,
                runId,
                subject,
                phase,
        ...
        const pendingReviews = await safeCountPendingMemoryReviews({
            ...
            subject,
            signal: dreamDeadline.signal,
            statementTimeoutMs: boundedRemainingTimeoutMs(dreamDeadline.remainingTimeoutMs(), MEMORY_REVIEW_SUMMARY_STATEMENT_TIMEOUT_MS) ?? MEMORY_REVIEW_SUMMARY_STATEMENT_TIMEOUT_MS,
        });
        dreamDeadline.throwIfExpired();
        const summary = summarizeDreamDecisions(decisions, Boolean(triggerInput.dryRun), {
            pendingReviews,
        });
        ...
        const reason = error instanceof Error ? error.message : 'unknown dreaming error';
        const timedOut = isMemoryOperationTimeoutError(error) || dreamDeadline.signal.aborted;
        const pendingReviews = await safeCountPendingMemoryReviews({
            db,
            subject,
        });
