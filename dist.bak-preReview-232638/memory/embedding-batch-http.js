import { EmbeddingProviderError, classifyEmbeddingHttpError, } from './memory-embedding-errors.js';
/**
...
export async function submitEmbeddingBatch(conn, params, signal) {
    ...
    form.append('file', new Blob([jsonl], { type: 'application/jsonl' }), 'memory-embeddings.jsonl');
    ...
    if (!fileRes.ok) {
        throw classifyEmbeddingHttpError(fileRes.status, await fileRes.text(), fileRes.headers);
    }
    const fileJson = (await fileRes.json());
    if (!fileJson.id) {
        throw new EmbeddingProviderError('retryable_provider_error', 'batch input file upload returned no id');
    }
    ...
    if (!batchRes.ok) {
        throw classifyEmbeddingHttpError(batchRes.status, await batchRes.text(), batchRes.headers);
    }
    const batchJson = (await batchRes.json());
    if (!batchJson.id) {
        throw new EmbeddingProviderError('retryable_provider_error', 'batch creation returned no id');
    }
    return { batchId: batchJson.id };
}
export async function pollEmbeddingBatch(conn, batchId, signal) {
...
export async function fetchEmbeddingBatchResults(conn, poll, signal) {
    const rows = [];
...
async function downloadFile(conn, fileId, signal) {
