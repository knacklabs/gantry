import { getMemoryModelRuntimeConfig, MEMORY_EXTRACTOR_MAX_FACTS, MEMORY_EXTRACTOR_MIN_CONFIDENCE, } from '../config/index.js';
import { logger } from '../infrastructure/logging/logger.js';
import { sleep } from '../shared/time/datetime.js';
import { MEMORY_EXTRACTION_FEW_SHOTS, MEMORY_EXTRACTION_SYSTEM_PROMPT, } from './prompts/extract.js';
...
function normalizeGroundingLoose(value) {
    return normalizeGroundingText(value)
        .replace(/[^a-z0-9\s]/g, ' ')
        .replace(/\s+/g, ' ')
        .trim();
}
function stripQuotedBoundaries(value) {
    return value.replace(/^["'`]+|["'`]+$/g, '').trim();
}
function buildGroundingCorpus(turns) {
...
function isGroundedWhy(why, corpus) {
    const strictNeedle = normalizeGroundingText(stripQuotedBoundaries(why));
    if (strictNeedle.length < MIN_GROUNDED_WHY_CHARS)
        return false;
    if (corpus.userStrict.includes(strictNeedle))
        return true;
    if (corpus.allStrict.includes(strictNeedle))
        return true;
    const looseNeedle = normalizeGroundingLoose(strictNeedle);
    if (looseNeedle.length < MIN_GROUNDED_WHY_CHARS)
        return false;
    return (corpus.userLoose.includes(looseNeedle) ||
...
function parseFacts(raw, options) {
    ...
    for (const row of raw) {
        ...
        if (looksLikePromptInjection(value))
            continue;
        if (/\b(today|tomorrow|right now|for now|temporary|temp|quick fix|debugging this run|in this turn|next step|later today)\b/i.test(value)) {
            continue;
        }
        ...
        if (!why)
            continue;
        if (looksLikePromptInjection(why))
            continue;
        if (!isGroundedWhy(why, groundingCorpus))
            continue;
        ...
        out.push(parsed);
        if (out.length >= options.maxFacts)
            break;
    }
...
function makeFallbackKey(kind, value) {
    const slug = value
        .toLowerCase()
        .normalize('NFKC')
        .replace(/[^a-z0-9]+/g, '-')
...
function summarizeRetrievedItemField(value, maxChars, label) {
...
function buildPromptParts(input) {
    ...
    const payload = {
        ...
        retrieved_items: (input.retrievedItems || []).slice(0, PROMPT_RETRIEVED_ITEM_LIMIT),
    };
...
export class LlmMemoryExtractionProvider {
    providerName = 'memory-llm';
    async extractFacts(input) {
        return (await this.extractFactsWithOutcome(input)).facts;
    }
    async extractFactsWithOutcome(input) {
        const { extractor: modelExtractor, modelProfiles } = getMemoryModelRuntimeConfig();
        ...
        const sanitizedRetrievedItems = (input.retrievedItems || [])
            ...
            const boundedKey = summarizeRetrievedItemField(key.text, PROMPT_RETRIEVED_KEY_CHAR_BUDGET, 'memory_key');
            const boundedValue = summarizeRetrievedItemField(value.text, PROMPT_RETRIEVED_VALUE_CHAR_BUDGET, 'memory_value');
        ...
        for (let attempt = 0; attempt < 2; attempt += 1) {
            try {
                ...
                if (!text.trim()) {
                    logger.warn({ model: modelExtractor, trigger: input.trigger }, 'LLM extraction returned malformed output; skipping this boundary extraction');
                ...
                return extractionResult(parseFacts(parsed, {
                    minConfidence: MEMORY_EXTRACTOR_MIN_CONFIDENCE,
                    maxFacts: MEMORY_EXTRACTOR_MAX_FACTS,
                    userId: input.userId,
                    turns,
                    retrievedKeys: new Set((input.retrievedItems || [])
                ...
                if (attempt === 0 && transient) {
                    logger.warn({ err, model: modelExtractor, retryInMs: EXTRACT_RETRY_DELAY_MS }, 'Transient extractor failure; retrying once');
                    await sleep(EXTRACT_RETRY_DELAY_MS);
                    continue;
                }
                logger.warn({ err, model: modelExtractor }, 'LLM extraction failed; skipping this boundary extraction');
                return extractionResult([], 'extractor_failed', 'extractor_failed');
            }
        }
        return extractionResult([], 'extractor_failed', 'extractor_failed');
