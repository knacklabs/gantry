...
export async function processBrainSearchRequest(request) {
    ...
    const searchResult = await runWithinMemoryDeadline(request, () => brain.search({
        appId,
        query,
...
export async function processBrainQueryRequest(request) {
    ...
    const queryResult = await runWithinMemoryDeadline(request, (signal, timeoutMs) => brain.query({
        appId,
        question,
        signal,
...
export async function processBrainWriteRequest(request, sourceAgentFolder) {
    ...
    const result = await brain.write({
        ...
        slug,
        markdown,
        title: typeof request.payload.title === 'string'
            ? request.payload.title
            : undefined,
        sourceKind: 'agent',
        sourceRef: typeof request.payload.source_ref === 'string'
