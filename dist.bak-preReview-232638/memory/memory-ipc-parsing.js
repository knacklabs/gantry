...
import { isDirectSaveMemoryKind, } from './memory-types.js';
export function parseOptionalString(value, opts = {}) {
...
export function parseOptionalNumber(value, opts = {}) {
...
export function parseSaveMemoryInput(payload) {
...
export function parsePatchMemoryInput(payload) {
...
export function parseDemoteMemoryInput(payload) {
    const input = objectPayload(payload, 'memory_demote');
    const id = str(input, 'id', 128);
    if (!id)
        throw new Error('memory_demote requires id');
    const expectedVersion = num(input, 'expected_version', { min: 1 });
    return omitUndefined({
        id,
        expectedVersion: expectedVersion === undefined ? undefined : Math.round(expectedVersion),
        reason: str(input, 'reason', 500),
    });
}
export function parseReviewDecisionRequest(payload) {
    ...
    if (!reviewId)
        throw new Error('memory_review_decision requires review_id');
    if (!isReviewDecision(decision)) {
        throw new Error('memory_review_decision.decision must be approve, reject, or edit_approve');
    }
    return omitUndefined({
        kind: 'single',
        reviewId,
...
export function parseSaveProcedureInput(payload) {
    ...
    return omitUndefined({
        ...
        origin: originRaw === 'explicit' || originRaw === 'accepted_suggestion'
            ? originRaw
            : undefined,
...
export function parsePatchProcedureInput(payload) {
...
function parseReviewBatchDecisions(value) {
    ...
    return value.map((item, index) => {
        ...
        if (!reviewId && number === undefined) {
            throw new Error(`memory_review_decision.decisions[${index}] requires number or review_id`);
        }
        return omitUndefined({
            number,
            reviewId,
...
function parseReviewDecisionNumber(value, index) {
    if (value === undefined)
        return undefined;
    if (typeof value !== 'number' || !Number.isInteger(value) || value < 1) {
        throw new Error(`memory_review_decision.decisions[${index}].number must be a positive integer`);
    }
    return value;
}
function parseReviewPageContext(input) {
...
function parseReviewPageContextSubject(value) {
    if (!isPlainObject(value)) {
    ...
    const subjectType = parseSubjectType(str(value, 'subject_type', 32) || str(value, 'subjectType', 32));
    const subjectId = str(value, 'subject_id', 512) || str(value, 'subjectId', 512);
    if (!appId || !agentId || !subjectType || !subjectId) {
        throw new Error('memory_review_decision.page_context.subject requires app_id, agent_id, subject_type, and subject_id');
    }
    return omitUndefined({
        appId,
        agentId,
...
function stringList(value, maxLen, dropInvalid = false) {
    if (!Array.isArray(value))
        return undefined;
    const parsed = value.map((item) => parseOptionalString(item, { maxLen }));
    if (!dropInvalid && parsed.some((item) => !item))
        return undefined;
    const out = parsed.filter((item) => Boolean(item));
...
function directSaveMemoryKind(input) {
    if (!Object.prototype.hasOwnProperty.call(input, 'kind'))
        return undefined;
    const kind = parseOptionalString(input.kind, { maxLen: 32 });
    if (isDirectSaveMemoryKind(kind))
        return kind;
    throw new Error('memory_save.kind must be one of preference, decision, fact, correction, or constraint');
}
