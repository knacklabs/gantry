...
import { processBrainQueryRequest, processBrainSearchRequest, processBrainWriteRequest, } from './memory-ipc-brain.js';
...
import { DEFAULT_MEMORY_APP_ID, memoryAgentIdForWorkspaceFolder, } from './app-memory-boundaries.js';
import { resolveScopedMemorySubject, canonicalConversationIdForMemory, searchInputForResolvedMemorySubject, } from './app-memory-subject-resolver.js';
import { describeAppMemorySearchOutcome } from './app-memory-recall.js';
import { AppMemoryService } from './app-memory-service.js';
import { parseDemoteMemoryInput, parsePatchMemoryInput, parsePatchProcedureInput, parseSaveMemoryInput, parseSaveProcedureInput, } from './memory-ipc-parsing.js';
export { parseOptionalNumber, parseOptionalString, } from './memory-ipc-parsing.js';
import { assertMemoryRequestNotExpired, deadlineUnavailableResponse, hasEnoughMemoryBudget, runWithinMemoryDeadline, } from './memory-ipc-deadline.js';
import { processMemoryReviewDecisionRequest, processPendingMemoryReviewRequest, } from './memory-review-ipc.js';
...
const DEFAULT_ALLOWED_MEMORY_IPC_ACTIONS = new Set([
    'memory_search',
    'memory_save',
    'brain_search',
    'brain_query',
...
function asMemoryDemoteService(memory) {
    const candidate = memory;
    if (typeof candidate.demote === 'function') {
        return candidate;
    }
    throw new Error('memory demote service is unavailable; AppMemoryService.demote(input) is required');
}
function asMemorySearchReadOnlyService(memory) {
    const candidate = memory;
    if (typeof candidate.searchReadOnly === 'function') {
        return candidate;
    }
    throw new Error('memory read-only search service is unavailable; AppMemoryService.searchReadOnly(input) is required');
}
...
function assertMemoryActionAllowed(request) {
    if (!MEMORY_IPC_ACTIONS.includes(request.action)) {
        throw new Error(`Unsupported memory action: ${request.action}`);
    }
    const allowedActions = request.allowedActions && request.allowedActions.length > 0
...
async function runMemoryMutation(request, work) {
    assertMemoryRequestNotExpired(request, nowMs);
    return work();
}
function continuityDeadlineUnavailableResponse(request, provider, subject) {
    return {
        ok: true,
...
export function resolveTrustedMemorySubject(sourceAgentFolder, context, scope) {
...
export async function processMemoryRequest(request, sourceAgentFolder) {
    ...
    try {
        ...
        switch (request.action) {
            case 'memory_search': {
                ...
                const subject = resolveTrustedMemorySubject(sourceAgentFolder, request.context);
                ...
                const searchResult = await runWithinMemoryDeadline(request, (signal, statementTimeoutMs) => readOnlySearch.searchReadOnly(searchInput, {
                    signal,
                    statementTimeoutMs,
                }), nowMs);
                ...
                if (results.length > 0 &&
                    ...
                    await runMemoryMutation(request, () => readOnlySearch.recordRecallEvents(searchInput, results));
                }
                const outcome = describeAppMemorySearchOutcome(searchInput, results.length);
                return {
                    ok: true,
                    requestId: request.requestId,
                    provider,
            ...
            case 'memory_save': {
                ...
                const subject = resolveTrustedMemorySubject(sourceAgentFolder, request.context, input.scope);
                if (!hasEnoughMemoryBudget(request, nowMs)) {
                    provider = 'postgres';
                    return deadlineUnavailableResponse(request, provider);
                }
                const saved = await runMemoryMutation(request, () => getMemory().save({
                    ...subject,
                    appId: subject.appId,
            ...
            case 'memory_patch': {
                ...
                const patched = await runMemoryMutation(request, () => getMemory().patch({
                    ...subject,
                    id: input.id,
                    appId: DEFAULT_MEMORY_APP_ID,
                    agentId: memoryAgentIdForWorkspaceFolder(sourceAgentFolder),
            ...
            case 'memory_demote': {
                ...
                const demoted = await runMemoryMutation(request, () => asMemoryDemoteService(getMemory()).demote({
                    ...subject,
                    appId: subject.appId,
                    agentId: subject.agentId,
                    subjectType: subject.subjectType,
            ...
            case 'continuity_summary': {
                ...
                if (!hasEnoughMemoryBudget(request, nowMs)) {
                    provider = 'postgres';
                    return continuityDeadlineUnavailableResponse(request, provider, subject);
                }
                const continuityResult = await runWithinMemoryDeadline(request, (signal, statementTimeoutMs) => getMemory().continuitySummary({
                    ...subject,
                    signal,
                    statementTimeoutMs,
            ...
            case 'memory_consolidate': {
                ...
                const result = await runMemoryMutation(request, () => getMemory().triggerDreaming({
                    ...subject,
                    appId: subject.appId,
                    agentId: subject.agentId,
                    subjectType: subject.subjectType,
...
export function writeMemoryResponse(workspaceFolder, requestId, response, privateKeyPem) {
