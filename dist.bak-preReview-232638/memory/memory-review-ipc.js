...
import { parseOptionalNumber, parseReviewDecisionRequest, } from './memory-ipc-parsing.js';
...
export async function processPendingMemoryReviewRequest(input) {
    ...
    const reviewsOutcome = await runWithinMemoryDeadline(input.request, (signal, statementTimeoutMs) => AppMemoryService.getInstance().listPendingReviewPage({
        ...input.subject,
        appId: input.subject.appId,
        agentId: input.subject.agentId,
        subjectType: input.subject.subjectType,
        subjectId: input.subject.subjectId,
    }, { signal, statementTimeoutMs, ...paging }), nowMs);
    if (reviewsOutcome.status === 'deadline_exceeded') {
        return deadlineUnavailableResponse(input.request, MEMORY_REVIEW_PROVIDER);
    }
    const page = reviewsOutcome.value;
    const reviewPage = page.reviewPage ||
...
export async function processMemoryReviewDecisionRequest(input) {
    ...
    if (!input.request.context?.userId) {
        throw new Error('memory_review_decision requires a trusted reviewer user id');
    }
    if (!input.request.context.reviewerIsControlApprover) {
        throw new Error('memory_review_decision requires a conversation control approver');
    }
    ...
    if (decisionInput.kind === 'batch') {
        if (!sameReviewPageSubject(decisionInput.pageContext, input.subject)) {
            throw new Error('memory_review_decision page_context is outside trusted subject scope');
        }
        const outcomes = [];
        for (const decision of decisionInput.decisions) {
            const resolved = resolveReviewIdFromBatchDecision(decisionInput.pageContext, decision);
            ...
            if (!reviewId) {
                outcomes.push({
                    ...
                    ok: false,
                    error: resolved.error || 'review number is not present in page_context',
                });
                continue;
            }
            try {
                const review = await runMemoryMutation(input.request, () => memory.decideReview({
                    ...input.subject,
                    appId: input.subject.appId,
                    agentId: input.subject.agentId,
                    subjectType: input.subject.subjectType,
        ...
        return {
            ...
            data: {
                decision_batch: {
                    ...
                    remaining_count: await countRemainingPendingReviews(memory, input.subject, input.request),
                    outcomes,
                },
            },
        };
...
function parsePendingReviewPaging(payload) {
...
function sameReviewPageSubject(pageContext, subject) {
    return (pageContext.subject.appId === subject.appId &&
...
function resolveReviewIdFromBatchDecision(pageContext, decision) {
...
async function countRemainingPendingReviews(memory, subject, request) {
    if (!hasEnoughMemoryBudget(request, nowMs))
        return null;
    try {
        const remainingOutcome = await runWithinMemoryDeadline(request, (signal, statementTimeoutMs) => memory.listPendingReviewPage({
            ...subject,
            appId: subject.appId,
            agentId: subject.agentId,
            subjectType: subject.subjectType,
            subjectId: subject.subjectId,
        }, { signal, statementTimeoutMs, limit: 1, offset: 0 }), nowMs);
...
function toReviewPageWire(page) {
    return {
