...
export declare const MEMORY_GLOBAL_WORKSPACE_FOLDER = "_global";
export declare const DIRECT_SAVE_MEMORY_KINDS: readonly ["preference", "decision", "fact", "correction", "constraint"];
export type DirectSaveMemoryKind = (typeof DIRECT_SAVE_MEMORY_KINDS)[number];
export declare function isDirectSaveMemoryKind(value: unknown): value is DirectSaveMemoryKind;
export type MemorySubjectType = 'user' | 'group' | 'channel' | 'common';
export type MemoryVisibility = MemorySubjectType;
export type MemoryEvidenceSource = 'session' | 'message' | 'tool' | 'manual' | 'knowledge_source';
export type DreamPhase = 'light' | 'rem' | 'deep' | 'all';
export type DreamDecisionAction = 'stage_candidate' | 'promote' | 'update' | 'skip' | 'blocked' | 'dry_run' | 'merge' | 'rewrite' | 'pin' | 'decay' | 'retire' | 'needs_review' | 'no_op';
export type MemoryReviewDecision = 'approve' | 'reject' | 'edit_approve';
export type MemoryProposalAction = 'stage_candidate' | 'promote' | 'update' | 'retire' | 'needs_review' | 'skip' | 'keep' | 'merge' | 'rewrite';
...
export type MemoryKind = 'preference' | 'decision' | 'fact' | 'correction' | 'constraint' | 'reference';
