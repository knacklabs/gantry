...
const CURRENT_MESSAGE_LINE_LIMITS = {
    messageContentBytes: Number.POSITIVE_INFINITY,
    quotedMessageContentBytes: CONVERSATION_CONTEXT_RENDER_LIMITS.quotedMessageContentBytes,
    renderedMessageBytes: Number.POSITIVE_INFINITY,
    attachmentsPerMessage: CONVERSATION_CONTEXT_RENDER_LIMITS.attachmentsPerMessage,
};
...
export function formatConversationContextMessages(context, timezone) {
    const header = `<context timezone="${escapeXml(timezone)}" />\n`;
    return renderBoundedConversationContext({
        header,
        recentChannelLines: formatMessageLineList(context.recentChannelContext, timezone),
        activeThreadLines: formatMessageLineList(context.activeThreadContext, timezone),
        currentMessages: context.currentMessages,
        timezone,
    });
}
function formatMessageLineList(messages, timezone, limits = CONVERSATION_CONTEXT_RENDER_LIMITS) {
    return messages
...
function renderBoundedConversationContext(input) {
    ...
    const currentMessageLines = formatMessageLineList(input.currentMessages, input.timezone, CURRENT_MESSAGE_LINE_LIMITS);
    const render = () => `${input.header}<recent_channel_context trust="untrusted_conversation_data">\n${recentChannelLines.join('\n')}\n</recent_channel_context>\n<active_thread_context trust="untrusted_conversation_data">\n${activeThreadLines.join('\n')}\n</active_thread_context>\n<current_message trust="untrusted_conversation_data">\n${currentMessageLines.join('\n')}\n</current_message>`;
    while (utf8Bytes(render()) >
...
function formatMessageLine(m, timezone, limits) {
    const displayTime = formatLocalTime(m.timestamp, timezone);
    const replyAttr = m.reply_to_message_id
        ? ` reply_to="${escapeContextAttr(m.reply_to_message_id)}"`
        : '';
    const replySnippet = limits.quotedMessageContentBytes > 0 &&
        m.reply_to_message_content &&
        m.reply_to_sender_name
        ? `\n  <quoted_message from="${escapeContextAttr(m.reply_to_sender_name)}">${escapeXml(boundedUtf8Text(m.reply_to_message_content, limits.quotedMessageContentBytes))}</quoted_message>`
        : '';
    const line = `<message sender="${escapeContextAttr(m.sender_name)}" time="${escapeXml(displayTime)}"${replyAttr}>${replySnippet}${formatAttachmentLines(m.attachments, limits)}${escapeXml(boundedUtf8Text(m.content, limits.messageContentBytes))}</message>`;
    if (utf8Bytes(line) <= limits.renderedMessageBytes) {
        return line;
    }
    return `<message sender="${escapeContextAttr(m.sender_name)}" time="${escapeXml(displayTime)}"${replyAttr}>${escapeXml(boundedUtf8Text(m.content, Math.min(limits.messageContentBytes, 400)))}</message>`;
}
function formatAttachmentLines(attachments, limits) {
    if (!attachments?.length)
        return '';
    const renderedAttachments = attachments.slice(0, limits.attachmentsPerMessage);
    ...
    const lines = renderedAttachments
        ...
        const attrs = [
            ...
            .filter((attr) => typeof attr[1] === 'string' && attr[1].length > 0)
            .map(([name, value]) => `${name}="${escapeXml(boundedAttachmentAttr(value))}"`)
            .join(' ');
        return `\n  <attachment ${attrs} />`;
    })
        .join('');
...
function boundedAttachmentAttr(value) {
    return value.length > 160 ? value.slice(0, 160) : value;
}
function escapeContextAttr(value) {
    return escapeXml(boundedUtf8Text(value, CONVERSATION_CONTEXT_RENDER_LIMITS.attributeBytes));
}
function boundedUtf8Text(value, maxBytes) {
    return truncateUtf8ToByteLimit(value, maxBytes, CONTEXT_TRUNCATION_SUFFIX);
}
function truncateUtf8ToByteLimit(text, maxBytes, suffix) {
    ...
    for (const char of text) {
        ...
        if (outBytes + charBytes > contentBytes)
            break;
        out += char;
        outBytes += charBytes;
    }
...
function formatGantryAttachmentRef(storageRef) {
    if (!storageRef?.startsWith('attachments/'))
        return undefined;
...
export function formatOutboundForChannel(rawText, channelId) {
