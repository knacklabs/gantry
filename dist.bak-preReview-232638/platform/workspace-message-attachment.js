...
import { MAX_MESSAGE_FILE_ATTACHMENT_BYTES, } from '../application/core-tools/send-message.js';
...
export async function readWorkspaceMessageAttachment(sourceAgentFolder, workspaceRelativePath) {
    try {
        if (!isSafeWorkspaceRelativePath(workspaceRelativePath)) {
            return {
                status: 'failed',
                reason: 'invalid path: workspace path must be relative and cannot contain .. segments',
            };
        }
        const workspaceRoot = await fs.realpath(resolveWorkspaceFolderPath(sourceAgentFolder));
        const canonicalPath = await fs.realpath(path.resolve(workspaceRoot, workspaceRelativePath));
        ...
        const openFlags = process.platform === 'darwin'
        ...
        try {
            if (process.platform === 'linux') {
                ...
                const openedRelative = path.relative(workspaceRoot, openedCanonicalPath);
                if (openedRelative === '..' ||
            ...
            while (bytesRead < content.byteLength) {
                const result = await handle.read(content, bytesRead, content.byteLength - bytesRead, bytesRead);
            ...
            return {
                ...
                attachment: {
                    ...
                    contentType: CONTENT_TYPES[path.extname(workspaceRelativePath).toLowerCase()] ??
