...
import { ASYNC_TASK_GANTRY_MCP_TOOL_NAMES, DELEGATED_TASK_GANTRY_MCP_TOOL_NAMES, gantryMcpFullToolName, parseEnabledGantryMcpToolNames, } from '../gantry-mcp-tool-surface.js';
...
import { ADMIN_MCP_TOOL_NAMES, adminMcpToolNameFromFullName, isAdminMcpToolName, } from '../../shared/admin-mcp-tools.js';
import { buildAgentToolAccessView, buildRequestableBrowserToolAccess, buildRequestableAdminToolAccess, formatAgentToolAccess, PERMISSION_GATED_NATIVE_TOOLS, } from '../../shared/tool-access-view.js';
...
export const BROWSER_IPC_AUTH_TOKEN = process.env.GANTRY_BROWSER_IPC_AUTH_TOKEN || IPC_AUTH_TOKEN;
export const MEMORY_IPC_AUTH_TOKEN = process.env.GANTRY_MEMORY_IPC_AUTH_TOKEN || IPC_AUTH_TOKEN;
export const IPC_RESPONSE_VERIFY_KEY = process.env.GANTRY_IPC_RESPONSE_VERIFY_KEY || '';
export const IPC_RESPONSE_KEY_ID = process.env.GANTRY_IPC_RESPONSE_KEY_ID || '';
if (process.env.GANTRY_GROUP_FOLDER !== undefined) {
    throw new Error('GANTRY_GROUP_FOLDER is no longer supported. Use GANTRY_WORKSPACE_KEY.');
}
export const chatJid = process.env.GANTRY_CHAT_JID;
export const providerAccountId = process.env.GANTRY_PROVIDER_ACCOUNT_ID?.trim() || undefined;
...
export const permissionLane = process.env.GANTRY_PERMISSION_LANE?.trim() === 'interactive'
    ? 'interactive'
    : 'autonomous';
export const jobRunId = process.env.GANTRY_JOB_RUN_ID?.trim() || undefined;
export const jobRunLeaseToken = process.env.GANTRY_JOB_RUN_LEASE_TOKEN?.trim() || undefined;
export const jobRunLeaseFencingVersion = process.env.GANTRY_JOB_RUN_LEASE_FENCING_VERSION?.trim() || undefined;
export const threadId = process.env.GANTRY_THREAD_ID?.trim() || undefined;
export const memoryUserId = process.env.GANTRY_MEMORY_USER_ID?.trim() || undefined;
export const memoryDefaultScope = process.env.GANTRY_MEMORY_DEFAULT_SCOPE === 'user' ? 'user' : 'group';
export const memoryReviewerIsControlApprover = process.env.GANTRY_MEMORY_REVIEWER_IS_CONTROL_APPROVER === '1';
export const memoryIpcAllowedActions = normalizeMemoryIpcActions(parseJsonStringArray(process.env.GANTRY_MEMORY_IPC_ACTIONS_JSON));
export const browserProfileName = process.env.GANTRY_BROWSER_PROFILE_NAME?.trim() || undefined;
...
export const lockedAccessPreset = process.env.GANTRY_AGENT_ACCESS_PRESET === 'locked';
export const deploymentMode = process.env.GANTRY_DEPLOYMENT_MODE === 'fleet' ? 'fleet' : 'workstation';
export const enabledAdminMcpTools = parseEnabledAdminMcpTools(process.env.GANTRY_ADMIN_MCP_TOOLS_JSON);
export const enabledGantryMcpTools = parseEnabledGantryMcpToolNames(process.env.GANTRY_MCP_TOOL_NAMES_JSON, { lockedPreset: lockedAccessPreset });
const asyncTaskToolNameSet = new Set([
    ...ASYNC_TASK_GANTRY_MCP_TOOL_NAMES,
    ...DELEGATED_TASK_GANTRY_MCP_TOOL_NAMES,
]);
const asyncTaskToolsEnabled = process.env.GANTRY_ASYNC_TASK_TOOLS_ENABLED === '1';
export const configuredAllowedTools = parseConfiguredAllowedTools(process.env.GANTRY_CONFIGURED_ALLOWED_TOOLS_JSON);
export const attachedSkillSourceIds = parseJsonStringArray(process.env.GANTRY_SELECTED_SKILLS_JSON);
export const selectedSkillDisplays = parseJsonStringArray(process.env.GANTRY_SELECTED_SKILL_DISPLAYS_JSON);
export const attachedMcpSourceIds = parseJsonStringArray(process.env.GANTRY_SELECTED_MCP_SERVERS_JSON);
export const availableSemanticCapabilities = parseSemanticCapabilities(process.env.GANTRY_SEMANTIC_CAPABILITIES_JSON);
export function isAdminMcpToolEnabled(toolName) {
    return currentEnabledAdminMcpTools().has(toolName);
}
export function currentEnabledAdminMcpTools() {
    ...
    for (const rule of readLiveToolRules({
        ...
        if (adminToolName)
            enabled.add(adminToolName);
    }
    return enabled;
}
export function currentConfiguredAllowedTools() {
...
function parseConfiguredAllowedTools(raw) {
...
function parseSemanticCapabilities(raw) {
    if (!raw?.trim())
        return [];
    try {
        const parsed = JSON.parse(raw);
        if (Array.isArray(parsed)) {
            const record = Object.fromEntries(parsed
                .filter((item) => Boolean(item &&
                typeof item === 'object' &&
                !Array.isArray(item) &&
                typeof item.capabilityId === 'string'))
                .map((item) => [item.capabilityId, item]));
            return Object.values(parseSemanticCapabilityDefinitionsRecord(record) ?? {});
        }
        return Object.values(parseSemanticCapabilityDefinitionsRecord(parsed) ?? {});
    }
    catch {
        return [];
    }
}
function parseEnabledAdminMcpTools(raw) {
    if (!raw?.trim())
        return new Set();
    try {
        const parsed = JSON.parse(raw);
        ...
        return new Set(parsed
            .map((item) => (typeof item === 'string' ? item.trim() : ''))
            .filter((item) => isAdminMcpToolName(item)));
    }
    catch {
        return new Set();
    }
...
export function capabilityStatusText() {
    ...
    const selectedSkillStatusItems = selectedSkillDisplays.length > 0
        ? selectedSkillDisplays
        : attachedSkillSourceIds;
    const availableToolNames = [...enabledGantryMcpTools].filter((toolName) => !isAdminMcpToolName(toolName) &&
    ...
    const normalActionToolNames = availableToolNames.filter((toolName) => toolName === 'send_message' ||
    ...
    const lines = [
        ...
        ...(normalActionToolNames.some((toolName) => toolName.startsWith('scheduler_'))
            ? [
                '',
        ...
        ...(attachedMcpSourceIds.length > 0
            ? attachedMcpSourceIds
                ...
                const selectedCapabilities = selectedMcpCapabilitiesForSource(sourceName);
                return [
    ...
    const view = buildAgentToolAccessView({
        ...
        defaultTools: normalActionToolNames
            .filter((toolName) => !ADMIN_MCP_TOOL_NAMES.includes(toolName))
            .map(gantryMcpFullToolName),
        availableButGatedTools: PERMISSION_GATED_NATIVE_TOOLS.filter((toolName) => !currentAllowedTools.some((configured) => configured === toolName || configured.startsWith(`${toolName}(`))),
        requestableAdminTools: [
            ...buildRequestableAdminToolAccess(currentAdminTools),
            ...requestableBrowserTools,
        ],
        source: 'settings.yaml selected capabilities plus action-first runtime defaults',
    });
...
function selectedMcpCapabilitiesForSource(serverName) {
    ...
    return availableSemanticCapabilities
        .filter((capability) => currentAllowedTools.includes(`capability:${capability.capabilityId}`) &&
