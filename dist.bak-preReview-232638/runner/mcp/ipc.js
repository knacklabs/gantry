...
import { nowMs, nowMs as currentTimeMs, toIso, } from '../../shared/time/datetime.js';
import { formatDuration } from '../../shared/human-format.js';
import { formatMemoryTimeoutError, getMemoryActionTimeoutMs, } from '../memory-timeouts.js';
import { BROWSER_REQUESTS_DIR, BROWSER_RESPONSES_DIR, BROWSER_IPC_AUTH_TOKEN, IPC_AUTH_TOKEN, IPC_RESPONSE_KEY_ID, IPC_RESPONSE_VERIFY_KEY, MEMORY_IPC_AUTH_TOKEN, MEMORY_REQUESTS_DIR, MEMORY_RESPONSES_DIR, TASK_RESPONSES_DIR, agentId, appId, chatJid, providerAccountId, memoryDefaultScope, memoryIpcAllowedActions, memoryReviewerIsControlApprover, memoryUserId, threadId, } from './context.js';
import { createSignedIpcRequestEnvelope, verifyIpcResponsePayload, } from '../../shared/ipc-signing.js';
...
function removeStaleRequestFile(filePath) {
    try {
...
export function writeIpcFile(dir, data) {
    ensurePrivateDirSync(dir);
    const filename = makeIpcJsonFilename();
    const filepath = path.join(dir, filename);
    // Atomic write: temp file then rename
    const tempPath = `${filepath}.tmp`;
    const existingContext = 'context' in data &&
...
export function hasValidIpcResponseSignature(raw, payload) {
    if (!IPC_RESPONSE_VERIFY_KEY)
        return false;
...
export async function requestMemoryAction(action, payload) {
    ...
    const requestEnvelope = createSignedIpcRequestEnvelope(MEMORY_IPC_AUTH_TOKEN, requestPayload);
    ...
    if (await waitForIpcResponseFile({ responsePath, deadlineMs: deadline })) {
        try {
            ...
            const responseRequestId = typeof raw.requestId === 'string' ? raw.requestId : '';
...
export async function requestBrowserAction(action, payload, options = {}) {
    ...
    const requestEnvelope = createSignedIpcRequestEnvelope(BROWSER_IPC_AUTH_TOKEN, requestPayload);
    writePrivateFileSync(tmpReqPath, JSON.stringify(requestEnvelope, null, 2));
    fs.renameSync(tmpReqPath, reqPath);
    const deadline = nowMs() + timeoutMs;
...
function parseTaskResponseEnvelope(raw) {
    if (typeof raw !== 'object' || raw === null || Array.isArray(raw)) {
        return null;
    }
    const row = raw;
    const taskId = typeof row.taskId === 'string' ? row.taskId.trim() : '';
    if (!taskId || typeof row.ok !== 'boolean')
        return null;
...
export async function waitForTaskResponse(taskId, timeoutMs = 15_000) {
    ensurePrivateDirSync(TASK_RESPONSES_DIR);
    const responsePath = path.join(TASK_RESPONSES_DIR, `task-${taskId}.json`);
    const deadline = nowMs() + timeoutMs;
    if (await waitForIpcResponseFile({ responsePath, deadlineMs: deadline })) {
        try {
            const parsedEnvelope = parseTaskResponseEnvelope(JSON.parse(fs.readFileSync(responsePath, 'utf-8')));
            fs.unlinkSync(responsePath);
            if (!parsedEnvelope) {
                return {
                    taskId,
            ...
            if (!hasValidIpcResponseSignature(parsedEnvelope.raw, {
                ...payload,
            })) {
                return {
                    taskId,
            ...
            return {
                taskId,
                ok: false,
                error: err instanceof Error ? err.message : 'Failed to parse task response',
            };
        }
    }
    return null;
