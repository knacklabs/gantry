...
import { chatJid, configuredAllowedTools, currentEnabledAdminMcpTools, IPC_DIR, attachedMcpSourceIds, selectedSkillDisplays, attachedSkillSourceIds, TASKS_DIR, threadId, } from '../context.js';
...
export function registerAdminPermissionTools(server, options) {
    server.tool('admin_permission_list', 'List local permission and capability selection signals visible to this runner. Read-only and available without an admin grant.', {}, async () => {
        return {
            content: [
                {
                    type: 'text',
    ...
    server.tool('admin_permission_revoke', 'Revoke one current-agent persistent tool grant. Requires selected agent tool grant mcp__gantry__admin_permission_revoke.', {
        tool_name: z
            .string()
            .optional()
        ...
        if (!response) {
            return {
                content: [
                    { type: 'text', text: 'Permission revoke timed out.' },
                ],
                isError: true,
            };
        }
        if (!response.ok) {
            return {
                content: [
                    {
                        type: 'text',
                        text: formatTaskFailureLines(response, 'Permission revoke was rejected.').join('\n'),
                    },
                ],
                isError: true,
            };
...
function formatAdminPermissionList() {
    ...
    return [
        ...
        ...ADMIN_MCP_TOOL_NAMES.map((toolName) => {
            const status = toolName === 'admin_permission_list'
                ? 'available (read-only)'
