...
export function registerFileTools(server) {
    server.tool('file', 'List, read, write, or promote Gantry FileArtifacts by virtual scope/path. Descriptors are compact by default; file content is returned only by read. Host filesystem paths and storage refs are never exposed.', fileToolSchema, async (args) => {
        const result = await handleFileToolAction(args);
        return { content: [{ type: 'text', text: result }] };
    });
}
export async function handleFileToolAction(args) {
    return requestHostFileArtifactAction(args);
}
async function requestHostFileArtifactAction(args) {
    ...
    const response = await waitForTaskResponse(taskId, FILE_ARTIFACT_TASK_TIMEOUT_MS);
    if (!response) {
...
export function measureFileToolPayloadSize(value) {
    return Buffer.byteLength(compactJson(value), 'utf-8');
}
function compactJson(value) {
    return JSON.stringify(value);
}
function compactPayload(args) {
    return Object.fromEntries(Object.entries(args).filter(([, value]) => value !== undefined));
}
export function descriptorPayloadBytes(descriptors) {
