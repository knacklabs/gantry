...
import { z } from 'zod';
import { nowIso, nowMs, nowMs as currentTimeMs, } from '../../../shared/time/datetime.js';
import { ensurePrivateDirSync, writePrivateFileSync, } from '../../../shared/private-fs.js';
import { agentId, appId, chatJid, providerAccountId, workspaceFolder, IPC_AUTH_TOKEN, IPC_DIR, IPC_RESPONSE_KEY_ID, MESSAGES_DIR, threadId, jobId, permissionLane, jobRunId, jobRunLeaseToken, jobRunLeaseFencingVersion, } from '../context.js';
...
import { sleepWithAbort, USER_QUESTION_POLL_INTERVAL_MS, USER_QUESTION_TIMEOUT_MS, waitForUserQuestionResponse, } from './user-question-response-wait.js';
const INTERACTION_BOUNDARY_WAIT_MS = 2_000;
const fallbackTextSchema = z
    .string()
    .trim()
    .min(1)
    .describe('Required plain-text fallback for clients that cannot render rich UI');
...
const richMediaItemSchema = z
    ...
    .strict();
async function requestUserInteractionBoundary(requestId, signal) {
    ...
    writePrivateFileSync(tmpPath, JSON.stringify({
    ...
    while (nowMs() < deadline) {
        if (!fs.existsSync(boundaryPath))
            return;
        const aborted = await sleepWithAbort(USER_QUESTION_POLL_INTERVAL_MS, signal);
        if (aborted)
            return;
    }
}
function richInteractionContext() {
    return {
        ...(appId ? { appId } : {}),
        ...(agentId ? { agentId } : {}),
...
function writeRichInteractionRequest(kind, title, fallbackText, payload) {
    if (jobId)
        return false;
...
function richInteractionQueuedText(queued, form = false) {
    if (!queued)
        return 'Rich interaction skipped for scheduled job.';
    return form ? 'Form queued.' : 'Rich interaction queued.';
}
function registerRichInteractionTools(server) {
    server.tool('render_status', 'Render a compact status view in the active conversation.', {
        title: richTitleSchema,
        status: z.enum(['info', 'success', 'warning', 'error']),
        body: z.string().trim().min(1).max(4000).optional(),
        fallback_text: fallbackTextSchema,
    }, async (args) => {
        const queued = writeRichInteractionRequest('status', args.title, args.fallback_text, {
            status: args.status,
            ...(args.body ? { body: args.body } : {}),
        });
        return {
            content: [
                { type: 'text', text: richInteractionQueuedText(queued) },
            ],
        };
    });
    server.tool('render_facts', 'Render labeled facts in the active conversation.', {
        title: richTitleSchema,
        facts: z.array(richFactSchema).min(1).max(20),
        fallback_text: fallbackTextSchema,
    }, async (args) => {
        const queued = writeRichInteractionRequest('facts', args.title, args.fallback_text, {
            facts: args.facts,
        });
        return {
            content: [
    ...
    server.tool('render_list', 'Render an ordered or unordered list in the active conversation.', {
        title: richTitleSchema,
        ordered: z.boolean().optional(),
        items: z.array(richListItemSchema).min(1).max(30),
        fallback_text: fallbackTextSchema,
    }, async (args) => {
        const queued = writeRichInteractionRequest('list', args.title, args.fallback_text, {
    ...
    server.tool('render_table', 'Render a small data table in the active conversation.', {
        title: richTitleSchema,
        columns: z.array(richTableColumnSchema).min(1).max(10),
        rows: z.array(z.record(z.string(), richScalarSchema)).min(1).max(20),
        fallback_text: fallbackTextSchema,
    }, async (args) => {
        const queued = writeRichInteractionRequest('table', args.title, args.fallback_text, {
    ...
    server.tool('render_form', 'Render a form in the active conversation. Form submission is non-blocking in this runtime version.', {
        title: richTitleSchema,
        fields: z.array(richFormFieldSchema).min(1).max(10),
        fallback_text: fallbackTextSchema,
    }, async (args) => {
        const queued = writeRichInteractionRequest('form', args.title, args.fallback_text, {
            fields: args.fields,
        });
        return {
            content: [
    ...
    server.tool('render_media', 'Render media references in the active conversation.', {
        title: richTitleSchema,
        items: z.array(richMediaItemSchema).min(1).max(10),
        fallback_text: fallbackTextSchema,
    }, async (args) => {
        const queued = writeRichInteractionRequest('media', args.title, args.fallback_text, {
            items: args.items,
        });
        return {
            content: [
    ...
    server.tool('render_progress', 'Render one compact progress line for a user-visible workflow. Repeated calls edit the active line in place; use it before and between meaningful steps of long installs, dependency setup, and renders.', {
        title: richTitleSchema,
        ...
        const queued = writeRichInteractionRequest('progress', args.title, args.fallback_text, {
...
export function registerMessagingTools(server, claimProbe) {
    server.tool('send_message', "Send a message to the user or group immediately while you're still running. Use this for live progress updates or to send multiple messages. In scheduled jobs, the scheduler sends the completion notification, so do not use this for job results.", {
        ...
        files: z
            ...
            .max(5)
            .optional()
            .describe("Files to send with the message. Omit source (or set it to 'artifact') for an owned FileArtifact. Set source to 'workspace' to read a relative path inside your workspace. Gantry never falls back between sources and degrades safely when the selected file or channel attachment is unavailable."),
        sender: z
            .string()
            .optional()
    ...
    server.tool('ask_user_question', 'Ask the user a structured multiple-choice question across the active channel. Use when you need the user to pick between discrete options (e.g. which database, which approach, which config). Returns the selected option(s).', {
        questions: z
            .array(z.object({
            question: z
                .string()
        ...
        const payload = {
            ...
            ...(permissionLane === 'autonomous'
                ? {
                    expiresAt: new Date(currentTimeMs() + USER_QUESTION_TIMEOUT_MS).toISOString(),
                }
                : {}),
        };
        // prettier-ignore
