...
function validateLocalCliImplementation(implementation, ctx, basePath = []) {
    ...
    const at = (field) => [...basePath, 'implementation', field];
    if (!executablePath) {
        ctx.addIssue({
...
export const schedulerAccessRequirementSchema = z
    ...
    target: z.union([
        z
            ...
            rule: z
                .string()
                .describe('Readable tool rule: capability:<id>, Browser, exact mcp__gantry__ tool, or scoped RunCommand(...).'),
        })
            .strict(),
        z
            .object({
