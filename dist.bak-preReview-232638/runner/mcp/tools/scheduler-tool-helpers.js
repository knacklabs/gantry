...
import { parseSchedulerTargetShortcut, resolveSchedulerShortcut, routeLabelForShortcut, } from '../scheduler-utils.js';
...
export async function requestSchedulerData(type, payload, timeoutMs = 20_000) {
...
export function normalizeSchedulerWaitTimeoutMs(value) {
...
export function schedulerTaskError(response, fallback) {
    if (!response) {
        return {
            content: [{ type: 'text', text: `${fallback} timed out.` }],
            isError: true,
        };
    }
    if (!response.ok) {
...
export async function submitSchedulerMutationTask(input) {
    ...
    const response = await waitForTaskResponse(input.taskId, input.timeoutMs ?? 20_000);
    if (!response) {
        return {
...
export function schedulerDataRecord(response) {
...
function normalizeExecutionContextArg(value) {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return null;
    const record = value;
    const conversationJid = typeof record.conversation_jid === 'string'
        ? record.conversation_jid.trim()
        : '';
    const workspaceKey = typeof record.workspace_key === 'string' ? record.workspace_key.trim() : '';
    const threadRaw = record.thread_id;
    const threadIdValue = threadRaw === null
        ? null
        : typeof threadRaw === 'string'
            ? threadRaw.trim()
            : undefined;
    const sessionRaw = record.session_id;
    const sessionIdValue = sessionRaw === null
        ? null
    ...
    if (threadRaw !== null && !threadIdValue)
        return null;
...
function normalizeNotificationRoutesArg(value) {
    if (value === undefined)
        return null;
...
export function canonicalTargetFromArgs(args, useAmbientDefault) {
    ...
    if (args.execution_context !== undefined && !executionContext) {
        return {
            ...
            notificationRoutes: [],
            error: 'execution_context must include conversation_jid, workspace_key, and thread_id.',
        };
    }
    const notificationRoutes = normalizeNotificationRoutesArg(args.notification_routes);
    if (args.notification_routes !== undefined && !notificationRoutes) {
        return {
            executionContext: executionContext ?? defaultExecutionContext,
            notificationRoutes: [],
            error: 'notification_routes entries require conversation_jid, thread_id, and label.',
        };
    }
    ...
    if (shortcut && executionContext) {
        ...
        if ((executionContext.threadId ?? null) !== shortcutThreadId) {
            return {
                ...
                notificationRoutes: [],
                error: 'execution_context.thread_id conflicts with the selected target shortcut.',
            };
        }
    }
