...
import { schedulerEventsSummary, schedulerJobSummary, schedulerJobsSummary, schedulerNotificationTargetsSummary, schedulerRunsSummary, } from './scheduler-formatters.js';
import { formatSchedulerJobPlan, schedulerJobConfirmationToken, } from '../../../shared/scheduler-job-plan.js';
import { canonicalTargetFromArgs, normalizeSchedulerWaitTimeoutMs, requestSchedulerData, schedulerDataRecord as dataRecord, schedulerTaskError as taskError, submitSchedulerMutationTask, SCHEDULER_WAIT_RESPONSE_GRACE_MS, } from './scheduler-tool-helpers.js';
...
import { schedulerModelRecommendationSchema, } from './scheduler-model-recommendation-schema.js';
...
function removedExecutionScopeFieldError(args) {
    const containers = [args, args.execution_context];
...
function unsupportedSchedulerArgError(args, allowedKeys) {
    const removedScopeError = removedExecutionScopeFieldError(args);
    if (removedScopeError)
        return removedScopeError;
    ...
    return {
        content: [
            {
                type: 'text',
                text: `Unsupported scheduler fields: ${unsupported.join(', ')}. Use execution_context and notification_routes for routing.`,
            },
        ],
        isError: true,
    };
...
const executionContextSchema = z
    ...
    if ('group_scope' in val || 'groupScope' in val) {
        ctx.addIssue({
            ...
            message: 'group_scope/groupScope is no longer accepted. Use workspace_key.',
        });
    }
});
function validateScheduleInput(args) {
    ...
    if (args.schedule_type === 'interval') {
        ...
        if (isNaN(ms) || ms <= 0) {
            return {
                content: [
                    { type: 'text', text: 'Invalid interval milliseconds.' },
                ],
                isError: true,
            };
        }
    ...
    if (args.schedule_type === 'once') {
        ...
        if (!date) {
            return {
                content: [{ type: 'text', text: 'Invalid once timestamp.' }],
                isError: true,
            };
        }
    }
...
export function registerSchedulerTools(server) {
    server.tool('scheduler_list_models', 'List supported model aliases for one-time and recurring scheduler jobs.', schedulerModelRecommendationSchema, async (rawArgs) => {
        const args = (rawArgs ?? {});
        return {
            content: [
                {
    ...
    server.tool('scheduler_upsert_job', 'Create or update a scheduler job. Idempotent by job ID.', {
        ...
        access_requirements: z
            ...
            .optional()
            .describe('Access this job needs as a single list. Each entry has a target: {kind:"capability", capability_id, implementation?} for reviewed semantic capabilities, {kind:"tool_rule", rule} for exact tools/Browser/scoped RunCommand(...), or {kind:"mcp_server", server}. Missing access pauses setup for user approval.'),
        ...
        confirm: z
            .boolean()
            .optional()
            .describe('Set true only after reviewing the returned plan and passing confirmation_token.'),
        confirmation_token: z
            .string()
            .optional()
            .describe('Token returned by the explain-before-confirm scheduler job plan.'),
    }, async (args) => {
        const unsupportedArgError = unsupportedSchedulerArgError(args, SCHEDULER_UPSERT_ARG_KEYS);
        if (unsupportedArgError)
            return unsupportedArgError;
        const scheduleError = validateScheduleInput(args);
        if (scheduleError)
            return scheduleError;
        const canonicalTarget = canonicalTargetFromArgs(args, true);
        if (canonicalTarget.error) {
            return {
                content: [{ type: 'text', text: canonicalTarget.error }],
                isError: true,
            };
        }
        const planInput = {
            ...
            accessRequirements: normalizeSchedulerAccessRequirements(args.access_requirements),
            silent: args.silent,
        ...
        return submitSchedulerMutationTask({
            ...
            timeoutText: 'Scheduler upsert timed out waiting for host confirmation.',
            rejectedText: 'Scheduler upsert was rejected.',
            successText: 'Scheduler job upsert completed.',
        });
    });
    server.tool('scheduler_get_job', 'Get one scheduler job by ID from the host scheduler.', { job_id: z.string() }, async (args) => {
    ...
    server.tool('scheduler_list_jobs', 'List scheduler jobs from the host scheduler.', {
        ...
        return {
            content: [
                { type: 'text', text: schedulerJobsSummary(result) },
            ],
        };
    });
    server.tool('scheduler_list_notification_targets', 'List valid notification targets for scheduler jobs in the current conversation context.', {}, async () => {
        const response = await requestSchedulerData('scheduler_list_notification_targets', {});
        const error = taskError(response, 'Scheduler notification target listing failed.');
        if (error)
            return error;
    ...
    server.tool('scheduler_update_job', 'Update mutable fields on a scheduler job.', {
        job_id: z.string(),
        ...
        const unsupportedArgError = unsupportedSchedulerArgError(args, SCHEDULER_UPDATE_ARG_KEYS);
        if (unsupportedArgError)
            return unsupportedArgError;
        const canonicalTarget = canonicalTargetFromArgs(args, false);
        if (canonicalTarget.error) {
            return {
                content: [{ type: 'text', text: canonicalTarget.error }],
                isError: true,
        ...
        return submitSchedulerMutationTask({
            ...
            timeoutText: 'Scheduler update timed out waiting for host confirmation.',
            rejectedText: 'Scheduler update was rejected.',
            successText: 'Scheduler job update completed.',
        });
    });
    server.tool('scheduler_delete_job', 'Delete a scheduler job.', { job_id: z.string() }, async (args) => {
        const taskId = makeIpcId('scheduler-delete');
        return submitSchedulerMutationTask({
            taskType: 'scheduler_delete_job',
            taskId,
            payload: { jobId: args.job_id },
            timeoutText: 'Scheduler delete timed out waiting for host confirmation.',
            rejectedText: 'Scheduler delete was rejected.',
            successText: 'Scheduler job delete completed.',
        });
    });
    server.tool('scheduler_pause_job', 'Pause a scheduler job.', { job_id: z.string() }, async (args) => {
    ...
    server.tool('scheduler_resume_job', 'Resume a paused scheduler job.', { job_id: z.string() }, async (args) => {
        const taskId = makeIpcId('scheduler-resume');
        return submitSchedulerMutationTask({
            taskType: 'scheduler_resume_job',
            taskId,
            payload: { jobId: args.job_id },
            timeoutText: 'Scheduler resume timed out waiting for host confirmation.',
            rejectedText: 'Scheduler resume was rejected.',
            successText: 'Scheduler job resume completed.',
        });
    });
    server.tool('scheduler_run_now', 'Queue an immediate run of an existing scheduler job.', { job_id: z.string() }, async (args) => {
    ...
    server.tool('scheduler_list_runs', 'List job runs from the host scheduler.', {
        job_id: z.string().optional(),
        limit: z.number().optional(),
    }, async (args) => {
        ...
        return {
            content: [
                { type: 'text', text: schedulerRunsSummary(result) },
            ],
        };
    });
    server.tool('scheduler_list_events', 'List scheduler lifecycle events from the host scheduler.', {
        job_id: z.string().optional(),
        ...
        return {
            content: [
                { type: 'text', text: schedulerEventsSummary(result) },
            ],
        };
    });
    server.tool('scheduler_wait_for_events', 'Wait for scheduler lifecycle events from the host scheduler.', {
        job_id: z.string().optional(),
        run_id: z.string().optional(),
        event_type: z.string().optional(),
        since_id: z.number().optional(),
        ...
        const response = await requestSchedulerData('scheduler_wait_for_events', {
            jobId: args.job_id,
            runId: args.run_id,
            eventType: args.event_type,
            sinceId: args.since_id,
    ...
    server.tool('scheduler_get_dead_letter', 'List dead-lettered job runs from the host scheduler.', { limit: z.number().optional() }, async (args) => {
