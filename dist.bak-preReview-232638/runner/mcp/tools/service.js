...
import { availableSemanticCapabilities, attachedMcpSourceIds, chatJid, currentConfiguredAllowedTools, deploymentMode, isAdminMcpToolEnabled, memoryUserId, memoryDefaultScope, providerAccountId, TASKS_DIR, threadId, } from '../context.js';
import { waitForTaskResponse, writeIpcFile } from '../ipc.js';
import { SKILL_APPROVAL_WAIT_MS } from './service-constants.js';
import { formatMcpApprovalResponse, formatSkillProposalResponse, } from './service-formatters.js';
...
export function registerServiceTools(server) {
    registerSkillProposalTool(server, 'request_skill_proposal', 'Request skill source setup for an agent-created or modified skill bundle. Approval makes the skill available as inventory; risky actions still require reviewed capability access.');
    server.tool('pattern_candidate_decision', 'Record the user decision for a pattern_id from [[PATTERNS_NOTICED]]. Use accept only after the user agrees to a scheduler_job, durable_capability, or memory_update fix; skill fixes go through request_skill_proposal.', {
    ...
    server.tool('proactive_surfacing_consent', 'Record the user explicit decision to keep flagging (enable) or stop flagging (opt_out) proactive patterns for this conversation. Use ONLY after the user explicitly says so this turn.', {
    ...
    server.tool('request_skill_install', 'Request skill source setup for same-conversation admin approval. Approval installs staged files, or runs an approved installer command in host-controlled staging and imports the resulting SKILL.md package. Skill source approval records inventory only; reviewed gantry.skill.json actions become capability candidates. Keep long installs visible with one render_progress line before and after meaningful stages.', {
        ...
        files: z
            ...
            .max(50)
            .optional()
            .describe('Staged skill files. Must include SKILL.md with name and description frontmatter.'),
        installCommandArgv: z
            .array(z.string())
            .min(1)
            .max(40)
            .optional()
            .describe('Optional installer command argv, such as an npx skills.sh or clawhub installer command. Use only when files are not available.'),
        reason: z.string().describe('Why this skill should be installed'),
    }, async (args) => {
        const wrongLaneGuidance = browserWrongLaneRequestGuidance('request_skill_install', {
            expectedFiles: args.expectedFiles ?? [],
            dependencies: args.dependencies ?? [],
            requiredEnvVars: args.requiredEnvVars ?? [],
            reason: args.reason,
        });
        if (wrongLaneGuidance)
            return wrongLaneGuidance;
        return submitCapabilityReviewTask('request_skill_install', 'Skill install', {
            expectedFiles: args.expectedFiles ?? [],
            dependencies: args.dependencies ?? [],
            requiredEnvVars: args.requiredEnvVars ?? [],
            files: args.files ?? [],
    ...
    server.tool('request_skill_dependency_install', deploymentMode === 'fleet'
    ...
    }, async (args) => submitCapabilityReviewTask('request_skill_dependency_install', 'Skill dependency install', {
    ...
    registerAccessRequestTool(server, submitCapabilityReviewTask, {
        ...
        isCapabilitySelected: (capabilityId) => currentConfiguredAllowedTools().includes(`capability:${capabilityId}`),
        isToolSelected: (toolName) => currentConfiguredAllowedTools().includes(toolName),
        validateRunCommandFallback: ({ argvPattern }) => {
            const currentAllowedTools = currentConfiguredAllowedTools();
            const selectedMcpCapabilities = availableSemanticCapabilities.filter((capability) => currentAllowedTools.includes(`capability:${capability.capabilityId}`) &&
                capability.implementationBindings.some((binding) => binding.kind === 'mcp_tool' ||
                    binding.kind === 'mcp_pattern' ||
                    Boolean(binding.mcpTool)));
            const selectedMcpCapabilityIds = selectedMcpCapabilities
                .map((capability) => capability.capabilityId)
                .sort();
            if (selectedMcpCapabilityIds.length === 0)
                return null;
            const requestedPattern = normalizeMcpServerName(argvPattern);
            const selectedMcpNames = [
                ...new Set(selectedMcpCapabilities.flatMap((capability) => mcpCapabilityNames(capability))),
            ].filter(Boolean);
            const targetsSelectedMcp = selectedMcpNames.some((name) => requestedPattern.includes(name));
            if (!targetsSelectedMcp)
                return null;
            return {
                isError: true,
                content: [
    ...
    server.tool('request_mcp_server', 'Request third-party MCP source setup for admin review. Approval connects the source; raw MCP tools are inventory and do not become durable capabilities until reviewed.', {
        name: z
            .string()
        ...
        args: z
            .array(z.string())
            .optional()
            .describe('Template arguments. npx-package requires exactly one safe npm package spec.'),
        sandboxProfileId: z
            .string()
        ...
        networkHosts: z
            .array(z.string())
            .optional()
            .describe('Outbound hosts the server may reach, as exact host or host:port (no URLs, wildcards, or private/localhost targets)'),
        reason: z.string().describe('Why this capability is needed'),
        docsUrl: z.string().optional().describe('Optional documentation URL'),
    }, async (args) => {
        const wrongLaneGuidance = browserWrongLaneRequestGuidance('request_mcp_server', {
            name: args.name,
            origin: undefined,
        ...
        return {
            content: [
                {
                    type: 'text',
                    text: formatMcpApprovalResponse(response.data, response.message ||
    ...
    server.tool('service_restart', 'Restart the Gantry service with config validation. Requires selected agent tool grant mcp__gantry__service_restart.', {}, async () => {
    ...
    server.tool('register_agent', `Register the current chat/channel agent so Gantry can respond to messages there. Requires selected agent tool grant mcp__gantry__register_agent and same-conversation approver approval.
...
The JID must be the current conversation. The folder name must be channel-prefixed: "{channel}_{conversation-name}" (e.g., "telegram_dev-team", "slack_eng", "discord_ops", "teams_engineering"). Use lowercase with hyphens for the conversation name part.`, {
        jid: z
            .string()
            .describe('The chat JID (e.g., "tg:-1001234567890", "sl:C0123456789", "dc:1234567890", "teams:19:abc@thread.v2")'),
        ...
        requiresTrigger: z
            .boolean()
            .optional()
            .describe('Whether messages must start with the trigger word. Default: false (respond to all messages). Set to true for busy groups with many participants where you only want the agent to respond when explicitly mentioned.'),
    }, async (args) => {
...
function mcpCapabilityNames(capability) {
    const names = [capability.capabilityId];
    const sourceServerName = mcpSourceServerName(capability.source);
    if (sourceServerName)
        names.push(sourceServerName);
    for (const binding of capability.implementationBindings) {
        if (binding.kind === 'mcp_pattern') {
            if (binding.mcpServer)
                names.push(binding.mcpServer);
            continue;
        }
        if (binding.kind !== 'mcp_tool' && !binding.mcpTool)
            continue;
...
function adminToolUnavailable(toolName) {
...
function existingMcpSourceForRequest(name) {
    ...
    if (attachedSourceName) {
        return {
            ...
            selectedCapabilities: selectedMcpCapabilitiesForSource(attachedSourceName),
        };
    }
...
function selectedMcpCapabilitiesForSource(serverName) {
    ...
    return availableSemanticCapabilities
        .filter((capability) => {
        if (!currentAllowedTools.includes(`capability:${capability.capabilityId}`)) {
            return false;
        }
        const sourceServerName = mcpSourceServerName(capability.source);
...
function mcpSourceServerName(source) {
...
function displayMcpSourceName(sourceId) {
...
async function submitCapabilityReviewTask(toolName, requestLabel, payload) {
    ...
    return {
        content: [
            {
                type: 'text',
                text: toolName === 'request_skill_install'
                    ? formatSkillProposalResponse(response.data, response.message || `${requestLabel} approved.`, { deploymentMode })
                    : response.message ||
...
function registerSkillProposalTool(server, toolName, description) {
    ...
    server.tool(toolName, description, {
        files: z
            ...
            .min(1)
            .max(50)
            .describe('Skill files. Must include SKILL.md with name and description frontmatter.'),
        ...
        return {
            content: [
                {
                    type: 'text',
                    text: formatSkillProposalResponse(response.data, response.message || `${requestLabel} installed.`, { deploymentMode }),
                },
            ],
        };
    });
