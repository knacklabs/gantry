...
import { formatGuidedActionPreview, } from '../../../application/guided-actions/guided-action-service.js';
import { makeIpcId } from '../ipc-ids.js';
const SETTINGS_APPROVAL_WAIT_MS = 5 * 60 * 1000;
export function registerSettingsTools(server, options) {
    server.tool('settings_desired_state', 'Read the current local settings.yaml desired state before requesting local Gantry configuration changes. Requires selected agent tool grant mcp__gantry__settings_desired_state.', {}, async () => {
    ...
    server.tool('guided_action_preview', 'Preview the current control-plane guided action (next action) before requesting any change. Read-only. Requires selected agent tool grant mcp__gantry__guided_action_preview.', {}, async () => {
        ...
        const preview = response.data && typeof response.data === 'object'
            ? response.data
            : undefined;
        return {
            content: [
    ...
    server.tool('request_settings_update', 'Request a reviewed update to local settings.yaml. Requires selected agent tool grant mcp__gantry__request_settings_update.', {
        replacementYaml: z
            .string()
            .describe('Complete replacement settings.yaml content'),
        expectedRevision: z
            .string()
            .describe('Revision returned by settings_desired_state for lost-update protection'),
        ...
        const response = await waitForTaskResponse(taskId, SETTINGS_APPROVAL_WAIT_MS);
        if (!response?.ok) {
            return {
                content: [
                    {
