...
import { chatJid, jobRunId, jobId, jobRunLeaseFencingVersion, jobRunLeaseToken, TASKS_DIR, agentId, appId, providerAccountId, threadId, } from '../context.js';
import { CALLABLE_AGENT_RESPONSE_TIMEOUT_MS, CALLABLE_AGENT_SYNC_WAIT_TIMEOUT_MS, callableAgentToolDescription, callableAgentToolName, createCallableAgentToolSchema, } from '../../../shared/callable-agent-manifest.js';
...
async function submitTaskLifecycleRequest(input) {
    ...
    const response = await waitForTaskResponse(taskId, input.responseTimeoutMs ?? TASK_TOOL_TIMEOUT_MS);
    if (!response) {
        return {
            content: [{ type: 'text', text: input.timeoutMessage }],
            isError: true,
        };
    }
    if (!response.ok) {
...
export function registerTaskLifecycleTools(server, callableAgentManifest = []) {
    server.tool('async_run_command', 'Start an approved shell command as a durable background task. Use only for long-running commands that should continue while you inspect status with task_get or task_list. The host enforces selected RunCommand(...) capability rules before it creates the task.', {
        command: z.string().min(1).max(20_000),
    }, async (args) => submitTaskLifecycleRequest({
        type: 'async_run_command',
        payload: { command: args.command },
        timeoutMessage: 'Async command start timed out.',
        fallbackError: 'Async command start failed.',
    }));
    server.tool('task_get', 'Read the current status and terminal receipt for one durable async task created by this agent in this conversation.', {
    ...
    server.tool('task_list', 'List recent durable async tasks created by this agent in this conversation.', {}, async () => submitTaskLifecycleRequest({
        type: 'task_list',
        payload: {},
        timeoutMessage: 'Task list timed out.',
        fallbackError: 'Task list failed.',
    }));
    server.tool('task_cancel', 'Cancel one running durable async task created by this agent in this conversation. Cancellation aborts the active command when it is still running.', {
        taskId: z.string().min(1).max(160),
    }, async (args) => submitTaskLifecycleRequest({
        type: 'task_cancel',
        payload: { taskId: args.taskId },
    ...
    server.tool('delegate_task', 'Start a durable async child agent run. Use task_get/task_list to inspect it and task_message to steer it while it is running.', {
    ...
    const dynamicServer = server;
    for (const entry of callableAgentManifest) {
        dynamicServer.registerTool(callableAgentToolName(entry), {
            ...
            payload: {
                ...args,
                syncWaitTimeoutMs: typeof args.syncWaitTimeoutMs === 'number'
    ...
    server.tool('task_message', 'Send a steering message to a running delegated async task. Terminal tasks and async command tasks reject steering messages.', {
        taskId: z.string().min(1).max(160),
    ...
    server.tool('todo_update', 'Publish and maintain a visible multi-step plan for this run, updating item status as work progresses; it renders as one live, in-place list per channel. This is display-only planning state only; it cannot grant tools, create permissions, change settings, or trigger work.', {
