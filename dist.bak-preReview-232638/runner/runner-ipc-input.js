...
function ipcInputDir() {
    ...
    if (!dir) {
        throw new Error('Missing required environment variable: GANTRY_IPC_INPUT_DIR');
    }
    return dir;
}
export function interactiveIpcInputDir() {
    return ipcInputDir();
}
function closeSentinelPath() {
...
export function drainIpcInput(log) {
    try {
