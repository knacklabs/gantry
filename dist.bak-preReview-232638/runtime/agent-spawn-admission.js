...
import { formatInlineAgentWorkerOnlyConfigError, inlineAgentSkillEngineConstraintError, inlineWorkerOnlyToolRuleLabels, } from '../shared/agent-runtime.js';
import { parseSemanticCapabilityRule } from '../shared/semantic-capability-ids.js';
import { fixedImageSetupRequiredMessage, missingImageCapabilities, readImageCapabilityInventory, } from '../shared/worker-image-inventory.js';
...
export function validateAgentPreSpawnAdmission(input) {
    ...
    if (inlineAdmissionError)
        return inlineAdmissionError;
    const controlError = agentControlAdmissionError(input);
    if (controlError)
        return controlError;
    return (validateRunnerAllowedTools(input.agentInput.toolPolicyRules ?? [], input.agentInput.runtimeAccess ?? []) ??
...
function agentControlAdmissionError(input) {
    ...
    if (agentInput.effort !== undefined) {
        ...
        if (modelEntry.supportedEffortLevels.length > 0 &&
...
function inlineRuntimePreSpawnAdmissionError(input) {
    ...
    for (const rule of inlineWorkerOnlyToolRuleLabels(input.agentInput.toolPolicyRules ?? [])) {
        labels.add(rule);
    }
    const stdioMcpSourceIds = new Set(input.stdioMcpSourceIds ?? []);
    for (const source of input.agentInput.attachedMcpSourceIds ?? []) {
        if (stdioMcpSourceIds.has(source))
            labels.add(source);
    }
    ...
    const workerOnlyError = labels.size === 0
        ? null
        : formatInlineAgentWorkerOnlyConfigError('agent', [...labels].sort());
    return [skillEngineError, workerOnlyError].filter(Boolean).join('; ') || null;
}
function fixedImageCapabilityPreflightError(input) {
    const imageInventory = readImageCapabilityInventory();
    if (!imageInventory)
        return null;
    const selectedSemanticCapabilityIds = new Set((input.toolPolicyRules ?? [])
        .map((rule) => parseSemanticCapabilityRule(rule))
        .filter((id) => Boolean(id)));
    const missing = missingImageCapabilities([...selectedSemanticCapabilityIds].map((capabilityId) => ({
        capabilityId,
    })), imageInventory);
