import { resolveAgentExecutionAdapter } from '../application/agent-execution/agent-execution-adapter-registry.js';
export function resolveSpawnExecutionAdapter(executionProviderId, options) {
    try {
        const executionAdapter = resolveAgentExecutionAdapter({
            executionProviderId,
            registry: options?.executionAdapters,
            fallback: options?.executionAdapter,
        });
        if (executionAdapter)
            return { ok: true, executionAdapter };
        return {
            ok: false,
            output: {
                status: 'error',
                result: null,
                error: 'No LLM execution adapter configured. Runtime bootstrap must provide an AgentExecutionAdapterRegistry.',
            },
        };
    }
    catch (err) {
        return {
            ok: false,
            output: {
                status: 'error',
                result: null,
                error: err instanceof Error ? err.message : String(err),
            },
        };
    }
}
