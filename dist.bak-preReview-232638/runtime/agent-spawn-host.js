...
import { AGENT_TIMEOUT, DATA_DIR, IDLE_TIMEOUT, getCredentialBrokerRuntimeConfig, getEffectiveModelConfig, getRuntimeSettingsForConfig, getSelectedAgentHarness, } from '../config/index.js';
...
import { resolveWorkspaceFolderPath, resolveWorkspaceIpcPath, } from '../platform/workspace-folder.js';
import { ensureWorkspaceIpcLayout, getHostAgentRunnerDistDir, } from './agent-spawn-layout.js';
import { resolveSpawnModel } from './agent-spawn-model-resolution.js';
import { compileSpawnSystemPrompt, resolveSpawnPromptAccessPreset, } from './agent-spawn-prompt.js';
import { getConfiguredModelProvidersForApp, getRuntimeFileArtifactStore, } from '../adapters/storage/postgres/runtime-store.js';
import { inlineCoreToolsMountMcpInventory } from './core-tools/registry.js';
export function getConfiguredAgentMaxRunTokens(agentFolder) {
    return getRuntimeSettingsForConfig().agents?.[agentFolder]?.maxRunTokens;
}
export function createConfiguredRunTokenBudget(agentFolder) {
    let maxRunTokens;
    let settingsRead = false;
    const seenUsageEventIds = new Set();
    ...
    return {
        ...
        enforce(output) {
            ...
            if (duplicate)
                return output;
            if (output.usageEventId)
                seenUsageEventIds.add(output.usageEventId);
            else
                seenUsage.add(output.usage);
...
export function withControls(input, defaults, conversationPermissionMode) {
    const { toolRules: _untrustedToolRules, permissionMode: _untrustedPermissionMode, ...trustedInput } = input;
    ...
    return {
        ...
        permissionMode: resolveEffectivePermissionMode(conversationPermissionMode, defaults?.permissionMode),
        ...(toolRules?.length ? { toolRules } : {}),
    };
}
export async function prepareInlineAgentHostContext(group, input) {
    ...
    const configuredAccessPreset = resolveAgentAccessPolicy(runtimeSettings.agents?.[group.folder]?.accessPreset).preset;
    const promptAccessPreset = resolveSpawnPromptAccessPreset(configuredAccessPreset, input.hideAuthorityTools === true ||
    ...
    const compiledSystemPrompt = resolvedModel.ok
        ...
        : undefined;
    const effectiveInput = withControls(input, runtimeSettings.agents?.[group.folder], group.agentConfig?.permissionMode);
    return {
        resolvedModel,
...
export async function getHostRuntimeCredentialEnv(agentIdentifier, broker, options = {}) {
    ...
    const runId = options.runId ??
        ...
        `credential-run:${randomUUID()}`;
    const bindingOptions = {
        purpose,
        appId: options.appId ?? options.runContext?.appId,
        agentId: options.agentId ?? options.runContext?.agentId,
        runId,
        jobId: options.jobId ?? options.runContext?.jobId,
        conversationId: options.conversationId ?? options.runContext?.chatJid,
        threadId: options.threadId ?? options.runContext?.threadId,
        modelRouteId: options.modelRouteId,
    };
    const injection = brokerConfig.mode === 'gantry'
...
function requireGantryBroker(broker) {
    if (!broker) {
        throw new Error('Gantry Model Gateway is enabled but no model credential broker was provided.');
    }
    return broker;
}
export function prepareHostRuntimeContext(group) {
