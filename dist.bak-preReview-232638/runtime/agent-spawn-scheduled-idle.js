...
export function scheduledJobIdleTimeoutMs() {
...
export function readScheduledJobHeartbeat(output) {
    for (const event of output.runtimeEvents ?? []) {
        ...
        return {
            ...
            currentTool: typeof record.currentTool === 'string' ? record.currentTool : undefined,
            lastActivityAt: typeof record.lastActivityAt === 'string'
                ? record.lastActivityAt
                : undefined,
            lastActivityAgoMs: typeof record.lastActivityAgoMs === 'number'
                ? record.lastActivityAgoMs
                : undefined,
            pendingPermissionRequests: typeof record.pendingPermissionRequests === 'number'
                ? record.pendingPermissionRequests
                : undefined,
            pendingPermissionToolNames: Array.isArray(record.pendingPermissionToolNames)
                ? record.pendingPermissionToolNames.filter((toolName) => typeof toolName === 'string')
                : undefined,
            totalToolCalls: typeof record.totalToolCalls === 'number'
