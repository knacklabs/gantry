...
import { DATA_DIR, PERMISSION_APPROVAL_TIMEOUT_MS, STORAGE_POSTGRES_SCHEMA, STORAGE_POSTGRES_URL, STORAGE_POSTGRES_URL_ENV, TIMEZONE, getDeploymentMode, getRuntimeSettingsForConfig, getEffectiveModelConfig, getSelectedAgentHarness, getSelectedAgentRuntime, } from '../config/index.js';
...
import { computeBrowserIpcAuthToken, computeMemoryIpcAuthToken, registerBrowserIpcAuthorization, revokeBrowserIpcAuthorization, revokeIpcResponseSigningKey, } from './ipc-auth.js';
import { getContinuationInputDir, taskContinuationThreadId, } from './continuation-input.js';
...
import { attachMcpSourceNetworkHosts, egressNetworkAttributionFromRuntimeAccess, localCliCredentialPathHintsFromRuntimeAccess, pickPreparedExecutionEnv, pickSafeHostEnv, pickSelectedCapabilityEnv, databaseNetworkHostFromUrl, resolveHomeRelativePaths, resolveRunnerMcpProjection, sandboxAllowedNetworkHostsFromRuntimeAccess, writeProtectedFilesystemEnv, } from './agent-spawn-runtime-policy.js';
import { getConfiguredModelProvidersForApp, getRuntimeFileArtifactStore, getRuntimeStorage, } from '../adapters/storage/postgres/runtime-store.js';
...
import { cleanupRunnerMcpConfigFile, cleanupRunnerTempDir, buildSandboxRuntimeNetworkProjection, deepAgentsFilesystemEnabledEnv, deepAgentsShellEnabledEnv, protectedWritePathsForOuterSandbox, sandboxRuntimeToolProcessEnv, sandboxRuntimeToolNetworkEnv, resolveRunnerSandboxStartup, uniqueStrings, buildRunnerSandboxSpawnInput, buildBaseRunnerEnv, buildAndLogRunnerRuntimeDetails, } from './agent-spawn-helpers.js';
...
import { resolveAgentSpawnLogContext, stripIncompleteRunLeaseIdentity, } from './agent-spawn-identity.js';
import { createRunnerTempDirectories } from './agent-spawn-temp-directories.js';
import { agentPersonasById, projectSpawnRunnerInput, } from './agent-spawn-input-projection.js';
...
async function spawnAgentWithContext(group, input, onProcess, options, turnTracker) {
    ...
    if (preparation.kind === 'inline')
        return preparation.output;
    const { agentRuntime, startTime, hostStartup, groupDir, processName } = preparation;
    const modelResolutionStarted = hostStartup.start();
    const runtimeSettings = getRuntimeSettingsForConfig();
    const modelConfig = getEffectiveModelConfig(input.isScheduledJob ? undefined : group.agentConfig?.model, input.isScheduledJob
        ? input.jobModelUseKind || 'recurringJob'
        : 'interactive', group.folder);
    ...
    input = host.withControls(input, agentSettings, group.agentConfig?.permissionMode);
    const agentEngine = resolvedModel.value.agentEngine;
    const effectiveModel = resolvedModel.value.runnerModel;
    const preSpawnAdmissionError = hostStartup.measure('preSpawnAdmissionMs', () => validateAgentPreSpawnAdmission({
        agentInput: input,
        agentEngine,
        agentRuntime,
    ...
    const { accessPreset, hideAuthorityTools, callableAgentManifest } = await prepareWorkerAuthorityProjection({
        agentInput: input,
        accessPreset: agentSettings?.accessPreset,
        delegates: agentSettings?.delegates ?? [],
        getConversationBoundAgentIds: () => conversationBoundAgentIdsForRoute({
    ...
    const { runnerInput, browserIpcEnabled, trustedToolPolicyRules } = projectSpawnRunnerInput({
        agentInput: input,
        workspaceFolder: group.folder,
    ...
    const adapterResolution = resolveSpawnExecutionAdapter(resolvedModel.value.executionProviderId, options);
    if (!adapterResolution.ok)
        return adapterResolution.output;
    const { executionAdapter } = adapterResolution;
    let mcpConfigPath;
    let sandboxConfigPath;
    let runnerTempDir;
    let providerToolTempDir;
    let egressGateway;
    const { ipcAuth, unregisterPermissionRunRestriction } = setupPermissionRunRestriction(group.folder, input, hideAuthorityTools);
    let hostCredentials;
    let preparedExecution;
    let output;
    try {
        const projectedCredentials = await hostStartup.measureAsync('credentialProjectionMs', () => credentials(agentIdentifier, options?.credentialBroker, {
        ...
        try {
            preparedExecution = await hostStartup.measureAsync('adapterPrepareMs', () => executionAdapter.prepare({
                group,
                ...
                browserIpcEnabled,
                packageRootFromRunner: (runnerPath) => resolvePackageRootFromSourceDir(path.dirname(runnerPath)),
                options,
            }));
        }
        catch (err) {
            ...
            const failure = generatedRuntimeError ??
        ...
        const ipcInputDir = getContinuationInputDir(group.folder, taskContinuationThreadId(input.threadId, input.parentTaskId));
        ...
        let reviewedMcpToolNames = [];
        let allMcpCapabilities = [];
        let selectedMcpServerNames = [];
        let projectedMcpSourceIds = [];
        let effectiveRuntimeAccess = input.runtimeAccess ?? [];
        await hostStartup.measureAsync('mcpProjectionMs', async () => {
            const mcpSourceRecords = options?.mcpServerRepository &&
            ...
            selectedMcpServerNames = uniqueStrings([
                ...
                ...attachedMcpSourceIds.map((sourceId) => sourceId.startsWith('mcp:')
            ...
            allMcpCapabilities =
                options?.mcpServerRepository &&
                    ...
                    ? await new McpServerService(options.mcpServerRepository, undefined, {
                        ...
                        appId: options.mcpContext.appId,
                        agentId: options.mcpContext.agentId,
                        serverIds: projectedMcpSourceIds,
                    ...
                    : [];
            effectiveRuntimeAccess = attachMcpSourceNetworkHosts(input.runtimeAccess ?? [], allMcpCapabilities);
        });
        runnerInput.runtimeAccess = effectiveRuntimeAccess;
        const networkAttribution = egressNetworkAttributionFromRuntimeAccess(effectiveRuntimeAccess);
        const memoryIpcAllowedActions = selectedMemoryIpcActionsFromToolRules(trustedToolPolicyRules ?? [], {
            memoryReviewerIsControlApprover: input.memoryReviewerIsControlApprover,
        });
        const upstreamProxyUrl = projectedCredentials.proxy?.https || projectedCredentials.proxy?.http;
        ...
        const checkpointerNetworkHost = databaseNetworkHostFromUrl(runnerInputPatch.deepAgentCheckpointer?.databaseUrl);
        ...
        const { runnerSandboxProviderId, sandboxWarmTemplate } = resolveRunnerSandboxStartup({
            provider: options?.runnerSandboxProvider,
            runtimeProvider: runtimeSandbox.provider,
            measure: hostStartup.measure,
        });
        const sandboxRuntimeNetwork = buildSandboxRuntimeNetworkProjection(runnerSandboxProviderId, sandboxAllowedNetworkHosts, runnerInput.modelCredentialEnv);
        ...
        egressGateway = await hostStartup.measureAsync('egressGatewayMs', () => ensureEgressGateway({
            ...
            ...(sandboxRuntimeNetwork.networkProjection.privateNetworkHostMappings
                ? {
                    privateNetworkHostMappings: sandboxRuntimeNetwork.networkProjection
        ...
        const toolNetworkEnv = sandboxRuntimeToolNetworkEnv(runnerSandboxProviderId, runnerInputPatch.toolNetworkEnv ??
            buildToolNetworkEnv({
                proxyUrl: egressGateway.proxyUrl,
                caBundlePath: runnerInputPatch.modelCredentialEnv?.NODE_EXTRA_CA_CERTS ??
        ...
        if (checkpointer)
            runnerInput.deepAgentCheckpointer = checkpointer;
        runnerInput.deepAgentSkills = runnerInputPatch.deepAgentSkills;
        const localCliCredentialPaths = resolveHomeRelativePaths(localCliCredentialPathHintsFromRuntimeAccess(effectiveRuntimeAccess), process.env);
        const workspaceExtraDir = path.join(DATA_DIR, 'sessions', group.folder, 'extra');
        ...
        const runnerToolProcessEnv = preparedExecution.providerId === 'deepagents:langchain'
            ? toolNetworkEnv
        ...
        const env = buildBaseRunnerEnv({
            ...
            browserIpcAuthToken: browserIpcEnabled
                ? computeBrowserIpcAuthToken(group.folder, input.chatJid, input.threadId)
                : undefined,
        ...
        const selectedSkillEnv = await hostStartup.measureAsync('selectedSkillEnvMs', () => resolveSelectedSkillEnvForSpawn({ options, effectiveRuntimeAccess }));
        Object.assign(env, pickSelectedCapabilityEnv(selectedSkillEnv.env));
        mcpConfigPath = hostStartup.measure('mcpConfigMs', () => allMcpCapabilities.length > 0
            ? writeRunnerMcpConfigFile(hostRuntime.workspaceIpcDir, withStdioMcpEgressEnv(allMcpCapabilities, toolNetworkEnv))
            : undefined);
        ...
        if (runnerVisibleMcpServerNames.length > 0) {
            env.GANTRY_SELECTED_MCP_SERVERS_JSON = JSON.stringify(runnerVisibleMcpServerNames);
        }
        ...
        const sandboxProtectedReadPaths = protectedFilesystemDenyReadPaths.filter((item) => !sandboxRunnerReadablePaths.includes(path.resolve(item)));
        const protectedFilesystemDenyWritePaths = [
            ...(preparedExecution.protectedFilesystemDenyWritePaths ??
                preparedExecution.protectedFilesystemPaths),
            ...localCliCredentialPaths,
        ];
        const sandboxProtectedWritePaths = protectedWritePathsForOuterSandbox(protectedFilesystemDenyWritePaths, providerConfigDir);
        writeProtectedFilesystemEnv({
            env,
        ...
        sandboxConfigPath = path.join(hostRuntime.workspaceIpcDir, `${processName}.sandbox-runtime.json`);
        const runnerPackageRoot = resolvePackageRootFromSourceDir(path.dirname(args[0] ?? hostRuntime.runnerDistDir));
        ...
        const finalAllowedNetworkHosts = configureSpawnAsyncCommandSandboxPolicy({
            ...
            protectedReadPaths: protectedFilesystemDenyReadPaths,
            protectedWritePaths: protectedFilesystemDenyWritePaths,
            gatewayAllowedNetworkHosts: sandboxRuntimeNetwork.networkProjection.allowedNetworkHosts,
        ...
        await publishRunnerHostStartupDiagnosticFromSpawn({
            ...
            snapshot: {
                ...
                runnerSandboxEnforcing: options?.runnerSandboxProvider?.enforcing === true,
        ...
        revokeIpcResponseSigningKey(ipcAuth.responseKeyId, group.folder, input.threadId);
    }
}
