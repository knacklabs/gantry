...
// prettier-ignore
import { acquireProfileLock, createProfile, getProfile, isValidBrowserProfileName, listProfiles, updateProfileMetadata } from './browser-profiles.js';
// prettier-ignore
...
import { browserProfileState, persistedBrowserStatus, runningBrowserStatus, stoppedBrowserStatus, } from './browser-status.js';
import { browserProcessProfileState, isPidAlive, isPidOwnedByBrowserProfile, isPidOwnedVisibleBrowserProfile, } from './browser-process.js';
import { browserProfileNeedsRestore, restoreBrowserProfileBeforeLaunch, } from './browser-profile-sync.js';
export const DEFAULT_BROWSER_PROFILE_NAME = 'gantry';
const sessions = new Map();
const pendingLaunches = new Map();
function cleanupChromeSingletonArtifacts(userDataDir) {
...
function resolveProfileName(profileName) {
    ...
    if (!isValidBrowserProfileName(normalized)) {
        throw new Error('Invalid browser profile name. Use lowercase letters, digits, dot, underscore, or hyphen.');
    }
    return normalized;
}
async function waitForCdpHttp(port, timeoutMs) {
    ...
    while (currentTimeMs() - startedAt < timeoutMs) {
        if (await isCdpHttpHealthy(port))
            return;
        await sleepWithinDeadline(startedAt, timeoutMs, 500);
    }
    throw new Error(`Chrome CDP did not become healthy on port ${port} within ${timeoutMs}ms`);
}
async function sleepWithinDeadline(startedAt, timeoutMs, maxSleepMs) {
    const elapsedMs = currentTimeMs() - startedAt;
    const remainingMs = Math.max(0, timeoutMs - elapsedMs);
    await new Promise((resolve) => setTimeout(resolve, Math.max(1, Math.min(maxSleepMs, remainingMs))));
}
function browserLaunchTimeoutMs(opts, maxTimeoutMs) {
...
function isChromeAlive(session) {
    return isPidAlive(session.pid);
}
async function isCdpHttpHealthy(port) {
...
async function closeUnhealthySession(profileName, session) {
    logger.warn({ profileName, pid: session.pid, port: session.port }, 'Closing unhealthy browser session');
    await closeBrowser(profileName).catch((err) => {
        logger.warn({ err, profileName, pid: session.pid, port: session.port }, 'Failed to close unhealthy browser session');
    });
}
async function closePersistedSessionBeforeRestore(profileName, profile) {
...
function touchSession(session) {
    const profile = getProfile(session.profileName) ?? createProfile(session.profileName);
    ...
    session.keepAliveTimer = setTimeout(() => {
        closeBrowser(session.profileName).catch((err) => {
            logger.warn({ err, profileName: session.profileName }, 'Failed to auto-close idle browser session');
        });
    }, session.keepAliveMs);
...
async function recoverPersistedBrowserSession(input) {
    ...
    if (!processState.owned) {
        logger.warn({ profileName: input.profileName, pid: record.pid }, 'Ignoring persisted browser session whose PID is not owned by the profile');
        clearBrowserSessionRecord(input.profile);
        updateProfileMetadata(input.profileName, { cdp_port: undefined });
        return null;
    }
    if (processState.headless || record.headless === true) {
        logger.warn({ profileName: input.profileName, pid: record.pid }, 'Terminating non-visible persisted browser session');
        await terminatePid(record.pid);
        clearBrowserSessionRecord(input.profile);
        updateProfileMetadata(input.profileName, { cdp_port: undefined });
        return null;
    ...
    if (!(await isCdpHttpHealthy(record.port))) {
        logger.warn({ profileName: input.profileName, pid: record.pid, port: record.port }, 'Terminating browser process with unhealthy persisted CDP session');
        await terminatePid(record.pid);
        clearBrowserSessionRecord(input.profile);
        updateProfileMetadata(input.profileName, { cdp_port: undefined });
        return null;
    ...
    logger.info({ profileName: input.profileName, pid: record.pid, port: record.port }, 'Adopted persisted browser profile session');
    return session;
}
function toStoppedStatus(profileName, error) {
...
function toRunningStatus(session) {
    const profile = getProfile(session.profileName) ?? createProfile(session.profileName);
...
export async function launchBrowser(opts = {}) {
    ...
    if (pending)
        return await waitForPendingLaunch(pending, opts);
...
async function launchBrowserInner(profileName, keepAliveMs, opts) {
    ...
    if (existing && (await isSessionHealthy(existing))) {
        if (await browserProfileNeedsRestore(profileName, profile.dir)) {
            logger.info({ profileName, pid: existing.pid, port: existing.port }, 'Closing browser session before restoring newer shared profile snapshot');
    ...
    let chromeProcess;
    try {
        const needsRestore = await browserProfileNeedsRestore(profileName, profile.dir);
        if (needsRestore) {
            ...
            if (recovered)
                return toRunningStatus(recovered);
        }
        ...
        const targetId = await ensureBrowserTarget(debuggingPort, typeof opts.deadlineAtMs === 'number'
        ...
        touchSession(session);
        logger.info({ profileName, port: debuggingPort }, 'Launched browser profile session');
...
async function waitForPendingLaunch(launch, opts) {
    const deadlineAtMs = opts.deadlineAtMs;
    if (typeof deadlineAtMs !== 'number' || !Number.isFinite(deadlineAtMs)) {
        return await launch;
    }
    ...
    return await new Promise((resolve, reject) => {
        const timer = setTimeout(() => reject(new Error('Browser launch deadline exceeded')), remainingMs);
        timer.unref?.();
        launch.then((status) => {
            clearTimeout(timer);
            resolve(status);
...
async function waitForProcessExit(session, timeoutMs) {
    if (!isChromeAlive(session))
        return true;
...
export async function getBrowserStatus(profileName = DEFAULT_BROWSER_PROFILE_NAME) {
...
export function getKnownBrowserStatus(profileName = DEFAULT_BROWSER_PROFILE_NAME) {
    const normalized = resolveProfileName(profileName);
    const session = sessions.get(normalized);
    if (!session || !isChromeAlive(session))
        return toStoppedStatus(normalized);
    return toRunningStatus(session);
}
export async function closeBrowser(profileName = DEFAULT_BROWSER_PROFILE_NAME) {
    const startedAt = currentTimeMs();
    const normalized = resolveProfileName(profileName);
    const session = sessions.get(normalized);
    if (!session) {
        ...
        try {
            ...
            const shouldTerminate = isPidAlive(record.pid) &&
                isPidOwnedByBrowserProfile(record.pid, profile);
            if (shouldTerminate) {
                await terminatePid(record.pid);
            }
            else {
                logger.warn({ profileName: normalized, pid: record.pid }, 'Clearing persisted browser session without terminating unverified PID');
            }
            clearBrowserSessionRecord(profile);
...
export async function listBrowserProfiles() {
    ...
    for (const profile of profiles) {
        ...
        const persistedState = persisted
            ...
            : undefined;
        const persistedRunning = Boolean(persisted &&
        ...
        const headless = session?.headless ?? persistedState?.headless ?? persisted?.headless;
        statuses.push({
            name: profile.name,
