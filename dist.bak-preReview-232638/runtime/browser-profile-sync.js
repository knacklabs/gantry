 ...
 */
const profileActivity = new Set();
export function markBrowserProfileActivity(profileName) {
    profileActivity.add(profileName);
}
/** Read-and-clear the activity flag for a profile. */
export function consumeBrowserProfileActivity(profileName) {
    return profileActivity.delete(profileName);
}
export function registerBrowserProfileSync(deps) {
    coordinator = deps;
}
export function isBrowserProfileSyncEnabled() {
    return coordinator !== null;
}
export async function browserProfileNeedsRestore(profileName, profileDir) {
    if (!coordinator)
        return false;
    const snapshot = await coordinator.repository.getBrowserProfileSnapshot(profileName);
    if (!snapshot)
        return false;
...
function readSnapshotMarker(profileDir) {
    try {
        const parsed = JSON.parse(fs.readFileSync(markerPath(profileDir), 'utf-8'));
...
function writeSnapshotMarker(profileDir, contentHash) {
 ...
 * snapshot).
 */
export async function collectUserDataFiles(userDataDir) {
    ...
    async function visit(directory) {
        ...
        for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
            ...
            const relative = path
                ...
                .split(path.sep)
                .join('/');
            if (isExcludedBrowserProfilePath(relative))
                continue;
 ...
 */
export async function snapshotBrowserProfile(input) {
    if (!coordinator)
        return { status: 'noop', reason: 'sync_disabled' };
    let lock;
    try {
        lock = await acquireProfileLock(input.profileName, SNAPSHOT_LOCK_TIMEOUT_MS);
    }
    catch {
        // Lock held by a live Chrome relaunch (or contended). Skip rather than
        // block finalize or snapshot a non-quiescent tree.
        logger.info({ profileName: input.profileName }, 'Skipped browser profile snapshot: profile lock held (browser relaunched mid-finalize)');
    ...
    try {
        ...
        const existing = await coordinator.repository.getBrowserProfileSnapshot(input.profileName);
 ...
 * restores.
 */
export async function restoreBrowserProfileBeforeLaunch(profileName, profile) {
    if (!coordinator)
        return;
    try {
        ...
        if (result.status === 'integrity_error') {
            logger.error({ profileName, quarantinePath: result.quarantinePath }, 'Browser profile snapshot failed integrity validation; quarantined the corrupt object and launching from local profile to preserve availability');
        }
    }
    catch (err) {
        logger.error({ err, profileName }, 'Failed to restore browser profile snapshot before launch');
        throw err;
    }
}
/**
 ...
 * check returns null).
 */
export async function restoreBrowserProfile(input) {
    if (!coordinator)
        return { status: 'noop', reason: 'sync_disabled' };
    const snapshot = await coordinator.repository.getBrowserProfileSnapshot(input.profileName);
    if (!snapshot)
        return { status: 'noop', reason: 'no_snapshot' };
    ...
    try {
        ...
        logger.info({ profileName: input.profileName, contentHash: materialized.contentHash }, 'Restored browser profile snapshot before launch');
