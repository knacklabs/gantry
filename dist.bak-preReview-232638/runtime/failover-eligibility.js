...
function matchesAny(error, patterns) {
    return patterns.some((pattern) => pattern.test(error));
}
export function isMissingProviderSessionError(error) {
...
export function isFailoverEligibleError(error) {
    ...
    if (!text)
        return false;
    if (matchesAny(text, NON_ELIGIBLE_PATTERNS))
        return false;
