import { resolveTurnPromptCapabilityCatalog, resolveTurnSemanticCapabilities, resolveTurnSelectedMcpServerIds, resolveTurnSelectedSkillContext, resolveTurnToolPolicy, } from './group-run-context.js';
...
export async function resolveGroupAgentAccessContext(input) {
    ...
    const [configuredToolPolicy, selectedSkillContext, semanticCapabilities] = await Promise.all([
    ...
    const attachedMcpSourceIds = await resolveTurnSelectedMcpServerIds(input.deps, input.turnContext);
    const capabilityCatalog = await resolveTurnPromptCapabilityCatalog(input.deps, input.catalogScope, configuredToolPolicy.semanticCapabilities);
    const lockStatus = input.deps.getAgentLockStatus?.(input.agentFolder);
    const accessPreset = resolveSpawnPromptAccessPreset(lockStatus === 'locked' || lockStatus === 'unknown' ? 'locked' : 'full', process.env.GANTRY_NO_PERMISSION_TOOLS === '1');
