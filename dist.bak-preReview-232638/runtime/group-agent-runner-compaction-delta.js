...
export async function prepareCompactionDeltaReplay(input) {
    ...
    if (!input.turnContext?.latestProviderSessionReady ||
    ...
    const getDeltaMessages = input.repository.getContextMessagesSince ??
        input.repository.getMessagesSince;
    const messages = !tooStale && baseCursor
        ? await getDeltaMessages.call(input.repository, input.chatJid, baseCursor, COMPACTION_DELTA_REPLAY_MAX_MESSAGES + 1, {
