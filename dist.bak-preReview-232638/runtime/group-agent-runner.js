...
import { resolveSingleNonSelfSenderId, buildApprovedSkillContextBlock, buildRuntimeRunOptions, completeFailedRuntimeSessionRun, completeSuccessfulRuntimeSessionRun, createRuntimeResultSummaryAccumulator, summarizeRuntimeResultForPersistence, } from './session-resume-runtime.js';
...
import { loadPatternsContext, markPatternsContextSurfaced, } from '../shared/pattern-candidate-block.js';
import { patternSubjectForScope } from '../shared/pattern-candidate-subject.js';
import { memoryAgentIdForWorkspaceFolder } from '../memory/app-memory-boundaries.js';
import { runFamilyFailoverLoop, publishRunFailoverEvent, } from './failover-candidate-loop.js';
import { outcomeForPatternCandidateStatus } from './proactive-surfacing-metrics.js';
import { proactiveSurfacingAllowed, publishProactiveSurfacingOutcomeEvent, } from './proactive-surfacing-gate.js';
...
import { logger, redactString, updateLogContext, withLogContext, } from '../infrastructure/logging/logger.js';
...
function redactRuntimeError(error) {
    return error ? redactString(error) : undefined;
}
function isStoppedByRequest(output) {
...
export function createGroupAgentRunner(input) {
    ...
    async function runAgentWithContext(group, prompt, chatJid, queueJid, onOutput, options) {
        const agentHarness = deps.getSelectedAgentHarness(group.folder);
        const turnAppId = appIdFromConversationJid(chatJid) ?? 'default';
        const defaultInteractiveModel = deps.getDefaultInteractiveModel?.(group.folder) ?? 'opus';
        ...
        const maintenanceCompactionPrompt = options?.maintenanceCompaction
            ? maintenanceCompactionPromptForExecutionProvider(executionProviderId, deps)
            : undefined;
        ...
        const loadTurnContext = async (promoteReadyProviderSession) => ops().getAgentTurnContext?.({
            appId: turnAppId,
            ...
            query: options?.memoryContext?.source === 'message'
        ...
        const defaultRuntimeModel = group.agentConfig?.model ?? defaultInteractiveModel;
        const forwardedRuntimeEventKeys = new Set();
        const defaultMemoryScope = memoryScopeForConversationKind(group.conversationKind);
        const memoryReviewerUserId = resolveSingleNonSelfSenderId(options?.turnMessages ?? []);
        const memoryReviewerIsControlApprover = await memoryReviewerApproverAllowed(deps, chatJid, group.folder, memoryReviewerUserId);
        const runState = {};
        const liveRunFenced = !!options?.existingRunLeaseToken;
        let latestProviderSessionId = turnContext?.externalSessionId?.trim() || undefined;
        let resumeProviderSessionId = options?.maintenanceProviderSession?.providerSessionId ??
            turnContext?.providerSessionId;
        let resumeExternalSessionId = options?.maintenanceProviderSession?.externalSessionId ??
        ...
        const updateRunProviderMetadata = async (input) => {
            ...
            await repository
                ...
                .catch((err) => {
                logger.warn({ err, group: group.name, runId: runState.runId }, 'Failed to update runtime run provider metadata');
            });
        };
        const persistProviderSessionFromOutput = async (output) => {
            if (output.status === 'error')
                return;
            if (turnContext?.latestProviderSessionLocked ||
                options?.maintenanceProviderSession)
                return;
            const nextSessionId = (output.providerSession?.externalSessionId ?? output.newSessionId)?.trim();
            if (!nextSessionId ||
            ...
            const persisted = await ops().setSession(group.folder, nextSessionId, sessionThreadId, {
                appId: runtimeAppId,
                executionProviderId,
                conversationJid: chatJid,
                providerAccountId: group.providerAccountId,
            ...
            if (persisted === false) {
                logger.warn({ group: group.name }, 'Provider session update skipped because turn ownership changed');
                return;
            }
            latestProviderSessionId = nextSessionId;
            await updateRunProviderMetadata({ providerSessionId: nextSessionId });
        };
        const wrappedOnOutput = async (output) => {
            ...
            if (output.usage) {
                try {
                    ...
                    logger.warn({ err, group: group.name }, 'Failed to prepare normalized model usage runtime event');
                }
            }
            ...
            if (normalizedUsageRuntimeEvent) {
                try {
                    ...
                    logger.warn({ err, group: group.name }, 'Failed to publish normalized model usage runtime event');
                }
            }
            if (output.compactBoundary &&
                turnContext?.agentSessionId &&
        ...
        const { configuredToolPolicy, selectedSkillContext, semanticCapabilities, attachedMcpSourceIds, capabilityCatalog, currentAccessFingerprint, } = await resolveGroupAgentAccessContext({
            deps,
            turnContext,
            catalogScope: {
                appId: turnContext?.appId ?? runtimeAppId,
                agentId: turnContext?.agentId ?? memoryAgentIdForWorkspaceFolder(group.folder),
            },
            agentFolder: group.folder,
        });
        if (turnContext?.providerSessionId &&
            turnContext.externalSessionId &&
            !providerSessionAccessFingerprintMatches(turnContext.providerSessionAccessFingerprint, currentAccessFingerprint)) {
        ...
        let patternsContext = { block: '', surfacedCandidateIds: [] };
        if (surfacingGate.allowed) {
            try {
                patternsContext = await loadPatternsContext(patternCandidateRepo, surfacingScope);
            }
            catch {
        ...
        runState.runId = options?.existingRunId
            ...
            : turnContext?.agentSessionId
                ? await ops().createSessionAgentRun?.({
                    ...
                    cause: options?.memoryContext?.source === 'command'
                        ? 'control'
                        : 'message',
                })
                : undefined;
        ...
        try {
            ...
            const expireTurnProviderSession = async (reason) => {
                if (!turnContext?.providerSessionId ||
                    !turnContext.agentSessionId ||
                    !turnContext.externalSessionId ||
                ...
                logger.warn({ group: group.name, reason: redactString(reason) }, 'Expired stale provider session and retrying without resume');
                return true;
            };
            const invokeAgent = (agentInput) => runAgentImpl(group, {
                ...
                ...(options?.existingRunId &&
                    ...
                    ? {
                        ...
                        runLeaseFencingVersion: options.existingRunLeaseFencingVersion,
                    }
                    : {}),
                ...
                const registerOptions = memoryReviewerIsControlApprover && memoryReviewerUserId
                ...
                deps.queue.registerProcess(queueJid, proc, runHandle, group.folder, stopAliasJids, options?.memoryContext?.threadId, registerOptions);
            }, wrappedOnOutput, { ...runOptions, correlationRunId: runState.runId }).then((output) => runTokenBudget.enforce(output));
            ...
            const adapterMissingProviderSession = activeExecutionAdapter?.isMissingProviderSessionError?.(output.error) === true;
            const missingProviderSession = adapterMissingProviderSession ||
            ...
            output = await runFamilyFailoverLoop({
                ...
                log: (message) => logger.warn({ group: group.name }, redactString(message)),
            });
            await forwardRuntimeEvents({
                output,
                publishRuntimeEvent: deps.publishRuntimeEvent,
            ...
            if (output.status === 'error') {
                ...
                logger.error({ group: group.name, error: redactRuntimeError(output.error) }, 'Agent runner error');
                if (!liveRunFenced) {
                    await completeFailedRuntimeSessionRun({
                        ops: ops(),
                        runId: runState.runId,
            ...
            await markPatternsContextSurfaced(patternCandidateRepo, patternsContext.surfacedCandidateIds);
            try {
                ...
                if (surfacedSubject && patternCandidateRepo) {
                    for (const id of patternsContext.surfacedCandidateIds) {
                        ...
                        publishProactiveSurfacingOutcomeEvent({
                            ...
                            outcome: outcomeForPatternCandidateStatus(candidate.candidateStatus),
                        });
                    }
                }
            }
