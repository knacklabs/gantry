...
import { isSenderAllowed, loadSenderAllowlist, shouldDropMessage, } from '../platform/sender-allowlist.js';
import { buildConversationContextPacket, CONVERSATION_CONTEXT_LIMITS, } from './conversation-context.js';
...
export async function buildGroupTurnConversationContext(input) {
    ...
    const droppedHydratedMessages = rawHydratedMessages.length - hydratedMessages.length;
    ...
    return {
        prompt: formatConversationContextMessages(conversationContext, input.timezone),
...
function shouldHydrateConversationContext(context) {
    if (context.metadata.activeThreadId) {
        return (!context.metadata.activeThreadWindowComplete ||
...
function filterHydratedMessagesForPersistence(input) {
    ...
    return input.messages.filter((message) => {
        ...
        return isSenderAllowed(input.chatJid, message.sender, allowlistCfg, input.agentFolder);
    });
}
function hydrationErrorDiagnostics(err) {
