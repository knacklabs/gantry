...
import { createRuntimeResultSummaryAccumulator, createRuntimeUserVisibleResultAccumulator, createRuntimeUserVisibleStreamSanitizer, } from './session-resume-runtime.js';
export function createGroupOutputBuffer(input) {
    ...
    const flushBufferedOutput = async (reason, options = {}) => {
        if (!pendingOutputHasParts)
            return false;
        ...
        input.log.info({ group: input.groupName }, `Agent output: ${rawChars} chars`);
        if (!text)
            return false;
        if (input.supportsStreamingChunks) {
            const settlement = await settleDeliveryAttempt(() => input.channelRuntime.sendStreamingChunk(input.chatJid, finalStreamDelta, input.buildStreamingOptions({ done })), { scope: 'runtime-streaming-output-final', target: input.chatJid }).catch((err) => {
                input.log.warn({ err, group: input.groupName, reason }, 'Failed to send finalized streaming output');
            ...
            const settlement = await settleDeliveryAttempt(() => input.sendMessageToChannel(text, messageOptions), { scope: 'runtime-output-message-final', target: input.chatJid });
    ...
    return {
        appendRawOutput: async (raw) => {
            ...
            if (!safeDelta)
                return;
            const settlement = await settleDeliveryAttempt(() => input.channelRuntime.sendStreamingChunk(input.chatJid, safeDelta, input.buildStreamingOptions({ done: false })), { scope: 'runtime-streaming-output-live', target: input.chatJid });
