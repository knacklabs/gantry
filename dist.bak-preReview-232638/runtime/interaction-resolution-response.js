import { getIpcResponseSigningPrivateKey, unsealIpcResponseSigningPrivateKey, } from './ipc-auth.js';
...
export function writeResolvedInteractionResponse(payload) {
    ...
    if (!kind || !requestId || !sourceAgentFolder || !ipcBaseDir)
        return false;
    if (!responseKeyId)
        return false;
    const privateKey = getIpcResponseSigningPrivateKey(sourceAgentFolder, threadId, responseKeyId) ??
        unsealIpcResponseSigningPrivateKey(stringValue(callbackRoute?.responsePrivateKeySeal));
    if (!privateKey)
        return false;
    if (kind === 'permission') {
        const rawMode = stringValue(resolution?.mode);
        const mode = rawMode === 'allow_once' ||
            rawMode === 'allow_persistent_rule' ||
            rawMode === 'cancel'
            ? rawMode
            : 'cancel';
        const approved = status !== 'cancelled' &&
            mode !== 'cancel' &&
            resolution?.approved === true;
        writePermissionIpcResponse(ipcBaseDir, sourceAgentFolder, {
            requestId,
            ...
            ...(stringValue(resolution?.decisionClassification)
                ? {
                    decisionClassification: stringValue(resolution?.decisionClassification),
                }
                : {}),
        }, privateKey);
        return true;
    ...
    if (kind === 'question') {
        writeUserQuestionIpcResponse(ipcBaseDir, sourceAgentFolder, {
            requestId,
