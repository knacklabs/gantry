...
import { IPC_REQUEST_MAX_AGE_MS, validateIpcRequestFreshness, verifyIpcRequestPayload, } from '../infrastructure/ipc/request-signing.js';
...
import { computeBrowserIpcAuthToken, computeIpcAuthToken, computeMemoryIpcAuthToken, } from './ipc-auth.js';
const consumedIpcRequestIds = new Map();
function replayStoreDir() {
    return path.join(DATA_DIR, 'ipc-replay');
}
function replayMarkerPath(key) {
    const digest = createHash('sha256').update(key).digest('hex');
    return path.join(replayStoreDir(), `${digest}.json`);
}
function readThreadIdField(value, label) {
...
function readPayloadThreadIdField(value, label) {
    if (value === null)
        return null;
    return readThreadIdField(value, label);
}
function readResponseKeyIdField(value, label) {
...
function readProviderAccountIdField(value, label) {
...
function readAppIdField(value, label) {
    const parsed = toTrimmedString(value, { maxLen: 128 });
    if (parsed === undefined) {
        throw new Error(`${label} must be a string up to 128 characters`);
    }
...
function readAgentIdField(value, label) {
...
function readTrustedThreadBinding(raw, label) {
    const context = isPlainObject(raw.context) ? raw.context : undefined;
    const hasContextThreadId = !!context && Object.prototype.hasOwnProperty.call(context, 'threadId');
    const hasPayloadThreadId = Object.prototype.hasOwnProperty.call(raw, 'threadId');
    ...
    const trustedThreadId = hasContextThreadId
        ? contextThreadId
        : payloadThreadId;
    const responseKeyId = context && Object.prototype.hasOwnProperty.call(context, 'responseKeyId')
        ? readResponseKeyIdField(context.responseKeyId, `${label} responseKeyId`)
        : undefined;
    const contextAppId = context && Object.prototype.hasOwnProperty.call(context, 'appId')
    ...
    const contextAgentId = context && Object.prototype.hasOwnProperty.call(context, 'agentId')
    ...
    const contextProviderAccountId = context &&
        Object.prototype.hasOwnProperty.call(context, 'providerAccountId')
        ? readProviderAccountIdField(context.providerAccountId, `${label} context.providerAccountId`)
        : undefined;
    const payloadProviderAccountId = Object.prototype.hasOwnProperty.call(raw, 'providerAccountId')
        ? readProviderAccountIdField(raw.providerAccountId, `${label} providerAccountId`)
        : undefined;
    if (contextProviderAccountId &&
    ...
    return {
        ...
        authThreadId: typeof trustedThreadId === 'string' && trustedThreadId
            ? trustedThreadId
            : undefined,
...
let consumedIpcRequestIdSweep;
let consumedIpcRequestIdSweepInProgress = false;
let replayMarkerSweepDir;
let replayMarkerSweepGeneration = 0;
let replayMarkerSweepInProgress = false;
let replayMarkerSweepTimer;
function replayMarkerSweepWithinBudget(startedAt) {
    const elapsedMs = Number(process.hrtime.bigint() - startedAt) / 1_000_000;
    return elapsedMs < REPLAY_MARKER_SWEEP_TIME_BUDGET_MS;
}
function yieldReplayMarkerSweep() {
    return new Promise((resolve) => setImmediate(resolve));
}
async function pruneConsumedIpcRequestIds() {
    if (consumedIpcRequestIdSweepInProgress)
        return;
    ...
    try {
        while (generation === replayMarkerSweepGeneration) {
            ...
            for (let inspected = 0; inspected < REPLAY_MARKER_SWEEP_BATCH_SIZE; inspected += 1) {
            ...
            if (!foundExpired || !replayMarkerSweepWithinBudget(startedAt))
                return;
...
async function closeReplayMarkerSweepDir(dir) {
    if (replayMarkerSweepDir === dir)
...
async function readReplayMarkerExpiry(markerPath) {
    try {
        const parsed = JSON.parse(await fs.promises.readFile(markerPath, 'utf-8'));
...
async function reclaimMalformedReplayMarker(markerPath, graceExpiredBefore) {
    try {
        const markerStat = await fs.promises.stat(markerPath);
        if (markerStat.mtimeMs > graceExpiredBefore)
            return false;
        const confirmedStat = await fs.promises.stat(markerPath);
        if (confirmedStat.mtimeMs > graceExpiredBefore)
            return false;
...
async function reclaimGraceExpiredReplayMarker(markerPath, graceExpiredBefore) {
    ...
    if (expiresAtMs > graceExpiredBefore)
        return false;
    const confirmedExpiresAtMs = await readReplayMarkerExpiry(markerPath);
    if (confirmedExpiresAtMs === undefined ||
...
async function pruneReplayMarkersOnDisk() {
    if (replayMarkerSweepInProgress)
        return;
    ...
    try {
        ...
        while (generation === replayMarkerSweepGeneration) {
            ...
            const graceExpired = await Promise.allSettled(markerPaths.map((markerPath) => reclaimGraceExpiredReplayMarker(markerPath, graceExpiredBefore)));
            if (generation !== replayMarkerSweepGeneration) {
                await closeReplayMarkerSweepDir(dir);
                return;
            }
            ...
            if (!graceExpired.some((result) => result.status === 'fulfilled' && result.value) ||
...
function ensureReplayMarkerSweepTimer() {
...
function stopReplayMarkerSweepTimer() {
...
function reserveIpcReplayMarker(key, expiresAtMs) {
    const now = nowMs();
...
export function clearConsumedIpcRequestIds(input) {
    ...
    if (input?.durable !== true && input?.durable !== 'consumed')
        return;
...
function reserveFreshIpcRequestId(replayKey, expiresAtMs, label) {
    ensureReplayMarkerSweepTimer();
    if (!reserveIpcReplayMarker(replayKey, expiresAtMs)) {
        throw new Error(`Invalid ${label} replay`);
    }
}
export function validateIpcAuthRequest(raw, sourceAgentFolder, label, options) {
    ...
    const freshness = validateIpcRequestFreshness(payload, validationNowMs, maxAgeMs);
    if (!freshness.ok) {
...
export function validateInteractionIpcAuthRequest(raw, sourceAgentFolder, label) {
    ...
    return validateIpcAuthRequest(raw, sourceAgentFolder, label, ipcInteractionAuthValidationOptions(raw.permissionLane));
}
export function validateBrowserIpcAuthRequest(raw, sourceAgentFolder, label) {
    const binding = readTrustedThreadBinding(raw, label);
    const context = isPlainObject(raw.context) ? raw.context : undefined;
    ...
    delete payload.signature;
    delete payload.authToken;
    const requestSigningKey = computeBrowserIpcAuthToken(sourceAgentFolder, chatJid, binding.authThreadId);
    if (!verifyIpcRequestPayload(requestSigningKey, payload, signature)) {
        throw new Error(`Invalid ${label} signature`);
    }
    ...
    if (requestId) {
        ...
        reserveFreshIpcRequestId(replayKey, nowMs() + IPC_REQUEST_MAX_AGE_MS, label);
    }
    return { ...binding, chatJid };
}
export function validateMemoryIpcAuthRequest(raw, sourceAgentFolder, label) {
    const binding = readTrustedThreadBinding(raw, label);
    const context = isPlainObject(raw.context) ? raw.context : undefined;
    const chatJid = toTrimmedString(context?.chatJid, { maxLen: 255 });
    ...
    const defaultScope = defaultScopeRaw === 'user' || defaultScopeRaw === 'group'
        ? defaultScopeRaw
        : undefined;
    const allowedActions = normalizeMemoryIpcActions(Array.isArray(context?.allowedActions)
        ? context.allowedActions.filter((action) => typeof action === 'string')
        : undefined);
