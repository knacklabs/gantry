...
import { resolveRuntimeSecurityPosture, } from '../shared/security-posture.js';
import { normalizeMemoryIpcActions } from '../shared/memory-ipc-actions.js';
function resolveIpcAuthSecurityEnv() {
    return {
...
const IPC_AUTH_SECRET = GANTRY_IPC_AUTH_SECRET ||
    (() => {
        if (resolveRuntimeSecurityPosture(resolveIpcAuthSecurityEnv())
            .requiresProductionSecrets) {
            throw new Error('GANTRY_IPC_AUTH_SECRET is required in production or remote control mode.');
        }
        const generated = randomBytes(32).toString('hex');
        console.warn('GANTRY_IPC_AUTH_SECRET not set; using ephemeral secret (IPC tokens will not survive restarts)');
        return generated;
    })();
function authScope(workspaceKey, threadId, scope) {
    ...
    if (!normalizedAppId && !normalizedAgentId)
        return threadScope;
...
const browserIpcAuthorizations = new Map();
const RESPONSE_PRIVATE_KEY_SEAL_AAD = Buffer.from('gantry:ipc-response-private-key:v1');
function responsePrivateKeySealKey() {
    return createHash('sha256').update(IPC_AUTH_SECRET).digest();
}
export function responseSigningKeyId(publicKeyPem) {
    return createHash('sha256').update(publicKeyPem).digest('base64url');
}
export function computeIpcAuthToken(workspaceKey, threadId, scope) {
    return createHmac('sha256', IPC_AUTH_SECRET)
        .update(authScope(workspaceKey, threadId, scope))
        .digest('hex');
}
export function computeBrowserIpcAuthToken(workspaceKey, chatJid, threadId) {
    return createHmac('sha256', IPC_AUTH_SECRET)
...
export function registerBrowserIpcAuthorization(input) {
    ...
    browserIpcAuthorizations.set(key, (browserIpcAuthorizations.get(key) ?? 0) + 1);
}
...
export function isBrowserIpcAuthorized(input) {
    return ((browserIpcAuthorizations.get(browserIpcAuthorizationKey(input)) ?? 0) > 0);
}
export function computeMemoryIpcAuthToken(workspaceKey, input) {
    const normalizedChatJid = input.chatJid?.trim() || '';
    const normalizedUserId = input.userId?.trim() || '';
    const normalizedDefaultScope = input.defaultScope || 'group';
    const normalizedAllowedActions = normalizeMemoryIpcActions(input.allowedActions ?? undefined).join(',');
    const reviewerScope = input.reviewerIsControlApprover ? 'approver' : 'user';
    return createHmac('sha256', IPC_AUTH_SECRET)
        .update(`memory\0${authScope(workspaceKey, input.threadId)}\0chat\0${normalizedChatJid}\0user\0${normalizedUserId}\0scope\0${normalizedDefaultScope}\0actions\0${normalizedAllowedActions}\0reviewer\0${reviewerScope}`)
        .digest('hex');
}
export function validateIpcAuthToken(workspaceKey, candidateToken, threadId, scope) {
    if (!candidateToken)
        return false;
...
export function createIpcAuthEnvelope(workspaceKey, threadId, scope) {
...
export function getIpcResponseSigningPrivateKey(workspaceKey, threadId, responseKeyId) {
    ...
    if (!keys)
        return undefined;
    if (keys.workspaceKey !== workspaceKey)
        return undefined;
    if (keys.threadId !== normalizedThreadId(threadId))
        return undefined;
    return keys.privateKeyPem;
}
export function sealIpcResponseSigningPrivateKey(privateKeyPem) {
    if (!privateKeyPem)
        return undefined;
...
export function unsealIpcResponseSigningPrivateKey(sealed) {
    if (!sealed)
        return undefined;
    ...
    try {
        const decipher = createDecipheriv('aes-256-gcm', responsePrivateKeySealKey(), Buffer.from(ivB64, 'base64url'));
...
export function revokeIpcResponseSigningKey(responseKeyId, workspaceKey, threadId) {
    const keyId = responseKeyId?.trim();
    if (!keyId)
        return false;
    const keys = responseSigningKeys.get(keyId);
