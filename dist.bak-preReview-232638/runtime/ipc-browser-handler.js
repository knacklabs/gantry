...
import { BROWSER_BACKEND_ACTIONS, } from '../shared/browser-backend-actions.js';
...
import { DEFAULT_BROWSER_PROFILE_NAME, closeBrowser, ensureBrowserReady, getBrowserStatus, } from './browser-capability.js';
import { beginBrowserUsage, finishBrowserUsage, rememberBrowserUsageSite, } from './browser-usage-governor.js';
import { ensureBrowserTarget, foregroundBrowserTarget, } from './browser-cdp-targets.js';
import { browserUsagePayloadUrl, resolveActiveBrowserUrlForUsage, } from './browser-usage-active-site.js';
...
const BROWSER_IPC_UNAUTHORIZED_ERROR = 'Browser IPC is not authorized for this run. Select the canonical Browser capability before using browser actions.';
...
const FOREGROUND_BEFORE_DISPATCH_ACTIONS = new Set([
    ...POINTER_ACTIONS,
    'screenshot',
]);
...
function assertPayloadKeys(action, payload, allowedKeys) {
    const allowed = new Set(allowedKeys);
    const unknown = Object.keys(payload).filter((key) => !allowed.has(key));
    if (unknown.length > 0) {
        throw new Error(`${action} does not support payload field(s): ${unknown.sort().join(', ')}`);
    }
}
function getProfileNameFromPayload(payload, context) {
    void payload;
    return context.browserProfileName || DEFAULT_BROWSER_PROFILE_NAME;
}
function sanitizeBrowserStatus(value) {
...
function browserResizeDimensions(payload) {
...
function browserToolResultText(result) {
...
function createBrowserIpcDeadline(timeoutMs, deadlineAtMs) {
...
function browserCdpOptions(deadline) {
    if (deadline.deadlineAtMs === undefined)
        return undefined;
    browserIpcRemainingMs(deadline);
    return { deadlineAtMs: deadline.deadlineAtMs };
}
function browserBackendTimeoutMs(deadline) {
...
async function callBrowserResizeBackend(input) {
    if (!input.context.callBrowserTool) {
        throw new Error('Browser action backend is unavailable for viewport resize.');
    }
    ...
    const result = await input.context.callBrowserTool({
        ...
        fileAccessRoot: path.join(DATA_DIR, 'sessions', input.context.sourceAgentFolder, 'extra'),
        timeoutMs: backendTimeoutMs,
    });
    if (browserToolResultIsError(result)) {
        throw new Error(browserToolResultText(result) || 'Browser viewport resize failed.');
    }
}
async function getBrowserUsageSettings(context) {
    return context.getBrowserUsageSettings
        ? await context.getBrowserUsageSettings()
        : undefined;
}
async function handleBrowserToolActionInner(request, context, profileName, markProfileTouched) {
    const deadline = createBrowserIpcDeadline(context.timeoutMs, context.deadlineAtMs);
    ...
    const fileAccessRoot = path.join(DATA_DIR, 'sessions', context.sourceAgentFolder, 'extra');
...
async function handleBrowserToolAction(request, context) {
    ...
    try {
        if (!usageDecision.allowed) {
            response = {
                ok: false,
                error: usageDecision.warning || 'Browser usage policy denied this action.',
            };
            return response;
        }
        response = await handleBrowserToolActionInner(request, context, profileName, () => {
            profileTouched = true;
        });
...
async function publishBrowserJobActivity(input) {
    if (!input.request.jobId || !input.request.runId || !input.publish)
        return;
    try {
        await input.publish({
...
export async function processBrowserIpcRequest(request, context) {
    try {
...
export function writeBrowserIpcResponse(ipcBaseDir, sourceAgentFolder, response, privateKeyPem) {
    const responseDir = path.join(ipcBaseDir, sourceAgentFolder, 'browser-responses');
