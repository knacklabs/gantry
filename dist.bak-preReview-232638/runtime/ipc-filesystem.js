...
const IPC_ERROR_ARCHIVE_NAME_PATTERN = /^(\d+)-[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}-.+$/i;
let lastIpcErrorArchiveSweepAt = 0;
let ipcErrorArchivesSinceLastCompletedSweep = 0;
export function isTrustedDirectory(dirPath) {
    try {
...
export function ensureWorkspaceIpcLayout(ipcBaseDir, workspaceFolder) {
...
export function hasCompleteTrustedWorkspaceIpcLayout(ipcBaseDir, workspaceFolder) {
    const workspaceDir = path.join(ipcBaseDir, workspaceFolder);
    if (!isTrustedDirectory(workspaceDir))
        return false;
    for (const subdir of IPC_WORKSPACE_SUBDIRS) {
        if (!isTrustedDirectory(path.join(workspaceDir, subdir)))
            return false;
    }
    return true;
}
export function claimIpcFile(filePath) {
    const stat = fs.lstatSync(filePath);
    if (!stat.isFile() || stat.isSymbolicLink()) {
        throw new Error('IPC payload must be a regular file');
    }
    const claimed = path.join(path.dirname(filePath), `.processing-${process.pid}-${nowMs()}-${randomUUID()}-${path.basename(filePath)}`);
    fs.renameSync(filePath, claimed);
    return claimed;
}
export function isPendingIpcJsonFile(filename) {
    return filename.endsWith('.json') && !filename.startsWith('.processing-');
}
export function archiveIpcErrorFile(ipcBaseDir, sourceAgentFolder, filename, claimedPath, lane = path.basename(path.dirname(claimedPath))) {
    ...
    function archiveFilename() {
        ...
        const tail = Buffer.from(`${sourceAgentFolder}-${lane}-${path.basename(filename)}`);
        let tailEnd = Math.min(tail.length, IPC_ERROR_ARCHIVE_NAME_MAX_BYTES - Buffer.byteLength(prefix));
        while (tailEnd > 0 && (tail[tailEnd] & 0xc0) === 0x80)
            tailEnd -= 1;
        return `${prefix}${tail.subarray(0, tailEnd).toString('utf8')}`;
    }
}
function pruneExpiredIpcErrorArchives(errorDir) {
    const sweptAt = nowMs();
    if (ipcErrorArchivesSinceLastCompletedSweep < IPC_ERROR_ARCHIVE_MAX_ENTRIES &&
    ...
    retainedArchives.sort((left, right) => left.archivedAt - right.archivedAt || left.name.localeCompare(right.name));
    ...
    ipcErrorArchivesSinceLastCompletedSweep = Math.min(retainedArchives.length, IPC_ERROR_ARCHIVE_MAX_ENTRIES);
    function removeArchive(name) {
        try {
...
export function readIpcRootLockDetails(lockPath) {
    try {
        ...
        const pid = typeof pidRaw === 'number' && Number.isInteger(pidRaw) && pidRaw > 0
            ? pidRaw
            : undefined;
...
function isProcessAlive(pid) {
    try {
        ...
        if (code === 'EPERM')
            return true;
        logger.warn({ err, pid }, 'Unable to validate IPC lock PID liveness, assuming process is active');
        return true;
    }
}
export function recoverStaleIpcRootLock(lockPath) {
...
export function acquireIpcRootLock(ipcBaseDir) {
    const lockPath = path.join(ipcBaseDir, '.lock');
    writePrivateFileSync(lockPath, JSON.stringify({
        pid: process.pid,
        startedAt: nowIso(),
    }), { flag: 'wx' });
    return lockPath;
