...
import { permissionUpdateAllowedToolRules, persistentPermissionUpdates, } from '../shared/permission-tool-rules.js';
...
import { getIpcResponseSigningPrivateKey, sealIpcResponseSigningPrivateKey, } from './ipc-auth.js';
import { applyPermissionInteractionDecision, DurableInteractionPersistenceError, isActiveRunLeaseForInteraction, releasePermissionInteractionCallback, replayPersistedPermissionDecisionForRequest, resolvePendingInteractionRecord, } from '../application/interactions/pending-interaction-durability.js';
import { beginDurablePermissionInteraction, beginDurableQuestionInteraction, durablePermissionRequestSnapshot, finishDurableQuestionInteraction, resolveDurablePermissionInteractionOutcome, } from '../application/interactions/durable-interaction-handler.js';
import { resolveAgentLockStatus, } from '../config/profiles.js';
import { memoryAgentIdForWorkspaceFolder } from '../memory/app-memory-boundaries.js';
import { processUserQuestionIpcRequest, writePermissionIpcResponse, writeUserQuestionIpcResponse, } from './ipc-interaction-handler.js';
import { writePermissionInteractionFailure, writeUserQuestionInteractionFailure, } from './ipc-interaction-failure.js';
...
import { publishPendingInteractionRuntimeEvent, publishPermissionRuntimeEvent, } from './ipc-interaction-runtime-events.js';
...
export async function processPermissionInteractionIpc(input) {
    ...
    input.request.suggestions ??= synthesizeHostPermissionSuggestions(input.request.toolName, input.request.toolInput);
    try {
        ...
        await beginDurablePermissionInteraction({
            ...
            callbackRoute: {
                ...
                responsePrivateKeySeal: sealIpcResponseSigningPrivateKey(getIpcResponseSigningPrivateKey(input.sourceAgentFolder, input.request.threadId, input.request.responseKeyId)) ?? null,
                responseNonce: input.request.responseNonce ?? null,
            },
        });
        await publishPendingInteractionRuntimeEvent(input.deps, input.request, 'permission', input.sourceAgentFolder);
        ...
        const applied = await applyPermissionInteractionDecision({
            ...
            permissionPersistence: {
                ...
                mirrorAgentToolRulesToSettings: input.deps.mirrorAgentToolRulesToSettings,
            ...
            onPersistentGrantApplied: async (recovery) => {
                const persistentScopeRequest = persistentPermissionScopeRequest(input.request);
                const persistedContext = permissionTelemetryContext(persistentScopeRequest, {
                    sourceAgentFolder: input.sourceAgentFolder,
                    decision: 'persisted',
                    persistedRules: permissionUpdateAllowedToolRules(claimedDecision.updatedPermissions).map(formatDurableAccessRuleForEvent),
                });
        ...
        if (!applied) {
            ...
            input.logger.warn(decisionContext, 'Withholding permission IPC response because grant application failed');
            return;
        }
        ...
        if (decision.approved) {
            ...
            input.logger.info?.(resumedContext, 'Permission resumed current tool call');
            await publishPermissionRuntimeEvent(input.deps, input.request, {
        ...
        const responsePermissionUpdates = persistentPermissionUpdates(decision);
        await assertActiveScheduledPermissionLease(input);
        const resolveInput = {
            request: input.request,
            sourceAgentFolder: input.sourceAgentFolder,
        ...
        let resolutionOutcome = await resolveDurablePermissionInteractionOutcome(resolveInput);
        if (resolutionOutcome === 'retryable_error') {
            input.logger.warn(permissionTelemetryContext(input.request, {
                sourceAgentFolder: input.sourceAgentFolder,
                decision: permissionDecisionName(decision),
        ...
        if (resolutionOutcome !== 'resolved') {
            ...
            archiveIpcErrorFile(input.ipcBaseDir, input.sourceAgentFolder, input.file, input.claimedPath);
            return;
        }
        await assertActiveScheduledPermissionLease(input);
        ...
        }, getIpcResponseSigningPrivateKey(input.sourceAgentFolder, input.request.threadId, input.request.responseKeyId));
        ...
        if (err instanceof DurableInteractionPersistenceError) {
            input.logger.error({ file: input.file, sourceAgentFolder: input.sourceAgentFolder, err }, 'Withholding permission IPC response after durable persistence failure');
            archiveIpcErrorFile(input.ipcBaseDir, input.sourceAgentFolder, input.file, input.claimedPath);
            return;
        }
        incrementOperationalError('interaction', 'permission_request');
...
async function releasePermissionDecisionClaim(decision) {
...
async function denyLockedPermissionInteraction(input, lockStatus) {
    input.logger.warn({
        sourceAgentFolder: input.sourceAgentFolder,
        requestId: input.request.requestId,
        toolName: input.request.toolName,
...
function formatPersistentPermissionOutcome(recovery) {
...
async function sendPermissionOutcomeMessage(deps, request, input) {
...
function pathForGroupIpc(ipcBaseDir, sourceAgentFolder) {
...
export async function processUserQuestionInteractionIpc(input) {
    try {
        ...
        await publishPendingInteractionRuntimeEvent(input.deps, request, 'question', input.sourceAgentFolder);
        ...
        }, getIpcResponseSigningPrivateKey(input.sourceAgentFolder, request.threadId, request.responseKeyId));
        fs.unlinkSync(input.claimedPath);
    }
    catch (err) {
        ...
        if (err instanceof DurableInteractionPersistenceError) {
            input.logger.error({ file: input.file, sourceAgentFolder: input.sourceAgentFolder, err }, 'Withholding user question IPC response after durable persistence failure');
            archiveIpcErrorFile(input.ipcBaseDir, input.sourceAgentFolder, input.file, input.claimedPath);
            return;
        }
        incrementOperationalError('interaction', 'user_question_request');
        ...
        input.logger.error({ file: input.file, sourceAgentFolder: input.sourceAgentFolder, err }, 'Error processing user question IPC request');
        archiveIpcErrorFile(input.ipcBaseDir, input.sourceAgentFolder, input.file, input.claimedPath);
    }
}
