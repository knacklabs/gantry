...
export function parsePermissionCancellationIpcRequest(raw, sourceAgentFolder) {
    if (!isPlainObject(raw)) {
        throw new Error('Invalid permission cancellation IPC payload');
    }
    const binding = validateIpcAuthRequest(raw, sourceAgentFolder, 'permission cancellation IPC', {
...
export function parseQuestionCancellationIpcRequest(raw, sourceAgentFolder) {
    if (!isPlainObject(raw)) {
        throw new Error('Invalid question cancellation IPC payload');
    }
    const binding = validateIpcAuthRequest(raw, sourceAgentFolder, 'question cancellation IPC', {
        extendedAuthPurpose: 'cancellation-retention',
        extendedMaxAgeMs: IPC_CANCELLATION_RETENTION_TTL_MS,
    });
    if (!binding.appId) {
