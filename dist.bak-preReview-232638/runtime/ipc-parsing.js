import { MEMORY_IPC_ACTIONS } from '@gantry/contracts';
import { BROWSER_BACKEND_ACTIONS, PUBLIC_BROWSER_GATEWAY_TOOL_NAMES, } from '../shared/browser-backend-actions.js';
...
import { validateBrowserIpcAuthRequest, validateInteractionIpcAuthRequest, validateIpcAuthRequest, validateMemoryIpcAuthRequest, } from './ipc-auth-validation.js';
import { parseInteractionDescriptor } from './ipc-interaction-descriptor-parsing.js';
import { parsePermissionCancellationIpcRequest, parsePermissionLifecycle, parseQuestionCancellationIpcRequest, } from './ipc-parsing-permission-lifecycle.js';
...
const PERMISSION_DESTINATIONS = new Set(['userSettings', 'projectSettings', 'localSettings', 'session', 'cliArg']);
const PERMISSION_DECISION_MODES = new Set([
    'allow_once',
...
function parsePermissionRuleValues(raw) {
...
function parsePermissionDirectories(raw) {
    if (!Array.isArray(raw))
        return undefined;
...
function parsePermissionApprovalUpdates(raw) {
    if (!Array.isArray(raw))
        return undefined;
    const updates = [];
    for (const item of raw.slice(0, 20)) {
        ...
        if (!PERMISSION_UPDATE_TYPES.has(type)) {
            continue;
        }
        const update = {
            type: type,
        ...
        if (PERMISSION_DESTINATIONS.has(destination)) {
...
function parseClosestPermissionRule(raw) {
...
function parsePermissionDecisionOptions(raw) {
    if (!Array.isArray(raw))
        return undefined;
    const options = raw
        .slice(0, 8)
        .map((entry) => toTrimmedString(entry, { maxLen: 64 }))
        .filter((entry) => PERMISSION_DECISION_MODES.has(entry));
    return options.length ? [...new Set(options)] : undefined;
}
function normalizeBrowserBackendAction(rawAction) {
    if (BROWSER_BACKEND_ACTIONS.includes(rawAction)) {
        return rawAction;
    }
    return undefined;
}
export function parseIpcMessage(raw, sourceAgentFolder) {
    if (!isPlainObject(raw))
        throw new Error('Invalid IPC message payload');
    const { appId, authThreadId: threadId } = validateIpcAuthRequest(raw, sourceAgentFolder, 'IPC message');
    const context = isPlainObject(raw.context) ? raw.context : undefined;
    const providerAccountId = toTrimmedString(raw.providerAccountId, { maxLen: 255 }) ??
    ...
    if (!chatJid || !text)
        throw new Error('Invalid IPC message fields');
...
export function parseMemoryIpcRequest(raw, sourceAgentFolder) {
    if (!isPlainObject(raw))
        throw new Error('Invalid memory IPC payload');
    const { authThreadId: threadId, chatJid, responseKeyId, userId, defaultScope, reviewerIsControlApprover, allowedActions, } = validateMemoryIpcAuthRequest(raw, sourceAgentFolder, 'memory IPC');
    if (!responseKeyId) {
    ...
    if (!MEMORY_IPC_ACTIONS.includes(action)) {
...
export function parsePermissionIpcRequest(raw, sourceAgentFolder) {
    if (!isPlainObject(raw))
        throw new Error('Invalid permission IPC payload');
    // prettier-ignore
    ...
    if (!toolName)
        throw new Error('Permission IPC toolName is required');
    ...
    const jobName = toTrimmedString(raw.jobName, { maxLen: 200 }) ??
    ...
    const payloadRunLeaseFencingVersion = toPositiveInteger(raw.runLeaseFencingVersion);
    const contextRunLeaseFencingVersion = toPositiveInteger(context?.runLeaseFencingVersion);
    if (payloadRunLeaseFencingVersion &&
    ...
    const runLeaseFencingVersion = payloadRunLeaseFencingVersion ?? contextRunLeaseFencingVersion;
    ...
    const hostInjectedCommandPrefix = toTrimmedString(raw.hostInjectedCommandPrefix, { maxLen: HOST_INJECTED_COMMAND_PREFIX_MAX_LENGTH });
    ...
    const decisionToolInput = stripShellCommandEnvPrefix(toolName, raw.toolInput, hostInjectedCommandPrefix);
    const { toolInput, altered: toolInputSanitized, alteredPaths: toolInputSanitizedPaths, } = sanitizeIpcToolInput(decisionToolInput);
    const { toolInput: classifierToolInput, redactedPaths: toolInputRedactedPaths, truncatedPaths: toolInputTruncatedPaths, } = sanitizeIpcToolInput(decisionToolInput, PERMISSION_CLASSIFIER_MAX_STRING_LENGTH);
    const suggestions = parsePermissionApprovalUpdates(raw.suggestions);
    const semanticCapabilityDefinitions = parseSemanticCapabilityDefinitionsRecord(raw.semanticCapabilityDefinitions);
...
export function parseUserQuestionIpcRequest(raw, sourceAgentFolder) {
    if (!isPlainObject(raw))
        throw new Error('Invalid user question IPC payload');
    const binding = validateInteractionIpcAuthRequest(raw, sourceAgentFolder, 'user question IPC');
    ...
    const runLeaseFencingVersion = payloadRunLeaseFencingVersion ?? contextRunLeaseFencingVersion;
    if (jobId && runId && (!runLeaseToken || !runLeaseFencingVersion)) {
        throw new Error('user question IPC scheduled job lease identity is required');
    }
    const payloadTargetJid = toTrimmedString(raw.targetJid, { maxLen: 255 });
    const contextTargetJid = toTrimmedString(context?.chatJid, { maxLen: 255 });
    if (payloadTargetJid &&
    ...
    const questions = raw.questions.map((item, index) => {
        ...
        const options = item.options.map((option, optionIndex) => {
            if (!isPlainObject(option)) {
                throw new Error(`Invalid option payload at index ${index}:${optionIndex}`);
            }
...
export function parseRichInteractionIpcRequest(raw, sourceAgentFolder) {
    if (!isPlainObject(raw)) {
        throw new Error('Invalid rich interaction IPC payload');
    }
    const binding = validateIpcAuthRequest(raw, sourceAgentFolder, 'rich interaction IPC');
    const requestId = toTrimmedString(raw.requestId, { maxLen: 128 });
    if (!requestId || !IPC_REQUEST_ID_PATTERN.test(requestId)) {
    ...
    const descriptor = parseInteractionDescriptor(raw.interaction ?? raw.descriptor);
    if (!descriptor?.rich) {
...
export function parseBrowserIpcRequest(raw, sourceAgentFolder) {
    if (!isPlainObject(raw))
        throw new Error('Invalid browser IPC payload');
    const { authThreadId: threadId, chatJid, responseKeyId, } = validateBrowserIpcAuthRequest(raw, sourceAgentFolder, 'browser IPC');
    if (!responseKeyId) {
