import { decisionForMode, firstPersistentRule, } from '../domain/permission-decision.js';
import { evaluatePermissionDeterministicRails, permissionRiskForDeterministicRailDecision, } from '../domain/permission-deterministic-rails.js';
import { resolveEffectivePermissionMode } from '../shared/permission-mode.js';
import { findConversationRouteForQueue, makeAgentThreadQueueKey, } from '../shared/thread-queue-key.js';
import { agentIdForFolder } from '../domain/agent/agent-folder-id.js';
import { consultPermissionClassifierBeforePrompt, permissionPromotionHintCount, recordHumanPermissionPromotionSignal, } from './permission-classifier.js';
...
import { computePermissionEffectHash, EFFECT_SCHEMA_VERSION, RAIL_CATALOG_VERSION, } from '../domain/permission-effect-key.js';
import { evaluateYoloModeDenylist, yoloModeDenylistDenyReason, } from '../shared/yolo-mode-policy.js';
import { buildAgentToolExecutionRequest, evaluateProtectedCapabilityToolUse, ToolExecutionClassifier, ToolExecutionPolicyService, } from '../shared/tool-execution-policy-service.js';
import { coordinatePermissionDecision, permissionRunRestriction, } from './permission-decision-coordinator.js';
export async function resolvePermissionIpcDecision(input) {
    const settings = input.deps.getPermissionRuntimeSettings?.();
    const agentSettings = settings?.agents[input.sourceAgentFolder];
    const approvedCapabilityIds = agentSettings?.capabilities?.map(({ id }) => id) ?? [];
    ...
    const fixedImageRestricted = input.request.responseKeyId
        ...
        : false;
    const protectedCapability = evaluateProtectedCapabilityToolUse(input.request.toolName, input.request.toolInput);
    const yoloMode = settings?.permissions?.yoloMode;
    ...
    return coordinatePermissionDecision({
        ...
        reviewedRuleDecision: async () => {
            ...
            return new ToolExecutionPolicyService().evaluate({
                request: buildAgentToolExecutionRequest(new ToolExecutionClassifier(), input.request.toolName, input.request.toolInput, {
                ...
                semanticCapabilityDefinitions: Object.fromEntries(policy.semanticCapabilities.map((capability) => [
        ...
        tail: () => resolvePermissionIpcDecisionTail({
            ...input,
            effectHash,
            decisionMemory,
            railRisk,
...
async function resolvePermissionIpcDecisionTail(input) {
    const route = input.request.targetJid
        ? findConversationRouteForQueue(input.deps.conversationRoutes?.() ?? {}, makeAgentThreadQueueKey(input.request.targetJid, agentIdForFolder(input.sourceAgentFolder), input.request.threadId, input.request.providerAccountId), (candidate) => agentIdForFolder(candidate.folder))
        : undefined;
    const settings = input.deps.getPermissionRuntimeSettings?.();
    const approvedCapabilityIds = settings?.agents[input.sourceAgentFolder]?.capabilities?.map(({ id }) => id) ?? [];
    ...
    const classifierConfig = settings
        ...
        : undefined;
    const permissionMode = resolveEffectivePermissionMode(route?.folder === input.sourceAgentFolder
        ? route.agentConfig?.permissionMode
        : undefined, settings?.agents[input.sourceAgentFolder]?.permissionMode);
    ...
    const promotion = promotionRepository
        ...
        : undefined;
    const shouldConsultClassifier = input.deps.publishRuntimeEvent &&
        classifierConfig &&
        (permissionMode === 'auto' || permissionMode === 'auto_strict');
    const toolRepository = input.deps.getToolRepository?.();
    const reviewedMcpReadBindings = shouldConsultClassifier &&
        toolRepository &&
        ...
        ? ((await resolveAgentToolRuntimePolicy({
            ...
            errorSubject: 'Configured agent tool',
            skillRepository: input.deps.getSkillRepository?.(),
        }).catch(() => undefined))?.reviewedMcpReadBindings ?? [])
        : [];
    const classifierDecision = shouldConsultClassifier
        ? await consultPermissionClassifierBeforePrompt({
            ...
            policyDecisionReason: input.request.decisionReason ?? 'Human approval is required.',
            approvedCapabilityIds,
        ...
        : undefined;
    const railVetoedClassifierAllow = classifierDecision?.decision === 'allow' && input.railRequiresApproval;
    const primaryRisk = selectPrimaryPermissionRisk(input.railRisk, classifierDecision
        ? {
    ...
    if (classifierDecision?.decision === 'allow' && !input.railRequiresApproval) {
        return withRequestRisk(input.request, decisionForMode(input.request, 'allow_once', 'auto_classifier'));
    }
    if ((permissionMode === 'auto' || permissionMode === 'auto_strict') &&
        input.request.unattended) {
        return withRequestRisk(input.request, {
            ...decisionForMode(input.request, 'cancel', railVetoedClassifierAllow ? 'deterministic_rails' : 'runtime'),
    ...
    if (classifierDecision?.denylistHit) {
        ...
        return withRequestRisk(input.request, await input.deps.requestPermissionApproval(input.request));
    }
    ...
    const effectiveDecisionOptions = input.request.decisionOptions?.length
        ...
        : firstPersistentRule(input.request)
            ...
            : ['allow_once', 'cancel'];
    if (input.request.promotionHintCount &&
...
function withRequestRisk(request, decision) {
    return {
        ...decision,
...
const PERMISSION_RISK_SEVERITY_RANK = {
    low: 0,
    medium: 1,
    high: 2,
    critical: 3,
};
function selectPrimaryPermissionRisk(railRisk, classifierRisk) {
    if (!railRisk)
        return classifierRisk;
    if (!classifierRisk)
        return railRisk;
    if (PERMISSION_RISK_SEVERITY_RANK[classifierRisk.level] <=
