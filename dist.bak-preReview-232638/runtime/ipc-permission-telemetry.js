...
export function permissionDecisionName(decision) {
    if (decision.approved)
        return 'allowed';
    return decision.mode === 'cancel' ? 'cancelled' : 'denied';
}
export function permissionDecisionEventType(decision) {
    if (decision.approved)
        return RUNTIME_EVENT_TYPES.PERMISSION_ALLOWED;
    return decision.mode === 'cancel'
        ? RUNTIME_EVENT_TYPES.PERMISSION_CANCELLED
        : RUNTIME_EVENT_TYPES.PERMISSION_DENIED;
}
export function permissionTelemetryContext(request, extra) {
...
function permissionCanonicalCapability(request) {
...
function safeCommandTelemetry(command) {
    if (!command)
        return {};
    return {
