...
export async function processRichInteractionIpc(input) {
    try {
        ...
        if (!targetJid)
            throw new Error('Rich interaction target is missing');
        ...
        const progressDelivered = progress && input.deps.renderAgentTodo
            ? await input.deps.renderAgentTodo(targetJid, progress, options)
            : false;
        const delivered = progressDelivered ||
            (input.deps.renderRichInteraction
                ? await input.deps.renderRichInteraction(targetJid, input.request, options)
                : false);
        if (!delivered) {
...
function progressRender(request) {
    if (request.descriptor.rich?.kind !== 'progress')
        return undefined;
...
async function sendRichInteractionFallback(deps, request) {
    if (!request.targetJid)
        return;
    await deps.sendMessage(request.targetJid, `${RICH_INTERACTION_NATIVE_FALLBACK_TEXT}\n\n${request.descriptor.rich?.fallbackText ?? request.descriptor.fallbackText ?? ''}`.trim(), {
