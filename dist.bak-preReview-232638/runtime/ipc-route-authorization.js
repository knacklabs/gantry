...
export function resolveRunnerIpcRoute(input) {
    ...
    const matches = Object.entries(input.routes)
        ...
        const providerAccountId = parsed.providerAccountId ?? route.providerAccountId?.trim();
        return { parsed, route, providerAccountId };
    })
        .filter(({ parsed, route, providerAccountId }) => {
        if (route.folder !== input.sourceAgentFolder)
            return false;
        if (input.targetJid && parsed.chatJid !== input.targetJid)
            return false;
        if (parsed.threadId && parsed.threadId !== input.threadId)
            return false;
    ...
    const distinctIdentities = (items) => new Set(items.map(({ parsed, route, providerAccountId }) => `${parsed.chatJid}::${route.conversationId ?? ''}::${providerAccountId ?? ''}`));
    ...
    if (distinctIdentities(selected).size > 1 && requestedProviderAccountId) {
        const qualified = selected.filter(({ parsed }) => parsed.providerAccountId === requestedProviderAccountId);
