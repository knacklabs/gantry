...
async function validateScheduledInteractionIpcJobExecutionTarget(input) {
    ...
    if (!execution?.conversationJid) {
        throw new Error(`Scheduled job ${kind} IPC requires canonical execution_context`);
    }
    const executionWorkspaceKey = normalizeNullableString(execution.workspaceKey) ??
        normalizeNullableString(job.workspace_key);
    if (executionWorkspaceKey && executionWorkspaceKey !== sourceAgentFolder) {
        throw new Error(`Scheduled job ${kind} IPC source does not match job execution context`);
    }
    if (execution.conversationJid !== request.targetJid) {
        throw new Error(`Scheduled job ${kind} IPC target does not match job execution context`);
    }
    if (normalizeNullableString(execution.threadId) !==
        normalizeNullableString(request.threadId)) {
        throw new Error(`Scheduled job ${kind} IPC thread does not match job execution context`);
    }
