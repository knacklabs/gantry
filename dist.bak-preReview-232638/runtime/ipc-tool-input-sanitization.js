...
export const SENSITIVE_TOOL_INPUT_KEY_PATTERN = /(secret|token|password|passphrase|credential|api[_-]?key|key|authorization|bearer|cookie|session)/i;
const AUTH_VALUE_PATTERN = /\b(?:bearer|basic)\s+[A-Za-z0-9._~+/=-]{8,}/gi;
const KNOWN_TOKEN_PATTERN = /\b(?:sk-[A-Za-z0-9_-]{8,}|gh[po]_[A-Za-z0-9_]{8,}|github_pat_[A-Za-z0-9_]{8,}|xox[abp]-[A-Za-z0-9-]{8,}|AKIA[0-9A-Z]{16}|AIza[0-9A-Za-z_-]{20,})\b/g;
const ENV_VALUE_PATTERN = /\b([A-Za-z_][A-Za-z0-9_-]*)(\s*(?:=|:)\s*)("[^"]*"|'[^']*'|[^\s,;}]+)/g;
const URL_USERINFO_PATTERN = /(:\/\/)[^\s/@:]+:[^\s/@]+@/g;
export function redactSensitiveToolInputString(value) {
    return value
        .replace(URL_USERINFO_PATTERN, '$1[REDACTED]@')
        .replace(AUTH_VALUE_PATTERN, '[REDACTED]')
        .replace(KNOWN_TOKEN_PATTERN, '[REDACTED]')
        .replace(ENV_VALUE_PATTERN, (match, key, separator) => SENSITIVE_TOOL_INPUT_KEY_PATTERN.test(key)
...
function sanitizeValue(value, depth, path, state) {
    ...
    if (typeof value === 'string') {
        ...
        if (redacted.length <= state.maxStringLength)
            return redacted;
        state.alteredPaths.add(path);
        state.truncatedPaths.add(path);
    ...
    if (Array.isArray(value)) {
        const kept = value
            ...
            .map((entry, index) => sanitizeValue(entry, depth + 1, childPath(path, index), state));
        for (let index = TOOL_INPUT_MAX_ARRAY_ENTRIES; index < value.length; index += 1) {
...
export function sanitizeIpcToolInput(value, maxStringLength = TOOL_INPUT_MAX_STRING_LENGTH) {
    if (!isPlainObject(value)) {
    ...
    const toolInput = sanitizeValue(value, 0, '', state);
