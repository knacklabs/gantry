...
import { isPermissionClassifierEligible, } from '../application/permissions/permission-classifier.js';
import { PERMISSION_PROMOTION_ALLOW_THRESHOLD, } from '../application/permissions/permission-promotion.js';
import { permissionSuggestionKey, synthesizeHostPermissionSuggestions, } from '../application/permissions/permission-suggestion-synthesis.js';
...
import { isMemoryOperationTimeoutError, runWithMemoryOperationTimeout, } from '../shared/memory-dreaming-timeout.js';
import { resolveModelSelectionForWorkload } from '../shared/model-catalog.js';
import * as yolo from '../shared/yolo-mode-policy.js';
import { classifierUserPayload, parsePermissionClassifierResponse, permissionClassifierSystemPrompt, serializePermissionClassifierToolInput, } from './permission-classifier-prompt.js';
...
export async function consultPermissionClassifier(input) {
    ...
    let response;
    try {
        response = await runWithMemoryOperationTimeout((signal) => {
            const query = {
                appId: input.appId,
    ...
    if (!verdict.ok) {
        return failedResult(verdict.failureCode, startedAt, verdict.error, modelSelection.model);
    }
    return {
...
export async function consultPermissionClassifierBeforePrompt(input) {
    if ((input.permissionMode !== 'auto' &&
        input.permissionMode !== 'auto_strict') ||
        !isPermissionClassifierEligible(input.canonicalToolName, input.requestFamily)) {
        return undefined;
    }
    const suggestions = input.suggestions ??
        synthesizeHostPermissionSuggestions(input.canonicalToolName, input.toolInput);
    ...
    const inputTruncated = pathTruncated ||
    ...
    const deterministicGate = inputTruncated || yoloDenylistMatch
        ? undefined
    ...
    const classifierResult = inputTruncated
        ? {
            risk_level: 'high',
            reason: 'Classifier skipped because its tool input view was incomplete; ask the user.',
            latencyMs: 0,
        ...
        : // prettier-ignore
            yoloDenylistMatch ? { risk_level: 'high', reason: `YOLO-mode denylist backstop matched "${yoloDenylistMatch.pattern}"; ask the user for explicit approval.`, latencyMs: 0 }
                : !deterministicGate?.allowed && input.permissionMode === 'auto_strict'
                    ? {
                        risk_level: 'high',
                        reason: deterministicGate?.reason ??
                    ...
                    : await (input.classifierConsult ?? consultPermissionClassifier)({
                        ...
                        approvedCapabilityIds: input.approvedCapabilityIds,
                        recentlyApprovedExactToolShape: wasRecentlyApproved(promotionCounter),
    ...
    if (yoloDenylistMatch && !inputTruncated) {
        ...
        await input
            ...
            .catch((error) => {
            logger.warn({ err: error, toolName: input.canonicalToolName }, 'Failed to publish YOLO denylist hit event');
        });
    }
...
export function recordHumanPermissionPromotionSignal(input) {
    if (!input.repository || !isHumanPermissionDecision(input.decision))
        return;
    const suggestionKey = permissionSuggestionKey(input.agentFolder, input.request.suggestions);
    if (!suggestionKey)
        return;
    const nowIso = new Date().toISOString();
    const operation = input.decision.approved && input.decision.mode === 'allow_once'
        ...
        : input.decision.mode === 'cancel' ||
            ...
            : undefined;
    void operation?.catch((error) => logger.warn({
        error,
        appId: input.appId,
        agentFolder: input.agentFolder,
        suggestionKey,
...
function isHumanPermissionDecision(decision) {
    const decidedBy = decision.decidedBy?.trim().toLowerCase();
    return Boolean(decidedBy && !['auto_classifier', 'runtime', 'system'].includes(decidedBy));
}
async function readPromotionCounter(input) {
    if (!input.promotion || !input.suggestionKey)
        return null;
    try {
        ...
        logger.warn({ error, appId: input.appId, agentFolder: input.agentFolder }, 'Permission promotion context read failed');
        return null;
    }
}
function wasRecentlyDenied(counter) {
    if (!counter?.deniedAt)
        return false;
    const deniedAtMs = Date.parse(counter.deniedAt);
    return (Number.isFinite(deniedAtMs) &&
        deniedAtMs >= Date.now() - RECENT_PERMISSION_SIGNAL_MS);
}
function wasRecentlyApproved(counter) {
    if (!counter ||
...
function resolveClassifierModel(input) {
    const resolved = resolveModelSelectionForWorkload(input.autoModeModel?.trim() || input.memoryModelConfig.extractor, 'memory_extractor');
    if (!resolved.ok)
...
export async function publishPermissionClassifierDecision(input) {
...
function failedResult(failureCode, startedAt, error, model) {
    logger.warn({ failureCode, reasonCode: failureCode, ...(error ? { error } : {}) }, 'Permission classifier consultation failed');
    return {
        risk_level: 'high',
...
function isClassifierTimeoutError(error) {
    return (isMemoryOperationTimeoutError(error) ||
