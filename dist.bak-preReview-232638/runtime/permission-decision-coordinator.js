...
import { EFFECT_SCHEMA_VERSION, RAIL_CATALOG_VERSION, } from '../domain/permission-effect-key.js';
 ...
 */
export async function coordinatePermissionClassifierRisk(input) {
    return input.riskLevel === 'low' || input.riskLevel === 'medium'
        ? input.allow()
        : input.tail();
}
export async function coordinatePermissionDecision(input) {
    if (input.hardDenyReason) {
        return denied(input.request, input.hardDenyReason, 'hard_deny');
    }
    if (input.accessPreset === 'locked') {
        return denied(input.request, 'capability not provisioned: this agent runs with a locked access preset.', 'locked_preset');
    }
    if (input.fixedImageRestricted) {
        return denied(input.request, 'capability not provisioned: this run uses a fixed authority image.', 'fixed_image');
    }
    const reviewedRuleDecision = typeof input.reviewedRuleDecision === 'function'
    ...
    if (input.effectHash && input.decisionMemory) {
        ...
        if (cached?.decision === 'allow') {
            ...
            return {
                ...decisionForMode(input.request, 'allow_once', 'cached_classifier_verdict'),
...
const TRUSTED_ROOT_LEARN_OPTIONS = [
 ...
 */
async function resolveTrustedRootStage(input, railFn, railsInput) {
    ...
    if (!workspaceRoot || typeof memory?.list !== 'function')
        return undefined;
...
function denied(request, reason, decidedBy) {
    return {
