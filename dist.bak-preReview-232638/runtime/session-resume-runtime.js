...
import { PROVIDER_SESSION_FIELD_NAME_LIST, PROVIDER_SESSION_HANDLE_START_LIST, redactProviderSessionHandlesInText, } from '../shared/provider-session-redaction.js';
...
const RUNTIME_RESULT_SUMMARY_TRUNCATION_PREFIX = '[output truncated; showing tail]\n';
export function truncateRuntimeResultSummary(value, maxChars) {
    ...
    const prefix = maxChars > RUNTIME_RESULT_SUMMARY_TRUNCATION_PREFIX.length
...
export function summarizeRuntimeResultForPersistence(value) {
    if (value == null)
        return null;
    return truncateRuntimeResultSummary(redactProviderSessionHandlesInText(value), RUNTIME_RESULT_SUMMARY_MAX_CHARS);
}
export function createRuntimeResultSummaryAccumulator(input) {
    const maxChars = Math.max(0, Math.floor(input?.maxChars ?? RUNTIME_RESULT_SUMMARY_MAX_CHARS));
    const prefix = maxChars > RUNTIME_RESULT_SUMMARY_TRUNCATION_PREFIX.length
        ? RUNTIME_RESULT_SUMMARY_TRUNCATION_PREFIX
        : '';
...
const PROVIDER_SESSION_FIELD_END_PATTERN = /\b(?:sessionId|newSessionId|providerSessionId|externalSessionId|latestProviderSessionId|session_id)\s*(?::|=|\s)\s*[^\s"',}\]]*$/i;
const PROVIDER_SESSION_TOKEN_CHARS = /^[A-Za-z0-9._:-]*$/;
const STREAM_SANITIZER_MAX_CARRY_CHARS = 512;
function partialSuffixStart(value, candidates) {
...
function splitInternalCarry(value, done) {
    if (done)
        return { body: value, carry: '' };
...
function stripInternalBlocksIncrementally(value, state) {
    let remaining = value;
    let out = '';
    while (remaining) {
...
function splitProviderSessionCarry(value, done) {
    if (done)
        return { body: value, carry: '' };
    ...
    if (partialStart >= 0)
        carryStart = Math.min(carryStart, partialStart);
...
export function createRuntimeUserVisibleResultAccumulator(input) {
    ...
    const flush = (delta, done) => {
        const { body: internalBody, carry: internalCarry } = splitInternalCarry(`${state.carry}${delta}`, done);
        state.carry = internalCarry;
        const withoutInternal = stripInternalBlocksIncrementally(internalBody, state);
        const { body: providerBody, carry: providerCarry } = splitProviderSessionCarry(withoutInternal, done);
...
export async function archiveCurrentRuntimeSession(input) {
    const turnContext = await input.ops.getAgentTurnContext?.({
        ...
        executionProviderId: input.executionProviderId ?? resolveRuntimeExecutionProviderId(),
    ...
    if (turnContext?.agentSessionId && collectMemory) {
        const trigger = input.cause === 'manual-compact' ? 'precompact' : 'session-end';
        try {
            ...
            memory = 'degraded';
            logger.warn({ group: input.group.name, err, trigger }, 'Failed to collect durable memory at session boundary');
        }
    }
    logger.info({ group: input.group.name, agentSessionId: turnContext?.agentSessionId }, 'Archived Gantry session boundary memory; provider transcripts are not runtime state');
    return { memory };
}
export function buildRuntimeRunOptions(input) {
    const resolvedSkillContext = input.skillContext
        ...
        : input.turnContext
            ...
            : undefined;
    const skillOptions = input.skillRepository && input.skillArtifactStore && resolvedSkillContext
        ? {
        ...
        : {};
    const mcpOptions = input.mcpServerRepository &&
...
export async function completeSuccessfulRuntimeSessionRun(input) {
    if (input.runId) {
        try {
            ...
            logger.warn({ err, runId: input.runId }, 'Failed to complete runtime session run; continuing with outer run finalization');
        }
    }
...
export async function completeFailedRuntimeSessionRun(input) {
    ...
    try {
        await input.ops.completeSessionAgentRun?.({
            ...
            errorSummary: summarizeRuntimeResultForPersistence(redactString(input.errorSummary)),
        });
    }
    catch (err) {
        logger.warn({ err, runId: input.runId }, 'Failed to complete runtime session run; continuing with outer run finalization');
...
export async function failRuntimeSessionRun(ops, runId, errorSummary) {
...
export async function buildApprovedSkillContextBlock(input) {
    ...
    sections.push('Full SKILL.md instructions and supporting files are read only through the selected provider or DeepAgents skill mechanism after the skill matches the task and the run has authority.');
    sections.push('[[/APPROVED_SKILLS_AVAILABLE_THIS_SESSION]]');
    return sections.length > 3 ? sections.join('\n') : '';
}
export function resolveMemoryUserId(messages) {
...
export function resolveNonSelfSenderIds(messages) {
...
export function resolveSingleNonSelfSenderId(messages) {
