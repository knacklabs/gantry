function parsePermissionsCommand(text) {
    if (text === '/permissions')
        return { kind: 'permissions_show', raw: '/permissions' };
    const value = text.match(/^\/permissions\s+(ask|auto|auto_strict|default)$/)?.[1];
    if (value === 'default')
...
function parseThinkingCommand(text) {
    if (text === '/thinking')
        return { kind: 'thinking_show', raw: '/thinking' };
    ...
    if (effortMatch) {
        return {
            ...
            value: { mode: 'adaptive', effort: effortMatch[1] },
        };
    }
...
function parseMentionFriendlySessionCommand(text) {
    if (!text.startsWith('!'))
        return null;
    ...
    if (commandText === 'dream')
        return { kind: 'dream', raw: '/dream' };
    if (commandText === 'memory-status') {
        return { kind: 'memory_status', raw: '/memory-status' };
    }
    if (commandText === 'models')
        return { kind: 'models_list', raw: '/models' };
    if (commandText === 'status')
        return { kind: 'status', raw: '/status' };
    if (commandText === 'model')
        return { kind: 'model_show', raw: '/model' };
    ...
    if (thinkingMatch) {
        return parseThinkingCommand(thinkingMatch[1] ? `/thinking ${thinkingMatch[1].trim()}` : '/thinking');
    }
    const permissionsMatch = commandText.match(/^permissions(?:\s+(.+))?$/);
    if (permissionsMatch) {
        return parsePermissionsCommand(permissionsMatch[1]
            ? `/permissions ${permissionsMatch[1].trim()}`
            : '/permissions');
    }
    return null;
}
function normalizeGantryCommandEnvelope(text) {
...
export function extractSessionCommand(content, triggerPattern) {
    ...
    const hasTriggerPrefix = new RegExp(triggerPattern.source, triggerPattern.flags.replace(/g/g, '')).test(text);
    ...
    if (text === '/model')
        return { kind: 'model_show', raw: '/model' };
    const saveProcedureMatch = text.match(/^\/save-procedure(?:\s+"([^"\n]{1,80})"|\s+([^\n]{1,80}))(?:\n([\s\S]+))?$/);
 ...
 */
export function isSessionCommandAllowed(isFromMe, isSenderControlAllowlisted) {
