...
import { extractSessionCommand, isSessionCommandAllowed, } from './session-command-parse.js';
...
import { getModelFamily, resolveModelSelectionForWorkloadWithFamilies, } from '../shared/model-families.js';
import { formatModelDisplay } from '../shared/model-catalog-format.js';
import { describeThinking, formatBrowserStatus, formatCompactionStatus, formatCurrentModel, formatModelsList, formatModelStatus, formatMemoryStatus, formatModelWhy, } from './session-command-format.js';
import { formatSessionCommandsHelp } from './session-command-help.js';
import { defaultModelStatusSelection, } from './session-model-status.js';
import { COMPACTION_ALREADY_RUNNING_MESSAGE, COMPACTION_QUEUED_MESSAGE, hasQueuedSessionCompaction, queueSessionCompaction, } from './session-compaction-command.js';
import { prepareNewSessionArchive, runNewSessionArchiveFinalizer, } from './session-new-archive.js';
import { encodeGroupMessageCursor, toGroupMessageCursor, } from '../shared/message-cursor.js';
function isDreamQueueResult(value) {
...
async function readConfiguredProviders(deps) {
    if (!deps.getConfiguredModelProviders)
        return undefined;
    try {
        return await deps.getConfiguredModelProviders();
    }
    catch (err) {
        logger.warn({ err }, 'Failed to read configured model providers for session command');
        return undefined;
    }
}
...
function sanitizeErrorText(text) {
    ...
    const normalized = noAnsi
        ...
        .replace(/\s+/g, ' ')
        .trim();
    if (normalized.length <= MAX_MODEL_ERROR_MESSAGE_CHARS)
        return normalized;
    return `${normalized.slice(0, MAX_MODEL_ERROR_MESSAGE_CHARS - 1)}…`;
}
const FORBIDDEN_PROCEDURE_TITLE_PREFIX = /^(Found it|Findings|Critical|End-to-end|No answer|Three full|On it|##|\*\*)/i;
function hasAtLeastTwoNumberedSteps(body) {
...
export async function handleSessionCommand(opts) {
    ...
    if (!isSessionCommandAllowed(cmdMsg.is_from_me === true, deps.isSenderControlAllowlisted(cmdMsg))) {
    ...
    if (command.kind === 'stop') {
        ...
        deps.advanceCursor(cmdMsg);
        await deps.sendMessage(stopped ? 'Stopping current run.' : 'No active run to stop.');
        return { handled: true, success: true };
    }
    ...
    if (preCommandMsgs.length > 0) {
        ...
        if (preResult !== 'success' || hadPreError) {
            logger.warn({ group: groupName }, 'Pre-command processing failed, aborting session command');
            await deps.sendMessage(`Failed to process messages before ${command.raw}. Try again.`);
            if (preOutputSent) {
        ...
        try {
            ...
            if (isDreamQueueResult(result)) {
                if (!result.queued && result.reason === 'deduped') {
                    await deps.sendMessage(`Dreaming already in progress.\n${JSON.stringify(result).slice(0, 1500)}`);
                    return { handled: true, success: true };
                }
                if (!result.queued) {
            ...
            const summary = result && typeof result === 'object'
                ...
                : String(result ?? '');
            await deps.sendMessage(`Dreaming completed.\n${summary.slice(0, 1500) || 'no output'}`);
            return { handled: true, success: true };
        }
        catch (err) {
    ...
    if (command.kind === 'compact') {
        deps.advanceCursor(cmdMsg);
        const queueResult = await queueSessionCompaction(groupName, deps, encodeGroupMessageCursor(toGroupMessageCursor(cmdMsg)));
        await deps.sendMessage(queueResult === 'queued'
    ...
    if (command.kind === 'memory_status') {
        ...
        try {
            ...
            const message = err instanceof Error ? err.message : String(err);
            await deps.sendMessage(`/memory-status failed: ${sanitizeErrorText(message)}`);
            return { handled: true, success: true };
        }
    }
    if (command.kind === 'save_procedure') {
        ...
        if (FORBIDDEN_PROCEDURE_TITLE_PREFIX.test(command.title)) {
            await deps.sendMessage('/save-procedure rejected: title looks like transcript text, not a reusable procedure.');
            return { handled: true, success: true };
        }
        const procedureBody = command.body || deps.formatMessages(preCommandMsgs, timezone).trim();
        if (!procedureBody) {
            await deps.sendMessage('/save-procedure requires steps in the message body or prior context in this batch.');
            return { handled: true, success: true };
        }
        if (!hasAtLeastTwoNumberedSteps(procedureBody)) {
            await deps.sendMessage('/save-procedure body must include at least two numbered steps.');
            return { handled: true, success: true };
        }
        ...
        if (assistantBodies.some((entry) => entry === normalizedBody)) {
            await deps.sendMessage('/save-procedure rejected: body matches a recent assistant reply verbatim.');
            return { handled: true, success: true };
        }
        try {
            ...
            await deps.sendMessage(`Saved procedure "${command.title}"${result && 'id' in result ? ` (${result.id})` : ''}.`);
            return { handled: true, success: true };
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
            await deps.sendMessage(`/save-procedure failed: ${sanitizeErrorText(message)}`);
            return { handled: true, success: true };
        }
    }
    ...
    if (command.kind === 'model_show') {
        deps.advanceCursor(cmdMsg);
        await deps.sendMessage(formatCurrentModel(defaultModel, groupOverrideModel));
        return { handled: true, success: true };
    }
    if (command.kind === 'models_list') {
        deps.advanceCursor(cmdMsg);
        const configuredProviders = await readConfiguredProviders(deps);
        await deps.sendMessage(formatModelsList({
            defaults: {
                chat: defaultModel,
    ...
    if (command.kind === 'model_why') {
        deps.advanceCursor(cmdMsg);
        const configuredProviders = await readConfiguredProviders(deps);
        await deps.sendMessage(formatModelWhy({
            value: command.value,
            configuredProviders,
            familyOrder: deps.getModelFamilyOrder?.(),
        }));
    ...
    if (command.kind === 'status') {
        ...
        const shownCompactionStatus = hasQueuedSessionCompaction(compactionScopeKey) &&
            (!compactionStatus || compactionStatus.state === 'idle')
            ? { state: 'queued' }
            : compactionStatus;
        await deps.sendMessage(`${modelStatusText}${shownCompactionStatus
    ...
    if (command.kind === 'permissions_show') {
        deps.advanceCursor(cmdMsg);
        await deps.sendMessage(`Current permission mode: ${groupPermissionModeOverride ?? deps.getDefaultPermissionMode()} (${groupPermissionModeOverride ? 'conversation override' : 'agent/default'}).`);
        return { handled: true, success: true };
    }
    if (command.kind === 'model_set') {
        ...
        const resolved = resolveModelSelectionForWorkloadWithFamilies(command.value, 'chat', deps.getModelFamilyOrder?.());
        if (!resolved.ok) {
            deps.advanceCursor(cmdMsg);
        ...
        try {
            ...
            logger.error({ group: groupName, err, model: resolved.alias }, 'Failed to persist /model override');
            await deps.sendMessage(`Failed to set model to ${resolved.alias}. Override unchanged.`);
            return { handled: true, success: false };
        }
        deps.advanceCursor(cmdMsg);
    ...
    if (command.kind === 'model_default') {
        try {
            ...
            deps.updateModelStatusSelection?.(defaultModelStatusSelection(defaultModel));
        }
        catch (err) {
            logger.error({ group: groupName, err }, 'Failed to clear /model override');
            await deps.sendMessage('Failed to clear model override. Override unchanged.');
            return { handled: true, success: false };
        }
        deps.advanceCursor(cmdMsg);
        if (defaultModel) {
            const defaultEntry = findModelByRunnerModel(defaultModel);
            await deps.sendMessage(`Model override cleared. Using default model: ${defaultEntry ? formatModelDisplay(defaultEntry) : defaultModel}.`);
        }
        else {
            await deps.sendMessage('Model override cleared. Using CLI default model selection.');
        }
        return { handled: true, success: true };
    }
    if (command.kind === 'thinking_set') {
        try {
            ...
            logger.error({ group: groupName, err, thinking: command.value }, 'Failed to persist /thinking override');
            await deps.sendMessage('Failed to set thinking. Override unchanged.');
            return { handled: true, success: false };
        }
        deps.advanceCursor(cmdMsg);
        await deps.sendMessage(`Thinking set to ${describeThinking(command.value)} for this group.`);
        return { handled: true, success: true };
    }
    if (command.kind === 'thinking_default') {
        try {
            ...
            logger.error({ group: groupName, err }, 'Failed to clear /thinking override');
            await deps.sendMessage('Failed to clear thinking override. Override unchanged.');
            return { handled: true, success: false };
        }
        deps.advanceCursor(cmdMsg);
        await deps.sendMessage('Thinking override cleared. Using default thinking: adaptive (effort medium).');
        return { handled: true, success: true };
    }
    if (command.kind === 'permissions_set' ||
        command.kind === 'permissions_default') {
        const value = command.kind === 'permissions_set' ? command.value : undefined;
        try {
            await deps.setGroupPermissionModeOverride(value);
        }
        catch (err) {
            logger.error({ group: groupName, err, permissionMode: value ?? null }, 'Failed to persist /permissions override');
            await deps.sendMessage(value
