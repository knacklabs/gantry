...
export const COMPACTION_QUEUED_MESSAGE = "Compaction queued. You can keep messaging me; I'll use the compacted context when it's ready.";
export const COMPACTION_ALREADY_RUNNING_MESSAGE = 'Compaction is already running or queued. You can keep messaging me.';
const COMPACTION_READY_MESSAGE = "Compaction ready. I'll use the compacted context and updated memory on your next message.";
const COMPACTION_DEGRADED_MESSAGE = "Compaction ready, but memory extraction did not finish. I'll use compacted context and existing memory.";
const COMPACTION_FRESH_CHECKPOINT_READY_MESSAGE = "Compaction ready. I'll use updated memory and a fresh provider context on your next message.";
const COMPACTION_FRESH_CHECKPOINT_DEGRADED_MESSAGE = "Compaction ready, but memory extraction did not finish. I'll use a fresh provider context and existing memory.";
const COMPACTION_FAILED_MESSAGE = "Compaction did not finish. I'll keep using current continuity and memory.";
...
async function publishSessionCompactionEvent(groupName, deps, state, details) {
    try {
        await deps.publishSessionCompactionEvent?.(state, details);
    }
    catch (err) {
        logger.warn({ group: groupName, err, state }, 'Failed to publish session compaction runtime event');
    }
}
export function hasQueuedSessionCompaction(scopeKey) {
    return queuedCompactions.has(scopeKey);
}
export async function queueSessionCompaction(groupName, deps, baseCursor) {
    const dedupeKey = deps.compactionScopeKey?.trim() || groupName;
    const admittedTask = await deps.admitSessionCompactionTask?.();
    if (admittedTask && !admittedTask.admitted)
    ...
    void (async () => {
        ...
        if (admittedTask) {
            ...
            task =
                (await deps.markSessionCompactionTaskRunning?.(admittedTask.task, locked)) ?? admittedTask.task;
        ...
        try {
            ...
            const strategy = (await deps.getSessionCompactionStrategy?.()) ?? 'provider_compaction';
            if (strategy === 'fresh_checkpoint') {
                ...
                const outcome = archiveOutcome && archiveOutcome.memory === 'degraded'
                    ? 'degraded'
                    : 'ready';
                ...
                await deps.sendMessage(outcome === 'degraded'
                    ? COMPACTION_FRESH_CHECKPOINT_DEGRADED_MESSAGE
                    : COMPACTION_FRESH_CHECKPOINT_READY_MESSAGE);
                return;
            }
            const compactResult = await deps.runSessionCompaction(async (result) => {
            ...
            await deps.finishSessionCompactionTask?.(task, archiveOutcome && archiveOutcome.memory === 'degraded'
                ? 'degraded'
                : 'ready');
            await publishSessionCompactionEvent(groupName, deps, archiveOutcome && archiveOutcome.memory === 'degraded'
                ? 'degraded'
                : 'ready', {
                task,
                strategy,
            });
            await deps.sendMessage(archiveOutcome && archiveOutcome.memory === 'degraded'
