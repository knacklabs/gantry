import { isRunCommandToolRule, publicGantryToolNameForSdkTool, RUN_COMMAND_TOOL_NAME, } from './gantry-tool-facades.js';
...
export function inlineWorkerOnlyToolRuleLabels(rules) {
    return [...new Set(rules.filter(isInlineWorkerOnlyToolRule))].sort();
}
export function formatInlineAgentWorkerOnlyConfigError(subject, labels) {
...
export function inlineAgentSkillEngineConstraintError(input) {
    ...
    if ((input.agentRuntime ?? 'worker') !== 'inline' ||
...
export function isInlineWorkerOnlyToolRule(rule) {
    ...
    return (publicName === RUN_COMMAND_TOOL_NAME ||
