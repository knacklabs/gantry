...
export function sedReadFileArgs(args) {
...
export function genericReadFileArgs(executable, args) {
    const fileArgs = [];
    ...
    if (executable === 'uniq' && fileArgs.length > 1)
        return undefined;
    return fileArgs;
}
function genericReadOptionValue(executable, option) {
    if (executable === 'cut') {
        if (/^-[sz]+$/.test(option) ||
    ...
    if (executable === 'rev')
        return undefined;
    if (executable === 'sort') {
        if (/^-[bdfghinMRrsuVz]+$/.test(option) ||
            /^-(?:k|t).+/.test(option) ||
            /^--(?:dictionary-order|ignore-case|ignore-nonprinting|general-numeric-sort|human-numeric-sort|month-sort|numeric-sort|reverse|stable|unique|version-sort|zero-terminated|check(?:=\w+)?|key=.+|field-separator=.+)$/.test(option)) {
            return false;
        }
    ...
    if (executable === 'uniq') {
        if (/^-[cduiz]+$/.test(option) ||
            /^-[fsw]\d+$/.test(option) ||
            /^--(?:count|repeated|unique|ignore-case|zero-terminated|skip-fields=\d+|skip-chars=\d+|check-chars=\d+)$/.test(option)) {
            return false;
        }
        if (/^-[fsw]$/.test(option))
            return 'number';
...
export function hasHiddenPathSegment(value) {
    return value
        .replaceAll('\\', '/')
        .split('/')
