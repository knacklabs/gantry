export function parseAutonomousToolDenial(value) {
    if (!value)
        return null;
    const prefixMatch = value.match(/Tool not on autonomous (?:run|job) allowlist:\s*/i);
    ...
    const toolName = (sentenceBoundary >= 0 ? afterPrefix.slice(0, sentenceBoundary) : afterPrefix)
        .trim()
        .replace(/\.$/, '')
        .trim();
    if (!toolName)
...
function findToolRuleSentenceBoundary(value) {
