...
export function parseBashCommand(command) {
    ...
    if (!trimmed)
        return { ok: false, reason: 'Bash command is empty.' };
    if (trimmed.length > 4096) {
        return { ok: false, reason: 'Bash command is too long to parse safely.' };
    }
    return parseSegment(trimmed);
}
export function firstDestructiveRedirectTarget(command) {
    ...
    for (const leaf of parsed.leaves) {
        ...
        if (redirect)
            return `${redirect.operator} ${redirect.target}`;
    }
    return undefined;
}
export function destructiveBashCommandHint(command) {
    ...
    if (parsed.ok) {
        for (const leaf of parsed.leaves) {
            ...
            if (commandName === 'dd')
                return 'Writes raw disk data';
            if (commandName === 'mkfs' || commandName.startsWith('mkfs.')) {
                return 'Formats a filesystem';
            }
        }
    }
    if (/\b(?:DROP|DELETE|TRUNCATE|ALTER\s+TABLE|rm\s+-|mkfs(?:\.\w+)?|dd)\b/i.test(text)) {
        return 'Review the full command before approving';
    }
    return undefined;
}
export function bashLeafRuleContent(leaf) {
    return leaf.commandText;
}
export function normalizeBashLeafRuleContent(leaf) {
...
export function normalizePersistentBashRuleContent(ruleContent) {
    ...
    if (leaf.redirects.length > 0)
        return trimmed;
    return normalizeScriptLeafRuleContent(leaf) ?? trimmed;
}
export function nonDurableBashLeafReason(leaf) {
...
export function wildcardSensitiveBashLeafReason(leaf, scope) {
    const command = executableName(leaf.argv[0] ?? '');
    const scriptPath = pythonScriptPathForLeaf(leaf.argv);
    if (scriptPath && isReviewedSkillScriptPath(scriptPath)) {
        return undefined;
...
export function bashExecutableName(command) {
    return executableName(command);
}
export function formatBashArgv(argv) {
 ...
 */
export function summarizeBashCommandPrograms(command) {
    const parsed = parseBashCommand(command);
    ...
    for (let i = 0; i < command.length; i += 1) {
        ...
        if (quote) {
            ...
            if (quote === '"' && (ch === '`' || ch === '$')) {
                return {
                    ok: false,
                    reason: 'Bash command uses shell expansion that cannot be persisted safely.',
                };
            }
            token += ch;
            continue;
        ...
        if (ch === '(') {
            if (token.trim()) {
                return {
                    ok: false,
                    reason: 'Bash process substitution and function syntax are not supported.',
                };
            }
...
function parseRedirect(command, index, currentToken) {
...
function buildLeaf(argv, redirects) {
    if (argv.length === 0) {
        return {
            ok: false,
    ...
    if (metaReason)
        return { ok: false, reason: metaReason };
    return {
        ok: true,
        leaf: {
            argv,
...
function unsafeMetaExecutorReason(argv) {
...
function isDestructiveRedirect(operator, target) {
...
function findMatchingParen(command, openIndex) {
    let depth = 0;
    let quote = null;
    let escaped = false;
...
function normalizeBashArg(arg) {
    const url = normalizeUrlArg(arg);
    if (url)
        return url;
    if (isVolatileGitRef(arg))
        return '*';
    return arg;
}
...
function normalizeScriptLeafRuleContent(leaf) {
    const argv = leaf.argv;
    if (argv.length === 0)
        return undefined;
    const scriptArg = pythonScriptPathForLeaf(argv);
    if (!scriptArg || !isReviewedSkillScriptPath(scriptArg))
        return undefined;
    return `${normalizeBashArg(scriptArg)} *`;
}
function isPythonScriptPath(value) {
    return value.endsWith('.py');
}
function pythonScriptPathForLeaf(argv) {
    if (argv.length === 0)
        return undefined;
...
function isReviewedSkillScriptPath(value) {
