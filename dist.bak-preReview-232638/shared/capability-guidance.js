...
export const SOURCE_INVENTORY_AUTHORITY_GUIDANCE = 'Sources only provide inventory and setup metadata. Durable authority is the reviewed action capability granted to the agent.';
export const UNREVIEWED_DISCOVERY_GUIDANCE = 'CLI help, MCP tool lists, skill text, and adapter discovery can guide review, but the agent should use the reviewed action capability as the public contract.';
...
export function renderDefaultCapabilityRules(options) {
    ...
    if (options?.includeSettingsTools) {
        lines.push('- Agents with selected admin capabilities may use settings_desired_state before local configuration changes and request_settings_update for reviewed settings.yaml changes; do not edit settings directly.');
    }
    lines.push('- Agents with selected admin capabilities may use service_restart after approved changes and register_agent for conversation installs.', '- Never run dependency installs or edit local skill files, MCP config, settings, or generated runtime config directly.');
    return lines.join('\n');
}
