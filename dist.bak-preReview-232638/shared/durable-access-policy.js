import { isAdminMcpToolFullName, isAuthorityChangingGantryMcpToolFullName, isDecisionActorGantryMcpToolFullName, isDelegationDispatcherFullName, isDurableGrantExcludedDispatcherFullName, isDurableGantryMcpToolFullName, isGantryMcpWildcardRule, parseExactGantryMcpToolName, } from './admin-mcp-tools.js';
import { nonDurableBashLeafReason, parseBashCommand, wildcardSensitiveBashLeafReason, } from './bash-command-parser.js';
import { BROWSER_PROJECTED_MCP_RULE_REJECTION_REASON, isCanonicalBrowserCapabilityRule, isGantryFacadeExactToolRule, isProjectedBrowserMcpToolRule, parseReadableScopedToolRule, RUN_COMMAND_TOOL_NAME, validateReadableAgentToolRule, } from './agent-tool-references.js';
import { containsGeneratedRuntimePath, containsGeneratedRuntimeSkillPath, GENERATED_RUNTIME_SKILL_PATH_DURABLE_REJECTION_REASON, } from './generated-runtime-paths.js';
import { parseSemanticCapabilityRule } from './semantic-capability-ids.js';
import { classifySensitiveMemoryMaterial, detectPotentialUnredactedSecret, redactSensitiveText, } from './sensitive-material.js';
/**
 ...
 */
export const DURABLE_ACCESS_RULE_REJECTION_REASON = 'Persistent access approvals support only trusted projected semantic capabilities, canonical Browser, exact Gantry file/web tools, scoped RunCommand(...), or exact Gantry tools except unscoped dispatchers, delegation dispatchers, authority/config-changing tools, and decision actors.';
export const AUTHORITY_CHANGING_GANTRY_MCP_TOOL_REJECTION_REASON = 'Authority/config-changing Gantry tools cannot be granted persistent access because they can change what an agent is permitted to do, alter runtime configuration or topology, or restart the service; review each request explicitly.';
export const DECISION_ACTOR_GANTRY_MCP_TOOL_REJECTION_REASON = 'Gantry tools that record a durable user consent or review decision cannot be granted persistent access because the agent must not assert the human decision without per-request review.';
export const DURABLE_GRANT_EXCLUDED_DISPATCHER_REJECTION_REASON = 'Unscoped Gantry dispatchers cannot be granted persistent access because an exact grant on the dispatcher cannot constrain the arbitrary tool or command it dispatches to; use a scoped command or reviewed target-specific capability instead.';
export const DELEGATION_DISPATCHER_REJECTION_REASON = "Gantry delegation dispatchers cannot be granted persistent access because an exact grant cannot bound the tools or commands used by another agent's delegated work; review each delegation or steering request explicitly.";
export function validateDurableAccessRule(rule, options = {}) {
    ...
    if (isDurableGantryMcpToolFullName(trimmed))
        return { ok: true };
    if (scoped?.toolName === RUN_COMMAND_TOOL_NAME) {
        ...
        if (containsGeneratedRuntimePath(scoped.scope)) {
            return {
                ok: false,
                reason: 'Persistent RunCommand rules cannot reference generated runtime paths; use a reviewed stable capability or let Gantry-owned runtime scratch reads stay internal.',
            };
        }
    }
    const readableValidation = validateReadableAgentToolRule(trimmed);
    if (!readableValidation.ok)
        return readableValidation;
    if (isGantryFacadeExactToolRule(trimmed))
        return { ok: true };
    if (scoped) {
        if (scoped.toolName !== RUN_COMMAND_TOOL_NAME) {
            return {
    ...
    if (capabilityId) {
        ...
        if (!definition) {
            if (options.allowUnknownSemanticCapability)
                return { ok: true };
            return {
                ok: false,
    ...
    if (isCanonicalBrowserCapabilityRule(trimmed))
        return { ok: true };
...
export function isDurableAccessRuleAllowed(rule, options) {
    return validateDurableAccessRule(rule, options).ok;
}
export function formatDurableAccessRulesForUser(rules, options = {}) {
    return rules
        .map((rule) => formatDurableAccessRuleForUser(rule, options))
        .join(', ');
}
export function formatDurableAccessRuleForEvent(rule) {
    return formatDurableAccessRuleForUser(rule);
}
export function durableAccessRuleAuditPreview(rule) {
    return formatDurableAccessRuleForUser(rule);
}
function formatDurableAccessRuleForUser(rule, options = {}) {
    const trimmed = rule.trim();
    const scoped = parseReadableScopedToolRule(trimmed);
    if (scoped?.toolName === RUN_COMMAND_TOOL_NAME) {
