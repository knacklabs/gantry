...
const GANTRY_FACADE_EXACT_TOOL_NAME_SET = new Set(GANTRY_FACADE_EXACT_TOOL_NAMES);
const GANTRY_DELEGATION_TOOL_NAME_SET = new Set([
    'delegate_task',
    'task_message',
]);
export function canonicalGantryToolRuleName(toolName, context) {
    ...
    const canonicalToolName = gantryNamespaced
        ...
        : toolName;
    const manifestCallable = context?.callableAgentToolNames?.has(toolName) === true ||
...
const PROVIDER_NATIVE_TOOL_REPLACEMENTS = new Map([
    ['WebFetch', 'WebRead'],
    ['Glob', 'FileSearch'],
    ['Grep', 'FileSearch'],
    ['Read', 'FileRead'],
...
export const PROVIDER_NATIVE_TOOL_REJECTION_REASON = 'Provider-native SDK tools are execution-harness projections and cannot be persisted as Gantry tool rules; select a Gantry capability such as Browser, a Gantry file/web tool, a semantic capability, an exact Gantry admin MCP tool, or a scoped RunCommand(...) fallback.';
export const DEFAULT_GANTRY_HARNESS_TOOL_PROJECTION = {
    exactTools: {
        WebSearch: ['WebSearch'],
        WebRead: ['WebFetch'],
...
export const GANTRY_FACADE_INPUT_SCHEMAS = {
...
export function providerNativeToolReplacement(toolName) {
...
export function providerNativeToolRejectionReason(toolName) {
    return `${PROVIDER_NATIVE_TOOL_REJECTION_REASON}${providerNativeToolReplacement(toolName)}`;
}
export function isProviderNativeExactToolRule(value) {
    const rule = value.trim();
    if (parseReadableScopedToolRule(rule))
        return false;
    return PROVIDER_NATIVE_EXACT_TOOL_NAMES.has(rule);
}
export function isGantryFacadeExactToolRule(value) {
    return GANTRY_FACADE_EXACT_TOOL_NAME_SET.has(value.trim());
}
export function isGantryFacadeExactToolName(value) {
    return GANTRY_FACADE_EXACT_TOOL_NAME_SET.has(value);
}
export function isRunCommandToolRule(value) {
    const rule = value.trim();
...
export function publicGantryToolNameForSdkTool(toolName) {
    const normalized = toolName.trim();
    if (normalized === BASH_TOOL_NAME)
        return RUN_COMMAND_TOOL_NAME;
    return PROVIDER_NATIVE_TOOL_REPLACEMENTS.get(normalized) ?? normalized;
}
export function publicCapabilityAllowedToolRules(tools) {
    const rules = new Set();
    for (const tool of tools) {
        const publicName = publicGantryToolNameForSdkTool(tool);
        if (publicName === RUN_COMMAND_TOOL_NAME)
            continue;
        rules.add(publicName);
    }
    return [...rules];
}
export function sdkToolsForGantryFacadeTool(toolName) {
    return projectGantryToolRuleForHarness(toolName, DEFAULT_GANTRY_HARNESS_TOOL_PROJECTION);
}
export function projectGantryToolRuleForHarness(toolRule, projection) {
    ...
    if (!isGantryFacadeExactToolName(rule))
        return [];
    return [...(projection.exactTools[rule] ?? [])];
}
export function validateGantryFacadeToolInput(toolName, input) {
...
function parseReadableScopedToolRule(value) {
    const rule = value.trim();
...
function exactPathSchema(description) {
    return jsonSchema({
        type: 'object',
        additionalProperties: false,
        required: ['path'],
...
function validateFileSearchFacadeInput(record) {
    ...
    if (mode === 'content' && hasGlobSyntax(record.query)) {
        return {
            ok: false,
            reason: 'FileSearch content queries do not accept glob patterns; use include/exclude filters for globs.',
        };
    }
...
function validateFacadePathInput(record, toolName) {
...
function validateOptionalGlobFilter(value, fieldName) {
    if (value === undefined)
        return { ok: true };
...
function exactSafeRelativePathRejectionReason(value) {
    const path = value.trim();
    if (!path)
        return 'File path must be non-empty.';
    if (path.length > 4096)
        return 'File path is too long.';
    if (path.includes('\0'))
        return 'File path cannot contain NUL bytes.';
