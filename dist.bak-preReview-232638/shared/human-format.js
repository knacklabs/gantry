...
export function formatRunShortId(run) {
...
export function formatRunLabel(run) {
    ...
    if (startedAgo)
        parts.push(`started ${startedAgo} ago`);
...
function formatStartedAgo(startedAt, nowMs = Date.now()) {
    const timestamp = startedAt instanceof Date ? startedAt.getTime() : Date.parse(startedAt);
    if (!Number.isFinite(timestamp))
        return '';
    return formatDuration(Math.max(0, nowMs - timestamp));
}
function formatAttempt(run) {
    if (run.attempt &&
