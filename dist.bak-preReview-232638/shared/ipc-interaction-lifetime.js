...
const filesystemIpcRequestClaimProbe = (requestPath) => {
    const requestFile = path.basename(requestPath);
    return fs
        .readdirSync(path.dirname(requestPath))
        .some((file) => file.startsWith('.processing-') && file.endsWith(`-${requestFile}`));
};
export function hasIpcRequestClaimMarker(requestPath, probe = filesystemIpcRequestClaimProbe) {
    try {
...
export function ipcInteractionAuthEnvelopeOptions(unbounded) {
    return {
...
export function ipcInteractionAuthValidationOptions(permissionLane) {
...
export function ipcInteractionUnclaimableReason(kind) {
...
export function ipcQuestionWaitExpiredReason(permissionLane) {
    return permissionLane === 'interactive'
