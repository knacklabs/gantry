export function jobSetupBlockerFromUnknown(value) {
...
export function setupBlockerLabel(blocker, fallbackState) {
    if (!blocker)
        return humanizeIdentifier(fallbackState);
...
export function setupActionLabel(blocker) {
    if (!blocker)
        return 'Fix setup, then resume the job.';
    ...
    if (blocker.requirementType === 'credential' && nextAction)
        return nextAction;
    return setupActionLabelFromNextAction(nextAction);
}
export function setupActionLabelFromNextAction(nextAction, fallback = 'Fix setup, then resume the job.') {
...
function semanticCapabilityLabel(capabilityId) {
    return capabilityId
 ...
 */
export function setupReadinessLabel(state) {
    if (state === 'ready' || !state)
        return 'Ready';
    if (state === 'missing_capability')
        return 'Needs approval';
    if (state === 'credential_unknown' ||
...
function isLocalCliCapabilityAction(value) {
