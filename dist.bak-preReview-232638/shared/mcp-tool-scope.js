...
export function normalizeMcpToolScope(input) {
    ...
    if (input.definitionPatterns.length === 0) {
        throw new Error(`MCP tool scope cannot be narrowed for ${input.serverName} because the server definition has no reviewed tools.`);
    }
    for (const pattern of requested) {
        const covered = input.definitionPatterns.some((allowed) => allowed === pattern ||
            (allowed.endsWith('*') && pattern.startsWith(allowed.slice(0, -1))));
        if (!covered) {
            throw new Error(`MCP tool scope ${pattern} is not within the reviewed tools for ${input.serverName}.`);
        }
    }
    return requested;
}
export function intersectMcpToolRulesWithSourceScopes(toolRules, sources) {
    ...
    for (const source of sources) {
        ...
        for (const rule of toolRules) {
            ...
            for (const sourcePattern of source.allowedToolPatterns) {
                const intersection = intersectMcpToolPatterns(authorityPattern, sourcePattern);
                if (intersection)
                    intersections.add(`${prefix}${intersection}`);
            }
        }
    }
...
export function mcpToolNameAllowedBySourceScope(input) {
    ...
    if (!input.fullToolName.startsWith(prefix))
        return false;
    if (input.allowedToolPatterns.length === 0)
        return true;
    const toolName = input.fullToolName.slice(prefix.length);
    return input.allowedToolPatterns.some((pattern) => mcpToolPatternCovers(pattern, toolName));
}
export function mcpToolPatternCovers(pattern, candidate) {
    return (pattern === candidate ||
        pattern === '*' ||
        (pattern.endsWith('*') && candidate.startsWith(pattern.slice(0, -1))));
}
function intersectMcpToolPatterns(left, right) {
    if (mcpToolPatternCovers(left, right))
        return right;
    if (mcpToolPatternCovers(right, left))
        return left;
    return undefined;
}
