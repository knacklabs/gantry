...
export function composeSystemPromptAppend(compiledPrompt, hasMemoryContext) {
    const parts = [
...
function stringifyForPolicy(value) {
...
export function denyMemoryBoundaryToolUse(toolName, input, permissionOpts, memoryBlock, 
...
isThirdPartyMcpTool = false) {
    ...
    const haystack = [
        ...
        .filter(Boolean)
        .join('\n');
    const highRiskPattern = /\b(rm\s+-rf|sudo|curl\b.{0,80}\|\s*(sh|bash)|wget\b.{0,80}\|\s*(sh|bash)|chmod\s+\+x|api[_ -]?key|bearer\s+token|secret|credential|exfiltrat|system\s+prompt|developer\s+message|ignore\s+(previous|all)|override\s+(instruction|policy)|disregard\s+(instruction|policy))\b/i;
    if (!highRiskPattern.test(haystack))
        return null;
