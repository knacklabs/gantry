...
export function resolveModelCacheProvider(entry) {
    if (!entry || entry.cacheMode === 'none')
        return 'none';
...
export function resolveModelCacheSupport(entry) {
    const provider = getModelProviderDefinition(entry.modelRoute.id);
    if (!provider) {
    ...
    const promptSupported = cacheProvider === 'anthropic' ||
        cacheProvider === 'openai' ||
        cacheProvider === 'openrouter-provider';
    const responseAvailable = provider.cacheSupport.response.mode !== 'none';
    const responseSuffix = responseAvailable && !provider.cacheSupport.response.enabledByDefault
