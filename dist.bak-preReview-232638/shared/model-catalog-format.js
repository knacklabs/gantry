...
import { recommendModelAlias, } from './model-recommendation.js';
import { resolveModelCacheSupport } from './model-cache-support.js';
import { listModelFamilies, } from './model-families.js';
import { availabilityBadgeForProvider, describeFamilyAvailability, familyAvailabilityBadge, } from './model-catalog-availability.js';
...
// precision.
export function formatCostPerMillion(entry) {
...
export function formatModelCatalog(options = {}) {
    ...
    if (options.recommendation) {
        const recommendation = recommendModelAlias({
            ...
            configuredProviders: options.recommendation.configuredProviders ?? configuredProviders,
        });
        if (recommendation) {
            lines.unshift(`Recommended model: ${recommendation.alias}. Why: ${recommendation.reason}.`, '');
        }
    }
    for (const entry of listModelCatalogEntries()) {
        ...
        const availability = availabilityBadgeForProvider(entry.modelRoute.id, configuredProviders);
        for (const alias of entry.aliases) {
            const badges = [];
            if (alias === entry.recommendedAlias)
                badges.push('recommended');
            else
                badges.push('pinned');
            if (defaults.chat === alias)
                badges.push('chat default');
            ...
            if (defaults.memoryExtractor === alias)
                badges.push('memory extractor');
            if (defaults.memoryDreaming === alias)
                badges.push('memory dreaming');
    ...
    if (families.length) {
        ...
        lines.push('', 'Model families (provider auto-selected by configured key)', familyHeader, familyHeader.replace(/[^|]+/g, '---'));
