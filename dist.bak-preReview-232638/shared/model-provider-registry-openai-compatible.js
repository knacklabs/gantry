...
function deepAgentsExecutionRoute(supportedCredentialModes = ['api_key']) {
    return {
...
function automaticPrefixCache(readTokensField, options = {}) {
    return {
        prompt: {
...
const BEDROCK_CREDENTIAL_MODES = [
    {
        ...
        helpText: 'Use the host AWS credential chain for SigV4 Bedrock Chat Completions. In production, prefer an ECS task role, EC2 instance profile, EKS IRSA, or assumed role.',
        version: 1,
        fields: [
            {
                name: 'region',
    ...
    },
    {
        ...
        helpText: 'Resolve an Amazon Bedrock API key from AWS Secrets Manager at gateway time.',
        version: 1,
        fields: [
            {
                name: 'region',
    ...
    },
    {
        ...
        helpText: 'Use an Amazon Bedrock API key for OpenAI-compatible chat completions.',
        version: 1,
        fields: [
            {
                name: 'region',
...
const VERTEX_CREDENTIAL_MODES = [
    {
        ...
        helpText: 'Use Google Application Default Credentials to mint a host-side OAuth token for Vertex AI.',
        version: 1,
        fields: [
            {
                name: 'region',
    ...
    },
    {
        ...
        helpText: 'Resolve a Google service account JSON key from Google Secret Manager at gateway time.',
        version: 1,
        fields: [
            {
                name: 'region',
    ...
    },
    {
        ...
        helpText: 'Use a Google Cloud service account JSON key for OpenAI-compatible chat completions.',
        version: 1,
        fields: [
            {
                name: 'region',
...
function resolveBedrockUpstream(input) {
    if (!['aws_default_chain', 'bedrock_api_key_ref', 'bedrock_api_key'].includes(input.authMode)) {
        throw new Error(`Unsupported Amazon Bedrock credential mode ${input.authMode}.`);
    }
    const region = requirePattern(requireField(input.payload, 'region', 'AWS region'), AWS_REGION_PATTERN, 'AWS region');
    return {
        origin: `https://bedrock-mantle.${region}.api.aws`,
        pathPrefix: BEDROCK_CHAT_PATH_PREFIX,
    };
}
function resolveVertexUpstream(input) {
    const region = requirePattern(requireField(input.payload, 'region', 'Google Cloud location'), GOOGLE_VERTEX_LOCATION_PATTERN, 'Google Cloud location');
    const projectId = requirePattern(requireField(input.payload, 'projectId', 'Google Cloud project ID'), GOOGLE_PROJECT_PATTERN, 'Google Cloud project ID');
    return {
...
function openAiCompatibleSdkProjection(providerId) {
    return {
        baseUrlEnv: 'OPENAI_BASE_URL',
        tokenEnv: 'OPENAI_API_KEY',
        credentialProviderEnvKey: 'OPENAI_API_KEY',
