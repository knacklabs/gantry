...
import { getModelFamily } from './model-families.js';
import { describeFamilyAvailability, providerLabel, } from './model-catalog-availability.js';
...
export function formatModelWhy(input) {
    ...
    if (family) {
        ...
        if (configuredProviders === undefined) {
            lines.push('resolves to: the first member whose provider key is configured (configured set unavailable here).');
            return lines.join('\n');
        }
        if (description.selectedConfigured) {
            const skipped = description.members
                .filter((entry) => entry.member !== description.selectedMember && !entry.configured)
                .map((entry) => entry.providerLabel);
            const reason = skipped.length
                ? ` (${skipped.join(', ')} not configured)`
                : '';
            lines.push(`resolves to: ${family.alias} → ${description.selectedMember} via ${description.selectedProviderLabel}${reason}`);
        }
        else {
            const providers = [
                ...new Set(description.members.map((entry) => entry.providerLabel)),
            ];
            lines.push(`resolves to: ${family.alias} → ${description.selectedMember} via ${description.selectedProviderLabel} (no provider configured; falls back to the first member and fails loudly with its setup message)`, `needs a key for one of: ${providers.join(', ')}`);
        }
        return lines.join('\n');
    }
    ...
    if (configuredProviders !== undefined) {
        lines.push(configuredProviders.has(providerId)
...
export function resolvedProviderIdForWhy(input) {
    ...
    if (family) {
        return describeFamilyAvailability(family, input.configuredProviders, input.familyOrder).selectedProviderId;
    }
