 ...
 */
export function parseDeclaredNetworkHost(value) {
    const trimmed = value.trim().toLowerCase();
    if (!trimmed)
        return fail('entries cannot be empty.');
    if (/\s/.test(trimmed))
        return fail('entries cannot contain whitespace.');
    if (trimmed.includes('://') ||
        trimmed.includes('@') ||
        trimmed.includes('/') ||
        trimmed.includes('?') ||
        trimmed.includes('#')) {
        return fail('must be host or host:port values, not URLs, schemes, or paths.');
    }
    if (trimmed.includes('*'))
        return fail('cannot use wildcards.');
 ...
 */
export function declaredNetworkAuthority(value) {
    const trimmed = value.trim().toLowerCase();
    if (!trimmed)
        return undefined;
    const split = splitHostPort(trimmed);
...
function splitHostPort(value) {
    if (value.startsWith('[')) {
        ...
        if (end === -1)
            return fail('bracketed IPv6 hosts must close the bracket.');
...
export function hostnameForNetwork(input) {
...
function parseIpv6Bytes(address) {
    ...
    if (left.some((part) => !Number.isInteger(part)) ||
...
function isValidHostnameLabels(host) {
