...
export const SURFACEABLE_STATUSES = [
    'detected',
    'suggested',
];
export const PATTERN_SUGGESTED_FOLLOWUP_HOURS = 24;
export function isSurfaceable(status) {
    return SURFACEABLE_STATUSES.includes(status);
}
export function snoozeUntil(nowIso) {
    return new Date(new Date(nowIso).getTime() + PATTERN_SNOOZE_DAYS * DAY_MS).toISOString();
}
export function meetsRecurrenceValueFloor(input) {
    const spanDays = Math.floor((new Date(input.windowEnd).getTime() -
        new Date(input.windowStart).getTime()) /
        DAY_MS);
    return (input.occurrences >= PATTERN_VALUE_FLOOR_MIN_OCCURRENCES &&
 ...
 */
export function candidateStatusForChoice(choice) {
    switch (choice) {
        case 'create_draft':
            return 'accepted';
        case 'not_now':
...
export function shouldResetSnooze(input) {
    if (input.status !== 'snoozed')
        return false;
    const elapsed = input.snoozedUntil !== null &&
        new Date(input.snoozedUntil).getTime() <= new Date(input.nowIso).getTime();
    const intensified = input.newOccurrences - input.previousOccurrences >= PATTERN_INTENSIFY_DELTA;
