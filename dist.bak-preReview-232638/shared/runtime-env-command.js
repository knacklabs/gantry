...
const LOOPBACK_PROXY_URL_RE = /^https?:\/\/(?:127\.0\.0\.1|localhost|\[::1\])(?::(\d+))?\/?$/;
...
export function stripRuntimeEnvPrefix(command) {
    const parsed = splitRuntimeEnvAssignments(command, ({ key }) => RUNTIME_ENV_ASSIGNMENT_KEYS.has(key));
...
export function stripHostInjectedEnvPrefix(command) {
...
function isHostInjectedAssignment({ key, shellWordComplete, value, }) {
    if (!shellWordComplete)
        return false;
    if (key === 'GODEBUG')
        return value === 'netdns=go';
    if (HOST_PROXY_ENV_KEYS.has(key))
        return isLoopbackProxyUrl(value);
    if (HOST_NO_PROXY_ENV_KEYS.has(key)) {
        const tokens = value.split(',').map((token) => token.trim());
        return (tokens.length > 0 &&
            tokens.every((token) => token.length > 0 && LOOPBACK_NO_PROXY_TOKENS.has(token)));
    }
    if (key === 'NODE_USE_ENV_PROXY')
        return value === '1';
    if (!NEUTRAL_CA_TRUST_ENV_KEY_SET.has(key))
        return false;
    const trustedValues = [process.env.NODE_EXTRA_CA_CERTS, process.env[key]]
        .map((trustedValue) => trustedValue?.trim())
        .filter((trustedValue) => Boolean(trustedValue));
...
function splitRuntimeEnvAssignments(command, shouldStrip) {
    let offset = 0;
    const assignments = [];
    while (offset < command.length) {
        const next = parseRuntimeEnvAssignment(command, offset);
        if (!next || !shouldStrip(next))
            break;
        assignments.push(command.slice(skipSpaces(command, offset), next.nextOffset).trim());
...
function parseRuntimeEnvAssignment(command, offset) {
    ...
    if (!parsedValue || parsedValue.nextOffset === cursor)
        return null;
    return {
        key: keyMatch[1],
        shellWordComplete: parsedValue.nextOffset === command.length ||
            /[\s;|&<>()]/.test(command[parsedValue.nextOffset] ?? ''),
        value: parsedValue.value,
        nextOffset: skipSpaces(command, parsedValue.nextOffset),
    };
}
function parseShellAssignmentValue(command, offset) {
...
function skipSpaces(command, offset) {
    let cursor = offset;
