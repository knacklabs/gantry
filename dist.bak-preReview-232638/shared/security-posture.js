...
export function isLocalControlHost(host) {
    const normalized = (host || '127.0.0.1').trim().toLowerCase();
    return LOCAL_CONTROL_HOSTS.has(normalized);
}
export function resolveRuntimeSecurityPosture(env) {
    const securityPosture = env.GANTRY_SECURITY_POSTURE?.trim().toLowerCase() || '';
    ...
    const remoteDeployment = securityPosture === 'remote' || runtimeEnv === 'remote';
    const port = Number(env.GANTRY_CONTROL_PORT?.trim() || 0);
    const remoteControl = port > 0 && !isLocalControlHost(env.GANTRY_CONTROL_HOST);
    return {
        production,
        remoteControl,
...
export function validateProductionSecurityGate(input) {
    ...
    if (!posture.requiresProductionSecrets)
        return [];
    const failures = [];
    if (posture.requiresEnforcingSandbox &&
        input.sandboxProvider !== 'sandbox_runtime') {
        failures.push('runtime.sandbox.provider must be sandbox_runtime in production or remote control mode.');
    }
    if (!hasValidEncryptionSecret(env)) {
        failures.push('SECRET_ENCRYPTION_KEY or SECRET_ENCRYPTION_KEYRING_JSON must provide a strong base64-encoded 32-byte active key.');
    }
    if (!env.GANTRY_IPC_AUTH_SECRET?.trim()) {
        ...
        failures.push('GANTRY_IPC_AUTH_SECRET must be at least 32 characters of non-trivial secret material in production or remote control mode.');
    }
    if (isEnabledEnvFlag(env.REMOTE_CONTROL_AUTO_ACCEPT)) {
        failures.push('REMOTE_CONTROL_AUTO_ACCEPT is local-development-only and cannot be enabled in production or remote control mode.');
    }
    try {
        ...
        if (posture.requiresProductionSecrets && keys.length === 0) {
            failures.push('GANTRY_CONTROL_API_KEYS_JSON must include at least one valid control API key.');
        }
    }
    catch (error) {
    ...
    try {
        ...
        for (const [keyId, value] of Object.entries(parsed.keys)) {
            if (!keyId.trim() || typeof value !== 'string')
                return false;
            if (!isStrongBase64Encoded32ByteSecret(value))
                return false;
            if (keyId === active)
                activeFound = true;
        }
...
function isStrongBase64Encoded32ByteSecret(value) {
    try {
