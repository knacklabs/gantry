...
import { hasBashShellControlSyntax, isHostPrivateBrowserMcpServerName, RUN_COMMAND_TOOL_NAME, validateReadableAgentToolRule, } from './agent-tool-references.js';
import { parseDeclaredNetworkHost } from './network-host-declaration.js';
import { isValidSemanticCapabilityId, parseSemanticCapabilityRule, semanticCapabilityIdValidationReason, semanticCapabilityRule, } from './semantic-capability-ids.js';
...
const LOCAL_CLI_MISSING_BINDING_REASON = 'Local CLI capabilities require a local_cli implementation binding.';
const LOCAL_CLI_BINDING_SOURCE_REASON = 'Local CLI bindings require credentialSource local_cli.';
const NETWORK_HOSTS_SOURCE_REASON = 'networkHosts are only supported for local_cli or skill action capabilities.';
const LOCAL_CLI_PROTECTED_PATHS_REASON = 'protectedPaths are only supported for local_cli capabilities.';
...
const NEUTRAL_CA_TRUST_ENV_KEY_SET = new Set(NEUTRAL_CA_TRUST_ENV_KEYS);
export function semanticCapabilityInputSchema(capability) {
    return {
        format: SEMANTIC_CAPABILITY_SCHEMA_FORMAT,
        schema: cloneCapabilityDefinition(capability),
    };
}
export function parseSemanticCapabilityDefinitionsRecord(raw) {
...
export function semanticCapabilityRuntimeRules(capability) {
    const rules = capability.implementationBindings.flatMap((binding) => {
        ...
        if (binding.kind === 'local_cli' &&
            ...
            return (binding.commandTemplates ?? []).map((template) => `${RUN_COMMAND_TOOL_NAME}(${template.trim()})`);
        }
        return [];
    });
...
export function expandSemanticCapabilityPermissionRules(input) {
    ...
    for (const rule of input.rules) {
        ...
        for (const runtimeRule of semanticCapabilityRuntimeRules(definition)) {
            if (runtimeRule.trim())
                out.add(runtimeRule.trim());
        }
    }
    return [...out];
}
export function validateSemanticCapabilityDefinition(capability) {
    const idReason = semanticCapabilityIdValidationReason(capability.capabilityId);
    if (idReason)
        return { ok: false, reason: idReason };
    ...
    const hasLocalCliBinding = capability.implementationBindings.some((binding) => binding.kind === 'local_cli');
    const hasNetworkHosts = (capability.networkHosts ?? []).some((host) => host.trim());
    ...
    for (const protectedPath of capability.protectedPaths ?? []) {
        ...
        const validPathHint = path.isAbsolute(trimmedPath) ||
            /^(~|\$\{[A-Za-z_][A-Za-z0-9_]*\}|\$[A-Za-z_][A-Za-z0-9_]*|%[A-Za-z_][A-Za-z0-9_]*%)(?:[/\\]|$)/.test(trimmedPath);
        if (!validPathHint) {
            return {
                ok: false,
                reason: 'Protected paths must be absolute, home-relative, or env-rooted paths.',
            };
        }
    }
    return { ok: true };
}
function isSkillActionCapability(capability) {
    const source = capability.source;
    return (Boolean(source) &&
        typeof source === 'object' &&
        !Array.isArray(source) &&
        source.kind === 'skill_action');
}
...
export function capabilityDisplayNameForRule(rule) {
    const capabilityId = parseSemanticCapabilityRule(rule);
    if (!capabilityId)
        return undefined;
    return skillActionCapabilityDisplayName(capabilityId);
}
export function skillActionCapabilityDisplayName(capabilityId) {
...
export function mcpPatternBindingRuntimeRules(binding) {
    if (binding.kind !== 'mcp_pattern')
        return [];
...
function validateMcpPatternBinding(binding) {
    const server = binding.mcpServer?.trim() ?? '';
    ...
    if (patterns.length === 0) {
        return {
            ok: false,
            reason: 'mcp_pattern bindings require at least one reviewed tool pattern.',
        };
    }
...
function validateSemanticCapabilityBinding(binding) {
    ...
    if (binding.kind === 'mcp_tool') {
        return {
            ok: false,
            reason: 'mcp_tool bindings are no longer supported; use an exact mcp_pattern binding.',
        };
    }
    if (binding.kind === 'mcp_pattern') {
...
function validateLocalCliBinding(binding) {
    ...
    for (const commandTemplate of templates) {
        const validation = validateLocalCliCommandTemplate(executablePath, commandTemplate);
        if (!validation.ok)
            return validation;
    }
    if (binding.authPreflightCommand) {
        const validation = validateLocalCliCommandTemplate(executablePath, binding.authPreflightCommand, { allowWildcard: false });
        if (!validation.ok)
            return validation;
    }
    return { ok: true };
}
function validateLocalCliCommandTemplate(executablePath, commandTemplate, options = { allowWildcard: true }) {
    ...
    if (hasBashShellControlSyntax(trimmed)) {
        return {
            ok: false,
            reason: 'Local CLI command templates cannot contain shell control syntax.',
        };
    }
    if (hasShellRedirectionSyntax(trimmed)) {
        return {
            ok: false,
            reason: 'Local CLI command templates cannot contain shell redirection syntax.',
        };
    }
    if (!trimmed.startsWith(`${executablePath} `)) {
        return {
            ok: false,
            reason: 'Local CLI command templates must start with the pinned executable path.',
        };
    }
    if (containsDeniedEnvAssignment(trimmed)) {
        return {
            ok: false,
            reason: 'Local CLI command templates cannot override token, credential, proxy, config, or CA environment keys.',
        };
    }
    ...
    if (args.length === 0 || args[0] === '*') {
        return {
            ok: false,
            reason: 'Local CLI command templates must scope a concrete subcommand, not the whole executable.',
        };
    }
...
function hasShellRedirectionSyntax(value) {
    let quote = null;
    let escaped = false;
...
function containsDeniedEnvAssignment(command) {
...
function parseSemanticCapabilitySchema(inputSchema) {
    if (!inputSchema || typeof inputSchema !== 'object')
        return undefined;
    const record = inputSchema;
    if (record.format !== SEMANTIC_CAPABILITY_SCHEMA_FORMAT)
        return undefined;
...
function humanizeCapabilityWord(word, index) {
    if (index > 0)
        return word;
