...
export function semanticCapabilityRule(capabilityId) {
    return `${SEMANTIC_CAPABILITY_RULE_PREFIX}${capabilityId.trim()}`;
}
export function parseSemanticCapabilityRule(value) {
    const trimmed = value.trim();
    if (!trimmed.startsWith(SEMANTIC_CAPABILITY_RULE_PREFIX))
        return undefined;
...
export function isSemanticCapabilityRule(value) {
    return parseSemanticCapabilityRule(value) !== undefined;
}
export function isValidSemanticCapabilityId(value) {
    const trimmed = value.trim();
    return SEMANTIC_CAPABILITY_ID_RE.test(trimmed);
}
export function semanticCapabilityIdValidationReason(capabilityId) {
    const trimmed = capabilityId.trim();
    if (isValidSemanticCapabilityId(trimmed))
        return undefined;
