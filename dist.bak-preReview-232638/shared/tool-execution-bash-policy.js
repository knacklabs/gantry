import { firstProtectedPathMention, hasProtectedPathReference, PROVIDER_CLI_NAME, } from './tool-execution-protected-paths.js';
...
const BASH_REDIRECT_GLOBAL_PATTERN = /(^|[^<])>>?\s*("[^"]+"|'[^']+'|[^\s;&|]+)/g;
const BASH_DD_OF_PATTERN = /\bof=("[^"]+"|'[^']+'|[^\s;&|]+)/i;
const BASH_DD_OF_GLOBAL_PATTERN = /\bof=("[^"]+"|'[^']+'|[^\s;&|]+)/gi;
const BASH_OUTPUT_OPTION_PATTERN = /\b(?:--output|-o)\s+("[^"]+"|'[^']+'|[^\s;&|]+)/i;
const BASH_OUTPUT_OPTION_GLOBAL_PATTERN = /\b(?:--output|-o)\s+("[^"]+"|'[^']+'|[^\s;&|]+)/gi;
const GH_TEXT_PAYLOAD_COMMAND_PATTERN = /\bgh\s+(?:issue|pr)\s+(?:create|edit|comment)\b/i;
const GH_TEXT_PAYLOAD_FILE_OPTIONS = new Set([
    '--body-file',
    '--template',
    '-F',
]);
const PROVIDER_MCP_MUTATION_PATTERN = new RegExp(`\\b${PROVIDER_CLI_NAME}\\s+mcp\\s+(add|add-json|remove|reset-project-choices|reset)\\b`, 'i');
const GH_TOKEN_PATTERN = /"[^"]*"|'[^']*'|`[^`]*`|\$\([^)]*\)|[^\s]+/g;
export function commandText(input) {
    return stringField(input, 'command') ?? stringField(input, 'cmd');
}
export function isProviderMcpMutationCommand(command) {
    return PROVIDER_MCP_MUTATION_PATTERN.test(command);
}
export function hasBashMutationVerb(command) {
    return BASH_MUTATION_VERB_PATTERN.test(command);
}
export function hasBashRedirect(command) {
    return BASH_REDIRECT_PATTERN.test(command);
}
export function inferBashTarget(command) {
    if (PROVIDER_MCP_MUTATION_PATTERN.test(command))
        return 'provider-mcp-config';
...
export function inferBashMutationTargets(command) {
    return [
...
export function isSafeProtectedPathTextPayloadCommand(command) {
    if (!GH_TEXT_PAYLOAD_COMMAND_PATTERN.test(command))
        return false;
    return (!/[;&|]/.test(command) &&
        !/[\r\n]/.test(command) &&
...
export function hasProtectedPathInGhTextPayloadCommand(command) {
    if (!GH_TEXT_PAYLOAD_COMMAND_PATTERN.test(command))
        return false;
    return (hasProtectedPathPayloadArgument(command) ||
        hasProtectedPathCommandSubstitution(command) ||
        hasProtectedPathPositionalArgument(command));
}
function inferBashRedirectTarget(command) {
    const match = BASH_REDIRECT_PATTERN.exec(command);
    if (!match)
        return undefined;
    return unquote(match[2]);
}
function inferBashDdOutputTarget(command) {
    const match = BASH_DD_OF_PATTERN.exec(command);
    if (!match)
        return undefined;
    return unquote(match[1]);
}
function inferBashOutputOptionTarget(command) {
    const match = BASH_OUTPUT_OPTION_PATTERN.exec(command);
    if (!match)
        return undefined;
    return unquote(match[1]);
}
function allMatches(command, pattern, group) {
    pattern.lastIndex = 0;
    const matches = [...command.matchAll(pattern)].flatMap((match) => match[group] ? [match[group]] : []);
    pattern.lastIndex = 0;
    return matches;
}
...
function isCleanPathlikeTarget(target) {
    return (!/[(){}<>`]/.test(target) &&
        !/^\$[\w{]/.test(target) &&
        !target.includes('$('));
}
function hasProtectedPathPayloadArgument(command) {
    ...
    for (let index = 0; index < tokens.length; index++) {
        ...
        if (!option)
            continue;
        if (!GH_TEXT_PAYLOAD_FILE_OPTIONS.has(option.name))
            continue;
...
function hasProtectedPathPositionalArgument(command) {
    const tokens = command.match(GH_TOKEN_PATTERN);
...
function hasProtectedPathCommandSubstitution(command) {
...
function ghOptionConsumesNextToken(optionName) {
    return (['--body', '--title', '--field'].includes(optionName) ||
