...
const NEUTRAL_CA_RUNTIME_VALUE_RE = /^(?:\$NODE_EXTRA_CA_CERTS|\$\{NODE_EXTRA_CA_CERTS\}|"[$]NODE_EXTRA_CA_CERTS"|"[$]\{NODE_EXTRA_CA_CERTS\}"|'[$]NODE_EXTRA_CA_CERTS'|'[$]\{NODE_EXTRA_CA_CERTS\}')\s+/;
...
export function normalizeRuntimeOwnedBashCommandForMatching(command) {
    let normalized = canonicalizeGeneratedRuntimeSkillPaths(command
        .trim()
        .replace(/(["']?)\$\{CLAUDE_PROJECT_DIR\}\/skills\//g, (_match, quote) => `${quote}skills/`)
        .replace(/(["']?)\$CLAUDE_PROJECT_DIR\/skills\//g, (_match, quote) => `${quote}skills/`));
...
function stripRuntimeNetworkAssignment(command, sawRuntimePrefix) {
    ...
    if (!assignment.quoted || assignment.hasShellExpansion)
        return command;
    return command.slice(assignment.end);
}
export function leadingAssignmentKeys(command) {
    const keys = [];
...
function readLeadingAssignment(command) {
