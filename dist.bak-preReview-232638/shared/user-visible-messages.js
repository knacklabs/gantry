export function formatAvailableNowMessage(input) {
    ...
    const secretLines = formatDeclaredGantrySecretLines(input.requiredEnvVars ?? [], `This ${input.noun}`);
    return secretLines.length > 0 ? [ready, ...secretLines].join('\n') : ready;
}
export function formatDeclaredGantrySecretLines(names, subject = 'This capability') {
...
export function formatMissingGantrySecretsMessage(names) {
    const unique = uniqueNames(names);
...
export function formatApprovalRequestedMessage(displayName) {
...
export function humanizeTechnicalIdentifier(value) {
