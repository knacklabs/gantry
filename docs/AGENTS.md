...
- Capability docs must keep the semantic model primary: users approve `capability:<id>` records such as `google.sheets.write`, while raw request ids, command hashes, scoped Bash rules, sandbox profiles, executable paths, and provider implementation details stay in Details/audit sections.
