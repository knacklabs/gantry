...
### 4. Kubernetes image garbage collection deletes myclaw-agent image

**Symptoms**: `Container exited with code 125: pull access denied for myclaw-agent` — the container image disappears overnight or after a few hours, even though you just built it.

**Cause**: If your container runtime has Kubernetes enabled (Rancher Desktop enables it by default), the kubelet runs image garbage collection when disk usage exceeds 85%. MyClaw containers are ephemeral (run and exit), so `myclaw-agent:latest` is never protected by a running container. The kubelet sees it as unused and deletes it — often overnight when no messages are being processed. Other images (docker-compose services) survive because they have long-running containers referencing them.
...
```bash
grep -i "myclaw" ~/Library/Logs/rancher-desktop/k3s.log
# Look for: "Removing image to free bytes" with the myclaw-agent image ID
```

Check MyClaw logs for image status:
```bash
grep -E "image found|image NOT found|image missing" logs/myclaw.log
```
...
# Expected: PID  0  com.myclaw (PID = running, "-" = not running, non-zero exit = crashed)

# 2. Any running containers?
docker ps --format '{{.Names}} {{.Status}}' 2>/dev/null | grep myclaw

# 3. Any stopped/orphaned containers?
docker ps -a --format '{{.Names}} {{.Status}}' 2>/dev/null | grep myclaw

# 4. Recent errors in service log?
grep -E 'ERROR|WARN' logs/myclaw.log | tail -20

# 5. Are channels connected? (look for last connection event)
grep -E 'Connected|Connection closed|connection.*close|channel.*ready' logs/myclaw.log | tail -5
...
grep -E 'Container timeout|timed out' logs/myclaw.log | tail -10
...
grep 'New messages' logs/myclaw.log | tail -10

# Check if messages are being processed (container spawned)
grep -E 'Processing messages|Spawning container' logs/myclaw.log | tail -10

# Check if messages are being piped to active container
grep -E 'Piped messages|sendMessage' logs/myclaw.log | tail -10

# Check the queue state — any active containers?
grep -E 'Starting container|Container active|concurrency limit' logs/myclaw.log | tail -10
...
grep -E 'Mount validated|Mount.*REJECTED|mount' logs/myclaw.log | tail -10

# Verify the mount allowlist is readable
cat ~/.config/myclaw/mount-allowlist.json
...
docker run -i --rm --entrypoint ls myclaw-agent:latest /workspace/extra/
```
...
grep 'QR\|authentication required\|qr' logs/myclaw.log | tail -5

# Check auth files exist
ls -la store/auth/
```

If auth state is missing or expired, rerun the guided setup flow in Codex or Claude Code with `/setup`.
