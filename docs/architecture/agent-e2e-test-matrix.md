...
| One jid = ONE conversation row: a conversation created without a provider account (sessions path) converges with later admission (was: forked legacy+qualified rows, session split from replies) | integration      | ✅ live-admission-work-items.postgres.integration.test.ts (convergence, mutation-verified)                             |
