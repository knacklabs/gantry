...
| 7 | Agent-sender classification is not implemented. Slack/Discord drop bot/self messages, but Teams has no live bot gate; the current participant persistence can auto-create a human Person before runtime identity/memory checks. | major | `apps/core/src/channels/slack/channel-message-ingest.ts`, `apps/core/src/channels/discord/index.ts`, `apps/core/src/channels/teams.ts` | Resolve normalized agent aliases before participant/message persistence; unify with existing bot/self filters and add Slack, Teams, and Discord collision/no-run tests. |
