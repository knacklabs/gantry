...
Router: [router.tsx](apps/web/src/app/router.tsx:15). The protected root loads `/ui/api/auth/session` and accepts only `administrator|viewer` into route context: [root-route.tsx](apps/web/src/app/root-route.tsx:17).
...
All browser routes are `no-store`, reject Bearer credentials, and are only mounted on the full Control profile: [index.ts](apps/core/src/control/server/index.ts:191). The common mutation guard is exact canonical Origin plus synchronizer CSRF: [browser-auth.ts](apps/core/src/control/server/routes/browser-auth.ts:151); browser scope classification is exhaustive in [browser-scope-policy.ts](apps/core/src/control/server/browser-scope-policy.ts:16).

Decision 0132 requires opaque revocable sessions, no browser-held API/OIDC/secret credentials, Bearer rejection, exact-Origin+CSRF mutations, and default-deny scope classification. Decision 0135 applies that rule to the narrow provider facade: direct service calls, Viewer reads/Admin writes, recent reauth, audit, and redacted safe responses—never a `/v1` proxy or secret payload. [0132](docs/decisions/0132-adaptive-browser-authentication-access.md:23) [0135](docs/decisions/0135-browser-model-provider-credential-facade.md:21)

Design system: shadcn/Radix-style local primitives + compositions, Tailwind v4, Lucide, Spline fonts, light/dark CSS tokens: [styles.css](apps/web/src/styles.css:3). Reuse `PageHeader`, `Panel`, `DataTable`, `TextField`/`SelectField`, `Dialog`/`AlertDialog`, and `PageState`; there is no toast system (use inline `role="alert"`/live receipts).

Sidebar order: Operations → Configure → Administration → Conversations → Runtime → Workflows → Account, exactly as [app-navigation.tsx](apps/web/src/app/app-navigation.tsx:26).

- Directory: `/people` + detail tabs—filters, URL state, table, empty state: [people-route.tsx](apps/web/src/features/people/routes/people-route.tsx:22).
...
3. Mount it before static UI in [index.ts](apps/core/src/control/server/index.ts:191), including Bearer rejection/no-store.
4. Read: `activeSession` + `browserRoleAllowsScope`; mutation: shared Origin/CSRF guard plus hosted recent-reauth.
5. Classify its Control scope in [browser-scope-policy.ts](apps/core/src/control/server/browser-scope-policy.ts:16) and add unit boundary tests under `apps/core/test/unit/auth/browser-*-routes.test.ts`.
...
Testing: web uses Vitest only (`pnpm --filter @gantry/web test`), with static/source-contract tests rather than Playwright or browser-interaction tests: [vitest.config.ts](apps/web/vitest.config.ts:5). For a `user_facing: true` story, implementer tests + verify + three reviews are required; then functional-checker validates visible/end-to-end behavior and records a 0–10 artifact, requiring score ≥8 and acceptance evidence: [QUALITY.md](docs/QUALITY.md:41), [tester-functional.md](factory/prompts/tester-functional.md:1).
...
|---|---|---|
| Agent | `agents` + `GET /v1/agents` ([schema](</Users/dev/Workdir/myclaw/apps/core/src/adapters/storage/postgres/schema/agents.ts:28), [route](</Users/dev/Workdir/myclaw/apps/core/src/control/server/routes/agents.ts:130)) | Exists; browser facade needs UIFACADE-1 |
...
| Conversation installs | `conversation_installs`; live per-agent route is `GET /v1/agents/:id/conversation-installs` ([schema](</Users/dev/Workdir/myclaw/apps/core/src/adapters/storage/postgres/schema/providers.ts:71)) | Exists; facade needs UIFACADE-1 |
...
| Access preset | Desired-state agent config `accessPreset: full|locked` ([types](</Users/dev/Workdir/myclaw/apps/core/src/config/settings/runtime-settings-types.ts:215)) | Exists; needs live facade |
| Tool-rules summary | Desired-state `toolRules`; effective sources/capabilities/tool access from `GET /v1/agents/:id/access` ([route](</Users/dev/Workdir/myclaw/apps/core/src/control/server/routes/capability-catalog.ts:85)) | Exists; needs live facade |
| Owner | No agent-to-PrincipalRef relation | Needs RBAC-1 |
| Approvers | `conversation_approvers.external_user_id`, per conversation—not principals ([schema](</Users/dev/Workdir/myclaw/apps/core/src/adapters/storage/postgres/schema/conversations.ts:110)) | Needs IDENT-2 + RBAC-1 |
| Current-month tokens | `/v1/usage?from=&to=&agentId=`, aggregating `runtime_events` ([route](</Users/dev/Workdir/myclaw/apps/core/src/control/server/routes/usage.ts:20)) | Exists for display; needs COST-1 |
| Last run | `agent_runs.agent_id/started_at`; no per-agent list/last-run API ([schema](</Users/dev/Workdir/myclaw/apps/core/src/adapters/storage/postgres/schema/runs.ts:24)) | Storage exists; needs read-model/facade |
...
|---|---|---|
| Existing detail | Fixture-backed tabs: Identity, Profile, Sources, Capabilities, Skills, MCP, Access, Conversations ([route](</Users/dev/Workdir/myclaw/apps/web/src/features/agents/routes/agent-detail-route.tsx:43)) | All visible controls only open the connection gate |
...
| Guardrails | Preview cards for sandbox, egress, permission evaluation, denylist ([route](</Users/dev/Workdir/myclaw/apps/web/src/features/runtime/routes/guardrails-route.tsx:18)) | Not an access editor; fixture-only and its buttons do not persist |

The real Control API already has bearer-only `GET|PUT /v1/agents/:id/access`; browser sessions are explicitly rejected from `/v1/*` ([boundary](</Users/dev/Workdir/myclaw/apps/core/src/control/server/index.ts:170)).

### Audit read model

| Source | Unified row contribution | Agent filter today |
|---|---|---|
| `runtime_events` | `time, actor, eventType/action, agent/run/conversation target, payload/outcome` | Row has `agent_id`, but generic list lacks an `agentId` filter ([repo](</Users/dev/Workdir/myclaw/apps/core/src/adapters/storage/postgres/repositories/runtime-event-repository.postgres.ts:317)) |
...
|---|---|---|
| People | `/v1/people` returns paged people, kind, status, aliases—including retired aliases—and memory counts; `/v1/people/:id` returns detail ([route](</Users/dev/Workdir/myclaw/apps/core/src/control/server/routes/people.ts:200)) | No browser facade: add `/ui/api/people` list/get; offboard action needs IDENT-4, reauth, CSRF/Origin, audit |
| Approvals | Read-only session conversation list can show pending interactions: time, tool, summary, expiry, available decisions | No per-agent/conversation queue or recent history. `pending_interactions` is app/run scoped; session route filters all-app rows in memory ([implementation](</Users/dev/Workdir/myclaw/apps/core/src/control/server/session-interaction-approvals.ts:101)). No browser approval authority in V1.0. |
| Usage | Month picker, agent, request count, input/output tokens; label UTC range aggregation | COST-1 is V1.0.x display work. Mark memory extraction/dreaming and permission-classifier calls as **uncounted**: they report `onUsage` but do not emit `model.usage` ([gap analysis](</Users/dev/Workdir/myclaw/docs/architecture/ai-employee-v1-gap-analysis.md:69)). |
...
|---|---|---|---|---|
| Sign-in | Partial: local one-time link; hosted generic PKCE/OIDC but Google-branded, one active issuer. [auth pages](apps/web/src/features/auth/auth-pages.tsx), [OIDC](apps/core/src/control/server/browser-oidc.ts) | Issuer picker/label, multiple issuers incl. Entra | Existing `/auth/oidc/start`, callback; extend issuer-aware start | RBAC-1 |
| Authentication & Access | Yes: admin settings can test/activate one issuer, invite, list/disable grants and sessions. [route](apps/web/src/features/auth/authentication-access-route.tsx) | “Users & roles” is this page, but add `approver`, issuer collection, per-issuer `hd`/`tid` auto-grant policy, access/audit identity view | Existing `/ui/api/auth/config*`, `/access`, `/invitations`, `/sessions`; extend payloads | RBAC-1 |
| Access request/approval | Partial: ungranted login gets `GNT-…` reference; CLI `gantry auth access approve <ref> --role administrator\|viewer`; browser invitations/grant edits exist. [route](apps/core/src/control/server/routes/browser-auth.ts) | `approver` role in CLI, DB, browser unions and grant editor; no browser risky-action approval inbox in V1 | Existing auth facade; keep risky approvals API-key-only | RBAC-1 |
| Issuer policy | Missing: one `activeOidc.companyDomain`; adapter reads Google `hd`, not Entra `tid`. [settings types](apps/core/src/config/settings/runtime-settings-types.ts) | Per-issuer tenant/domain claim policy, issuer CRUD and test/activate lifecycle | Extend `/ui/api/auth/config/*` | RBAC-1 |

### 2. Agent owner, pause/resume, approvers

| Screen/control | Exists | Needs | Facade route(s) | Story |
|---|---|---|---|---|
| Directory row | Preview-only agent list; no owner, approvers, usage, or live mutation. [agents route](apps/web/src/features/agents/routes/agents-route.tsx) | Show status, owner, monthly usage; row pause/resume only as a quick action | New `GET /ui/api/agents`, `PATCH /ui/api/agents/:id/status` | RBAC-1, DIR-UI-1 |
| Agent detail | Preview pause button is connection-gated. [detail](apps/web/src/features/agents/routes/agent-detail-route.tsx) | Put owner assignment, approver assignment, pause/resume, and offboard here; this is the primary control surface | New `PATCH /ui/api/agents/:id/{owner,status,approvers}`, plus principal picker read route | RBAC-1, DIR-UI-1 |
...
| Screen/control | Exists | Needs | Facade route(s) | Story |
|---|---|---|---|---|
| MCP Servers | Yes: remote/local source add, sanitized credential mappings, bindings, diagnostics, replace/disable. [spec](docs/specs/mcp-management-web-ui.md), [UI](apps/web/src/features/operations/routes/mcp-servers-route.tsx) | Keep separate: generic MCP bindings are optional/many-agent; a connector account must have one owning agent | Existing `/ui/api/mcp-servers*` remains source management | — |
...
| Screen/control | Exists | Needs | Facade route(s) | Story |
|---|---|---|---|---|
| Model Providers | Yes: registry-backed provider list, write-only credential update, disable/remove, verify. [UI](apps/web/src/features/operations/routes/providers-route.tsx), [facade](apps/core/src/control/server/routes/browser-model-providers.ts) | `openai_compatible` is not implemented, but once registered its Base URL + secret-ref fields and verify action fit this dialog with little/no special UI work | Existing `GET /ui/api/model-providers`; `PUT/PATCH/DELETE /:id`; `POST /:id/verify` | SOV-1 |
| Effective egress policy | Missing live surface. Diagnostics and Runtime Guardrails are preview data. [diagnostics](apps/web/src/features/operations/routes/diagnostics-route.tsx) | Read-only effective allowlist/default-deny result, protected components, and named violations; requires enforcement first | New browser-safe `GET /ui/api/doctor` or `/ui/api/runtime/egress-policy` | EGRESS-1 |
...
| Screen/control | Exists | Needs | Facade route(s) | Story |
|---|---|---|---|---|
| Monthly agent usage | Missing: `/agents` is preview-backed and no page calls `/v1/usage`. [usage route](apps/core/src/control/server/routes/usage.ts) | Add a current-month token panel to agent detail, with a compact value in the directory row and incomplete-path disclosure | New `GET /ui/api/usage?agentId&from&to&group_by=agent` wrapping `/v1/usage` | COST-1 |
...
| Secret references | Partial pattern only: provider values are write-only and never returned; MCP maps named credentials without values. [dialog](apps/web/src/features/operations/routes/provider-dialogs.tsx) | No reusable secret-ref component exists. Add a small shared validated `SecretRefField` for `env:`, `gantry-secret:`, or `aws-sm:` references; retain write-only input for raw-secret modes | Reuse model-provider facade pattern; connector/provider schemas validate refs | CONN-1, SOV-1 |
