...
|---|---|---|---|---|
| 1 | IDENTITY-01 is merged with the canonical alias store `users` + `user_aliases(provider, providerAccountId, externalUserId)`. Merge and `people:admin` exist; `person_merge_audit.actor` is scalar text. | major | `apps/core/src/adapters/storage/postgres/schema/apps.ts`, `apps/core/src/adapters/storage/postgres/repositories/person-identity-repository.postgres.ts`, `apps/core/src/control/server/routes/people.ts` | IDENT-2 retains those field names and adds service-alias collision checks and PrincipalRef migration. Do not rename alias storage to `(kind, authorityId, subject)`. |
...
| 2 | `/agents` now uses a live browser directory and configuration read model, including source and capability summaries; it still does not project identity aliases, installs, or the identity lifecycle status required for AI employees. | major | `apps/web/src/features/agents/agents-queries.ts`, `apps/web/src/features/agents/routes/agents-route.tsx` | Extend the existing directory read model with the identity projections once IDENT-2 ships. |
| 3 | A same-origin `/ui/api/agents` facade exists for browser configuration management, but it does not yet expose the identity and offboarding projections required by this roadmap. | blocker | `apps/core/src/control/server/routes/browser-agents.ts`, `apps/core/src/control/server/index.ts` | Extend the existing facade with the IDENT-2 directory and lifecycle read models, preserving its Viewer/Administrator, CSRF, Origin, reauth, and audit boundary. |
