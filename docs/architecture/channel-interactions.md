...
`request_mcp_server`, `request_permission`, `settings_desired_state`,
...
| Install a skill package                                        | `request_skill_install`                       | Renders `SKILL.md` preview, files, hashes, dependencies, or an exact installer command such as `npx ... install <skill>` that will run in host-controlled staging and import the produced package, with activation timing and approve/deny.                                                                                                                                                                                                                            |
...
| Add a third-party MCP server                                   | `request_mcp_server`                          | Renders transport, origin, Gantry Secret refs, allowed tool patterns, SSRF checks, approve/deny.                                                                                                                                                                                                                                                                                                                                                                       |
| Search semantic capabilities                                   | `capability_search`                           | Lists built-in capabilities such as `google.sheets.write`, risk, credential source, and the `propose_capability` call.                                                                                                                                                                                                                                                                                                                                                 |
| Propose capability                                             | `propose_capability`                          | Requests an approved semantic capability when the id already exists, or renders a reviewed `local_cli` capability. Local CLI proposals include pinned executable path/version/hash, command templates, auth preflight, protected paths, denied env patterns, and account label before runtime projects scoped command authority.      |
...
| Request SDK, host, or channel permission                       | `request_permission`                          | Renders `Allow <access>?` with source, scope, action preview, and safe Details. Semantic capability requests keep capability/account/can/cannot copy primary; one-off Gantry file/web facade access, canonical Browser, scoped `RunCommand(...)`, and provider capability requests keep raw rules, request id, command hash, sandbox profile, and executable path in Details/audit. `Always allow` remains limited to semantic capabilities, Browser, exact Gantry file/web facades, exact Gantry admin tools, or scoped `RunCommand(...)`. |
