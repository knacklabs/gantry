...
`Allow once`, `Always allow for this agent/job`, or `Cancel`. Details hold raw
...
| Request SDK, host, or channel permission                       | `request_permission`                          | Renders semantic capability fields when present; otherwise renders one-off exact tool access, canonical Browser, scoped Bash rules, or provider capability. Bash prompts list separate parsed leaf rules when a compound command can be persisted; destructive redirects are shown as redirect targets and remain one-time only. Raw rule, request id, command hash, sandbox profile, and executable path stay in Details. `Always allow` is limited to semantic capabilities, Browser, exact MyClaw admin tools, or scoped Bash. |
