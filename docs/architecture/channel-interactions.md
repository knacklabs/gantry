...
`ask_user_question`, selected memory tools such as `continuity_summary`, `file`,
`request_skill_install`, `request_skill_proposal`,
`request_skill_dependency_install`, `request_mcp_server`, `request_access`,
`settings_desired_state`, `request_settings_update`,
`admin_permission_list`, `admin_permission_revoke`, `service_restart`, and
`register_agent` requests, but approval authority stays with the configured
conversation/conversation approver rules.
...
| Inspect current continuity                                     | selected `continuity_summary`                 | Returns scoped continuity data for agent reasoning when that tool is selected; channels render any human-facing summary only through normal message descriptors.                                                                                                                                                                |
