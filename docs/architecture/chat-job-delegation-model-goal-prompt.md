...
- `docs/architecture/neutral-task-lifecycle-wrapper-plan.md`
...
- `apps/core/src/adapters/storage/postgres/schema/task-lifecycle.ts`
...
Exact UX contract:
- Never show users `Waiting for an available worker` or `Waiting for an available interactive worker`.
