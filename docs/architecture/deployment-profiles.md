...
| 4   | **Migration vs old worker**            | Additive-only migrations; one pg advisory lock (inside `migrate()`) serializes every explicit migrator. Every deployment role runs the same explicit migrator before runtime starts; runtime only asserts that the schema is current and fails readiness closed if it is not. Old workers run against the newer additive schema. | Migration runs once (lock holder); losers wait; failure exits non-zero or readiness stays red if schema was not seeded or is not current                                            |
