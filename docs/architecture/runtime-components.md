...
- accept inbound messages from Slack, Discord, Telegram, and the SDK app channel
...
| Channels                   | `apps/core/src/app/bootstrap/channel-wiring.ts`, `apps/core/src/channels/channel-provider.ts`, `apps/core/src/channels/slack/`, `apps/core/src/channels/discord.ts`, `apps/core/src/channels/telegram/`                            | Connect Slack, Discord, Telegram, and app channel adapters; route inbound messages, outbound replies, progress, streaming, typing, and permission prompts. Teams remains setup/discovery-only. |
...
1. Channel inbound path: Slack, Discord, and Telegram adapters send normalized inbound messages through channel wiring. The persistence handlers enforce sender policy, store chat metadata, and insert a durable message.
