...
- Source inventory (`mcp_servers`, bindings) and reviewed action authority
  (semantic capabilities projecting to exact MCP tool rules) stay separate
  stores; connecting or binding a server grants no tool-call authority.
- `mcp_call_tool` is gated on both a connected source and a current-run
  reviewed capability; raw discovered tool names are never durable ids.
- Server-side tool-list or schema drift cannot silently widen authority.
