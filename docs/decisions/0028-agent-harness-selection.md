...
date: 2026-06-14
supersedes: 0026-agent-engine-selection
---
...
> `docs/decisions/0026-agent-engine-selection.md`. That older decision is
...
- `agentHarness` (settings `agent_harness`) is the public harness selector;
  explicit `anthropic_sdk`/`deepagents` is user intent and must fail before
  runner spawn when the selected model is incompatible.
- `auto` preserves provider-derived behavior (Claude -> `anthropic_sdk`;
  OpenAI-compatible providers -> `deepagents`).
- Public `agentEngine` vocabulary is retired; jobs/conversations inherit the
  bound agent harness rather than selecting their own.
- The 0026-agent-engine-selection record becomes historical context for
  `auto`, not the active public contract.
