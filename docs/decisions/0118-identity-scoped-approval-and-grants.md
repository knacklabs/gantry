...
- **Self-grant gate.** A durable "allow for future" in a DM is permitted only for a person
  the identity model already deems established (personal-memory-eligible); otherwise that
  person may approve for the current run only. A DM approval spends the bot's own
  infrastructure, so it is not unconditional.
...
- Three seams are built as replaceable points so the later organisation tier is additive,
  not a rewrite: **approver resolution** (org adds manager/IT routing), the **grant scope
  column** (org subdivides `null` into org-policy/team tiers), and the **self-grant gate**
  (org reads it as "within the admin-defined grantable catalog"). See the org deferral.
