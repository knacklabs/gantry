...
## Amendment (2026-09-10, decision 0161 review)

The clauses "never by the classifier, which remains excluded from autonomous
runs" and "no classifier on autonomous runs" are obsolete. Decision 0157
supersedes 0121 and puts jobs on the chat permission ladder, which reaches the
classifier. What survives from this record is the projection itself: the host
computes the learned-decision projection before the ladder consults anything
else, so a remembered allow still settles a job request deterministically and
without a classifier call. The classifier is now the step after that projection
and the rails, not an excluded party. Cite 0043 and 0157 for ladder behaviour.
