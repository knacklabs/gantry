...
`canvases:write`, `commands`, `app_mentions:read`, `channels:read`, `channels:history`,
`groups:read`, `groups:history`, `im:read`, `im:history`, `mpim:read`, and
`mpim:history`.

The canonical machine-readable list is `SLACK_APP_MANIFEST` in
`apps/core/src/channels/slack/app-manifest.ts`; both setup flows render that same
