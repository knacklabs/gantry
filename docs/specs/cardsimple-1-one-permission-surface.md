...
status: confirmed
saved: 2026-09-01T00:18:16+00:00
---
...
- One card for ALL need classes (grill R1b): the row/action mapping is defined over the REAL unions — action shapes `approve_grant | fix_proposal | instruction` (shared/job-setup-action.ts) crossed with the blocker-type axis (tool, credential, browser, MCP, local-CLI — domain/job-types.ts); a compound command is a RunCommand content case inside the tool blocker type, not an action shape. Blockers whose action is `instruction` render as instruction rows on the same card, never as prose messages.
...
- Card delivery/revision becomes the sole owner of `notified_fingerprint` (today the prose send marks it — execution-readiness.ts:245); a durable "sole card established/pending" signal suppresses the setup-terminal send so a second actionable surface can never leak (execution-notifications.ts:396 path). Delivery keeps decision 0124's bounded automatic retries and terminal outcomes; after exhaustion, recovery is operator/owner-initiated and re-delivers under the SAME durable prompt identity (one identity through ambiguous/exhausted recovery — never a second surface).
...
- Risk gating with a REAL enforcement point (corrects the draft; scope ruled at requirements grill 2026-09-01): rails precede ONLY `RunCommand(<literal argv0> *)` family-rule matches — a family match is provisional, the deterministic destructive/privileged/egress rails evaluate the EXACT command before the match is honored; a rail hit downgrades to ask, and the resulting Allow is once-only. Exact reviewed rules and capability grants behave exactly as today. Autonomous runs remain classifier-free; YOLO denylist is not the gate. The new decision record EXPLICITLY AMENDS 0121/0144's approval-permanence wording for the family rail-hit case (rails-before-family-match ordering; today reviewed-rule matches return first — permission-decision-coordinator.ts:84).
...
- Tap-time liveness is explicit: the callback checks the run lease and returns a typed `live | late` outcome. Live ⇒ today's flow. Late ALLOW ⇒ record the decision exactly as if live (rule breadth included), reply with ONE receipt line ("Run's over — saved for future runs") plus a Run now button; never auto-rerun. Late DENY (ruled at requirements grill 2026-09-01) ⇒ one denial receipt line, NO Run now button, and the denied row stays/re-renders with one-tap Reconsider — 0144's shape, unchanged by lateness.
...
Remaining GRACE-1 findings (lost retry timer, waiting-job reminders, unregistered-channel courtesy reply, provider-unknown log), egress-gateway policy, YOLO policy, interactive-lane classifier behavior, any change to 0134's piped-command invariant, and non-job/live-chat permission cards (their consolidation and late-tap behavior are unchanged by this story).
