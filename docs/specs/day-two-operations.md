...
agent replies with a fixed, configurable line.
...
freeze/unfreeze, offboarding, self-revisions applied. Other stories reuse it.

### Backup, upgrade, rollback (V1.0)

Scripted backup/restore drill tested in CI; upgrade with pre-flight migration check,
release notes per tag, pinned image digest and a rollback command (decision 0147).

### Data lifecycle (V1.0.x)

Retention per class with holds; `conversation erase` for anonymous callers;
...
appends a CAS-fenced corrective revision from the detail page or CLI.
