...
5. The jobs path (`ensureAgentExists`) and the binding/setup flows are
...
No migration; no compensating SELECTs; `ensureApp` survives for the jobs
path (hot-path call sites removed only); the `ensureThread` change stays
backward-compatible for existing mocks (optional input field).
