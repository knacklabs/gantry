...
To run updated code against an existing local claim store, stop the old server first and set `STATE_FILE` and `EVIDENCE_DIR` to that store's existing paths before `npm start`. Keep both paths together; starting a fresh default store will not show prior claims.
...
4. Select Gantry's built-in `notifications.send` and `claims.evidence.store` capabilities for MIA. These are not MCP server tools. Set `MOTOR_INSURANCE_SERVICE_URL=http://127.0.0.1:14319`, `MOTOR_CLAIM_EVIDENCE_AGENT_FOLDER` to MIA's local agent folder, and `MOTOR_CLAIM_REVIEW_CHANNEL` to the internal sales channel name in Gantry's runtime environment. Decision outcomes return to the channel where MIA sent the review request; no outcome channel is configured. Restart Gantry after changing its runtime environment.
...
Policy facts, pricing, and coverage rules are invented fixtures, not real insurance guidance. Quotes omit taxes and do not update no-claim bonus based on claims. Evidence completion moves a draft into review; only an authorized human review decision can approve or decline it. Review approval is not payout approval. No dispatch, booking, purchase, renewal, cancellation or settlement is implemented. Explicit confirmation is required before claim creation. Do not put real personal or insurance data into this local JSON-file setup.
