...
Read `factory/skills/rejected/` before proposing anything. A pattern
...
Write each proposal to `factory/skills/proposed/<slug>.md` with:
...
STOP. A human reviews `factory/skills/proposed/` and promotes: skills move to
`.claude/skills/<name>/SKILL.md` (Claude) or get an AGENTS.md line (both
runtimes). Rejected proposals are MOVED to `factory/skills/rejected/` with
