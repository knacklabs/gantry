...
the shared canon (AGENTS.md, docs/, constitution/, harness/, factory/) and the
...
Usage: python3 factory/scripts/check_dual_runtime.py [target-dir]
...
def check_canon_exists(root: Path) -> None:
    required = ["AGENTS.md", "docs", "factory/prompts", "factory/scripts", "harness.yaml"]
    for rel in required:
                    ...
                    for script in scripts:
                        if not script.startswith("factory/scripts/"):
                            violation(
                                ...
                                "must live in factory/scripts/ (thin-adapter rule)."
                            )
...
def check_prototype_isolation(root: Path) -> None:
    ...
    for rel in proc.stdout.splitlines():
        if rel.startswith(("prototype/", "factory/", ".claude/", ".codex/")):
            continue
...
def check_schemas(root: Path) -> None:
    ...
    every pinned generator in factory/schemas/*.json must appear in
