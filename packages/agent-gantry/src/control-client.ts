...
export class GantryControlClient {
  ...
  constructor(private readonly config: GantryControlClientConfig) {
    ...
    if (!config.apiKey.trim()) throw new Error("Gantry Control apiKey is required.");
  }
  ...
  async triggerJob(jobId: string): Promise<GantryTriggerJobResult> {
    return await this.postJson(`/v1/jobs/${encodeURIComponent(jobId)}/trigger`, {});
  }
  ...
  private async postJson<T>(path: string, body: unknown): Promise<T> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Gantry Control ${path} failed (${response.status}): ${text.slice(0, 500)}`);
      }
      return payload as T;
    } finally {
...
export function createGantryControlClient(config: GantryControlClientConfig): GantryControlClient {
