...
/**
 * Builds the AAD context for a model credential row. Moved here from core's
 * model-credential-repository so both core and connectors build a byte-identical
 * AAD. Callers pass an already-normalized providerId (core normalizes via
 * normalizeModelCredentialProvider at read/write time; for 'anthropic' this is an
 * identity), so this does not re-normalize and stays free of core's registry.
 */
export function modelCredentialAadContext(input: {
  appId: string;
  providerId: string;
  authMode: string;
  schemaVersion: number;
}): CredentialSecretAadContext {
  return {
    appId: input.appId,
