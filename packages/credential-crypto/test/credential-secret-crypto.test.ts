...
describe('credential secret crypto', () => {
  ...
  it('modelCredentialAadContext round-trips: same inputs decrypt, changed provider fails', () => {
    const aad = modelCredentialAadContext({
      appId: 'default',
    ...
    const token = '{"oauthToken":"sk-ant-oat01-test"}';
    const encrypted = encryptCredentialSecretValue(token, aad, provider());

    // Identical AAD decrypts.
    expect(decryptCredentialSecretValue(encrypted, aad, provider())).toBe(token);

    // A different provider id changes the AAD -> auth failure.
    const otherAad = modelCredentialAadContext({
      appId: 'default',
      providerId: 'openai',
      authMode: 'claude_code_oauth',
      schemaVersion: 1,
    ...
    expect(() =>
      decryptCredentialSecretValue(encrypted, otherAad, provider()),
    ).toThrow(/failed authentication/i);
  });
});
