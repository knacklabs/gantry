...
Governing decision: `docs/decisions/0078-lat-3a-single-memory-hydration-per-turn.md`
Sign-off: `docs/decisions/0079-client-signoff.md`
...
provisional context. That is resolved in decision 0078 and corrected in
...
- The scheduled-job hydrating call at `apps/core/src/jobs/execution.ts:324-335`
  — deferred as **D-0022** with a revisit trigger. A job is not an inbound turn
...
- Carrying an expected session identity from admission into the runner. Named
  by the roadmap, **explicitly rejected** in decision 0078; see the Decisions section.
...
- `docs/decisions/0078-lat-3a-single-memory-hydration-per-turn.md` — **the
  ...
  two.
- `docs/decisions/0079-client-signoff.md` — LAT-3A sign-off. Needed its own
...
- Deferral **D-0022** — the scheduled-job hydrating path, with a revisit
  trigger.
- Deferral **D-0023** — whether the `first_visible` recall timeout behaves
...
| CLI / ops | **Unchanged by design** | No command, setting, or `settings.yaml` key. Decision 0078 forbids a rollout flag, so there is nothing to operate. |
...
| Deferred | **Deferred** | Scheduled-job hydration (D-0022) and the `first_visible` recall-timeout question (D-0023), both with revisit triggers in `plans/deferrals.md`. |
...
- **A second hydration-only field is added to the return type later**, and
  ...
  hydration-derived field. Mitigation: decision 0078 records that this must be
...
- **The pending-delta path is subtle** and its provisional context is
  ...
  table in Technical Approach and decision 0078 exist to stop it recurring.
