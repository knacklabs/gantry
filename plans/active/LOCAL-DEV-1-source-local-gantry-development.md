...
A plan-gate cold-read grill (recorded, `.factory/stories/LOCAL-DEV-1/grills/plan.json`) raised four further gaps the owner has now resolved directly: (1) `reset-db` drops the `settings_revisions` table it just validated as authoritative, so restart must not silently fall back to stale `settings.yaml` — the owner chose **preserve desired state**: before the destructive schema step, write the just-validated authoritative revision's content out as `settings.yaml` (a matching recovery copy), so a genuinely-fresh-looking home after `reset-db` still reflects the same desired configuration instead of stale defaults; a later, independent requirements re-grill additionally found that a stale reset-in-progress marker's "rerun the same command" remediation deterministically refuses forever, since the command cannot tell that the operator already checked local state by hand — the owner resolved that separately with an explicit `--after-manual-recovery` acknowledgment flag (see Reset crash recovery in the spec and Technical Approach below). (2) The plan writes `GANTRY_CONTROL_HOST`/`GANTRY_CONTROL_PORT` into `.env`, which decision 0006 does not authorize for non-secret configuration — the owner chose to **record a narrow decision amendment**, now `docs/decisions/0162-local-bootstrap-host-port-env-exception.md` (accepted), scoping the exception to exactly these two bind-time keys for the bootstrap moment before any settings source is reachable. (3) Decision 0001 says CLI adapters must not directly mutate persistence, but `local.ts` is already close to the 700-line architecture ceiling before this safety work lands — the owner chose a **bounded deviation plus one new file**: extract the direct Postgres calls (`ownedPostgres`, the latest-settings-revision read, and the `pg_stat_activity` check) into one new local-operator-only helper file, keeping the deviation visible and file-scoped rather than spreading raw persistence access across the coordinator. (4) The current staged diff plus this remaining safety work is large for one task — the owner chose to **keep one task with an explicit size budget** rather than split it (see Task Decomposition).
...
- `package.json`, `apps/core/src/cli/index.ts`, `apps/core/src/cli/local.ts`, `apps/web/vite.config.ts`, `apps/core/test/unit/cli/index-local-routing.test.ts`, `scripts/architecture-exceptions.json`, `README.md`, and Forge proof/docs artifacts. Exactly one new source file is in scope: `apps/core/src/cli/local-postgres.ts` (and its unit test), a bounded local-operator-only helper isolating the direct Postgres calls per the plan-gate grill's resolved deviation from decision 0001 — no other new files.
...
- Reset crash recovery: write a durable reset-in-progress marker (naming the interrupted variant) before the destructive database step, clear it only after that reset variant's last destructive step completes; a subsequent command finding a stale marker refuses with a clear message naming the variant, lifted only by an explicit `--after-manual-recovery` acknowledgment flag the operator passes after inspecting local state — not by blindly rerunning the same command. This is a durable flag and a refusal, not a new lock.
- Reset-db desired-state recovery copy: before `reset-db`'s destructive schema step drops the `settings_revisions` table it just validated as authoritative, write that validated revision's content out as `settings.yaml` (the matching recovery copy) so restart reflects the same desired configuration instead of falling back to stale/default YAML on what would otherwise look like a genuinely fresh home.
- Local bootstrap host/port env exception: `GANTRY_CONTROL_HOST`/`GANTRY_CONTROL_PORT` may be bootstrapped into `.env` under the narrow exception recorded in decision 0162, for the sole purpose of binding the control server before any settings source is reachable; no other non-secret configuration moves into `.env`.
- Bounded local-operator persistence deviation: extract `ownedPostgres`, the latest-settings-revision read, and the `pg_stat_activity` active-connection check into one new file, `apps/core/src/cli/local-postgres.ts`, documented as a narrow, file-scoped deviation from decision 0001 for local-operator-only tooling — it is not an application service and must not be imported by agent/tool/job code paths.
...
Keep the existing implementation shape and tighten only the Full Forge gaps. `local.ts` remains the local source-mode coordinator because it already owns runtime-home resolution, local DB handling, resets, child supervision, health readiness, and auth-link issuance. Splitting this into a new service layer would add ceremony without changing the trusted local-operator boundary. The one exception, resolved at the plan gate, is `local-postgres.ts`: a bounded, file-scoped deviation from decision 0001 that moves the direct Postgres calls (`ownedPostgres`, the latest-settings-revision read, `pg_stat_activity`) out of `local.ts` so the coordinator stays under the architecture line ceiling — it is a narrow local-operator helper, not a new service layer or a precedent for other CLI/persistence coupling.
...
Auth link generation should continue to reuse the same underlying issuance function `gantry ui authorize` uses, called in-process rather than by inheriting stdio from a spawned subprocess (which would print the raw URL before the supervisor's TTY gate could apply) — no change to `apps/core/src/cli/auth.ts` itself. That keeps expiry, single-use storage, hashing, and browser-auth redemption in the existing auth path. The supervisor controls printing directly from the returned URL, gated on `process.stdout.isTTY`; it must not persist plaintext authorization URLs.
...
- **Ownership marker, bootstrap-only.** When `localEnvironment` bootstraps a home that did not exist before this invocation created it (the same branch that currently seeds `.env` defaults for a fresh home), also write a small Gantry-owned marker file (`<home>/.gantry-owned`, atomic temp-write-then-rename). It is never written to an already-existing, unmarked home — the requirements grill found that "retroactive marking" is exactly the compatibility/adoption path decision 0003 forbids. `validateLocalHome`'s existing unsafe-path/symlink checks are necessary but not sufficient — they rule out obviously wrong homes, not confirm Gantry created this one. Filesystem reset refuses on a home without the marker, with a manual remediation (move/delete the old home, or use a new `--runtime-home` so a fresh, marked home is bootstrapped). A full `reset` rewrites the marker only on a home that already carried one (re-affirming existing ownership, not first-time retroactive marking); a `reset` target with no marker is refused exactly like filesystem reset above — this resolves the plan-gate grill's "never after bootstrap" vs. "rewritten by full reset" tension. Home-directory/`.env`/marker/container-volume creation for a genuinely fresh home are additive bootstrap steps, not destructive ones, so they may precede authority checks and are safe to leave in place if a later check fails; on a failed check, stop only a container this invocation itself started, never one already running.
- **Database ownership bound to the reset endpoint.** `resetLocalDatabase` currently accepts any loopback DB literally named `gantry`/schema `gantry`, and `ensureLocalDatabase` only calls the existing `ownedPostgres` check on its fresh-start path, returning early (skipping the check) whenever the target is already reachable. Call `ownedPostgres` unconditionally before any reset, for the same container the supervisor is about to migrate/reset, and extend that check to verify the container's actual published host/port binding matches the endpoint being reset (not just its mount/checkout identity); refuse reset outright for a non-default `GANTRY_DATABASE_URL`, since a custom target's ownership can never be verified against a specific Docker container.
- **Reset-db/reset desired-state recovery copy.** Before the destructive schema step in either variant, write the just-validated authoritative settings revision's content out as `settings.yaml`, so a post-reset restart imports that same desired configuration as its first revision instead of stale/default YAML.
- **Reversible bootstrap-then-check for settings authority.** Decision 0025 makes the latest Postgres settings revision authoritative, but reading it needs Postgres up, and nothing destructive may run before authority is confirmed. Start (or confirm running) only the verified home-owned container as a reversible preflight step, read the latest settings revision with the canonical revision parser and `min_reader_version` skew rejection (the same fence normal startup already enforces), falling back to `settings.yaml` only when the `settings_revisions` table does not exist yet (a genuinely fresh database, distinguished from any other query failure, which is a real error) — a genuinely fresh home has no revision yet. Read both storage authority (`storage.postgres.url_env`/`.schema`) and authentication authority (`canonicalOrigin`, mode) from that SAME revision object, not storage from the revision and authentication from YAML. Validate `storage.postgres.url_env` (the actual key; not `urlEnv`) and `.schema` plus `GANTRY_SETTINGS_POSTGRES_SCHEMA`/URL `schema=`/`GANTRY_DB_SCHEMA` against the local-mode defaults, then either continue or stop that container again (only if this invocation started it) before returning a precondition failure.
...
- **Authentication-mode authority.** Before starting any child process, refuse when `authentication.mode` is not `local` or the origin is not loopback, reading from the same authoritative revision as storage above — the gap is only that `local.ts` does not check first, so it starts a healthy stack and then fails authorization forever.
- **Reset-in-progress marker for crash recovery.** Write a durable marker file (`<home>/.gantry-reset-in-progress`, recording which variant — `reset` or `reset-db` — and when) immediately before `resetLocalDatabase` runs; remove it only after that reset variant's last destructive step (filesystem cleanup for `reset`, schema recreation for `reset-db`) completes. A subsequent command that finds a stale marker refuses, naming the interrupted variant; because rerunning the identical command cannot distinguish "already recovered" from "still broken," the refusal is lifted only by an explicit `--after-manual-recovery` flag the operator passes after inspecting local state, not by retrying blindly. This is a durable flag plus a refusal, not a lock — concurrent-reset mutual exclusion stays deferred (D-0093).
- **TTY-gated token output via in-process issuance.** Issue the authorization link through the same underlying issuance function `gantry ui authorize` uses, called in-process (not by inheriting stdio from a spawned subprocess, which would print the raw URL before any gate could apply) — this requires no change to `apps/core/src/cli/auth.ts`. Print the raw authorization URL only when `process.stdout.isTTY` is true; otherwise print the stable UI URL plus a `--runtime-home`-inclusive retry command and never the raw token, so non-interactive capture (shell redirection, IDE task logs, CI) can never persist the credential. Issuance is a mandatory-attempt, fail-open step after every healthy start — its failure never fails the command.
- **Orphan-stop refusal via the database, not a public probe.** Core binds a private ephemeral port behind Vite, so a public-origin health probe cannot see an orphaned core process if Vite died. Reuse the existing `pg_stat_activity` active-connection check (already used by `resetLocalDatabase`) in `runLocalCommand`'s `stop` branch, run after this invocation's own managed children are shut down: the check detects any other active database client (which includes, but is not limited to, an orphaned core process) — if one remains, refuse to stop Postgres and report the orphaned state.
- **Vite orphan detection via a local PID file.** `pg_stat_activity` cannot see an orphaned Vite process (detached, no DB connection). `local start` writes core's and Vite's PIDs to `<home>/.gantry-local-pids` as they are spawned; `local stop` and the next `local start`'s preflight read it, verify each recorded PID is still alive and still Gantry's process, and terminate a leftover one; the file is removed once both children are confirmed stopped.
- **CLI-surface reduction.** Remove `local status`/`local doctor` (duplicates of the existing top-level `gantry status`/`gantry doctor`); make bare `gantry local` print usage instead of dispatching to `start`. Update stale mentions of these removed commands in `docs/architecture/overview.md`, `docs/SPEC.md`, the recovery message in `storage-readiness.ts`, and the roadmap entry's session/link criterion.
...
Workflow: this is one Full Forge story with one implementation task, kept as one task/one branch/one PR under the explicit size budget in Task Decomposition rather than split. The additions above are all inside the same coordinator file and its tests, plus the one resolved `local-postgres.ts` extraction — no other new module, new dependency, or new architectural seam, so splitting into more tasks would add ceremony without a real boundary to split on. The plan-gate grill found the acceptance criteria include a real end-user surface — a developer redeeming the browser authorization link and confirming a working UI session — even though no React component or style changes: the decomposition therefore records `user_facing: true`, requiring the functional-checker to actually walk the start → link → browser-redeem → restart → session-survival path, and requiring the implementer to attest the `emil-design-eng`/`frontend-design` skills in `skills_used` as reviewed-with-no-visual-changes (the proxy wiring changes, no component/styling/motion does). After the draft is approved, record the plan, record decomposition, run the task grill, implement only the scoped files, record tests, run deterministic verify, run the single autoreview helper for quality/performance/security, record reviews, record outcome, and run `pr_ready.py` before opening the PR.
...
This task reuses the existing npm/Vitest/TypeScript/Prettier/ESLint/Docker toolchain already standard across the repository (see `docs/architecture/current-verification-commands.md` and every completed plan in `plans/completed/`) — no new package manager, test runner, linter, or container tool is introduced, so no fresh tooling justification is owed beyond consistency with the rest of the codebase. CLI output (auth links, refusals, retry commands) is operator-facing terminal text, not structured application logging; it carries no `RuntimeEvent`/audit obligation because it is local-operator bootstrap tooling, not runtime code (see the durable-state-boundary note in the spec's reset-ownership section).
...
1. `LOCAL-DEV-FULL-T1` — Full local source-dev supervisor closeout
   ...
   Story visibility: true for CLI behavior. Task `user_facing: true` (see Technical Approach): the acceptance path includes a real browser session, so the functional-checker runs and the implementer attests the required design skills as reviewed-with-no-visual-changes.
...
- `npm run format:check` and `npm run format:check:web`
- `npm run typecheck`
- `npm run build:core` (confirms the extracted script still produces backend artifacts)
- `npm run build` and `npm run build:runtime` (confirm identical sorted `dist/` file-path listings against the pre-change build, per the falsifiable build-equivalence criterion)
- `python3 scripts/check_architecture.py`
- Package-script routing proof: `npm run dev`, `npm run dev:stop`, `npm run reset`, `npm run reset:db` each resolve to their documented `gantry local <subcommand>` invocation (e.g. via `npm run <script> -- --help` or a dry inspection of the resolved command), not just the underlying CLI tested directly.
- `python3 factory/scripts/verify.py --print-only` to confirm the deterministic gate shape before the full run
...
- Kill the process mid-`local reset` after its database step (a deliberate `kill -TERM` between schema recreation and filesystem cleanup on disposable state) and confirm the next command refuses with the reset-in-progress remediation instead of starting; confirm the marker names the interrupted variant, that a blind rerun still refuses, and that `--after-manual-recovery` clears it and lets the same command proceed. Use the repo-built CLI (`npm run build:core` then the built binary, or `tsx` against the source entrypoint) against a freshly created temporary parent directory (not a reused or preexisting `--runtime-home`) so the kill timing and starting state are deterministic, and confirm no fixed Postgres container name collides with an already-running one.
- Against a disposable Postgres, set a conflicting `storage.postgres.schema`/`GANTRY_SETTINGS_POSTGRES_SCHEMA` and confirm the database-target-authority refusal names the conflict before any destructive step; separately, orphan a core process (start core directly against the disposable DB without going through the supervisor) and confirm `local stop` refuses to stop Postgres and reports the orphaned connection.
- Kill only the Vite child (not the supervisor) via its recorded PID, then run `local stop` and confirm it detects and terminates the leftover Vite process via the PID file rather than reporting a clean stop.
