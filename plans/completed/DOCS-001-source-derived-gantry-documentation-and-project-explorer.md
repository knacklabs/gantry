...
- record both passing checks and known failures honestly.
...
4. `bug.md` contains only reproduced findings, distinguishes inherited
   failures from audit-environment limitations, and provides evidence and
...
8. Architecture checks, type checking, all unit tests, and the complete build
   pass; lint and production audit results are recorded exactly.
...
- [0068-client-signoff](../../docs/decisions/0068-client-signoff.md) records
...
| Docs | Changed | Adds the guide and bug report, refreshes current entry points, and repairs broken links. |
...
1. **Current-state documentation:** finalize the source-derived guide, audit
   report, active README/architecture corrections, and repository metadata.
...
4. **Verification and evidence:** run repository-wide links, formatting,
   architecture, typecheck, unit, build, lint/audit reproduction, desktop and
   mobile functional checks, then record factory test/review artifacts. Serves
   all criteria.
...
- **Inherited red checks:** lint and dependency audit are already red on the
  audited base. Mitigation: reproduce exact results in `bug.md`; do not label
  them as introduced or passed.
...
5. Run `npm run lint` and `npm audit --omit=dev`; require the recorded result to
   match `bug.md`, not an artificial pass.
6. Serve `index.html` locally and verify desktop/mobile rendering, no
   document-level horizontal overflow, search filtering, feature expansion,
   navigation targets, and browser console errors.
7. Run deterministic factory verification with repository-specific npm command
