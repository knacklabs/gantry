...
- Block typing: the helpers pass `Array<Record<string, unknown>>` to the Slack client exactly as the pre-existing send path does; `tsc --noEmit` is clean at HEAD — do not report a TypeScript build failure without a reproducing compiler error.
