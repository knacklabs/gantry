...
- `scheduledJobRunPrompt(job)` appends ONE trailing paragraph to the job prompt for scheduled runs (idempotent; system jobs such as memory dreaming are returned unchanged) stating that the final message must begin with `Outcome: <one sentence…>`.
